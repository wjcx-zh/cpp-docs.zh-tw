...
|[CRect::TopLeft](#topleft)|傳回的左上角點`CRect`。|  
...
|名稱|說明|  
|----------|-----------------|  
|[CRect::operator-](#operator_-)|減去指定的位移，從`CRect`或 deflate `CRect` ，並傳回所產生的`CRect`。|  
|[CRect::operator LPCRECT](#operator_lpcrect)|將轉換`CRect`至**LPCRECT**。|  
|[CRect::operator LPRECT](#operator_lprect)|將轉換`CRect`到`LPRECT`。|  
|[CRect::operator ！ =](#operator_neq)|決定是否`CRect`不等於矩形。|  
|[CRect::operator&amp;](#operator_amp)|建立交集`CRect`和矩形，並傳回所產生的`CRect`。|  
|[CRect::operator&amp;=](#operator_amp_eq)|設定`CRect`等於交集`CRect`和矩形。|  
|[CRect::operator |](#operator_or)|建立的聯集`CRect`和矩形，並傳回所產生的`CRect`。|  
|[CRect::operator |=](#operator_or_eq)|設定`CRect`等於的聯集`CRect`和矩形。|  
|[CRect::operator +](#operator_add)|將指定的位移`CRect`或擴大`CRect`，並傳回所產生的`CRect`。|  
|[CRect::operator + =](#operator_add_eq)|加入至指定的位移`CRect`或擴大`CRect`。|  
|[CRect::operator =](#operator_eq)|將複製的矩形維度`CRect`。|  
|[CRect::operator =](#operator_-_eq)|減去指定的位移，從`CRect`或 deflate `CRect`。|  
...
>  這個類別衍生自**tagRECT**結構。 (名稱**tagRECT**是小於-常用名稱`RECT`結構。)這表示，資料成員 (**左**，**頂端**，**右**，和**下**) 的`RECT`結構是可存取的資料成員的`CRect`。  
 ...
 當指定`CRect`，您必須非常小心建構它，都會被標準化 — 換句話說，這類的左方座標值是否小於右邊和 top 會小於底部。 比方說，左上角 (10,10) 和右下方 (20,20) 定義的標準化的矩形左上角 (20,20) 但右下方 (10,10) 定義非標準化的矩形。 如果矩形不會正規化，許多`CRect`成員函式可能會傳回不正確的結果。 (請參閱[Normalizerect](#normalizerect)如需這些函式的清單。)在呼叫的函式需要標準化的矩形之前，您可以將非標準化矩形正規化藉由呼叫`NormalizeRect`函式。  
  
 操作時請小心`CRect`與[CDC::DPtoLP](../../mfc/reference/cdc-class.md#dptolp)和[CDC::LPtoDP](../../mfc/reference/cdc-class.md#lptodp)成員函式。 顯示內容的對應模式，如果會 y 範圍中為負數，做為`MM_LOENGLISH`，然後`CDC::DPtoLP`會將轉換`CRect`使其 top 大於底部。 這類函數**高度**和**大小**會傳回已轉換的高度的負數值`CRect`，矩形會非正規化。  
...
### <a name="remarks"></a>備註  
 ...
 `CRect`(**Const RECT i**) 和`CRect`(**LPCRECT**) 建構函式執行[CopyRect](#copyrect)。 其他建構函式直接初始化物件的成員變數。  
 ...
 *y*  
 ...
 `size`  
 A[大小](http://msdn.microsoft.com/library/windows/desktop/dd145106)或[CSize](csize-class.md) ，指定要 deflate 的單位數`CRect`。 `cx`值會指定要 deflate 左邊和右邊的單位數和`cy`值會指定要 deflate 頂端和底端的單位數。  
...
> [!NOTE]
>  這兩個矩形必須正規化，或此函式可能會失敗。 您可以呼叫[NormalizeRect](#normalizerect)正規化之前呼叫這個函式的矩形。  
...
> [!NOTE]
>  矩形必須正規化，或此函式可能會失敗。 您可以呼叫[NormalizeRect](#normalizerect)正規化之前呼叫這個函式的矩形。  
 ...
 *y*  
 ...
 `size`  
 A[大小](http://msdn.microsoft.com/library/windows/desktop/dd145106)或[CSize](csize-class.md) ，指定要水平地擴大的單位數`CRect`。 `cx`值會指定要水平地擴大左邊和右邊的單位數和`cy`值會指定要水平地擴大的頂端和底部的單位數。  
...
> [!NOTE]
>  下列`CRect`成員函式需要正規化的矩形，才能正常運作︰[高度](#height)，[寬度](#width)，[大小](#size)， [IsRectEmpty](#isrectempty)， [PtInRect](#ptinrect)， [EqualRect](#equalrect)， [UnionRect](#unionrect)， [IntersectRect](#intersectrect)， [SubtractRect](#subtractrect)，[運算子 = =](#operator_eq_eq)，[運算子 ！ =](#operator_neq)，[運算子 |](#operator_or)，[運算子 | =](#operator_or_eq)，[運算子 i](#operator_amp)，和[運算子 i =](#operator_amp_eq)。  
...
### <a name="parameters"></a>參數  
 *srcRect*  
 是指來源矩形。 Can be a [RECT](../../mfc/reference/rect-structure1.md) or `CRect`.  
...
### <a name="remarks"></a>備註  
 ...
 第三個多載 deflate`CRect`單位指定在每個成員的參數數目。 請注意，這個多載的運作方式如同[DeflateRect](#deflaterect)。  
...
### <a name="remarks"></a>備註  
 ...
 第三個多載會傳回新`CRect`等於`CRect`縮小單位指定在每個成員的參數數目。 請注意，這個多載的運作方式如同[DeflateRect](#deflaterect)，而非[SubtractRect](#subtractrect)。  
...
### <a name="remarks"></a>備註  
 ...
 比方說，如果`lpRectSrc1`已 10,10 (100,100） 和`lpRectSrc2`已 50,50 (150,150），所指矩形`lpRectSrc1`就是不變，當函式傳回。 如果`lpRectSrc1`已 10,10 (100,100） 和`lpRectSrc2`已 50,10 (150,150），不過，該矩形所指`lpRectSrc1`會包含座標 （10,10、 50100） 函式傳回時。  
  
 `SubtractRect`不是相同[運算子-](#operator_-)或[運算子-=](#operator_-_eq)。 這些運算子都曾經呼叫`SubtractRect`。  
