...
- **StrTraitATL < wchar_t** &#124; **char** &#124; **TCHAR、ChTraitsCRT < wchar_t** &#124; **char** &#124; **TCHAR > >**
   ...
   類別需要 CRT 支援，並在 `m_hInstResource` 所指定的模組中搜尋資源字串（應用程式模組類別的成員）。

- **StrTraitATL < wchar_t** &#124; **char** &#124; **TCHAR、ChTraitsOS < wchar_t** &#124; **char** &#124; **TCHAR > >**

   類別不需要 CRT 支援，而且會在 `m_hInstResource` 所指定的模組中搜尋資源字串（應用程式模組類別的成員）。
...
|名稱|描述|
|----------|-----------------|
|[CStringT：： CStringT](#cstringt)|以各種方式來構造 `CStringT` 物件。|
...
|[CStringT：： Stringbuilder.appendformat](#appendformat)|將格式化的資料附加至現有的 `CStringT` 物件。|
...
|[CStringT：:D 刪除](#delete)|從字串中刪除一個或多個字元。|
...
|[CStringT：： Loadstring 時](#loadstring)|從 Windows 資源載入現有的 `CStringT` 物件。|
...
|[CStringT：： SetSysString](#setsysstring)|設定現有的 BSTR 物件，其中包含來自 `CStringT` 物件的資料。|
|[CStringT：： SpanExcluding](#spanexcluding)|從字串中，以第一個字元開頭，而不是 `pszCharSet`所識別的字元集中的字元。|
...
`CStringT` 繼承自[CSimpleStringT 類別](../../atl-mfc-shared/reference/csimplestringt-class.md)。 先進的功能，例如字元操作、排序和搜尋，都是由 `CStringT`來執行。

> [!NOTE]
> `CStringT` 物件可以擲回例外狀況。 當 `CStringT` 物件因為任何原因而用盡記憶體時，就會發生這種情況。

`CStringT` 物件是由可變長度的字元序列所組成。 `CStringT` 使用類似于「基本」的語法來提供函式和運算子。 串連和比較運算子加上簡化的記憶體管理，讓 `CStringT` 物件比一般字元陣列更容易使用。

> [!NOTE]
>  雖然您可以建立包含內嵌 null 字元的 `CStringT` 實例，但我們還是建議您不要這麼做。 在包含內嵌 null 字元的 `CStringT` 物件上呼叫方法和運算子可能會產生非預期的結果。

藉由使用不同的 `BaseType` 和 `StringTraits` 參數組合，`CStringT` 物件可以採用下列型別，這些類型已由 ATL 程式庫預先定義。

如果在 ATL 應用程式中使用：

從 MFC DLL （MFC90）匯出 `CString`、`CStringA`和 `CStringW`。DLL），而不是來自使用者 Dll。 這樣做的目的是為了防止定義 `CStringT`。
...
- `CStringT` 物件可以因串連作業而成長。

- `CStringT` 物件會遵循「值的語義」。 請將 `CStringT` 的物件視為實際的字串，而不是字串的指標。

- 您可以自由取代 `PCXSTR` 函式引數的 `CStringT` 物件。
...
因為 `CStringT` 使用樣板引數來定義支援的字元類型（ [wchar_t](../../c-runtime-library/standard-types.md)或[char](../../c-runtime-library/standard-types.md)），所以方法參數類型有時可能會變得很複雜。 為了簡化這個問題，會定義一組預先定義的類型，並在整個 `CStringT` 類別中使用。 下表列出各種類型：

|名稱|描述|
|----------|-----------------|
|`XCHAR`|與 `CStringT` 物件具有相同字元類型的單一字元（ **wchar_t**或**char**）。|
|`YCHAR`|以相反字元類型做為 `CStringT` 物件的單一字元（ **wchar_t**或**字元**）。|
|`PXSTR`|字元字串（ **wchar_t**或**字元**）的指標，與 `CStringT` 物件具有相同的字元類型。|
|`PYSTR`|字元字串（ **wchar_t**或**字元**）的指標，其字元類型與 `CStringT` 物件相反。|
|`PCXSTR`|與 `CStringT` 物件具有相同字元類型之**const**字元字串（ **wchar_t**或**char**）的指標。|
|`PCYSTR`|以相反字元類型做為 `CStringT` 物件的**const**字元字串（ **wchar_t**或**char**）指標。|

> [!NOTE]
>  先前使用 `CString` （例如 `AssignCopy`）未記載之方法的程式碼，必須取代為使用下列 `CStringT` 檔方法的程式碼（例如 `GetBuffer` 或 `ReleaseBuffer`）。 這些方法會繼承自 `CSimpleStringT`。
...
此函式會格式化並在 `CStringT`中附加一系列的字元和值。 每個選擇性引數（如果有的話）都會根據*pszFormat*中的對應格式規格，或從*nFormatID*所識別的字串資源進行轉換和附加。
...
如果字串相同，則為零; 如果這個 `CStringT` 物件小於*psz*，則 < 0; 如果這個 `CStringT` 物件大於*psz*，則為 > 0。

### <a name="remarks"></a>備註

泛型文字函數 `_tcscoll`，在 TCHAR 中定義。H，會對應至 `strcoll`、`wcscoll`或 `_mbscoll`，視編譯時間所定義的字元集而定。 每個函式都會根據目前使用中的字碼頁，執行字串的區分大小寫比較。 如需詳細資訊，請參閱[strcoll、wcscoll、_mbscoll、_strcoll_l、_wcscoll_l、_mbscoll_l](../../c-runtime-library/reference/strcoll-wcscoll-mbscoll-strcoll-l-wcscoll-l-mbscoll-l.md)。
...
如果字串完全相同（忽略大小寫），則為零; 如果這個 `CStringT` 物件小於*psz* （忽略大小寫），則 < 0; 如果這個 `CStringT` 物件大於*psz* （忽略大小寫），則為 > 0。

### <a name="remarks"></a>備註

泛型文字函數 `_tcscoll`，在 TCHAR 中定義。H，會對應至 `stricoll`、`wcsicoll`或 `_mbsicoll`，視編譯時間所定義的字元集而定。 每個函式都會根據目前使用中的字碼頁，執行字串的不區分大小寫比較。 如需詳細資訊，請參閱[strcoll、wcscoll、_mbscoll、_strcoll_l、_wcscoll_l、_mbscoll_l](../../c-runtime-library/reference/strcoll-wcscoll-mbscoll-strcoll-l-wcscoll-l-mbscoll-l.md)。
...
泛型文字函數 `_tcscmp`，在 TCHAR 中定義。H，會對應至 `strcmp`、`wcscmp`或 `_mbscmp`，視編譯時間所定義的字元集而定。 每個函式會執行區分大小寫的字串比較，而且不會受到地區設定的影響。 如需詳細資訊，請參閱[strcmp、wcscmp、_mbscmp](../../c-runtime-library/reference/strcmp-wcscmp-mbscmp.md)。
...
泛型文字函數 `_tcsicmp`，在 TCHAR 中定義。H 會對應至 `_stricmp`、`_wcsicmp` 或 `_mbsicmp`，視編譯時間所定義的字元集而定。 每個函數都會執行字串的不區分大小寫比較。 比較取決於地區設定的 LC_CTYPE 方面，但不會 LC_COLLATE。 如需詳細資訊，請參閱[_stricmp、_wcsicmp、_mbsicmp、_stricmp_l、_wcsicmp_l、_mbsicmp_l](../../c-runtime-library/reference/stricmp-wcsicmp-mbsicmp-stricmp-l-wcsicmp-l-mbsicmp-l.md)。
...
*pszSrc*<br/>
要複製到這個 `CStringT` 物件中的以 null 結束的字串。

*pStringMgr*<br/>
`CStringT` 物件之記憶體管理員的指標。 如需 `CStringT`之 `IAtlStringMgr` 和記憶體管理的詳細資訊，請參閱[使用 CStringT 的記憶體管理](../../atl-mfc-shared/memory-management-with-cstringt.md)。

*strSrc*<br/>
要複製到這個 `CStringT` 物件中的現有 `CStringT` 物件。 如需 `CThisString` 和 `CThisSimpleString`的詳細資訊，請參閱備註一節。
...
因為此函式會將輸入資料複製到新配置的儲存體，所以您應該注意記憶體例外狀況可能會產生。 請注意，其中有些函式會作為轉換函式。 這可讓您以需要 `CStringT` 物件的 LPTSTR 來替代。

- `CStringT`（`LPCSTR` `lpsz`）：從 ANSI 字串構造 Unicode `CStringT`。 您也可以使用此函數來載入字串資源，如下列範例所示。

- `CStringT(` `LPCWSTR` `lpsz`）：從 Unicode 字串中構造 `CStringT`。

- `CStringT`（`const unsigned char*` `psz`）：可讓您從指向不**帶正負**號之字元的指標來建立 `CStringT`。

> [!NOTE]
>  定義 _CSTRING_DISABLE_NARROW_WIDE_CONVERSION 宏，以關閉 ANSI 和 Unicode 字串之間的隱含字串轉換。 宏會從支援轉換的編譯函數中排除。

請注意， *strSrc*參數可以是 `CStringT` 或 `CThisSimpleString` 物件。 針對 `CStringT`，請使用其中一個預設的具現化（`CString`、`CStringA`或 `CStringW`）;針對 `CThisSimpleString`，請使用**this**指標。 `CThisSimpleString` 宣告[CSimpleStringT 類別](../../atl-mfc-shared/reference/csimplestringt-class.md)的實例，這是較小的字串類別，其內建功能比 `CStringT` 類別少。

多載運算子 `CSimpleStringT<>&()` 從 `CSimpleStringT` 宣告中，建立 `CStringT` 物件。
...
*iIndex*<br/>
要刪除的 `CStringT` 物件中第一個字元之以零為起始的索引。
...
這個 `CStringT` 物件中，符合所要求之子字串的第一個字元之以零為起始的索引;如果找不到子字串或字元，則為-1。

### <a name="remarks"></a>備註

函式會多載以接受兩個單一字元（類似于執行時間函數 `strchr`）和字串（類似于 `strstr`）。
...
以[sprintf_s](../../c-runtime-library/reference/sprintf-s-sprintf-s-l-swprintf-s-swprintf-s-l.md)將資料格式化成 C 樣式字元陣列的相同方式，將格式化的資料寫入 `CStringT`。
...
此函式會將一系列的字元和值，格式化並儲存在 `CStringT`中。 每個選擇性引數（如果有的話）都會根據*pszFormat*中的對應格式規格，或*nFormatID*所識別的字串資源，進行轉換和輸出。

如果將字串物件本身當做參數提供給 `Format`，則呼叫將會失敗。 例如，下列程式碼會導致無法預期的結果：
...
> [!NOTE]
> `FormatMessage` 會嘗試為新格式化的字串配置系統記憶體。 如果此嘗試失敗，就會自動擲回記憶體例外狀況。
...
函數需要訊息定義做為輸入，並由*pszFormat*決定。 函式會將格式化的郵件內文和引數的可變清單複製到 `CStringT` 物件，並在要求時處理任何內嵌的插入序列。

> [!NOTE]
> `FormatMessageV` 會呼叫[CStringT：： FormatMessage](#formatmessage)，它會嘗試為新格式化的字串配置系統記憶體。 如果此嘗試失敗，就會自動擲回記憶體例外狀況。
...
以 `vsprintf_s` 將資料格式化成 C 樣式字元陣列的相同方式，將格式化字串和引數的可變清單寫入 `CStringT` 字串。
...
從這個 `CStringT` 物件中解壓縮最左邊的*nCount*字元，並傳回已解壓縮之子字串的複本。
...
##  <a name="loadstring"></a>CStringT：： Loadstring 時

將由*nID*識別的 Windows 字串資源讀入現有的 `CStringT` 物件中。
...
從位置*iFirst* （以零為基底）開始，從這個 `CStringT` 物件中，解壓縮長度*nCount*字元的子字串。
...
*iFirst*<br/>
這個 `CStringT` 物件中第一個字元之以零為基底的索引，要包含在解壓縮的子字串中。
...
將這個 `CStringT` 物件中的所有字元，從 OEM 字元集轉換為 ANSI 字元集。
...
指派運算子會接受另一個 `CStringT` 物件、字元指標或單一字元。 請注意，每當您使用此運算子時，都會發生記憶體例外狀況，因為可以配置新的儲存空間。

如需 `CThisSimpleString`的詳細資訊，請參閱[CStringT：： CStringT](#cstringt)的「備註」一節。
...
`CStringT::operator+` 函數有七種多載形式。 第一個版本會串連兩個現有的 `CStringT` 物件。 接下來的兩個串連 `CStringT` 物件和以 null 結束的字串。 接下來的兩個串連 `CStringT` 物件和 ANSI 字元。 最後兩個會串連 `CStringT` 物件和 Unicode 字元。
...
運算子會接受另一個 `CStringT` 物件、字元指標或單一字元。 您應該注意，每當您使用這個串連運算子時，就會發生記憶體例外狀況，因為新的儲存空間可以配置給加入此 `CStringT` 物件的字元。
...
`Replace`有兩個版本。第一個版本會使用另一個子字串來取代子字串的一個或多個複本。 這兩個子字串都是以 null 結束。 第二個版本會使用另一個字元來取代一個或多個字元的複本。 這兩個版本都是針對儲存在 `CStringT`中的字元資料進行操作。
...
`CStringT` 實例的範例包括 `CString`、`CStringA`和 `CStringW`。

針對 `CStringA`，`Replace` 可與 ANSI 或多位元組（MBCS）字元搭配使用。 針對 `CStringW`，`Replace` 適用于寬字元。

針對 `CString`，會在編譯時期選取字元資料類型，這是根據下表中的常數是否已定義而定。
...
從這個 `CStringT` 物件中，解壓縮最後一個（也就是最右邊的） *nCount*字元，並傳回已解壓縮之子字串的複本。
...
如果*nCount*超過字串長度，則會解壓縮整個字串。 `Right` 類似于基本的 `Right` 函數（Basic 中的索引是以零為基底）。
...
將*pbstr*所指向的 BSTR 重新配置，並將 `CStringT` 物件的內容複寫到其中，包括 Null 字元。
...
根據 `CStringT` 物件的內容， *pbstr*所參考的 BSTR 值可能會變更。 如果沒有足夠的記憶體存在，函式會擲回 `CMemoryException`。
...
包含不在*pszCharSet*中之字元的子字串，以字串中的第一個字元開頭，並以字串中的第一個字元（也就是在*pszCharSet*中的第一個字元開頭，也就是在字串中找到*pszCharSet*的第一個字元）來結束。 如果在字串中找不到*pszCharSet*中的任何字元，則會傳回整個字串。

### <a name="remarks"></a>備註

`SpanExcluding` 會從*pszCharSet*中解壓縮並傳回字元第一次出現之前的所有字元（換句話說， *pszCharSet*中的字元和字串後面的所有字元都不會傳回）。 如果在字串中找不到*pszCharSet*中的任何字元，則 `SpanExcluding` 會傳回整個字串。
...
子字串，其中包含字串中的字元（ *pszCharSet*中），從字串中的第一個字元開始，並在字串中找到不在*pszCharSet*的字元時結束。 如果字串中的第一個字元不在指定的集合中，`SpanIncluding` 會傳回空的子字串。

### <a name="remarks"></a>備註

如果字串的第一個字元不在字元集中，`SpanIncluding` 會傳回空字串。 否則，它會傳回集合中連續的字元序列。
...
`CStringT` 物件，其中包含目前的 token 值。

### <a name="remarks"></a>備註

`Tokenize` 函式會在目標字串中尋找下一個 token。 *PszTokens*中的一組字元會指定要尋找之標記的可能分隔符號。 在每次呼叫時 `Tokenize` 函式會從*iStart*開始，略過前置分隔符號，並傳回包含目前 token 的 `CStringT` 物件，也就是字元的字串，直到下一個分隔符號字元為止。 *IStart*的值會更新為結尾分隔符號後面的位置，如果已到達字串的結尾，則為-1。 `Tokenize`的一系列呼叫，可以將更多的 token 從目標字串的其餘部分細分出來，使用*iStart*來追蹤在字串中要讀取下一個權杖的位置。 當沒有其他權杖時，此函式會傳回空字串，而且*iStart*會設定為-1。

不同于 CRT token 化函數（例如[strtok_s、_strtok_s_l、wcstok_s、_wcstok_s_l、_mbstok_s、_mbstok_s_l](../../c-runtime-library/reference/strtok-s-strtok-s-l-wcstok-s-wcstok-s-l-mbstok-s-mbstok-s-l.md)），`Tokenize` 不會修改目標字串。
...
`CStringT& TrimRight(XCHAR chTarget)` 版本接受一個字元參數，並從 `CStringT` 字串資料的結尾移除該字元的所有複本。 它會從字串的結尾開始，一直往前邁進。 當它找到不同的字元，或 `CSTringT` 用完字元資料時，就會停止。

`CStringT& TrimRight(PCXSTR pszTargets)` 版本會接受以 null 終止的字串，其中包含要搜尋的所有不同字元。 它會移除 `CStringT` 物件中這些字元的所有複本。 它會從字串的結尾開始，一直往前邁進。 當它找到不在目標字串中的字元，或 `CStringT` 用完字元資料時，就會停止。 它不會在 `CStringT`結尾處，嘗試將整個目標字串與子字串相符。
