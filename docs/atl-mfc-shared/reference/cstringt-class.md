...
- **StrTraitATL< wchar_t** &#124; **`char`** &#124; **TCHAR、ChTraitsCRT< wchar_t** &#124; **`char`** &#124; **TCHAR > >**
   ...
   類別需要 CRT 支援，並在所指定的模組 `m_hInstResource` （應用程式的模組類別的成員）中搜尋資源字串。

- **StrTraitATL< wchar_t** &#124; **`char`** &#124; **TCHAR、ChTraitsOS< wchar_t** &#124; **`char`** &#124; **TCHAR > >**

   類別不需要 CRT 支援，而且會在所指定的模組 `m_hInstResource` （應用程式的模組類別的成員）中搜尋資源字串。
...
|名稱|說明|
|----------|-----------------|
|[CStringT：： CStringT](#cstringt)|`CStringT`以各種方式來構造物件。|
...
|[CStringT：： SetSysString](#setsysstring)|使用來自物件的資料來設定現有的 BSTR 物件 `CStringT` 。|
|[CStringT：： SpanExcluding](#spanexcluding)|從字串中，以第一個字元開頭，而不是由所識別的字元集中的字元 `pszCharSet` 。|
...
`CStringT`繼承自[CSimpleStringT 類別](../../atl-mfc-shared/reference/csimplestringt-class.md)。 先進的功能，例如字元操作、排序和搜尋，都是由所執行 `CStringT` 。
...
> [!NOTE]
> 雖然您可以建立 `CStringT` 包含內嵌 null 字元的實例，但我們還是建議您不要這麼做。 在包含內嵌 null 字元的物件上呼叫方法和運算子 `CStringT` 可能會產生非預期的結果。

藉由使用不同的 `BaseType` 和 `StringTraits` 參數組合， `CStringT` 物件可以包含下列類型，這些型別已由 ATL 程式庫預先定義。

如果在 ATL 應用程式中使用：

`CString`、 `CStringA` 和 `CStringW` 是從 MFC DLL （MFC90.DLL）匯出，而不是從使用者 dll 匯出。 這麼做是為了避免 `CStringT` 定義過多次。
...
- 您可以自由地替代 `CStringT` `PCXSTR` 函數引數的物件。
...
因為 `CStringT` 使用樣板引數來定義支援的字元類型（ [wchar_t](../../c-runtime-library/standard-types.md)或[char](../../c-runtime-library/standard-types.md)），所以方法參數類型有時可能會很複雜。 為了簡化這個問題，會定義一組預先定義的類型，並在整個類別中使用 `CStringT` 。 下表列出各種類型：

|名稱|說明|
|----------|-----------------|
|`XCHAR`|**`wchar_t`** **`char`** 與物件具有相同字元類型的單一字元（或） `CStringT` 。|
|`YCHAR`|以 **`wchar_t`** **`char`** 相反字元類型做為物件的單一字元（或） `CStringT` 。|
|`PXSTR`|字元字串（ **`wchar_t`** 或 **`char`** ）的指標，與物件具有相同的字元類型 `CStringT` 。|
|`PYSTR`|字元字串的指標（ **`wchar_t`** 或），其 **`char`** 字元類型與 `CStringT` 物件相反。|
|`PCXSTR`|**`const`** 字元字串（ **`wchar_t`** 或 **`char`** ）的指標，與物件具有相同的字元類型 `CStringT` 。|
|`PCYSTR`|字元字串的指標 **`const`** （ **`wchar_t`** 或），其 **`char`** 字元類型與 `CStringT` 物件相反。|

> [!NOTE]
> 先前使用未記載的方法（例如）的程式碼， `CString` `AssignCopy` 必須以使用下列檔之方法 `CStringT` （例如或）的程式碼取代 `GetBuffer` `ReleaseBuffer` 。 這些方法是從繼承而來 `CSimpleStringT` 。
...
配置類型 BSTR 的 Automation 相容字串，並將物件的內容複寫 `CStringT` 到其中，包括終止的 null 字元。
...
將這個物件中的所有字元， `CStringT` 從 ANSI 字元集轉換成 OEM 字元集。
...
此函式會格式化，並在中附加一系列的字元和值 `CStringT` 。 每個選擇性引數（如果有的話）都會根據*pszFormat*中的對應格式規格，或從*nFormatID*所識別的字串資源進行轉換和附加。
...
在 TCHAR 中定義的泛型文字函數 `_tcscoll` 。H 會對應至 `strcoll` 、 `wcscoll` 或 `_mbscoll` ，視編譯時期所定義的字元集而定。 每個函式都會根據目前使用中的字碼頁，執行字串的區分大小寫比較。 如需詳細資訊，請參閱[strcoll、wcscoll、_mbscoll、_strcoll_l、_wcscoll_l、_mbscoll_l](../../c-runtime-library/reference/strcoll-wcscoll-mbscoll-strcoll-l-wcscoll-l-mbscoll-l.md)。
...
如果字串相同（忽略大小寫），則為零; 如果這個 `CStringT` 物件小於*psz* （忽略大小寫） >，則 < 0; 如果這個 `CStringT` 物件大於*psz* （忽略大小寫），則為0。

### <a name="remarks"></a>備註

在 TCHAR 中定義的泛型文字函數 `_tcscoll` 。H 會對應至 `stricoll` 、 `wcsicoll` 或 `_mbsicoll` ，視編譯時期所定義的字元集而定。 每個函式都會根據目前使用中的字碼頁，執行字串的不區分大小寫比較。 如需詳細資訊，請參閱[strcoll、wcscoll、_mbscoll、_strcoll_l、_wcscoll_l、_mbscoll_l](../../c-runtime-library/reference/strcoll-wcscoll-mbscoll-strcoll-l-wcscoll-l-mbscoll-l.md)。
...
在 TCHAR 中定義的泛型文字函數 `_tcscmp` 。H 會對應至 `strcmp` 、 `wcscmp` 或 `_mbscmp` ，視編譯時期所定義的字元集而定。 每個函式會執行區分大小寫的字串比較，而且不會受到地區設定的影響。 如需詳細資訊，請參閱[strcmp、wcscmp、_mbscmp](../../c-runtime-library/reference/strcmp-wcscmp-mbscmp.md)。
...
在 TCHAR 中定義的泛型文字函數 `_tcsicmp` 。H，會對應到 `_stricmp` `_wcsicmp` 或 `_mbsicmp` ，視編譯時期所定義的字元集而定。 每個函數都會執行字串的不區分大小寫比較。 比較取決於地區設定的 LC_CTYPE 方面，但不會 LC_COLLATE。 如需詳細資訊，請參閱[_stricmp、_wcsicmp、_mbsicmp、_stricmp_l、_wcsicmp_l、_mbsicmp_l](../../c-runtime-library/reference/stricmp-wcsicmp-mbsicmp-stricmp-l-wcsicmp-l-mbsicmp-l.md)。
...
*pszSrc*<br/>
要複製到這個物件中以 null 結束的字串 `CStringT` 。

*pStringMgr*<br/>
物件之記憶體管理員的指標 `CStringT` 。 如需和之 `IAtlStringMgr` 記憶體管理的詳細資訊 `CStringT` ，請參閱[使用 CStringT 的記憶體管理](../../atl-mfc-shared/memory-management-with-cstringt.md)。

*strSrc*<br/>
`CStringT`要複製到這個物件中的現有物件 `CStringT` 。 如需和的詳細資訊 `CThisString` `CThisSimpleString` ，請參閱備註一節。
...
因為此函式會將輸入資料複製到新配置的儲存體，所以您應該注意記憶體例外狀況可能會產生。 請注意，其中有些函式會作為轉換函式。 例如，這可讓您以預期的物件取代 LPTSTR `CStringT` 。

- `CStringT`（ `LPCSTR` `lpsz` ）： `CStringT` 從 ANSI 字串中構造 Unicode。 您也可以使用此函數來載入字串資源，如下列範例所示。
...
請注意， *strSrc*參數可以是 `CStringT` 或 `CThisSimpleString` 物件。 針對 `CStringT` ，請使用其中一個預設的具現化（ `CString` 、 `CStringA` 或 `CStringW` ）; 針對 `CThisSimpleString` ，使用 **`this`** 指標。 `CThisSimpleString`宣告[CSimpleStringT 類別](../../atl-mfc-shared/reference/csimplestringt-class.md)的實例，這是較小的字串類別，其內建功能比 `CStringT` 類別少。
...
## <a name="cstringtdelete"></a><a name="delete"></a>CStringT：:D 刪除
...
*iIndex*<br/>
要刪除的物件中第一個字元之以零為基底的索引 `CStringT` 。
...
這個物件中符合所要求之子字串的第一個字元之以零為基底的索引 `CStringT` ，如果找不到子字串或字元，則為-1。

### <a name="remarks"></a>備註

函式會多載以接受單一字元（類似于執行時間函式 `strchr` ）和字串（類似于 `strstr` ）。
...
以 `CStringT` [sprintf_s](../../c-runtime-library/reference/sprintf-s-sprintf-s-l-swprintf-s-swprintf-s-l.md)將資料格式化成 C 樣式字元陣列的相同方式，將格式化的資料寫入。
...
此函式會將一系列的字元和值格式化並儲存在中 `CStringT` 。 每個選擇性引數（如果有的話）都會根據*pszFormat*中的對應格式規格，或*nFormatID*所識別的字串資源，進行轉換和輸出。

如果字串物件本身當做參數提供給，呼叫將會失敗 `Format` 。 例如，下列程式碼會導致無法預期的結果：
...
*pszFormat*<br/>
指向格式控制字元串。 系統會針對插入進行掃描，並據此進行格式化。 格式字串類似于執行時間函 `printf` 式樣式格式字串，但它允許以任意順序插入參數。
...
以 `CStringT` 將 `vsprintf_s` 資料格式化成 C 樣式字元陣列的相同方式，將格式化字串和引數清單寫入字串。
...
從這個物件解壓縮最左邊的*nCount*字元 `CStringT` ，並傳回已解壓縮之子字串的複本。
...
將由*nID*識別的 Windows 字串資源讀入現有的物件中 `CStringT` 。
...
從*nCount* `CStringT` 位置*iFirst* （以零為基底）開始，從這個物件中解壓縮長度 nCount 字元的子字串。
...
將這個物件中的所有字元， `CStringT` 從 OEM 字元集轉換成 ANSI 字元集。
...
如需的詳細資訊 `CThisSimpleString` ，請參閱[CStringT：： CStringT](#cstringt)的「備註」一節。
...
函數有七種多載形式 `CStringT::operator+` 。 第一個版本會串連兩個現有 `CStringT` 的物件。 接下來的兩個會串連 `CStringT` 物件和以 null 結束的字串。 接下來的兩個會串連 `CStringT` 物件和 ANSI 字元。 最後兩個會串連一個 `CStringT` 物件和一個 Unicode 字元。
...
運算子會接受另一個 `CStringT` 物件、字元指標或單一字元。 您應該注意，每當您使用這個串連運算子時，就會發生記憶體例外狀況，因為新的儲存空間可以配置給加入此物件的字元 `CStringT` 。
...
有兩個版本 `Replace` 。第一個版本會使用另一個子字串來取代子字串的一個或多個複本。 這兩個子字串都是以 null 結束。 第二個版本會使用另一個字元來取代一個或多個字元的複本。 這兩個版本都是針對儲存在中的字元資料進行操作 `CStringT` 。
...
若為 `CStringA` ，則 `Replace` 使用 ANSI 或多位元組（MBCS）字元。 針對 `CStringW` ， `Replace` 適用于寬字元。

若為 `CString` ，則會在編譯時期選取字元資料類型，這是根據下表中的常數是否已定義而定。
...
這個物件中，與所要求的字元相符之最後一個字元的以零為起始的索引 `CStringT` ，如果找不到該字元，則為-1。
...
從這個物件中解壓縮最後一個（也就是最右邊的） *nCount*字元 `CStringT` ，並傳回已解壓縮之子字串的複本。
...
重新配置*pbstr*所指向的 BSTR，並將物件的內容複寫 `CStringT` 到其中，包括 Null 字元。
...
根據物件的內容 `CStringT` ， *pbstr*所參考的 BSTR 值可能會變更。 如果沒有足夠的記憶體存在，函式會擲回 `CMemoryException` 。
...
`SpanExcluding`從*pszCharSet*中，解壓縮並傳回字元第一次出現之前的所有字元（換句話說， *pszCharSet*中的字元和字串後面的所有字元都不會傳回）。 如果在字串中找不到*pszCharSet*中的任何字元，則會傳回 `SpanExcluding` 整個字串。
...
如果字串的第一個字元不在字元集中，則會傳回 `SpanIncluding` 空字串。 否則，它會傳回集合中連續的字元序列。
...
函式會 `Tokenize` 尋找目標字串中的下一個 token。 *PszTokens*中的一組字元會指定要尋找之標記的可能分隔符號。 函式的每個呼叫 `Tokenize` 都會從*iStart*開始，略過前置分隔符號，並傳回 `CStringT` 包含目前標記的物件，也就是字元的字串，直到下一個分隔符號為止。 *IStart*的值會更新為結尾分隔符號後面的位置，如果已到達字串的結尾，則為-1。 透過一系列的呼叫，可以將更多的 token 從目標字串的其餘部分中斷 `Tokenize` ，使用*iStart*追蹤字串中下一個權杖的讀取位置。 當沒有其他權杖時，此函式會傳回空字串，而且*iStart*會設定為-1。

不同于 CRT token 化函數（例如[strtok_s、_strtok_s_l、wcstok_s、_wcstok_s_l、_mbstok_s、_mbstok_s_l](../../c-runtime-library/reference/strtok-s-strtok-s-l-wcstok-s-wcstok-s-l-mbstok-s-mbstok-s-l.md)）， `Tokenize` 並不會修改目標字串。
...
*pszTargets*<br/>
字串的指標，其中包含要修剪的目標字元。 *PszTarget*中所有開頭和結尾的字元都會從物件中修剪 `CStringT` 。
...
`CStringT& TrimRight(XCHAR chTarget)`版本接受一個字元參數，並從字串資料的結尾移除該字元的所有複本 `CStringT` 。 它會從字串的結尾開始，一直往前邁進。 當它找到不同的字元或字元資料用盡時，就會停止 `CSTringT` 。

`CStringT& TrimRight(PCXSTR pszTargets)`版本會接受以 null 終止的字串，其中包含要搜尋的所有不同字元。 它會移除物件中這些字元的所有複本 `CStringT` 。 它會從字串的結尾開始，一直往前邁進。 當它找到不在目標字串中的字元，或當字元資料用盡時，就會停止 `CStringT` 。 它不會嘗試比對整個目標字串與結尾的子字串 `CStringT` 。

`CStringT& TrimRight()`版本不需要任何參數。 它會修剪字串結尾的任何尾端空白字元 `CStringT` 。 空白字元可以是分行符號、空格或索引標籤。
