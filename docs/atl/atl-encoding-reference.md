...
ms.locfileid: "68375861"
---
# <a name="atl-encoding-reference"></a>ATL 編碼方式參考

在 atlenc 中找到的程式碼支援以多種常見的網際網路標準 (例如, uuencode、十六進位和 UTF8) 進行編碼。
...
|[AtlHexDecode](reference/atl-text-encoding-functions.md#atlhexdecode)|將已編碼為十六進位文字的資料字串 (例如先前的[AtlHexEncode](reference/atl-text-encoding-functions.md#atlhexencode)呼叫) 解碼。|
...
|[IsExtendedChar](reference/atl-text-encoding-functions.md#isextendedchar)|呼叫此函式可找出指定的字元是否為擴充字元 (小於 32, 大於 126, 而不是定位字元、換行字元或回車符)|
...
|[QPDecode](reference/atl-text-encoding-functions.md#qpdecode)|將已以引號可列印格式編碼的資料字串 (例如先前的[QPEncode](reference/atl-text-encoding-functions.md#qpencode)呼叫) 解碼。|
...
|[UUDecode](reference/atl-text-encoding-functions.md#uudecode)|將已 uuencode 編碼的資料字串 (例如先前呼叫的[UUEncode](reference/atl-text-encoding-functions.md#uuencode)) 解碼。|
