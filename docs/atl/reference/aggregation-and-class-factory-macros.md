...
ATL 物件通常會藉由衍生自[CComCoClass](../../atl/reference/ccomcoclass-class.md)來取得 Class Factory。 這個類別包含宏[DECLARE_CLASSFACTORY](#declare_classfactory)，它會宣告`CComClassFactory`為預設 Class Factory。 若要`CComClassFactorySingleton`使用，請在物件的類別定義中指定[DECLARE_CLASSFACTORY_SINGLETON](#declare_classfactory_singleton)宏。 例如：
...
如果嘗試對`CoCreate`您的物件做為非匯總物件，DECLARE_ONLY_AGGREGATABLE 會造成錯誤（E_FAIL）。
...
使用 DECLARE_POLY_AGGREGATABLE 的優點是，您可以避免在模組`CComAggObject`中`CComObject`同時擁有和來處理匯總和非匯總案例。 單一`CComPolyObject`物件可處理這兩種情況。 這表示只會有一個 vtable 複本，而您的模組中會有一份函數複本。 如果您的 vtable 很大，這可能會大幅降低您的模組大小。 不過，如果您的 vtable 很小， `CComPolyObject`使用可能會產生稍微大一點的模組大小，因為它不會針對匯總或非匯總物件進行優化`CComAggObject` ， `CComObject`如同和。
