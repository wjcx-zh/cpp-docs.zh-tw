...
`CComClassFactory` 會執行[IClassFactory](/windows/win32/api/unknwnbase/nn-unknwnbase-iclassfactory)介面，其中包含用來建立特定 CLSID 物件的方法，以及在記憶體中鎖定 Class Factory 以允許更快速地建立新物件。 您必須針對在系統登錄中註冊的每個類別，以及您要指派 CLSID 的目標，來執行 `IClassFactory`。

ATL 物件通常會藉由衍生自[CComCoClass](../../atl/reference/ccomcoclass-class.md)來取得 Class Factory。 這個類別包含宏[DECLARE_CLASSFACTORY](#declare_classfactory)，它會將 `CComClassFactory` 宣告為預設 Class Factory。 若要覆寫此預設值，請在您的類別定義中指定其中一個 DECLARE_CLASSFACTORY*XXX*宏。 例如， [DECLARE_CLASSFACTORY_EX](#declare_classfactory_ex)宏會針對 Class Factory 使用指定的類別：

[!code-cpp[NVC_ATL_COM#8](../../atl/codesnippet/cpp/aggregation-and-class-factory-macros_3.h)]

上述類別定義會指定 `CMyClassFactory` 將用來做為物件的預設 Class Factory。 `CMyClassFactory` 必須衍生自 `CComClassFactory` 並覆寫 `CreateInstance`。
...
*.lic*<br/>
在執行 `VerifyLicenseKey`、`GetLicenseKey`和 `IsLicenseValid`的類別。
...
`CComClassFactory2` 會執行[IClassFactory2](/windows/win32/api/ocidl/nn-ocidl-iclassfactory2)介面，這是[IClassFactory](/windows/win32/api/unknwnbase/nn-unknwnbase-iclassfactory)的延伸模組。 `IClassFactory2` 透過授權來控制物件的建立。 在授權的電腦上執行的 Class Factory 可以提供執行時間授權金鑰。 當完整的電腦授權不存在時，此授權金鑰可讓應用程式具現化物件。
...
`CMyLicense`（要 `CComClassFactory2`的範本參數）必須實作用 `VerifyLicenseKey`、`GetLicenseKey`和 `IsLicenseValid`的靜態函式。 以下是簡單授權類別的範例：

[!code-cpp[NVC_ATL_COM#3](../../atl/codesnippet/cpp/aggregation-and-class-factory-macros_5.h)]

`CComClassFactory2` 衍生自 `CComClassFactory2Base` 和*授權*。 `CComClassFactory2Base`又衍生自 `IClassFactory2`，而**CComObjectRootEx\< CComGlobalsThreadModel >** 。
...
`CComClassFactoryAutoThread` 類似于[CComClassFactory](../../atl/reference/ccomclassfactory-class.md)，但可讓您在多個單元中建立物件。 若要利用這項支援，請從[CComAutoThreadModule](../../atl/reference/ccomautothreadmodule-class.md)衍生您的 EXE 模組。
...
ATL 物件通常會藉由衍生自[CComCoClass](../../atl/reference/ccomcoclass-class.md)來取得 Class Factory。 這個類別包含宏[DECLARE_CLASSFACTORY](#declare_classfactory)，它會將 `CComClassFactory` 宣告為預設 Class Factory。 若要使用 `CComClassFactorySingleton`，請在物件的類別定義中指定[DECLARE_CLASSFACTORY_SINGLETON](#declare_classfactory_singleton)宏。 例如，

[!code-cpp[NVC_ATL_COM#10](../../atl/codesnippet/cpp/aggregation-and-class-factory-macros_7.h)]

##  <a name="declare_get_controlling_unknown"></a>DECLARE_GET_CONTROLLING_UNKNOWN

`GetControllingUnknown`宣告虛擬函數。
...
如果您收到未定義 `GetControllingUnknown` 的編譯器錯誤訊息（例如，在 `CComAggregateCreator`中），請將此宏新增至您的物件。
...
如果嘗試在您的物件上進行匯總，DECLARE_NOT_AGGREGATABLE 會導致 `CreateInstance` 傳回錯誤（CLASS_E_NOAGGREGATION）。
...
如果嘗試 `CoCreate` 您的物件做為非匯總物件，DECLARE_ONLY_AGGREGATABLE 會造成錯誤（E_FAIL）。
...
在建立期間，會核取 [外部未知] 的值。 如果它是 Null，則會為非匯總物件執行 `IUnknown`。 如果外部未知不是 Null，則會為匯總物件執行 `IUnknown`。

使用 DECLARE_POLY_AGGREGATABLE 的優點是，您可以避免模組中的 `CComAggObject` 和 `CComObject` 處理匯總和非匯總案例。 單一 `CComPolyObject` 物件會處理這兩種情況。 這表示只會有一個 vtable 複本，而您的模組中會有一份函數複本。 如果您的 vtable 很大，這可能會大幅降低您的模組大小。 不過，如果您的 vtable 很小，使用 `CComPolyObject` 可能會產生稍微大一點的模組大小，因為它不會針對匯總或非匯總物件進行優化，如同 `CComAggObject` 和 `CComObject`。
