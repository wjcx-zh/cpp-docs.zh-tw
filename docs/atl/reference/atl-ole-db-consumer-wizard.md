...
- **資料來源**
  ...
  - **連線**選項卡
      ...
      此索引標籤的內容取決於您選取的提供者。 儘管有許多類型的提供程式,本節涵蓋兩個最常見的連接:SQL 和 ODBC 數據。 其他的則為此處所述欄位上的類似變體。

      針對 SQL 資料：

      1. **選擇或輸入伺服器名稱:** 按下拉清單功能表以顯示網路上的所有已註冊數據伺服器,然後選擇一個。

      1. **輸入要登入伺服器的資訊:** 輸入要登錄到數據伺服器的使用者名稱和密碼。
      ...
      1. **選擇伺服器上的資料庫:** 按下拉清單功能表以顯示數據伺服器上的所有已註冊資料庫,然後選擇一個資料庫。

         \- 或 -

         **將資料庫檔案附加到資料庫名稱:** 指定要用作資料庫的檔;輸入顯式路徑名稱。

      針對 ODBC 資料：

      1. **指定資料來源:** 可以使用資料源名稱或連接字串。

         **使用資料來源名稱:** 此下拉清單顯示在計算機中註冊的數據來源。 您可以事先使用 ODBC 資料來源管理員來設定資料來源

         \- 或 -

         **使用連接字串:** 輸入已取得的連接字串,或按下 **「生成**」按鈕;將顯示 **「選擇資料源**」 對話框。 選取檔案或機器資料來源，然後按一下 [確定]****。
  ...
  - **進階**選項卡
      ...
      **網路設定:** 指定**類比等級**(伺服器在類比用戶端時允許使用的類比等級;直接對應於 RPC 類比等級)**和保護等級**(用戶端和伺服器之間發送的數據保護級別;直接對應於 RPC 保護級別)。

      **其他:** 在**Connect 超時中**,指定超時發生前允許的空閑時間秒數。 在 [存取權限]**** 中，指定資料連線的存取權限。
