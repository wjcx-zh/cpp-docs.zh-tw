...
- **資料來源**
   ...
   **資料來源**按鈕可讓您設定使用指定的 OLE DB 提供者的指定的資料來源。 當您按一下此按鈕時，**資料連結屬性** 對話方塊隨即出現。 如需有關建置連接字串和**資料連結屬性** 對話方塊中，請參閱[資料連結 API 概觀](/previous-versions/windows/desktop/ms718102)Windows SDK 文件。
   ...
   - **連接** 索引標籤
      ...
      1. **輸入要登入伺服器的資訊：** 輸入使用者名稱和密碼來登入的資料伺服器。
      ...
      1. **選取的伺服器上的資料庫：** 按一下下拉式清單功能表來顯示所有已註冊的資料庫伺服器上的資料，並選取其中一個。

         \-或-

         **附加資料庫檔案做為資料庫名稱：** 指定要做為資料庫檔案輸入明確的路徑名稱。

      對於 ODBC 資料：

      1. **指定資料的來源：** 您可以使用資料來源名稱或連接字串。

         **使用資料來源名稱：** 此下拉式清單會顯示在您的電腦中註冊的資料來源。 您可以設定資料來源，事先使用 ODBC 資料來源管理員

         \-或-

         **使用連接字串：** 輸入您已取得，或按一下 [連接字串**建置**按鈕則**選取資料來源**] 對話方塊隨即出現。 選取檔案或機器的資料來源，然後按一下**確定**。

         > [!NOTE]
         > 您可以檢視現有的連接中的屬性，以取得連接字串**伺服器總管**，或者您可以建立連線，按兩下**加入連接**在**伺服器總管**。

      1. **輸入要登入伺服器的資訊：** 輸入使用者名稱和密碼來登入的資料伺服器。

      1. 輸入要使用的初始目錄。

      1. 按一下 **測試連接**; 如果測試成功，按一下**確定**。 如果沒有，檢查您的登入資訊，請嘗試另一個資料庫，或嘗試另一部資料伺服器。

   - **進階** 索引標籤

      **網路設定：** 指定**模擬層級**（的伺服器可以模擬用戶端時使用; 直接對應至 RPC 模擬層級的模擬層級） 和**保護層級**(用戶端與伺服器之間傳送資料的保護層級直接對應到 RPC 保護層級）。

      **其他：** 在 **連接逾時**，指定的逾時之前所允許的閒置時間的秒數。 在 **存取權限**，資料連線指定的存取權限。
