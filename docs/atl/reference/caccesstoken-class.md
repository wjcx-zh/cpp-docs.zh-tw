...
|[CAccessToken::Detach](#detach)|呼叫此方法以撤銷存取權杖的擁有權。|
|[CAccessToken::DisablePrivilege](#disableprivilege)|呼叫這個方法, 以停用`CAccessToken`物件中的許可權。|
|[CAccessToken::DisablePrivileges](#disableprivileges)|呼叫這個方法, 以停用物件中的`CAccessToken`一或多個許可權。|
...
|[CAccessToken::GetDefaultDacl](#getdefaultdacl)|呼叫這個方法, 以`CAccessToken`傳回物件的預設 DACL。|
|[CAccessToken::GetEffectiveToken](#geteffectivetoken)|呼叫這個方法, `CAccessToken`使物件等於目前線程的作用中存取權杖。|
|[CAccessToken::GetGroups](#getgroups)|呼叫這個方法, 以`CAccessToken`傳回物件的 token 群組。|
...
|[CAccessToken::Impersonate](#impersonate)|呼叫這個方法, 將模擬指派`CAccessToken`給執行緒。|
...
|[CAccessToken::PrivilegeCheck](#privilegecheck)|呼叫這個方法, 以判斷`CAccessToken`物件中是否已啟用一組指定的許可權。|
...
`CheckTokenMembership`方法會檢查使用者的 sid 是否存在, 以及存取權杖的群組 sid。 如果 SID 存在且具有 SE_GROUP_ENABLED 屬性, 則*pbIsMember*會設定為 TRUE;否則, 它會設定為 FALSE。
...
[Security attributes 這個](/previous-versions/windows/desktop/legacy/aa379560\(v=vs.85\))結構的指標, 指定新執行緒的安全描述項, 並決定子進程是否可以繼承傳回的控制碼。 如果*pThreadAttributes*為 Null, 則執行緒會取得預設的安全描述項, 而且無法繼承控制碼。
...
*SidsToDisable*<br/>
指定僅拒絕 sid 的物件。`CTokenGroups`

*SidsToRestrict*<br/>
指定限制 sid 的物件。`CTokenGroups`
...
呼叫這個方法, 以`CAccessToken`傳回物件的預設 DACL。
...
呼叫這個方法, `CAccessToken`使物件等於目前線程的作用中存取權杖。
...
呼叫這個方法, 以`CAccessToken`傳回物件的 token 群組。
