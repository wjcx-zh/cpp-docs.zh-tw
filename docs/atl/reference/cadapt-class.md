---
title: CAdapt 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-atl
...
## <a name="remarks"></a>備註  
 ...
 `CAdapt` 的主要角色是隱藏 `T` 類別所定義的傳址運算子，但是仍然保留所配接類別的特性。 `CAdapt` 藉由保留的公用成員，可藉此角色[m_t&lt](#m_t)，型別的`T`，以及定義轉換運算子、 比較運算子和複製建構函式特製化`CAdapt`視為如同它們是型別的物件`T`。  
