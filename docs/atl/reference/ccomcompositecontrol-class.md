...
衍生自類別`CComCompositeControl`的類別會繼承 ActiveX 複合控制項的功能。 衍生自`CComCompositeControl`的 ActiveX 控制項是由標準對話方塊所主控。 這些控制項類型稱為複合控制項, 因為它們能夠裝載其他控制項 (原生 Windows 控制項和 ActiveX 控制項)。

`CComCompositeControl`藉由尋找子類別中的列舉資料成員, 識別要用於建立複合控制項的對話方塊資源。 這個子類別的成員 IDD 會設定為將當做控制項視窗使用之對話資源的資源識別碼。 下列是衍生自之`CComCompositeControl`類別所應包含的資料成員範例, 以識別要用於控制項視窗的對話方塊資源:
...
衍生類別所`CComCompositeControl`執行的控制項具有內建的預設 tab 鍵行為。 當控制項在包含的應用程式中以索引標籤式的方式取得焦點時, 連續按下 TAB 鍵會使焦點迴圈到所有複合控制項的包含控制項, 然後從複合控制項移出, 然後放到容器的定位順序。 裝載控制項的定位順序是由對話資源決定, 並決定進行 tab 鍵的順序。

> [!NOTE]
>  為了讓快速鍵能夠正常`CComCompositeControl`搭配運作, 您必須在建立控制項時載入快速鍵對應表、將控制碼和加速器數目傳遞回[IOleControlImpl:: GetControlInfo](../../atl/reference/iolecontrolimpl-class.md#getcontrolinfo), 最後再摧毀資料表釋放控制項時。
...
|E_POINTER  |此錯誤通常表示控制項的事件接收對應中的專案發生問題, 或在`IDispEventImpl`或`IDispEventSimpleImpl`基類中使用的樣板引數發生問題。|
|CONNECT_E_ADVISELIMIT  |連接點已經達到其連線限制, 因此無法再接受其他。|
|CONNECT_E_CANNOTCONNECT  |接收不支援此連接點所需的介面。|
|CONNECT_E_NOCONNECTION  |Cookie 值不代表有效的連接。 此錯誤通常表示控制項的事件接收對應中的專案發生問題, 或在`IDispEventImpl`或`IDispEventSimpleImpl`基類中使用的樣板引數發生問題。|

### <a name="remarks"></a>備註

這個方法的基底實作為搜尋事件接收對應中的專案。 然後, 它會針對事件接收器對應的接收專案所描述的 COM 物件, 建議或 unadvises 連接點。 這個成員方法也會依賴衍生類別`IDispEventImpl`針對接收對應中每個要建議或 unadvised 的控制項, 繼承自其中一個實例的事實。
...
*dwInitParam*<br/>
要在控制項建立期間傳遞至控制項的資料。 當做*dwInitParam*傳遞的資料會顯示為[WM_INITDIALOG](/windows/win32/dlgbox/wm-initdialog)訊息的 LPARAM 參數, 在建立時, 將會傳送到複合控制項。
