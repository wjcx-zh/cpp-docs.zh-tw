...
|名稱|描述|
|----------|-----------------|
|[CComControlBase::AppearanceType](#appearancetype)|如果您的 `m_nAppearance` stock 屬性不是**short**類型，則覆寫。|
...
|[CComControlBase：:D oesVerbActivate](#doesverbactivate)|檢查 `IOleObjectImpl::DoVerb` 所使用的*iVerb*參數會啟動控制項的使用者介面（*iVerb* equals OLEIVERB_UIACTI加值稅E）、定義使用者按兩下控制項（*iVerb* equals OLEIVERB_PRIMARY）時所採取的動作、顯示控制項（*iVerb* Equals OLEIVERB_SHOW），或啟動控制項（*iVerb* equals OLEIVERB_INPLACEACTI加值稅E）。|
|[CComControlBase：:D oesVerbUIActivate](#doesverbuiactivate)|檢查 `IOleObjectImpl::DoVerb` 所使用的*iVerb*參數會導致控制項的使用者介面啟動並傳回 TRUE。|
|[CComControlBase：:D oVerbProperties](#doverbproperties)|顯示控制項的屬性頁。|
|[CComControlBase::FireViewChange](#fireviewchange)|呼叫這個方法來指示容器重繪控制項，或通知已註冊的通知接收器，控制項的 view 已變更。|
|[CComControlBase::GetAmbientAppearance](#getambientappearance)|抓取 DISPID_AMBIENT_APPEARANCE，此為控制項的目前外觀設定：0代表平面，1代表3D。|
|[CComControlBase::GetAmbientAutoClip](#getambientautoclip)|抓取 DISPID_AMBIENT_AUTOCLIP，此旗標表示容器是否支援控制項顯示區域的自動裁剪。|
|[CComControlBase::GetAmbientBackColor](#getambientbackcolor)|抓取容器所定義之所有控制項的環境背景色彩 DISPID_AMBIENT_BACKCOLOR。|
...
|[CComControlBase::GetAmbientFontDisp](#getambientfontdisp)|抓取容器的環境 `IFontDisp` 分派介面的指標。|
|[CComControlBase::GetAmbientForeColor](#getambientforecolor)|抓取容器所定義之所有控制項的環境前景色彩 DISPID_AMBIENT_FORECOLOR。|
|[CComControlBase::GetAmbientLocaleID](#getambientlocaleid)|抓取 DISPID_AMBIENT_LOCALEID，此為容器所使用之語言的識別碼。|
|[CComControlBase::GetAmbientMessageReflect](#getambientmessagereflect)|抓取 DISPID_AMBIENT_MESSAGEREFLECT，此旗標表示容器是否想要接收視窗訊息（例如 WM_DRAWITEM）做為事件。|
...
|[CComControlBase::GetAmbientScaleUnits](#getambientscaleunits)|抓取 DISPID_AMBIENT_SCALEUNITS，這是標籤顯示的容器環境單位（例如，英寸或釐米）。|
...
|[CComControlBase::GetAmbientTextAlign](#getambienttextalign)|抓取 DISPID_AMBIENT_TEXTALIGN，這是容器慣用的文字對齊方式：0代表一般對齊（數位 right、文字左邊）、1代表左對齊，2表示置中對齊，而3則表示右對齊。|
|[CComControlBase::GetAmbientTopToBottom](#getambienttoptobottom)|抓取 DISPID_AMBIENT_TOPTOBOTTOM，這是容器顯示內容的方向。|
|[CComControlBase::GetAmbientUIDead](#getambientuidead)|抓取 DISPID_AMBIENT_UIDEAD，此旗標表示容器是否希望控制項回應使用者介面動作。|
...
|[CComControlBase::OnDrawAdvanced](#ondrawadvanced)|預設 `OnDrawAdvanced` 準備正規化的裝置內容以進行繪製，然後呼叫控制項類別的 `OnDraw` 方法。|
...
|[CComControlBase：： m_bDrawFromNatural](#m_bdrawfromnatural)|旗標，表示 `IDataObjectImpl::GetData` 和 `CComControlBase::GetZoomInfo` 應該從 `m_sizeNatural` 而不是從 `m_sizeExtent`設定控制項大小。|
|[CComControlBase：： m_bDrawGetDataInHimetric](#m_bdrawgetdatainhimetric)|旗標，表示在繪製時，`IDataObjectImpl::GetData` 應該使用 HIMETRIC 單位，而不是圖元。|
...
|[CComControlBase：： m_spAmbientDispatch](#m_spambientdispatch)|`CComDispatchDriver` 物件，可讓您透過 `IDispatch` 指標來抓取和設定容器的屬性。|
...
如果您的 `m_nAppearance` stock 屬性不是**short**類型，則覆寫。
...
ATL 控制項嚮導會加入 short 類型的 `m_nAppearance` stock 屬性。 如果您使用不同的資料類型，請覆寫 `AppearanceType`。
...
將控制項大小初始化為 5080X5080 HIMETRIC 單位（2「X2」），並將 `CComControlBase` 的資料成員值初始化為 Null 或 FALSE。
...
如果控制項是視窗型，`~CComControlBase` 呼叫[DestroyWindow](/windows/win32/api/winuser/nf-winuser-destroywindow)來終結它。
...
檢查 `IOleObjectImpl::DoVerb` 所使用的*iVerb*參數會啟動控制項的使用者介面（*iVerb* equals OLEIVERB_UIACTI加值稅E）、定義使用者按兩下控制項（*iVerb* equals OLEIVERB_PRIMARY）時所採取的動作、顯示控制項（*iVerb* Equals OLEIVERB_SHOW），或啟動控制項（*iVerb* equals OLEIVERB_INPLACEACTI加值稅E）。
...
檢查 `IOleObjectImpl::DoVerb` 所使用的*iVerb*參數會導致控制項的使用者介面啟動並傳回 TRUE。
...
這個方法也會抓取控制項的 `IOleInPlaceSite`、`IOleInPlaceSiteEx`或 `IOleInPlaceSiteWindowless` 介面指標，並將它儲存在控制項類別的資料成員[CComControlBase：： m_spInPlaceSite](#m_spinplacesite)中。 控制項類別資料成員[CComControlBase：： m_bInPlaceSiteEx](#m_binplacesiteex)、 [CComControlBase：： m_bWndLess](#m_bwndless)、 [CComControlBase：： m_bWasOnceWindowless](#m_bwasoncewindowless)和[CComControlBase：： m_bNegotiatedWnd](#m_bnegotiatedwnd)會適當地設定為 true。
...
這個旗標是由 `IOleObjectImpl::SetExtent` 檢查，如果為 TRUE，則會導致函式傳回 E_FAIL。
...
旗標，表示 `IDataObjectImpl::GetData` 和 `CComControlBase::GetZoomInfo` 應該從 `m_sizeNatural` 而不是從 `m_sizeExtent`設定控制項大小。
...
旗標，表示在繪製時，`IDataObjectImpl::GetData` 應該使用 HIMETRIC 單位，而不是圖元。
...
這表示控制項可見，而且其視窗（如果有的話）可見，但其功能表和工具列可能不在作用中。 `m_bUIActive` 旗標表示控制項的使用者介面（例如功能表）也是使用中的。
...
資料成員 `m_spInPlaceSite` 會指向[IOleInPlaceSite](/windows/win32/api/oleidl/nn-oleidl-ioleinplacesite)、 [IOleInPlaceSiteEx](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesiteex)或[IOleInPlaceSiteWindowless](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesitewindowless)介面，視 `m_bWndLess` 的值和 `m_bInPlaceSiteEx` 旗標而定。 （資料成員 `m_bNegotiatedWnd` 必須為 TRUE，`m_spInPlaceSite` 指標才會是有效的）。

如果 `m_bWndLess` 為 FALSE 且 `m_bInPlaceSiteEx` 為 TRUE，則 `m_spInPlaceSite` 為 `IOleInPlaceSiteEx` 介面指標。 如需顯示這三個資料成員之間關聯性的資料表，請參閱[m_spInPlaceSite](#m_spinplacesite) 。
...
這個旗標是由[IOleObjectImpl：： SetExtent](../../atl/reference/ioleobjectimpl-class.md#setextent)檢查，如果為 TRUE，`SetExtent` 會通知容器的視圖變更。 如果設定此旗標，則也應該設定[OLEMISC](/windows/win32/api/oleidl/ne-oleidl-olemisc)列舉中的 OLEMISC_RECOMPOSEONRESIZE 位。
...
您可以使用[CComControlBase：： SetDirty](#setdirty)來設定 `m_bRequiresSave` 的值，並使用[CComControlBase：： GetDirty](#getdirty)來抓取。
...
此旗標是由 `IOleObjectImpl::SetExtent` 檢查，如果為 TRUE，則會將傳遞至 `SetExtent` 的大小指派給 `m_sizeNatural`。

無論 `m_bResizeNatural`的值為何，傳遞至 `SetExtent` 的大小一律都會指派給 `m_sizeExtent`。
...
`m_bInPlaceActive` 旗標表示控制項為使用中，而不是其使用者介面為使用中。
...
如果 `m_bWndLess` 為 TRUE，`m_spInPlaceSite` 就是 `IOleInPlaceSiteWindowless` 介面指標。 如需顯示這些資料成員之間完整關聯性的資料表，請參閱[CComControlBase：： m_spInPlaceSite](#m_spinplacesite) 。
...
這種大小是固定的，而 `m_sizeExtent` 中的大小會以顯示星號調整。
...
`CComDispatchDriver` 物件，可讓您透過 `IDispatch` 指標來抓取和設定物件的屬性。
...
介面 `m_spDataAdviseHolder` 會[執行 IDataObject：:D 的建議](/windows/win32/api/objidl/nf-objidl-idataobject-dadvise)和[IDataObject：:D unadvise](/windows/win32/api/objidl/nf-objidl-idataobject-dunadvise)方法，以建立和刪除容器的諮詢連線。 控制項的容器必須藉由支援[IAdviseSink](/windows/win32/api/objidl/nn-objidl-iadvisesink)介面來實作為建議的接收。
...
只有在[m_bNegotiatedWnd](#m_bnegotiatedwnd)旗標為 TRUE 時，`m_spInPlaceSite` 指標才有效。

下表顯示 `m_spInPlaceSite` 指標類型如何相依于[m_bWndLess](#m_bwndless)和[m_bInPlaceSiteEx](#m_binplacesiteex)資料成員旗標：
...
當您使用 ATL 控制項嚮導來建立控制項時，會自動將 `OnDraw` 方法加入至控制項類別。 Wizard 的預設 `OnDraw` 會繪製具有 "ATL 8.0" 標籤的矩形。
...
預設 `OnDrawAdvanced` 準備正規化的裝置內容以進行繪製，然後呼叫控制項類別的 `OnDraw` 方法。
...
如果*wParam*不是 Null，`OnPaint` 會假設它包含有效的 HDC 並使用它，而不是[CComControlBase：： m_hWndCD](#m_hwndcd)。
...
`SendOnViewChange` 會呼叫[IAdviseSink：： OnViewChange](/windows/win32/api/objidl/nf-objidl-iadvisesink-onviewchange)。 目前僅支援*lindex*的值為-1，表示整個視圖都是相關的。
...
應該呼叫 `SetDirty(TRUE)` 來旗標，表示控制項自上次儲存後已變更。 `m_bRequiresSave` 的值是使用[CComControlBase：： GetDirty](#getdirty)來抓取。
