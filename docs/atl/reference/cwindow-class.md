...
此方法的第二個版本可讓您將文字儲存在 BSTR 中;第三個版本可讓您將結果儲存在[CString](../../atl-mfc-shared/reference/cstringt-class.md)中，因為 `CSimpleString` 是的基類 `CString` 。
...
此函式會取代 `CWindow::SetWindowLong` 方法。 若要撰寫與 Windows 32 位和64位版本相容的程式碼，請使用 `CWindow::SetWindowLongPtr` 。
...
這個方法的第二個版本會使用[RECT](/windows/win32/api/windef/ns-windef-rect)結構來設定視窗的新位置、寬度和高度。
