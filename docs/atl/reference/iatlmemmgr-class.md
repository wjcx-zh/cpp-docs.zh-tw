...
|[可用](#free)|呼叫此方法以釋放記憶體區塊。|  
|[GetSize](#getsize)|呼叫此方法以擷取已配置的記憶體區塊的大小。|  
...
> [!NOTE]
>  本機和全域堆積函式低於其他記憶體管理函式，但並未提供許多功能。 因此，新的應用程式應該使用[堆積函式](http://msdn.microsoft.com/library/windows/desktop/aa366711)。 在使用這些項目[CWin32Heap](../../atl/reference/cwin32heap-class.md)類別。  
...
### <a name="remarks"></a>備註  
 呼叫[IAtlMemMgr::Free](#free)或[IAtlMemMgr::Reallocate](#reallocate)釋放這個方法所配置的記憶體。  
...
### <a name="remarks"></a>備註  
 使用這個方法來釋放所佔用的記憶體[IAtlMemMgr::Allocate](#allocate)或[IAtlMemMgr::Reallocate](#reallocate)。  
  
### <a name="example"></a>範例  
 如需範例，請參閱[IAtlMemMgr 概觀](../../atl/reference/iatlmemmgr-class.md)。  
  
##  <a name="getsize"></a>  IAtlMemMgr::GetSize  
 呼叫此方法以擷取已配置的記憶體區塊的大小。  
...
### <a name="remarks"></a>備註  
 ...
 在概念上這個方法會釋放現有的記憶體，並且會配置新的記憶體區塊。 事實上，可能會擴充或否則重複使用現有的記憶體。  
...
### <a name="remarks"></a>備註  
 當`SetAmbientDispatch`呼叫與新介面的指標，這個新介面將用來叫用任何屬性或方法要求輸入裝載控制項 — 如果這些屬性所未提供的[IAxWinAmbientDispatch](../../atl/reference/iaxwinambientdispatch-interface.md).  
...
### <a name="remarks"></a>備註  
 請參閱[IAxWinHostWindow::CreateControl](#createcontrol)其餘的參數和傳回值的說明。  
...
##  <a name="createcontrollicex"></a>  IAxWinHostWindowLic::CreateControlLicEx  
 建立授權的 ActiveX 控制項、 初始化，然後將它裝載於指定的視窗，類似於[IAxWinHostWindow::CreateControl](#createcontrol)。  
...
### <a name="remarks"></a>備註  
 請參閱[IAxWinHostWindow::CreateControlEx](#createcontrolex)其餘的參數和傳回值的說明。  
