...
ms.locfileid: "69495573"
---
# <a name="ipropertypageimpl-class"></a>IPropertyPageImpl 類別

這個類別`IUnknown`會執行並提供[IPropertyPage](/windows/win32/api/ocidl/nn-ocidl-ipropertypage)介面的預設實值。
...
|[IPropertyPageImpl::Apply](#apply)|將目前的屬性頁值套用至透過`SetObjects`指定的基礎物件。 ATL 實作為傳回 S_OK。|
|[IPropertyPageImpl::Deactivate](#deactivate)|損毀以`Activate`建立的視窗。|
...
|[IPropertyPageImpl::SetObjects](#setobjects)|提供與屬性頁`IUnknown`相關聯之物件的指標陣列。 這些物件會透過呼叫來`Apply`接收目前的屬性頁值。|
|[IPropertyPageImpl::SetPageSite](#setpagesite)|提供具有`IPropertyPageSite`指標的屬性頁, 此屬性頁會透過它與屬性框架通訊。|
...
|[IPropertyPageImpl::m_pPageSite](#m_ppagesite)|指向屬性頁`IPropertyPageSite`用來與屬性框架通訊的介面。|
|[IPropertyPageImpl::m_ppUnk](#m_ppunk)|指向與屬性頁相關`IUnknown`聯之物件的指標陣列。|
|[IPropertyPageImpl::m_size](#m_size)|儲存屬性頁之對話方塊的高度和寬度 (以圖元為單位)。|

## <a name="remarks"></a>備註

[IPropertyPage](/windows/win32/api/ocidl/nn-ocidl-ipropertypage)介面可讓物件管理屬性工作表中的特定屬性頁。 類別`IPropertyPageImpl`提供此介面的預設執行, 並藉`IUnknown`由將資訊傳送至偵錯工具組建中的傾印裝置來實現。
...
將目前的屬性頁值套用至透過`SetObjects`指定的基礎物件。
...
`GetPageInfo`載入與[m_dwDocString](#m_dwdocstring)、 [m_dwHelpFile](#m_dwhelpfile)和[m_dwTitle](#m_dwtitle)相關聯的字串資源。
...
`IsPageDirty`如果頁面自啟動後已變更, 則傳回 S_OK。
...
如有必要`SetDirty` , 會通知框架屬性頁已變更。

##  <a name="setobjects"></a>IPropertyPageImpl::SetObjects

提供與屬性頁`IUnknown`相關聯之物件的指標陣列。
