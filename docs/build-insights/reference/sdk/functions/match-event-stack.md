...
`MatchEventStack` 函數是用來比對事件堆疊與特定的事件階層架構。 符合的階層會轉送至處理常式，以供進一步處理。 若要深入瞭解事件、事件堆疊和階層，請參閱[事件資料表](../event-table.md)。
...
*TExtraArgs*\
傳遞至 `MatchEventStack`之額外引數的類型。

*eventStack*\
要比對*TEvent*和*TEvents*所描述之事件種類階層的事件堆疊。

可呼叫*的\*
在成功比對事件堆疊與*TEvent*和*TEvents*所描述的事件種類階層架構之後，`MatchEventStack` 會叫用可呼叫*的。* 它會針對事件階層中的每個型*別，將一個 r*值引數傳遞給。 *ExtraArgs*參數套件在可呼叫的剩餘參數中是完美轉送*的。*
