...
description: C++ BUILD Insights SDK StartTracingSession 函數參考。
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
`StartTracingSession` 函數會啟動追蹤會話。 呼叫此函式的可執行檔必須具有系統管理員許可權。
...
*sessionName*\
要啟動之追蹤會話的名稱。 呼叫[StopTracingSession](stop-tracing-session.md)或任何其他的 stop 追蹤函數時，請使用相同的名稱。
