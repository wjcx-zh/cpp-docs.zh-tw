...
description: C++ BUILD Insights SDK StopAndAnalyzeTracingSessionW 函數參考。
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
`StopAndAnalyzeTracingSessionW` 函式會停止進行中的追蹤會話，並將產生的追蹤儲存在暫存檔案中。 接著會使用暫存檔做為輸入，立即開始分析會話。 呼叫此函式的可執行檔必須具有系統管理員許可權。
...
*統計資料*\
[TRACING_SESSION_STATISTICS](../other-types/tracing-session-statistics-struct.md)物件的指標。 `StopAndAnalyzeTracingSessionW` 在傳回之前，會在此物件中寫入追蹤集合統計資料。

*analysisDescriptor*\
[ANALYSIS_DESCRIPTOR](../other-types/analysis-descriptor-struct.md)物件的指標。 使用此物件來設定 `StopAndAnalyzeTracingSessionW`啟動的分析會話。
