...
Windows on ARM 會在[結構化例外狀況處理](/windows/win32/debug/structured-exception-handling)(SEH) 期間, 使用回溯*代碼*來控制堆疊回溯。 回溯程式碼是儲存在可執行映像檔 .xdata 區段的位元組序列。 它們以抽象方法描述函式序言和結尾程式碼的作業，因此函式序言的影響可以在準備回溯至呼叫者堆疊框架時復原。
...
|1|0-1|*旗*標是2位欄位, 表示如何解讀第二個 pdata 字的剩餘30位。 如果*旗*標為 0, 則其餘的位會形成*例外狀況資訊 RVA* (低兩個位隱含 0)。 如果*旗*標為非零, 則其餘的位會形成*封裝*的回溯資料結構。|
...
|1|13-14|*Ret*是2位欄位, 表示函式傳回的方式:<br /><br />-00 = 透過 pop {pc} 傳回 (在此情況下, *l*旗標位必須設定為 1)。<br />-01 = 使用16位分支傳回。<br />-10 = 使用32位分支傳回。<br />-11 = 完全沒有結尾。 這在描述可能僅包含序言而結尾在別處的不連續函式片段非常有用。|
|1|15|*H*是1位旗標, 指出函式 "房屋" 整數參數是否會在函式的開頭推送, 以註冊 (r0-r3), 並在傳回之前解除配置16個位元組的堆疊。 (0 = 不將寄存器參數寫入堆疊，1 = 將寄存器參數寫入堆疊)。|
|1|16-18|*Reg*是一個3位欄位, 表示最後儲存的非 volatile 暫存器的索引。 如果*R*位是 0, 則只會儲存整數暫存器, 並假設為 R4-rN 的範圍, 其中 N 等於 4 + *Reg*。如果*R*位是 1, 則只會儲存浮點暫存器, 並假設為 d8 dN 的範圍, 其中 N 等於 8 + *Reg*。*R* = 1 和*Reg* = 7 的特殊組合表示不會儲存任何暫存器。|
|1|19|*R*是1位旗標, 指出儲存的非 volatile 暫存器是否為整數暫存器 (0) 或浮點數暫存器 (1)。 如果*R*設定為 1, 而且*Reg*欄位設定為 7, 則不會推送任何非暫時性暫存器。|
...
如果指定非摺疊調整，則指令 6 為明確堆疊調整。 由於*PF*與*EF*無關, 因此在沒有指令6的情況下, 可能會有指令 5, 反之亦然。

指示7和8使用與序言相同的邏輯來判斷要從堆疊還原哪些暫存器, 但有這兩項變更: 第一種是使用*EF*來取代*PF*;第二, 如果*Ret* = 0, 則會將 LR 取代為註冊清單中的 PC, 而結尾會立即結束。

如果設定了*H* , 則會出現指令9a 或9b。 當*L*是0時, 會使用指令 9a, 以指出 LR 不在堆疊上。 在此情況下, 會以手動方式調整堆疊, 而*Ret*必須是1或 2, 才能指定明確的傳回。 當*L*是1時, 會使用指令 9b, 以指出結尾的初期結束, 並同時傳回和調整堆疊。
...
1. 1 或 2 字組標頭描述 .xdata 結構的整體大小，並提供關鍵函式資料。 只有當 [*結尾計數*] 和 [程式*代碼文字*] 欄位都設定為0時, 才會出現第二個單字。 下表詳細說明這些欄位：
   ...
   |0|18-19|[類型] 是2位欄位, 可描述剩餘 .xdata 的版本。 目前僅定義版本 0，保留值 1-3。|
   ...
   |0|23-27|*結尾 Count*是有兩個意義的5位欄位, 視*電子*位的狀態而定:<br /><br /> -如果*E*為 0, 此欄位就是第3節中所述的例外狀況範圍總數的計數。 如果函式中存在超過31個範圍, 則此欄位和 [程式*代碼文字*] 欄位必須都設定為 0, 以指出需要擴充字組。<br />-如果*E*是 1, 這個欄位就會指定描述唯一結尾之第一個回溯程式碼的索引。|
   |0|28-31|程式*代碼字*是4位欄位, 指定包含第4節中所有回溯程式碼所需的32位字組數目。 如果超過63個回溯程式碼位元組需要超過15個字組, 則此欄位和 [*結尾計數*] 欄位必須都設定為 0, 以指出需要擴充字。|
...
1. 在例外狀況資料之後 (如果標頭中的*E*位設定為 0), 就會是結尾範圍的相關資訊清單, 這會封裝成單字, 並以遞增的起始位移順序儲存。 每一個範圍包含下列欄位：
...
1. 如果標頭中的*X*欄位為 1, 則回溯程式碼位元組後面會接著例外狀況處理常式資訊。 這包含一個*例外狀況處理常式 RVA* , 其中包含例外狀況處理常式的位址, 後面緊接著例外狀況處理常式所需的 (可變長度) 資料量。
...
在第三個和第四個案例中, 藉由將 pdata 記錄的*旗*標欄位設定為 2, 或將 .xdata 標頭中的*F*旗標設定為 1, 來指定虛擬序言的存在。 任何一種情況，都會忽略對部分序言回溯的檢查，且所有非結尾回溯都會被視為完整的。
...
### <a name="example-1-leaf-function-no-locals"></a>範例 1：分葉函數, 沒有區域變數
...
### <a name="example-2-nested-function-with-local-allocation"></a>範例 2：具有本機配置的嵌套函數
...
### <a name="example-3-nested-variadic-function"></a>範例 3：Nested Variadic 函數
...
- Word 1:結尾範圍位於 offset 0xC6 (= 0x18C/2), 在0x00 處啟動回溯程式碼索引, 以及0x0E 的條件 (always)
