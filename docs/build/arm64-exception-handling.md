...
1. 在所有情況下，提供足夠的描述以允許回溯，而不需要程式碼探查。
   ...
   - 分析程式碼需要將程式碼分頁。 這可避免在某些情況下（追蹤、取樣、調試）進行回溯。

   - 分析程式碼很複雜;編譯器必須謹慎地產生回溯器能夠解碼的指示。

   - 如果回溯無法透過使用回溯代碼來完整描述，在某些情況下，它必須切換回指令解碼。 這會增加整體複雜度，而且最好是避免的。

1. 支援在初初初和 mid 中回溯。

   - 在 Windows 中，回溯會用於例外狀況的處理，因此，即使在初構或終解程式碼序列的中間，我們還是可以執行精確的回溯。
...
1. 初構和 epilogs 通常會彼此鏡像。 藉由利用此共同特性，描述回溯所需的中繼資料大小可以大幅降低。 在函式的主體中，不論初構的作業是否復原，或終的作業是以正向的方式執行，都不會有任何影響。 這兩個作業會產生相同的結果。

1. 函式通常會整體相對較小。 為了達到最有效率的資料封裝，空間的數個優化會依賴這項功能。

1. Epilogs 中沒有條件式程式碼。

1. 專用框架指標註冊：如果 sp 儲存在初構中的另一個暫存器（x29）中，該暫存器在整個函式中維持不變，因此原始 sp 可以隨時復原。
...
1. Unchained，分葉函數（lr 未儲存）
   ...
   所有區域變數都是根據 SP 來存取。 @no__t 0x29，lr > 指向上一個畫面。 針對框架大小 \< = 512，"sub sp，..."如果 regs 儲存的區域移至堆疊底部，可以將其優化。 其缺點是，它與上述的其他版面配置不一致，而且儲存的 regs 會包含 regs 和前置和後置索引位移定址模式的部分範圍。
...
1. 連鎖，#framesz \< = 512，#outsz = 0
   ...
   相較于上述 #1 初構，優點是所有的暫存器儲存指示都已準備好在只有一個堆疊配置指令之後才執行。 因此，不會對 sp 造成任何不相依性，而導致無法進行指令層級平行處理。
...
Pdata 記錄是固定長度專案的已排序陣列，描述 PE 二進位檔中的每個堆疊操作函數。 請注意，「堆疊操作」一詞：不需要任何本機儲存體，而且不需要儲存/還原非 volatile 暫存器的分葉函數並不需要 pdata 記錄;您應該明確地省略這些，以節省空間。 其中一個函式的回溯可以直接取得 LR 的傳回位址，以將其移至呼叫端。

ARM64 的每個 pdata 記錄長度為8個位元組。 每筆記錄的一般格式會將函式的32位 RVA 放在第一個單字中，後面接著第二個，其中包含 .xdata 區塊的指標，或描述標準函式回溯順序的封裝字組。
...
- **旗**標是2位欄位，表示如何解讀第二個 pdata 字的剩餘30位。 如果**旗**標為0，則其餘的位會形成**例外狀況資訊 RVA** （低兩個位隱含0）。 如果**旗**標為非零，則其餘的位會形成**封裝**的回溯資料結構。
...
1. 描述結構整體大小和提供關鍵函數資料的1或2個字組標頭。 只有當 [終**計數**] 和 [程式**代碼字**] 欄位都設定為0時，才會出現第二個單字。 這些是標頭中的位欄位：

   a. **函數長度**是18位欄位，指出函式的總長度（以位元組為單位），除以4。 如果函數大於1百萬，則必須使用多個 pdata 和 .xdata 記錄來描述函數。 如需詳細資訊，請參閱[大型函數](#large-functions)一節。

   b. 「類型 **」是2位欄位，描述**剩餘 .xdata 的版本。 在撰寫本文時，只會定義版本0，因此不允許1-3 的值。

   c. **X**是1位欄位，指出例外狀況資料的目前狀態（1）或缺勤（0）。

   d. **E**是一個位欄位，指出描述單一終終的資訊會封裝成標頭（1），而不是稍後需要額外的範圍字組（0）。

   e. [終**計數**] 是有兩個意義的5位欄位，視**電子**位的狀態而定：

      1. 如果**E**設定為0：它會指定第2節中所述之終解範圍總數的計數。 如果函式中存在超過31個範圍，則 [程式**代碼文字**] 欄位必須設定為0，以指出需要擴充字組。

      2. 如果**E**設定為1，則此欄位會指定第一個回溯程式碼的索引，以描述其中一個，而且只會進行終解。

   f. 程式**代碼字**是一個5位欄位，指定包含第3節中所有回溯程式碼所需的32位字組數目。 如果需要超過31個單字（也就是超過124回溯程式碼位元組），則此欄位必須設定為0，以指出需要擴充字組。

   g. **擴充**的終數和擴充的程式**代碼字**組分別為16位和8位的欄位，可提供更多的空間來編碼異常大量的 epilogs 或異常大量的回溯程式碼字組。 只有當第一個標頭單字中的 [終**計數**] 和 [程式**代碼字**] 欄位設定為0時，才會顯示包含這些欄位的延伸字。

1. 在上述標頭和選用的擴充標頭之後，如果終**計數**不是零，就會是有關終解範圍、封裝一個到單字，並以遞增起始位移順序儲存的資訊清單。 每個範圍都包含下列位：

   a. 終解**開始位移**是一個18位欄位，描述相對於函式開頭之終的位移（以位元組除以4）

   b. **Res**是保留給未來擴充的4位欄位。 其值必須為 0。

   c. 終解**開始索引**是10位（比擴充的程式**代碼單字**還要多2個位）欄位，指出描述此終終的第一個回溯程式碼的位元組索引。

1. 在終的範圍清單之後，會產生包含回溯程式碼的位元組陣列，稍後的章節會有詳細說明。 此陣列在最近完整字組界面的結尾處填補。 回溯代碼會寫入至此陣列，從最接近函式主體的一個開始，移至函式的邊緣。 每一個回溯程式碼的位元組都是以位元組由大到小的順序儲存，因此可以直接提取，從最重要的位元組開始，以識別作業和其餘程式碼的長度。

1. 最後，在回溯程式碼位元組之後，如果標頭中的**X**位設為1，則會提供例外狀況處理常式資訊。 這包含單一的**例外狀況處理常式 RVA** ，提供例外狀況處理常式本身的位址，後面緊接著例外狀況處理常式所需的可變長度資料量。

上述 .xdata 記錄的設計可讓您提取前8個位元組，並從中計算記錄的完整大小（減去後面的可變大小例外狀況資料長度）。 下列程式碼片段會計算記錄大小：
...
請注意，雖然初構和每個終解在回溯程式碼中都有自己的索引，但資料表會在它們之間共用，而且完全可行（而且並非全部罕見）它們都可以共用相同的程式碼（請參閱範例章節 bel 中的範例2）。允許）。 編譯器撰寫者應該針對此情況進行優化，特別是因為可指定的最大索引為255，因而限制特定函式的回溯程式碼總數。

### <a name="unwind-codes"></a>回溯代碼

回溯程式碼的陣列是順序的集區，其中描述如何以作業需要復原的順序來復原初構的效果。 回溯代碼可以視為迷你指令集，並編碼為位元組的字串。 執行完成時，呼叫函式的傳回位址會在 lr 暫存器中，而且所有的非 volatile 暫存器都會在呼叫函數時還原為其值。

如果例外狀況保證只會發生在函式主體內（而且不含初構或任何終解），則只需要一個序列。 不過，Windows 回溯模型需要我們能夠在部分執行的初構或終解內進行回溯。 為了符合此需求，已仔細設計回溯程式碼，使其能夠明確地將1:1 對應至初構和終解中的每個相關 opcode。 這有下列幾個含意：

1. 藉由計算回溯程式碼的數目，可以計算初構和終解的長度。

1. 藉由計算結束範圍開始後的指令數目，可以略過對等的回溯程式碼數目，並執行序列的其餘部分，以完成終執行的部分執行回溯。

1. 藉由計算初構結尾之前的指令數目，可以略過相同數目的回溯程式碼，並執行序列的其餘部分，只復原已完成執行的初構部分。

回溯代碼是根據下表進行編碼。 所有回溯程式碼都是單一/雙位元組，但配置大型堆疊的則除外。 完全有21個回溯程式碼。 每個回溯程式碼都只會對應初構/終解中的一個指令，以允許回溯部分執行的初構和 epilogs。

|回溯程式碼|位和轉譯|
|-|-|
|`alloc_s`|000xxxxx：配置大小 \< 512 （2 ^ 5 * 16）的小型堆疊。|
|`save_r19r20_x`|    001zzzzz：儲存 \<x19、x20 > 組，位於 [sp #Z * 8]！，預先索引的位移 > =-248 |
|`save_fplr`|        01zzzzzz：在 [sp + #Z * 8] 上儲存 \<x29，lr > 配對，位移 \< = 504。 |
|`save_fplr_x`|        10zzzzzz：在 [sp-（#Z + 1） * 8]！，預先索引的位移 > =-512，儲存 \<x29，lr > 配對 |
|`alloc_m`|        11000xxx'xxxxxxxx：配置大小 \< 16k （2 ^ 11 * 16）的大型堆疊。 |
|`save_regp`|        110010xx'xxzzzzzz：在 [sp + #Z * 8] 上儲存 x （19 + #X）組，位移 \< = 504 |
...
|`save_fregp`|        1101100x'xxzzzzzz：在 [sp + #Z * 8] 上儲存配對 d （8 + #X），位移 \< = 504 |
|`save_fregp_x`|        1101101x'xxzzzzzz：儲存配對 d （8 + #X），位於 [sp-（#Z + 1） * 8]！，預先索引的位移 > =-512 |
...
|`arithmetic(add)`|    11100111 ' 000zxxxx：將 cookie reg （z）新增至 lr （0 = x28，1 = sp）;新增 lr、lr、reg （z） |
|`arithmetic(sub)`|    11100111 ' 001zxxxx：來自 lr 的 sub cookie reg （z）（0 = x28，1 = sp）;子 lr、lr、reg （z） |
...
在涵蓋多個位元組的大型值指示中，會先儲存最重要的位。 上述回溯程式碼的設計是只要查閱程式碼的第一個位元組，就可以知道回溯程式碼的總大小（以位元組為單位）。 假設每個回溯程式碼都與初構/終解中的指令完全對應，以計算初構或終解的大小，則所有需要完成的動作都是使用查閱資料表或類似的裝置，來判斷 cor 的時間長度。回應作業碼為。

請注意，在初構中不允許使用後置索引位移定址。 所有位移範圍（#Z）都符合 STP/STR 定址的編碼，但 `save_r19r20_x`，其中248對所有儲存區域都已足夠（10個 Int 暫存器 + 8 FP 暫存器 + 8 個輸入暫存器）。

`save_next` 必須遵循 Int 或 FP volatile 暫存器配對的儲存： `save_regp`、`save_regp_x`、`save_fregp`、`save_fregp_x`、`save_r19r20_x` 或另一個 `save_next`。 它會在接下來的16個位元組插槽中，儲存「成長」順序的下一個暫存器配對。 `save-next` 之後，代表最後一個 Int 暫存器配對的 `save_next` 會參考第一個 FP 暫存器配對。

由於一般傳回和跳躍指示的大小相同，因此不需要針對尾呼叫案例使用分隔的 @no__t 0 回溯程式碼。

`end_c` 是設計用來處理不連續的函式片段，以供優化之用。 @No__t-0，表示目前範圍中回溯程式碼的結尾後面必須接著另一系列的回溯程式碼，並以真實的 `end` 結束。 @No__t-0 和 `end` 之間的回溯代碼代表父區域（"虛設" 初構）中的初構作業。  如需更多詳細資料和範例，請參閱下一節。

### <a name="packed-unwind-data"></a>封裝的回溯資料

對於初構和 epilogs 遵循下面所述標準格式的函式，可以使用封裝的回溯資料，而不需要徹底 .xdata 記錄，並大幅降低提供回溯資料的成本。 標準初構和 epilogs 的設計目的，是為了符合不需要例外狀況處理常式的簡單函式的一般需求，而這會以標準循序執行其設定和清除作業。
...
- **旗**標是如上所述的2位欄位，具有下列意義：
  ...
  - 01 = 使用如下所述的封裝回溯資料，其中包含單一初構並在範圍的開頭和結尾終止
  - 10 = 針對沒有任何初構和終解的程式碼使用的封裝回溯資料，如下所述：這適用于描述分隔的函數區段。
  - 11 = 已保留;
- **函數長度**是一個11位欄位，提供整個函式的長度（以位元組為單位），除以4。 如果函數大於8k，則必須改用完整的 .xdata 記錄。
- **畫面格大小**是9位欄位，指出配置給此函式的堆疊位元組數（除以16）。 配置大於（8k-16）個位元組之堆疊的函數必須使用完整的 .xdata 記錄。 這包括 [區域變數] 區域、[外寄參數] 區域、[被呼叫端儲存的 Int] 和 [FP] 區域，以及 [首頁] 參數區域，但不包括動態配置區域
...
屬於類別1、2（不含傳出參數區域）、3和4的標準初構會以封裝的回溯格式來表示。  標準函式的 epilogs 遵循非常相似的形式，但**H**不會有任何作用、省略了 `set_fp` 指令，以及每個步驟中的指令順序會在終止時反轉。 封裝 .xdata 的演算法會遵循下列步驟，如下表所述：
...
在執行初構或終解時，如果發生例外狀況或中斷，則更難以正確地回溯。 在這些情況下，只會部分結構化堆疊框架，而此技巧是要判斷正確復原所做的確切動作。
...
每個 opcode 的旁邊是描述這項作業的適當回溯程式碼。 要注意的第一件事是，初構的回溯程式碼系列是終（不會計算終解的最後一個指令）的確切鏡像影像。 這是常見的情況，因此初構的回溯程式碼一律假設為從初構的執行順序以相反順序儲存。

因此，在初構和終初，我們都會留下一組常用的回溯程式碼：

`set_fp`, `save_regp 0,240`, `save_fregp,0,224`, `save_fplr_x_256`, `end`

從終的情況開始（以一般順序比較簡單），在終（從函式的 offset 0x100 開始）內的位移0，我們預期會執行完整的回溯順序，因為尚未進行清除。 如果我們在中發現自己的一個指示（在終的位移2），我們可以略過第一個回溯程式碼，成功回溯。 將這種情況一般化，假設 opcode 和回溯程式碼之間的1:1 對應，我們可以指出如果我們從終的指令 n 回溯，我們應該略過前 n 個回溯程式碼，並從該處開始執行。

其實初構的類似邏輯也適用于初構，但相反。 如果我們從初構中的位移0回溯，我們不想要執行任何操作。 如果我們從 offset 2 （這是中的一個指令）中展開，則我們會想要從結尾開始執行回溯程式碼（請記住，程式碼會以反向順序儲存）。 我們也可以把它一般化，如果我們從初構中的指令 n 回溯，就應該從程式代碼清單的結尾開始執行 n 回溯程式碼。

現在，初構和終解程式碼完全相符的情況不一定如此。 基於這個理由，回溯陣列可能需要包含數個程式碼序列。 若要判斷開始處理代碼的位移，請使用下列邏輯：

1. 如果是從函式主體內回溯，只需要在索引0開始執行回溯程式碼，並繼續進行，直到達到「結束」 opcode 為止。
...
由於這些規則的結果，初構的回溯程式碼必須一律是陣列中的第一個，而且它們也是用來在從主體內回溯的一般情況下回溯的程式碼。 任何終解特定的程式碼序列應該緊接在之後。

### <a name="function-fragments"></a>函數片段

基於程式碼優化的目的和其他原因，最好是將函式分割成分隔的片段（也稱為「區域」）。 完成這項作業時，每個產生的函式片段都需要自己的個別 pdata （可能是 .xdata）記錄。

針對具有自己初構的分隔次要片段，預期在其初構中不會進行堆疊調整。 次要區域所需的所有堆疊空間都必須由其父區域（或稱為「主機區域」）預先配置。 這會在函式的原始初構中嚴格保持堆疊指標操作。
...
1. 僅初構（區域1：所有 epilogs 都位於不同的區域）：
   ...
   只需要描述初構。 這不能以 pdata 格式表示。 在完整的 .xdata 案例中，這可以藉由設定 [終數] = 0 來表示。 請參閱上述範例中的區域1。

   回溯代碼： `set_fp`，`save_regp 0,240`，`save_fplr_x_256`，`end`。

1. 僅限 Epilogs （區域2：初構位於主機區域）

   這是假設，當時間控制項跳入此區域時，就會執行所有初構程式碼。 部分回溯可能會在 epilogs 中發生，方式與一般函式相同。 此類型的區域無法以 pdata 表示。 在完整 .xdata 記錄中，可以使用「虛設」初構來編碼，並以 @no__t 0 和 @no__t 1 回溯程式碼配對括住。  前置的 `end_c` 表示初構的大小為零。 將單一終解點的開始索引到 `set_fp`。

   區域2： `end_c`，`set_fp`，`save_regp 0,240`，`save_fplr_x_256`，`end` 的回溯程式碼。

1. 沒有初構或 epilogs （區域3：初構和所有 epilogs 都在其他片段中）：

   Pdata 格式可以透過設定旗標 = 10 來套用。 具有完整的 .xdata 記錄，終計數 = 1。 回溯程式碼與上述區域2相同，但終解開始索引也會指向 `end_c`。 部分回溯永遠不會發生在此程式碼區域中。

函式片段的另一個較複雜的案例是「壓縮包裝」，而該編譯器可能會選擇延遲儲存一些被呼叫者儲存的暫存器，直到函式進入初構的外部。
...
在區域1的初構中，會預先配置堆疊空間。 請注意，區域2將會有相同的回溯程式碼，即使它已移出其主機函式也一樣。
...
您可以利用片段來描述大於 .xdata 標頭中位欄位所加上1M 限制的功能。 若要描述非常大的函式，只需要將其分割成小於1百萬的片段。 每個片段都應該調整，讓它不會將終解分割成多個部分。
...
請注意，EpilogStart 索引 [0] 會指向相同的初構回溯程式碼序列。
...
注意：EpilogStart 索引 [4] 指向初構回溯程式碼的中間（部分重複使用的回溯陣列）。
