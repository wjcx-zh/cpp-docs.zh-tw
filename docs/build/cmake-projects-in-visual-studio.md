...
- Visual Studio 會執行*cmake，並產生預設設定*的 cmake 快取，也就是 x64 Debug。 CMake 命令列會連同 CMake 的其他輸出一起顯示在 [輸出] 視窗中。
...
Visual Studio 使用名為**CMakeSettings**的檔案來儲存 cmake 的環境變數或命令列選項。 **CMakeSettings**也可讓您定義和儲存多個 CMake 組建設定，並可在 IDE 中輕鬆切換它們。 在 Visual Studio 2019 中，[ **CMake 設定編輯器**] 提供一個方便的方式來編輯您的設定。 如需詳細資訊，請參閱[自訂 CMake 設定](customize-cmake-settings.md)。
...
當您開啟現有的 CMake 快取時，Visual Studio 不會嘗試管理您的快取和組建樹狀結構。 這可讓您的自訂或慣用工具完整控制 CMake 如何設定您的專案。 您可以在 Visual Studio 中開啟現有的快取， **> 開啟 > CMake** ，並流覽至現有的 cmakecache.txt。 或者，如果您已在 Visual Studio 中開啟專案，您可以使用與新增設定相同的方式，將現有的快取加入其中。 如需詳細資訊，請參閱我們在[Visual Studio 中開啟現有](https://devblogs.microsoft.com/cppblog/open-existing-cmake-caches-in-visual-studio/)快取的 blog 文章。
...
您可以自訂群組建設定、環境變數、命令列引數和其他設定，而不需要使用 [ **CMake 設定編輯器**] 修改 remote monitoring.h cmakelists.txt。 如需詳細資訊，請參閱[自訂 CMake 設定](customize-cmake-settings.md)。
...
當您使用 MSVC 編譯器為 Windows 建立時，如果 Visual Studio 中啟用此選項，則 CMake 專案僅支援編譯器和連結器中的程式碼偵錯工具。 若要變更此設定，請移至 **工具**  > **選項** @no__t **-3** **一般** @no__t。
...
根據預設，除非發生錯誤，否則會隱藏大部分的設定訊息。 您可以在 **工具**  > **選項** 中啟用這項功能，以查看所有訊息  > **CMake**。
