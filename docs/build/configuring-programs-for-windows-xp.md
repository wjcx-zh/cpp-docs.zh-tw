...
> [!NOTE]
>  如果您使用[!INCLUDE[vs_dev11_long](../build/includes/vs_dev11_long_md.md)]，您必須安裝[!INCLUDE[vs_dev11_long](../build/includes/vs_dev11_long_md.md)]加入平台工具組支援 Update 4[!INCLUDE[winxp](../build/includes/winxp_md.md)]和[!INCLUDE[WinXPSvr](../build/includes/winxpsvr_md.md)]。 若要下載並安裝一份[!INCLUDE[vs_dev11_long](../build/includes/vs_dev11_long_md.md)]更新 4，請參閱[Microsoft Visual Studio Express 2012 for Windows Desktop](http://go.microsoft.com/fwlink/p/?linkid=265464) Microsoft Download Center 中。 然後安裝[Visual Studio 2012 Update 4](http://go.microsoft.com/fwlink/p/?linkid=335900)以取得 v110_xp 平台工具組。 安裝後，即可使用 Windows Update 來接收最新的軟體更新。  
  
## <a name="windows-xp-targeting-experience"></a>以 Windows XP 為目標的體驗  
 隨附於 Visual Studio 中的 Windows XP 平台工具組是版本的[!INCLUDE[win7](../build/includes/win7_md.md)]SDK 隨附的[!INCLUDE[vs_dev10_long](../build/includes/vs_dev10_long_md.md)]，但是它會使用目前的 c + + 編譯器。 它也會將專案屬性設定為適當的預設值，例如設定下層目標的相容連結器規格。 使用 Windows XP 平台工具組所建立的桌面應用程式執行的 Windows[!INCLUDE[winxp](../build/includes/winxp_md.md)]和[!INCLUDE[WinXPSvr](../build/includes/winxpsvr_md.md)]，但這些應用程式也可在較新的 Windows 作業系統上執行。  
  
#### <a name="to-target-windows-xp"></a>以 Windows XP 為目標  
  
1.  在方案總管中，開啟專案的捷徑功能表，然後選擇 [屬性]。  
  
2.  在**屬性頁**對話方塊中的專案，在**組態屬性**，**一般**，將**平台工具組**所需的 Windows XP 工具組屬性。 例如，選擇**Visual Studio 2015-Windows XP (v140_xp)**建立程式碼[!INCLUDE[winxp](../build/includes/winxp_md.md)]和[!INCLUDE[WinXPSvr](../build/includes/winxpsvr_md.md)]所使用的 Microsoft Visual c + + 2015年編譯器。  
...
-   **C + + 語言功能**  
     ...
     中的 c + + 語言功能實作[!INCLUDE[vs_dev11_long](../build/includes/vs_dev11_long_md.md)]使用 v110_xp 平台工具組的應用程式中支援。 中的 c + + 語言功能實作[!INCLUDE[vs_dev12](../atl-mfc-shared/includes/vs_dev12_md.md)]使用 v120_xp 平台工具組的應用程式中支援。 建置使用舊版平台工具組時，visual Studio 會使用對應的編譯器。 若要充分利用該版本的編譯器中實作的其他 c + + 語言功能使用最新的 Windows XP 平台工具組。  
