...
在 MFC 應用程式中，請使用[AfxFreeLibrary](../mfc/reference/application-information-and-management.md#afxfreelibrary) ，而不是 `FreeLibrary` 來卸載 MFC 擴充 DLL。 `AfxFreeLibrary` 的介面（函數原型）與 `FreeLibrary`相同。
...
[在 Visual Studio\C++中建立 C/dll](dlls-in-visual-cpp.md)
