...
|`inheritEnvironments`|array|指定一組繼承自多個來源的環境變數。 您可以在檔案中定義一些變數，例如*CMakeSettings*或*CppProperties* ，並讓它們可供 debug 內容使用。  **Visual Studio 16.4：**：使用`env.VARIABLE_NAME`語法來指定每個目標的環境變數。 若要取消設定變數，請將它設為 "null"。|
...
|`quoteArgs`|boolean|如果個別引數包含字元（例如空格或索引標籤），是否應該加上引號？ 如果`false`為，偵錯工具命令就不會再自動加上引號。 預設值為 `true`。|
...
|`useForBreakpoints`|boolean|設定中斷點時，應該使用此來源對應。 如果`false`為，則只會使用 filename 和行號來設定中斷點。 如果`true`為，則只有在使用此來源對應時，才會使用檔案的完整路徑和行號來設定中斷點。 否則，只會在設定中斷點時使用檔案名和行號。 預設值為 `true`。|
