...
*image*<br/>
使用所建立的.exe 或.dll 檔案的完整路徑[/GENPROFILE](reference/genprofile-fastgenprofile-generate-profiling-instrumented-build.md)， [/FASTGENPROFILE](reference/genprofile-fastgenprofile-generate-profiling-instrumented-build.md)，或[/ltcg: pginstrument](reference/ltcg-link-time-code-generation.md)選項。
...
**Pgosweep**命令適用於使用所建置的程式[/GENPROFILE 或 /FASTGENPROFILE](reference/genprofile-fastgenprofile-generate-profiling-instrumented-build.md)選項，或已被取代[/ltcg: pginstrument](reference/ltcg-link-time-code-generation.md)選項。 它會中斷執行中的程式，並將設定檔資料寫入至新的.pgc 檔案。 根據預設，命令會重設計數之後每一個寫入作業。 如果您指定 **/noreset**選項時，命令將記錄的值，但不是在執行中的程式重設。 此選項可讓您重複的資料時，稍後擷取設定檔資料。
