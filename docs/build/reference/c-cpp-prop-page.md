...
description: Visual Studio Microsoft C/C++ Project 屬性頁屬性的參考指南。
ms.date: 02/09/2020
ms.topic: article
...
ms.locfileid: "77257750"
---
# <a name="cc-property-pages"></a>C/C++屬性頁

下列屬性頁位於 **專案** > **屬性** 下 > 設定**屬性** > **C/C++** ：
...
將編譯器警告視為錯誤。 針對新的專案，最好是在每次編譯中使用[/wx](wx-treat-linker-warnings-as-errors.md) 。 解決所有警告，將難以發現的程式碼缺失降至最低。
...
- **僅 __inline** -只展開標記為**inline**、`__inline`、`__forceinline`或 `__inline`的函式。 或者，在C++成員函式中，定義于類別宣告內。
- **任何適合**展開的函式，標示為**內嵌**或 `__inline`，以及編譯器所選擇的任何其他函數。 （擴充是以編譯器的判斷進行，通常稱為「*自動內嵌*」）。
...
預先處理 C C++和來源檔案，並將前置處理過的輸出寫入檔案。 此選項會隱藏編譯，而且不會產生 *`.obj`* 檔案。
...
編譯器只會在程式映射中建立一個相同字串的唯讀複本。 它會產生較小的程式，也就是稱為「*字串*共用」的優化。 [/O1、/O2](o-options-optimize-code.md)和[/zi](z7-zi-zi-debug-information-format.md)會自動設定[/gf](gf-eliminate-duplicate-strings.md)選項。

### <a name="enable-minimal-rebuild"></a>啟用最少重建

啟用最少重建，以決定是否要C++重新編譯包含已變更C++之類別定義的原始程式檔，並儲存在標頭 *`.h`* 檔案中。
...
啟用基本執行時間錯誤檢查，與 debug 以外的任何優化類型不相容。 設定[/RTCs、/RTCu、/rtc1 相同](rtc-run-time-error-checks.md)。
...
- **多執行緒的 Debug** -定義 _DEBUG 和 _MT。 此選項也會讓編譯器將程式庫名稱*libcmtd.lib*放入 *`.obj`* 檔案中，使連結器可以使用*libcmtd.lib*來解析外部符號。
- **多執行緒 dll** -讓您的應用程式使用多執行緒和 DLL 特定版本的執行時間程式庫。 定義 _MT 和 _DLL，並讓編譯器將程式庫名稱*msvcrt.lib*放入 *`.obj`* 檔案中。
- **多執行緒的 DEBUG DLL** -定義 _DEBUG、_MT 和 _DLL，並讓您的應用程式使用 Debug 多執行緒和 DLL 特定版本的執行時間程式庫。 它也會讓編譯器將程式庫名稱*msvcrtd.lib*放入 *`.obj`* 檔案中。
...
- **1 個**位元組套件的結構在1個位元組的界限上。 與 **`/Zp`** 相同。
...
允許編譯器在啟用優化時，為使用 `#pragma loop(hint_parallel[(n)])` 所識別的迴圈產生平行程式碼。

### <a name="enable-enhanced-instruction-set"></a>啟用增強的指令集

啟用在支援增強型指令集的處理器上找到的指示。 例如，SSE、SSE2、AVX 和 AVX2 對 IA-32 的增強功能。 而且，AVX 和 AVX2 對 x64 的增強功能。 目前只有在建立 x86 架構時，才可以使用 **`/arch:SSE`** 和 **`/arch:SSE2`** 。 如果未指定任何選項，編譯器會使用在支援 SSE2 的處理器上找到的指示。 您可以使用 **`/arch:IA32`** 來停用增強的指示。 如需詳細資訊，請參閱[/arch （x86）](arch-x86.md)、 [/arch （x64）](arch-x64.md)和[/arch （ARM）](arch-arm.md)。
...
- **Strict** -最嚴格的浮點模型。 **`/fp:strict`** 會導致 **`fp_contract`** 關閉並 **`fenv_access`** 為開啟。 **`/fp:except`** 是隱含的，而且可以藉由明確指定 **`/fp:except-`** 來停用。 與 **`/fp:except-`** 搭配使用時， **`/fp:strict`** 會強制執行嚴格的浮點語義，但不考慮例外事件。
...
當指定時，類型**wchar_t**會變成對應至 `__wchar_t` 的原生類型，這種方式會與**short**對應 `__int16`的相同。 [/Zc： wchar_t](zc-wchar-t-wchar-t-is-native-type.md)預設為開啟。
...
決定編譯器C++所啟用的語言標準。 盡可能使用最新版本。 設定[/std： c + + 14、/std： c + + 17、/std： c + + 最新版本](std-specify-language-standard-version.md)。
...
指定所產生之 XML 檔檔案的名稱;可以是檔案或目錄名稱。 設定[/doc：\<名稱 >](doc-process-documentation-comments-c-cpp.md)。

## <a name="cc-browse-information-properties"></a>C/C++流覽資訊屬性

### <a name="enable-browse-information"></a>啟用流覽資訊

指定 *`.bsc`* 檔案中的流覽資訊層級。 設定[/fr](fr-fr-create-dot-sbr-file.md)。

### <a name="browse-information-file"></a>流覽資訊檔

指定瀏覽器資訊檔的選擇性名稱。 設定[/fr\<名稱 >](fr-fr-create-dot-sbr-file.md)。
...
- **__cdecl** -指定所有函式的 __cdecl 呼叫慣例， C++但成員函式和函式標記為 __stdcall 或 __fastcall 以外的函數。
- **__fastcall** -指定所有函式的 __fastcall 呼叫慣例， C++但成員函式和函式標記為 __cdecl 或 __stdcall 以外的函數。 所有 __fastcall 函數都必須具有原型。
- **__stdcall** -指定所有函式的 __stdcall 呼叫慣例， C++但成員函式和函式標記為 __cdecl 或 __fastcall 以外的函數。 所有 __stdcall 函數都必須具有原型。
- **__vectorcall** -指定所有函式的 __vectorcall 呼叫慣例， C++但成員函式和函式（標記為 __cdecl、__fastcall 或 __stdcall 除外）。 所有 __vectorcall 函數都必須具有原型。

### <a name="compile-as"></a>編譯為

針對 *`.c`* 和 *`.cpp`* 檔案選取 [編譯語言選項]。 設定[/tc、/tp](tc-tp-tc-tp-specify-source-file-type.md)。
...
停用指定的警告編號。 將警告號碼放在以分號分隔的清單中。 設定[/wd\<num >](compiler-option-warning-level.md)。

### <a name="forced-include-file"></a>強制 Include 檔案

一或多個強制 Include 檔案。 將[/fi\<名稱 >](fi-name-forced-include-file.md)。
...
不包含 *`.obj`* 檔案中的預設程式庫名稱。 設定[/zl](zl-omit-default-library-name.md)。
