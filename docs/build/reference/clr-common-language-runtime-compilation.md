...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "cl.exe 編譯器, Common Language Runtime 選項"
  - "-clr 編譯器選項 [C++]"
  - "clr 編譯器選項 [C++]"
  - "/clr 編譯器選項 [C++]"
  - "Managed Extensions for C++, 編譯"
 ...
 **\/clr:safe**  
 產生純 MSIL \(非原生可執行程式碼\) 且可驗證的輸出檔。**\/clr:safe** 允許驗證診斷 \([PEVerify Tool \(Peverify.exe\)](../Topic/Peverify.exe%20\(PEVerify%20Tool\).md)\)。  
  
 如需詳細資訊，請參閱 [NIB：撰寫可驗證的類型安全程式碼](http://msdn.microsoft.com/zh-tw/d18f10ef-3b48-4f47-8726-96714021547b)。  
 ...
 **noAssembly** 選項已被取代。 請改用 [\/LN \(建立 MSIL 模組\)](../../build/reference/ln-create-msil-module.md)。  
...
## 備註  
 Managed 程式碼是可由 CLR 檢查及管理的程式碼。 Managed 程式碼可以存取 Managed 物件。 如需詳細資訊，請參閱[\/clr 限制](../../build/reference/clr-restrictions.md)。  
 ...
 若要在 Managed 應用程式上啟用偵錯，請參閱 [\/ASSEMBLYDEBUG \(加入 DebuggableAttribute\)](../../build/reference/assemblydebug-add-debuggableattribute.md)。  
  
 只有 CLR 類型會在記憶體回收堆積上具現化。 如需詳細資訊，請參閱[Classes and Structs](../../windows/classes-and-structs-cpp-component-extensions.md)。 若要將函式編譯為原生程式碼，請使用 `unmanaged` pragma。 如需詳細資訊，請參閱[managed、unmanaged](../../preprocessor/managed-unmanaged.md)。  
 ...
 **\/clr** 意指要使用 **\/EHa**，且沒有其他 **\/EH** 選項受 **\/clr** 支援。 如需詳細資訊，請參閱[\/EH \(例外狀況處理模型\)](../../build/reference/eh-exception-handling-model.md)。  
 ...
 使用 **\/clr** 時，`_MANAGED` 符號會定義為 1。 如需詳細資訊，請參閱[預先定義的巨集](../../preprocessor/predefined-macros.md)。  
  
 原生物件檔中的全域變數會先初始化 \(如果可執行檔是 DLL，會在 DllMain 期間執行\)，然後初始化 Managed 區段中的全域變數 \(在任何 Managed 程式碼執行之前\)。`#pragma`[init\_seg](../../preprocessor/init-seg.md) 只會影響 Managed 與 Unmanaged 類別中的初始設定順序。  
  
 使用 **\/clr:safe** 進行編譯，類似於以 C\# 之類的語言使用 [\/platform:anycpu](../Topic/-platform%20\(C%23%20Compiler%20Options\).md) 進行編譯。  
...
## 安全和純映像  
 ...
 純映像或安全映像的每個進入點都會受到管理。 當您使用 **\/clr** 進行編譯時，進入點是原生的。 如需詳細資訊，請參閱[\_\_clrcall](../../cpp/clrcall.md)。  
 ...
 **cl \/clr:safe t.cpp \/link \/clrimagetype:pure \/entry:?main@@$$HYMHXZ \/subsystem:console**  
 ...
 如需如何取得裝飾名稱的相關資訊，請參閱[裝飾名稱](../../build/reference/decorated-names.md)。 如需 64 位元程式設計的相關資訊，請參閱[為 64 位元設定程式](../../build/configuring-programs-for-64-bit-visual-cpp.md)。 如需使用純 CLR 程式碼的相關資訊，請參閱 [如何：移轉至 \/clr:pure](../../dotnet/how-to-migrate-to-clr-pure-cpp-cli.md) 與 [純粹的和可驗證的程式碼](../../dotnet/pure-and-verifiable-code-cpp-cli.md)。  
...
## Managed Extensions for C\+\+  
 ...
 如果您有使用 Managed Extensions for C\+\+ 的程式碼，建議您將其移植以使用 C\+\+\/CLI 語法。 如需如何移植程式碼的相關資訊，請參閱 [C\+\+\/CLI 移轉入門](../../dotnet/cpp-cli-migration-primer.md)。  
...
3.  在 \[**一般**\] 屬性頁面上，修改 \[**Common Language Runtime 支援**\] 屬性。  
    ...
    >   
    >  此外，當您偵錯 **\/clr** 應用程式時，請將 \[**偵錯工具類型**\] 屬性設為 \[**混合**\] 或 \[**僅限 Managed**\]。 如需詳細資訊，請參閱[C\+\+ 偵錯組態的專案設定](../Topic/Project%20Settings%20for%20a%20C++%20Debug%20Configuration.md)。  
  
     如需如何建立模組的相關資訊，請參閱 [\/NOASSEMBLY \(建立 MSIL 模組\)](../../build/reference/noassembly-create-a-msil-module.md)。  
