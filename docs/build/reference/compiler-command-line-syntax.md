...
|進入|意義|
|-----------|-------------|
|*option*|一或多個[CL 選項](../../build/reference/compiler-options.md)。 請注意，所有選項都會都套用至所有指定的原始程式檔。 選項會指定正斜線 （/） 或破折號 （-）。 如果選項需要引數，此選項的描述文件，是否允許選項和引數之間的空格。 （除了 /HELP 選項） 的選項名稱會區分大小寫。 請參閱[CL 選項的順序](../../build/reference/order-of-cl-options.md)如需詳細資訊。|
...
|*command-file*|包含多個選項和檔名的檔案。 請參閱[CL 命令檔](../../build/reference/cl-command-files.md)如需詳細資訊。|
|*link-opt*|一或多個[連結器選項](../../build/reference/linker-options.md)。 CL 會將這些選項給連結器。|
