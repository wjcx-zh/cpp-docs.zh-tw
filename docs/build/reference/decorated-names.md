...
ms.locfileid: "91504244"
---
# <a name="decorated-names"></a>裝飾名稱

C 和 C++ 程式中的函式、資料和物件在內部以其裝飾名稱表示。 *裝飾名稱*是編譯器在物件、資料或函式定義的編譯期間所建立的編碼字串。 它會記錄呼叫慣例、類型、函式參數和其他資訊，並連同名稱一起記錄。 這個名稱裝飾（也稱為 *名稱重整*）可協助連結器在連結可執行檔時尋找正確的函式和物件。

裝飾的命名慣例在不同版本的 Visual Studio 中已經變更，而且在不同的目標架構上也可能不同。 若要正確地連結使用 Visual Studio 所建立的原始程式檔，您應該使用相同的編譯器工具組、旗標和目標架構來編譯 C 和 c + + Dll 和程式庫。
...
C 函式的裝飾形式取決於它的宣告中所使用的呼叫慣例，如下表所示。 這也是 C++ 程式碼宣告為有 `extern "C"` 連結時所使用的裝飾格式。 預設呼叫慣例為 **`__cdecl`** 。 請注意，在 64 位元環境中，函式未裝飾。

|呼叫慣例|裝飾|
|------------------------|----------------|
|**`__cdecl`**|前置底線 (**`_`**) |
|**`__stdcall`**|前置底線 (**`_`**) 和尾端的正負號 (**`@`**) 後面接著參數清單中的位元組數目（以小數表示）|
|**`__fastcall`**|開頭和尾端的符號 (**`@`**) 後面接著十進位數，代表參數清單中的位元組數目|
|**`__vectorcall`**|兩個尾端的正負號 (**`@@`**) 後面接著參數清單中的十進位數位節|
...
1. 編譯包含資料、物件或函式定義或原型的原始程式檔，並將 [ [列出檔案類型](fa-fa-listing-file.md) ] 編譯器選項設定為 [具有原始程式碼的元件] (**/FAs**) ，以產生清單。
   ...
   例如， `cl /c /FAs example.cpp` 在開發人員命令提示字元中輸入，以產生清單檔，例如 .asm。
...
2. 若要尋找裝飾形式的符號，請尋找括號中的未裝飾名稱。 裝飾名稱是在管線 ( # A0) 字元和未裝飾名稱稱之前的同一行。

## <a name="viewing-undecorated-names"></a><a name="Undecorated"></a> 查看未修飾的名稱
...
[其他 MSVC Build Tools](c-cpp-build-tools.md)<br/>
