...
|--------------------|----------|--------------|
|.asmx|來源|部署檔案。|
|.asp|來源|Active Server Page 檔。|
|.atp|隨附此逐步解說的專案|應用程式範本專案檔。|
|.bmp、.dib、.gif、.jpg、.jpe、.png|資源|一般影像檔。|
|.bsc|編譯|瀏覽器程式碼檔。|
|.cpp、。c|來源|您的應用程式的主要原始程式碼檔。|
|.cur|資源|資料指標點陣圖形檔。|
|.dbp|隨附此逐步解說的專案|資料庫專案檔。|
|.disco|來源|動態探索文件檔。 處理 XML Web 服務探索。|
|.exe、.dll|隨附此逐步解說的專案|可執行檔或動態連結程式庫檔。|
|.h|來源|標頭 (Include) 檔。|
|.htm、.html、.xsp、.asp、.htc、.hta、.xml|資源|一般 Web 檔案。|
|.HxC|隨附此逐步解說的專案|說明專案檔。|
|.ico|資源|圖示點陣圖形檔。|
...
|.mfcribbon-ms|資源|資源檔，其中包含定義功能區中 MFC 按鈕、控制項和屬性的 XML 程式碼。 如需詳細資訊，請參閱 [Ribbon Designer](../../mfc/ribbon-designer-mfc.md)。|
|.obj、.o||目的檔，已編譯但尚未連結。|
|.pch|偵錯|先行編譯標頭檔。|
|.rc、.rc2|資源|用以產生資源的[資源指令碼檔](../../windows/working-with-resource-files.md) 。|
|.sbr|編譯|原始程式瀏覽器中繼檔案。 [BSCMAKE](bscmake-options.md)的輸入檔。|
|.sln|解決方法|[「方案」](/visualstudio/ide/solutions-and-projects-in-visual-studio)檔。|
|.suo|解決方法|方案選項檔。|
|.txt|資源|文字檔，通常是「讀我」檔案。|
|.vap|隨附此逐步解說的專案|Visual Studio Analyzer 專案檔。|
|.vbg|解決方法|相容專案群組檔。|
|.vbp、.vip、.vbproj|隨附此逐步解說的專案|Visual Basic 專案檔。|
|.vcxitems|隨附此逐步解說的專案|用於在多個 C++ 專案之間共用程式碼檔的共用項目專案。 如需詳細資訊，請參閱[專案和方案](project-and-solution-files.md)檔。|
|.vcxproj|隨附此逐步解說的專案|Visual Studio 專案檔案。 如需詳細資訊，請參閱[專案和方案](project-and-solution-files.md)檔。|
|.vcxproj.filters|隨附此逐步解說的專案|當您使用方案總管將檔案新增至專案時使用。 篩選檔案會根據檔案的副檔名，定義要在方案總管樹狀檢視中新增檔案的位置。|
|.vdproj|隨附此逐步解說的專案|Visual Studio 部署專案檔。|
|.vmx|隨附此逐步解說的專案|巨集專案檔。|
|.vup|隨附此逐步解說的專案|公用程式專案檔。|
