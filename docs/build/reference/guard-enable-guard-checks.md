...
藉由編譯原始程式碼時[/guard: cf](../../build/reference/guard-enable-control-flow-guard.md)選項，編譯器就會會控制流程分析藉由檢查所有可能目標位址的間接呼叫。 編譯器會插入程式碼，在執行階段確認間接呼叫指令的目標位址位於已知目標位址的清單中。 支援 CFG 的作業系統會停止無法通過 CFG 執行階段檢查的程式。 這使得攻擊者更難使用資料損毀變更呼叫目標來執行惡意程式碼。
