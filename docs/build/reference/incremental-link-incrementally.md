...
## <a name="remarks"></a>備註  
 ...
 以累加方式連結的程式在功能上等於非累加方式連結的程式。 不過，因為它已準備用於後續的累加連結，以累加方式連結的可執行檔、 靜態程式庫或動態連結程式庫檔案：  
  
-   大於非累加方式連結的程式因為程式碼和資料的填補。 填補可以讓連結器增加函式和資料的大小，就必須重新建立檔案。  
...
-   可能包含跳躍點 (Jump) Thunk，用以將函式重新配置到新的位址。  
 ...
 若要在不考慮預設值的情況下以累加方式連結，請指定 /INCREMENTAL。 選取此選項時，連結器就無法以累加的方式，連結會發出警告，然後非累加方式連結程式。 有些選項和情況會覆寫 /INCREMENTAL。  
...
-   選取 /ORDER  
 ...
 / 增量當隱含[偵錯](../../build/reference/debug-generate-debug-info.md)指定。  
