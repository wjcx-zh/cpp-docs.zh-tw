...
您可以也將加密資訊傳遞給使用連結器[/KEYCONTAINER](keycontainer-specify-a-key-container-to-sign-an-assembly.md)。 使用[/DELAYSIGN](delaysign-partially-sign-an-assembly.md)要部分簽署組件。 請參閱[強式名稱組件 （組件簽署） (C++/CLI)](../../dotnet/strong-name-assemblies-assembly-signing-cpp-cli.md)如需有關簽署組件。
