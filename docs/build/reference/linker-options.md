...
- [保留文字](reserved-words.md)

在命令列上，連結器選項不區分大小寫;例如，`/base` 和 `/BASE` 表示相同的東西。 如需有關如何命令列或在 Visual Studio 中指定每個選項的詳細資訊，請參閱該選項的文件。
...
|[/ALLOWBIND](allowbind-prevent-dll-binding.md)|指定無法系結 DLL。|
...
|[/DYNAMICBASE](dynamicbase-use-address-space-layout-randomization.md)|指定是否要使用位址空間配置隨機載入（ASLR）功能，在載入時產生重定基底的可執行映射。|
|[/ENTRY](entry-entry-point-symbol.md)|設定開始位址。|
|[/ERRORREPORT](errorreport-report-internal-linker-errors.md)| 已取代。 錯誤報表是由[Windows 錯誤報告（WER）](/windows/win32/wer/windows-error-reporting)設定所控制。 |
...
|[/GENPROFILE、/FASTGENPROFILE](genprofile-fastgenprofile-generate-profiling-instrumented-build.md)|這兩個選項都指定由連結器產生 *`.pgd`* 檔案，以支援特性指引優化（PGO）。 /GENPROFILE 和 /FASTGENPROFILE 使用不同的預設參數。|
...
|[/IGNOREIDL](ignoreidl-don-t-process-attributes-into-midl.md)|防止將屬性資訊處理成 *`.idl`* 檔案。|
...
|[/NATVIS](natvis-add-natvis-to-pdb.md)|將 Natvis 檔案中的偵錯工具視覺化程式新增至程式資料庫（PDB）。|
...
|[/PGD](pgd-specify-database-for-profile-guided-optimizations.md)|指定特性指引優化的 *`.pgd`* 檔案。|
...
|[/RELEASE](release-set-the-checksum.md)|在 *`.exe`* 標頭中設定總和檢查碼。|
...
|[/SUBSYSTEM](subsystem-specify-subsystem.md)|告訴作業系統如何執行 *`.exe`* 檔案。|
|[/SWAPRUN](swaprun-load-linker-output-to-swap-file.md)|指示作業系統在執行之前，將連結器輸出複製到交換檔。|
