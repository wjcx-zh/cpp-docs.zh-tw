...
這個屬性會告知連結器不要將這個組建所產生的任何 .lib 輸出連結到任何相依專案。 它可讓專案系統處理不會在建立時產生 .lib 檔案的 .dll 檔案。 如果專案相依於另一個會產生 DLL 的專案，則專案系統會自動連結該個子專案所產生的 .lib 檔。 這個屬性在產生 COM Dll 或僅限資源 Dll 的專案中可能是不必要的，因為這些 Dll 沒有任何有意義的匯出。 如果 DLL 沒有任何匯出，連結器不會產生 .lib 檔案。 如果沒有匯出 .lib 檔案，而且專案系統告訴連結器連結遺失的 DLL，連結就會失敗。 請使用 [忽略匯入程式庫] 屬性來解決此問題。 當設定為 **[是]** 時，專案系統會忽略 .lib 檔案的存在與否，並使相依于此專案的任何專案不會與不存在的 .lib 檔案連結。
...
Visual Studio 中的註冊傳統上都在 HKEY_CLASSES_ROOT (HKCR) 進行。 使用 Windows Vista 和更新版本的作業系統，若要存取 HKCR 您必須以提升權限模式執行 Visual Studio。 開發人員不一定要在提高許可權的模式下執行，但仍然必須使用註冊。 個別使用者重新導向可讓您註冊，而不需要在提高許可權模式下執行。
...
指定是否要連結由相依專案所產生的 .lib 檔。 一般來說，您會想要連結 .lib 檔案，但是某些 Dll 可能不會發生這種情況。

您也可以提供檔案名稱和相對路徑以指定 .obj 檔案，例如 "..\\..\MyLibProject\MyObjFile.obj"。 如果 .obj 檔案的原始程式碼 #includes 先行編譯的標頭（例如，pch），則 pch 檔案位於與 MyObjFile 相同的資料夾中。您也必須將 pch 新增為額外的相依性。

### <a name="use-library-dependency-inputs"></a>使用程式庫相依性輸入

指定連結專案相依性的程式庫輸出時，是否要使用 [管理員] 工具的輸入，而不是文件庫檔案本身。 在大型專案中，當相依專案產生 .lib 檔時，會停用累加連結。 如果有許多相依專案產生 .lib 檔，建置應用程式可能會花很長的時間。 當這個屬性設定為 **[是]** 時，專案系統會連結相依專案所產生之 .lib 檔案中的 .obj 檔案，以啟用累加式連結。
...
[/ALLOWBIND](allowbind-prevent-dll-binding.md)： [否] 會在 DLL 標頭中設定一個位，指出不允許系結影像的 Bind。 如果 DLL 已數位簽署，您便可能不想要繫結 DLL (繫結會使簽章無效)。
...
指定要忽略的一或多個預設程式庫名稱。 以分號分隔多個程式庫。 （/NODEFAULTLIB： [名稱，名稱，...]）
...
[/SWAPRUN](swaprun-load-linker-output-to-swap-file.md)選項會告訴作業系統先將連結器輸出複製到交換檔，然後從該處執行映射。 此選項是 Windows NT 4.0 （和更新版本）的功能。 當指定**CD**時，作業系統會將卸載式磁片上的映射複製到分頁檔，然後將它載入。
