...
使用 **/LTCG**執行整個程式最佳化或以建立特性指引最佳化 (PGO) 檢測、 執行訓練，並建立特性指引最佳化組建。

## <a name="syntax"></a>語法

> **/LTCG**[**:**{**INCREMENTAL**|**NOSTATUS**|**狀態**|**OFF**}]<br/>

已被取代啟動 Visual Studio 2015 中的這些選項：
...
**累加**（選擇性）<br/>
指定的連結器僅適用於整個程式最佳化或連結時間程式碼產生 (LTCG) 受影響的編輯，而不是整個專案的檔案集。 根據預設，此旗標時不會設定 **/LTCG**指定，並使用整個程式最佳化連結整個專案。

**NOSTATUS** &#124; **狀態**（選擇性）<br/>
指定連結器是否會顯示進度指示器，以顯示連結完成的百分比。 根據預設，不顯示這項狀態資訊。

**關閉**（選擇性）<br/>
停用連結時產生程式碼。 此行為相當於何時 **/LTCG**未在命令列上指定。

**PGINSTRUMENT** （選擇性）<br/>
此選項啟動 Visual Studio 2015 中已被取代。 請改用 **/LTCG**和[/GENPROFILE 或 /FASTGENPROFILE](genprofile-fastgenprofile-generate-profiling-instrumented-build.md)產生經過檢的組建進行特性指引最佳化。 從檢測測試回合所收集的資料用來建立最佳化的映像。 如需詳細資訊，請參閱[特性指引最佳化](profile-guided-optimizations.md)。 這個選項的簡短形式是 **/ltcg: pgi**。

**PGOPTIMIZE** （選擇性）<br/>
此選項啟動 Visual Studio 2015 中已被取代。 請改用 **/LTCG**和[/USEPROFILE](useprofile.md)建置最佳化的映像。 如需詳細資訊，請參閱[特性指引最佳化](../../build/reference/profile-guided-optimizations.md)。 這個選項的簡短形式是 **/ltcg: pgo 進行**。

**PGOPTIMIZE** （選擇性）<br/>
此選項啟動 Visual Studio 2015 中已被取代。 請改用 **/LTCG**和 **/USEPROFILE**重建最佳化的映像。 如需詳細資訊，請參閱[特性指引最佳化](../../build/reference/profile-guided-optimizations.md)。 這個選項的簡短形式是 **/LTCG:PGU**。

## <a name="remarks"></a>備註

**/LTCG**選項會告訴連結器呼叫編譯器並且執行整個程式最佳化。 您也可以執行特性指引最佳化。 如需詳細資訊，請參閱[特性指引最佳化](../../build/reference/profile-guided-optimizations.md)。

有下列例外狀況中，您無法加入連結器選項的 PGO 組合 **/LTCG**和 **/USEPROFILE**未指定先前 PGO 初始化組合 **/LTCG**和 **/GENPROFILE**選項：
...
指定搭配任何連結器選項 **/LTCG**和 **/GENPROFILE**選項，以初始化 PGO 不需要指定當您建立使用 **/LTCG**和 **/USEPROFILE**; 它們隱含。

本文章的其餘部分將討論 **/LTCG**方面連結時產生程式碼。

**/LTCG**隱含與[/GL](../../build/reference/gl-whole-program-optimization.md)。
...
> [!NOTE]
> 如果您使用 **/LTCG**重新定義`mainCRTStartup`，您的應用程式可以有無法預期的行為與全域物件初始化以前執行之使用者程式碼相關。 若要解決此問題的三種方式： 不要重新定義`mainCRTStartup`，不會編譯包含的檔案`mainCRTStartup`使用 **/LTCG**，或以靜態方式初始化全域變數和物件。
...
您也可以套用 **/LTCG**至特定組建，藉由選擇**建置** > **特性指引最佳化**功能表列上，或選擇其中一個設定檔在專案的捷徑功能表上的引導式的最佳化選項。
