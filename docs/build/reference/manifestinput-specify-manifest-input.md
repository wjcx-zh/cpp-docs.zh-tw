...
無法直接在 Visual Studio 中設定這個選項。 請改用**額外的資訊清單檔案**的專案，以指定要包含的其他資訊清單檔案的屬性。 如需詳細資訊，請參閱 <<c0> [ 輸入和輸出、 資訊清單工具、 組態屬性\<專案名稱 > 屬性頁對話方塊](input-and-output-manifest-tool.md)。
