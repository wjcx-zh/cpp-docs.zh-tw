...
|選項|描述|  
|------------|-----------------|  
|**/MD**|讓應用程式使用多執行緒特定與 DLL 特定版本的執行階段程式庫。 定義 `_MT` 和 `_DLL`，並讓編譯器將程式庫名稱 MSVCRT.lib 放入 .obj 檔。<br /><br /> 使用這個選項編譯的應用程式，都以靜態方式連結至 MSVCRT.lib。 此程式庫提供可讓連結器解析外部參考的程式碼層。 實際運作的程式碼包含在 MSVCR*versionnumber*。必須有在執行階段應用程式的 DLL 連結到 MSVCRT.lib。|  
...
|**/LDd**|建立偵錯 DLL。 定義 `_MT` 和 `_DEBUG`。|  
 ...
 傳遞至指定的引動過程連結器的所有模組必須有使用相同的執行階段程式庫編譯器選項都編譯 (**/MD**， **/MT**， **/LD**)。  
 ...
 如需詳細資訊的 Dll，請參閱[Visual c + + 中的 Dll](../../build/dlls-in-visual-cpp.md)。  
