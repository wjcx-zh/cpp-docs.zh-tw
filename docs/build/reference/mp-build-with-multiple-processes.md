...
###  <a name="effective_processors"></a> Effective Processors  
 ...
 如果您在 `processMax` /MP **選項中略過** 引數，則編譯器會從作業系統中取得有效處理器的數目，然後為每個有效的處理器建立一個處理序。 不過，編譯器無法保證哪個處理序會在特定處理器上執行，因為這是由作業系統決定。  
  
### <a name="number-of-processes"></a>處理序數目  
 編譯器會計算要用來編譯原始程式檔的處理序數目。 您在命令列上所指定的原始程式檔數目，以及使用 **/MP** 選項明確或隱含指定的處理序數目，兩者中較小的那個數目就是處理序的數目值。 如果您提供 `processMax` /MP **選項的** 引數，就可以明確設定處理序數目的最大值。 或者，如果您略過 `processMax` 引數，則可以使用預設值，也就是電腦中有效處理器的數目。  
...
#### <a name="the-msbuildexe-tool"></a>MSBUILD.exe 工具  
 [!INCLUDE[vsprvs](../../assembler/masm/includes/vsprvs_md.md)] 會使用 [MSBuild.exe](/visualstudio/msbuild/msbuild-reference) 工具來建置方案和專案。 MSBuild.exe 工具的 **/maxcpucount:**`number` (或 **/m:**`number`) 命令列選項可以同時建置多個專案。 而 **/MP** 編譯器選項可同時建置多個編譯單位。 如果應用程式適用的話，請使用 **/MP** 及/或 **/maxcpucount**來改善您的方案建置時間。  
  
 您的方案建置時間有一部分取決於執行組建的處理序數目。 `number` /maxcpucount [MSBuild 選項的](/visualstudio/msbuild/msbuild-command-line-reference) 引數會指定要同時建置的專案數目上限。 同樣地， `processMax` /MP **編譯器選項的** 引數會指定要同時建置的編譯單位數目上限。 如果 **/maxcpucount** 選項指定 *P* 專案，而 **/MP** 選項指定 *C* 處理序，則會同時執行最多 *P*x*C* 個處理序。  
