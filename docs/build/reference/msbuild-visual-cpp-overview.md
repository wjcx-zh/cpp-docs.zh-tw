...
ms.locfileid: "88610840"
---
# <a name="msbuild-internals-for-c-projects"></a>C++ 專案的 MSBuild 內部項目

當您在 IDE 中設定專案屬性，然後儲存專案時，Visual Studio 會將專案設定寫入至您的專案檔。 專案檔包含專案的唯一設定。 但是，它不包含建立專案所需的所有設定。 專案檔含有 `Import` 元素，其中包含其他*支援檔案*的網路。 支援檔案包含建立專案所需的其餘屬性、目標和設定。
...
根據預設，主要的 Visual Studio 支援檔案會位於下列目錄中。 這項資訊是版本特定的資訊。

### <a name="visual-studio-2019"></a>Visual Studio 2019

- % VSINSTALLDIR% MSBuild \\ Microsoft \\ VC \\ *版本*\\

  包含目標所使用的主要目標檔案 (.targets) 和屬性檔案 (.props)。 根據預設，$(VCTargetsPath) 巨集會參考這個目錄。 *版本*預留位置參考 Visual Studio 版本：適用于 Visual Studio 2019 的 v160，Visual Studio 2017 的 v150。

- % VSINSTALLDIR% MSBuild \\ Microsoft \\ VC \\ *版本* \\ 平臺 \\ *平臺*\\

  包含平台特定的目標和屬性檔案，會覆寫其上層目錄中的目標和屬性。 此目錄也包含一個 DLL，會定義此目錄中的目標所使用的工作。 *platform* 預留位置代表 ARM、Win32 或 x64 子目錄。

- % VSINSTALLDIR% MSBuild \\ Microsoft \\ VC \\ *版本* \\ 平臺 \\ *平臺* \\ PlatformToolsets \\ *工具*組\\
...
- *磁片磁碟機*： \\ Program Files * (X86) * \\ MSBuild \\ Microsoft .cpp \\ \\ *version* \\ \\ *platform platform 版平臺*\\

  包含平台特定的目標和屬性檔案，會覆寫其上層目錄中的目標和屬性。 此目錄也包含一個 DLL，會定義此目錄中的目標所使用的工作。 *platform* 預留位置代表 ARM、Win32 或 x64 子目錄。

- *磁片磁碟機*： \\ Program Files * (X86) * \\ MSBuild \\ Microsoft .cpp \\ \\ *version* \\ platform 版 \\ *平臺* \\ PlatformToolsets \\ *工具*組\\

  包含可讓組建使用指定的 *toolset* 產生 C++ 應用程式的目錄。 *版本*預留位置是 V110，適用于 Visual Studio 2012、V120 （適用于 Visual Studio 2013）和 V140 （Visual Studio 2015）。 *platform* 預留位置代表 ARM、Win32 或 x64 子目錄。 *工具*組預留位置代表工具組子目錄。 例如，它是使用 Visual Studio 2015 工具組建立 Windows 應用程式的 v140。 或者，v120_xp 使用 Visual Studio 2013 工具組來建立 Windows XP。

- *磁片磁碟機*： \\ Program Files * (X86) * \\ MSBuild \\ Microsoft .cpp \\ v 4.0 \\ 平臺 \\ *平臺* \\ PlatformToolsets \\ *工具*組\\
...
| .props | 包含 `Property Group` 和使用者定義的 `Property` XML 元素，會指定在建置期間使用的檔案和參數設定。<br /><br /> 也可能包含指定其他設定的 `ItemDefinitionGroup` 和使用者定義 `Item` XML 元素。 專案定義群組中定義的專案與屬性類似，但無法從命令列存取。 Visual Studio 專案檔通常會使用項目來代表設定，而不是使用屬性。<br /><br /> 如需詳細資訊，請參閱 [ (msbuild) 的 ItemGroup 元素 ](/visualstudio/msbuild/itemgroup-element-msbuild)、 [ItemDefinitionGroup 元素 (Msbuild) ](/visualstudio/msbuild/itemdefinitiongroup-element-msbuild)和 [專案專案 (msbuild) ](/visualstudio/msbuild/item-element-msbuild)。 |
| .xml | 包含宣告和初始化 IDE 使用者介面元素的 XML 元素。 例如，屬性工作表、屬性頁、textbox 控制項和 listbox 控制項。<br /><br /> .xml 檔案可直接支援 IDE，而非 MSBuild。 不過，IDE 屬性的值會指派給組建屬性和項目。<br /><br /> 大部分的 .xml 檔案會位於地區設定特定子目錄中。 例如，英文（美國）區域的檔案為 $ (VCTargetsPath) \\ 1033 \\ 。 |

## <a name="user-targets-and-properties"></a>使用者目標和屬性

若要有效地使用 MSBuild，它有助於知道哪些屬性和目標是有用且相關的。 大部分的屬性和目標都有助於實行 Visual Studio 組建系統，因此與使用者無關。 本節說明使用者導向的屬性和目標，值得您瞭解。

### <a name="platformtoolset-property"></a>PlatformToolset 屬性

`PlatformToolset` 屬性會決定要在組建中使用的 MSVC 工具組。 依預設會使用目前的工具組。 當設定此屬性時，其值會與常值字串串連以形成路徑。 它是包含為特定平臺建立專案所需之屬性和目標檔案的目錄。 必須安裝平台工具組，才能使用該平台工具組版本來建置。
...
`PreferredToolArchitecture` 屬性會決定要在組建中使用 32 位元還是 64 位元編譯器和工具。 這個屬性不會影響輸出平臺的架構或設定。 根據預設，如果未設定此屬性，MSBuild 會使用 x86 版本的編譯器和工具。
...
根據預設，目前專案的平台特定設定會覆寫 PATH、INCLUDE、LIB、LIBPATH、CONFIGURATION 和 PLATFORM 等環境變數。 將 `UseEnv` 屬性設為， **`true`** 以保證不會覆寫環境變數。

`msbuild myProject.vcxproj /p:UseEnv=true`

### <a name="targets"></a>目標

Visual Studio 支援檔案中有數百個目標。 不過，大部分都是使用者可忽略的系統導向目標。 大部分的系統目標的前置詞為底線 (`_`) ，或名稱開頭為 "PrepareFor"、"Compute"、"Before"、"After"、"Pre" 或 "Post"。
...
[Xdcmake.exe 工作](/visualstudio/msbuild/xdcmake-task)
