 ...
 **/Opt: ref**消除函式和資料從未參考。**/Opt: noref 則**函式和永不參考的資料會保留。  
 ...
 啟用 /OFT:REF 時，LINK 會移除未參考的封裝函式和資料。 物件包含的封裝函式和資料 (Comdat) 如果編譯使用[/Gy](../../build/reference/gy-enable-function-level-linking.md)選項。 這種最佳化稱為可轉移 COMDAT 刪除。 根據預設， **/opt: ref**非偵錯組建中已啟用。 若要覆寫此預設值，而未參考的 Comdat 保留在程式中，指定**/opt: noref 則**。 您可以使用[/包含](../../build/reference/include-force-symbol-references.md)選項來覆寫對特定符號的移除。  
