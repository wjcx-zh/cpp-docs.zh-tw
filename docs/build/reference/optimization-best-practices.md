...
Visual c + + 支援*特性指引最佳化*(PGO)。 此最佳化 」 可使用的版本已經過檢測的應用程式的訓練執行的分析資料來更新應用程式最佳化的磁碟機。 使用 PGO 非常耗時，因此，可能不會有每位開發人員使用，但我們建議使用 PGO 最終發行版本的產品。 如需詳細資訊，請參閱[特性指引最佳化](../../build/reference/profile-guided-optimizations.md)。

此外，*整個程式最佳化*（也稱為連結時間產生程式碼） 和 **/O1**和 **/O2**經過改進最佳化。 一般情況下，使用其中一個選項編譯的應用程式將會較快，使用舊版的編譯器編譯的相同應用程式。 
...
**/Gy**參數也是非常有用。 它會產生個別 COMDAT，每個函數提供連結器更大的彈性時移除未參考的 Comdat 和 COMDAT 摺疊。 使用的唯一缺點 **/Gy**是偵錯時，它可能造成問題。 因此，通常建議使用它。 如需詳細資訊，請參閱 [/Gy (啟用函式階層連結)](../../build/reference/gy-enable-function-level-linking.md)。
...
內建函式是函式呼叫編譯器有內建函式呼叫的相關知識，而不是文件庫中呼叫函式，它會發出程式碼，該函式。 標頭檔\<intrin.h > 包含所有可用的內建函式的每個支援的硬體平台。

內建函式可讓程式設計人員進入深度程式碼而不必使用組件。 有使用內建的幾個好處：

- 程式碼就更容易移植。 數個內建函式無法在多個 CPU 架構。

- 您的程式碼很容易閱讀，因為仍 C/c + + 撰寫的程式碼。

- 您的程式碼取得編譯器最佳化的好處。 因為編譯器變得更好，可改善內建函式的程式碼產生。
