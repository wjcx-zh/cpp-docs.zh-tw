...
-   [浮點數會失去精確度的原因](../../build/reference/why-floating-point-numbers-may-lose-precision.md)  
 ...
 下列各種主題提供有關如何建立、 載入，及執行您的程式碼最佳化的資訊。  
...
-   [!INCLUDE[crabout](../../build/reference/includes/crabout_md.md)]如何在減少的時間載入 DLL 的方法，請參閱"下其實"中的資料行"MSDN Magazine"中的 < 最佳化 DLL 載入時間效能 > [MSDN Library](http://go.microsoft.com/fwlink/?linkid=556)網站。  
  
-   [!INCLUDE[crabout](../../build/reference/includes/crabout_md.md)]如何最小化應用程式中的分頁，請參閱 < 改善執行階段效能與 Smooth 工作設定工具 」 和 「 Smooth 工作以改善執行階段效能組工具 — 組件 2 」 中 「 MSDN Magazine"的"》"專欄上[MSDN程式庫](http://go.microsoft.com/fwlink/?linkid=556)網站。  
