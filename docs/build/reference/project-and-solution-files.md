...
|*Projname*.vcxproj|*Projname*|不會顯示在方案總管中|「專案」檔。 它會儲存每個專案的特定資訊。 (在舊版中，這個檔案名為 *Projname*.vcproj 或 *Projname*.dsp。)如需 C++ 專案檔 (.vcxproj) 的範例，請參閱[專案檔](project-files.md)。|
...
|*Projname*.idl|*Projname*|Source|(特定專案) 包含控制項類型程式庫的介面描述語言 (IDL) 原始程式碼。 Visual C++ 會使用這個檔案來產生類型程式庫。 產生的程式庫會向其他自動化用戶端公開控制項的介面。 如需詳細資訊，請參閱 Windows SDK 中的 [Interface Definition (IDL) File](/windows/win32/Rpc/the-interface-definition-language-idl-file) (介面定義 (IDL) 檔)。|
