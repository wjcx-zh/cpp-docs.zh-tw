...
## 備註  
 ...
 BOOT\_APPLICATION  
 在 Windows 開機環境中執行的應用程式。  如需開機應用程式的詳細資訊，請參閱[關於 BCD WMI 提供者](http://msdn.microsoft.com/library/aa362639.aspx) \(英文\)。  
  
 主控台  
 Windows 字元模式應用程式。  作業系統會提供主控台的主控台應用程式。  
  
 可延伸韌體介面 \(EFI\) 影像  
 EFI 子系統選項說明可延伸韌體介面環境中執行的可執行檔影像。  這種環境通常提供硬體，並在載入作業系統之前執行。  EFI 影像類型之間的主要差異是載入影像的記憶體位置，和呼叫影像時傳回時所採取的動作。  控制權傳回時會卸載 EFI\_APPLICATION 影像。  只有當控制項傳回錯誤碼時，EFI\_BOOT\_SERVICE\_DRIVER 或 EFI\_RUNTIME\_DRIVER 才會卸載。  從 ROM 執行 EFI\_ROM 影像。  如需詳細資訊，請參閱[統一 EFI 論壇](http://www.uefi.org/) 網站上的規格。  
 ...
 WINDOWS  
 在 Windows 圖形化環境中執行的應用程式。  這包括桌面應用程式和 Windows 市集應用程式。  
