...
|[auto\[-\]](zc-auto-deduce-variable-type.md)|強制執行新的標準C++表示`auto`(在預設情況下)。|
...
|[forScope\[-\]](zc-forscope-force-conformance-in-for-loop-scope.md)|強制執行標準C++`for`範圍規則 (在預設情況下)。|
...
|[rvalueCast\[-\]](zc-rvaluecast-enforce-type-conversion-rules.md)|強制執行標準C++明確類型轉換規則 （預設為關閉）。|
