---
title: '/Zc: strictstrings （停用字串常值類型轉換） |Microsoft Docs'
ms.custom: ''
ms.date: 03/06/2018
ms.technology:
- cpp-tools
...
如果 **/zc: strictstrings**指定時，編譯器會強制執行標準 c + +`const`限定性條件字串常值，做為類型 '的陣列`const char`' 或 ' 的陣列`const wchar_t`'，這取決於宣告。 字串常值不可變，如果嘗試修改其內容，則會導致執行階段存取違規錯誤。 您必須將字串指標宣告為 `const`，以使用字串常值來對其進行初始化，或者使用明確的 `const_cast`，對非 `const` 指標進行初始化。 根據預設，或如果 **/Zc:strictStrings-** 指定時，編譯器不會強制使用 standard c + +`const`使用字串常值初始化的字串指標的資格。
...
> [!NOTE]
> Visual Studio 2013 中的 c + + 標準程式庫不支援 **/zc: strictstrings**編譯器選項，在偵錯組建。 如果您看到幾[C2665](../../error-messages/compiler-errors-2/compiler-error-c2665.md)在組建中的錯誤輸出，這可能是原因。
