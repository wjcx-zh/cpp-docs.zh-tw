 ...
 **旗標**  
 ...
 UNW_FLAG_CHAININFO 這的回溯資訊結構並不是主要程序。 相反地，鏈結的回溯資訊項目是先前的 RUNTIME_FUNCTION 項目的內容。 請參閱下列文字，以了解鏈結的回溯資訊結構。 如果設定此旗標，則必須先清除 UNW_FLAG_EHANDLER 和 UNW_FLAG_UHANDLER 旗標。 此外，框架登錄 」 以及 「 固定的堆疊配置欄位必須具有相同的值和主要的回溯資訊。  
