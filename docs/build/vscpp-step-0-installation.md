...
description: 安裝視覺效果的 Visual Studio 支援C++
ms.custom: mvc
ms.date: 04/02/2019
ms.topic: tutorial
ms.devlang: cpp
...
> [!NOTE]
> 本主題適用于 Windows 上的 Visual Studio 安裝。 [Visual Studio Code](https://code.visualstudio.com/)是輕量、跨平臺的開發環境，可在 Windows、Mac 和 Linux 系統上執行。 適用于 Visual Studio Code 延伸模組的 Microsoft [C/C++ ](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)支援 IntelliSense、偵錯工具、程式碼格式設定、自動完成。 Visual Studio for Mac 不支援 Microsoft C++，但支援 .net 語言和跨平臺開發。 如需安裝指示，請參閱[Install Visual Studio for Mac](/visualstudio/mac/installation/)。
...
安裝安裝程式之後，您可以藉由選取您想要的*工作負載*或功能集，使用它來自訂安裝。 方式如下：

1. 在 [安裝 Visual Studio] 畫面中找到您想要的工作負載。

   ![Visual Studio 2019：安裝工作負載](../get-started/media/vs-installer-workloads.png)

   如需C++核心支援，請選擇「使用C++桌面開發」工作負載。 它隨附預設核心編輯器，其中包括超過 20 種語言的基本程式碼編輯支援、能夠從任何資料夾開啟及編輯程式碼而不需要專案，以及整合的原始程式碼控制。

   其他工作負載支援其他種類C++的開發。 例如，選擇 [通用 Windows 平臺開發] 工作負載，以建立使用 Microsoft Store Windows 執行階段的應用程式。 選擇「遊戲開發C++」以建立使用 DirectX、Unreal 和 Cocos2d 的遊戲。 選擇 [使用C++linux 開發] 來以 linux 平臺為目標，包括 IoT 開發。
...
1. 安裝程式會顯示一份工作負載清單，而工作負載是特定開發區域的相關選項群組。 的C++支援現在是預設不會安裝的選擇性工作負載的一部分。

   ![使用工作負載C++進行桌面開發](media/desktop-development-with-cpp.png "使用 C++ 的桌面開發")

   針對C++選取 [使用工作負載**進行C++桌面開發**]，然後選擇 [**安裝**]。

   ![使用C++工作負載安裝桌面開發](media/vscpp-concierge-choose-workload.gif "使用C++工作負載安裝桌面開發")

1. 當安裝完成時，選擇 [**啟動**] 按鈕以啟動 Visual Studio。

   第一次執行 Visual Studio 時，系統會要求您使用 Microsoft 帳戶登入。 如果您還沒有帳戶，可以免費建立一個帳戶。 您也必須選擇主題。 別擔心，如果您想要的話，可以稍後再變更。
...
若要安裝 Visual Studio 2015，請前往[下載舊版 Visual Studio](https://www.visualstudio.com/vs/older-downloads/)。 執行安裝程式，並選擇 [自訂安裝]，然後選擇 C++ 元件。 若要C++將支援新增至現有的 Visual Studio 2015 安裝，請按一下 Windows [開始] 按鈕，然後輸入 [新增] [**移除程式**]。 從 [結果] 清單中開啟程式，然後在已安裝的程式清單中尋找您的 Visual Studio 2015 安裝。 按兩下該檔案，然後選擇 [**修改**]，選取要C++安裝的視覺元件。
