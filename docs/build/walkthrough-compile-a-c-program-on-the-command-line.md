...
Visual Studio 版本的 Visual Studio 建置工具只會安裝命令列工具組、 編譯器、 工具和您要建立 C 和 c + + 程式庫。 它非常適合組建實驗室或教室會執行，並會安裝相當快速。 若要安裝的命令列工具組，請下載[Build Tools for Visual Studio](https://go.microsoft.com/fwlink/p/?linkid=875721)並執行安裝程式。

您可以在命令列上建置 C 或 c + + 程式之前，您必須確認確認已安裝的工具，以及您可以從命令列存取它們。 Visual c + + 為了尋找工具、 標頭和程式庫，它會使用具有複雜的命令列環境的需求。 **您無法使用 Visual c + + 中的純文字的命令提示字元視窗**沒有一些準備工作。 您需要*開發人員命令提示字元*視窗中，這可能會擁有所有必要的環境變數設定的一般的命令提示字元視窗。 幸運的是，Visual c + + 會安裝為您啟動已設定為命令列組建環境的開發人員命令提示字元捷徑。 不幸的是，開發人員命令提示字元 捷徑，和它們的所在位置的名稱是幾乎每個版本的 Visual c + +，並在不同版本的 Windows 中。 您的第一個逐步解說工作是尋找正確的捷徑使用。
...
2. 接下來，確認 Visual c + + 開發人員命令提示字元已正確設定。 在命令提示字元 視窗中，輸入`cl`並確認輸出看起來像下面這樣：
   ...
   如果您找不到開發人員命令提示字元 捷徑，或如果您收到錯誤訊息，當您輸入`cl`，Visual c + + 安裝可能會有問題。 如果您使用 Visual Studio 2017，請嘗試重新安裝**的 c + + 桌面應用程式開發**Visual Studio 安裝程式中的工作負載。 如需詳細資訊，請參閱[安裝 c + + 支援 Visual Studio 中的](../build/vscpp-step-0-installation.md)。 或者，您也可以重新安裝[Build Tools for Visual Studio](https://go.microsoft.com/fwlink/p/?linkid=875721)。 不要繼續進行下一節之前可以這麼做。 如需有關安裝和疑難排解 Visual Studio 的詳細資訊，請參閱[安裝 Visual Studio](/visualstudio/install/install-visual-studio)。
...
6. 若要編譯您的程式，請輸入**cl simple.c**在開發人員命令提示字元。
   ...
   > [!NOTE]
   > 如果您收到不同的編譯器或連結器錯誤或警告，請檢閱您的原始程式碼，更正任何錯誤，然後加以儲存，並再次執行編譯器。 如需特定錯誤的資訊，在這個頁面頂端使用搜尋方塊尋找錯誤號碼。
