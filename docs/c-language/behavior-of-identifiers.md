---
title: 識別碼的行為 | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
ms.topic: language-reference
dev_langs:
- C++
ms.assetid: 2ce0890f-9f04-47d8-9c77-187627576b94
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: 2aedebf2177e9633eab8216048e8b19c7bf5d1bd
ms.sourcegitcommit: be2a7679c2bd80968204dee03d13ca961eaa31ff
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/03/2018
ms.locfileid: "32380797"
---
# <a name="behavior-of-identifiers"></a>識別碼的行為
-   [沒有外部連結的有效字元數](../c-language/significant-characters-without-external-linkage.md)  
  
-   [有外部連結的有效字元數](../c-language/significant-characters-with-external-linkage.md)  
  
-   [大寫和小寫](../c-language/uppercase-and-lowercase.md)  
  
## <a name="see-also"></a>請參閱  
 [使用 extern 指定連結](../cpp/using-extern-to-specify-linkage.md)