 ...
 **signed char**、**signed int**、**signed short int** 與 **signed long int** 類型加上與其對應的 `unsigned` 和 `enum`，稱為「整數」類型。 **float**、**double**與 `long double` 類型指定名稱稱為「浮點」或「浮點數」類型。 您可以在變數或函式宣告中使用任何整數或浮點類型指定名稱。 如果未在宣告中提供 *type-specifier*，則會被當作 `int`。  
  
 選擇性關鍵字 **signed** 與 `unsigned` 可以在任何整數類型之前或之後 (`enum` 除外)，而且也可單獨作為類型指定名稱使用，在此情況下，這類關鍵字即分別為 **signed int** 與 `unsigned int`。 單獨使用時，會假設關鍵字 `int` 為 **signed**。 單獨使用時，關鍵字 **long** 與 **short** 即為 **long int** 與 `short int`。  
  
 列舉類型被視為基本類型。 列舉類型的類型指定名稱會在[列舉宣告](../c-language/c-enumeration-declarations.md)中討論。  
  
 關鍵字 `void` 有三個用途：指定函式傳回類型、指定不使用引數之函式的引數類型清單，以及指定未指定類型的指標。 您可以使用 `void` 類型宣告不傳回值的函式或宣告未指定類型的指標。 如需有關 `void` 單獨出現在函式名稱之後並以括弧括住時的詳細資訊，請參閱[引數](../c-language/arguments.md)。  
