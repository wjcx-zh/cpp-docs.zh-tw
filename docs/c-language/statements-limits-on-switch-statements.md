---
title: 陳述式：Switch 陳述式的限制 | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
ms.topic: language-reference
dev_langs:
- C++
ms.assetid: d327ef3f-2b11-4bd6-abfe-539697fd21a7
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: ba3d4e2b6e9661f936337cefcd6e660fc91c35eb
ms.sourcegitcommit: be2a7679c2bd80968204dee03d13ca961eaa31ff
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/03/2018
ms.locfileid: "32385737"
---
# <a name="statements-limits-on-switch-statements"></a>陳述式：Switch 陳述式的限制
**ANSI 3.6.4.2**：`switch` 陳述式中，**case** 值的最大數目  
  
 Microsoft C 不會限制 **switch** 陳述式中 **case** 值的數目。 這些數目只會受到可用記憶體的限制。  
  
## <a name="see-also"></a>請參閱  
 [實作定義的行為](../c-language/implementation-defined-behavior.md)