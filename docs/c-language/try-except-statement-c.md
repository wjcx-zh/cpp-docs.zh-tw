...
helpviewer_keywords: 
  - "__except 關鍵字 [C]"
  - "__except 關鍵字 [C], 在 try-except 中"
  - "__try 關鍵字 [C]"
  - "結構化例外狀況處理, try-except"
  - "try-catch 關鍵字 [C]"
 ...
 **try\-except** 陳述式是 Microsoft C 語言的擴充功能，可讓應用程式在發生通常會終止程式執行的事件時取得程式控制權。  這類事件稱為例外狀況，而處理例外狀況的機制稱為結構化例外狀況處理。  
...
## 範例  
 以下是例外狀況處理常式和終止處理常式的範例。  如需有關終止處理常式的詳細資訊，請參閱 [try\-finally 陳述式](../c-language/try-finally-statement-c.md)。  
