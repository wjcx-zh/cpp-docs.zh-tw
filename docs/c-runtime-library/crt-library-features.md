...
|MSVCPRTD.LIB|多執行緒的動態連結 (MSVCP\<版本>D.DLL 的匯入程式庫)|**/MDd**|_DEBUG、_MT、_DLL|  
 ...
 當您建置發行版本的專案時，預設會依據您選擇的編譯器選項 (多執行緒、DLL、/clr)，連結到基本 C 執行階段程式庫 (LIBCMT.LIB、MSVCMRT.LIB、MSVCRT.LIB) 中的一個。 如果您在程式碼中包含其中一個 [C++ 標準程式庫標頭檔](../standard-library/cpp-standard-library-header-files.md)，Visual C++ 就會在編譯時間自動連入 C++ 標準程式庫。 例如:   
