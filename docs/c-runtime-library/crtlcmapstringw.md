...
## 備註  
 如果 `cchSrc` 大於零且 `lpSrcStr` 為以 null 終止的字串，`__crtLCMapStringW` 會將 `cchSrc` 設定為字串的長度。 然後 `__crtLCMapStringW` 會呼叫寬字串 \(Unicode\) 版本的 `LCMapString` 函式並指定其參數。 如需此函式之參數和傳回值的詳細資訊，請參閱 [MSDN Library](http://go.microsoft.com/fwlink/?linkID=150542) 中的 `LCMapString` 函式。  
