...
## 備註  
 每個 `_exec` 函式都會載入並執行新處理序。  所有 `_exec` 函式都會使用相同的作業系統函式 \([CreateProcess](http://msdn.microsoft.com/library/windows/desktop/ms682425.aspx) \(英文\)\)。  `_exec` 函式會根據目前使用中的多位元組字碼頁，自動將多位元組字元字串引數處理為適當且可辨識的多位元組字元序列。  `_wexec` 函式是寬字元版本的 `_exec` 函式。  `_wexec` 函式的行為和其 `_exec` 系列對應項目一樣，只不過他們不處理多位元組字元字串。  
...
> [!NOTE]
>  字串中嵌入的空格可能會導致未預期的行為；例如，傳遞字串 `"hi there"` 至 `_exec` 會導致新處理序取得兩個引數 `"hi"` 和 `"there"`。  若目的是要使新處理序開啟名為 "hi there" 的檔案，則處理序會失敗。  您可以用引號括住字串來避免此情況：`"\"hi there\""`。  
  
> [!IMPORTANT]
>  請勿在沒有明確檢查內容的情況下將使用者輸入傳遞至 `_exec`。  `_exec` 會致使呼叫 [CreateProcess](http://msdn.microsoft.com/library/windows/desktop/ms682425.aspx) \(英文\)，所以請記得，不合格的路徑名稱可能會導致潛在的安全性漏洞。  
...
-   [\<caps:sentence id\="tgt114" sentenceid\="628d3b0b3928d3339d0f73bebf79c8d2" class\="tgtSentence"\>System::Diagnostics::Process 類別\<\/caps:sentence\>](https://msdn.microsoft.com/en-us/library/system.diagnostics.process.aspx)  
  
-   [\<caps:sentence id\="tgt115" sentenceid\="1f7e7c2cf2fa400c45c5c92839d0f5f2" class\="tgtSentence"\>System::Diagnostics::ProcessStartInfo 類別\<\/caps:sentence\>](https://msdn.microsoft.com/en-us/library/system.diagnostics.processstartinfo.aspx)  
