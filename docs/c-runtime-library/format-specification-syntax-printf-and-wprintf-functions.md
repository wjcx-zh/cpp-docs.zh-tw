...
**%**[[*旗標*](#flags)][[*width*](#width)][.[*精確度*](#precision)][[*大小*](#size)][*類型*](#type)
...
遵循格式字串的引數會根據對應的 *type* 字元和選擇性的 [size](#size) 前置詞解譯。 字元類型和的 **`char`** 轉換 **`wchar_t`** 是使用**c**或**c**來指定，而單一位元組和多位元組或寬字元字串則是使用**s**或**s**來指定，視使用的格式函數而定。 使用**c**和**s**所指定的字元和字串引數，會由系列函式 **`char`** **`char*`** `printf` 或 as **`wchar_t`** 和 `wchar_t*` by `wprintf` 系列函式來解讀。 使用**C**和**S**所指定的字元和字串引數，會由系列函式 **`wchar_t`** `wchar_t*` `printf` 或 as **`char`** 和 **`char*`** by `wprintf` 系列函式來解讀。 這是 Microsoft 特有的行為。

整數類型（例如 **`short`** 、 **`int`** 、 **`long`** 、 **`long long`** 和它們的 **`unsigned`** variant）是使用**d**、 **i**、 **o**、 **u**、 **x**和**x**所指定。浮點類型（例如 **`float`** 、 **`double`** 和 **`long double`** ）是使用、 **a**、 **e**、 **a** **e**、 **f**、 **f**、 **g**和**g**來指定。根據預設，除非由*大小*前置詞修改，否則會將整數引數強制轉 **`int`** 型為類型，並將浮點引數強制轉成 **`double`** 。 在64位系統上， **`int`** 是32位值; 因此，除非使用**Ll**或**I64**的*大小*前置詞，否則會在格式化為輸出時截斷64位整數。 由 **p** 指定的指標類型會使用此平台的預設指標。
...
|**X**|整數|不帶正負號的十六進位整數；使用 "ABCDEF"。|
|**版**|浮點|帶正負號的值，格式為 [-]*d.dddd*__e±__*dd*\[*d*]，其中 *d* 是一個十進位數，*dddd* 是一或多個十進位數字 (視指定的精確度而定)，或根據預設為六，而 *dd*\[*d*] 是兩個或三個十進位數 (視[輸出格式](../c-runtime-library/set-output-format.md)和指數的大小而定)。|
|**版**|浮點|與 **e** 格式相同，除了由 **E** 而非 **e** 引入該指數以外。|
...
|**為**|浮點|帶正負號的十六進位雙精確度浮點值，形式為 [-]0x*h.hhhh*__p±__*dd*，其中 *h.hhhh* 是尾數的十六進位數 (使用小寫字母)，而 *dd* 是指數的一或多個位數。 指定小數點之後位數的精確度。|
...
||當它與**e**、 **e**、 **f**、 **f**、 **a****或格式**搭配使用時，旗標會 **#** 強制輸出值包含小數點。|小數點只有在後面有數字時才會顯示。|
...
在轉換規格中，選擇性寬度規格欄位會在任何 *flags* 字元後面出現。 *width* 引數是非負數十進位整數，其控制輸出的字元數目下限。 如果輸出值中的字元數小於指定的寬度，則會在值的左邊或右邊加上空白（視是否指定左對齊旗標（）而定）， **-** 直到到達最小寬度為止。 如果 *width* 前面加上 0，前置字元為零會新增至整數或浮點數轉換，直到到達最小寬度為止 (轉換為無限或 NAN 則除外)。

寬度規格一律不會截斷值。 輸出值中的字元數若大於指定的寬度，或如果未給定 *width*，則值的所有字元皆為輸出，視 *precision* 規格而定。

如果寬度規格是星號（ `*` ）， **`int`** 引數清單中的引數會提供值。 *width* 引數的前面必須加上已在引數清單中格式化的值，如下例所示︰
...
不同於寬度規格，精確度規格會造成輸出值截斷或浮點數值四捨五入。 如果有效*位數*指定為0，且要轉換的值為0，則結果為無字元輸出，如下列範例所示：

`printf( "%.0d", 0 );      /* No characters output */`

如果精確度規格是星號（ \* ）， **`int`** 引數清單中的引數會提供值。 在引數清單中， *precision* 引數的前面必須加上已格式化的值，如下例所示︰
...
|類型|意義|預設|
|----------|-------------|-------------|
|**a**、**A**|指定小數點之後位數的精確度。|預設精確度為 13。 如果 precision 為0，除非使用旗標，否則不會列印小數點 **#** 。|
|**c**、 **c**|精確度無效果。|列印字元。|
...
在轉換規格中，*size*欄位是 *type* 轉換規範的引數長度修飾詞。 *type* 欄位的 *size* 欄位前置詞 (**hh**、**h**、**j**、**l** (lowercase L)、**L**、**ll**、**t**、**w**、**z**、**I** (大寫 i)、**I32** 及 **I64**) 會根據所修改的轉換規範來指定對應引數的「大小」：長或短、32 位元或 64 位元、單一位元組字元或寬字元。 這些大小前置詞可在 `printf` 和 `wprintf` 系列函式中搭配 *type* 字元使用，以指定引數大小的轉譯，如下表所示。 *size* 對於某些引數類型是選擇性欄位。 未指定大小前置詞時，格式器會使用整數引數（例如，已簽署或不帶正負號的、、、 **`char`** **`short`** **`int`** **`long`** 和列舉類型）做為32位 **`int`** 類型，而 **`float`** 、和 **`double`** **`long double`** 浮點引數會當做64位 **`double`** 類型使用。 這個行為符合變數引數清單的預設引數提升規則。 如需引數提升的詳細資訊，請參閱後置[運算式](../cpp/postfix-expressions.md)中的省略號和預設引數。 在 32 位元和 64 位元系統上，64 位元整數引數的轉換規格必須包含大小前置詞 **ll** 或 **I64**。 否則，格式子的行為未定義。
...
`ptrdiff_t`和 `size_t` 類型在 **`__int32`** **`unsigned __int32`** 32 位平臺上為或，在 **`__int64`** **`unsigned __int64`** 64 位平臺上為或。 **I** (大寫 i)、**j**、**t** 及 **z** 大小前置詞為平台採用正確的引數寬度。

在 Visual C++ 中，雖然 **`long double`** 是相異類型，但它與具有相同的內部標記法 **`double`** 。

**hc** 或 **hC** 類型規範，與 `printf` 函式中的 **c** 和 `wprintf` 函式中的 **C** 同義)。 **Lc**、 **lc**、 **wc**或**wc**類型規範與函式中的**c**和函式中 `printf` 的**c**同義 `wprintf` 。 **hs** 或 **hS** 類型規範，與 `printf` 函式中的 **s** 和 `wprintf` 函式中的 **S** 同義)。 **ls**、**lS**、**ws** 或 **wS** 類型規範，與 `printf` 函式中的 **S** 和 `wprintf` 函式中的 **s** 同義)。

> [!NOTE]
> **Microsoft 特定：**\
> **I** (大寫 i)、**I32**、**I64** 及 **w** 引數大小修飾詞前置詞為 Microsoft 延伸模組，且與 ISO C 不相容。 **H**前置詞與類型的資料搭配使用時， **`char`** 以及**l** （小寫 l）前置詞搭配類型的資料使用時， **`double`** 都是 Microsoft 擴充功能。
