...
遵循格式字串的引數會根據對應的 *type* 字元和選擇性的 [size](#size) 前置詞解譯。 字元類型 `char` 和 `wchar_t` 的轉換可使用 **c** 或 **C** 來指定，而單一位元組、多位元組或寬字元字串可使用 **s** 或 **S** 來指定，視使用何種格式化函式而定。 使用 **c** 和 **s** 所指定的字元和字串引數會由 `char` 系列函式解譯為 `char*` 和 `printf`，或是由 `wchar_t` 系列函式解譯為 `wchar_t*` 和 `wprintf`。 使用 **C** 和 **S** 所指定的字元和字串引數會由 `wchar_t` 系列函式解譯為 `wchar_t*` 和 `printf`，或是由 `char` 系列函式解譯為 `char*` 和 `wprintf`。 這是 Microsoft 特有的行為。
...
**hc** 或 **hC** 類型規範，與 **函式中的**c`printf` 和 **函式中的**C`wprintf` 同義)。 **lc**、**lC**、**wc** 或 **wC** 類型規範，與 **函式中的**C`printf` 和 **函式中的**c`wprintf` 同義。 **hs** 或 **hS** 類型規範，與 **函式中的**s`printf` 和 **函式中的**S`wprintf` 同義)。 **ls**、**lS**、**ws** 或 **wS** 類型規範，與 **函式中的**S`printf` 和 **函式中的**s`wprintf` 同義)。
