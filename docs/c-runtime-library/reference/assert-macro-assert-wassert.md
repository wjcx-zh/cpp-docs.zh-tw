...
當應用程式與執行時間程式庫的 debug 版本連結時, **assert**會建立含有三個按鈕的訊息方塊:[**中止**]、[**重試**] 和 [**忽略**]。 如果使用者按一下 [中止]，程式會立即中止。 如果使用者按一下 [重試]，會呼叫偵錯工具，而使用者可以偵錯程式是否已啟用 Just-In-Time (JIT) 偵錯。 如果使用者按一下 [**忽略**] , 則判斷提示會繼續正常執行: 建立含有 [**確定]** 按鈕的訊息方塊。 請注意，如果在發生錯誤情況時按一下 [忽略] ，可能會導致未定義的行為。
