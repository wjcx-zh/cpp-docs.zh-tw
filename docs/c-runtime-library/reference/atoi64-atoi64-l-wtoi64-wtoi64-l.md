...
每個函式會傳回 **__int64**產生的輸入的字元解譯為數字值。 傳回值是 0 **_atoi64**如果輸入無法轉換成該類型的值。

在大型正值的整數值，溢位的情況下 **_atoi64**傳回**I64_MAX**和**I64_MIN**在大負整數值溢位。
...
**_wtoi64**等同於 **_atoi64**不同之處在於它需要寬字元字串做為參數。
...
此程式會顯示如何以字串的形式儲存的數字可以轉換成數值資料，請使用 **_atoi64**函式。
