...
當傳遞有效的偵錯堆積的指標， **_CrtReportBlockType**函式傳回的區塊類型與子類型的形式**int**。當傳遞無效的指標時，函式會傳回 -1。

## <a name="remarks"></a>備註

若要擷取的型別與所傳回的子類型 **_CrtReportBlockType**，使用巨集 **_BLOCK_TYPE**和 **_BLOCK_SUBTYPE** （兩者都定義在 crtdbg.h 裡） 傳回的值。
