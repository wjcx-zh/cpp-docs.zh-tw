...
## <a name="remarks"></a>備註  
 ...
 `_execle` 函式會驗證它們的參數。 如果 `cmdname` 或 `arg0` 為 Null 指標或空字串，則這些函式會叫用無效的參數處理常式，如[參數驗證](../../c-runtime-library/parameter-validation.md)中所述。 如果允許繼續執行，這些函式會將 `errno` 設定為 `EINVAL` ，並傳回 -1。 未啟動任何新的處理序。  
