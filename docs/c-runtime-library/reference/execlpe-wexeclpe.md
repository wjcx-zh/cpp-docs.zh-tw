...
## 備註  
 ...
 `_execlpe` 函式會驗證它們的參數。  如果 `cmdname` 或 `arg0` 之一是 NULL 指標或空字串，這些函式呼叫無效參數處理常式，如 [參數驗證](../../c-runtime-library/parameter-validation.md) 裏所述。  如果允許繼續執行，這些函式會將 `errno` 設定為 `EINVAL` 並傳回 \-1。  不會啟動新的處理程序。  
