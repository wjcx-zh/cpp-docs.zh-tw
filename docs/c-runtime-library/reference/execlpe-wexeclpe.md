...
**_Execlpe**函式會驗證它們的參數。 如果有任一個*cmdname*或*arg0*是 null 指標或空字串，這些函式叫用無效參數處理常式中所述[參數驗證](../../c-runtime-library/parameter-validation.md)。 如果允許繼續執行，這些函式會將**errno**至**EINVAL**並傳回-1。 未啟動任何新的處理序。
