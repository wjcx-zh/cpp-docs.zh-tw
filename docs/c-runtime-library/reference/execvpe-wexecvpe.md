...
## 備註  
 ...
 `_execvpe` 函式會驗證它們的參數。  如果 `cmdname` 為 null 指標，或如果 `argv` 為 null 指標、指向空陣列的指標或指向包含空字串做為第一個引數之陣列的指標，則這些函式會叫用無效參數處理常式，如[參數驗證](../../c-runtime-library/parameter-validation.md)中所述。  如果允許繼續執行，這些函式會將 `errno` 設為 `EINVAL`，並傳回 \-1。  未啟動任何處理序。  
