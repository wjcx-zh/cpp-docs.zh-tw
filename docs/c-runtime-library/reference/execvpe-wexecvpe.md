...
**_Execvpe**函式會驗證它們的參數。 如果*cmdname*為 null 指標，或如果*argv*為 null 指標、 指向空陣列或指向包含空字串做為第一個引數的陣列，這些函式叫用無效參數處理常式，如中所述[參數驗證](../../c-runtime-library/parameter-validation.md)。 如果允許繼續執行，這些函式會將**errno**至**EINVAL**並傳回-1。 未啟動任何處理序。
