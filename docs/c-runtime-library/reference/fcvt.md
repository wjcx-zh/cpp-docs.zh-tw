...
**_Fcvt**函數會將浮點數轉換成以 null 結束的字元字串。 *Value*參數是要轉換的浮點數。 **_fcvt**會將*值*的數位儲存為字串，並附加 null 字元（' \ 0 '）。 *Count*參數指定小數點之後要儲存的位數。 多餘的數位會四捨五入以*計算*位置。 如果精確度的位數少於*計數*，字串會以零填補。
...
**_Ecvt**和 **_fcvt**之間的差異在於*count*參數的轉譯。 **_ecvt**會將*count*解讀為輸出字串中的總位數，而 **_fcvt**會將*count*解讀為小數點後的位數。
