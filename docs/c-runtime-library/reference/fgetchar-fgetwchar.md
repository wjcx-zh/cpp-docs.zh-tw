...
|`_fgetwchar`|\<stdio.h> 或 \<wchar.h>|  
 ...
 通用 Windows 平台 (UWP) 應用程式中不支援主控台。 在主控台與相關聯的標準資料流控制代碼 —`stdin`， `stdout`，和`stderr`— C 執行階段函式才能使用它們在必須重新導向 [！INCLUDEUWP 應用程式。 如需相容性的詳細資訊，請參閱 [相容性](../../c-runtime-library/compatibility.md)。  
