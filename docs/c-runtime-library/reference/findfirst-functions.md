...
## <a name="return-value"></a>傳回值  
 如果成功，`_findfirst` 會傳回唯一搜尋控制代碼以識別符合 `filespec` 規格的檔案或檔案群組，這可用於後續 [_findnext](../../c-runtime-library/reference/findnext-functions.md) 或 `_findclose` 呼叫。 否則，`_findfirst`傳回-1，並設定`errno`至下列值之一。  
