...
**_Getche**和 **_getwche**函式會讀取具有回應，這表示該字元會顯示在主控台的主控台中的單一字元。 這些函式都不可以用來讀取 CTRL+C。 在讀取功能鍵或方向鍵時，每個函式必須呼叫兩次；第一次呼叫會傳回 0 或 0xE0，而第二次呼叫會傳回實際的按鍵碼。
