...
## <a name="return-value"></a>傳回值  
 如果 `c` 目前的地區設定中具有 `desc` 所指定的屬性，則 `_isctype` 和`iswctype` 會傳回非零值，如果不具備則傳回 0。 尾碼為 `_l` 的這些函式版本是一樣的，只不過與地區設定相關的行為使用了傳入的地區設定，而不是目前的地區設定。 如需詳細資訊，請參閱[地區設定](../../c-runtime-library/locale.md)。  
