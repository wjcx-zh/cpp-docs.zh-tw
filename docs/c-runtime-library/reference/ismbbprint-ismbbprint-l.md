...
如果運算式為， **_ismbbprint**會傳回非零值：

`isprint(c) || _ismbbkprint(c)`

*c*為非零值; 如果不是，則為0。 **_ismbbprint**會針對任何與地區設定相關的行為使用目前的地區設定。 **_ismbbprint_l**是相同的，不同之處在于它會改為使用傳入的地區設定。 如需詳細資訊，請參閱 [Locale](../../c-runtime-library/locale.md)。
