...
這些函式版本是相同的不同之處在於則有 **_l**後置詞使用傳入的地區設定相關的行為，而不是目前的地區設定的地區設定。 如需詳細資訊，請參閱 [Locale](../../c-runtime-library/locale.md)。
...
|常式|測試條件|字碼頁 932 範例|
|-------------|--------------------|---------------------------|
|**_ismbcgraph**|圖形|傳回非零，才*c*是以外的任何 ASCII 或片假名可列印字元泛空白字元 （） 的單一位元組表示法。|
|**_ismbcprint**|可列印|傳回非零，才*c*是任何 ASCII 或片假名可列印的字元包括空格 （） 的單一位元組表示法。|
|**_ismbcpunct**|標點符號|傳回非零，才*c*是任何 ASCII 字元或片假名標點符號字元的單一位元組表示法。|
|**_ismbcblank**|空格或水平索引標籤|傳回非零，才*c*空間或水平定位字元： *c*= 0x20 或*c*= 0x09。|
|**_ismbcspace**|空白字元|傳回非零，才*c*為泛空白字元： *c*= 0x20 或 0x09 < =*c*< = 0x0D。|
