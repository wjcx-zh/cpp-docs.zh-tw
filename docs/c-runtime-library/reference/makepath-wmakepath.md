...
*硬碟磁碟機*<br/>
包含對應至所需磁碟機的代號 (A、B 等) 及選擇性後置冒號。 **_makepath**在複合路徑中自動插入冒號（如果遺漏的話）。 如果*drive*是**Null**或指向空字串，複合*路徑*字串中就不會出現任何磁碟機號。
...
*分機*<br/>
包含實際副檔名，可包含或不含前置句號 (.)。 如果未出現在*ext*中， **_makepath**會自動插入句號。如果*ext*是**Null**或指向空字串，複合*路徑*字串中就不會插入任何副檔名。

## <a name="remarks"></a>備註

**_Makepath**函式會從個別元件建立複合路徑字串，並將結果儲存在*path*中。 *路徑*可能包含磁碟機號、目錄路徑、檔案名和副檔名。 **_wmakepath**是寬字元版本的 **_makepath**;**_wmakepath**的引數是寬字元字串。 相反地， **_wmakepath**和 **_makepath**的行為相同。
