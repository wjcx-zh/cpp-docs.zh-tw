...
**_malloc_dbg**是偵錯版本的[malloc](malloc.md)函式。 當[_DEBUG](../../c-runtime-library/debug.md)未定義時，每次呼叫 **_malloc_dbg**呼叫會降**malloc**。 同時**malloc**和 **_malloc_dbg**配置基底堆積中的記憶體區塊，但 **_malloc_dbg**提供數種偵錯功能： 使用者的任一端使用緩衝區一部分的區塊，以測試遺漏，來追蹤特定配置類型的區塊型別參數和*filename*/*linenumber*來判斷來源的資訊配置要求。
...
**_malloc_dbg**設定**errno**至**ENOMEM**若記憶體配置失敗或需要的記憶體數量 （包含之前提到的額外負荷） 超過 **_HEAP_MAXREQ**。 如需此錯誤碼和其他錯誤碼的資訊，請參閱 [errno、_doserrno、_sys_errlist 和 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。
...
如需使用方式的範例 **_malloc_dbg**，請參閱[crt_dbg1](https://github.com/Microsoft/VCSamples/tree/master/VC2010Samples/crt/crt_dbg1)。
