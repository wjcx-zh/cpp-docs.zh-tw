...
**malloc**會傳回已配置空間的 void 指標，如果沒有足夠的記憶體可用，則傳回**Null** 。 若要傳回**void**以外類型的指標，請在傳回值上使用類型轉換。 此傳回值所指向的儲存空間保證會經過適當調整，因此可儲存對齊需求小於或等於基本對齊需求的任何物件類型 （在視覺C++效果中，基本對齊是**double**或8個位元組所需的對齊方式。 在以 64 位元平台為目標的程式碼中，則為 16 個位元組)。針對具有較大對齊需求的物件（例如，SSE 類型[__m128](../../cpp/m128.md)和 **__m256**，以及使用`__declspec(align( n ))`宣告的類型，其中**n**大於8），使用[_aligned_malloc](aligned-malloc.md)來配置儲存區。 如果*size*為0， **malloc**會在堆積中配置零長度專案，並傳回該專案的有效指標。 請一律檢查**malloc**的傳回，即使要求的記憶體數量很小也一樣。
...
**malloc**已標記`__declspec(noalias)`為`__declspec(restrict)`和，這表示函式保證不會修改全域變數，而且傳回的指標沒有別名。 如需詳細資訊，請參閱 [noalias](../../cpp/noalias.md) 和 [restrict](../../cpp/restrict.md)。
