...
## <a name="remarks"></a>備註  
 `_mbcjistojms` 函式會將日本業界標準 (JIS) 字元轉換成 Microsoft 漢字 (Shift JIS) 字元。 只有在前導位元組和後隨位元組介於 0x21-0x7E 的範圍時，才會轉換此字元。 如果前導位元組或後隨位元組超出此範圍，`errno` 會設定為 `EILSEQ`。 如需此錯誤碼和其他錯誤碼的詳細資訊，請參閱 [errno、_doserrno、_sys_errlist 和 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。  
  
 `_mbcjmstojis` 函式會將 Shift JIS 字元轉換成 JIS 字元。 只有在前導位元組介於 0x81-0x9F 或 0xE0-0xFC 的範圍，且後隨位元組介於 0x40-0x7E 或 0x80-0xFC 的範圍時，才會轉換此字元。 請注意，該範圍內的某些字碼指標未指派一個字元，因此無法進行轉換。  
 ...
 在舊版中，`_mbcjistojms` 和 `_mbcjmstojis` 分別稱為 `jistojms` 和 `jmstojis`。 應改用 `_mbcjistojms`、`_mbcjistojms_l`、`_mbcjmstojis` 和 `_mbcjmstojis_l`。  
