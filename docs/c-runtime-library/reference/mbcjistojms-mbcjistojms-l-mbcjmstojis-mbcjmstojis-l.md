...
**_Mbcjistojms**函式會將日本業界標準 (JIS) 字元轉換成 Microsoft 漢字 (Shift JIS) 字元。 只有前置和後隨位元組 0x21-0x7E 範圍中，將轉換的字元。 負責人或試用位元組超出此範圍中，如果**errno**設**EILSEQ**。 如需此錯誤碼和其他錯誤碼的詳細資訊，請參閱 [errno、_doserrno、_sys_errlist 和 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。
