...
## <a name="remarks"></a>備註  
 將 `dest` 的前 `count` 個字元設定為字元 `c`。  
  
 **安全性提示**：確定目的緩衝區的空間足以容納至少 `count` 個字元。 如需詳細資訊，請參閱 [Avoiding Buffer Overruns (避免緩衝區滿溢)](http://msdn.microsoft.com/library/windows/desktop/ms717795)。  
