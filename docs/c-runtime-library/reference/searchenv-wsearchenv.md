...
使用環境路徑來搜尋檔案。  這些函式已有更安全的版本可用，請參閱 [\_searchenv\_s、\_wsearchenv\_s](../../c-runtime-library/reference/searchenv-s-wsearchenv-s.md)。  
...
## 備註  
 ...
 如果 `filename` 或 `pathname` 為 `NULL` 指標，則會叫用無效參數處理常式，如[參數驗證](../../c-runtime-library/parameter-validation.md)中所述。  如果允許繼續執行，這些函式會傳回 \-1，並將 `errno` 設為 `EINVAL`。  
