...
*sizeOfBuffer*<br/>
輸出的儲存位置大小。 在大小**位元組**如 **_snprintf_s**或大小**字**如 **_snwprintf_s**。
...
**_snprintf_s**傳回儲存在中的字元數*緩衝區*，不計結束的 null 字元。 **_snwprintf_s**傳回儲存在寬字元數目*緩衝區*，不計結束的 null 寬字元。

如果儲存的資料和結束的 null 所需的儲存體超過*sizeOfBuffer*、 無效參數處理常式會叫用中所述[參數驗證](../../c-runtime-library/parameter-validation.md)。 如果無效參數處理常式之後，繼續執行，這些函式會將*緩衝區*為空字串，設定**errno**至**為 ERANGE**，並傳回-1。
...
**_Snprintf_s**函式的格式化並儲存*計數*或較少的字元*緩衝區*並在後面附加結束的 null。 （如果有的話），每個引數是轉換和輸出中的對應格式規格根據*格式*。 格式都是一致的**printf**系列的函式，請參閱 <<c4> [ 格式規格語法： printf 和 wprintf 函式](../../c-runtime-library/format-specification-syntax-printf-and-wprintf-functions.md)。 如果在重疊的字串之間進行複製，則行為是未定義的。

如果*計數*是[_TRUNCATE](../../c-runtime-library/truncate.md)，然後 **_snprintf_s**寫入的字串會盡量納入*緩衝區*時留出空間給正在結束的 null。 如果整個字串 （包括結束 null) 納入*緩衝區*，然後 **_snprintf_s**傳回寫入字元數 （不包括結束的 null）; 否則 **_snprintf_s**會傳回 – 1，表示該截斷發生。

> [!IMPORTANT]
> 確認 *format* 不是使用者定義的字串。

**_snwprintf_s**是寬字元版本的 **_snprintf_s**; 指標引數 **_snwprintf_s**是寬字元字串。 編碼錯誤偵測 **_snwprintf_s**可能不同於 **_snprintf_s**。 **_snwprintf_s**、 like **swprintf_s**，字串，而非類型的目的地會將輸出寫入**檔案**。
