...
**_Sopen**函數會開啟*filename*所指定的檔案，並準備檔案以供共用讀取或寫入，如*oflag*和*shflag*所定義。 **_wsopen**是寬字元版本的 **_sopen**; **_wsopen**的*filename*引數是寬字元字串。 相反地， **_wsopen**和 **_sopen**的行為相同。
...
如果使用 **_O_WRONLY**  |  **_O_APPEND** （附加模式）和 **_O_WTEXT**、 **_O_U16TEXT**或 **_O_U8TEXT**呼叫 **_sopen** ，它會先嘗試開啟檔案進行讀取和寫入、讀取 BOM，然後重新開啟它僅限寫入。 若開啟檔案以供讀取和寫入失敗，則會僅針對寫入開啟檔案，並使用 Unicode 模式設定的預設值。
...
只有在指定 **_O_CREAT**時，才需要*pmode*引數。 如果檔案不存在， *pmode*就會指定檔案的許可權設定，當第一次關閉新的檔案時，就會設定這個值。 否則，會忽略*pmode* 。 *pmode*是一個整數運算式，其中包含一或兩個資訊清單常數 **_S_IWRITE**和 **_S_IREAD**，它們定義于\<sys\stat.h > 中。 指定兩個常數時，會使用位元 OR 運算子加以結合。 *Pmode*的意義如下所示。
...
**_sopen**會在設定許可權之前，將目前的檔案許可權遮罩套用至*pmode* 。 (請參閱 [_umask](umask.md))。
