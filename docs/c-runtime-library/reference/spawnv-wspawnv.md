...
## 傳回值  
 來自同步的 `_spawnv` 或 `_wspawnv` \(為 `mode` 指定的 `_P_WAIT`\) 之傳回值是新處理序的結束狀態。 非同步 `_spawnv` 或 `_wspawnv` \(為 `mode` 指定的 `_P_NOWAIT` 或 `_P_NOWAITO`\) 的傳回值是處理序控制代碼。 如果處理序正常終止，結束狀態為 0。 如果繁衍的處理序會用非零的引數明確呼叫 `exit` 常式，您就可以將結束狀態設為非零值。 如果新處理序未明確設定確定的結束狀態，所謂確定的結束狀態表示因中止或中斷而異常結束。 傳回值為 –1 表示錯誤 \(新處理序未啟動\)。 在這種情況下，`errno` 會設為下列其中一個值。  
