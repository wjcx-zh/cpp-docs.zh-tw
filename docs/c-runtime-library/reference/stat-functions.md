...
**_stat**相當於 **_stat64i32**，而 **`struct`** **_stat**則包含64位時間。 除非已定義 **_USE_32BIT_TIME_T** （在此情況下，舊的行為會生效），否則為 true。**_stat**使用32位時間， **`struct`** **_stat**包含32位時間。 **_Stati64**的情況也是如此。
