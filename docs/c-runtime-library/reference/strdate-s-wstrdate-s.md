...
這些函式提供更安全的 **_strdate**和 **_wstrdate**版本。 **_Strdate_s**函式會將目前的系統日期複製到*緩衝區*所指向的緩衝區。 它的格式為 `mm/dd/yy`，其中 `mm` 是兩位數的月份，`dd` 是兩位數的日期，而 `yy` 是年份的最後兩個數字。 例如，字串 `12/05/99` 代表 1999 年 12 月 5 日。 緩衝區長度至少必須為9個字元。
...
在C++中，範本多載簡化了這些函式的使用方式。 多載可以自動推斷緩衝區長度，這樣就不需要指定*大小*引數。 而且，它們可以自動將不安全的函式取代為其較新且更安全的對應專案。 如需詳細資訊，請參閱[安全範本多載](../../c-runtime-library/secure-template-overloads.md)。
