...
**_strtoi64**會傳回字串*strSource*中所表示的值，但標記法會造成溢位，在這種情況下，它會傳回 **_I64_MAX**或 **_I64_MIN**。 如果沒有任何轉換可執行，函式會傳回 0。 **_wcstoi64**會傳回類似至**strtoi64**的值。

**_I64_MAX**和 **_I64_MIN**定義于限制中。H.
...
**_Strtoi64**函數會將*strSource*轉換為 **__int64**。 這兩個函式會在無法辨識為數字一部分的第一個字元處停止讀取字串*strSource* 。 這可能是終止的 null 字元，或者它可能是大於或等於*base*的第一個數位字元。 **_wcstoi64**是寬字元版本的 **_strtoi64**;其*strSource*引數是寬字元字串。 除此之外，這些函式的行為相同。
...
地區設定的**LC_NUMERIC**類別設定會決定*strSource*中的基數位符辨識;如需詳細資訊，請參閱[setlocale](setlocale-wsetlocale.md)。 沒有 _l 尾碼的函式會使用目前的地區設定; **_strtoi64_l**和 **_wcstoi64_l**與對應的函式相同，但不含 **_l**尾碼，不同之處在于它們會改用傳入的地區設定。 如需詳細資訊，請參閱 [Locale](../../c-runtime-library/locale.md)。
