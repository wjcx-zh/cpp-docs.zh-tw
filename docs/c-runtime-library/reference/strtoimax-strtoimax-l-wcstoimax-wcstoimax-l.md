...
## <a name="remarks"></a>備註  
 ...
 地區設定的 `LC_NUMERIC` 類別設定會決定 `nptr` 中的基底字元辨識，如需詳細資訊，請參閱 [setlocale、_wsetlocale](../../c-runtime-library/reference/setlocale-wsetlocale.md)。 這些沒有 `_l` 尾碼的函式使用目前的地區設定，而 `_strtoimax_l` 和 `_wcstoimax_l` 與對應之無 `_l` 尾碼的函式相同，只不過它們改用傳入的地區設定。 如需詳細資訊，請參閱 [Locale](../../c-runtime-library/locale.md)。  
