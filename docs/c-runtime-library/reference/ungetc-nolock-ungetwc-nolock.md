...
如果成功，所有這些函式都會傳回字元引數*c*。 如果無法將*c*推送回來，或未讀取任何字元，則輸入資料流程不會變更，而且 **_Ungetc_nolock**會傳回**EOF**; **_ungetwc_nolock**會傳回**WEOF**。 如果*stream*為**Null**，則會傳回**EOF**或**WEOF** ，並將**errno**設定為**EINVAL**。
