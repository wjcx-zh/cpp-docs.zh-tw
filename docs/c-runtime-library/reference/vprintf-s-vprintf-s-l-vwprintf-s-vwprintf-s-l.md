...
**vprintf_s**並**vwprintf_s**傳回寫入的字元數，如果發生輸出錯誤，不包括結束的 null 字元或為負值。 如果*格式*為 null 指標，或如果格式字串包含無效格式化字元，無效參數處理常式會叫用，如中所述[參數驗證](../../c-runtime-library/parameter-validation.md)。 如果允許繼續執行，函式會傳回-1，並設定**errno**要**EINVAL**。
...
這些函式的安全版本不同**vprintf**並**vwprintf**僅在於安全版本會檢查格式字串包含有效格式化字元。

**vwprintf_s**是寬字元版本**vprintf_s**; 如果資料流以 ANSI 模式開啟兩個函式行為相同。 **vprintf_s**目前不支援輸出至 UNICODE 資料流。
