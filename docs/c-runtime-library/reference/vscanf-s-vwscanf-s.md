...
這會在成功轉換並指派時傳回欄位數；傳回值不包含已讀取但尚未指派的欄位。 傳回值 0 表示未指派任何欄位。 傳回值為**EOF**表示發生錯誤，或在第一次嘗試讀取字元時遇到檔案結尾字元或字串結尾字元。 如果*format*是**Null**指標，則會叫用不正確參數處理常式，如[參數驗證](../../c-runtime-library/parameter-validation.md)中所述。 如果允許繼續執行， **vscanf_s**和**Vwscanf_s**會傳回**EOF** ，並將**errno**設為**EINVAL**。
...
**Vscanf_s**函式會從標準輸入資料流程**stdin**讀取資料，並將資料寫入*arglist*引數清單所提供的位置。 清單中的每個引數都必須是對應于*格式*之類型規範的類型變數的指標。 如果在重疊的字串之間進行複製，則行為是未定義的。

**vwscanf_s**是寬字元版本的**vscanf_s**;**vwscanf_s**的*格式*引數是寬字元字串。 如果資料流程是以 ANSI 模式開啟， **vwscanf_s**和**vscanf_s**的行為會相同。 **vscanf_s**不支援來自 UNICODE 資料流程的輸入。

不同于**vscanf**和**vwscanf**， **vscanf_s**和**vwscanf_s**需要為**c**、 **c**、 **s**、 **s**或字串控制集（包含在 **[]** 中）的所有輸入參數指定緩衝區大小。 以字元為單位的緩衝區大小會作為緊接指標的額外參數傳遞至緩衝區或變數。 字串的緩衝區大小（以字元為 **`wchar_t`** 單位）與大小（以位元組為單位）不同。
