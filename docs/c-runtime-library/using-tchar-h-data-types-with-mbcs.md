---
title: "搭配 _MBCS 使用 TCHAR.H 資料類型 | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
**Microsoft 特定的**  
 ...
 如泛型文字常式對應表格 (請參閱[泛型文字對應](../c-runtime-library/generic-text-mappings.md)) 所指，當已定義資訊清單常數 `_MBCS` 時，給定的泛型文字常式會對應到下列一種常式：  
...
-   MBCS 特定常式。 在此案例中，字串引數類型必須是 `unsigned char*`。 例如，`_tcsrev` 對應到 `_mbsrev`，它預期並傳回類型為 `unsigned char*` 的字串。 再強調一次，若您為字串類型使用 `_TCHAR` 泛型文字資料類型，可能會發生類型衝突，因為 `_TCHAR` 對應到類型 `char`。  
...
-   使用預設行為。 TCHAR.H 為執行階段程式庫中的常式提供泛型文字常式原型，如下列範例所示。  
    ...
    ```  
     ...
     在預設案例中，`_tcsrev` 的原型透過 LIBC.LIB 中的 Thunk 對應到 `_mbsrev`。 這會將 `_mbsrev` 傳入參數與傳出結果值的類型從 `_TCHAR *` (例如 `char *`) 變更為 `unsigned char *`。 此方法可確保當您使用 `_TCHAR` 時類型能相符，但由於函式呼叫額外負荷之故，速度相當緩慢。  
