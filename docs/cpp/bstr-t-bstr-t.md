...
*fCopy*<br/>
如果為 FALSE，則會將*bstr*引數附加至新的物件，而不需要藉由呼叫 `SysAllocString`來建立複本。
...
|建構函式|描述|
|-----------------|-----------------|
|`_bstr_t( )`|建立封裝 null `BSTR` 物件的預設 `_bstr_t` 物件。|
|`_bstr_t( _bstr_t&`  `s1`  `)`|將 `_bstr_t` 物件建構為另一個物件的複本。<br /><br /> 這是*淺層*複製，會遞增封裝 `BSTR` 物件的參考計數，而不是建立新的物件。|
...
|`_bstr_t( BSTR``bstr` `, bool``fCopy``)`|從現有的 `_bstr_t` 建構 `BSTR` 物件 (而非 `wchar_t*` 字串)。 如果*fCopy*為 false，則提供的 `BSTR` 會附加至新的物件，而不會使用 `SysAllocString`建立新的複本。<br /><br /> 類型程式庫標題中的包裝函式會使用此建構函式封裝，並取得以介面方法傳回之 `BSTR` 的擁有權。|
