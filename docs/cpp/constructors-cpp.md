...
若要自訂類別成員初始化的方式，或叫用函式，建立您的類別物件時，定義*建構函式*。 建構函式的名稱與類別的名稱相同，但沒有傳回值。 您可以定義多多載建構函式視需要自訂初始化，以各種方式。 一般而言，建構函式具有公用存取範圍，使類別定義或繼承階層架構之外的程式碼可以建立類別的物件。 您也可以宣告為建構函式，但是**保護**或**私人**。

建構函式 （選擇性） 可以取得成員初始化清單。 這是更有效率的方式，來初始化類別成員比指派建構函式主體中的值。 下列範例顯示的類別`Box`三個多載建構函式。 最後兩個使用成員初始化清單：
...
- 建構函式可以宣告為**內嵌**，[明確](#explicit_constructors)， **friend**或[constexpr](#constexpr_constructors)。
- 建構函式可以初始化物件的已宣告為**const**， **volatile**或**常數 volatile**。 在物件變成**const**建構函式完成之後。
- 若要實作檔案中定義的建構函式，指定限定的名稱如同任何其他成員函式： `Box::Box(){...}`。
...
識別項必須參考類別成員。它會使用引數的值進行初始化。 引數可以是其中一個建構函式參數，函式呼叫或[std:: initializer_list\<T >](../standard-library/initializer-list-class.md)。 

**const**成員和參考類型的成員必須初始化成員初始設定式清單中。

呼叫參數化的基底類別建構函式應該會在初始設定式清單以確保基底類別完全初始化之前衍生的建構函式的執行。

## <a name="default_constructors"></a> 預設建構函式

 *預設建構函式*通常有任何參數，但是可以具有預設值的參數。
...
預設建構函式是其中一種[特殊成員函式](special-member-functions.md)。 如果類別中不宣告任何建構函式，編譯器會提供隱含**內嵌**預設建構函式。
...
如果您依賴隱含的預設建構函式時，請務必初始化成員的類別定義中，在上述範例所示。 沒有這些初始設定式中，成員會是未初始化，Volume() 呼叫會產生記憶體回收的值。 一般情況下，它是個不錯的做法初始化成員，如此一來，即使不依賴隱含的預設建構函式。

您可以防止編譯器產生的隱含的預設建構函式定義為[刪除](#explicitly_defaulted_and_deleted_constructors):
...
如果任何類別成員不是預設建構這個類別已刪除，將會定義編譯器產生預設建構函式。 例如，類別型別的所有成員和其類別類型成員，必須有預設建構函式和解構函式的存取。 所有資料成員的參考都類型，以及為**const**成員必須具有預設成員初始設定式。

當您呼叫編譯器產生的預設建構函式，並使用括號，再試一次時，就會發出警告：
...
如需詳細資訊，請參閱[初始設定式](initializers.md)。

## <a name="copy_and_move_constructors"></a> 複製建構函式

A*複製建構函式*複製的物件相同類型的成員值來初始化物件。 如果您的類別成員是所有的簡單類型，例如純量值，編譯器產生的複製建構函式已足夠，您不需要定義自己。 如果您的類別需要更複雜的初始設定，則您需要實作自訂的複製建構函式。 例如，如果類別成員是指標，然後您需要定義配置新的記憶體，並將值複製從對方的指向物件的複製建構函式。 編譯器產生的複製建構函式只會複製指標，使新的指標仍然指向對方的記憶體位置。
...
當您定義的複製建構函式時，您也應該定義複製指派運算子 （=）。 如需詳細資訊，請參閱[指派](assignment.md)和[複製建構函式和複製指派運算子](copy-constructors-and-copy-assignment-operators-cpp.md)。
...
嘗試複製此物件會產生錯誤*C2280： 嘗試參考已刪除的函式*。

## <a name="move_constructors"></a> 移動建構函式
A*移動建構函式*會移到新的變數的擁有權的現有物件的資料，而不複製原始資料的特殊成員函式。 它採用右值參考做為其第一個參數，以及任何其他參數必須具有預設值。 移動建構函式傳遞大型物件時，可能會大幅增加您的程式效率。 移動建構函式接受右值參考做為其第一個參數。 任何其他參數必須有預設值。

```cpp
Box(Box&& other);
```

編譯器會選擇移動建構函式在某些情況下，物件即將終結，而且不再需要它的相同類型的另一個物件正在初始化其中的資源。 下列範例會示範其中一種情況，當移動建構函式多載解析會選取。 變數*方塊*傳回 get_Box() 是*xvalue* （將過期的值） 這是超出範圍。 若要提供此範例中的動機，讓我們方塊大型字串，代表其內容的向量。 而非複製向量，其字串、 移動建構函式 」 竊取 」 它從過期的值 「 方塊 」 以便向量現在屬於新物件。 呼叫`std::move`已因為所需要的是同時`vector`和`string`類別會實作自己的移動建構函式。
...
如果類別並未定義的移動建構函式，編譯器會產生隱含的一個沒有使用者宣告複製建構函式、 複製指派運算子、 移動指派運算子或解構函式。 如果定義不明確或隱含的移動建構函式，則會使用移動建構函式的作業會改為使用複製建構函式。 如果在類別宣告移動建構函式或移動指派運算子，以隱含方式宣告的複製建構函式被定義為刪除。

為已刪除，如果任何類別類型的成員缺少解構函式或編譯器無法判斷要用於移動作業的建構函式定義隱含宣告的移動建構函式。
...
如需詳細資訊，請參閱[明確預設和刪除函式](../cpp/explicitly-defaulted-and-deleted-functions.md)。

## <a name="constexpr_constructors"></a> constexpr 建構函式

建構函式可以宣告為[constexpr](constexpr-cpp.md)如果

- 它會是宣告為預設或 else 滿足所有條件，則[constexpr 函式](constexpr-cpp.md#constexpr_functions)一般而言;
- 此類別具有沒有虛擬基底類別。
- 每個參數是[常值型別](trivial-standard-layout-and-pod-types.md#literal_types);
- 主體不是函式 try 區塊。
- 所有非靜態資料成員和基底類別子物件初始化。
- 如果類別是具有 variant 的成員 （a) 等位，或 （b） 具有匿名等位，會將其中一個等位成員初始化;
- 類別類型的每個非靜態資料成員和所有基底類別的子物件有 constexpr 建構函式


## <a name="init_list_constructors"></a> 初始設定式清單建構函式

如果建構函式接受[std:: initializer_list\<T\> ](../standard-library/initializer-list-class.md)多載解析會選取它的參數，以及任何其他參數有預設引數，該建構函式，類別為透過直接初始化執行個體化。 您可以使用 initializer_list 來初始化可接受的任何成員。 例如，假定 （先前所示） 的方塊類別的`std::vector<string>`成員**m_contents**。 您可以提供類似的建構函式：
...
在某些情況下，這類轉換十分有用，但它們可能更常導致您程式碼中的細微但嚴重的錯誤。 一般而言，您應該使用**明確**建構函式 （和使用者定義運算子），以防止這種隱含類型轉換關鍵字：
...
A*委派建構函式*會呼叫不同的建構函式中相同的類別，以執行某些初始化工作。 當您有多個建構函式都可以執行類似的工作時，這非常有用。 您可以撰寫一個建構函式中的主要邏輯，並叫用它從其他人。 在下列簡單範例中，Box(int) 委派 Box(int,int,int) 其工作：
...
在任何建構函式完成時，建構函式建立的物件會立即完全初始化。 如需詳細資訊，請參閱[統一初始設定和委派建構函式](../cpp/uniform-initialization-and-delegating-constructors.md)。
