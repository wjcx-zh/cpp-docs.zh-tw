...
嚴格來說，此C++語言沒有內建的字串類型。**char**和**wchar_t**儲存單一字元-您必須宣告這些類型的陣列，以近似字串，並將結束的 null 值（例如，ASCII `'\0'`）新增至最後一個有效字元之後的陣列元素（也稱為*C 樣式字串*）。 C-Style 字串需要撰寫更多程式碼或使用外部字串公用程式庫函式。 但是在現代化C++中，我們有標準程式庫類型 `std::string` （適用于8位**char**類型字元字串）或 `std::wstring` （適用于16位**wchar_t**類型的字元字串）。 這些C++標準程式庫容器可以視為原生字串類型，因為它們是包含在任何符合規範C++之組建環境中的標準程式庫的一部分。 只要使用 `#include <string>` 指示詞，即可在您的程式中使用這些類型。 （如果您使用 MFC 或 ATL，CString 類別也可以使用，但不是C++標準的一部分）。強烈建議您不要在新式C++中使用以 null 終止的字元陣列（先前提到的 C 樣式字串）。
...
更正的程式碼範例會使用本機堆疊記憶體，建立 `pNumber` 所指向的備份存放區。 我們為了簡單起見使用基本類型。 實際上，指標的備份存放區最常是使用者定義的型別，這些型別是以**新**的關鍵字運算式（在 C 樣式的程式設計中，舊版 @no_，以動態方式配置在稱為*堆積*的記憶體區域中已使用 _t_3 C 執行時間程式庫函式）。 一旦配置之後，這些變數通常稱為「物件」，特別是當它們是以類別定義為基礎時。 使用**new**所配置的記憶體，必須由對應的**delete**語句刪除（或者，如果您使用 `malloc()` 函式來配置它，C 執行時間函式會 `free()`）。
