...
**Void**類型是一種特殊類型;您不能宣告**void**類型的變數，但是可以__宣告 void 類型\*的變數（__ **void**的指標），這在配置原始（不具類型）的記憶體時有時是必要的。 不過， **void**的指標並不是型別安全的，而且通常在現代化C++中不鼓勵使用。 在函式宣告中， **void**傳回值表示函數不會傳回值。這是一種常見且可接受的**void**用法。 雖然 C 語言所需的函式具有零個參數，以在參數清單中宣告**void** （ `fou(void)`例如），但這種作法C++不鼓勵現代化， `fou()`而且應該進行宣告。 如需詳細資訊，請參閱[類型轉換和型別安全](../cpp/type-conversions-and-type-safety-modern-cpp.md)。
...
在 C 語言的最早期版本中，C++ 繼續讓您使用特殊宣告子 `*` (星號) 宣告指標類型的變數。 指標類型會將在儲存實際資料值之位置的位址儲存在記憶體中。 在現代化C++中，這些稱為*原始指標*，可透過特殊運算子`*` （星號）或`->` （使用大於的虛線）在您的程式碼中存取。 這稱為「取值 *」，而*您使用哪一個，取決於您要對純量的指標或物件中成員的指標取值。 處理指標類型一直以來都是 C 及 C++ 程式開發最具挑戰性和令人困惑的一面。 本章節會概述一些事實和作法，以協助使用原始指標（如果您想要的話C++ ），但在現代化的情況下，由於[智慧型指標](../cpp/smart-pointers-modern-cpp.md)的演進，已不再需要（或建議）使用原始指標來取得物件擁有權（已討論更多）在本節結尾）。 使用原始指標觀察物件仍然有用且安全，但若要將其用於物件擁有權，就必須小心使用，並非常仔細地考量如何建立和終結所擁有的物件。
...
更正的程式碼範例會使用本機堆疊記憶體，建立 `pNumber` 所指向的備份存放區。 我們為了簡單起見使用基本類型。 實際上，指標的備份存放區最常是使用者定義的型別，這些型別是使用**新**的關鍵字*運算式（在*C 樣式的程式設計中，是 `malloc()`已使用 C 執行時間程式庫函式）。 一旦配置之後，這些變數通常稱為「物件」，特別是當它們是以類別定義為基礎時。 以**new**配置的記憶體必須由對應的**delete**語句刪除（或者，如果`malloc()`您使用函式來配置`free()`它，則為 C 執行時間函式）。
