...
ms.locfileid: "69498635"
---
# <a name="c-type-system-modern-c"></a>C++ 類型系統 (現代 C++)

*類型*的概念在中C++非常重要。 每個變數、函式引數和函式傳回值都必須有類型才能編譯。 此外，在評估之前，編譯器會以隱含的方式指定每一個運算式 (包括常值) 的類型。 某些類型的範例包括用來儲存整數值的**int** 、用來儲存浮點值的**雙精度**浮點數 (也稱為純量資料類型), 或標準程式庫類別[std:: basic_string](../standard-library/basic-string-class.md)來儲存文字。 您可以藉由定義**類別**或**結構**來建立自己的類型。 此類型會指定配置給變數 (或運算式結果) 的記憶體數量、可在該變數中存放的值種類、這些值 (位元模式) 的解譯方式，以及可對其執行的作業。 本文包含 C++ 類型系統主要功能的簡略概觀。
...
**POD 類型**(一般舊資料):在中C++ , 這個非正式的資料類型類別是指純量類型 (請參閱基本類型一節) 或*POD 類別*。 POD 類別的靜態資料成員同時也是 POD，沒有使用者定義的建構函式、使用者定義解構函式或使用者定義的指派運算子。 此外，POD 類別沒有虛擬函式、沒有基底類別，也沒有私用或受保護的非靜態資料成員。 POD 類型通常用於外部資料交換，例如以 C 語言撰寫的模組 (其中只有 POD 類型)。
...
![&#45;]內建類型的大小 (以位元組為單位) 內建類型的(../cpp/media/built-intypesizes.png "大小 (以位元組為單位)&#45; ")

下表列出最常用的基本類型：

|類型|大小|註解|
|----------|----------|-------------|
|ssNoversion|4 個位元組|整數值的預設選項。|
...
|不&nbsp;帶正負號的字元|1 個位元組|C++ 沒有內建 `byte` 類型。  使用 unsigned char 表示位元組值。|
|unsigned int|4 個位元組|位元旗標的預設選項。|
|long long|8 個位元組|表示極大的整數值。|

## <a name="the-void-type"></a>void 類型

**Void**類型是一種特殊類型;您不能宣告**void**類型的變數, 但是可以宣告 void 類型 __\*__ 的變數 ( **void**的指標), 這在配置原始 (不具類型) 的記憶體時有時是必要的。 不過, **void**的指標並不是型別安全的, 而且通常在現代化C++中不鼓勵使用。 在函式宣告中, **void**傳回值表示函數不會傳回值。這是一種常見且可接受的**void**用法。 雖然 C 語言所需的函式具有零個參數, 以在參數清單中宣告**void** ( `fou(void)`例如), 但這種作法C++不鼓勵現代化, `fou()`而且應該進行宣告。 如需詳細資訊, 請參閱[類型轉換和型別安全](../cpp/type-conversions-and-type-safety-modern-cpp.md)。
...
**Const**類型有別于其非 const 版本;例如, **const int**是**int**的相異類型。當您必須從C++變數中移除*常數性質*時, 可以在很罕見的情況下使用**const_cast**運算子。 如需詳細資訊, 請參閱[類型轉換和型別安全](../cpp/type-conversions-and-type-safety-modern-cpp.md)。

## <a name="string-types"></a>字串類型

嚴格來說, 此C++語言沒有內建的字串類型。**char**和**wchar_t**儲存單一字元-您必須宣告這些類型的陣列, 以近似字串, 並將終止的 null 值 (例如 ASCII `'\0'`) 新增至最後一個有效字元之後的陣列元素 (也稱為*C 樣式字串*)。 C-Style 字串需要撰寫更多程式碼或使用外部字串公用程式庫函式。 但是在現代化C++的中, 我們有標準連結`std::string`庫類型 (適用于8位**char**類型的字元字串`std::wstring` ) 或 (適用于16位**wchar_t**類型的字元字串)。 這些C++標準程式庫容器可以視為原生字串類型, 因為它們是包含在任何符合規範C++之組建環境中的標準程式庫的一部分。 只要使用 `#include <string>` 指示詞，即可在您的程式中使用這些類型。 (如果您正在使用 MFC 或 ATL，CString 類別也是可用的，但是它不是 C++ 標準的一部分)。強烈建議您不要在現代 C++ 中使用以 null 終止的字元陣列 (舊稱 C-Style 字串)。

## <a name="user-defined-types"></a>使用者定義類型

當您定義**類別**、**結構**、等位或**列舉**時, 該結構會在程式碼的其餘部分用來當做基本類型。 它在記憶體中的大小已知，而且套用了有關其在編譯時間檢查、執行階段和程式存留期之使用方式的特定規則。 基本內建類型和使用者定義類型之間的主要差異如下：
...
在 C 語言的最早期版本中，C++ 繼續讓您使用特殊宣告子 `*` (星號) 宣告指標類型的變數。 指標類型會將在儲存實際資料值之位置的位址儲存在記憶體中。 在現代化C++中, 這些稱為*原始指標*, 可透過特殊運算子`*` (星號) 或`->` (使用大於的虛線) 在您的程式碼中存取。 這稱為「取值」, 而您使用哪一個, 取決於您要對純量的指標或物件中成員的指標取值。 處理指標類型一直以來都是 C 及 C++ 程式開發最具挑戰性和令人困惑的一面。 本章節會概述一些事實和作法, 以協助使用原始指標 (如果您想要的話C++ ), 但在現代化的情況下, 由於[智慧型指標](../cpp/smart-pointers-modern-cpp.md)的演進, 已不再需要 (或建議) 使用原始指標來取得物件擁有權 (已討論更多)在本節結尾)。 使用原始指標觀察物件仍然有用且安全，但若要將其用於物件擁有權，就必須小心使用，並非常仔細地考量如何建立和終結所擁有的物件。
...
更正的程式碼範例會使用本機堆疊記憶體，建立 `pNumber` 所指向的備份存放區。 我們為了簡單起見使用基本類型。 實際上, 指標的備份存放區最常是使用者定義的型別, 這些型別是使用**新**的關鍵字運算式 (在 C 樣式的程式設計中, 是 `malloc()`已使用 C 執行時間程式庫函式)。 一旦配置之後, 這些變數通常稱為「物件」, 特別是當它們是以類別定義為基礎時。 以**new**配置的記憶體必須由對應的**delete**語句刪除 (或者, 如果`malloc()`您使用函式來配置`free()`它, 則為 C 執行時間函式)。
