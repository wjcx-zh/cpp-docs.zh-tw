...
ms.locfileid: "70060145"
---
# <a name="destructors-c"></a>解構函式 (C++)

「析構函式」是一種成員函式, 會在物件超出範圍時自動叫用, 或由呼叫**刪除**來明確終結。 析構函式的名稱與類別相同, 前面加上波狀符號`~`()。 例如，`String` 類別的解構函式宣告為：`~String()`。
...
在上述範例中, 此析構`String::~String`函式會使用**delete**運算子來解除配置動態配置給文字儲存區的空間。
...
- 不能宣告為**const**、 **volatile**或**static**。 不過, 它們可以在被宣告為**const**、 **volatile**或**static**的物件遭到破壞時叫用。
...
- 使用**new**運算子配置的物件會使用**delete**明確解除配置。
...
![顯示虛擬基類的繼承圖形](../cpp/media/vc392j1.gif "顯示虛擬基類的繼承圖形") <br/>
顯示虛擬基底類別的繼承圖形
...
明確呼叫解構函式不是必要的步驟。 不過，這對放置於絕對位址的物件執行清除作業可能會很有用。 這些物件通常會使用接受位置引數的使用者定義**new**運算子進行配置。 **Delete**運算子無法解除配置此記憶體, 因為它不是從免費儲存區配置的 (如需詳細資訊, 請參閱[New 和 delete 運算子](../cpp/new-and-delete-operators.md))。 不過，呼叫解構函式時，可以執行適當的清除作業。 若要明確呼叫物件的解構函式 (`s` 類別的 `String`)，請使用下列其中一種陳述式：
...
在下一個範例中, 隱含產生的複製程式將會建立`str1.text`指標`str2.text`並參考相同的記憶體, 而當我們從`copy_strings()`傳回時, 該記憶體會被刪除兩次, 這是未定義的行為:
...
[複製建構函式和複製指派運算子](../cpp/copy-constructors-and-copy-assignment-operators-cpp.md)</br>
[移動建構函式和移動指派運算子](../cpp/move-constructors-and-move-assignment-operators-cpp.md)
