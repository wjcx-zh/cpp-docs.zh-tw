...
```
通知編譯器，函式不會擲回任何例外狀況。 However, in **/std:c++14** mode this could lead to undefined behavior if the function does throw an exception. Therefore we recommend using the [noexcept](../cpp/noexcept-cpp.md) operator instead of the one above:
...
C 函式不允許明確例外狀況規格。 A C function is assumed not to throw exceptions under **/EHsc**, and may throw structured exceptions under **/EHs**, **/EHa**, or **/EHac**.
...
|C++ function with `noexcept`, `noexcept(true)`, or `throw()` exception specification|否|否|[是]|[是]|
