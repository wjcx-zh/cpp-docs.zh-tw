...
ms.locfileid: "58565970"
---
# <a name="function-overloading"></a>函式多載

C++ 允許在相同範圍內指定多個同名的函式。 這些函式會呼叫*多載*函式。 多載函式可讓您提供不同的語意是函式，根據型別和引數數目。

例如，`print`函式接受`std::string`引數可能會執行非常不同的工作，比另一個會採用類型的引數**double**。 多載可讓您不必使用名稱，例如`print_string`或`print_double`。 在編譯時期，編譯器會選擇要使用哪個多載會根據傳入的呼叫端的引數的類型。  如果您呼叫`print(42.0)`，則`void print(double d)`函式會叫用。 如果您呼叫`print("hello world")`，然後在`void print(std::string)`多載會叫用。
...
|[Ref-qualifier](#ref-qualifiers)|是|
...
預設引數不是被視為函式類型的一部分。 因此，它不會在選取的多載函式。 兩個函式的不同之處只有在其預設引數是視為多重定義而不是多載函式。

無法為多載提供預設引數。

## <a name="argument-matching"></a>引數對應

多載函式會在為了找出目前範圍中最符合函式呼叫所提供引數的函式宣告項目時選取。 如果找到適合的函式，就會呼叫該函式。 在此內容中，「 適合 」 表示其中一個：
...
每個引數都會有一組建置的「最相符函式」，而且選取的函式是所有集合的交集。 如果交集包含多個函式，則多載會變得模稜兩可並產生錯誤。 最後選取的函式對於至少一個引數來說，永遠是比群組中其他每一個函式更理想的相符項目。 如果沒有任何明確的得獎者，則函式呼叫會產生錯誤。
...
|設定 1:第一個引數為 Fraction 類型的候選函式|設定 2:候選函式的第二個引數可以轉換成類型**int**|
|--------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
|Variant 1|Variant 1 (**int**可以轉換成**長**使用標準轉換)|
|Variant 3||

集合 2 中的函式會包含從實質參數類型的隱含轉換成正式參數類型，而且這類函式之間沒有實際的參數類型轉換為其型式參數類型的 「 成本 」 的函式最小。
...
|設定 1:候選函式，具有第一個引數的型別**int**|設定 2:候選函式，具有第二個引數的型別**int**|
|---------------------------------------------------------------------|----------------------------------------------------------------------|
|Variant 2 (**int**可以轉換成**長**使用標準轉換)|Variant 1 (**int**可以轉換成**長**使用標準轉換)|

因為這兩個集合的交集是空的則編譯器會產生一則錯誤訊息。

比對的函式的引數*n*預設引數會被視為*n*+ 1 個別函式，各有不同數目的引數。

省略符號 (...) 做為萬用字元使用，它會比對任何實質引數。 它可能會導致許多模稜兩可的集合，如果您不要設計您的多載函式集，與必須特別小心。

> [!NOTE]
>  無法判斷模稜兩可的多載函式，直到遇到函式呼叫為止。 遇到函式呼叫時，會為函式呼叫中的每個引數建置集合，如此就可以判斷是否有明確的多載存在。 這表示，模稜兩可的情況可能會持續存在程式碼中，直到特定函式呼叫撤銷這些情況為止。
...
不過，函式多載機制可以區別會限定的參考**const**並**volatile**和基底類型的參考。 它會進行下列可能如下的程式碼：
...
第一個序列中，雖然它可以實現所需的目標，但不是最相符序列，較短的序列存在。
...
1. 完全相符。 呼叫函式所使用的類型與函式原型中所宣告的類型之間完全相符的項目，必定是最相符項目。 一般轉換的序列會分類為完全相符項目。 不過，不進行任何這些轉換的序列會被視為比轉換的序列更好：
...
從 `C*` 類型轉換成 `B*` 類型，比從 `C*` 類型轉換成 `A*` 類型更理想。 這是因為它們位於相同路徑上，且 `B*` 較接近。 不過，從類型轉換`C*`鍵入`D*`不是比轉換成輸入`A*`; 沒有偏好設定因為轉換遵循不同的路徑。
...
1. 使用省略符號的相符項目。 符合宣告中省略符號的任何序列都會分類為使用省略符號的相符項目。 它將其視為最弱的相符項目。
...
在上述範例中，使用者定義的轉換**運算子 long**，會叫用來轉換`udc`鍵入**長**。 如果沒有使用者定義的轉換成類型**長**已定義，則轉換會有繼續進行，如下所示：型別`UDC`會轉換為類型**int**使用使用者定義的轉換。 然後類型標準轉換**int**鍵入**長**會套用至符合宣告中的引數。

如果任何使用者定義的轉換所需的引數，評估最符合項目時，就不會使用標準轉換。 即使一個以上的候選函式需要使用者定義的轉換，則函式會視為相等。 例如: 
...
即使第二個需要標準轉換和使用者定義的轉換，這兩種轉換仍會視為相等。
...
- 類型"array of"和"pointer to"會被視為相同的目的區別多載的函式之間，但只適用於單一維度陣列。 這就是為什麼這些多載函式發生衝突，並產生錯誤訊息：
...
類別範圍會受到嚴密的觀察;因此，基底類別中宣告的函式不是在相同的範圍內，做為衍生類別中宣告。 如果在衍生類別中的函式宣告與基底類別，衍生類別函式中的虛擬函式同名*會覆寫*基底類別函式。 如需詳細資訊，請參閱 <<c0> [ 虛擬函式](../cpp/virtual-functions.md)。

如果基底類別函式未宣告為 'virtual'，則稱為衍生的類別函式*隱藏*它。 覆寫和隱藏會區別多載。

區塊範圍會受到嚴密的觀察;因此，在檔案範圍中宣告的函式不是在相同的範圍內，做為區域宣告。 如果本機宣告的函式與在檔案範圍中宣告的函式相同名稱，本機宣告的函式會隱藏檔案範圍涵式，而不會引發多載。 例如: 
...
這個範例的目的在於提供 `Account` 類別，在此類別中需要有正確的密碼才能執行儲放。 它是由使用多載。

若要在呼叫`Deposit`在`Account::Deposit`呼叫私用成員函式。 這個呼叫是正確因為`Account::Deposit`為成員函式，且可以存取類別的私用成員。
