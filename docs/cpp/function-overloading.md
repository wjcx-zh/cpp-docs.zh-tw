...
ms.locfileid: "80179779"
---
# <a name="function-overloading"></a>函式多載

C++ 允許在相同範圍內指定多個同名的函式。 這些*函數稱為多*載函式。 多載函式可讓您針對函式提供不同的語義，視引數的類型和數目而定。

例如，接受 `std::string` 引數的 `print` 函式，可能會執行與採用**double**類型之引數的工作非常不同。 多載可讓您不必使用 `print_string` 或 `print_double`之類的名稱。 在編譯時期，編譯器會根據呼叫者傳入的引數類型，選擇要使用的多載。  如果您呼叫 `print(42.0)`，則會叫用 `void print(double d)` 函數。 如果您呼叫 `print("hello world")`，則會叫用 `void print(std::string)` 多載。
...
|集合 1：第一個引數為 Fraction 類型的候選函式|Set 2：第二個引數可以轉換成**Int**類型的候選函式|
|--------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
|Variant 1|Variant 1 （**int**可以轉換成**long** ，使用標準轉換）|
|Variant 3||

Set 2 中的函式是指有從實際的參數類型隱含轉換成正式參數類型的函數，而在這類函數中，有一個函數可將實際參數類型轉換成其型式參數類型的「成本」為工資.
...
|Set 1：具有**Int**類型之第一個引數的候選函式|Set 2：具有**Int**類型之第二個引數的候選函式|
|---------------------------------------------------------------------|----------------------------------------------------------------------|
|Variant 2 （**int**可以轉換成**long** ，使用標準轉換）|Variant 1 （**int**可以轉換成**long** ，使用標準轉換）|
...
基於相同的理由， **const**或**volatile**所修改之類型的函數引數，不會因多載目的而以不同的基底類型來處理。

不過，函式多載機制可以區分**const**所限定的參考 **，以及基**底類型的參考。 它可以讓程式碼如下：
...
轉換產生的序列 (如果有的話) 稱為「最相符序列」(Best Matching Sequence)。 有數種方式可使用標準轉換，將**int**類型的物件轉換為不**帶正負**號的 Long 類型（如[標準轉換](../cpp/standard-conversions.md)中所述）：

- 從**int**轉換成**long** ，然後從**long**轉換成不**帶正負**號的 long。

- 從**int**轉換為不**帶正負**號的 long。
...
|*類型-名稱*|**const** *類型-名稱*|
|*類型-名稱*|**變動***類型名稱*|
|*類型名稱* __\*__|**const** *類型-名稱* __\*__|
|*類型名稱* __\*__|**變動***型別名稱* __\*__|
...
1. 完全相符。 呼叫函式所使用的類型與函式原型中所宣告的類型之間完全相符的項目，必定是最相符項目。 一般轉換的序列會分類為完全相符項目。 不過，未進行任何轉換的序列，會被視為優於轉換的序列：
   ...
   - 從指標到**volatile**的指標（`type` <strong>\*</strong>到**volatile** `type` <strong>\*</strong>）。

   - 從參考，到**const** （`type` **&** 到**const** `type` **&** ）的參考。

   - 從參考，到**volatile**的參考（`type` **&** 至**volatile** `type` **&** ）。

1. 使用提升的相符項目。 任何未分類為完全相符的序列，只包含整數提升、從**float**轉換為**double**，而一般轉換則是使用升級分類為相符項。 雖然不如任何完全相符項目一般合適，但是使用提升的相符項目與使用標準轉換的相符項目相比之下仍較為理想。
...
![顯示&#45;慣用轉換的多重繼承](../cpp/media/vc391t2.gif "顯示&#45;慣用轉換的多重繼承") <br/>
顯示慣用轉換的多重繼承圖形

從 `C*` 類型轉換成 `B*` 類型，比從 `C*` 類型轉換成 `A*` 類型更理想。 這是因為它們位於相同路徑上，且 `B*` 較接近。 不過，從類型 `C*` 轉換成類型 `D*` 不是比較理想的轉換成類型 `A*`;因為轉換會遵循不同的路徑，所以沒有任何喜好設定。
...
類別 `UDC` 可用的使用者定義轉換是來自類型**int**和**long**類型。 因此，編譯器會考慮所要比對之物件的類型轉換：`UDC`。 已有**int**的轉換，而且已選取它。
...
在上述範例中，會叫用使用者定義的轉換（ **operator long**），將 `udc` 轉換成**long**類型。 如果未定義任何使用者定義的類型**long**轉換，則轉換會繼續進行，如下所示：類型 `UDC` 會使用使用者定義的轉換，轉換成**int**類型。 然後，會套用從**int**類型到**long**類型的標準轉換，以符合宣告中的引數。
...
這兩個版本的 `Func` 都需要使用者定義的轉換，將**int**類型轉換成類別類型引數。 可能的轉換包括：

- 從**int**類型轉換成類型 `UDC1` （使用者定義的轉換）。

- 從**int**類型轉換成**long**類型;然後轉換成類型 `UDC2` （兩步驟的轉換）。
...
類別成員函式的處理方式不同，取決於它們是否宣告為**靜態**。 由於非靜態函式具有提供**this**指標的隱含引數，因此非靜態函式會被視為具有一個以上的引數，而不是靜態函數;否則，它們會宣告為相同。

這些非靜態成員函式要求隱含**this**指標符合呼叫函式所使用的物件類型，或者，如果是多載運算子，則會要求第一個引數必須符合套用運算子的物件。 （如需多載運算子的詳細資訊，請參閱多載[運算子](../cpp/operator-overloading.md)）。

不同于多載函式中的其他引數，不會引進任何暫存物件，也不會在嘗試比對**此**指標引數時嘗試轉換。

當 `->` 成員選取運算子用來存取類別 `class_name`的成員函式時，**這個**指標引數的類型會是 `class_name * const`。 如果成員宣告為**const**或**volatile**，則類型會分別 `const class_name * const` 和 `volatile class_name * const`。
...
Ref 限定詞可以讓成員函式根據**此**所指向的物件是右值或左值來多載。  這項功能可在您選擇不提供資料指標存取的情況下，用來避免不必要的複製作業。 例如，假設 class `C` 會在其函式中初始化一些資料，並在成員函式 `get_data()`中傳回該資料的複本。 如果 `C` 類型的物件是即將終結的右值，則編譯器會選擇 `get_data() &&` 多載，這會移動資料而不復制它。
...
- **typedef**宣告不會定義新的類型;它們會為現有的類型引進同義字。 它們不會影響多載機制。 請考慮下列程式碼：
   ...
   上述兩個函式擁有相同的引數清單。 `PSTR` 是類型 `char *`的同義字。 在成員範圍內，這個程式碼會產生錯誤。
...
上述程式碼會顯示函式 `func` 的兩個定義。 採用類型之引數的定義 `char *` 是本機的 `main` 因為**extern**語句。 因此，會隱藏採用**int**類型之引數的定義，而第一次呼叫 `func` 會發生錯誤。
...
呼叫 `Account::Deposit` 中的 `Deposit` 會呼叫私用成員函式。 這個呼叫是正確的，因為 `Account::Deposit` 是成員函式，而且可以存取類別的私用成員。
