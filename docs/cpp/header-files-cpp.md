...
下列範例列出用於宣告類別，並在不同來源檔案中使用它的常見方式。我們將會從標頭檔 `my_class.h` 開始。它包含類別定義，但請注意，定義是不完整的；未定義 `do_something` 成員函式：
...
現在我們可以在其他 .cpp 檔案使用 `my_class`。我們會 #include 標頭檔，以讓編譯器提取宣告。編譯器必須知道 my_class 是一個類別，它包含一個稱為 `do_something()` 的公用成員函式。
...
因為標頭檔可能會被包含在多個檔案內，因此它不能包含可能產生多個相同名稱之定義的定義。下列所述是不允許，或被視為非常糟糕的作法：
...
使用 **using** 指示詞不一定會造成錯誤，但可能會因為它將命名空間帶入每個直接或間接包含該標頭的 .cpp 檔案範圍，而導致問題。

## <a name="sample-header-file"></a>範例標頭檔

下列範例顯示標頭檔中允許的各種宣告和定義：
