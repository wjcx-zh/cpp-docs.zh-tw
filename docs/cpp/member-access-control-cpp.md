...
存取控制項可讓您區隔類別的 [public](../cpp/public-cpp.md) 介面與 [private](../cpp/private-cpp.md) 實作詳細資料以及僅供衍生類別使用的 [protected](../cpp/protected-cpp.md) 成員。  除非發現下一個存取規範，否則存取規範會套用至在其後宣告的所有成員。  
...
|[public](../cpp/public-cpp.md)|任何函式都可以使用宣告為 **public** 的類別成員。|  
...
-   衍生類別是否在 **class\-head** \(*class\-head* 將於*定義類別類型*中的「文法」一節中說明\) 中使用 [public](http://msdn.microsoft.com/zh-tw/e8c65425-0f3a-4dca-afc2-418c3b1e57da) 存取指定名稱宣告基底類別。  
...
```  
 ...
 您可以在未使用基底類別存取指定名稱的情況下宣告衍生類別。  在這種情況下，如果使用 **class** 關鍵字宣告衍生類別，則會將該衍生視為 private。  如果衍生類別宣告使用 `struct` 關鍵字，則會將該衍生視為 public。  例如，下列程式碼：  
...
## 存取虛擬函式  
 套用至[虛擬](../cpp/virtual-cpp.md)函式的存取控制是由進行函式呼叫的類型所決定。  覆寫函式的宣告不會影響特定類型的存取控制。  例如:  
...
## 具有多重繼承的存取控制  
 ...
 ![依序繼承圖形的路徑進行存取](../cpp/media/vc38v91.png "vc38V91")  
依序繼承圖形的路徑進行存取  
