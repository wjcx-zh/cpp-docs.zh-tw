...
如[多重繼承](http://msdn.microsoft.com/zh-tw/3b74185e-2beb-4e29-8684-441e51d2a2ca)中所述，類別可以從多個基底類別衍生。  在多重繼承模型中 \(其中類別是從多個基底類別衍生\)，基底類別是使用 *base\-list* 文法項目所指定 \(請參閱[概觀](../misc/overview-of-derived-classes.md)中的＜文法＞一節\)。  例如，可以指定衍生自 `CollectionOfBook` 和 `Collection` 的 `Book` 類別宣告：  
...
-   建構函式進行初始化的順序。  如果程式碼要求 `Book` 的 `CollectionOfBook` 部分必須在 `Collection` 部分之前初始化，則指定的順序就很重要。  初始化會依照在 *base\-list* 中指定類別的順序進行。  
...
## 虛擬基底類別  
 ...
 ![模擬的午餐流程圖形](../cpp/media/vc38xp1.png "vc38XP1")  
模擬的午餐線圖表  
 ...
 ![模擬的午餐流程物件](../cpp/media/vc38xp2.png "vc38XP2")  
模擬的午餐線物件  
...
```  
 ...
 ![模擬的午餐流程物件和虛擬基底類別](../cpp/media/vc38xp3.png "vc38XP3")  
含有虛擬基底類別的模擬 Lunch\-Line 物件  
  
 一個類別可以擁有一個虛擬元件和一個特定類型的非虛擬元件。  此情況會發生在如下圖中示範的情況下。  
  
 ![類別的虛擬和非虛擬元件](../cpp/media/vc38xp4.png "vc38XP4")  
相同類別的虛擬與非虛擬元件  
 ...
 ![物件配置中的虛擬和非虛擬繼承](../cpp/media/vc38xp5.png "vc38XP5")  
具有虛擬和非虛擬繼承的物件配置  
...
2.  如果多載函式並非模稜兩可，就會進行解析   \(如需函式多載模稜兩可的詳細資訊，請參閱[引數比對](../misc/argument-matching.md)\)。  
...
### 語意模糊和虛擬基底類別  
 ...
 ![虛擬衍生和非虛擬衍生](../cpp/media/vc38xr1.png "vc38XR1")  
虛擬與非虛擬的衍生  
