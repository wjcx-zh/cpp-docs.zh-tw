...
|[明確類型轉換運算子](../cpp/explicit-type-conversion-operator-parens.md)|*型別名稱* **（)**|  
 ...
 *後置運算式*以上版本可能是主要運算式或其他後置運算式。  請參閱**主要運算式**。  後置運算式由左至右分組，因此可讓運算式鏈結在一起，如下所示：  
 ...
 *Cast 關鍵字*是其中一個`dynamic_cast`，`static_cast`或`reinterpret_cast`。  詳細資訊可能位於**dynamic_cast**， **static_cast**和**reinterpet_cast**。  
...
-   每個型式引數都會使用它在運算式清單中的對應實質引數初始化  (型式引數是在函式標頭中宣告並且在函式主體中使用的引數)。轉換就像是由初始化所完成，標準和使用者定義的轉換都會執行，將實質引數轉換成正確的類型。 所執行的初始化將以下列程式碼提供概念上的說明：  
    ...
    ```  
     ...
     請注意，初始化的執行方式就如同使用等號語法，而不是括號語法。 將值傳遞至函式之前，會先製作 `i` 的複本  (如需詳細資訊，請參閱[初始設定式](../cpp/initializers.md)和[轉換](../cpp/user-defined-type-conversions-cpp.md))。  
  
     因此，如果函式原型 （宣告） 呼叫類型的引數**長**，而且呼叫程式會提供類型的實質引數`int`，實際的引數使用標準類型轉換提升若要輸入**長**(請參閱[標準轉換](../cpp/standard-conversions.md))。  
  
     提供沒有轉換成型式引數類型之標準或使用者定義轉換的實質引數是不正確的做法。  
  
     對於類別類型的實質引數，型式引數會藉由呼叫類別的建構函式進行初始化。 (請參閱[建構函式](../cpp/constructors-cpp.md)如需有關這些特殊類別成員函式。)  
...
```  
 ...
 當`func`main，型式參數會從呼叫`param1`初始化的值`i`(`i`轉換為類型**長**以對應至正確的型別使用的標準轉換），和型式參數`param2`初始化其值為`j`(`j`轉換成型別**double**使用標準轉換)。  
  
## <a name="treatment-of-argument-types"></a>引數類型的處理方式  
 宣告為 const 類型的正式引數無法在函式主體內變更。 函式可變更的類型不是任何引數**const**。 不過，為函式的本機變更，而且除非實質引數不是型別物件的參考，不會影響實質引數的值**const**。  
...
-   如果實質引數是型別**float**，它會升級輸入**double**函式呼叫之前。  
  
-   任何帶正負號或不帶正負號`char`，**簡短**、 列舉型別，或位元欄位會轉換成帶正負號或不帶正負號`int`使用整數提升。  
  
-   任何類別類型的引數都會以傳值的方式做為資料結構傳遞，而複本會以二進位檔複製的方式建立，而不會以叫用類別之複製建構函式 (如果有的話) 的方式建立。  
  
 若使用省略符號，則必須是引數清單中最後宣告的項目。 如需傳遞可變引數數目的詳細資訊，請參閱的討論[va_arg、 va_start 和 va_list](../c-runtime-library/reference/va-arg-va-copy-va-end-va-start.md)中*執行階段程式庫參考*。  
  
 如需 CLR 程式設計中預設引數的資訊，請參閱[變數引數清單 （...）(C + + /CLI)](../windows/variable-argument-lists-dot-dot-dot-cpp-cli.md).  
  
 預設引數可讓您指定函式呼叫中未提供值時，引數應該假設的值。 下列程式碼片段將示範預設引數的運作方式。 如需指定預設引數有關限制的詳細資訊，請參閱[預設引數](../cpp/default-arguments.md)。  
...
```  
 ...
 上述程式會宣告接受兩個引數的函式 `print`。 不過，第二個引數 `terminator` 的預設值為 `"\n"`。 在**主要**前, 兩個呼叫`print`允許預設第二個引數提供新行字元以終止列印的字串。 第三個呼叫會為第二個引數指定明確的值。 程式的輸出為  
