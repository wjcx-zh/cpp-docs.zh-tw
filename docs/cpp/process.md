...
ms.locfileid: "34704902"
---
# <a name="process"></a>處理序

指定您的 Managed 應用程式處理序應該使用特定全域變數的單一複本、靜態成員變數，或在處理序中的所有應用程式定義域中共用的靜態區域變數。 這主要要與編譯時才能使用 **/clr: pure**，是 Visual Studio 2017 中已被取代，不支援的 Visual Studio 2017 中。 編譯時 **/clr**，全域和靜態變數是每個處理序預設並不需要使用`__declspec(process)`。
