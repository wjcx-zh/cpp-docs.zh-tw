...
-   大於或等於 (**>=**)  
 ...
 關係運算子具有由左到右的順序關聯性。 關係運算子的兩個運算元都必須是算術或指標類型。 這些類型會產生 `bool` 類型的值。 傳回的值為**false** (0)，若在運算式中的關聯性是 false，否則，傳回的值是**true** (1)。  
...
```  
 ...
 在上述範例中的運算式必須括在括號，因為資料流插入運算子 (**<<**) 的優先順序高於關係運算子。 因此，不加括弧的第一個運算式會評估為：  
...
## <a name="comparing-pointers"></a>比較指標  
 比較相同類型的兩個物件指標時，結果是取決於程式的位址空間中所指向的物件位置。 指標也可以與判斷值為 0 的常數運算式或 void * 類型的指標進行比較。 如果進行指標比較是針對類型的指標 void \*，另一個指標會隱含轉換成 void 類型\*。 然後才進行比較。  
  
 兩個不同類型的指標無法進行比較，除非：  
  
-   其中一個類型是衍生自另一個類型的類別類型。  
  
-   至少有一個指標已明確轉換 (轉型) 為 void * 類型  (另一個指標會隱含轉換成 void 類型\*來進行轉換。)  
  
 相同類型且指向相同物件之兩個指標的比較結果一定為相等。 如果將物件的兩個非靜態成員指標相互比較，則適用下列規則：  
  
-   如果類別類型不是等位，而且如果以不分隔的兩個成員*存取規範*，例如 public、 protected 或 private，宣告的成員指標最後將會比較大於宣告的成員指標稍早。  
  
-   如果兩個成員之間隔開*存取規範*，結果是未定義。  
