...
在「單一繼承」\(一種常見的繼承形式\) 中，任何類別都只能有一個基底類別。  請考慮下圖中說明的關係。  
 ...
 ![基本的單一繼承圖形](../cpp/media/vc38xj1.png "vc38XJ1")  
簡單的單一繼承圖形  
...
```  
 ...
 上述範例中使用存取規範 **public**。  public、protected 和 private 繼承的意義會在[成員存取控制](../cpp/member-access-control-cpp.md)中說明。  
  
 類別可做為許多特定類別的基底類別，如下圖所示。  
  
 ![有向非循環圖形](../cpp/media/vc38xj2.png "vc38XJ2")  
導向非循環圖的範例  
  
 上圖中 \(稱為「導向非循環圖」或 "DAG"\) 的某些類別是多個衍生類別的基底類別。  不過，反向並不成立：其中只有一個指定衍生類別的直接基底類別。  此圖中描述了一個「單一繼承」結構。  
  
> [!NOTE]
>  導向非循環圖不是唯一的單一繼承。  它們也可用來描述多重繼承圖形。  這個主題涵蓋在[多重繼承](http://msdn.microsoft.com/zh-tw/3b74185e-2beb-4e29-8684-441e51d2a2ca)中。  
