...
- [例外狀況處理常式](../cpp/writing-an-exception-handler.md)或 **__except**組塊, 這些區塊可以回應或關閉例外狀況。

- [終止處理常式](../cpp/writing-a-termination-handler.md)或一律會呼叫的 **__finally**區塊, 不論例外狀況是否會造成終止。
...
如果您使用 **/ehsc**來編譯此程式碼, 但未定義本機`CPPEX`測試控制項宏, 則不會執行`TestClass`此析構函式, 而且輸出看起來會像這樣:
...
如果您使用 **/ehsc**來編譯器代碼, 並`CPPEX`使用`/DCPPEX`定義 (因此會擲回C++例外狀況), 則`TestClass`會執行析構函式, 而且輸出看起來會像這樣:
...
如果您使用 **/eha**來編譯器代碼, 則不論`TestClass`是否使用`std::throw`來擲回例外狀況, 或使用 SEH 來觸發例外狀況 (也就是是否`CPPEX`已定義), 都會執行此析構函式。 輸出顯示如下：
