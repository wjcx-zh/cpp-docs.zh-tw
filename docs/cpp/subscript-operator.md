 ...
 *expression1* **[***expression2***] [***expression3***]**...  
 ...
 註標運算式的關聯是由左至右。 最左邊的下標運算式 *expression1 ***[*** expression2***]** 會最先評估。 *expression1* 和 *expression2* 相加所產生的位址會形成指標運算式，然後 *expression3* 會加入這個指標運算式形成新的指標運算式，依此類推，直到加入最後一個註標運算式為止。 間接取值運算子 (**\***) 之後，會套用最後一個註標的運算式評估時，除非最後一個指標值定址為陣列類型。  
...
```  
 ...
 註標運算子可以交替。 因此，運算式*陣列*[*index*] 和*陣列*[*陣列*] 並保證只要註標是對等運算子未多載 (請參閱[多載運算子](../cpp/operator-overloading.md))。 第一種形式是最常用的程式撰寫作法，但兩種都可以運作。  
