...
2.  當終止處理常式完成時之後, 會繼續執行 **__finally**陳述式。 不論如何保護區段結束 (例如，透過**goto**保護主體外部的或有**傳回**陳述式)，會執行終止處理常式*之前*控制流程移出保護區段。  
 ...
 ![終止的順序&#45;處理常式執行](../cpp/media/vc38cx1.gif "vc38CX1")  
終止處理常式的執行順序  
