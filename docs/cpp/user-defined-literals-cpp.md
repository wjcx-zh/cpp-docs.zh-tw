---
title: "使用者定義常值 (C++) | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
常值有六種主要分類：整數、字元、浮點、字串、布林值和指標。  從 C\+\+ 11 開始，您可以根據這些分類來定義專屬常值，以提供常見慣用語的語法捷徑，並提升類型安全。  例如，假設您有一個 Distance 類別。  您可以針對公里定義一個常值，以及針對英哩定義另一個常值，並鼓勵使用者只要撰寫下列項目來明確指定度量單位：auto d \= 42.0\_km 或 auto d \= 42.0\_mi。  使用者定義常值沒有任何效能優勢或缺點；它們主要是為了方便起見，或針對編譯時期類型推斷。  「標準程式庫」具有 std:string、std::complex 以及 \<chrono\> 標頭中時間和持續時間作業單位的使用者定義常值：  
...
## 處理後的常值  
 在原始程式碼中，任何常值 \(不論是否由使用者定義\) 基本上是一連串的英數字元 \(例如 `101` 或 `54.7`，或者 `"hello"` 或 `true`\)。  編譯器會將這一連串英數字元解譯為 integer、float、const char\* string 等。  接受編譯器指派給常值的類型做為輸入的使用者定義常值，非正式地稱為*「處理後的常值」*\(Cooked Literal\)。  所有上面的運算子 \(`_r` 和 `_t` 除外\) 都是處理後的常值。  例如，常值 `42.0_km` 會繫結至簽章與 \_b 類似的 \_km 運算子，常值 `42_km` 則繫結至簽章與 \_a 類似的運算子。  
...
```  
 ...
 請注意，常值數字必須使用小數點，否則數字會解譯為整數，而類型會與運算子不相容。  也請注意，浮點輸入的類型必須是 `long double`，而整數類型必須是 `long long`  
