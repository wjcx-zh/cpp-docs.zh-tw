...
除非例外狀況是整數或浮點溢位，否則這個處理常式會將控制項傳遞至另一個處理常式。 如果是，處理常式會呼叫函式 (`ResetVars` 只是範例，不是應用程式開發介面函式) 重設部分全域變數。 *語句-block-2*, 在此範例中是空的, 永遠不會執行`Eval_Exception` , 因為絕不會傳回 EXCEPTION_EXECUTE_HANDLER (1)。
...
當您想要讓篩選準則評估為-1, EXCEPTION_CONTINUE_EXECUTION 時, 條件運算子會更有用。
