...
> [!NOTE]
>  某些例外狀況無法繼續執行。 如果*篩選*評估為-1 的這類例外狀況，則系統會引發新的例外狀況。 當您呼叫[RaiseException](http://msdn.microsoft.com/library/windows/desktop/ms680552)，您判斷例外狀況是否會繼續。  
  
 例如，下列程式碼會使用函式呼叫中的*篩選*運算式： 這個函式會處理問題，然後傳回 – 1，繼續進行控制項的正常流程：  
...
```  
 ...
 條件運算子是在您想要評估為-1，EXCEPTION_CONTINUE_EXECUTION 的篩選條件的情況下更有用。  
