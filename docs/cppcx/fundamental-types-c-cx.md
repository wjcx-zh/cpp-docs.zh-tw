...
除了標準C++內建類型， C++/CX 支援型別系統所提供的 typedef，Windows 執行階段基本類型對應至標準的 Windows 執行階段的架構定義C++類型... C++/CX 實作布林值、 字元和數值基本類型。 這些 Typedef 在 `default` 命名空間中已定義，永遠不需要明確指定。 此外， C++/CX 提供特定的 632Windows 執行階段型別和介面的包裝函式和具象實作。
...
|int64|64 位元帶正負號的整數。|long long  -or- __int64|-9,223,372,036,854, 775,808 透過 9,223,372,036,854,775,807|
