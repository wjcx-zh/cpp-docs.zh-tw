...
## <a name="agileagile-destructor"></a><a name="dtor"></a> Agile：： ~ Agile 函式
...
傳回值的類型實際上是不公開的內部類型。 保存傳回值的便利方法，是將它指派給以類型推算關鍵字宣告的變數 **`auto`** 。 例如，`auto x = myAgileTvariable->Get();`。
...
這個作業釋放 `T` 類型物件的目前表示 (如果有的話)、重新初始化 Agile 物件的資料成員、取得目前執行緒內容，然後傳回可以代表非 Agile 物件的物件控制代碼變數位址。 若要讓 Agile 類別實例表示物件，請使用指派運算子 ([agile：： operator =](#operator-assign)) 將物件指派給 agile 類別實例。
...
這個運算子實際傳回保密的內部類型。 保存傳回值的便利方法，是將它指派給以類型推算關鍵字宣告的變數 **`auto`** 。
