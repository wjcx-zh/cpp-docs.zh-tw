...
編譯器產生函式可驗證的型別*K*和*V*如果該類型無法儲存在對應中會提供易懂的錯誤訊息。
...
`true` 如果目前 Map 中現有項目的機碼符合*金鑰*，而且該項目的值部分設定為*值*。 `false` 如果目前 Map 中的沒有任何現有項目符合*金鑰*和*金鑰*和*值*參數會成為索引鍵-值組，而且接著加入目前 Map。
