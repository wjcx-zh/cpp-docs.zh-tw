---
title: "執行緒和封送處理 (C++/CX) | Microsoft Docs"
ms.custom: ""
ms.date: "12/30/2016"
...
---
# 執行緒和封送處理 (C++/CX)
在大部分情況下，您都可以從任何執行緒存取 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] 類別的執行個體 \(例如標準 C\+\+ 物件\)， 這類類別都稱為 "Agile"。 不過，有一小部分 Windows 隨付的 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] 類別不是 Agile，而且必須要當做 COM 物件 \(而非標準 C\+\+ 物件\) 般使用。 您不必非常了解 COM 才能使用非 agile 的類別，但是必須將類別的執行緒模型及其封送處理行為納入考量。 針對這些必須使用非 Agile 類別之執行個體的少見情況，本文將提供背景知識和指引供您參考。  
  
## 執行緒模型和封送處理行為  
 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]類別可透過不同的方式支援並行執行緒存取，這可由它所套用的兩個屬性來指定：  
...
## 使用 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]元件  
 當您建立 [!INCLUDE[win8_appname_long](../cppcx/includes/win8-appname-long-md.md)] 應用程式時，您可能會同時與 Agile 和非 Agile 元件互動。 當您與非 Agile 元件互動時，可能會出現下列警告。  
  
### 使用非 Agile 類別時的編譯器警告 \(C4451\)  
...
-   在類別範圍中宣告了非 Agile 變數，且使用的程式碼有可能挾帶指標，也就是未經正確的封送處理即將其用於不同的 Apartment。  
 ...
 如果您的情況不屬於上述兩者，則可以將包含類別標記為非 Agile。 換句話說，您應該只有在非 Agile 類別中直接保留非 Agile 物件，並且在 Agile 類別中透過 Platform::Agile\<T\> 保留非 Agile 物件。  
...
```  
 ...
 在 Visual C\+\+中，當您為封送處理行為是 "None" 的同處理序 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]類別建立參考時，編譯器會發出警告 C4451，但不建議您使用 `Platform::Agile<T>`。  除了此警告外，編譯器無法提供進一歩的協助，因此您必須正確使用類別，並確保您的程式碼只會從使用者介面執行緒呼叫 STA 元件，從背景執行緒呼叫 MTA 元件。  
  
## 撰寫 Agile [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]元件  
 當您在 [!INCLUDE[cppwrt_short](../cppcx/includes/cppwrt-short-md.md)] 中定義 ref 類別時，依預設它會是 Agile 類別，也就是說，此類別會具有 `ThreadingModel`\=Both 和 `MarshallingType`\=Agile。  如果您使用 [!INCLUDE[cppwrl](../cppcx/includes/cppwrl-md.md)]，則可以從 `FtmBase` \(它會使用 `FreeThreadedMarshaller`\) 進行衍生，使您的類別成為 Agile 類別。  如果您撰寫具有 `ThreadingModel`\=Both 或 `ThreadingModel`\=MTA 的類別，請確定該類別具備執行緒安全。 如需詳細資訊，請參閱[建立及使用物件 \(WRL\)](http://msdn.microsoft.com/zh-tw/d5e42216-e888-4f1f-865a-b5ccd0def73e)。  
  
 您可以修改 ref 類別的執行緒模型和封送處理行為。 但是，如果您進行變更而使其成為非 Agile 類別，您必須了解這些變更的相關影響。  
  
 下列範例說明如何將 `MarshalingBehavior` 和 `ThreadingModel` 屬性套用至 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]類別庫中的執行階段類別。 當應用程式使用 DLL 與 `ref new` 關鍵字啟動 `MySTAClass` 類別物件時，該物件將會在單一執行緒 Apartment 中啟動，且不支援封送處理。  
...
-   衍生類別中的 `ThreadingModel` 和 `MarshallingBehavior` 屬性值與基底類別中的值不相符。  
 ...
 協力廠商 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]元件所需的執行緒與封送處理資訊，會指定於該元件的應用程式資訊清單登錄資訊中。 建議您將所有的 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]元件設為 Agile。 這可確保用戶端程式碼能夠從應用程式中的任何執行緒呼叫您的元件，並且可改善這些呼叫的效能，因為這些都是沒有封送處理的直接呼叫。 如果您以這種方式撰寫類別，則用戶端程式碼無需 `Platform::Agile<T>` 即可使用您的類別。  
