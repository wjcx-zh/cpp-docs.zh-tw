...
## <a name="threading-model-and-marshaling-behavior"></a>執行緒模型和封送處理行為  
 Windows 執行階段類別可支援並行執行緒存取，以各種方式，由它所套用的兩個屬性來指定：  
...
## <a name="consuming-windows-runtime-components"></a>使用 Windows 執行階段元件  
 當您建立通用 Windows 平台應用程式時，您就可能會與 agile 和非 agile 元件互動。 當您與非 Agile 元件互動時，可能會出現下列警告。  
  
### <a name="compiler-warning-when-consuming-non-agile-classes-c4451"></a>使用非 Agile 類別時的編譯器警告 (C4451)  
...
-   在類別範圍中宣告了非 Agile 變數，且使用的程式碼有可能挾帶指標，也就是未經正確的封送處理即將其用於不同的 Apartment。  
 ...
 如果您的情況不屬於上述兩者，則可以將包含類別標記為非 Agile。 換句話說，您應該直接保留非 agile 物件，只能在非 agile 的類別，並透過 platform:: agile 保留非 agile 物件\<T > 在 agile 類別中。  
...
```  
 ...
 Visual c + + 中，當您建立的參考至具有封送處理行為是"None"的同處理序 Windows 執行階段類別編譯器發出警告 C4451，但不建議您考慮使用`Platform::Agile<T>`。  除了此警告外，編譯器無法提供進一歩的協助，因此您必須正確使用類別，並確保您的程式碼只會從使用者介面執行緒呼叫 STA 元件，從背景執行緒呼叫 MTA 元件。  
  
## <a name="authoring-agile-windows-runtime-components"></a>撰寫 agile Windows 執行階段元件  
 當您定義的 ref 類別在 C + + /CX 中，它會是 agile 依預設，也就是說，它有`ThreadingModel`= Both 和`MarshallingType`= Agile。  如果您使用 [!INCLUDE[cppwrl](../cppcx/includes/cppwrl-md.md)]，則可以從 `FtmBase`(它會使用 `FreeThreadedMarshaller`) 進行衍生，使您的類別成為 Agile 類別。  如果您撰寫具有 `ThreadingModel`=Both 或 `ThreadingModel`=MTA 的類別，請確定該類別具備執行緒安全。 如需詳細資訊，請參閱 [建立及使用物件 (WRL)](http://msdn.microsoft.com/en-us/d5e42216-e888-4f1f-865a-b5ccd0def73e)。  
  
 您可以修改 ref 類別的執行緒模型和封送處理行為。 但是，如果您進行變更而使其成為非 Agile 類別，您必須了解這些變更的相關影響。  
  
 下列範例示範如何套用`MarshalingBehavior`和`ThreadingModel`屬性，將 Windows 執行階段類別庫中的執行階段類別。 當應用程式使用 DLL 與 `ref new` 關鍵字啟動 `MySTAClass` 類別物件時，該物件將會在單一執行緒 Apartment 中啟動，且不支援封送處理。  
