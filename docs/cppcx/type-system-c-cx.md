...
使用 Windows 執行階段架構，您可以使用C++/CX，Visual Basic 中，視覺效果C#和 JavaScript 來撰寫應用程式和元件，直接存取 Windows API，並與其他 Windows 執行階段應用程式和元件相互操作。 通用 Windows 平台應用程式所撰寫的C++編譯為直接在 CPU 中執行的原生程式碼。 以 C# 或 Visual Basic 撰寫的通用 Windows 平台應用程式編譯成 Microsoft intermediate language (MSIL)，並在 common language runtime (CLR) 中執行。 在執行階段環境中，執行以 JavaScript 撰寫的通用 Windows 平台應用程式。 Windows 執行階段作業系統元件本身以C++和以機器碼形式執行。 所有這些元件和通用 Windows 平台應用程式直接透過 Windows 執行階段應用程式二進位介面 (ABI) 進行通訊。

若要啟用在現代化的 Windows 執行階段支援的C++慣用語，Microsoft 建立C++/CX。 C++/CX 提供內建的基底型別和實作的基本 Windows 執行階段類型，讓C++應用程式和元件跨 ABI 與使用其他語言所撰寫的應用程式進行通訊。 您可以使用任何 Windows 執行階段型別，，或建立類別、 結構、 介面和其他使用者定義的型別，可供其他通用 Windows 平台應用程式和元件。 通用 Windows 平台應用程式撰寫的C++/CX 也可以使用標準C++類別和結構只要他們沒有公用存取範圍。
...
當您編譯以撰寫的通用 Windows 平台應用程式C++，編譯器會產生可執行檔，以原生機器碼，並也會產生個別的 Windows 中繼資料 (.winmd) 檔案，包含公用的 Windows 執行階段的描述型別，包括類別、 結構、 列舉、 介面、 參數化的介面和委派。 中繼資料的格式類似 .NET Framework 組件中使用的格式。  在 C++ 元件中，.winmd 檔案只包含中繼資料，可執行程式碼位於另一個檔案中。 這是隨附於 Windows 的 Windows 執行階段元件的情況。 WinMD 檔案名稱必須符合原始程式碼中的根命名空間或為根命名空間的首碼。 (對於 .NET Framework 語言，.winmd 檔案包含程式碼和中繼資料，如同 .NET Framework 組件一樣)。
...
類型或方法是否出現在中繼資料中，取決於套用哪些存取範圍修飾詞。 必須在命名空間中宣告類型，且必須宣告為公用，類型才能為可見。 您的程式碼中，允許非公用 ref 類別做為內部協助程式類型，只是在中繼資料中不可見。 即使在公用 ref 類別中，所有成員也不一定都可見。 下表列出之間的關聯性C++的存取規範，在公用 ref 類別和 Windows 執行階段中繼資料可見性：
...
您可以使用 [ **物件瀏覽器** ] 來檢視 .winmd 檔案的內容。 隨附於 Windows 的 Windows 執行階段元件是 Windows.winmd 檔案中。 Default.winmd 檔案包含的基本類型，會在C++/CX 和 platform.winmd 包含來自 Platform 命名空間的其他型別。 根據預設，這三個.winmd 檔案包含在每個C++通用 Windows 平台應用程式的專案。
...
## <a name="windows-runtime-type-system-in-ccx"></a>中的 Windows 執行階段型別系統C++/CX

下列各節描述的主要功能的 Windows 執行階段類型系統和它們如何支援在C++/CX。
...
Windows 執行階段定義下列基本型別、 UInt8、 Int16、 UInt16、 Int32、 UInt32、 Int64、 UInt64、 單一、 Double、 Char16、 布林值和字串。 C++/CX uint16、 uint32、 uint64、 int16、 int32、 int64、 float32、 float64 和 char16 為其預設命名空間中支援的基本數值類型。 Boolean 和 String 也在 Platform 命名空間中定義。
...
Windows 執行階段支援任何型別的 1 維陣列。 不支援巢狀陣列。  在C++/CX，Windows 執行階段陣列以[platform:: array 類別](../cppcx/platform-array-class.md)。

如需詳細資訊，請參閱[Array 和 WriteOnlyArray](../cppcx/array-and-writeonlyarray-c-cx.md)

### <a name="ref-classes-and-structs"></a>Ref 類別與結構

Windows 執行階段類別在規劃C++/CX 為 ref 類別或 ref 結構表示，因為它們傳址方式複製。 ref 類別和 ref 結構的記憶體管理是由參考計數自動處理。 當物件的最後一個參考超出範圍時，就會終結物件。 ref 類別或 ref 結構可以：
...
### <a name="windows-runtime-collections-in-ccx"></a>Windows 執行階段集合中的C++/CX
...
Windows 執行階段中的列舉類別類似於在限定範圍的列舉C++。 基礎類型是 int32，但如果套用 [Flags] 屬性，則基礎類型是 uint32。

如需詳細資訊，請參閱 [列舉](../cppcx/enums-c-cx.md)中定義的介面的私用 C++ 專屬實作。

### <a name="delegates"></a>委派

在 Windows 執行階段委派是類似於 std:: function 物件C++。 這是特殊類型的 ref 類別，用來叫用用戶端提供的函式 (具有相容簽章)。  Windows 執行階段中最常使用委派做為事件的類型。
