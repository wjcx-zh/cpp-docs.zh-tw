...
名稱                                     | 描述
---------------------------------------- | -------------------------------------------------------------------------------------------------------------
[HString::Attach](#attach)               | 將指定`HString`的物件與目前`HString`的物件產生關聯。
[HString::CopyTo](#copyto)               | 將目前`HString`的物件複製到 HSTRING 物件。
[HString::Detach](#detach)               | 將指定`HString`的物件與其基礎值解除對應。
...
[HString::IsValid](#isvalid)             | 指出目前`HString`的物件是否有效。
[HString:: MakeReference](#makereference) | 從指定的字串參數建立物件。`HStringReference`
[HString::Release](#release)             | 刪除基礎字串值, 並將目前`HString`的物件初始化為空值。
...
名稱                                         | 描述
-------------------------------------------- | ----------------------------------------------------------------------------
[HString::operator=](#operator-assign)       | 將另一個`HString`物件的值移至目前`HString`的物件。
...
將指定`HString`的物件與目前`HString`的物件產生關聯。
...
第二個函式會`HString`將新的物件初始化為現有*其他*參數的值, 然後再終結*另*一個參數。

## <a name="isvalid"></a>HString::IsValid

指出目前`HString`的物件是否為空白。
...
如果目前`HString`的物件不是空的, 則為 true, 否則為**false**。

## <a name="makereference"></a>HString:: MakeReference

從指定的字串參數建立物件。`HStringReference`
...
物件, 其值與指定的 str 參數相同。 `HStringReference`

## <a name="operator-assign"></a>HString:: operator = 運算子

將另一個`HString`物件的值移至目前`HString`的物件。
...
現有*其他*物件的值會複製到目前`HString`的物件, 然後再終結*另*一個物件。
...
刪除基礎字串值, 並將目前`HString`的物件初始化為空值。
