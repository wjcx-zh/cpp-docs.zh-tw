...
`SyncLockWithStatusT`類別來實作[Mutex](mutex-class.md)並[號誌](semaphore-class.md)類別。
...
以物件為基礎的等候作業的結果`SyncLockWithStatusT`類別，例如[Mutex](mutex-class.md)或是[號誌](semaphore-class.md)。 零 (0) 表示等待作業傳回信號的狀態;否則，另一個狀態發生，例如逾時值過。
