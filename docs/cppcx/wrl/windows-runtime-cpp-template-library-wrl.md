...
[如何：啟用和使用 Windows 執行階段元件](how-to-activate-and-use-a-windows-runtime-component-using-wrl.md)<br/>
...
會使用`HttpRequest`在逐步解說中[定義的類別:在完整 UWP 應用程式的內容](../../parallel/concrt/walkthrough-connecting-using-tasks-and-xml-http-requests.md)中使用工作和 XML HTTP 要求進行連接。
...
[DirectX 大理石迷宮遊戲範例](https://code.msdn.microsoft.com/windowsapps/DirectX-Marble-Maze-Game-e4806345)<br/>
...
[EventSource](eventsource-class.md) 用來管理「 *委派* 」(Delegate) 事件處理常式。 使用 Windows 執行階段C++範本庫來執行委派, 並使用來`EventSource`新增、移除及叫用委派。
...
|[快速參考 (Windows 執行階段和視覺C++效果)](../../cppcx/quick-reference-c-cx.md)|簡要描述支援C++Windows 執行階段的/cx 功能。|
