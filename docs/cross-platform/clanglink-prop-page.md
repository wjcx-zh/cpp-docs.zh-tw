...
| 回報未解析的符號參考 | 當啟用時，此選項會報告無法解析的符號參考。 |
| 最佳化記憶體的使用 | 視需要重新讀取符號表，最佳化記憶體的使用。 |
| 忽略特定的預設程式庫 | 指定要忽略的一或多個預設程式庫名稱。 |
| 強制符號參考 | 強制在輸出檔案中將符號輸入為未定義的符號。 |
| 偵錯工具符號資訊 | 來自輸出檔中的偵錯工具符號資訊。 | **包含全部**<br /><br />**省略不需要的符號以重新配置處理**<br /><br />**僅省略偵錯工具符號資訊**<br /><br />**省略所有符號資訊** |
| 封裝偵錯工具符號資訊 | 封裝之前刪除偵錯工具符號資訊。  原始二進位檔的複本會用於進行偵錯工具。 |
...
| 程式庫相依性 | 此選項可指定要新增至連結器命令列的其他程式庫。 額外的程式庫會新增至連結器命令列的結尾，並以*lib*開頭，並以 *. a*或結尾。*因此，* 副檔名為。  (-lFILE) |
