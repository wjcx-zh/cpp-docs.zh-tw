...
2.  在  **ODBC Microsoft Excel 設定**對話方塊方塊中，選取的資料庫版本和活頁簿。  
...
2.  而不是終止此物件，呼叫其`OpenEx`或`Open`成員函式一次。  
