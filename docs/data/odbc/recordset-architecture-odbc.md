...
> [!NOTE]
> 本主題適用於衍生自尚未實作大量資料列擷取之 `CRecordset` 的物件。 如果已實作大量資料列擷取，則架構會類似。 要瞭解差異,請參閱[記錄集:批量提取記錄 (ODBC)。](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)
...
> [!NOTE]
> 參數的順序很重要。 有關此項的資訊和有關參數的詳細資訊,請參閱[記錄集:參數化記錄集 (ODBC)。](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md)
...
有關詳細資訊和示例,請參閱[記錄欄位交換:使用 RFX](../../data/odbc/record-field-exchange-using-rfx.md)。
