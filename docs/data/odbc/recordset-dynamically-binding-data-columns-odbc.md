...
> [!NOTE]
>  本主題適用於衍生自 `CRecordset` 的物件，其中尚未實作大量資料列擷取。 如果您要使用大量資料列擷取，則不建議使用一般說明的技術。 如需大量資料列擷取的詳細資訊，請參閱[資料錄集：大量擷取記錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。
