...
3.  設定您需要例如排序、 鎖定的模式或參數的其他任何選項。 指定的參數就特別有用。 如需參數化篩選的資訊，請參閱[資料錄集： 參數化資料錄集 (ODBC)](../../data/odbc/recordset-parameterizing-a-recordset-odbc.md)。  
  
4.  呼叫`Open`新的物件 (或`Requery`先前開啟的物件)。  
