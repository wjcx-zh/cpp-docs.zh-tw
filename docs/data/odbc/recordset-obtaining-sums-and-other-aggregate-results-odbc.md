---
title: 資料錄集： 取得 Sum 和其他彙總的結果 (ODBC) |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-data
...
2.  修改[DoFieldExchange](../../mfc/reference/crecordset-class.md#dofieldexchange)函式資料錄集。 取代字串，表示資料行名稱 (第二個引數[RFX](../../data/odbc/record-field-exchange-using-rfx.md)函式呼叫) 表示的資料行的彙總函式的字串。 比方說，程式碼取代：  
...
3.  開啟資料錄集。 彙總作業的結果會留在`m_dblSumSales`。  
