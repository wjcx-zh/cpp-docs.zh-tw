...
*TAccessor*<br/>
您想要讓命令使用之存取子類別的類型（例如 `CDynamicParameterAccessor`、`CDynamicStringAccessor`或 `CEnumeratorAccessor`）。 預設值為 `CNoAccessor`，指定類別不支援參數或輸出資料行。

*TRowset*<br/>
您想要讓命令使用的資料列集類別類型（例如 `CArrayRowset` 或 `CNoRowset`）。 預設值為 `CRowset`。
...
當您執行命令時，您應該在命令之後同時呼叫 `Close` 和[ReleaseCommand](../../data/oledb/ccommand-releasecommand.md) 。

當您想要重複執行相同的命令時，您應該在呼叫 `Execute`之前，先呼叫 `Close` 來釋放每個結果集存取子。 在數列的結尾，您應該藉由呼叫 `ReleaseCommand`來釋放參數存取子。 另一個常見的案例是呼叫具有輸出參數的預存程式。 在許多提供者（例如 SQL Server 的 OLE DB 提供者）中，輸出參數值將無法存取，直到您關閉結果集存取子為止。 呼叫 `Close` 以關閉傳回的資料列集和結果集存取子，但不是參數存取子，因此可讓您取得輸出參數值。
...
*bBind*<br/>
在指定是否要在執行後自動系結命令。 預設值為**true**，這會自動系結命令。 將*bBind*設定為**false**可防止自動系結命令，讓您可以手動系結。 （對 OLAP 使用者而言，手動系結特別重要）。
...
如果先前已提取結果集，此函式會釋放先前的結果集，並將資料行解除系結。 如果*bBind*為**true**，它會系結新的資料行。

只有當您已藉由設定 `CCommand` 範本參數*TMultiple*=`CMultipleResults`來指定多個結果時，才應該呼叫此函數。
...
*wszCommand*<br/>
在要執行的命令，傳遞為 Unicode 字串。 使用 `CAccessor`時，可以是 Null，在此情況下，會從傳遞至[DEFINE_COMMAND](../../data/oledb/define-command.md)宏的值抓取命令。 如需詳細資訊，請參閱*OLE DB 程式設計人員參考*中的[ICommand：： Execute](/previous-versions/windows/desktop/ms718095(v=vs.85)) 。
...
*pRowsAffected*<br/>
[in/out]傳回受命令影響的資料列計數之記憶體的指標。 如果 *\*pRowsAffected*是 Null，則不會傳回任何資料列計數。 否則，`Open` 會根據下列條件設定 *\*pRowsAffected* ：

|If|結果為|
|--------|----------|
|`pParams` 的 `cParamSets` 元素大於1|*\*pRowsAffected*代表在執行中指定的所有參數集所影響的資料列總數。|
...
第二種形式的 `Open` 採用 ANSI 命令字串，而且沒有預設值（提供給與現有 ANSI 應用程式的回溯相容性）。

第三種形式的 `Open` 可讓命令字串成為 Null，因為**int**類型的預設值為 null。 它是為了呼叫 `Open(session, NULL);` 或 `Open(session);` 而提供，因為 Null 的類型為**int**。此版本需要，並判斷提示**int**參數為 Null。

當您已建立命令，而且您想要執行單一[準備](../../data/oledb/ccommand-prepare.md)和多個執行時，請使用第四種形式的 `Open`。

> [!NOTE]
>  `Open` 會呼叫 `Execute`，然後再呼叫 `GetNextResult`。
...
第一種形式的 `Create` 採用 Unicode 命令字串。 第二種形式的 `Create` 採用 ANSI 命令字串（提供給與現有 ANSI 應用程式的回溯相容性）。
...
`ReleaseCommand` 會與 `Close`搭配使用。 請參閱[關閉](../../data/oledb/ccommand-close.md)以取得使用方式詳細資料。
