---
title: 實作簡單唯讀提供者
ms.date: 10/26/2018
helpviewer_keywords:
- read-only provider
ms.assetid: 22a8adb5-19da-43b5-9733-0ea72bb8684c
ms.openlocfilehash: 7e63cd2bea0f0d13f6dc2bfff5c1be46cf0de116
ms.sourcegitcommit: 72583d30170d6ef29ea5c6848dc00169f2c909aa
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/18/2019
ms.locfileid: "59028726"
---
# <a name="implementing-the-simple-read-only-provider"></a>實作簡單唯讀提供者

本主題中的擴充的範例示範如何編輯精靈所建立的檔案，以建立簡單唯讀提供者，以從文字檔讀取一組兩個字串。 若要建立此提供者從精靈檔案，加入程式碼以完成下列工作：

- [將字串讀入的提供者](../../data/oledb/reading-strings-into-the-ole-db-provider.md)

- [將字串儲存在提供者](../../data/oledb/storing-strings-in-the-ole-db-provider.md)

## <a name="see-also"></a>另請參閱

[建立簡單唯讀提供者](../../data/oledb/creating-a-simple-read-only-provider.md)<br/>
