...
若要指定限制，請參閱[附錄 b:結構描述資料列集](/previous-versions/windows/desktop/ms712921(v=vs.85))並尋找您所使用的資料列集。 例如，`CColumns`對應至[COLUMNS 資料列集](/previous-versions/windows/desktop/ms723052(v=vs.85)\(v%3dvs.85\)); 該主題列出 COLUMNS 資料列集的限制資料行：TABLE_CATALOG 排列，TABLE_SCHEMA、 TABLE_NAME、 COLUMN_NAME。 您必須依照該順序指定您的限制。
