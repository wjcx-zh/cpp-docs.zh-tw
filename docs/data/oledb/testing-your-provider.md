...
3.  執行 OLE DB 一致性測試 （包括 ADO 一致性測試），以確保您的提供者符合 OLE DB 提供者層級 0 的標準。 (如需層級 0 的說明，搜尋 「 OLE DB 層級 0 一致性測試 」 在[OLE DB 程式設計人員指南](/previous-versions/windows/desktop/ms713643\(v=vs.85\))。 這些測試與相關聯的文件會包含使用 Data Access SDK 中的 Visual c + +。 這些測試也有助於確保您的提供者執行其他彙總時，也[服務提供者](../../data/oledb/ole-db-resource-pooling-and-services.md)時特別有用，如果您修改或新增屬性。 如需有關一致性測試的詳細資訊，請參閱 Data Access SDK，位於其中一個 Visual Studio Cd 上讀我檔案。  
