...
- **效能。** 在此案例中，一或多個資料行包含大量資料，例如圖形、 音訊或視訊檔。 每次您移至一個資料列，您可能不要擷取資料行與大型資料檔案，因為這麼做會降低您的應用程式效能。  
  
     您可以設定個別存取子中的第一個存取子包含大型的資料以外的所有資料行和它的資料會從擷取這些資料行，而這是自動存取子。 第二個存取子擷取包含大型的資料，資料行，但它不會不自動擷取資料從這個資料行。 您可以有其他更新，或依需求擷取大型資料的方法。  
...
- **多個的 ISequentialStream 資料行。** 在此案例中，您有多個資料行包含`ISequentialStream`資料。 不過，每個存取子僅限一份`ISequentialStream`資料流。 若要解決此問題，請設定數個存取子，每一個都包含一個`ISequentialStream`指標。  
...
- 在非自動存取子中擷取不會發生直到您明確地呼叫方法這類`Update`， `Insert`， `Fetch`，或`Delete`。 在上面所述的案例中，您可能不想要擷取在每次移動的所有資料行。 您可以將一或多個資料行放在不同的存取子，並將該有非自動存取子，如下所示。  
  
下列範例會使用多個存取子來讀取和寫入作業的資料庫資料表的 SQL Server pubs 使用多重存取子。 這是最常用的多個存取子;請參閱上述 「 多個讀取/寫入資料列集 」 案例。  
