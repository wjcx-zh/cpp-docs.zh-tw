...
|[copy (STL/CLR)](#copy)|將值從來源範圍複製到目的範圍，逐一查看正向方向。|  
|[copy_backward (STL/CLR)](#copy_backward)|將值從來源範圍複製到目的範圍，逐一查看反向方向。|  
|[count (STL/CLR)](#count)|傳回範圍中值符合指定值的項目數目。|  
|[count_if (STL/CLR)](#count_if)|傳回範圍中值符合指定條件的項目數目。|  
|[equal (STL/CLR)](#equal)|比較兩個範圍，逐一元件。|  
|[equal_range (STL/CLR)](#equal_range)|搜尋值的排序的順序，並傳回兩個分隔的所有指定的項目相等的值序列的位置。|  
|[fill (STL/CLR)](#fill)|將相同的新值指派到指定範圍內的每個項目。|  
|[fill_n (STL/CLR)](#fill_n)|將新值指派給範圍中以特定元素開頭的指定元素數目。|  
|[find (STL/CLR)](#find)|傳回指定值的第一次出現的位置。|  
|[find_end (STL/CLR)](#find_end)|傳回與指定序列相同範圍中的最後一個子序列。|  
|[find_first_of (STL/CLR)](#find_first_of)|搜尋第一個出現的任何一個指定的項目範圍的範圍。|  
|[find_if (STL/CLR)](#find_if)|傳回值序列中第一個項目的位置，其中的項目符合指定的條件。|  
|[for_each (STL/CLR)](#for_each)|適用於每個項目值序列中指定的函式物件，並傳回函式物件。|  
|[generate (STL/CLR)](#generate)|指派給每個元素值序列中的函式物件產生的值。|  
|[generate_n (STL/CLR)](#generate_n)|指派至指定的項目數函式物件產生的值。|  
|[includes (STL/CLR)](#includes)|測試一個排序的範圍是否包含第二個排序範圍內的所有項目。|  
|[inplace_merge (STL/CLR)](#inplace_merge)|將兩個連續排序範圍中的項目結合成單一排序範圍中。|  
|[iter_swap (STL/CLR)](#iter_swap)|交換由一組指定之迭代器所參考的兩個值。|  
|[lexicographical_compare (STL/CLR)](#lexicographical_compare)|比較兩個序列，這項，識別哪些順序兩個較小者。|  
|[lower_bound (STL/CLR)](#lower_bound)|已排序序列中值的值大於或等於指定的值中尋找第一個項目的位置。|  
|[make_heap (STL/CLR)](#make_heap)|將項目在指定範圍內轉換在堆積上的第一個元素的最大堆積。|  
|[最大值 (STL/CLR)](#max))|比較兩個物件，並傳回兩者的較大者。|  
|[max_element (STL/CLR)](#max_element)|尋找最大的項目指定序列中的值。|  
|[merge (STL/CLR)](#merge))|將兩個排序的來源範圍的所有項目結合成單一排序目的範圍。|  
|[min (STL/CLR)](#min)|比較兩個物件，並傳回兩個較小者。|  
|[min_element (STL/CLR)](#min_element)|尋找最小項目指定序列中的值。|  
|[mismatch (STL/CLR)](#mismatch)|比較兩個範圍的項目並傳回差異發生的第一個位置。|  
|[next_permutation (STL/CLR)](#next_permutation)|讓原始順序由語彙方面下一個更大的排列取代如果存在的話，請重新排列範圍中的項目。|  
|[nth_element (STL/CLR)](#nth_element)|資料分割序列的項目正確放`n`序列 th 項目，使它前面的所有元素小於或等於它，而它後面的所有項目大於或等於它。|  
|[partial_sort (STL/CLR)](#partial_sort)|指定的範圍中較小的項目數目排列成遞減順序。|  
|[partial_sort_copy (STL/CLR)](#partial_sort_copy)|複製項目從來源範圍到目的範圍，來排序來源範圍中的項目。|  
|[partition (STL/CLR)](#partition)|範圍中的項目排列，以便滿足一元述詞，這些項目之前，這滿足。|  
|[pop_heap (STL/CLR)](#pop_heap)|然後 form 其餘項目從新的堆積和從堆積的前面移動最大的項目至結尾。|  
|[prev_permutation (STL/CLR)](#prev_permutation)|讓原始順序由辭典編纂順序先前更大的排列取代如果存在的話，請重新排列項目序列。|  
|[push_heap (STL/CLR)](#push_heap)|將在範圍結尾的項目加入至由範圍中之前項目所組成的現有堆積。|  
|[random_shuffle (STL/CLR)](#random_shuffle)|重新排列一系列`N`的其中一個範圍中的項目`N`！ 排列方式隨機選取的其中一個。|  
|[remove (STL/CLR)](#remove)|從指定範圍中刪除指定的值，而不會干擾其餘項目的順序，並傳回指定值的新範圍的結尾。|  
|[remove_copy (STL/CLR)](#remove_copy)|項目從來源範圍複製到目的範圍，不同之處在於不複製一個指定值的項目，也不會干擾其餘項目的順序。|  
|[remove_copy_if (STL/CLR)](#remove_copy_if)|複製項目從來源範圍到目的範圍，除了滿足述詞，也不會干擾其餘項目的順序。|  
|[remove_if (STL/CLR)](#remove_if)|刪除滿足述詞，從指定範圍也不會干擾其餘項目的順序的項目。 。|  
|[replace (STL/CLR)](#replace)|取代範圍中符合指定的值，以新值的項目。|  
|[replace_copy (STL/CLR)](#replace_copy)|複製項目從來源範圍到目的範圍，取代符合指定的值，以新值的項目。|  
...
|[reverse_copy (STL/CLR)](#reverse_copy)|反轉來源範圍內項目的順序，將其複製到目的範圍。|  
...
## <a name="partition"></a> 資料分割 (STL/CLR)
...
### <a name="remarks"></a>備註  
 此函式的行為與 c + + 標準程式庫函式相同`reverse`。 如需詳細資訊，請參閱[反向](../standard-library/algorithm-functions.md#reverse)。  
...
### <a name="remarks"></a>備註  
 此函式的行為與 c + + 標準程式庫函式相同`rotate`。 如需詳細資訊，請參閱[旋轉](../standard-library/algorithm-functions.md#rotate)。  
...
### <a name="remarks"></a>備註  
 此函式的行為與 c + + 標準程式庫函式相同`swap`。 如需詳細資訊，請參閱[交換](http://msdn.microsoft.com/Library/b471a2de-035e-4aff-b1c7-345d85d93972)。  
...
### <a name="remarks"></a>備註  
 此函式的行為與 c + + 標準程式庫函式相同`unique`。 如需詳細資訊，請參閱[唯一](../standard-library/algorithm-functions.md#unique)。  
