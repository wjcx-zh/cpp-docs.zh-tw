...
|名稱|說明|  
|---------|-----------|  
|[auto_handle::auto_handle](#auto-handle)|`auto_handle`函數。|  
|[auto_handle：： ~ auto_handle](#tilde-auto-handle)|`auto_handle`析構函式。|  
...
|[auto_handle::release](#release)|從管理釋放物件 `auto_handle` 。|
|[auto_handle::reset](#reset)|終結目前擁有的物件，並選擇性地擁有新的物件。|
|[auto_handle::swap](#swap)|交換物件與另一個物件 `auto_handle` 。|  
...
|[auto_handle::operator auto_handle](#operator-auto-handle)|與相容類型之間的類型轉換運算子 `auto_handle` 。|  
|[auto_handle::operator bool](#operator-bool)|`auto_handle`在條件運算式中使用的運算子。|
|[auto_handle：： operator！](#operator-logical-not)|`auto_handle`在條件運算式中使用的運算子。|  
...
*_right*<br/>
`auto_handle`要指派給目前的 `auto_handle` 。
