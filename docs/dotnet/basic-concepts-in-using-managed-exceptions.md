---
title: Managed 例外狀況中使用的基本概念 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
2.  一旦找到正確的處理常式，回溯堆疊時該點。 在堆疊上每個函式呼叫，其本機物件會解構和 __finally 區塊執行，大部分從向外的巢狀。  
  
3.  一旦回溯堆疊時，會執行 catch 子句。  
