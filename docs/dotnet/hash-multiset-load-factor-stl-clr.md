...
## 備註  
 成員函式傳回 `(float)`[hash\_multiset::size](../dotnet/hash-multiset-size-stl-clr.md)`() /` [hash\_multiset::bucket\_count](../dotnet/hash-multiset-bucket-count-stl-clr.md)`()`。  您會用它來判斷平均 Bucket 大小。  
