...
|[hash_set::difference_type (STL/CLR)](#difference_type)| (的類型可能簽署兩個專案之間的) 距離。|
...
|[hash_set::size_type (STL/CLR)](#size_type)|兩個元素之間 (非負) 距離的型別。|
...
|[hash_set::lower_bound (STL/CLR)](#lower_bound)|尋找符合指定索引鍵的範圍開頭。|
|[hash_set::make_value (STL/CLR)](#make_value)|結構值物件。|
...
|[hash_set::upper_bound (STL/CLR)](#upper_bound)|尋找符合指定索引鍵的範圍結尾。|
|[hash_set::value_comp (STL/CLR)](#value_comp)|針對兩個元素值複製順序委派。|
...
使用序列 [ `right.begin()` 、 `right.end()`) 、預設排序述詞和預設雜湊函數，初始化受控制的序列。 您可以使用它來指定初始受控制序列，這是由 hash_set 物件 *許可權*所控制之序列的複本，以及預設排序述詞和雜湊函數。
