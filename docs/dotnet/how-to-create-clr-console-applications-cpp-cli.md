...
- 這些 .NET Framework 命名空間的參考：
   ...
   - <xref:System.AppDomainManager>：包含基本類別和基類，其定義常用的值和參考資料類型、事件和事件處理常式、介面、屬性和處理例外狀況。
...
- 原始程式檔：
   ...
   - AssemblyInfo.cpp—包含您可用來修改專案組件中繼資料的資訊，其中包括屬性、檔案、資源、類型、版本資訊、簽章資訊等。 如需詳細資訊，請參閱[元件內容](/dotnet/framework/app-domains/assembly-contents)。
