...
> [!IMPORTANT]
> 已被取代的 visual Studio 2015 和 Visual Studio 2017 不支援 **/clr: pure**和 **/clr: safe**建立可驗證的專案。 如果您需要驗證的程式碼，我們建議您將轉譯成 C# 程式碼。

不過，如果您使用較舊版本的 Visual c + + 編譯器工具組支援 **/clr: pure**和 **/clr: safe**，專案可以轉換為可驗證。 本主題描述如何設定專案屬性和修改專案來轉換您的 Visual c + + 專案，以產生可驗證的應用程式的原始程式檔。
...
1. 在**一般**頁面**組態屬性** 節點，設定**Common Language Runtime 支援**屬性**安全 MSIL Common Language執行階段支援 (/: safe)**。

1. 在**進階**頁面**連結器** 節點，設定**CLR 映像類型**屬性**強制安全 IL 映像 (/ /clrimagetype: safe)**。
...
1. 在**進階**頁面**連結器**節點中，輸入`Main`為**進入點**屬性值。
