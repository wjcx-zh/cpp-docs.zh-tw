...
如果類型具有符合設計模式的基類，則會在呼叫衍生類別的析構函式時呼叫所有基類的析構函數。 （如果您的類型是以 Visual C++ 撰寫，則編譯器會確保您的類型會執行此模式）。換句話說，參考類別的析構函式會連結至其基底和成員（如 c + + 標準所指定）。首先會執行類別的析構函式，然後以它們的結構順序反轉其成員的析構函數，最後再以其結構化的順序反轉其基類的析構函數。
