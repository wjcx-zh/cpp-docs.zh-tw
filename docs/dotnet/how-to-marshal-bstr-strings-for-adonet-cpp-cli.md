---
title: "如何：封送處理 ADO.NET 的 BSTR 字串 (C++/CLI) | Microsoft Docs"
ms.custom: ""
ms.date: "12/14/2016"
...
manager: "ghogen"
---
# 如何：封送處理 ADO.NET 的 BSTR 字串 (C++/CLI)
[!INCLUDE[vs2017banner](../assembler/inline/includes/vs2017banner.md)]

示範如何將 COM 字串 \(`BSTR`\) 加入至資料庫，以及如何從資料庫將 <xref:System.String?displayProperty=fullName> 封送至 `BSTR`。  
...
## 範例  
 ...
 如同 `main` 之前的 `#pragma unmanaged` 指示詞所表示的，此範例中的其餘程式碼都是原生 C\+\+ 程式碼。  在此範例中，會建立 DatabaseClass 的新執行個體並呼叫它的方法，以建立資料表，並在此資料表中填入一些資料列。  請注意，COM 會傳遞至資料庫資料行 StringCol 做為其值。  在 DatabaseClass 之內，會使用 <xref:System.Runtime.InteropServices?displayProperty=fullName> 命名空間中可以找到的封送處理功能，將這些字串封送處理到 Managed 字串。  具體而言，<xref:System.Runtime.InteropServices.Marshal.PtrToStringBSTR%2A> 方法是用於將 `BSTR` 封送處理為 <xref:System.String>，而 <xref:System.Runtime.InteropServices.Marshal.StringToBSTR%2A> 方法則用於將 <xref:System.String> 封送處理為 `BSTR`。  
  
> [!NOTE]
>  由 <xref:System.Runtime.InteropServices.Marshal.StringToBSTR%2A> 所配置的記憶體必須透過呼叫 <xref:System.Runtime.InteropServices.Marshal.FreeBSTR%2A> 或 `SysFreeString` 才能取消配置。  
