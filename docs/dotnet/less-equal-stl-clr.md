...
樣板類別，描述，當呼叫，傳回 true 的功能，表示只在第一個引數小於或等於第二個。  您會用它來指定函式物件根據其引數型別。  
...
## 備註  
 樣板類別會描述兩個引數的功能。  它定義成員運算子 `operator()` ，如此一來，當物件，當做函式呼叫時，它會傳回 true，表示只在第一個引數小於或等於第二個。  
