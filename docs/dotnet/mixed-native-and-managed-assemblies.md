...
Visual c + + 只支援使用混合的 managed 組件產生 **/clr**編譯器選項。 **/Clr: pure**和 **/clr: safe**編譯器選項都是 Visual Studio 2015 中已被取代，並不支援的 Visual Studio 2017 中。 如果您需要純或可驗證的 managed 組件時，我們建議您建立新的 C#。

舊版的 Visual c + + 編譯器工具組支援三種不同類型的 managed 組件產生： 混合的、 純粹的和可驗證。 後面兩個中會討論[純粹的和可驗證程式碼 (C + + /CLI)](../dotnet/pure-and-verifiable-code-cpp-cli.md)。
...
[如何： /clr 編譯 MFC 和 ATL 程式碼使用](../dotnet/how-to-compile-mfc-and-atl-code-by-using-clr.md)<br/>
...
[以 /clr 避免 CLR 關機時耗用建置的 COM 物件上的例外狀況](../dotnet/avoiding-exceptions-on-clr-shutdown-when-consuming-com-objects-built-with-clr.md)<br/>
...
如需混合的組件的程式碼撰寫方針的詳細資訊，請參閱 MSDN 文章：[概觀的未受管理/受管理程式碼互通性](https://msdn.microsoft.com/en-us/library/ms973872.aspx)。
