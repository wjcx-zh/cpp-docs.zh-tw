...
|[multimap::difference_type (STL/CLR)](#difference_type)|兩個項目之間的 （可能是帶正負號） 距離的類型。|  
...
|[multimap::generic_value (STL/CLR)](#generic_value)|泛型介面的容器項目的類型。|  
|[multimap::iterator (STL/CLR)](#iterator)|受控制序列之迭代器的類型。|  
|[multimap::key_compare (STL/CLR)](#key_compare)|兩個索引鍵排序的委派。|  
...
|[multimap::size_type (STL/CLR)](#size_type)|（非負數） 之間的距離的兩個項目類型。|  
|[multimap::value_compare (STL/CLR)](#value_compare)|兩個項目值順序的委派。|  
...
|[multimap::count (STL/CLR)](#count)|計算指定的索引鍵相符的項目。|  
...
|[multimap::key_comp (STL/CLR)](#key_comp)|將複製兩個索引鍵的順序委派。|  
...
|[multimap::upper_bound (STL/CLR)](#upper_bound)|尋找符合指定之索引鍵的範圍結尾。|  
|[multimap::value_comp (STL/CLR)](#value_comp)|將複製兩個項目值的順序委派。|  
  
|運算子|描述|  
|--------------|-----------------|  
|[multimap::operator= (STL/CLR)](#op_as)|取代受控制的序列。|  
|[operator!= (multimap) (STL/CLR)](#op_neq)|決定如果`multimap`物件是否不等於另一個`multimap`物件。|  
|[operator< (multimap) (STL/CLR)](#op_lt)|決定如果`multimap`物件是否小於另一個`multimap`物件。|  
|[operator<= (multimap) (STL/CLR)](#op_lteq)|決定如果`multimap`物件是否小於或等於另一個`multimap`物件。|  
|[operator== (multimap) (STL/CLR)](#op_eq)|決定如果`multimap`物件是否等於另一個`multimap`物件。|  
|[operator> (multimap) (STL/CLR)](#op_gt)|決定如果`multimap`物件是否大於另一個`multimap`物件。|  
|[operator>= (multimap) (STL/CLR)](#op_gteq)|決定如果`multimap`物件是否大於或等於另一個`multimap`物件。|  
...
### <a name="remarks"></a>備註  
 成員函式可有效地呼叫[multimap:: erase (STL/CLR)](../dotnet/multimap-erase-stl-clr.md) `(` [multimap:: begin (STL/CLR)](../dotnet/multimap-begin-stl-clr.md) `(),` [multimap:: end (STL/CLR)](../dotnet/multimap-end-stl-clr.md)`())`. 您可以使用它來確定受控制的序列是空白。  
...
### <a name="remarks"></a>備註  
 第一個成員函式中移除指向受控制序列的項目`where`，並傳回指定移除的項目之外剩餘的第一個元素的迭代器或[multimap:: end (STL/CLR)](../dotnet/multimap-end-stl-clr.md)`()`如果沒有這類元素存在。 您可以使用它來移除單一項目。  
...
### <a name="remarks"></a>備註  
 成員函式決定的最後一個項目`X`具有對等順序，來控制序列中`key`。 如果沒有這類元素存在，或如果`X`是最後一個項目，在受控制序列中，它會傳回[multimap:: end (STL/CLR)](../dotnet/multimap-end-stl-clr.md)`()`; 否則它會傳回迭代器，指定之外的第一個項目`X`. 您可以使用它來目前受控制序列之符合指定之索引鍵中尋找的項目序列的結尾。  
...
### <a name="remarks"></a>備註  
 運算子函式會傳回`!(left == right)`。 使用它來測試是否`left`未經過排序相同`right`兩個的多重時項目所比較的項目。  
...
### <a name="remarks"></a>備註  
 運算子函式會傳回 true，只有當所控制的序列`left`和`right`具有相同長度和每個位置`i`， `left[i] ==` `right[i]`。 使用它來測試是否`left`排序相同`right`兩個的多重時項目所比較的項目。  
...
### <a name="remarks"></a>備註  
 運算子函式會傳回`!(left` `<` `right)`。 使用它來測試是否`left`之前未經過排序`right`兩個的多重時項目所比較的項目。  
