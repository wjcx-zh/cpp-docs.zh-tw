---
title: 變更大綱 (C + + /cli CLI) |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
ms.locfileid: "46415118"
---
# <a name="outline-of-changes-ccli"></a>變更大綱 (C++/CLI)

這個大綱顯示的範例的一些變更的語言，從 Managed Extensions for c + +，Visual c + +。 請遵循隨附於每個項目，如需詳細資訊的連結。

## <a name="no-double-underscore-keywords"></a>任何雙底線關鍵字

雙底線前面的所有關鍵字已移除，有一個例外狀況。 因此，`__value`會變成`value`，並`__interface`會變成`interface`，依此類推。 若要避免名稱衝突之間關鍵字與使用者程式碼中的識別項，關鍵字是主要視為內容。

請參閱[語言關鍵字 (C + + /cli CLI)](../dotnet/language-keywords-cpp-cli.md)如需詳細資訊。
...
請參閱[受管理的類型 (C + + /cli CL)](../dotnet/managed-types-cpp-cl.md)如需詳細資訊。
...
CLR 陣列已經過重新設計。 類似於 stl`vector`範本集合，但對應到基礎`System::Array`類別-也就是它不是範本的實作。
...
### <a name="array-as-return-type"></a>為傳回型別陣列
...
請參閱[明確介面成員的覆寫](../dotnet/explicit-override-of-an-interface-member.md)如需詳細資訊。
...
- 內建算術類型和物件類別階層架構之間的多載解析已反轉 Managed Extensions 與 Visual c + + 之間。 副作用，CLR 列舉是無法再隱含地轉換到算術類型。

- 在新語法中，CLR 列舉會維護自己的範圍，不是 Managed Extensions 中的案例。 以往，列舉值都是列舉的可見; 包含的範圍內現在，列舉值會封裝列舉的範圍內。
...
請參閱[Boxed 值的追蹤控制代碼](../dotnet/a-tracking-handle-to-a-boxed-value.md)如需詳細資訊。
