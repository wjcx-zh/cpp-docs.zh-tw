...
## 備註  
 成員函式來判斷在相同 Bucket 的項目為 `key`之受控制序列的最後一個項目的 `X` 。  如果沒有這類項目，或如果 `X` 是在受控制序列的最後一個項目，則會傳回 [set::end](../dotnet/set-end-stl-clr.md)`()`;否則會指定在 `X`之外的第一個項目的 Iterator 傳回它。  您可以使用它目前所在項目的結尾符合指定索引鍵受控制序列。  
