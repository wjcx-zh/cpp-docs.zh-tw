---
title: "編譯器錯誤 C3276 |Microsoft 文件"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- cpp-tools
ms.tgt_pltfrm: 
ms.topic: error-reference
f1_keywords:
- C3276
dev_langs:
- C++
helpviewer_keywords:
- C3276
ms.assetid: dd6b4fd2-094d-4d34-a467-a9afd59789f7
caps.latest.revision: 
author: corob-msft
ms.author: corob
manager: ghogen
ms.workload:
- cplusplus
ms.openlocfilehash: a10e4569af85599a55dc23cabdb934e94924c27c
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 12/21/2017
---
# <a name="compiler-error-c3276"></a>編譯器錯誤 C3276
'keyword': 跳出 __finally/finally 區塊有未定義的行為在終止處理期間  
  
 此錯誤是與相同[C4532](../../error-messages/compiler-warnings/compiler-warning-level-1-c4532.md)警告。 不過，當您使用 /clr 時，這種狀況無法停用與[警告](../../preprocessor/warning.md)pragma。