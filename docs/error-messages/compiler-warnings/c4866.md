...
這項警告是在 Visual Studio 2017 版本 15.9 因為 C + + 17 標準的編譯器一致性處理而引進的。 編譯而不發出警告的視覺效果之前，編譯器的版本中的程式碼C++現在可以產生 2017年版 15.9 **C4866**。 如需如何停用特定編譯器版本或更新版本所引進的警告的詳細資訊，請參閱[依編譯器版本的編譯器警告](compiler-warnings-by-compiler-version.md)。
