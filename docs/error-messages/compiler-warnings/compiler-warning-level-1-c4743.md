...
可以發出這個警告，c + + 時，沒有重要的案例。 如果您宣告具有相同名稱在兩個不同的檔案相同的類型，如果這些宣告包含虛擬函式，而且如果宣告不是相同，編譯器可發出警告 C4744 虛擬函式的資料表。 有兩個不同大小的虛擬函式做為相同的類型，和連結器必須選擇其中一個，將合併到可執行檔，就會發生此警告。  可以，這可能會導致您的程式錯誤的虛擬函式的呼叫。
