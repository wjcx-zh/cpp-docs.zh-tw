---
title: 編譯器警告 （層級 4） C4536 |Microsoft 文件
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-diagnostics
ms.topic: error-reference
f1_keywords:
- C4536
dev_langs:
- C++
helpviewer_keywords:
- C4536
ms.assetid: ab4d0686-f813-4e88-a264-b40d3630ed6c
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: ab098c240c75fa7b06c9bce73a64c61c1f46f99a
ms.sourcegitcommit: 76b7653ae443a2b8eb1186b789f8503609d6453e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/04/2018
ms.locfileid: "33302304"
---
# <a name="compiler-warning-level-4-c4536"></a>編譯器警告 (層級 4) C4536
'type name': 類型名稱超出中繼資料 'limit' 個字元的限制  
  
 型別名稱會被截斷中繼資料中，如果它是 managed 型別。 請參閱[C3180](../../error-messages/compiler-errors-2/compiler-error-c3180.md)如需詳細資訊。  
  
 此警告預設為關閉。 如需詳細資訊，請參閱 [預設為關閉的編譯器警告](../../preprocessor/compiler-warnings-that-are-off-by-default.md) 。