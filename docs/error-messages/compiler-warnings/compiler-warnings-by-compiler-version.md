...
C5028|'*名稱*':先前的宣告中指定的對齊方式 (*數字*) 定義中未指定
...
C4451|'*型別*':使用 ref 類別*型別*' 在此內容可能會導致不正確的封送處理物件跨內容
...
C4430|遺漏類型規範 - 假設為 int。 注意:C + + 不支援預設 int
C4431|遺漏類型規範 - 假設為 int。 注意:C 不再支援 default-int
