---
title: 編譯器警告 C4600 到 C4799 |Microsoft Docs
ms.date: 07/03/2018
ms.technology:
- cpp-diagnostics
...
|編譯器警告 （層級 1） C4603|'*識別碼*': 未定義巨集，或使用先行編譯標頭之後，定義是不同|
...
|編譯器警告 （層級 3 和層級 4） C4637|XML 文件註解目標：\<包含 > 捨棄的標籤。 原因|
...
|編譯器警告 （層級 1） C4655|'symbol': 變數類型有新推出的最新的組建，或其他處有不同定義|
...
|編譯器警告 （層級 4） C4754|位於 %s(%d) 的比較中的算術運算的轉換規則表示的一個分支無法執行。 轉換 '%s' '%s' （或 %d 位元組的類似類型）。|
...
|編譯器警告 （層級 1） C4794|執行緒區域儲存區變數 '%s' 從 '%s' 變更為 '%s' 的區段|
