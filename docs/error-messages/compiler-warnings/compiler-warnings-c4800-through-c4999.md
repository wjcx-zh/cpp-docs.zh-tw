...
|警告|訊息|
|-------------|-------------|
|[編譯器警告 (層級 3) C4800](compiler-warning-level-3-c4800.md)|'*類型*': 值強制設 bool 'true' 或 'false' （效能警告）|
|[編譯器警告 (層級 1) C4803](compiler-warning-level-1-c4803.md)|'*方法*': 產生的方法具有不同的儲存類別，從該事件中，'*事件*'|
|[編譯器警告 (層級 1) C4804](compiler-warning-level-1-c4804.md)|'*作業*': 不安全的類型 'bool' 作業中使用|
|[編譯器警告 (層級 1) C4805](compiler-warning-level-1-c4805.md)|'*作業*': 不安全的混用類型'*type1*'和型別'*type2*' 作業中|
|編譯器警告 （層級 1） C4806|'*作業*': 不安全的作業： 沒有值的型別'*type1*'升級為類型'*type2*' 可以等於所給的常數|
|編譯器警告 （層級 1） C4807|'*作業*': 不安全的混用類型'*type1*'和帶正負號位元欄位的型別'*type2*'|
|編譯器警告 （層級 1） C4808|案例 '*值*' 不是有效的型別參數條件值 'bool'|
...
|編譯器警告 （層級 1） C4813|'*函式*': 區域類別的 friend 函式必須先前已宣告|
...
|[編譯器警告 (層級 1) C4835](compiler-warning-level-1-c4835.md)|'*變數*': 匯出資料的初始設定式將不會執行，直到主機組件中第一次執行受管理的程式碼|
|編譯器警告 （層級 4） C4837|偵測到的三併詞: '？*字元*'取代'*字元*'|
|[編譯器警告 (層級 1) C4838](compiler-warning-level-1-c4838.md)|從轉換 '*type_1*'to'*type_2*' 必須是縮小轉換|
...
|編譯器警告 C4843|'*type1*': 陣列或函式類型之參考的例外狀況處理常式無法連線，請使用'*type2*' 改為|
|編譯器警告 C4844|' 匯出模組*適於*;' 已宣告模組介面的慣用的語法|
...
|編譯器警告 （層級 1） C4880|轉型從 ' const *type_1*'to'*type_2*': 轉型常數性的指標或參考可能會導致未定義的行為在 amp 限制函式|
|編譯器警告 （層級 4） C4881|建構函式和 （或） 解構函式將不會叫用 tile_static 變數 '*變數*'|
|編譯器警告 （層級 1） C4882|將具有非常數呼叫運算子函式傳遞給 concurrency:: parallel_for_each 已被取代|
|編譯器警告 C4900|之間的 Il 不符 '*tool1*'version'*version1*'和'*tool2*'version'*version2*'|
...
|編譯器警告 （層級 1） C4912|'*屬性*': 屬性有未定義的巢狀 UDT 上的行為|
...
|編譯器警告 （層級 4） C4918|'*字元*': 在 pragma 最佳化清單中有無效的字元|
...
|編譯器警告 （層級 4） C4937|'*text1*'和'*text2*'是否為引數'*指示詞*'|
...
|編譯器警告 （層級 1） C4944|'*符號*': 無法匯入從符號'*assembly1*': 為*符號*' 已經存在於目前的範圍|
|[編譯器警告 (層級 1) C4945](compiler-warning-level-1-c4945.md)|'*符號*': 無法匯入從符號'*assembly1*': 為*符號*'匯入了另一個組件'*assembly2*'|
|[編譯器警告 (層級 1) C4946](compiler-warning-level-1-c4946.md)|相關類別之間使用的 reinterpret_cast: '*class1*'和'*class2*'|
|編譯器警告 （層級 1） C4947|'*type_or_member*': 標記為過時|
|[編譯器警告 (層級 2) C4948](compiler-warning-level-2-c4948.md)|傳回型別 '*存取子*' 不符合對應 setter 的最後一個參數類型|
...
|編譯器警告 （層級 1） C4951|'*函式*' 已經編輯，因為已收集的分析資料，未使用的函式分析資料|
|編譯器警告 （層級 1） C4952|'*函式*': 沒有任何分析資料在程式資料庫中找到'*pgd_file*'|
|編譯器警告 （層級 1） C4953|內嵌項 '*函式*' 已經編輯，因為已收集的分析資料，未使用分析資料|
|編譯器警告 C4954|'*函式*': 未分析 （包含 __int64 switch 運算式）|
|編譯器警告 C4955|'*import2*': 匯入已忽略; 已經從匯入'*import1*'|
|編譯器警告 （層級 1，錯誤） C4956|'*類型*': 這個類型不是可驗證|
|編譯器警告 （層級 1，錯誤） C4957|'*轉換*': 明確的轉換，從'*cast_from*'to'*cast_to*' 不是可驗證|
|編譯器警告 （層級 1，錯誤） C4958|'*作業*': 指標算術不是可驗證|
|編譯器警告 （層級 1，錯誤） C4959|無法定義 unmanaged 的類型 '*類型*' /clr: safe 中因為存取它的成員會產生無法驗證程式碼|
