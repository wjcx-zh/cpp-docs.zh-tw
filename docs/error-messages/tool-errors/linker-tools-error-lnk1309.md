...
如果您嘗試建置部分信任的 CLR 純應用程式，並使用 ptrustu [d].lib，也會看到 LNK1309。 如需如何建立部分信任的應用程式的資訊，請參閱[How to:建立部分信任的應用程式移除 CRT 程式庫 DLL 相依性](../../dotnet/create-a-partially-trusted-application.md)。
