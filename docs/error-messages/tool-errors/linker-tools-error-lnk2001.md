...
這個錯誤可能因[遺漏函式主體或變數](../../error-messages/tool-errors/missing-function-body-or-variable.md)定義。 當您宣告，但未定義、 變數、 函數或類別在程式碼中，此錯誤是常見的。 編譯器只需要函式原型或`extern`變數的宣告來產生物件檔案不含錯誤，但連結器無法解析呼叫函式或變數的參考，因為沒有任何函式程式碼或變數保留的空間。 若要修正此問題，請確定每個參考的函式和變數是完全在中定義之原始程式檔或程式庫包含在您的連結。  
  
這個錯誤可能被因使用傳回和參數類型或不符合函式定義中的呼叫慣例的函式呼叫。 在 c + + 物件檔，[名稱裝飾](../../error-messages/tool-errors/name-decoration.md)納入最終的裝飾函式名稱，用以做為符號解析的函式呼叫從其他物件的檔案時，比對的呼叫慣例、 類別或命名空間範圍和函式的傳回和參數類型。 若要修正此問題，請確定宣告、 定義和呼叫所有函式使用相同範圍、 類型和呼叫慣例。  
...
[混合的內嵌和非內嵌編譯選項](../../error-messages/tool-errors/function-inlining-problems.md)在不同模組會造成 LNK2001。 C + + 程式庫會透過函式內嵌為開啟 (**/Ob1**或**/Ob2**)，但對應的標頭檔描述該函式已關閉內嵌 (沒有`inline`關鍵字)，就會發生此錯誤。 若要修正此問題，定義的函式`inline`您在其他原始程式檔中包含的標頭檔中。  
...
C + + 編譯器和連結器使用[名稱裝飾](../../error-messages/tool-errors/name-decoration.md)，亦稱為*名稱改變*，用來編碼有關變數的類型或傳回型別、 參數類型、 範圍和呼叫慣例函式中的符號名稱的額外資訊。 此裝飾的名稱是連結器搜尋來解析外部符號的符號名稱。  
