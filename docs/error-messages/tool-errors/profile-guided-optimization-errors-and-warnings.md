---
title: 特性指引最佳化錯誤與警告
ms.date: 04/17/2019
f1_keywords:
- PG0001
- PG0002
- PG0003
- PG0061
- PG0062
- PG0063
- PG0065
- PG0066
- PG0067
- PG0068
- PG0069
- PG0070
- PG0071
- PG0081
- PG0087
- PG0090
- PG0091
- PG0092
- PG0094
- PG0096
- PG0168
- PG0169
- PG0181
- PG0188
- PG1000
- PG1001
- PG1032
- PG1033
- PG1035
- PG1036
- PG1038
- PG1051
- PG1052
- PG1053
- PG1056
- PG1058
- PG1060
- PG1061
- PG1065
- PG1066
- PG1067
ms.assetid: f67b1011-fb64-4df3-9356-d52c9423ff3f
ms.openlocfilehash: 9be94ee3145dc23543fa5880c14e08a2ed25c76b
ms.sourcegitcommit: 283cb64fd7958a6b7fbf0cd8534de99ac8d408eb
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/28/2019
ms.locfileid: "64856091"
---
# <a name="profile-guided-optimization-errors-and-warnings-pgxxxx"></a>特性指引最佳化錯誤和警告 (PGxxxx)

本節是特性指引最佳化 (PGO) 工具所產生的錯誤的參考。 PGO 錯誤和警告有表單 PG*xxxx*，其中*xxxx*是四位數的數字。

[!INCLUDE[error-boilerplate](../../error-messages/includes/error-boilerplate.md)]

## <a name="pgo-errors"></a>PGO 錯誤

[特性指引最佳化錯誤 PG0165](../../error-messages/tool-errors/profile-guided-optimization-error-pg0165.md)

## <a name="pgo-warnings"></a>PGO 警告

[特性指引最佳化警告 PG1039](../../error-messages/tool-errors/profile-guided-optimization-warning-pg1039.md) \
[特性指引最佳化警告 PG1087](../../error-messages/tool-errors/profile-guided-optimization-warning-pg1087.md)

## <a name="see-also"></a>另請參閱

[C /C++編譯器及建置工具錯誤和警告](../compiler-errors-1/c-cpp-build-errors.md) \
[特性指引最佳化](../../build/profile-guided-optimizations.md)
