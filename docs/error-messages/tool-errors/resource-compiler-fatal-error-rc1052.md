...
若要修正這個問題，請減少資源檔中的巢狀 `#if` 和 `#ifdef` 指示詞數目。 如果問題是因為資源檔中所包含的標頭檔所造成，請減少標頭檔中的巢狀 `#if` 和 `#ifdef` 指示詞數目。 如果實際上不可行，請考慮在現有已包含的標頭檔上執行前置處理器，在資源檔中建立並包含新的標頭檔。 如需詳細資訊，請參閱[/p （](../../build/reference/p-preprocess-to-a-file.md)前置處理至檔案）編譯器選項。
