...
ms.locfileid: "51694253"
---
# <a name="add-a-property"></a>新增屬性

您可以使用 [[新增屬性精靈]](#names-add-property-wizard) 將方法新增至專案中的介面。
...
1. 在 [[新增屬性精靈]](#names-add-property-wizard) 中，提供建立屬性用的資訊。

1. 在精靈的 [[IDL 屬性]](#idl-attributes-add-property-wizard) 頁面中，指定屬性的任何介面定義語言 (IDL) 設定。
...
- **實作類型**
  ...
  |**成員變數**|指定屬性已新增為成員變數。 您可以新增自訂屬性或大多數內建屬性作為成員變數。 您無法為 `Caption`、`hWnd` 和 `Text` 屬性指定 [成員變數]。<br /><br /> 在 [變數名稱] 和 [通知函式] 下提供預設名稱。 您可以編輯此名稱。|
...
- `helpstring`
  ...
  指定用來描述所套用元素的字元字串。 根據預設，其會設定為 `property`&nbsp;*Property&nbsp;name*。 請參閱＜MIDL 參考＞中的 [helpstring](/windows/desktop/Midl/helpstring)。
