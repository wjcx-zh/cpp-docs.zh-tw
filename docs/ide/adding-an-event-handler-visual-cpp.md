...
3.  在捷徑功能表，按一下 [新增事件處理常式]，顯示 [事件處理常式精靈]。  
  
4.  在 [訊息類型] 方塊中選取事件，將其新增至 [類別清單] 方塊中選取的類別。  
  
5.  接受 [函式處理常式名稱] 方塊中的預設名稱，或提供您選擇的名稱。  
  
6.  按一下 [新增並編輯]，將事件處理常式新增至專案，並在新函式處開啟文字編輯器來新增適當的事件處理常式程式碼。  
