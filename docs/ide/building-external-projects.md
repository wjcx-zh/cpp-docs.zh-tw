...
外部專案是使用 makefile 或 （外部索引鍵或外部） 以外的其他功能的 Visual c + + 專案的 Visual c + + 開發環境。  
 ...
 如果您有外部專案 （例如，makefile 專案），您想要在 Visual c + + 開發環境中建置、 建立 Makefile 專案，並指定您的專案建置命令和 Makefile 應用程式精靈中的輸出。 如需詳細資訊，請參閱[建立 Makefile 專案](../ide/creating-a-makefile-project.md)。  
