...
您可以建立 Visual c + + 專案，選擇**檔案&#124;新增&#124;專案**，確定已在左窗格中，選取 Visual c + +，，然後選擇 從中間窗格中的專案範本的清單。 當您按一下範本時，在許多情況下將會顯示精靈，可讓您建立專案之前設定各種不同的專案屬性。 您可以檢視並修改這些屬性稍後使用專案屬性頁 (**專案&#124;屬性**)。  
  
 您也可以建立新專案的：  
  
-   選擇**檔案&#124;新增&#124;來自現有程式碼專案**並遵循提示來加入現有的原始程式碼檔案。 這個選項最適合用於相對較小且簡單的專案，可能是很少或沒有複雜相依性的 25 個或更少原始程式碼檔。  
