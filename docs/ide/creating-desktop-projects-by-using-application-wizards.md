...
2.  在 [專案類型] 窗格中，選取**Visual c + + 專案**資料夾。 在 [範本] 窗格中會出現各種 C++ 專案的圖示。  
  
3.  在 [範本] 窗格中，選取圖示以選擇專案類型。 窗格下面會出現訊息，指示所要建立專案的類型。  
  
4.  請指定專案屬性，或略過此步驟使用 Visual Studio 預設的專案屬性。 請參閱[使用專案屬性](../ide/working-with-project-properties.md)如需詳細資訊。  
  
5.  按一下**確定**，並為您的專案類型開啟精靈。 如果您想要讀取精靈的 [說明] 主題，請按**F1**。  
