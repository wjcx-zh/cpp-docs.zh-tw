...
|*Projname*.idl|*專案名稱*|來源|(特定專案) 包含控制項類型程式庫的介面描述語言 (IDL) 原始程式碼。 Visual C++ 會使用這個檔案來產生類型程式庫。 產生的程式庫會向其他自動化用戶端公開控制項的介面。 如需詳細資訊，請參閱[介面定義 (IDL) 檔](http://msdn.microsoft.com/library/windows/desktop/aa378712)Windows SDK 中。|  
