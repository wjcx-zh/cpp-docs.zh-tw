...
1. Visual c + + 專案系統目前不支援萬用字元專案項目中。 例如，不支援這種：

   ```xml
   <ClCompile Include="*.cpp"/>
   ```

1. Visual c + + 專案系統目前不支援的巨集的專案項目路徑中。 例如，不支援這種：

   ```xml
   <ClCompile Include="$(IntDir)\generated.cpp"/>
   ```

1. 才能正確地新增、 移除或修改中編輯時，專案屬性**專案屬性** 對話方塊中，這個檔案必須包含不同的群組，每個專案組態，且條件必須在這種形式：

   ```xml
   Condition="'$(Configuration)|$(Platform)'=='Debug|Win32'"
   ```

1. 每一個屬性必須指定正確的標籤，為指定的屬性規則檔中的群組中。 如需詳細資訊，請參閱[屬性頁 xml 規則檔案](property-page-xml-files.md)。
...
下列章節說明每個這些元素的目的和原因，它們的順序這種方式：

### <a name="project-element"></a>專案項目
...
### <a name="projectconfiguration-elements"></a>專案組態項目
...
### <a name="configuration-propertygroup-elements"></a>組態 PropertyGroup 項目

```xml
<PropertyGroup Label="Configuration" />
```

A`Configuration`屬性群組都具有附加的設定條件 (例如`Condition=”'$(Configuration)|$(Platform)'=='Debug|Win32'”`) 和多個複本，其中每個組態中出現。 此屬性群組主控的特定組態設定的屬性。 組態屬性包含 PlatformToolset 和控制的系統屬性工作表中包含**Microsoft.Cpp.props**。 例如，如果您定義屬性`<CharacterSet>Unicode</CharacterSet>`，然後系統屬性工作表**microsoft。Cpp.unicodesupport.props**將會包含。 如果您檢查**Microsoft.Cpp.props**，您會看到行： `<Import Condition=”'$(CharacterSet)' == 'Unicode'”   Project=”$(VCTargetsPath)\microsoft.Cpp.unicodesupport.props”/>`。
...
有多個此屬性群組中，所有的專案組態的設定每一個執行個體。 每個屬性群組必須有一個附加的設定條件。 如果任何設定遺失，則**專案屬性**對話方塊將無法正常運作。 不同於上述的屬性群組，在這個沒有標籤。 此群組包含專案組態層級設定。 這些設定套用至指定的項目群組一部分的所有檔案。 建置自訂項目定義的中繼資料會初始化這裡。

之後必須放在此 PropertyGroup`<Import Project="$(VCTargetsPath)\Microsoft.Cpp.props" />`必須沒有標籤之前的任何其他 PropertyGroup 和 （否則編輯專案屬性將不會正確運作）。

### <a name="per-configuration-itemdefinitiongroup-elements"></a>每個組態 ItemDefinitionGroup 項目

```xml
 <ItemDefinitionGroup />
```

包含項目定義。 這些必須遵循無標籤的各個組態 PropertyGroup 項目相同的規則條件。
...
在 IDE 中，當您設定**UseOfAtl**一般 屬性頁中的屬性，它會寫入專案檔案中的組態屬性群組時**TargetName**相同的屬性頁中的屬性寫入標籤小於每個組態屬性群組。 Visual Studio 會查看屬性頁的 xml 檔案，如需有關何處寫入每一個屬性。 如**一般**屬性頁面 （假設您有 Visual Studio Enterprise Edition 的英文版），該檔案是`%ProgramFiles(x86)%\Microsoft Visual Studio\2017\Enterprise\Common7\IDE\VC\VCTargets\1033\general.xml`。 屬性頁 XML 規則檔會定義規則和其所有屬性的靜態資訊。 這類的某項資訊是規則屬性的目的地檔案 （其值將寫入其中的檔案） 中的慣用的位置。 在專案檔項目 Label 屬性指定慣用的位置。
