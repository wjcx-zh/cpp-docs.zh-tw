...
在 Visual Studio 中，工作是組織在專案和方案中。 方案可以包含一個以上專案，例如 DLL 和參考此 DLL 的可執行檔。 如需詳細資訊，請參閱[方案和專案](/visualstudio/ide/solutions-and-projects-in-visual-studio)。
...
本逐步解說的這個部分說明如何將類別加入至專案。 當您新增類別時，Visual Studio 會新增對應的 .h 和 .cpp 檔案。 您可以在 [方案總管] 中查看結果。
...
1. 編輯 Cardgame.h 檔案，並進行下列變更：
   ...
   - 修改 Visual Studio 產生的預設建構函式。 在 `public:` 存取規範之後，尋找如下所示的程式碼行：
      ...
      將此建構函式修改成接受一個屬於 `int` 類型、名為 *players* 的參數。
   ...
   程式行 `#pragma once` 指示編譯器只包含標頭檔一次。 如需詳細資訊，請參閱 [once](../preprocessor/once.md)。 如需這個標頭檔中其他 C++ 關鍵字的資訊，請參閱 [class](../cpp/class-cpp.md)、[int](../cpp/fundamental-types-cpp.md)、[static](../cpp/storage-classes-cpp.md) 和 [public](../cpp/public-cpp.md)。
...
1. 刪除檔案的所有內容並取代為下列程式碼：
   ...
   > [!NOTE]
   > 輸入程式碼時，您可以使用自動完成。 例如，如果您使用鍵盤輸入此程式碼，您可以輸入 *pl* 或 *tot* 後再按 Ctrl+空格鍵。 自動完成功能會替您輸入 `players` 或 `totalParticipants`。
...
1. 若要執行專案，請在功能表列上選擇 [偵錯] > [啟動但不偵錯]。 主控台視窗應該會出現，且輸出應該會與以下相似：
...
**上一步：**[使用 Visual Studio IDE 進行 C++ 桌面程式開發](../ide/using-the-visual-studio-ide-for-cpp-desktop-development.md)。  
