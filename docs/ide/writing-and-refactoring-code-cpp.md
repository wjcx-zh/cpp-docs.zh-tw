...
自動省略某些符號可縮小結果範圍。 例如,在從類外部訪問類物件的成員時,默認情況下無法看到私有成員或受保護成員(如果您不在子類的上下文中)。 您可以使用底部的按鈕來調整篩選。

從下拉清單中選擇符號後,可以使用**Tab、Enter**或其他提交字元之**Enter**一自動 完成該符號(`{ } [ ] ( ) . , : ; + - * / % & | ^ ! = ? @ # \`預設情況下: )。 若要新增或移除此清單中的字元，請在 [快速啟動]**** (Ctrl + Q) 中搜尋 "IntelliSense"，然後選擇 [文字編輯器] > [C/C++] > [進階]**** 選項。 [成員清單認可字元]**** 選項可讓您以所需的變更自訂清單。
...
![C&#43;&#43; intelli 程式碼](../ide/media/intellicode-cpp.png "C++ IntelliCode")

::: moniker range="vs-2019"

在 Visual Studio 2019 中，IntelliCode 是以 **C++ 桌面開發**工作負載的選擇性元件形式提供。 為了確保 intelliCode 在 C++ 處於活動狀態,請轉到**工具** > **選項** > **IntelliCode** > **一般**並將**C++基本模型**設定為**啟用**。
...
要覆蓋**預測性 IntelliSense**並顯示較長的清單,請按**Ctrl = J**。如果 **「預測分析」** 處於打開位置,則呼叫**Ctrl + J**將刪除預測篩選器。 再次按下 **Ctrl + J** 則會從相關的成員清單結果中移除存取範圍篩選。 IntelliSense 下拉清單下的 (*) 按鈕與**Ctrl = J**執行相同的操作。將滑鼠懸停在按鈕上,查看有關顯示內容的工具提示資訊。

![C&#43;&#43; 预测性感知](../ide/media/predictive-intellisense-cpp.png "預測性感知")
...
![C&#43;&#43; 模板感知显示现有实例化](../ide/media/template-intellisense-cpp-1.png "樣本感知顯示現有實體")

按下**\<T>** 圖示以展開/折疊**樣本列**。 按一下鉛筆圖示，或按兩下**範本列**來開啟 [編輯]**** 視窗。

![C&#43;&#43; 模板感知](../ide/media/template-intellisense-cpp-3.png "範本 IntelliSense")

您在視窗中所進行的編輯內容會直接套用到原始程式碼，讓您能夠即時查看影響。

範本列可以根據程式碼中的具現化自動填入候選項目。 按一下 [新增所有現有具現化]****，以查看用來在整個程式碼基底中具現化範本的所有具體引數清單。

![C&#43;&#43; 模板感知结果列表](../ide/media/template-intellisense-cpp-2.png "樣本感知結果清單")

編輯器底部的視窗會顯示發現每個具現化的位置，以及其引數。

![C&#43;&#43; 模板感知实例图](../ide/media/template-intellisense-cpp-4.png "樣本感知實體映射")
...
其他格式選項位於主選單中的 **「編輯** > **高級」** 下。

![C++ 進階編輯選項](media/edit-advanced-cpp.png)

用於啟用和設定C++特定編輯功能的選項位於**工具** > **選項** > **文本編輯器** > **C/C++** 下。 選擇您想要設定的選項之後，您可以在對話方塊成為焦點時按下 **F1** 來取得更多說明。 針對一般程式碼格式化選項，請在 [快速啟動]**** 中鍵入 `Editor C++`。

![視覺工作室工具>選項](../ide/media/tools-options.png "編輯器選項")
