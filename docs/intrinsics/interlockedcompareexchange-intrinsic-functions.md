...
`_InterlockedCompareExchange`對`Destination` 值`Comparand`進行不可部分完成的比較。 如果 `Destination` 值等於 `Comparand` 值，則 `Exchange` 值會儲存在 `Destination` 所指定的位址。 否則, 不會執行任何作業。

`_InterlockedCompareExchange`為 Win32 Windows SDK [InterlockedCompareExchange](/windows/win32/api/winnt/nf-winnt-interlockedcompareexchange)函數提供編譯器內建支援。

有幾種不同`_InterlockedCompareExchange`的變化取決於它們所涉及的資料類型, 以及是否使用處理器特定的取得或發行語義。

當函式在長整數值上運作`_InterlockedCompareExchange8`時, 會在8位整數值`_InterlockedCompareExchange16`上運作、在短整數值`_InterlockedCompareExchange64`上運作, 並在64位整數值上運作。 `_InterlockedCompareExchange`
...
在下列範例中，`_InterlockedCompareExchange` 會用於簡單的低層級執行緒同步處理。 此方法有其限制, 做為多執行緒程式設計的基礎;其中提供說明連鎖內建函式的一般用法。 為求最佳結果，請使用 Windows API。 如需多執行緒程式設計的詳細資訊, 請參閱[撰寫多執行緒 Win32 程式](../parallel/multithreading-with-c-and-win32.md#writing-a-multithreaded-win32-program)。
