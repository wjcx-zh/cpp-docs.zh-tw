...
Visual Studio 會建立最小的 CMakeLists.txt 檔案，其中只包含可執行檔和名稱，以及所需的最低 CMake 版本。 不過，您可以依需要手動編輯此檔案；Visual Studio 將永遠不會覆寫您的變更。 您可以通過右鍵單擊**解決方案資源管理員**中的根 CMakelists.txt 檔案並選擇**專案的 CMake 設定**來指定 CMake 命令列參數和環境變數。 若要指定用於偵錯的選項，以滑鼠右鍵按一下專案節點，然後選擇 [偵錯並啟動設定]****。

::: moniker-end

當您打開包含現有 CMake 專案的資料夾時,Visual Studio 將使用 CMake 快取中的變數來配置 IntelliSense 並自動生成。 本機組態和偵錯設定會儲存在 JSON 檔案中，可選擇性地與其他使用 Visual Studio 的人員共用。

Visual Studio 不會修改 CMakeLists.txt 檔案，因此可讓其他正在使用相同專案的人員繼續使用任何他們已在使用的工具。 當您將編輯保存到 CMakelists.txt 或在某些情況下保存到 CMakeSettings.json 時,Visual Studio 會重新生成緩存。 但是，如果您要使用 [現有的快取]**** 設定，則 Visual Studio 不會修改快取。
...
CMake 專案的 Linux 支援需要在目標電腦上安裝 CMake 最新版本。 通常，發行版本的預設套件管理員所提供版本都不夠新，因此無法支援 Visual Studio 需要的所有功能。 Visual Studio 2019 會偵測 Linux 系統上是否已安裝最新版的 CMake。 如果未找到,Visual Studio 在編輯器窗格的頂部顯示一個資訊列,該欄[https://github.com/Microsoft/CMake/releases](https://github.com/Microsoft/CMake/releases)提供從 為您安裝它。
...
Visual Studio 中的 CMake 支援需要 CMake 3.8 中所引進的伺服器模式支援。 對於 Microsoft 提供的 CMake 變體,請下載[https://github.com/Microsoft/CMake/releases](https://github.com/Microsoft/CMake/releases)最新的預建構二 進位檔案。
...
要開始,請從主選單中選擇 **「檔案** > **打開** > **資料夾**」,或者`devenv.exe <foldername>`在命令行上鍵入。 您開啟的資料夾內應該有 CMakeLists.txt 檔案，以及您的原始程式碼。
...
對於遠端目標,Visual Studio 預設選擇清單中的第一個遠端系統,位於 **「工具** > **選項** > **跨平台** > **連接管理器**」下。 如果找不到遠端連線，系統會提示您建立一個連線。 如需詳細資訊，請參閱[連線到您的遠端 Linux 電腦](connect-to-your-remote-linux-computer.md)。

如果指定遠端 Linux 目標，您的來源會複製到遠端系統。

選取目標之後，CMake 會自動在 Linux 系統上執行，以便為您的專案產生 CMake 快取。

![Linux 上產生 CMake 快取](media/cmake-linux-1.png "Linux 上產生 CMake 快取")
...
若要自訂您程式的命令列引數，請按下 [方案總管]**** 頂端的 [切換目標]**** 按鈕，然後選擇 [目標檢視]****。 接著，以滑鼠右鍵按一下目標，然後選取 [偵錯並啟動設定]****。 這會開啟或建立包含您應用程式資訊的 launch.vs.json 設定檔。 要指定來源檔案的位置,請向檔案加入**來源映射**屬性,如以下範例的範例 :
...
## <a name="configure-cmake-settings-for-linux"></a><a name="configure_cmake_linux"></a>為 Linux 設定「製作」設定
...
若要在 Visual Studio 2017 中變更預設 CMake 設定，請從主功能表選擇 [CMake] | [變更 CMake 設定] | [CMakeLists.txt]****，或在 [方案總管]**** 中以滑鼠右鍵按一下 CMakeSettings.txt，然後選擇 [變更 CMake 設定]****。 Visual Studio 接著會在您的根專案資料夾中建立新 `CMakeSettings.json` 檔案。 您可以使用 **CMake 設定**編輯器來開啟檔案，或直接修改檔案。 有關詳細資訊,請參閱[自訂"自訂造拍"設定](../build/customize-cmake-settings.md)。
