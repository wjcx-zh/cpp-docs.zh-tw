...
CMake 專案的 Linux 支援需要目的電腦擁有最新版本的 CMake。 散發者的預設封裝管理員所提供的版本通常不夠新，無法支援 Visual Studio 所需的所有功能。 Visual Studio 2019 會偵測 Linux 系統上是否已安裝最新版的 CMake。 如果找不到，Visual Studio 會在編輯器窗格的頂端顯示資訊列。 它提供從[https://github.com/Microsoft/CMake/releases](https://github.com/Microsoft/CMake/releases)安裝 CMake 的。
...
Visual Studio 中的 CMake 支援需要 CMake 3.8 中所引進的伺服器模式支援。 如需 Microsoft 提供的 CMake 變異，請下載最新的預[https://github.com/Microsoft/CMake/releases](https://github.com/Microsoft/CMake/releases)建二進位檔，網址為。

二進位檔會安裝在`~/.vs/cmake`中。 部署二進位檔之後，您的專案會自動重新產生。 如果`cmakeExecutable` *CMakeSettings*中的欄位所指定的 CMake 無效（它不存在或不是不支援的版本），而且預先建立的二進位檔存在，Visual Studio 會`cmakeExecutable`忽略並使用預先建立的二進位檔。
...
Visual Studio 會建立最小的*remote monitoring.h cmakelists.txt* ，只包含可執行檔的名稱和所需的最低 CMake 版本。 不過，您可以依需要手動編輯此檔案；Visual Studio 將永遠不會覆寫您的變更。 您可以在此檔案中指定 CMake 命令列引數和環境變數。 以滑鼠右鍵按一下**方案總管**中的根*remote monitoring.h cmakelists.txt .txt*檔案，然後選擇 [**專案的 CMake 設定**]。 若要指定用於偵錯的選項，以滑鼠右鍵按一下專案節點，然後選擇 [偵錯並啟動設定]****。
...
Visual Studio 不會修改*remote monitoring.h cmakelists.txt .txt*檔案。 它是單獨的，讓其他使用相同專案的人可以繼續使用現有的工具。 當您將編輯儲存至*remote monitoring.h cmakelists.txt*或在某些情況下*CMakeSettings json*時，Visual Studio 會重新產生快取。 但是，如果您使用**現有**的快取設定，則 Visual Studio 不會修改快取。

如需有關 Visual Studio 中 CMake 支援的一般資訊，請參閱 [Visual Studio 中的 CMake 專案](../build/cmake-projects-in-visual-studio.md)。 請先閱讀該文，再於此處繼續。

若要開始使用，**請從主功能表選擇 [** > 檔案] [**開啟** > **資料夾**]，或在 [ `devenv.exe <foldername>` [開發人員命令提示](../build/building-on-the-command-line.md)字元] 視窗中輸入。 您開啟的資料夾中應該有一個*remote monitoring.h cmakelists.txt .txt*檔案，以及您的原始程式碼。

下列範例顯示簡單的*remote monitoring.h cmakelists.txt .txt*檔案和 .cpp 檔案：
...
*Remote monitoring.h cmakelists.txt .txt*：
...
一旦您開啟資料夾，Visual Studio 會剖析*remote monitoring.h cmakelists.txt .txt*檔案，並指定**x86-Debug**的 Windows 目標。 若要以遠端 Linux 系統為目標，請將專案設定變更為 **Linux-Debug** 或 **Linux-Release**。 (請參閱下方的[設定適用於 Linux 的 CMake 設定](#configure_cmake_linux))。
...
Visual Studio 會選擇 [**工具** > ] [**選項** > ] 底下的 [遠端目標的**跨平臺** > **連接管理員**] 清單中的第一個遠端系統。 如果找不到任何遠端連線，系統會提示您建立一個。 如需詳細資訊，請參閱[連線到您的遠端 Linux 電腦](connect-to-your-remote-linux-computer.md)。
...
若要自訂程式的命令列引數，請按**方案總管**頂端的 [**切換目標**] 按鈕，然後選擇 [**目標] View**。 接著，以滑鼠右鍵按一下目標，然後選取 [偵錯並啟動設定]****。 此命令會開啟或建立包含程式相關資訊的*啟動檔案與 json*設定檔案。 若要指定原始程式檔的位置，請將**sourceFileMap**屬性新增至檔案，如下列範例所示：
...
CMake Linux 專案中的*CMakeSettings*可以指定 [[自訂 CMake 設定](../build/customize-cmake-settings.md)] 中列出的所有屬性，再加上控制遠端 Linux 電腦上之組建設定的其他屬性。
...
此命令會顯示 [ **CMake 設定編輯器**]，您可以用來編輯根專案資料夾中的*CMakeSettings*檔案。 您也可以直接在編輯器中按一下 [編輯 JSON]**** 按鈕，開啟該檔案。 如需詳細資訊，請參閱[自訂 CMake 設定](../build/customize-cmake-settings.md)。
...
若要變更 Visual Studio 2017 中的預設 CMake 設定，請從主功能表選擇 [ **CMake** > ] [**變更 CMake 設定** > ] [**remote monitoring.h cmakelists.txt** ]。 或者，以滑鼠右鍵按一下**方案總管**中的 [ *CMakeSettings* ]，然後選擇 [**變更 CMake 設定**]。 Visual Studio 接著會在根專案資料夾中建立新的*CMakeSettings json*檔案。 您可以使用 **CMake 設定**編輯器來開啟檔案，或直接修改檔案。 如需詳細資訊，請參閱[自訂 CMake 設定](../build/customize-cmake-settings.md)。
