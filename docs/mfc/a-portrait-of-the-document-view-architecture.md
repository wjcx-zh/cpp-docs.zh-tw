...
## <a name="updating-multiple-views-of-the-same-document"></a>更新相同文件的多個檢視  
 具有多個檢視相同文件的應用程式中，例如文字編輯器中的分隔視窗 — 檢視第一次將新資料傳遞至文件。 然後它會呼叫文件的[UpdateAllViews](../mfc/reference/cdocument-class.md#updateallviews)成員函式，會告知所有檢視自行更新，以反映新的資料的文件。 這會同步處理檢閱。  
