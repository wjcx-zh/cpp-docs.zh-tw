---
title: ActiveX 控制項容器： 處理從 ActiveX 控制項的事件 |Microsoft Docs
ms.custom: ''
ms.date: 09/12/2018
ms.technology:
- cpp-mfc
...
5.  從下拉式清單方塊的右邊，選取**\<新增 > ClickCircctrl1**。  
  
6.  按兩下新的處理常式函式，可前往 事件處理常式程式碼，在實作中的 類別檢視 (。CPP) 檔案的`CContainerDlg`。  
