...
2.  在每個包含控制項標頭檔的原始程式檔中，在它的前面加入此類別的標頭檔。  
  
3.  在這個類別中，覆寫 `OnDataAvailable`。 每當有資料可供顯示時呼叫這個函式。 當資料可用時，您可以選擇的任何方法進行處理，例如逐步轉譯。  
...
5.  覆寫 `OnProgress`。 在 `OnProgress` 中，會傳遞給您一個數字顯示最大範圍以及一個數字顯示目前下載的進度。 您可以使用這些數字向使用者顯示狀態 (例如完成的百分比)。  
...
4.  將 `CDataPathProperty` 衍生類別的成員變數，宣告為 ActiveX 控制項類別。  
