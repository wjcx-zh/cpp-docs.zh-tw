---
title: 在網際網路上的非同步 Moniker |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
MFC 類別`CDataPathProperty`並[CCachedDataPathProperty](../mfc/reference/ccacheddatapathproperty-class.md)實作可以非同步載入的 ActiveX 控制項屬性。 非同步屬性會在同步初始之後載入。 非同步 ActiveX 控制項會重複叫用回呼，在長時間的屬性交換過程中表示新資料的可用性。

`CDataPathProperty` 衍生自 `CAsyncMonikerFile`。 `CCachedDataPathProperty` 衍生自 `CDataPathProperty`。 若要在您的 ActiveX 控制項中實作非同步屬性，衍生的類別`CDataPathProperty`或是`CCachedDataPathProperty`，並覆寫[OnDataAvailable](../mfc/reference/casyncmonikerfile-class.md#ondataavailable)和其他您想要接收的通知。

#### <a name="to-download-a-file-using-asynchronous-monikers"></a>使用非同步 Moniker 下載檔案

1. 宣告一個衍生自類別[CAsyncMonikerFile](../mfc/reference/casyncmonikerfile-class.md)。

1. 覆寫[OnDataAvailable](../mfc/reference/casyncmonikerfile-class.md#ondataavailable)來顯示資料。

1. 覆寫其他成員函式，包括[OnProgress](../mfc/reference/casyncmonikerfile-class.md#onprogress)， [OnStartBinding](../mfc/reference/casyncmonikerfile-class.md#onstartbinding)，並[OnStopBinding](../mfc/reference/casyncmonikerfile-class.md#onstopbinding)。

1. 宣告這個類別的執行個體並使用它來開啟 URL。

如需在 ActiveX 控制項非同步下載資訊，請參閱[網際網路上的 ActiveX 控制項](../mfc/activex-controls-on-the-internet.md)。
