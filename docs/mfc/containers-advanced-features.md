...
[伺服器：執行伺服器](../mfc/servers-implementing-a-server.md)一文會列出執行伺服器應用程式所需的許多工作。 如果您將容器應用程式轉換成容器/伺服器應用程式，則需要執行一些相同的工作，將程式碼加入至容器。 以下列出一些重要的考量：
...
如果您要建立新的應用程式，並且希望將它當做容器/伺服器應用程式執行，請在應用程式精靈的 [OLE 選項] 對話方塊中選擇該選項，如此就會自動建立這項支援。 如需詳細資訊，請參閱文章[總覽：建立 ActiveX 控制項容器](../mfc/reference/creating-an-mfc-activex-control-container.md)。 如需有關 MFC 範例的詳細資訊，請參閱「MFC 範例」。

請注意，您無法將 MDI 應用程式插入至本身。 做為容器/伺服器的應用程式無法插入至本身，除非它是 SDI 應用程式。

##  <a name="_core_links_to_embedded_objects"></a>内嵌物件的連結
...
1. 在您的應用程式類別的 `InitInstance` 成員函式中，執行下列動作：
   ...
   - 呼叫 `COleTemplateServer::RegisterAll` 成員函式，以使用 OLE 系統註冊所有類別物件。

   - 呼叫 `COleTemplateServer::UpdateRegistry`。 如果應用程式不是以 "帶有/embedded" 參數啟動，則應該*OAT_CONTAINER*唯一的 `UpdateRegistry` 參數。 這樣會將應用程式註冊為可支援內嵌物件連結的容器。

      如果應用程式啟動時帶有 "/Embedded" 參數，則不應該顯示其主視窗，就像伺服器應用程式一樣。

MFC OLE sample [OCLIENT](../overview/visual-cpp-samples.md)會執行這項功能。 如需如何完成這項作業的範例，請參閱 OCLIENT 中的 `InitInstance` 函數 *。* 這個範例應用程式的 CPP 檔案。
