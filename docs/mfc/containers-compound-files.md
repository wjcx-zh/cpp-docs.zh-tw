...
- `LockBytes`物件代表儲存物件與實體硬體之間的介面。 它們會決定實際的位元組如何寫入物件所存取的`LockBytes`任何存放裝置, 例如硬碟或全域記憶體的區域。 如需`LockBytes`物件`ILockBytes`和介面的詳細資訊, 請參閱 OLE 程式設計*人員參考*。
...
###  <a name="_core_incremental_access_to_files"></a>增量存取檔案
...
###  <a name="_core_size_and_performance_considerations"></a>大小和效能考慮
...
##  <a name="_core_using_compound_files_format_for_your_data"></a>針對您的資料使用複合檔案格式
