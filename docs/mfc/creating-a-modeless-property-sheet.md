...
無模式屬性工作表和外部物件之間通訊的一種方法（例如，在視圖中的目前選取範圍）是定義從屬性工作表到外部物件的指標。 定義 `CPropertySheet` 衍生類別中的函式（稱為 `SetMyExternalObject`），以在焦點從某個外部物件變更為另一個時變更指標。 @No__t-0 函數需要重設每個屬性頁的設定，以反映新選取的外部物件。 若要完成此工作，@no__t 0 函式必須能夠存取屬於 @no__t 2 類別的[CPropertyPage](../mfc/reference/cpropertypage-class.md)物件。

在屬性工作表中提供屬性頁存取最方便的方式，就是將 @no__t 0 物件內嵌在 `CPropertySheet` 衍生的物件中。 在 `CPropertySheet` 衍生物件中嵌入 `CPropertyPage` 物件與強制回應對話方塊的一般設計不同，其中屬性工作表的擁有者會建立 @no__t 2 物件，並透過[CPropertySheet：： AddPage](../mfc/reference/cpropertysheet-class.md#addpage)將它們傳遞至屬性工作表。
