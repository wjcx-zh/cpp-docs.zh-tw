...
`// Implementation` 區段是所有 MFC 類別宣告最重要的部分。  
 ...
 請注意從`CStdioFile`底下列出[註解的範例](../mfc/an-example-of-the-comments.md)下面所宣告成員`// Implementation`註解可以宣告為**公用**，**保護**，或**私人**。 您應該謹慎使用這些成員，因為它們在未來可能會變更。 宣告為成員群組的**公用**可能是必要項目類別程式庫實作能正確運作。 不過，這並不表示您可以安全地如其所宣告地使用那些成員。  
