...
MFC 程式庫是一種在 Microsoft Windows 中進行程式設計的應用程式架構。 以 C++ 撰寫，MFC 提供許多處理視窗、功能表和對話方塊、實作基本輸入/輸出、存放資料物件的集合等等功能所需的程式碼。 您需要做的就是將您的應用程式專屬程式碼加入至此架構。 本質的C++類別進行程式設計，就可以輕鬆地延伸或覆寫的基本功能所[MFC 架構](../mfc/framework-mfc.md)提供。
...
`MFC Samples`所提供的[VisualC++範例](../overview/visual-cpp-samples.md)
