...
2.  `Seek`成員函式也會提供將移到檔案中的特定位移。  
 ...
 `Read` 接受指標緩衝區和要讀取的位元組數目，並傳回實際讀取的位元組數目。 如果要求的位元組數無法讀取因為檔案結尾 (EOF) 為止，會傳回實際讀取的位元組數目。 如果發生任何讀取的錯誤時，會擲回例外狀況。 `Write` 類似於`Read`，但不是會傳回寫入的位元組數目。 如果發生寫入錯誤，包括沒有寫入所有指定的位元組，則會擲回例外狀況。 如果您具有有效`CFile`物件，您可以從中讀取或寫入其中，如下列範例所示：  
