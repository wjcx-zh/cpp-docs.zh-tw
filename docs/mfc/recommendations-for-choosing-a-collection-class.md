...
### <a name="collection-shape-features"></a><a name="_core_collection_shape_features"></a>集合元件特徵

|形狀|訂購時間|索引|插入項目|搜尋指定的項目|重複元素|
|-----------|--------------|--------------|-----------------------|----------------------------------|-------------------------|
|清單|是|否|快速|緩慢|是|
|Array|是|依 int|緩慢|緩慢|是|
...
### <a name="characteristics-of-mfc-collection-classes"></a><a name="_core_characteristics_of_mfc_collection_classes"></a>MFC集合類的特點

|類別|使用 C++<br /><br /> 範本|可以是<br /><br /> 已序列化|可以是<br /><br /> 已傾印|Is<br /><br /> Type-Safe - 類型安全|
|-----------|------------------------------|---------------------------|-----------------------|-----------------------|
|`CArray`|是|是 1|是 1|否|
...
1. 若要序列化，您必須明確呼叫集合物件的 `Serialize` 函式；若要傾印，您必須明確呼叫其 `Dump` 函式。 無法使用表單`ar << collObj`序列化或要傾印的表單`dmp``<< collObj`。
