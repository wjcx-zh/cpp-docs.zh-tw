...
|[AfxFreeLibrary](#afxfreelibrary)|遞減參考計數的載入的動態連結程式庫 (DLL) 模組。當參考計數到達零時，模組不相符。|  
...
|[AfxGetMainWnd](#afxgetmainwnd)|傳回目前的 「 主要 」 視窗的非 OLE 應用程式或伺服器應用程式的就地框架視窗的指標。|  
|[AfxGetPerUserRegistration](#afxgetperuserregistration)|使用此函數來判斷應用程式是否將登錄存取重新導向**HKEY_CURRENT_USER** ( **HKCU**) 節點。|  
|[AfxGetResourceHandle](#afxgetresourcehandle)|傳回`HINSTANCE`的應用程式的預設資源的來源。 使用此直接存取應用程式的資源。|  
...
|[AfxRegisterWndClass](#afxregisterwndclass)|註冊 Windows 以補充那些自動註冊 mfc 視窗類別。|  
...
|[AfxSocketInit](#afxsocketinit)|在呼叫`CWinApp::InitInstance`覆寫以初始化 Windows 通訊端。|  
|[AfxWinInit](#afxwininit)|由 MFC 提供呼叫`WinMain`函式，做為一部分[CWinApp](../../mfc/reference/cwinapp-class.md) GUI 型的應用程式，以初始化 MFC 的初始化。 必須針對使用 MFC 的主控台應用程式的直接呼叫。|  
...
### <a name="parameters"></a>參數  
 `pfnThreadProc`  
 背景工作執行緒控制函式的指標。 不能是**NULL**。 此函式必須宣告，如下所示︰  
 ...
 *pParam*  
 參數傳遞至控制函式中的函式宣告的參數中所示`pfnThreadProc`。  
 ...
 `nStackSize`  
 指定的大小，以位元組為單位的新執行緒的堆疊。 如果為 0，堆疊大小會預設為相同大小堆疊與建立的執行緒。  
  
 `dwCreateFlags`  
 指定另一個旗標會控制執行緒的建立。 這個旗標可以包含兩個值之一︰  
  
- **CREATE_SUSPENDED**啟動與暫停計數的其中一個執行緒。 使用**CREATE_SUSPENDED**如果您想要初始化的任何成員資料`CWinThread`物件，例如[m_bAutoDelete](../../mfc/reference/cwinthread-class.md#m_bautodelete)或衍生類別中，執行緒開始執行之前的任何成員。 當您初始化完成之後，使用[CWinThread::ResumeThread](../../mfc/reference/cwinthread-class.md#resumethread)開始執行的執行緒。 執行緒不會執行直到`CWinThread::ResumeThread`呼叫。  
  
- **0**建立後立即啟動執行緒。  
  
 `lpSecurityAttrs`  
 指向[ATTRIBUTES](http://msdn.microsoft.com/library/windows/desktop/aa379560)結構，指定執行緒的安全性屬性。 如果**NULL**，相同的安全性屬性，您將使用建立的執行緒。 如需這個結構的詳細資訊，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
  
### <a name="return-value"></a>傳回值  
 新建立的執行緒的物件、 指標或**NULL**發生故障。  
  
### <a name="remarks"></a>備註  
 第一種形式的`AfxBeginThread`建立背景工作執行緒。 第二個表單建立的執行緒，可能會做為使用者介面執行緒或背景工作執行緒。  
  
 `AfxBeginThread`建立新`CWinThread`物件，會呼叫其[CreateThread](../../mfc/reference/cwinthread-class.md#createthread)函數開始執行執行緒，並傳回執行緒的指標。 整個程序進行檢查以確定所有物件都都會解除配置建立的任何部分失敗。 若要結束的執行緒，請呼叫[AfxEndThread](#afxendthread)從內的執行緒或從工作者執行緒控制函式傳回。  
  
 必須啟用多執行緒應用程式。否則，此函式會失敗。 如需有關如何啟用多執行緒處理的詳細資訊，請參閱[/MD、 /MT、 /LD （使用執行階段程式庫）](../../build/reference/md-mt-ld-use-run-time-library.md)下*Visual c + + 編譯器選項*。  
...
### <a name="example"></a>範例  
 請參閱範例[CSocket::Attach](../../mfc/reference/csocket-class.md#attach)。  
...
### <a name="remarks"></a>備註  
 ...
 請務必使用`AfxFreeLibrary`和`AfxLoadLibrary`(而不是 Win32 函式**FreeLibrary**和**LoadLibrary**) 如果您的應用程式使用多個執行緒。 使用`AfxLoadLibrary`和`AfxFreeLibrary`可確保載入及卸載 DLL 的延伸模組未損毀的通用的 MFC 狀態時執行的啟動和關閉程式碼。  
  
### <a name="example"></a>範例  
 請參閱範例[AfxLoadLibrary](#afxloadlibrary)。  
  
### <a name="requirements"></a>需求  
  **標頭**afxdll_.h  
  
##  <a name="a-nameafxgetappa--afxgetapp"></a><a name="afxgetapp"></a>AfxGetApp  
 此函式所傳回的指標可以用來存取應用程式資訊，例如主要的訊息分派程式碼或最上層視窗。  
...
### <a name="remarks"></a>備註  
 如果這個方法會傳回 NULL，則可能表示細分 €™ 主視窗有尚未完全初始化。 它也可能表示有問題。  
...
##  <a name="a-nameafxgetmainwnda--afxgetmainwnd"></a><a name="afxgetmainwnd"></a>AfxGetMainWnd  
 如果您的應用程式是 OLE 伺服器，呼叫此函式可擷取而不是直接參考應用程式的使用中的主視窗的指標[m_pMainWnd](../../mfc/reference/cwinthread-class.md#m_pmainwnd)應用程式物件的成員。  
...
### <a name="remarks"></a>備註  
 如果您啟用登錄重新導向，架構會重新導向存取權**HKCR**至**HKEY_CURRENT_USER\Software\Classes**。 只有 MFC 和 ATL 架構會受到重新導向。  
...
##  <a name="a-nameafxgetthreada--afxgetthread"></a><a name="afxgetthread"></a>AfxGetThread  
 呼叫此函式可取得的指標[CWinThread](../../mfc/reference/cwinthread-class.md)物件，代表目前執行的執行緒。  
...
### <a name="return-value"></a>傳回值  
 指標至目前正在執行的執行緒。否則**NULL**。  
  
### <a name="remarks"></a>備註  
 必須從所需的執行緒內呼叫。  
  
> [!NOTE]
>  如果您要移植 MFC 專案呼叫`AfxGetThread`Visual c + + 版本 4.2、 5.0 或 6.0 中，從`AfxGetThread`呼叫[AfxGetApp](#afxgetapp)找不到任何執行緒時。 在 Visual c +.NET 和更新版本，`AfxGetThread`傳回**NULL**如果找不到任何執行緒。 如果您要應用程式執行緒，則必須呼叫 `AfxGetApp`。  
...
##  <a name="a-nameafxinitrichedita--afxinitrichedit"></a><a name="afxinitrichedit"></a>AfxInitRichEdit  
 呼叫此函式以初始化應用程式中的使用 rich edit 控制項 （1.0 版）。  
...
### <a name="remarks"></a>備註  
 ...
 `AfxInitRichEdit`載入 RICHED32。初始化 1.0 版的 rich edit 控制項的 DLL。 若要使用版本 2.0 和 3.0 的 rich edit 控制項，RICHED20。需要載入 DLL。 這是藉由呼叫[AfxInitRichEdit2](#afxinitrichedit2)。 如果您有使用 Visual c + +.NET 之前建立的 rich edit 控制項的對話方塊資源，windows rich edit 控制項都會自動 1.0 版。 Rich edit 控制項插入使用 Visual c + +.NET 的資源編輯器是 2.0 版。  
  
 若要更新現有的 Visual c + + 應用程式以 2.0 版中的豐富的編輯控制項，開啟。RC 檔為文字，變更 「 RichEdit20a 」 從 「 RICHEDIT 「 每個 rich edit 控制項的類別名稱。 然後取代呼叫`AfxInitRichEdit`與`AfxInitRichEdit2`。  
  
 此函式也會初始化通用控制項程式庫中，如果程式庫尚未初始化處理程序。 如果您直接從 MFC 應用程式中使用 rich edit 控制項，您應該呼叫此函式可確保 MFC 已正確初始化 windows rich edit 控制項的執行階段。 如果您呼叫的 Create 方法[CRichEditCtrl](../../mfc/reference/cricheditctrl-class.md)， [CRichEditView](../../mfc/reference/cricheditview-class.md)，或[CRichEditDoc](../../mfc/reference/cricheditdoc-class.md)，則通常不需要呼叫此函式，但在某些情況下可能需要。  
 ...
 *lpszModuleName*  
 ...
 如果此字串指定的路徑，但檔案不存在指定的目錄中，函式會失敗。  
  
 如果未指定路徑和檔名的副檔名省略，預設的延伸模組。DLL 會附加。 不過，檔案名稱的字串可以包含尾端點字元 （.） 來表示模組名稱並沒有副檔名。 未指定路徑，函式會搜尋下列程序中的檔案︰  
...
- **僅限 Windows NT:** 16 位元 Windows 系統目錄。 取得此目錄中，路徑的 Win32 函式，但它會搜尋。 這個目錄的名稱是系統。  
...
### <a name="return-value"></a>傳回值  
 如果函式成功，則傳回的值會是模組的控制代碼。 如果函式失敗，傳回的值是 NULL。  
  
### <a name="remarks"></a>備註  
 它會傳回的控制代碼，可用於[GetProcAddress](http://msdn.microsoft.com/library/windows/desktop/ms683212)取得 DLL 函式的位址。 `AfxLoadLibrary`也可以用來對應其他可執行模組。  
  
 每個處理序會維護每個載入的程式庫模組的參考計數。 此參考計數會遞增一次`AfxLoadLibrary`稱為，會減少每次`AfxFreeLibrary`呼叫。 在參考計數達到零時，模組就從呼叫處理序的位址空間取消對應，且控制代碼不再有效。  
  
 請務必使用`AfxLoadLibrary`和`AfxFreeLibrary`(而不是 Win32 函式**LoadLibrary**和**FreeLibrary**) 如果您的應用程式使用多個執行緒，而且它會以動態方式載入擴充 DLL。 使用`AfxLoadLibrary`和`AfxFreeLibrary`car 載入及卸載擴充 DLL 時執行的啟動和關閉程式碼未損毀的通用的 MFC 狀態。  
  
 使用`AfxLoadLibrary`應用程式中需要您以動態方式連結至 MFC 的 DLL 版本; 的標頭檔`AfxLoadLibrary`，Afxdll_.h，只會納入 MFC 會連結至 dll 的應用程式。 這是經過設計，因此您可以連結至 MFC 的 DLL 版本使用，或建立擴充 Dll。  
...
### <a name="parameters"></a>參數  
 *lpWndClass*  
 指標[WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576)結構，其中包含要註冊視窗類別相關資訊。 如需這個結構的詳細資訊，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
  
### <a name="return-value"></a>傳回值  
 **TRUE**如果類別成功登錄，否則**FALSE**。  
  
### <a name="remarks"></a>備註  
 如果您使用這個函式，類別會在卸載 DLL 時自動解除登錄。  
  
 在非 DLL 組建`AfxRegisterClass`識別項定義為巨集對應至 Windows 函式**RegisterClass**，因為登錄應用程式中的類別會自動解除登錄。 如果您使用`AfxRegisterClass`而不是**RegisterClass**，可以在應用程式和 DLL 中的變更若不使用程式碼。  
...
### <a name="parameters"></a>參數  
 *nClassStyle*  
 指定的 Windows 類別樣式或樣式，使用位元 OR 組合 ( **|**) 運算子，視窗類別。 如需類別樣式，請參閱[WNDCLASS](http://msdn.microsoft.com/library/windows/desktop/ms633576)結構[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。 如果**NULL**，其預設值會設定，如下所示︰  
  
-   將滑鼠樣式設定為**CS_DBLCLKS**，會將按兩下訊息至視窗程序當使用者按兩下滑鼠。  
  
-   將箭號游標樣式設定為 Windows 標準**IDC_ARROW**。  
  
-   若要設定的背景筆刷**NULL**，因此，視窗不會清除它的背景。  
 ...
 *hbrBackground*  
 指定將在從視窗類別建立之各個視窗中安裝的筆刷資源控制代碼。 如果您使用預設值是**0**，您必須**NULL**背景筆刷，因此您 視窗中，依預設，不會清除它的背景處理[WM_ERASEBKGND](http://msdn.microsoft.com/library/windows/desktop/ms648055)。  
  
 `hIcon`  
 指定將在從視窗類別建立之各個視窗中安裝的圖示資源控制代碼。 如果您使用預設值是**0**，您會取得標準的旗標 Windows 標誌圖示。  
...
### <a name="remarks"></a>備註  
 ...
 如果以相同參數對多個 CWnd 衍生類別呼叫 `AfxRegisterWndClass`，除了取得每個類別的獨立視窗類別之外，每個類別還會共用相同的視窗類別。 這會造成問題，如果**CS_CLASSDC**類別樣式。 除了多個**CS_CLASSDC**視窗類別，您會得到一個**CS_CLASSDC**視窗類別，並使用類別共用相同的 DC 的所有 c + + 視窗。 若要避免這個問題，請呼叫[AfxRegisterClass](#afxregisterclass)註冊類別。  
...
### <a name="remarks"></a>備註  
 之前[!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]，通常存取登錄的應用程式使用**HKEY_CLASSES_ROOT**節點。 不過，若使用[!INCLUDE[wiprlhext](../../c-runtime-library/reference/includes/wiprlhext_md.md)]，您必須在提高權限寫入模式中執行應用程式**HKCR**。  
  
 這個方法可讓您的應用程式來讀取和寫入登錄，而不需要在提高權限的模式下執行，藉由從登錄存取重新導向**HKCR**至**HKCU**。 如需詳細資訊，請參閱[連結器屬性頁](../../ide/linker-property-pages.md)。  
...
##  <a name="a-nameafxsocketinita--afxsocketinit"></a><a name="afxsocketinit"></a>AfxSocketInit  
 呼叫此函式，您`CWinApp::InitInstance`覆寫以初始化 Windows 通訊端。  
...
### <a name="parameters"></a>參數  
 `lpwsaData`  
 指標[WSADATA](../../mfc/reference/wsadata-structure.md)結構。 如果`lpwsaData`不等於`NULL`，然後位址`WSADATA`結構由呼叫填入`WSAStartup`。 此函式也可以確保，`WSACleanup`應用程式終止之前，進行呼叫。  
...
##  <a name="a-nameafxwininita--afxwininit"></a><a name="afxwininit"></a>AfxWinInit  
 由 MFC 提供呼叫此函式`WinMain`函式，做為一部分[CWinApp](../../mfc/reference/cwinapp-class.md) GUI 型的應用程式，以初始化 MFC 的初始化。  
 ...
 *hPrevInstance*  
 應用程式的上一個執行個體控制代碼。 Win32 架構應用程式中，這個參數是一律**NULL**。  
  
 `lpCmdLine`  
 指向以 null 結束的字串，指定命令列應用程式。  
  
 `nCmdShow`  
 指定會顯示 GUI 應用程式的主視窗的方式。  
  
### <a name="remarks"></a>備註  
 主控台應用程式，這不會使用 MFC 提供`WinMain`函式，您必須呼叫`AfxWinInit`直接以初始化 MFC。  
  
 如果您呼叫`AfxWinInit`自己，您應該宣告的執行個體`CWinApp`類別。 主控台應用程式，您可以選擇不衍生您自己從`CWinApp`，而是使用的執行個體`CWinApp`直接。 這項技術是如果您決定要保留您的應用程式的所有功能的實作中適當**主要**。  
  
> [!NOTE]
>  當它在建立啟用內容的組件時，MFC 會使用使用者模組所提供的資訊清單資源。 啟用內容可在 `AfxWinInit` 中建立。 如需詳細資訊，請參閱[MFC 模組狀態的啟用內容支援](../../mfc/support-for-activation-contexts-in-the-mfc-module-state.md)。  
