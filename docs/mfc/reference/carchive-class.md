...
使用`IsBufferEmpty`與`CSocketFile`物件相關聯的封存的原因是封存的緩衝區可能包含一個以上的訊息或記錄。 收到一則訊息之後，您應該`IsBufferEmpty`使用來控制在緩衝區空白之前繼續接收資料的迴圈。 如需詳細資訊，請參閱類別`CAsyncSocket`的 [Receive](../../mfc/reference/casyncsocket-class.md#receive) 成員函式，其會顯示如何使用。`IsBufferEmpty`
...
如果封存`IsLoading`狀態為非零值，則其狀態為0，反之亦然。 `IsStoring`
