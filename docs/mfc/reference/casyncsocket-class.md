...
|[CAsyncSocket：:D etach](#detach)|從 `CAsyncSocket` 物件卸離通訊端控制碼。|
|[CAsyncSocket：： FromHandle](#fromhandle)|指定通訊端控制碼，傳回 `CAsyncSocket` 物件的指標。|
...
|[CAsyncSocket：： ShutDown](#shutdown)|停用通訊端上的 `Send` 和/或 `Receive` 呼叫。|
...
|[CAsyncSocket：： OnReceive](#onreceive)|通知接聽通訊端，藉由呼叫 `Receive`來抓取資料。|
...
|名稱|描述|
|----------|-----------------|
|[CAsyncSocket：： m_hSocket](#m_hsocket)|指出附加至此 `CAsyncSocket` 物件的通訊端控制碼。|

## <a name="remarks"></a>備註

類別 `CAsyncSocket` 封裝 Windows 通訊端函式 API，為想要搭配 MFC 使用 Windows 通訊端的程式設計人員提供物件導向的抽象概念。
...
若要使用 `CAsyncSocket` 物件，請呼叫其函式，然後呼叫[create](#create)函式來建立基礎通訊端控制碼（類型 `SOCKET`），但已接受的通訊端除外。 若為伺服器通訊端，請呼叫[接聽](#listen)成員函式，並針對用戶端通訊端呼叫[Connect](#connect)成員函式。 伺服器通訊端應該在收到連接要求時呼叫[Accept](#accept)函式。 使用其餘的 `CAsyncSocket` 函數來執行通訊端之間的通訊。 完成時，如果已在堆積上建立 `CAsyncSocket` 物件，請將它摧毀。此析構函數會自動呼叫[Close](#close)函式。 SOCKET 資料類型會在[Windows 通訊端：背景](../../mfc/windows-sockets-background.md)一文中說明。
...
此常式會解壓縮暫止連接佇列中的第一個連接、使用與此通訊端相同的屬性來建立新的通訊端，並將其附加至*rConnectedSocket*。 如果佇列中沒有暫止的連接，`Accept` 會傳回零，而 `GetLastError` 會傳回錯誤。 接受的通訊端（ *rConnectedSocket）* 不能用來接受更多連接。 原始通訊端會保持開啟並接聽。

引數*lpSockAddr*是一個結果參數，會以連接通訊端的位址填入，如同通訊層所知。 `Accept` 用於以連接為基礎的通訊端類型，例如 SOCK_STREAM。
...
此常式會用於未連接的資料包或資料流程通訊端，然後再 `Connect` 或 `Listen` 呼叫。 接聽伺服器通訊端必須先選取埠號碼，並藉由呼叫 `Bind`，使其成為 Windows Socket，才可以接受連線要求。 `Bind` 會將區功能變數名稱稱指派給未命名的通訊端，藉以建立通訊端的本機關聯（主機位址/埠號碼）。
...
在建立物件之後，您必須呼叫其 `Create` 成員函式，以建立通訊端資料結構並系結其位址。 （在 Windows 通訊端通訊的伺服器端，當接聽通訊端建立要在 `Accept` 呼叫中使用的通訊端時，您不會呼叫該通訊端的 `Create`。）
...
此函式會釋出通訊端描述元，以便進一步參考它將會失敗，並出現錯誤 WSAENOTSOCK。 如果這是基礎通訊端的最後一個參考，則會捨棄相關聯的命名資訊和佇列資料。 通訊端物件的析構函式會為您呼叫 `Close`。

針對 `CAsyncSocket`，但不適用於 `CSocket`，`Close` 的語義會受到通訊端選項 SO_LINGER 和 SO_DONTLINGER 的影響。 如需詳細資訊，請參閱成員函式 `GetSockOpt`。
...
如果通訊端未系結，則系統會將唯一的值指派給本機關聯，並將通訊端標示為已系結。 請注意，如果名稱結構的 [位址] 欄位全部都是零，`Connect` 將會傳回零。 若要取得延伸錯誤資訊，請呼叫 `GetLastError` 成員函式。

對於資料流程通訊端（類型 SOCK_STREAM），會起始作用中的連接到外部主機。 當通訊端呼叫成功完成時，通訊端已準備好傳送/接收資料。

若為資料包通訊端（類型 SOCK_DGRAM），則會設定預設目的地，以用於後續的 `Send` 和 `Receive` 呼叫。
...
`Create` 會呼叫[Socket](#socket) ，如果成功，它會呼叫[bind](#bind)來將通訊端系結至指定的位址。 支援的通訊端類型如下：
...
> [!IMPORTANT]
> `Create`**不**是安全線程。  如果您要在多執行緒環境中呼叫它，而執行緒可以由不同的執行緒同時叫用，請務必使用 mutex 或其他同步處理鎖定來保護每個呼叫。
...
呼叫這個成員函式，從 `CAsyncSocket` 物件將*m_hSocket*資料成員中的通訊端控制碼卸離，並將*M_HSOCKET*設定為 Null。
...
`CAsyncSocket` 物件的指標，如果沒有任何 `CAsyncSocket` 物件附加至*hSocket*，則為 Null。

### <a name="remarks"></a>備註

當指定通訊端控制碼時，如果 `CAsyncSocket` 物件未附加至控制碼，則成員函式會傳回 Null。
...
當特定成員函式指出發生錯誤時，應該呼叫 `GetLastError` 來取得適當的錯誤碼。 如需適用的錯誤碼清單，請參閱個別成員函式描述。
...
此呼叫特別適用于已進行 `Connect` 呼叫，而未先執行 `Bind` 的作業;這個呼叫會提供唯一的方法，讓您判斷系統所設定的本機關聯。
...
*lpOptionValue*<br/>
緩衝區的指標，其中會傳回所要求之選項的值。 與選取的選項相關聯的值會在緩衝區*lpOptionValue*中傳回。 *LpOptionLen*所指向的整數原本應該包含這個緩衝區的大小（以位元組為單位）;傳回時，會將它設定為傳回值的大小。 針對 SO_LINGER，這會是 `LINGER` 結構的大小;針對所有其他選項，它會是 BOOL 或**int**的大小，視選項而定。 請在 [備註] 區段中查看選項及其大小的清單。
...
如果函式成功，則為非零;否則，您可以藉由呼叫[GetLastError](#getlasterror)來抓取特定的錯誤碼。 如果未使用 `SetSockOpt`設定選項，則 `GetSockOpt` 會傳回選項的預設值。 下列錯誤適用于此成員函式：
...
`GetSockOpt` 會在任何狀態下，抓取與任何類型之通訊端相關聯之通訊端選項的目前值，並將結果儲存在*lpOptionValue*中。 選項會影響通訊端作業，例如封包的路由、頻外資料傳輸等等。

`GetSockOpt`支援下列選項。 型別會識別*lpOptionValue*所定址的資料類型。 TCP_NODELAY 選項會使用層級 IPPROTO_TCP;所有其他選項都會使用層級 SOL_SOCKET。
...
`GetSockOpt` 不支援的 Berkeley 軟體發佈（BSD）選項如下：
...
使用不支援的選項呼叫 `GetSockOpt`，會導致從 `GetLastError`傳回 WSAENOPROTOOPT 的錯誤碼。
...
- FIONBIO 啟用或停用通訊端上的非封鎖模式。 *LpArgument*參數會指向 `DWORD`，如果要啟用非封鎖模式則為非零，如果要停用則為零。 如果已在通訊端上發出 `AsyncSelect`，則嘗試使用 `IOCtl` 將通訊端設定回封鎖模式將會失敗，並產生 WSAEINVAL。 若要將通訊端設定回封鎖模式並防止 WSAEINVAL 錯誤，應用程式必須先透過呼叫*lEvent*參數等於0的 `AsyncSelect` 來停用 `AsyncSelect`，然後再呼叫 `IOCtl`。

- FIONREAD 會決定可以使用此通訊端的一個 `Receive` 呼叫來讀取的最大位元組數目。 *LpArgument*參數會指向 `IOCtl` 儲存結果的 `DWORD`。 如果此通訊端屬於 SOCK_STREAM 類型，則 FIONREAD 會傳回可在單一 `Receive`中讀取的總數據量;這通常與通訊端上排入佇列的總數據量相同。 如果此通訊端屬於 SOCK_DGRAM 類型，則 FIONREAD 會傳回在通訊端上排入佇列的第一個資料包大小。

- SIOCATMARK 判斷是否已讀取所有頻外資料。 這只適用于 SOCK_STREAM 類型的通訊端，其已設定為可在任何頻外資料（SO_OOBINLINE）的行內接收。 如果沒有頻外資料正在等候讀取，作業會傳回非零值。 否則，它會傳回0，而在通訊端上執行的下一個 `Receive` 或 `ReceiveFrom` 將會抓取 "mark" 前面的部分或所有資料。應用程式應該使用 SIOCATMARK 作業來判斷是否有任何資料保留。 如果在「緊急」（頻外）資料之前有任何一般資料，則會依序接收。 （請注意，`Receive` 或 `ReceiveFrom` 絕對不會在相同的呼叫中混用頻外和一般資料）。*LpArgument*參數會指向 `IOCtl` 儲存結果的 `DWORD`。

此函式是 Berkeley 通訊端中使用的 `ioctl()` 子集。 特別的是，沒有相當於 FIOASYNC 的命令，而 SIOCATMARK 是唯一支援的通訊端層級命令。

##  <a name="listen"></a>CAsyncSocket：：接聽
...
- WSAEOPNOTSUPP 參考的通訊端不是支援 `Listen` 作業的類型。

### <a name="remarks"></a>備註

若要接受連接，會先使用 `Create`建立通訊端，然後使用 `Listen`指定連入連線的待處理專案，然後使用 `Accept`來接受連線。 `Listen` 僅適用于支援連線的通訊端，也就是 SOCK_STREAM 型別的介面。 此通訊端會進入「被動」模式，其中的傳入連線會被認可並排入佇列等待處理常式接受。

此函式通常是由伺服器（或任何想要接受連線的應用程式）所使用，其中一次可能會有一個以上的連線要求：如果連接要求抵達佇列已滿，用戶端將會收到錯誤，指出：WSAECONNREFUSED.

當沒有可用的埠（描述項）時，`Listen` 會嘗試繼續等理性判斷函數。 它會接受連線，直到佇列清空為止。 如果埠可供使用，稍後呼叫 `Listen` 或 `Accept` 會將佇列重新填滿目前或最新的「待處理專案」（如果可能的話），並繼續接聽連入連接。
...
*nErrorCode*<br/>
最新的通訊端錯誤。 下列錯誤碼適用于 `OnAccept` 成員函式：
...
*nErrorCode*<br/>
最新的通訊端錯誤。 下列錯誤代碼適用于 `OnClose` 成員函式：
...
*nErrorCode*<br/>
最新的通訊端錯誤。 下列錯誤代碼適用于 `OnConnect` 成員函式：
...
> [!NOTE]
>  在[CSocket](../../mfc/reference/csocket-class.md)中，永遠不會呼叫 `OnConnect` 通知函數。 針對連線，您只需要呼叫 `Connect`，這會在連接完成時傳回（不論是成功或錯誤）。 連接通知的處理方式是 MFC 的執行詳細資料。
...
*nErrorCode*<br/>
最新的通訊端錯誤。 下列錯誤代碼適用于 `OnOutOfBandData` 成員函式：
...
MFC 支援頻外資料，但不建議使用類別 `CAsyncSocket` 的使用者。 較簡單的方式是建立第二個通訊端來傳遞這類資料。 如需頻外資料的詳細資訊，請參閱[Windows socket：通訊端通知](../../mfc/windows-sockets-socket-notifications.md)。
...
*nErrorCode*<br/>
最新的通訊端錯誤。 下列錯誤代碼適用于 `OnReceive` 成員函式：
...
*nErrorCode*<br/>
最新的通訊端錯誤。 下列錯誤代碼適用于 `OnSend` 成員函式：
...
如果沒有發生錯誤，`Receive` 會傳回接收的位元組數目。 如果連接已關閉，它會傳回0。 否則，會傳回 SOCKET_ERROR 的值，並藉由呼叫[GetLastError](#getlasterror)來抓取特定的錯誤碼。 下列錯誤適用于此成員函式：
...
- WSAESHUTDOWN 通訊端已關閉;在*nHow*設定為0或2的情況下叫用 `ShutDown` 之後，就不能在通訊端上呼叫 `Receive`。
...
對於資料包通訊端，資料會從第一個已排入佇列的資料包（最大到提供的緩衝區大小）解壓縮。 如果資料包大於提供的緩衝區，則緩衝區會填入資料包的第一個部分，多餘的資料會遺失，而 `Receive` 會傳回 SOCKET_ERROR 的值，並將錯誤碼設定為 WSAEMSGSIZE。 如果通訊端沒有內送資料可供使用，則會傳回 SOCKET_ERROR 的值，並將錯誤碼設定為 WSAEWOULDBLOCK。 [OnReceive](#onreceive)回呼函數可以用來判斷何時會到達更多資料。

如果通訊端的類型 SOCK_STREAM，而遠端端已正常關閉連線，則 `Receive` 會立即完成，並收到0個位元組。 如果已重設連接，`Receive` 將會失敗，並出現錯誤 WSAECONNRESET。

每次呼叫[CAsyncSocket：： OnReceive](#onreceive)時，都只能呼叫一次 `Receive`。
...
如果沒有發生錯誤，`ReceiveFrom` 會傳回接收的位元組數目。 如果連接已關閉，它會傳回0。 否則，會傳回 SOCKET_ERROR 的值，並藉由呼叫 `GetLastError`來抓取特定的錯誤碼。 下列錯誤適用于此成員函式：
...
- WSAESHUTDOWN 通訊端已關閉;在*nHow*設定為0或2的情況下叫用 `ShutDown` 之後，就不能在通訊端上呼叫 `ReceiveFrom`。
...
針對類型 SOCK_STREAM 的通訊端，會傳回目前提供的緩衝區大小最多的資訊。 如果通訊端已設定為用於頻外資料的行內接收（通訊端選項 SO_OOBINLINE），而且未讀取頻外資料，則只會傳回頻外資料。 應用程式可以使用 [`IOCtlSIOCATMARK`] 選項或 [`OnOutOfBandData`]，來判斷是否仍會讀取其他超出範圍的資料。 SOCK_STREAM 通訊端會忽略*lpSockAddr*和*lpSockAddrLen*參數。

對於資料包通訊端，資料會從第一個已排入佇列的資料包（最大到提供的緩衝區大小）解壓縮。 如果資料包大於提供的緩衝區，則緩衝區會填入訊息的第一個部分，多餘的資料會遺失，而 `ReceiveFrom` 會傳回 SOCKET_ERROR 的值，並將錯誤碼設定為 WSAEMSGSIZE。

如果*lpSockAddr*為非零值，而通訊端的類型 SOCK_DGRAM，則傳送資料之通訊端的網路位址會複製到對應的[SOCKADDR](/windows/win32/winsock/sockaddr-2)結構。 *LpSockAddrLen*所指向的值會初始化為此結構的大小，並在傳回時修改，以表示儲存在該處的位址實際大小。 如果通訊端沒有內送資料可供使用，`ReceiveFrom` 呼叫會等待資料抵達，除非通訊端為非封鎖。 在此情況下，會傳回 SOCKET_ERROR 的值，並將錯誤碼設定為 WSAEWOULDBLOCK。 `OnReceive` 回呼可用來判斷何時會到達更多資料。

如果通訊端的類型 SOCK_STREAM，而遠端端已正常關閉連線，則 `ReceiveFrom` 會立即完成，並收到0個位元組。
...
如果沒有發生錯誤，`ReceiveFromEx` 會傳回接收的位元組數目。 如果連接已關閉，它會傳回0。 否則，會傳回 SOCKET_ERROR 的值，並藉由呼叫 `GetLastError`來抓取特定的錯誤碼。 下列錯誤適用于此成員函式：
...
- WSAESHUTDOWN 通訊端已關閉;在*nHow*設定為0或2的情況下叫用 `ShutDown` 之後，就不能在通訊端上呼叫 `ReceiveFromEx`。
...
對於資料包通訊端，資料會從第一個已排入佇列的資料包（最大到提供的緩衝區大小）解壓縮。 如果資料包大於提供的緩衝區，則緩衝區會填入訊息的第一個部分，多餘的資料會遺失，而 `ReceiveFromEx` 會傳回 SOCKET_ERROR 的值，並將錯誤碼設定為 WSAEMSGSIZE。

如果*lpSockAddr*為非零值，而通訊端的類型 SOCK_DGRAM，則傳送資料之通訊端的網路位址會複製到對應的[SOCKADDR](/windows/win32/winsock/sockaddr-2)結構。 *LpSockAddrLen*所指向的值會初始化為此結構的大小，並在傳回時修改，以表示儲存在該處的位址實際大小。 如果通訊端沒有內送資料可供使用，`ReceiveFromEx` 呼叫會等待資料抵達，除非通訊端為非封鎖。 在此情況下，會傳回 SOCKET_ERROR 的值，並將錯誤碼設定為 WSAEWOULDBLOCK。 `OnReceive` 回呼可用來判斷何時會到達更多資料。

如果通訊端的類型 SOCK_STREAM，而遠端端已正常關閉連線，則 `ReceiveFromEx` 會立即完成，並收到0個位元組。
...
如果沒有發生錯誤，`Send` 會傳回已傳送的字元總數。 （請注意，這可能小於*nBufLen*所指出的數位）。否則，會傳回 SOCKET_ERROR 的值，並藉由呼叫[GetLastError](#getlasterror)來抓取特定的錯誤碼。 下列錯誤適用于此成員函式：
...
- WSAESHUTDOWN 通訊端已關閉;在*nHow*設定為1或2的情況下叫用 `ShutDown` 之後，就無法在通訊端上呼叫 `Send`。
...
`Send` 可用來寫入已連接資料流程或資料包通訊端上的傳出資料。 對於資料包通訊端，必須小心不要超過基礎子網的 IP 封包大小上限，這是由 `AfxSocketInit`所傳回之[WSADATA](/windows/win32/api/winsock2/ns-winsock2-wsadata)結構中的 `iMaxUdpDg` 元素所指定。 如果資料太長而無法透過基礎通訊協定以不變的方式傳遞，則錯誤 WSAEMSGSIZE 會經由 `GetLastError`傳回，而且不會傳輸任何資料。

請注意，對於資料包通訊端，成功完成 `Send` 並不表示已成功傳遞資料。

在 SOCK_STREAM 類型的 `CAsyncSocket` 物件上，寫入的位元組數目可以介於1到要求的長度之間，視本機和外部主機上的緩衝區可用性而定。
...
如果沒有發生錯誤，`SendTo` 會傳回已傳送的字元總數。 （請注意，這可能小於*nBufLen*所指出的數位）。否則，會傳回 SOCKET_ERROR 的值，並藉由呼叫[GetLastError](#getlasterror)來抓取特定的錯誤碼。 下列錯誤適用于此成員函式：
...
- WSAESHUTDOWN 通訊端已關閉;在*nHow*設定為1或2的情況下叫用 `ShutDown` 之後，就無法在通訊端上呼叫 `SendTo`。
...
`SendTo` 用於資料包或資料流程通訊端上，並且用來寫入通訊端上的輸出資料。 對於資料包通訊端，必須小心不要超過基礎子網的 IP 封包大小上限，這是由[AfxSocketInit](../../mfc/reference/application-information-and-management.md#afxsocketinit)所填寫的[WSADATA](/windows/win32/api/winsock2/ns-winsock2-wsadata)結構中的 `iMaxUdpDg` 元素所指定。 如果資料太長而無法透過基礎通訊協定以不變的方式傳遞，則會傳回錯誤 WSAEMSGSIZE，而且不會傳輸任何資料。

請注意，成功完成 `SendTo` 並不表示已成功傳遞資料。

`SendTo` 只會在 SOCK_DGRAM 通訊端上使用，以將資料包傳送至*lpSockAddr*參數所識別的特定通訊端。
...
如果沒有發生錯誤，`SendToEx` 會傳回已傳送的字元總數。 （請注意，這可能小於*nBufLen*所指出的數位）。否則，會傳回 SOCKET_ERROR 的值，並藉由呼叫[GetLastError](#getlasterror)來抓取特定的錯誤碼。 下列錯誤適用于此成員函式：
...
- WSAESHUTDOWN 通訊端已關閉;在*nHow*設定為1或2的情況下叫用 `ShutDown` 之後，就無法在通訊端上呼叫 `SendToEx`。
...
`SendToEx` 用於資料包或資料流程通訊端上，並且用來寫入通訊端上的輸出資料。 對於資料包通訊端，必須小心不要超過基礎子網的 IP 封包大小上限，這是由[AfxSocketInit](../../mfc/reference/application-information-and-management.md#afxsocketinit)所填寫的[WSADATA](/windows/win32/api/winsock2/ns-winsock2-wsadata)結構中的 `iMaxUdpDg` 元素所指定。 如果資料太長而無法透過基礎通訊協定以不變的方式傳遞，則會傳回錯誤 WSAEMSGSIZE，而且不會傳輸任何資料。

請注意，成功完成 `SendToEx` 並不表示已成功傳遞資料。

`SendToEx` 只會在 SOCK_DGRAM 通訊端上使用，以將資料包傳送至*lpSockAddr*參數所識別的特定通訊端。
...
`SetSockOpt` 會在任何狀態下，設定與任何類型之通訊端相關聯之通訊端選項的目前值。 雖然選項可以存在於多個通訊協定層級，但此規格只會定義存在於最上方「通訊端」層級的選項。 選項會影響通訊端作業，例如，在一般資料流程中是否接收到快速資料、是否可以在通訊端上傳送廣播訊息等等。

通訊端選項有兩種類型：啟用或停用功能或行為的布林值選項，以及需要整數值或結構的選項。 若要啟用布林值選項， *lpOptionValue*會指向非零整數。 若要停用選項*lpOptionValue*指向等於零的整數。 *nOptionLen*應該等於布林值選項的 `sizeof(BOOL)`。 針對其他選項， *lpOptionValue*會指向包含選項所需值的整數或結構，而*nOptionLen*是整數或結構的長度。

SO_LINGER 控制當未傳送的資料在通訊端上排入佇列時所採取的動作，並呼叫 `Close` 函式來關閉通訊端。
...
為了通知 Windows 通訊端，不允許在通訊端上進行 `Bind` 呼叫，因為所需的位址已由另一個通訊端使用中，所以應用程式應該先設定通訊端的 SO_REUSEADDR 通訊端選項，然後再發出 `Bind` 呼叫。 請注意，只有在 `Bind` 呼叫時，才會解讀此選項：因此，在不會系結到現有位址的通訊端上設定此選項，以及在 `Bind` 呼叫不會影響此或任何其他通訊端之後，設定或重設此選項是不必要的（但無害）。
...
`SetSockOpt`支援下列選項。 型別會識別*lpOptionValue*所定址的資料類型。

|值|類型|意義|
|-----------|----------|-------------|
|SO_BROADCAST|BOOL|允許在通訊端上傳輸廣播訊息。|
|SO_DEBUG|BOOL|記錄偵錯資訊。|
|SO_DONTLINGER|BOOL|請不要封鎖 `Close` 等待傳送未傳送的資料。 設定此選項相當於設定 SO_LINGER，`l_onoff` 設定為零。|
|SO_DONTROUTE|BOOL|不路由：直接傳送至介面。|
|SO_KEEPALIVE|BOOL|傳送 keep-alive。|
|SO_LINGER|`struct LINGER`|如果沒有未傳送的資料，則會在 `Close` 逗留。|
...
`SetSockOpt` 不支援的 Berkeley 軟體發佈（BSD）選項如下：
...
`ShutDown` 會用於所有類型的通訊端上，以停用接收、傳輸或兩者。 如果*nHow*為0，則不允許在通訊端上進行後續的接收。 這不會影響較低的通訊協定層。
...
請注意，`ShutDown` 不會關閉通訊端，而附加至通訊端的資源將不會釋放，直到呼叫 `Close` 為止。 應用程式不應該依賴能夠在關閉後重複使用通訊端。 特別是，不需要 Windows 通訊端執行，就能支援在這類通訊端上使用 `Connect`。
...
這個方法會配置通訊端控制碼。 它不會呼叫[CAsyncSocket：： bind](#bind)將通訊端系結至指定的位址，因此您必須在稍後呼叫 `Bind` 以將通訊端系結至指定的位址。 您可以使用[CAsyncSocket：： SetSockOpt](#setsockopt)來設定通訊端選項，然後再進行系結。
