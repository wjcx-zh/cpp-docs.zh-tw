...
## <a name="remarks"></a>備註  
 ...
 若要建立您自己的檢查清單方塊，您必須衍生您自己的類別從`CCheckListBox`。 若要衍生您自己的類別，撰寫在衍生類別的建構函式，然後呼叫`Create`。  
...
### <a name="remarks"></a>備註  
 您建構`CCheckListBox`兩個步驟中的物件。 先定義類別，衍生自`CCheckListBox`，然後呼叫`Create`，其中初始化 Windows 檢查清單方塊，並將它附加至`CCheckListBox`物件。  
...
### <a name="remarks"></a>備註  
 您建構`CCheckListBox`兩個步驟中的物件。 首先，定義類別，衍生自`CcheckListBox`，然後呼叫`Create`，其中初始化 Windows 檢查清單方塊，並將它附加至`CCheckListBox`。 請參閱[CCheckListBox::CCheckListBox](#cchecklistbox)範例。  
  
 當`Create`執行時，Windows 會傳送[WM_NCCREATE](../../mfc/reference/cwnd-class.md#onnccreate)， [WM_CREATE](../../mfc/reference/cwnd-class.md#oncreate)， [WM_NCCALCSIZE](../../mfc/reference/cwnd-class.md#onnccalcsize)，和[WM_GETMINMAXINFO](../../mfc/reference/cwnd-class.md#ongetminmaxinfo)檢查清單方塊控制項的訊息。  
 ...
 **ItemAction**和**itemState**成員`DRAWITEMSTRUCT`結構會定義要執行的繪圖動作。  
 ...
 如果檢查清單方塊項目不是所有具有相同的高度，檢查清單方塊樣式 (指定在`Create`) 必須是**LBS_OWNERVARIABLE**，而且您必須覆寫[MeasureItem](#measureitem)函式。  
