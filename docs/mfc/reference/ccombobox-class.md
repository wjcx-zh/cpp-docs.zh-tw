...
您可以 `CComboBox` 從對話方塊範本或直接在程式碼中建立物件。 在這兩種情況下，請先呼叫函式 `CComboBox` 來建立 `CComboBox` 物件，然後呼叫[create](#create)成員函式來建立控制項，並將它附加至 `CComboBox` 物件。

如果您想要處理下拉式方區塊轉送給其父系的 Windows 通知訊息（通常是衍生自的類別 `CDialog` ），請將訊息對應專案和訊息處理常式成員函式新增至每個訊息的父類別。
...
- ON_CBN_SELCHANGE 下拉式方塊清單方塊中的選取專案即將變更，這是因為使用者在清單方塊中按一下，或使用方向鍵變更選取範圍的結果。 處理此訊息時，只能透過或其他類似的函式來抓取下拉式方塊編輯控制項中的文字 `GetLBText` 。 `GetWindowText`無法使用。

- ON_CBN_SETFOCUS 下拉式方塊會收到輸入焦點。

如果您在 `CComboBox` 對話方塊中建立物件（透過對話資源）， `CComboBox` 當使用者關閉對話方塊時，就會自動終結物件。

如果您將 `CComboBox` 物件內嵌在另一個視窗物件中，則不需要終結它。 如果您在 `CComboBox` 堆疊上建立物件，它會自動終結。 如果您使用函式在 `CComboBox` 堆積上建立物件 **`new`** ，您必須 **`delete`** 在物件上呼叫，以在終結 Windows 下拉式方塊時終結它。

**注意**如果您想要處理 WM_KEYDOWN 並 WM_CHAR 訊息，您必須將下拉式方塊的編輯和清單方塊控制項子類別化，從和衍生類別， `CEdit` `CListBox` 然後將這些訊息的處理常式加入至衍生類別。 如需詳細資訊，請參閱[CWnd：： subclasswindow 前允許](../../mfc/reference/cwnd-class.md#subclasswindow)。
...
> [!NOTE]
> Windows 控制項不支援這個函式 `ComboBoxEx` 。 如需這個控制項的詳細資訊，請參閱 Windows SDK 中的[ComboBoxEx 控制項](/windows/win32/Controls/comboboxex-controls)。
...
表示結構中所描述的兩個專案的相對位置 `COMPAREITEMSTRUCT` 。 可以是下列其中任何一個值：
...
當使用者從主控描繪物件中刪除專案或終結下拉式方塊時，由架構呼叫 `CComboBox` 。
...
`itemAction`結構的成員會 `DRAWITEMSTRUCT` 定義要執行的繪圖動作。 如需此結構的說明，請參閱[CWnd：： OnDrawItem](../../mfc/reference/cwnd-class.md#ondrawitem) 。

根據預設，此成員函式不會執行任何工作。 覆寫這個成員函式，以執行主控描繪物件的繪圖 `CComboBox` 。 在此成員函式終止之前，應用程式應該還原針對*lpDrawItemStruct*中提供的顯示內容所選取的所有圖形裝置介面（GDI）物件。
...
如果下拉式方塊是使用擁有者繪製樣式所建立，但沒有[CBS_HASSTRINGS](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles)樣式，則 `FindStringExact` 會嘗試比對*lpszFind*的值。
...
項目數。 傳回的計數大於最後一個專案的索引值（索引以零為基底）。 如果發生錯誤，就會 CB_ERR。
...
在第一個多載中， [CString](../../atl-mfc-shared/using-cstring.md)物件，其中包含提示橫幅文字（如果有的話）。否則為 `CString` 長度為零的物件。
...
呼叫成員函式 `GetDroppedControlRect` ，以抓取下拉下拉式方塊的可見（下拉）清單方塊的螢幕座標。
...
呼叫成員函式 `GetDroppedState` ，以判斷下拉式方塊的清單方塊是否可見（已中斷）。
...
32位值可以使用[SetItemData](#setitemdata)成員函式呼叫的*dwItemData*參數來設定。 `GetItemDataPtr`如果要抓取的32位值是指標（），請使用成員函式 **`void`** <strong>\*</strong> 。
...
將與指定之下拉式列示方塊專案關聯的應用程式提供的32位值，抓取為指標（ **`void`** <strong>\*</strong> ）。
...
呼叫成員函式 `GetItemHeight` ，以取得下拉式方塊中清單專案的高度。
...
這個成員函式的第二種形式會 `CString` 以專案的文字填入物件。
...
在使用的[SubclassDlgItem](../../mfc/reference/cwnd-class.md#subclassdlgitem)成員函式所建立的主控描繪下拉式方塊中使用 CBS_OWNERDRAWFIXED 樣式 `CWnd` 牽涉到進一步的程式設計考慮。 請參閱技術提示[14](../../mfc/tn014-custom-controls.md)中的討論。
...
如果成員函式成功，則為非零;否則為0。 如果 `CComboBox` 具有[CBS_DROPDOWNLIST](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles)樣式，或沒有清單方塊，就會 CB_ERR。
...
呼叫成員函式 `SetExtendedUI` ，為具有[CBS_DROPDOWN](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles)或[CBS_DROPDOWNLIST](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles)樣式的下拉式方塊選取預設使用者介面或擴充使用者介面。
...
呼叫成員函式 `SetItemHeight` ，以設定下拉式方塊中清單專案的高度，或下拉式方塊的編輯控制項（或靜態文字）部分的高度。
