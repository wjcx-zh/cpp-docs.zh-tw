...
|[CDateTimeCtrl::Create](#create)|建立日期和時間選擇器控制項, 並將其附加`CDateTimeCtrl`至物件。|
...
|[CDateTimeCtrl::GetMonthCalCtrl](#getmonthcalctrl)|抓取與`CMonthCalCtrl`日期和時間選擇器控制項相關聯的物件。|
...
|[CDateTimeCtrl::GetTime](#gettime)|從日期和時間選擇器控制項抓取目前選取的時間, 並將它放在`SYSTEMTIME`指定的結構中。|
...
這個方法會傳送[DTM_CLOSEMONTHCAL](/windows/win32/Controls/dtm-closemonthcal)訊息, 如 Windows SDK 中所述。
...
建立日期和時間選擇器控制項, 並將其附加`CDateTimeCtrl`至物件。
...
*pParentWnd*<br/>
[CWnd](../../mfc/reference/cwnd-class.md)物件的指標, 這是日期和時間選擇器控制項的父視窗。 不得為 Null。
...
1. 呼叫這個成員函式, 它會建立 Windows 日期和時間選擇器控制項, 並將`CDateTimeCtrl`其附加至物件。
...
這個方法會傳送[DTM_GETDATETIMEPICKERINFO](/windows/win32/Controls/dtm-getdatetimepickerinfo)訊息, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [DTM_GETMCCOLOR](/windows/win32/Controls/dtm-getmccolor)的行為, 如 Windows SDK 中所述。
...
抓取與`CMonthCalCtrl`日期和時間選擇器控制項相關聯的物件。
...
日期和時間選擇器控制項會在使用者按一下下拉箭號時, 建立一個月曆控制項。 當不再`CMonthCalCtrl`需要物件時, 它會被終結, 因此, 您的應用程式不能依賴儲存代表日期時間選擇器控制項之子月份行事曆的物件。
...
傳回`CFont`值所指向的物件是暫存物件, 並在下一個空閒處理時間終結。
...
這個方法會傳送[DTM_GETMCSTYLE](/windows/win32/Controls/dtm-getmcstyle)訊息, 如 Windows SDK 中所述。
...
*pMaxRange*<br/>
物件`COleDateTime` `CDateTimeCtrl`或物件的指標, 其中包含物件中允許的最新時間。 `CTime`

### <a name="return-value"></a>傳回值

DWORD 值, 其中包含表示已設定之範圍的旗標。 如果
...
此成員函式會執行 Win32 message [DTM_GETRANGE](/windows/win32/Controls/dtm-getrange)的行為, 如 Windows SDK 中所述。 在 MFC 的執行中, 您可以指定`COleDateTime`或`CTime`使用方法。
...
從日期和時間選擇器控制項抓取目前選取的時間, 並將它放在`SYSTEMTIME`指定的結構中。
...
在第一個版本中, 如果成功將時間寫入`COleDateTime`物件, 則為非零, 否則為0。 在第二個和第三個版本中, DWORD 值等於[NMDATETIMECHANGE](/windows/win32/api/commctrl/ns-commctrl-nmdatetimechange)結構中所設定的*dwFlag*成員。 如需詳細資訊, 請參閱下面的「**備註**」一節。

### <a name="remarks"></a>備註

此成員函式會執行 Win32 message [DTM_GETSYSTEMTIME](/windows/win32/Controls/dtm-getsystemtime)的行為, 如 Windows SDK 中所述。 在`GetTime`的 MFC 執行中, 您可以使用`COleDateTime`或`CTime`類別, 也可以使用`SYSTEMTIME`結構來儲存時間資訊。
...
這個方法會傳送[DTM_GETIDEALSIZE](/windows/win32/Controls/dtm-getidealsize)訊息, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [DTM_SETFORMAT](/windows/win32/Controls/dtm-setformat)的行為, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [DTM_SETMCCOLOR](/windows/win32/Controls/dtm-setmccolor)的行為, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [DTM_SETMCFONT](/windows/win32/Controls/dtm-setmcfont)的行為, 如 Windows SDK 中所述。
...
> [!NOTE]
>  如果您使用此`CDialog`程式碼, 您會想要將衍生類別的成員稱為類型`CFont`的*m_MonthFont* 。
...
這個方法會傳送[DTM_SETMCSTYLE](/windows/win32/Controls/dtm-setmcstyle)訊息, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [DTM_SETRANGE](/windows/win32/Controls/dtm-setrange)的行為, 如 Windows SDK 中所述。 在 MFC 的執行中, 您可以指定`COleDateTime`或`CTime`使用方法。 `COleDateTime`如果物件的狀態為 Null, 則會移除範圍。 `CTime`如果指標`COleDateTime`或指標是 Null, 則會移除範圍。
...
此成員函式會執行 Win32 message [DTM_SETSYSTEMTIME](/windows/win32/Controls/dtm-setsystemtime)的行為, 如 Windows SDK 中所述。 在的 MFC `SetTime`執行中, 您可以`COleDateTime`使用或`CTime`類別, 也可以使用`SYSTEMTIME`結構來設定時間資訊。
