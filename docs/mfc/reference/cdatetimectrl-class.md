...
|[CDateTimeCtrl：： GetMonthCalCtrl](#getmonthcalctrl)|抓取 `CMonthCalCtrl` 與日期和時間選擇器控制項相關聯的物件。|
...
*各種*<br/>
[RECT](/windows/win32/api/windef/ns-windef-rect)結構的參考，這是日期和時間選擇器控制項的位置和大小。
...
1. 呼叫這個成員函式，它會建立 Windows 日期和時間選擇器控制項，並將其附加至 `CDateTimeCtrl` 物件。
...
抓取 `CMonthCalCtrl` 與日期和時間選擇器控制項相關聯的物件。
...
日期和時間選擇器控制項會在使用者按一下下拉箭號時，建立一個月曆控制項。 當 `CMonthCalCtrl` 不再需要物件時，它會被終結，因此，您的應用程式不能依賴儲存代表日期時間選擇器控制項之子月份行事曆的物件。
...
*pMinRange*<br/>
[COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md)物件或[CTime](../../atl-mfc-shared/reference/ctime-class.md)物件的指標，其中包含物件中允許的最早時間 `CDateTimeCtrl` 。

*pMaxRange*<br/>
物件或物件的指標， `COleDateTime` `CTime` 其中包含物件中允許的最新時間 `CDateTimeCtrl` 。
...
此成員函式會依照 Windows SDK 中的說明，實作用[DTM_GETRANGE](/windows/win32/Controls/dtm-getrange)Win32 訊息的行為。 在 MFC 的執行中，您可以指定 `COleDateTime` 或 `CTime` 使用方法。
...
在第一個版本中，如果成功將時間寫入物件，則為非零， `COleDateTime` 否則為0。 在第二個和第三個版本中，DWORD 值等於[NMDATETIMECHANGE](/windows/win32/api/commctrl/ns-commctrl-nmdatetimechange)結構中所設定的*dwFlag*成員。 如需詳細資訊，請參閱下面的「**備註**」一節。

### <a name="remarks"></a>備註

此成員函式會依照 Windows SDK 中的說明，實作用[DTM_GETSYSTEMTIME](/windows/win32/Controls/dtm-getsystemtime)Win32 訊息的行為。 在的 MFC 執行中 `GetTime` ，您可以使用 `COleDateTime` 或 `CTime` 類別，也可以使用 `SYSTEMTIME` 結構來儲存時間資訊。
...
此成員函式會依照 Windows SDK 中的說明，實作用[DTM_SETRANGE](/windows/win32/Controls/dtm-setrange)Win32 訊息的行為。 在 MFC 的執行中，您可以指定 `COleDateTime` 或 `CTime` 使用方法。 如果 `COleDateTime` 物件的狀態為 Null，則會移除範圍。 如果 `CTime` 指標或 `COleDateTime` 指標是 Null，則會移除範圍。
...
此成員函式會依照 Windows SDK 中的說明，實作用[DTM_SETSYSTEMTIME](/windows/win32/Controls/dtm-setsystemtime)Win32 訊息的行為。 在的 MFC 執行中 `SetTime` ，您可以使用 `COleDateTime` 或 `CTime` 類別，也可以使用 `SYSTEMTIME` 結構來設定時間資訊。
