...
|[CDialog：： InitModalIndirect](#initmodalindirect)|從記憶體中的對話方塊範本建立強制回應對話方塊（不是以資源為基礎）。 參數會儲存到呼叫函數 `DoModal` 為止。|
...
|名稱|描述|
|----------|-----------------|
|[CDialog：： OnCancel](#oncancel)|覆寫以執行 [取消] 按鈕或 ESC 鍵動作。 預設會關閉對話方塊，`DoModal` 會傳回 IDCANCEL。|
|[CDialog：： OnOK](#onok)|覆寫以執行強制回應對話方塊中的 [確定] 按鈕動作。 預設會關閉對話方塊，`DoModal` 會傳回 IDOK。|

## <a name="remarks"></a>備註

對話方塊有兩種類型：強制回應和非強制回應。 使用者必須先關閉強制回應對話方塊，應用程式才能繼續。 非強制回應對話方塊可讓使用者顯示對話方塊，並返回另一項工作，而不需要取消或移除對話方塊。

`CDialog` 物件是對話方塊範本和 `CDialog`衍生類別的組合。 使用對話方塊編輯器來建立對話方塊範本，並將其儲存在資源中，然後使用 [加入類別] wizard 建立衍生自 `CDialog`的類別。
...
若要建立強制回應對話方塊，請使用衍生對話方塊類別的函式，在堆疊上建立物件，然後呼叫 `DoModal` 來建立對話方塊視窗和其控制項。 如果您想要建立非強制回應對話方塊，請在對話類別的函式中呼叫 `Create`。
...
交換和驗證資料對應是以新增至新對話方塊類別的 `CWnd::DoDataExchange` 覆寫來寫入。 如需 exchange 和驗證功能的詳細資訊，請參閱 `CWnd` 中的[DoDataExchange](../../mfc/reference/cwnd-class.md#dodataexchange)成員函式。

程式設計人員和架構呼叫都是透過呼叫[CWnd：： UpdateData](../../mfc/reference/cwnd-class.md#updatedata)間接 `DoDataExchange`。

當使用者按一下 [確定] 按鈕以關閉強制回應對話方塊時，架構會呼叫 `UpdateData`。 （如果按一下 [取消] 按鈕，則不會抓取資料）。[OnInitDialog](#oninitdialog)的預設實值也會呼叫 `UpdateData` 來設定控制項的初始值。 您通常會覆寫 `OnInitDialog` 以進一步初始化控制項。 在建立所有對話方塊控制項並在對話方塊顯示之前，會呼叫 `OnInitDialog`。

在執行強制回應或非強制回應對話方塊時，您可以隨時呼叫 `CWnd::UpdateData`。

如果您以手動方式開發對話方塊，您可以自行將必要的成員變數新增至衍生的對話方塊類別，並加入成員函式來設定或取得這些值。

當使用者按下 [確定] 或 [取消] 按鈕，或當您的程式碼呼叫 `EndDialog` 成員函式時，強制回應對話方塊會自動關閉。

當您執行非強制回應對話方塊時，請一律覆寫 `OnCancel` 成員函式，並從它內呼叫 `DestroyWindow`。 請勿呼叫基類 `CDialog::OnCancel`，因為它會呼叫 `EndDialog`，這會使對話方塊不可見，但不會將其損毀。 您也應該覆寫非強制回應對話方塊的 `PostNcDestroy`，**以便刪除，** 因為非強制回應對話方塊通常是以**新**的來配置。 強制回應對話方塊通常會在框架上建立，不需要 `PostNcDestroy` 清除。

如需 `CDialog`的詳細資訊，請參閱[對話方塊](../../mfc/dialog-boxes.md)。
...
若要從記憶體中的範本來建立強制回應對話方塊，請先叫用無參數、受保護的構造函式，然後呼叫 `InitModalIndirect`。

當您使用上述其中一種方法來建立強制回應對話方塊之後，請呼叫 `DoModal`。

若要建立非強制回應對話方塊，請使用受保護的 `CDialog` 的格式函數。 此函式會受到保護，因為您必須衍生自己的對話方塊類別，才能執行非強制回應對話方塊。 非強制回應對話方塊的結構是一個兩個步驟的處理常式。 第一次呼叫此函式;然後，呼叫 `Create` 成員函式來建立以資源為基礎的對話方塊，或呼叫 `CreateIndirect`，從記憶體中的範本建立對話方塊。
...
您可以將 `Create` 的呼叫放在函式內，或在叫用此函式之後呼叫它。

提供兩種形式的 `Create` 成員函式，可透過範本名稱或範本識別碼（例如 IDD_DIALOG1）存取對話方塊範本資源。
...
使用 `CWnd::DestroyWindow` 函式，終結由 `Create` 函數所建立的對話方塊。
...
*hDialogTemplate*<br/>
包含包含對話方塊範本之全域記憶體的控制碼。 此範本的格式為對話方塊中每個控制項的 `DLGTEMPLATE` 結構和資料。
...
如果在建立父視窗時應該出現對話方塊，請使用對話方塊範本中的 WS_VISIBLE 樣式。 否則，您必須呼叫 `ShowWindow`，使其顯示。 如需如何在範本中指定其他對話方塊樣式的詳細資訊，請參閱 Windows SDK 中的[DLGTEMPLATE](/windows/win32/api/winuser/ns-winuser-dlgtemplate)結構。

使用 `CWnd::DestroyWindow` 函式，終結由 `CreateIndirect` 函數所建立的對話方塊。
...
如果使用者按一下對話方塊中的其中一個按鈕，例如 [確定] 或 [取消]，則會呼叫訊息處理常式成員函式（例如[OnOK](#onok)或[OnCancel](#oncancel)）來嘗試關閉對話方塊。 預設 `OnOK` 成員函式會驗證和更新對話方塊資料，並關閉含有 result IDOK 的對話方塊，而預設 `OnCancel` 成員函式會關閉具有結果 IDCANCEL 的對話方塊，而不會驗證或更新對話方塊資料。 您可以覆寫這些訊息處理常式函式來改變其行為。

> [!NOTE]
> 現在會針對強制回應對話方塊訊息處理呼叫 `PreTranslateMessage`。
...
*N 結果*<br/>
包含要從對話方塊傳回的值，`DoModal`的呼叫端。

### <a name="remarks"></a>備註

此成員函式會傳回*n 結果*做為 `DoModal`的傳回值。 每當建立強制回應對話方塊時，您都必須使用 `EndDialog` 函數來完成處理。

您可以隨時呼叫 `EndDialog`，即使是在[OnInitDialog](#oninitdialog)中，在此情況下，您應該在顯示或設定輸入焦點之前關閉對話方塊。
...
呼叫 `GetDefID` 成員函式，以取得對話方塊預設按鈕控制項的識別碼。
...
若要取得控制項（子視窗）的指標以傳遞為*pWndCtrl*，請呼叫 `CWnd::GetDlgItem` 成員函式，此函式會傳回[CWnd](../../mfc/reference/cwnd-class.md)物件的指標。
...
若要間接建立強制回應對話方塊，請先配置記憶體的全域區塊，並使用對話方塊範本加以填入。 然後呼叫空的 `CDialog` 的函式，以建立對話方塊物件。 接下來，呼叫 `InitModalIndirect`，將您的控制碼儲存至記憶體中的對話方塊範本。 當呼叫[DoModal](#domodal)成員函式時，會建立並顯示 [Windows] 對話方塊。
...
如果您在非強制回應對話方塊中執行 [**取消**] 按鈕，則必須覆寫 `OnCancel` 方法，並在其中呼叫[DestroyWindow](../../mfc/reference/cwnd-class.md#destroywindow) 。 請勿呼叫基類方法，因為它會呼叫 `EndDialog`，這會使對話方塊不可見，但不會損毀。

> [!NOTE]
>  當您在 Windows XP 下編譯的程式中使用 `CFileDialog` 物件時，無法覆寫這個方法。 如需 `CFileDialog`的詳細資訊，請參閱[CFileDialog 類別](../../mfc/reference/cfiledialog-class.md)。
...
Windows 會在 [[建立](#create)]、[ [CreateIndirect](#createindirect)] 或 [ [DoModal](#domodal)呼叫] 期間，將 `WM_INITDIALOG` 訊息傳送至對話方塊，這會在對話方塊顯示之前立即出現。

如果您想要在初始化對話方塊時執行特殊處理，請覆寫這個方法。 在覆寫的版本中，先呼叫基類 `OnInitDialog`，但忽略其傳回值。 您通常會從覆寫的方法傳回 `TRUE`。

Windows 會使用所有 MFC 程式庫對話方塊通用的標準全域對話方塊程式來呼叫 `OnInitDialog` 函式。 它不會透過您的訊息對應來呼叫此函式，因此，您不需要此方法的訊息對應專案。

> [!NOTE]
> 當您在 Windows Vista 或更新版本的作業系統下編譯的程式中使用 `CFileDialog` 物件時，無法覆寫這個方法。 如需 Windows Vista 和更新版本下 `CFileDialog` 變更的詳細資訊，請參閱[CFileDialog 類別](../../mfc/reference/cfiledialog-class.md)。
...
如果您在非強制回應對話方塊中執行 [**確定]** 按鈕，則必須覆寫 `OnOK` 方法，並在其中呼叫[DestroyWindow](../../mfc/reference/cwnd-class.md#destroywindow) 。 請勿呼叫基類方法，因為它會呼叫[EndDialog](#enddialog) ，這會使對話方塊不可見，但不會損毀。
