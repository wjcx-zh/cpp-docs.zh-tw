...
### <a name="remarks"></a>備註  
 ...
 使用**WS_VISIBLE**對話方塊範本中設定樣式，如果父視窗建立時，應該會出現對話方塊。 否則，您必須呼叫`ShowWindow`，讓它出現。 如需有關如何指定其他對話方塊樣式範本中的詳細資訊，請參閱[DLGTEMPLATE](http://msdn.microsoft.com/library/windows/desktop/ms645394) Windows SDK 中的結構。  
...
> [!NOTE]
> 當您使用時，您無法覆寫這個方法`CFileDialog`Windows Vista 或更新版本的作業系統下編譯的程式中的物件。 如需有關變更`CFileDialog`在 Windows Vista 和更新版本，請參閱[CFileDialog 類別](../../mfc/reference/cfiledialog-class.md)。  
