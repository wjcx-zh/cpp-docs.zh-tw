...
|名稱|描述|
|----------|-----------------|
|[CEdit：： CEdit](#cedit)|構造 `CEdit` 控制項物件。|
...
|[CEdit：： GetLimitText](#getlimittext)|取得此 `CEdit` 可以包含的最大文字量。|
|[CEdit：： GetLine](#getline)|從編輯控制項抓取一行文字。|
|[CEdit：： GetLineCount](#getlinecount)|抓取多行編輯控制項中的行數。|
|[CEdit：： GetMargins](#getmargins)|取得此 `CEdit`的左邊和右邊邊界。|
...
|[CEdit：： SetLimitText](#setlimittext)|設定此 `CEdit` 可以包含的最大文字量。|
|[CEdit：： SetMargins](#setmargins)|設定此 `CEdit`的左邊和右邊邊界。|
...
您可以從對話方塊範本或直接在程式碼中建立編輯控制項。 在這兩種情況下，請先呼叫 `CEdit` 的函式，以建立 `CEdit` 物件，然後呼叫[create](#create)成員函式來建立 Windows 編輯控制項，並將它附加至 `CEdit` 物件。

在衍生自 `CEdit`的類別中，結構可以是一個步驟的進程。 撰寫衍生類別的函式，並從函式內呼叫 `Create`。

`CEdit` 從 `CWnd`繼承重要的功能。 若要設定和抓取 `CEdit` 物件中的文字，請使用 `CWnd` 成員函式[SetWindowText](cwnd-class.md#setwindowtext)和[GetWindowText](cwnd-class.md#getwindowtext)，這會設定或取得編輯控制項的完整內容，即使它是多行控制項也一樣。 多行控制項中的文字會以 ' \r\n ' 字元序列分隔。 此外，如果編輯控制項是多行，請藉由呼叫 `CEdit` 成員函式[GetLine](#getline)、 [SetSel](#setsel)、 [GetSel](#getsel)和[ReplaceSel](#replacesel)，取得並設定部分的控制項文字。

如果您想要處理編輯控制項傳送給其父系的 Windows 通知訊息（通常是衍生自 `CDialog`的類別），請將訊息對應專案和訊息處理常式成員函式新增至每個訊息的父類別。

每個訊息對應專案會採用下列格式：

  **ON_** _通知_ **（** _識別碼_ **、** _memberFxn_ **）**

其中 `id` 會指定用於傳送通知之編輯控制項的子視窗識別碼，而 `memberFxn` 則是您已撰寫來處理通知之父成員函式的名稱。
...
如果您在對話方塊中建立 `CEdit` 物件，當使用者關閉對話方塊時，就會自動終結 `CEdit` 物件。

如果您使用對話方塊編輯器從對話資源建立 `CEdit` 物件，當使用者關閉對話方塊時，`CEdit` 物件會自動終結。

如果您在視窗中建立 `CEdit` 物件，您可能也需要將它摧毀。 如果您在堆疊上建立 `CEdit` 物件，它就會自動終結。 如果您使用**新**的函式在堆積上建立 `CEdit` 物件，您必須在物件上呼叫**delete** ，以在使用者終止 Windows 編輯控制項時終結它。 如果您在 `CEdit` 物件中配置任何記憶體，請覆寫 `CEdit` 的析構函式以處置配置。
...
如需 `CEdit`的詳細資訊，請參閱[控制項](../../mfc/controls-mfc.md)。
...
如果可以透過呼叫 `Undo` 成員函式來復原最後一個編輯作業，則為非零值。如果無法復原，則為0。
...
呼叫此函式可抓取此 `CEdit` 控制項中最接近指定點的字元之以零為起始的行和字元索引
...
藉由呼叫[Undo](#undo)成員函式，可以復原 `Clear` 所執行的刪除作業。
...
您可以使用兩個步驟來建立 `CEdit` 物件。 首先，呼叫 `CEdit` 的函式，然後呼叫 `Create`，這會建立 Windows 編輯控制項並將其附加至 `CEdit` 物件。
...
根據預設，這些訊息會由 `CWnd` 基類中的[OnNcCreate](cwnd-class.md#onnccreate)、 [OnNcCalcSize](cwnd-class.md#onnccalcsize)、 [OnCreate](cwnd-class.md#oncreate)和[OnGetMinMaxInfo](cwnd-class.md#ongetminmaxinfo)成員函式來處理。 若要擴充預設訊息處理，請從 `CEdit`衍生類別，將訊息對應加入至新的類別，並覆寫上述的訊息處理常式成員函式。 例如，覆寫 `OnCreate`，以針對新的類別執行所需的初始化。
...
藉由呼叫[Undo](#undo)成員函式，可以復原 `Cut` 所執行的刪除作業。
...
軟換行是由兩個回車傳回，而且在行尾插入的換行是因為斷字而中斷。 硬式分行符號包含一個回車和一個換行字元。 以硬式分行符號結尾的行不會受到 `FmtLines`的影響。
...
只有在使用已設定 DS_LOCALEDIT 樣式旗標的對話方塊建立時，才會在對話方塊中呼叫多行編輯控制項的 `GetHandle`。 如果未設定 DS_LOCALEDIT 樣式，您仍然會得到非零的傳回值，但您將無法使用傳回的值。

> [!NOTE]
> `GetHandle` 將無法用於 Windows 95/98。 如果您在 Windows 95/98 中呼叫 `GetHandle`，它會傳回 Null。 `GetHandle` 會如 Windows NT 3.51 版和更新版本中所記載的方式工作。
...
這個 `CEdit` 物件的目前文字限制（以 TCHARs 為限）。
...
*nMaxLength*<br/>
指定可以複製到緩衝區的 TCHAR 字元數目上限。 `GetLine` 在呼叫 Windows 之前，會將此值放在*lpszBuffer*的第一個字組中。
...
變更文字限制只會限制使用者可以輸入的文字。 它不會影響已在編輯控制項中的任何文字，也不會影響 `CWnd`中的[SetWindowText](cwnd-class.md#setwindowtext)成員函式複製到編輯控制項的文字長度。 如果應用程式使用 `SetWindowText` 函式，將更多文字放入編輯控制項中，而不是在 `LimitText`的呼叫中指定，則使用者可以刪除編輯控制項中的任何文字。 不過，文字限制會讓使用者無法以新的文字取代現有的文字，除非刪除目前的選取範圍會導致文字低於文字限制。
...
`LineScroll` 可用來水準滾動到任何一行的最後一個字元之後。
...
呼叫此函式可取得此 `CEdit` 物件內指定字元的位置（左上角）。
...
字元是藉由提供其以零為基底的索引值來指定。 如果*nChar*大於這個 `CEdit` 物件中最後一個字元的索引，則傳回值會指定緊接在這個 `CEdit` 物件中最後一個字元之後的字元位置座標。
...
在應用程式設定新的記憶體控制碼之前，它應該使用[GetHandle](#gethandle)成員函式來取得目前記憶體緩衝區的控制碼，並使用 `LocalFree` Windows 函數釋放該記憶體。

`SetHandle` 會清除復原緩衝區（ [CanUndo](#canundo)成員函式會傳回0）和內部修改旗標（ [GetModify](#getmodify)成員函式會傳回0）。 [編輯控制] 視窗會重新繪製。
...
這個方法會傳送[EM_SETHILITE](/windows/win32/Controls/em-sethilite)訊息，如 Windows SDK 所述。  這個方法會傳送[EM_SETHILITE](/windows/win32/Controls/em-sethilite)訊息，如 Windows SDK 所述。 `SetHighlight` 和 `GetHighlight` 僅針對 UNICODE 組建啟用。

##  <a name="setlimittext"></a>CEdit：： SetLimitText

呼叫這個成員函式，以設定這個 `CEdit` 物件的文字限制。
...
呼叫 `SetPasswordChar` 成員函式時，`CEdit` 會使用*ch*所指定的字元來重新繪製所有可見字元。

如果使用[ES_PASSWORD](styles-used-by-mfc.md#edit-styles)樣式建立編輯控制項，則預設密碼字元會設定為星號（ <strong>\*</strong>）。 如果以設定為0的*ch*呼叫 `SetPasswordChar`，就會移除此樣式。
...
使用 `SetRect` 設定多行編輯控制項的格式化矩形。 格式化矩形是文字的限制矩形，與編輯控制視窗的大小無關。 第一次建立編輯控制項時，[格式化] 矩形會與 [編輯控制] 視窗的工作區相同。 藉由使用 `SetRect` 成員函式，應用程式可以讓格式化矩形大於或小於編輯控制視窗。

如果編輯控制項沒有捲軸，則如果格式化矩形的設定大於視窗，則會裁剪文字，而不會換行。 如果編輯控制項包含框線，則格式化矩形會縮小框線的大小。 如果您調整 `GetRect` 成員函式所傳回的矩形，就必須先移除框線的大小，然後再將矩形傳遞給 `SetRect`。

當呼叫 `SetRect` 時，編輯控制項的文字也會重新格式化並重新顯示。
...
*rgTabStops*<br/>
指向不帶正負號整數的陣列，指定對話方塊單位中的定位停駐點。 對話單位是水準或垂直距離。 一個水準對話單位等於目前對話基底寬度單位的四分之一，而1個垂直對話方塊單位等於目前對話基底高度單位的八分之一。 對話方塊基本單位是根據目前系統字型的高度和寬度計算。 `GetDialogBaseUnits` Windows 函式會傳回目前的對話方塊基底單位（以圖元為單位）。
...
|*ttiIcon*|在**INT** ，指定要與氣球提示相關聯的圖示類型。 預設值為 TTI_NONE。 如需詳細資訊，請參閱[EDITBALLOONTIP](/windows/win32/api/commctrl/ns-commctrl-editballoontip)結構的 `ttiIcon` 成員。|
...
下列程式碼範例會定義用來存取目前編輯控制項的變數 `m_cedit`。 下一個範例中會使用此變數。
...
復原作業也可以復原。 例如，您可以在第一次呼叫 `Undo`時還原已刪除的文字。 只要沒有中間的編輯作業，您就可以使用 `Undo`的第二個呼叫，再次移除文字。
