...
|[CFrameWnd：： CreateView](#createview)|在不是衍生自的框架中建立一個視圖 `CView` 。|
|[CFrameWnd：:D ockControlBar](#dockcontrolbar)|停駐控制列。|
|[CFrameWnd：： EnableDocking](#enabledocking)|允許固定控制列。|
|[CFrameWnd：： EndModalState](#endmodalstate)|結束框架視窗的強制回應狀態。 啟用所有停用的 windows `BeginModalState` 。|
...
|[CFrameWnd：： InitialUpdateFrame](#initialupdateframe)|會 `OnInitialUpdate` 呼叫屬於框架視窗中所有 views 的成員函式。|
...
|[CFrameWnd：： SetActiveView](#setactiveview)|設定使用中 `CView` 物件。|
...
|[CFrameWnd：： ShowOwnedWindows](#showownedwindows)|顯示屬於物件子系的所有視窗 `CFrameWnd` 。|
...
|[CFrameWnd：： rectDefault](#rectdefault)|建立物件時，請將此靜態當做 `CRect` 參數傳遞 `CFrameWnd` ，以允許 Windows 選擇視窗的初始大小和位置。|

## <a name="remarks"></a>備註

若要為您的應用程式建立有用的框架視窗，請從衍生類別 `CFrameWnd` 。 將成員變數加入至衍生類別，以儲存應用程式特定的資料。 實作訊息處理常式成員函式，和衍生類別中對應的訊息，以指定訊息被導向至視窗時會發生什麼事。
...
在您呼叫 `Create` 或之前 `LoadFrame` ，您必須使用 c + + 運算子，在堆積上建立框架視窗物件 **`new`** 。 在呼叫之前 `Create` ，您也可以註冊具有[AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass)全域函式的視窗類別，以設定框架的圖示和類別樣式。
...
當 `CFrameWnd` 物件包含視圖和檔時，架構會間接建立它們，而不是直接由程式設計人員建立。 `CDocTemplate`物件會協調框架的建立、建立包含的視圖，以及將視圖連接到適當的檔。 此函式的參數會 `CDocTemplate` 指定 `CRuntimeClass` 涉及的三個類別（檔、框架和視圖）的。 `CRuntimeClass`當使用者指定時，架構會使用物件以動態方式建立新的框架（例如，使用 [檔案] [新增] 命令或 [多重文件介面（MDI）] 視窗的 [新增] 命令）。

衍生自的框架視窗類別 `CFrameWnd` 必須使用 DECLARE_DYNCREATE 宣告，上述的 RUNTIME_CLASS 機制才能正常運作。

`CFrameWnd`包含預設的執行方式，可在適用于 Windows 的一般應用程式中執行下列主要視窗的功能：

- `CFrameWnd`框架視窗會追蹤目前現用的視圖，而不受 Windows 使用中視窗或目前的輸入焦點影響。 重新開機畫面格時，會透過呼叫來通知作用中的視圖 `CView::OnActivateView` 。

- 框架視窗會將命令訊息和許多常見的畫面格通知訊息（包括由的、和函式處理） `OnSetFocus` `OnHScroll` `OnVScroll` `CWnd` 委派 `CFrameWnd` 給目前的現用視圖。
...
- `CFrameWnd`框架視窗具有使用設定的選擇性 [說明識別碼] `LoadFrame` ，用於即時線上說明。 框架視窗是 semimodal 狀態的主要協調器，例如即時線上說明（SHIFT + F1）和預覽列印模式。

- `CFrameWnd`框架視窗會開啟從 [檔案管理員] 拖曳並放在框架視窗上的檔案。 如果副檔名已註冊並與應用程式相關聯，框架視窗會回應在使用者于 [檔案管理員] 中開啟資料檔案時，或呼叫 Windows 函式時所發生的動態資料交換（DDE）開啟要求 `ShellExecute` 。

- 如果框架視窗是主應用程式視窗（也就是 `CWinThread::m_pMainWnd` ），當使用者關閉應用程式時，框架視窗會提示使用者儲存任何修改過的檔（針對 `OnClose` 和 `OnQueryEndSession` ）。

- 如果框架視窗是主應用程式視窗，框架視窗就是執行 WinHelp 的內容。 如果框架視窗已啟動以取得此應用程式的說明，將會關閉它 WINHELP.EXE。
...
*lpszClassName*<br/>
指向以 null 結束的字元字串，其命名為 Windows 類別。 類別名稱可以是任何向全域函式或 Windows 函式註冊的名稱 `AfxRegisterWndClass` `RegisterClass` 。 如果是 Null，則會使用預先定義的預設 `CFrameWnd` 屬性。
...
`CFrameWnd`以兩個步驟來建立物件。 首先，叫用用來構造物件的函式， `CFrameWnd` 然後呼叫 `Create` ，它會建立 Windows 框架視窗，並將它附加至 `CFrameWnd` 物件。 `Create`初始化視窗的類別名稱和視窗名稱，並為其樣式、父系和相關聯的功能表註冊預設值。
...
使用此成員函式來建立不會 `CView` 在框架內衍生的「views」。 呼叫之後 `CreateView` ，您必須手動將此視圖設定為 [作用中]，並將它設定為可見; 這些工作不會由自動執行 `CreateView` 。
...
如果沒有作用中的 MDI 子系，或是應用程式是單一檔介面（SDI），則 **`this`** 會傳回隱含指標。

## <a name="cframewndgetactiveview"></a><a name="getactiveview"></a>CFrameWnd：： GetActiveView

呼叫這個成員函式，以取得附加至框架視窗（）之現用視圖（如果有的話）的指標 `CFrameWnd` 。
...
呼叫 MDI 主框架視窗（）時，此函式會傳回 Null `CMDIFrameWnd` 。 在 MDI 應用程式中，MDI 主框架視窗並沒有相關聯的視圖。 相反地，每個個別的子視窗（ `CMDIChildWnd` ）都會有一或多個相關聯的視圖。 您可以先尋找作用中的 MDI 子視窗，然後尋找該子視窗的現用視圖，藉以取得 MDI 應用程式中的現用視圖。 藉由呼叫函式或，即可找到作用中的 MDI 子視窗，如下 `MDIGetActive` `GetActiveFrame` 所示：
...
*NID*參數會參考傳遞至控制列之方法的唯一識別碼 `Create` 。 如需控制列的詳細資訊，請參閱標題為[控制](../../mfc/control-bars.md)列的主題。
...
接著，您可以使用或，將的內容寫入 `CDockState` 至儲存體 `CDockState::SaveState` `Serialize` 。 如果您稍後想要將控制列還原到先前的狀態，請使用或載入 `CDockState::LoadState` 狀態 `Serialize` ，然後呼叫將 `SetDockState` 先前的狀態套用至框架視窗的控制列。
...
`IntitialUpdateFrame`使用建立新的框架之後，呼叫 `Create` 。
...
在呼叫之前，必須先將您想要還原的設定寫入登錄 `LoadBarState` 。 藉由呼叫[CWinApp：： SetRegistryKey](../../mfc/reference/cwinapp-class.md#setregistrykey)，將資訊寫入登錄。 藉由呼叫[SaveBarState](#savebarstate)，將資訊寫入至 INI 檔案。
...
`CFrameWnd`以兩個步驟來建立物件。 首先，叫用構造物件的函式， `CFrameWnd` 然後呼叫 `LoadFrame` ，它會載入 Windows 框架視窗和相關聯的資源，並將框架視窗附加至 `CFrameWnd` 物件。 *NIDResource*參數會指定功能表、快速鍵對應表、圖示，以及框架視窗標題的字串資源。
...
如果您的應用程式是 OLE 容器，則會將 `OnContextHelp` 包含在框架視窗物件中的所有就地專案放在說明模式中。 游標會變更為箭號和問號，然後使用者可以移動滑鼠指標，然後按下滑鼠左鍵，以選取對話方塊、視窗、功能表或命令按鈕。 此成員函式會呼叫 Windows 函式 `WinHelp` ，並在游標下使用物件的說明內容。
...
覆寫此函式以覆寫在物件中傳遞的值， `CCreateContext` 或變更框架視窗主要工作區中控制項的建立方式。 `CCreateContext`您可以覆寫的成員會在[CCreateCoNtext](../../mfc/reference/ccreatecontext-structure.md)類別中說明。
...
這個成員函式的預設實值會呼叫成員函式 `CWnd` `RepositionBars` ，將框架中的所有控制列以及主用戶端視窗（通常是 `CView` 或 MDICLIENT）重新置放。
...
建立視窗時，請將這個靜態當做 `CRect` 參數傳遞，讓 Windows 選擇視窗的初始大小和位置。
...
若要還原控制項列先前的狀態，您可以使用或載入預存狀態 `CDockState::LoadState` ， `Serialize` 然後使用將 `SetDockState` 它套用至框架視窗的控制列。 先前的狀態會儲存在 `CDockState` 物件中，並具有`GetDockState`
...
*nProgressPos*<br/>
指定要設定的位置。 它必須在所設定的範圍內 `SetProgressBarRange` 。
...
呼叫這個成員函式，以顯示屬於物件子系的所有視窗 `CFrameWnd` 。
