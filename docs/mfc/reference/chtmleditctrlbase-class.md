---
title: "CHtmlEditCtrlBase 類別 |Microsoft 文件"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- devlang-cpp
ms.tgt_pltfrm: 
ms.topic: reference
f1_keywords:
- CHtmlEditCtrlBase
dev_langs:
- C++
helpviewer_keywords:
- CHtmlEditCtrlBase class
ms.assetid: e0cc74b4-8320-4570-b673-16c03d2ae266
caps.latest.revision: 21
author: mikeblome
ms.author: mblome
manager: ghogen
translation.priority.ht:
- cs-cz
- de-de
- es-es
- fr-fr
- it-it
- ja-jp
- ko-kr
- pl-pl
- pt-br
- ru-ru
- tr-tr
- zh-cn
- zh-tw
translationtype: Machine Translation
ms.sourcegitcommit: 0e0c08ddc57d437c51872b5186ae3fc983bb0199
ms.openlocfilehash: e5541025653cca908d5d0c7666a434aa3f81ab5b
ms.lasthandoff: 02/24/2017

---
# <a name="chtmleditctrlbase-class"></a>CHtmlEditCtrlBase 類別
表示 HTML 編輯元件。  
  
## <a name="syntax"></a>語法  
  
```  
template <class T> class CHtmlEditCtrlBase  
```  
  
## <a name="members"></a>Members  
  
### <a name="public-methods"></a>公用方法  
  
|名稱|描述|  
|----------|-----------------|  
|[CHtmlEditCtrlBase::AddToGlyphTable](#addtoglyphtable)|指定映像以顯示在設計模式中的特定標記的圖像 （glyph） 資料表中新增一個項目。|  
|[CHtmlEditCtrlBase::Bold](#bold)|切換所選取文字的粗體顯示的狀態。|  
|[CHtmlEditCtrlBase::Button](#button)|會覆寫目前的選取範圍上的按鈕控制項。|  
|[CHtmlEditCtrlBase::CheckBox](#checkbox)|會覆寫目前的選取範圍 核取方塊控制項。|  
|[CHtmlEditCtrlBase::ClearSelection](#clearselection)|清除目前的選取範圍。|  
|[CHtmlEditCtrlBase::Copy](#copy)|將目前的選取範圍複製到剪貼簿。|  
|[CHtmlEditCtrlBase::Cut](#cut)|將目前的選取範圍複製到剪貼簿，然後將它刪除。|  
|[CHtmlEditCtrlBase::Delete](#delete)|刪除目前選取範圍。|  
|[CHtmlEditCtrlBase::DropDownBox](#dropdownbox)|會覆寫的下拉式清單選取控制項，在目前選取範圍。|  
|[CHtmlEditCtrlBase::EmptyGlyphTable](#emptyglyphtable)|隱藏所有影像顯示在設計模式中的標籤圖像 （glyph） 資料表中移除所有項目。|  
|[CHtmlEditCtrlBase::ExecCommand](#execcommand)|執行命令。|  
|[CHtmlEditCtrlBase::Font](#font)|開啟 [字型] 對話方塊，讓使用者變更文字色彩、 字型和目前的選取範圍的字型大小。|  
|[CHtmlEditCtrlBase::GetAbsolutePosition](#getabsoluteposition)|傳回項目的位置的屬性是否為 「 絕對 」。|  
|[CHtmlEditCtrlBase::GetBackColor](#getbackcolor)|擷取目前的選取範圍的背景色彩。|  
|[CHtmlEditCtrlBase::GetBlockFormat](#getblockformat)|擷取目前的區塊格式標記。|  
|[CHtmlEditCtrlBase::GetBlockFormatNames](#getblockformatnames)|擷取可用的區塊格式標記相對應的字串。|  
|[CHtmlEditCtrlBase::GetBookMark](#getbookmark)|擷取書籤錨點名稱。|  
|[CHtmlEditCtrlBase::GetDocument](#getdocument)|擷取的文件物件。|  
|[CHtmlEditCtrlBase::GetDocumentHTML](#getdocumenthtml)|擷取目前的文件的 HTML。|  
|[CHtmlEditCtrlBase::GetDocumentTitle](#getdocumenttitle)|擷取文件的標題。|  
|[CHtmlEditCtrlBase::GetEvent](#getevent)|擷取包含最新事件的相關資訊的事件物件的介面指標。|  
|[CHtmlEditCtrlBase::GetEventSrcElement](#geteventsrcelement)|擷取引發事件的物件。|  
|[CHtmlEditCtrlBase::GetFontFace](#getfontface)|擷取目前的選取範圍的字型名稱。|  
|[CHtmlEditCtrlBase::GetFontSize](#getfontsize)|擷取目前的選取範圍的字型大小。|  
|[CHtmlEditCtrlBase::GetForeColor](#getforecolor)|擷取目前的選取範圍的 （文字） 的前景色彩。|  
|[CHtmlEditCtrlBase::GetFrameZone](#getframezone)|傳回目前頁面的安全性區域中，網頁瀏覽器中。|  
|[CHtmlEditCtrlBase::GetIsDirty](#getisdirty)|指出是否已變更的 HTML 文件。|  
|[CHtmlEditCtrlBase::GetShowAlignedSiteTags](#getshowalignedsitetags)|傳回所有項目是否會顯示圖像**styleFloat**屬性。|  
|[CHtmlEditCtrlBase::GetShowAllTags](#getshowalltags)|傳回 WebBrowser 是否顯示這些圖像會顯示文件中的所有標記的位置。|  
|[CHtmlEditCtrlBase::GetShowAreaTags](#getshowareatags)|擷取是否 WebBrowser 顯示區域標記的圖像。|  
|[CHtmlEditCtrlBase::GetShowBRTags](#getshowbrtags)|擷取是否 WebBrowser 顯示 br 標記的圖像。|  
|[CHtmlEditCtrlBase::GetShowCommentTags](#getshowcommenttags)|擷取是否 WebBrowser 顯示註解標記的圖像。|  
|[CHtmlEditCtrlBase::GetShowMiscTags](#getshowmisctags)|擷取是否 WebBrowser 顯示 Microsoft Internet Explorer 4.0 中所顯示的所有標記。|  
|[CHtmlEditCtrlBase::GetShowScriptTags](#getshowscripttags)|擷取是否 WebBrowser 顯示所有指令碼標記的圖像。|  
|[CHtmlEditCtrlBase::GetShowStyleTags](#getshowstyletags)|擷取是否 WebBrowser 顯示樣式的所有標記的圖像。|  
|[CHtmlEditCtrlBase::GetShowUnknownTags](#getshowunknowntags)|擷取是否 WebBrowser 顯示所有未知的標記的圖像。|  
|[CHtmlEditCtrlBase::HorizontalLine](#horizontalline)|在目前選取範圍的水平列會覆寫。|  
|[CHtmlEditCtrlBase::HyperLink](#hyperlink)|插入目前的選取範圍的超連結。|  
|[CHtmlEditCtrlBase::IE50Paste](#ie50paste)|執行與 Microsoft Internet Explorer 5 相容，貼上作業。|  
|[CHtmlEditCtrlBase::Iframe](#iframe)|會覆寫目前的選取範圍的內嵌框架。|  
|[CHtmlEditCtrlBase::Image](#image)|會覆寫目前的選取範圍上的影像。|  
|[CHtmlEditCtrlBase::Indent](#indent)|一個縮排的增量，增加所選文字的縮排。|  
|[CHtmlEditCtrlBase::InsFieldSet](#insfieldset)|會覆寫目前的選取範圍中的方塊。|  
|[CHtmlEditCtrlBase::InsInputButton](#insinputbutton)|會覆寫目前的選取範圍上的按鈕控制項。|  
|[CHtmlEditCtrlBase::InsInputHidden](#insinputhidden)|插入隱藏的控制項目前的選取範圍。|  
|[CHtmlEditCtrlBase::InsInputImage](#insinputimage)|影像控制項在目前選取範圍會覆寫。|  
|[CHtmlEditCtrlBase::InsInputPassword](#insinputpassword)|會覆寫密碼上的控制項目前的選取範圍。|  
|[CHtmlEditCtrlBase::InsInputReset](#insinputreset)|重設控制項目前的選取範圍會覆寫。|  
|[CHtmlEditCtrlBase::InsInputSubmit](#insinputsubmit)|在目前選取範圍的送出控制項將會覆寫。|  
|[CHtmlEditCtrlBase::InsInputUpload](#insinputupload)|會覆寫檔案上傳控制項，在目前選取範圍。|  
|[CHtmlEditCtrlBase::Is1DElement](#is1delement)|判斷以靜態方式是否置於項目。|  
|[CHtmlEditCtrlBase::Is2DElement](#is2delement)|判斷項目會具有絕對位置。|  
|[CHtmlEditCtrlBase::Italic](#italic)|切換目前的選取範圍斜體與 nonitalic 之間。|  
|[CHtmlEditCtrlBase::JustifyCenter](#justifycenter)|區塊的格式將置中在其目前的選取範圍位於。|  
|[CHtmlEditCtrlBase::JustifyLeft](#justifyleft)|靠左對齊格式區塊中目前選取範圍所在。|  
|[CHtmlEditCtrlBase::JustifyRight](#justifyright)|靠右對齊區塊的格式中目前選取範圍所在。|  
|[CHtmlEditCtrlBase::ListBox](#listbox)|會覆寫目前的選取範圍的清單方塊選取項目控制項。|  
|[CHtmlEditCtrlBase::Marquee](#marquee)|會覆寫目前的選取範圍的空跑馬燈。|  
|[CHtmlEditCtrlBase::NewDocument](#newdocument)|建立新的文件。|  
|[CHtmlEditCtrlBase::OrderList](#orderlist)|切換目前的選取範圍的已排序的清單之間的一般格式的區塊。|  
|[CHtmlEditCtrlBase::Outdent](#outdent)|減少縮排格式區塊的一個遞增值中目前選取範圍所在。|  
|[CHtmlEditCtrlBase::Paragraph](#paragraph)|會覆寫目前的選取範圍的分行符號。|  
|[CHtmlEditCtrlBase::Paste](#paste)|會覆寫目前的選取範圍剪貼簿的內容。|  
|[CHtmlEditCtrlBase::PrintDocument](#printdocument)|列印目前文件。|  
|[CHtmlEditCtrlBase::PrintPreview](#printpreview)|開啟目前文件使用預設的預覽列印範本或自訂範本的預覽列印視窗。|  
|[CHtmlEditCtrlBase::QueryStatus](#querystatus)|呼叫這個方法來查詢命令的狀態。|  
|[CHtmlEditCtrlBase::RadioButton](#radiobutton)|會覆寫目前的選取範圍的選項按鈕控制項。|  
|[CHtmlEditCtrlBase::RefreshDocument](#refreshdocument)|重新整理目前的文件。|  
|[CHtmlEditCtrlBase::RemoveFormat](#removeformat)|移除目前選取範圍的格式標記。|  
|[CHtmlEditCtrlBase::SaveAs](#saveas)|將目前網頁儲存至檔案。|  
|[CHtmlEditCtrlBase::SelectAll](#selectall)|選取整份文件。|  
|[CHtmlEditCtrlBase::Set2DPosition](#set2dposition)|允許拖曳移動的絕對定位項目。|  
|[CHtmlEditCtrlBase::SetAbsolutePosition](#setabsoluteposition)|項目的位置將屬性設定為 「 絕對 」 或 「 靜態 」。|  
|[CHtmlEditCtrlBase::SetAtomicSelection](#setatomicselection)|設定不可部分完成的選取模式。|  
|[CHtmlEditCtrlBase::SetAutoURLDetectMode](#setautourldetectmode)|開啟自動 URL 偵測和關閉。|  
|[CHtmlEditCtrlBase::SetBackColor](#setbackcolor)|設定目前選取範圍的背景色彩。|  
|[CHtmlEditCtrlBase::SetBlockFormat](#setblockformat)|設定目前的區塊格式標記。|  
|[CHtmlEditCtrlBase::SetBookMark](#setbookmark)|建立目前的選取範圍或插入點的書籤錨點。|  
|[CHtmlEditCtrlBase::SetCSSEditingLevel](#setcsseditinglevel)|選取的 CSS 層級 （CSS1 或 CSS2） 編輯器將支援，如果有的話。|  
|[CHtmlEditCtrlBase::SetDefaultComposeSettings](#setdefaultcomposesettings)|呼叫這個方法，以設定預設撰寫設定。|  
|[CHtmlEditCtrlBase::SetDesignMode](#setdesignmode)|設定設計模式。|  
|[CHtmlEditCtrlBase::SetDisableEditFocusUI](#setdisableeditfocusui)|停用陰影的框線，並編輯焦點的項目控點。|  
|[CHtmlEditCtrlBase::SetDocumentHTML](#setdocumenthtml)|設定目前文件的 HTML。|  
|[CHtmlEditCtrlBase::SetFontFace](#setfontface)|設定目前選取範圍的字型。|  
|[CHtmlEditCtrlBase::SetFontSize](#setfontsize)|設定目前選取範圍的字型大小。|  
|[CHtmlEditCtrlBase::SetForeColor](#setforecolor)|設定目前選取範圍 （文字） 的前景色彩。|  
|[CHtmlEditCtrlBase::SetIE5PasteMode](#setie5pastemode)|設定 Microsoft Internet Explorer 5 與貼上作業。|  
|[CHtmlEditCtrlBase::SetLiveResize](#setliveresize)|會導致更新的項目外觀的調整大小或移動作業期間持續 WebBrowser。|  
|[CHtmlEditCtrlBase::SetMultiSelect](#setmultiselect)|可讓多個選取項目。|  
|[CHtmlEditCtrlBase::SetOverrideCursor](#setoverridecursor)|命令永遠不會以變更滑鼠指標 WebBrowser。|  
|[CHtmlEditCtrlBase::SetOverwriteMode](#setoverwritemode)|切換插入之間的文字輸入模式，並覆寫。|  
|[CHtmlEditCtrlBase::SetRespectVisInDesign](#setrespectvisindesign)|會隱藏在設計模式中不可見的項目。|  
|[CHtmlEditCtrlBase::SetShowAlignedSiteTags](#setshowalignedsitetags)|顯示所有項目具有的圖像**styleFloat**屬性。|  
|[CHtmlEditCtrlBase::SetShowAllTags](#setshowalltags)|會顯示這些圖像會顯示文件中的所有標記的位置。|  
|[CHtmlEditCtrlBase::SetShowAreaTags](#setshowareatags)|顯示區域的所有標記的圖像。|  
|[CHtmlEditCtrlBase::SetShowBRTags](#setshowbrtags)|顯示所有 br 標記的圖像。|  
|[CHtmlEditCtrlBase::SetShowCommentTags](#setshowcommenttags)|顯示所有註解標記的圖像。|  
|[CHtmlEditCtrlBase::SetShowMiscTags](#setshowmisctags)|會顯示在 Microsoft Internet Explorer 4.0 中所顯示的所有標記。|  
|[CHtmlEditCtrlBase::SetShowScriptTags](#setshowscripttags)|顯示所有指令碼標記的圖像。|  
|[CHtmlEditCtrlBase::SetShowStyleTags](#setshowstyletags)|顯示樣式的所有標記的圖像。|  
|[CHtmlEditCtrlBase::SetShowUnknownTags](#setshowunknowntags)|顯示所有未知的標記的圖像。|  
|[CHtmlEditCtrlBase::TextArea](#textarea)|會覆寫目前的選取範圍上的多行文字輸入的控制項。|  
|[CHtmlEditCtrlBase::TextBox](#textbox)|會覆寫目前的選取範圍的文字控制項。|  
|[CHtmlEditCtrlBase::UnBookmark](#unbookmark)|移除目前選取範圍中的任何書籤。|  
|[CHtmlEditCtrlBase::Underline](#underline)|切換目前的選取範圍之間加上底線，而且不加上底線。|  
|[CHtmlEditCtrlBase::Unlink](#unlink)|移除目前選取範圍中的任何超連結。|  
|[CHtmlEditCtrlBase::UnorderList](#unorderlist)|切換目前的選取範圍的已排序的清單之間的一般格式的區塊。|  
  
#### <a name="parameters"></a>參數  
 `T`  
 衍生類別的名稱。  
  
## <a name="remarks"></a>備註  
 **CHtmlEditCtrlBase** WebBrowser HTML 編輯命令，例如提供成員函式[粗體](#bold)。 (或者，您可以呼叫[ExecCommand](#execcommand)執行**IDM_BOLD**命令。)  
  
 **CHtmlEditCtrlBase**不是要獨立獲得。 它設計為可公開 （expose） 的 HTML 編輯功能的 WebBrowser 的衍生類別的基底類別 (請參閱[CHtmlEditCtrl](../../mfc/reference/chtmleditctrl-class.md)和[CHtmlEditView](../../mfc/reference/chtmleditview-class.md))。  
  
## <a name="inheritance-hierarchy"></a>繼承階層  
 `CHtmlEditCtrlBase`  
  
## <a name="requirements"></a>需求  
 **Header:** afxhtml.h  
  
##  <a name="a-nameaddtoglyphtablea--chtmleditctrlbaseaddtoglyphtable"></a><a name="addtoglyphtable"></a>CHtmlEditCtrlBase::AddToGlyphTable  
 指定映像以顯示在設計模式中的特定標記的圖像 （glyph） 資料表中新增一個項目。  
  
```  
HRESULT AddToGlyphTable(
    LPCTSTR szTag,  
    LPCTSTR szImgUrl,  
    unsigned short nTagType,  
    unsigned short nAlignment,  
    unsigned short nPosInfo,  
    unsigned short nDirection,  
    unsigned int nImgWidth,  
    unsigned int nImgHeight) const;  
```  
  
### <a name="parameters"></a>參數  
 `szTag`  
 標記名稱 （例如，"P"或 「 資料表 」）。  
  
 *szImgUrl*  
 影像 URL。  
  
 *nTagType*  
 標記類型︰ 0 表示映像為開頭標記。 1 表示映像為結尾標記。 2 表示影像的開頭和結尾標記。 單一標記，例如 br 和註解必須加入標記類型設定為 0。  
  
 *nAlignment*  
 對齊方式 （只有矩形項目）︰ 這個參數會指出影像是否具有對齊屬性的項目。 剩餘 = 0，中心 = 1、 權限 = 2，且未定義 = 3。 左、 右邊或 center 屬性必須明確設定項目上。  
  
 *nPosInfo*  
 位置資訊。 決定哪些階層式樣式表 (CSS) 的位置值圖像套用對象，在靜態定位 = 0，絕對位置 = 1，相對位置 = 2，且所有 = 3。 此欄位可讓您指定的標記時，它不位於一個圖像 （glyph） 和另一個圖像 （glyph），若要顯示此標籤位於錨點。  
  
 *nDirection*  
 方向。 這個參數會指定目前語言的讀取順序為基礎的標記的影像。 0 指定左到右、 1 指定由右至左、 2 指定由上而下的、 3 會指定從下至上，和 4 指定所有。 您通常會將此欄位設定為 4。  
  
 *nImgWidth*  
 影像寬度，單位為像素。  
  
 *nImgHeight*  
 影像高度，單位為像素。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需有關參數的詳細資訊，請參閱 「 圖像資料表字串格式 」 中[使用編輯圖像 （glyph)](https://msdn.microsoft.com/library/aa969614.aspx)。  
  
 這個方法會傳送[IDM_ADDTOGLYPHTABLE 命令 ID](https://msdn.microsoft.com/library/aa769891.aspx) WebBrowser 控制項。  
  
##  <a name="a-namebolda--chtmleditctrlbasebold"></a><a name="bold"></a>CHtmlEditCtrlBase::Bold  
 切換所選取文字的粗體顯示的狀態。  
  
```  
HRESULT Bold() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BOLD 命令 ID](https://msdn.microsoft.com/library/aa769861.aspx) WebBrowser 控制項。  
  
##  <a name="a-namebuttona--chtmleditctrlbasebutton"></a><a name="button"></a>CHtmlEditCtrlBase::Button  
 會覆寫目前的選取範圍上的按鈕控制項。  
  
```  
HRESULT Button(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 按鈕控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BUTTON 命令 ID](https://msdn.microsoft.com/library/aa769966.aspx) WebBrowser 控制項。  
  
##  <a name="a-namecheckboxa--chtmleditctrlbasecheckbox"></a><a name="checkbox"></a>CHtmlEditCtrlBase::CheckBox  
 會覆寫目前的選取範圍 核取方塊控制項。  
  
```  
HRESULT CheckBox(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 核取方塊控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_CHECKBOX 命令 ID](https://msdn.microsoft.com/library/aa769972.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameclearselectiona--chtmleditctrlbaseclearselection"></a><a name="clearselection"></a>CHtmlEditCtrlBase::ClearSelection  
 清除目前的選取範圍。  
  
```  
HRESULT ClearSelection() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_CLEARSELECTION 命令 ID](https://msdn.microsoft.com/library/aa770038.aspx) WebBrowser 控制項。  
  
##  <a name="a-namecopya--chtmleditctrlbasecopy"></a><a name="copy"></a>CHtmlEditCtrlBase::Copy  
 將目前的選取範圍複製到剪貼簿。  
  
```  
HRESULT Copy() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_COPY 命令 ID](https://msdn.microsoft.com/library/aa769872.aspx) WebBrowser 控制項。  
  
##  <a name="a-namecuta--chtmleditctrlbasecut"></a><a name="cut"></a>CHtmlEditCtrlBase::Cut  
 將目前的選取範圍複製到剪貼簿，然後將它刪除。  
  
```  
HRESULT Cut() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_CUT 命令 ID](https://msdn.microsoft.com/library/aa769875.aspx) WebBrowser 控制項。  
  
##  <a name="a-namedeletea--chtmleditctrlbasedelete"></a><a name="delete"></a>CHtmlEditCtrlBase::Delete  
 刪除目前選取範圍。  
  
```  
HRESULT Delete() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_DELETE 命令 ID](https://msdn.microsoft.com/library/aa769876.aspx) WebBrowser 控制項。  
  
##  <a name="a-namedropdownboxa--chtmleditctrlbasedropdownbox"></a><a name="dropdownbox"></a>CHtmlEditCtrlBase::DropDownBox  
 會覆寫的下拉式清單選取控制項，在目前選取範圍。  
  
```  
HRESULT DropDownBox(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 下拉式清單選取項目控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_DROPDOWNBOX 命令 ID](https://msdn.microsoft.com/library/aa769984.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameemptyglyphtablea--chtmleditctrlbaseemptyglyphtable"></a><a name="emptyglyphtable"></a>CHtmlEditCtrlBase::EmptyGlyphTable  
 隱藏所有影像顯示在設計模式中的標籤圖像 （glyph） 資料表中移除所有項目。  
  
```  
HRESULT EmptyGlyphTable() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_EMPTYGLYPHTABLE 命令 ID](https://msdn.microsoft.com/library/aa769907.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameexeccommanda--chtmleditctrlbaseexeccommand"></a><a name="execcommand"></a>CHtmlEditCtrlBase::ExecCommand  
 執行命令。  
  
```  
HRESULT ExecCommand(
    long cmdID,  
    long cmdExecOpt,  
    VARIANT* pInVar = NULL,  
    VARIANT* pOutVar = NULL) const;  
  
HRESULT ExecCommand(
    const GUID* pGuid,  
    long cmdID,  
    long cmdExecOpt,  
    VARIANT* pInVar = NULL,  
    VARIANT* pOutVar = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `cmdID`  
 要執行的命令識別碼。 如需清單，請參閱[MSHTML 命令識別碼](https://msdn.microsoft.com/library/aa741315.aspx)。  
  
 `cmdExecOpt`  
 值取自[OLECMDEXECOPT](http://msdn.microsoft.com/library/windows/desktop/ms683930)列舉，描述物件執行命令的方式。  
  
 *pInVar*  
 輸入引數。  
  
 *pOutVar*  
 命令輸出中。  
  
 *pGuid*  
 命令群組 GUID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法提供的功能[IOleCommandTarget::Exec](http://msdn.microsoft.com/library/windows/desktop/ms690300)。  
  
##  <a name="a-namefonta--chtmleditctrlbasefont"></a><a name="font"></a>CHtmlEditCtrlBase::Font  
 開啟 [字型] 對話方塊，讓使用者變更文字色彩、 字型和目前的選取範圍的字型大小。  
  
```  
HRESULT Font() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_FONT 命令 ID](https://msdn.microsoft.com/library/aa769913.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetabsolutepositiona--chtmleditctrlbasegetabsoluteposition"></a><a name="getabsoluteposition"></a>CHtmlEditCtrlBase::GetAbsolutePosition  
 傳回項目的位置的屬性是否為 「 絕對 」。  
  
```  
HRESULT GetAbsolutePosition(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果此項目的位置屬性設定為 「 絕對。 」，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_ABSOLUTE_POSITION 命令 ID](https://msdn.microsoft.com/library/aa769889.aspx)。  
  
##  <a name="a-namegetbackcolora--chtmleditctrlbasegetbackcolor"></a><a name="getbackcolor"></a>CHtmlEditCtrlBase::GetBackColor  
 擷取目前的選取範圍的背景色彩。  
  
```  
HRESULT GetBackColor(int& nColor) const;  
```  
  
### <a name="parameters"></a>參數  
 `nColor`  
 背景色彩。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BACKCOLOR 命令 ID](https://msdn.microsoft.com/library/aa769858.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetblockformata--chtmleditctrlbasegetblockformat"></a><a name="getblockformat"></a>CHtmlEditCtrlBase::GetBlockFormat  
 擷取目前的區塊格式標記。  
  
```  
HRESULT GetBlockFormat(CString& strFormat) const;  
```  
  
### <a name="parameters"></a>參數  
 *strFormat*  
 目前的區塊格式標記。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BLOCKFMT 命令 ID](https://msdn.microsoft.com/library/aa769883.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetblockformatnamesa--chtmleditctrlbasegetblockformatnames"></a><a name="getblockformatnames"></a>CHtmlEditCtrlBase::GetBlockFormatNames  
 擷取可用的區塊格式標記相對應的字串。  
  
```  
HRESULT GetBlockFormatNames(CStringArray& sa) const;  
```  
  
### <a name="parameters"></a>參數  
 *sa*  
 可用的區塊格式標記，做為字串的陣列。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_GETBLOCKFMTS 命令識別碼](https://msdn.microsoft.com/library/aa769884.aspx)WebBrowser 控制項。  
  
##  <a name="a-namegetbookmarka--chtmleditctrlbasegetbookmark"></a><a name="getbookmark"></a>CHtmlEditCtrlBase::GetBookMark  
 擷取書籤錨點名稱。  
  
```  
HRESULT GetBookMark(CString& strAnchor) const;  
```  
  
### <a name="parameters"></a>參數  
 *strAnchor*  
 書籤錨點名稱。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_BOOKMARK 命令 ID](https://msdn.microsoft.com/library/aa769873.aspx)。  
  
##  <a name="a-namegetdocumenta--chtmleditctrlbasegetdocument"></a><a name="getdocument"></a>CHtmlEditCtrlBase::GetDocument  
 擷取的文件物件。  
  
```  
HRESULT GetDocument(IHTMLDocument2** ppDoc) const;  
```  
  
### <a name="parameters"></a>參數  
 `ppDoc`  
 文件物件。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-namegetdocumenthtmla--chtmleditctrlbasegetdocumenthtml"></a><a name="getdocumenthtml"></a>CHtmlEditCtrlBase::GetDocumentHTML  
 擷取目前的文件的 HTML。  
  
```  
HRESULT GetDocumentHTML(CString& szHTML) const;  
```  
  
### <a name="parameters"></a>參數  
 `szHTML`  
 HTML。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-namegetdocumenttitlea--chtmleditctrlbasegetdocumenttitle"></a><a name="getdocumenttitle"></a>CHtmlEditCtrlBase::GetDocumentTitle  
 擷取文件的標題。  
  
```  
HRESULT GetDocumentTitle(CString& szTitle) const;  
```  
  
### <a name="parameters"></a>參數  
 *szTitle*  
 文件的標題。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-namegeteventa--chtmleditctrlbasegetevent"></a><a name="getevent"></a>CHtmlEditCtrlBase::GetEvent  
 擷取包含最新事件的相關資訊的事件物件的介面指標。  
  
```  
HRESULT GetEvent(IHTMLEventObj** ppEventObj) const;  
```  
  
### <a name="parameters"></a>參數  
 `ppEventObj`  
 事件物件。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-namegeteventsrcelementa--chtmleditctrlbasegeteventsrcelement"></a><a name="geteventsrcelement"></a>CHtmlEditCtrlBase::GetEventSrcElement  
 擷取引發事件的物件。  
  
```  
HRESULT GetEventSrcElement(IHTMLElement** ppSrcElement) const;  
```  
  
### <a name="parameters"></a>參數  
 *ppSrcElement*  
 引發事件的項目。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-namegetfontfacea--chtmleditctrlbasegetfontface"></a><a name="getfontface"></a>CHtmlEditCtrlBase::GetFontFace  
 擷取目前的選取範圍的字型名稱。  
  
```  
HRESULT GetFontFace(CString& strFace) const;  
```  
  
### <a name="parameters"></a>參數  
 `strFace`  
 字型名稱。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如果目前的選取範圍會使用多個字型，`strFace`將會是空的字串。  
  
 這個方法會傳送[IDM_FONTNAME 命令 ID](https://msdn.microsoft.com/library/aa769880.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetfontsizea--chtmleditctrlbasegetfontsize"></a><a name="getfontsize"></a>CHtmlEditCtrlBase::GetFontSize  
 擷取目前的選取範圍的字型大小。  
  
```  
HRESULT GetFontSize(short& nSize) const;  
```  
  
### <a name="parameters"></a>參數  
 `nSize`  
 字型大小。  
  
### <a name="return-value"></a>傳回值  
 傳回的 HTML 字型的大小 (1-7)。 如果選取範圍包含多個字型的大小，，傳回 0。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_FONTSIZE 命令 ID](https://msdn.microsoft.com/library/aa769881.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetforecolora--chtmleditctrlbasegetforecolor"></a><a name="getforecolor"></a>CHtmlEditCtrlBase::GetForeColor  
 擷取目前的選取範圍的 （文字） 的前景色彩。  
  
```  
HRESULT GetForeColor(int& nColor);
```  
  
### <a name="parameters"></a>參數  
 `nColor`  
 前景色彩。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_FORECOLOR 命令 ID](https://msdn.microsoft.com/library/aa769882.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetframezonea--chtmleditctrlbasegetframezone"></a><a name="getframezone"></a>CHtmlEditCtrlBase::GetFrameZone  
 傳回目前頁面的安全性區域中，網頁瀏覽器中。  
  
```  
HRESULT GetFrameZone(short& nZone) const;  
```  
  
### <a name="parameters"></a>參數  
 *nZone*  
 安全性區域中。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_GETFRAMEZONE 命令 ID](https://msdn.microsoft.com/library/aa769916.aspx) WebBrowser 控制項。  
  
##  <a name="a-namegetisdirtya--chtmleditctrlbasegetisdirty"></a><a name="getisdirty"></a>CHtmlEditCtrlBase::GetIsDirty  
 指出是否已變更的 HTML 文件。  
  
```  
HRESULT GetIsDirty() const;  
```  
  
### <a name="remarks"></a>備註  
 指出是否已變更文件。 `GetIsDirty`傳回`HRESULT`從[IPersistStorage::IsDirty](http://msdn.microsoft.com/library/windows/desktop/ms683910)。  
  
##  <a name="a-namegetshowalignedsitetagsa--chtmleditctrlbasegetshowalignedsitetags"></a><a name="getshowalignedsitetags"></a>CHtmlEditCtrlBase::GetShowAlignedSiteTags  
 傳回所有項目是否會顯示圖像**styleFloat**屬性。  
  
```  
HRESULT GetShowAlignedSiteTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果圖像 （glyph） 會顯示所有項目則為 true **styleFloat**屬性則為 false 會顯示任何圖像 （glyph）。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWALIGNEDSITETAGS 命令 ID](https://msdn.microsoft.com/library/aa769947.aspx)。  
  
##  <a name="a-namegetshowalltagsa--chtmleditctrlbasegetshowalltags"></a><a name="getshowalltags"></a>CHtmlEditCtrlBase::GetShowAllTags  
 傳回 WebBrowser 是否顯示這些圖像會顯示文件中的所有標記的位置。  
  
```  
HRESULT GetShowAllTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示這些圖像會顯示在文件中的所有標記的位置，則為 true。如果沒有，false。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWALLTAGS 命令 ID](https://msdn.microsoft.com/library/aa769948.aspx)。  
  
##  <a name="a-namegetshowareatagsa--chtmleditctrlbasegetshowareatags"></a><a name="getshowareatags"></a>CHtmlEditCtrlBase::GetShowAreaTags  
 擷取是否 WebBrowser 顯示區域標記的圖像。  
  
```  
HRESULT GetShowAreaTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示區域標記的圖像 false 如果沒有，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWAREATAGS 命令 ID](https://msdn.microsoft.com/library/aa769949.aspx)。  
  
##  <a name="a-namegetshowbrtagsa--chtmleditctrlbasegetshowbrtags"></a><a name="getshowbrtags"></a>CHtmlEditCtrlBase::GetShowBRTags  
 擷取是否 WebBrowser 顯示 br 標記的圖像。  
  
```  
HRESULT GetShowBRTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示 br 標記的圖像 false 如果沒有，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWWBRTAGS 命令 ID](https://msdn.microsoft.com/library/aa769956.aspx)。  
  
##  <a name="a-namegetshowcommenttagsa--chtmleditctrlbasegetshowcommenttags"></a><a name="getshowcommenttags"></a>CHtmlEditCtrlBase::GetShowCommentTags  
 擷取是否 WebBrowser 顯示註解標記的圖像。  
  
```  
HRESULT GetShowCommentTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示註解標記的圖像 false 如果沒有，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWCOMMENTTAGS 命令 ID](https://msdn.microsoft.com/library/aa769950.aspx)。  
  
##  <a name="a-namegetshowmisctagsa--chtmleditctrlbasegetshowmisctags"></a><a name="getshowmisctags"></a>CHtmlEditCtrlBase::GetShowMiscTags  
 擷取是否 WebBrowser 顯示 Microsoft Internet Explorer 4.0 中所顯示的所有標記。  
  
```  
HRESULT GetShowMiscTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 會顯示在 Microsoft Internet Explorer 4.0，false，如果沒有顯示的所有標記，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWMISCTAGS 命令 ID](https://msdn.microsoft.com/library/aa769952.aspx)。  
  
##  <a name="a-namegetshowscripttagsa--chtmleditctrlbasegetshowscripttags"></a><a name="getshowscripttags"></a>CHtmlEditCtrlBase::GetShowScriptTags  
 擷取是否 WebBrowser 顯示所有指令碼標記的圖像。  
  
```  
HRESULT GetShowScriptTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示圖像的所有指令碼標記，false 如果沒有，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWSCRIPTTAGS 命令 ID](https://msdn.microsoft.com/library/aa769953.aspx)。  
  
##  <a name="a-namegetshowstyletagsa--chtmleditctrlbasegetshowstyletags"></a><a name="getshowstyletags"></a>CHtmlEditCtrlBase::GetShowStyleTags  
 擷取是否 WebBrowser 顯示樣式的所有標記的圖像。  
  
```  
HRESULT GetShowStyleTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示圖像的所有樣式標記，false 如果沒有，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWSTYLETAGS 命令 ID](https://msdn.microsoft.com/library/aa769954.aspx)。  
  
##  <a name="a-namegetshowunknowntagsa--chtmleditctrlbasegetshowunknowntags"></a><a name="getshowunknowntags"></a>CHtmlEditCtrlBase::GetShowUnknownTags  
 擷取是否 WebBrowser 顯示所有未知的標記的圖像。  
  
```  
HRESULT GetShowUnknownTags(bool& bCurValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bCurValue`  
 如果 WebBrowser 顯示所有未知的標記的圖像 false 如果沒有，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 如需詳細資訊，請參閱[IDM_SHOWUNKNOWNTAGS 命令 ID](https://msdn.microsoft.com/library/aa769955.aspx)。  
  
##  <a name="a-namehorizontallinea--chtmleditctrlbasehorizontalline"></a><a name="horizontalline"></a>CHtmlEditCtrlBase::HorizontalLine  
 在目前選取範圍的水平列會覆寫。  
  
```  
HRESULT HorizontalLine(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 *szID*  
 水平列的識別碼。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_HORIZONTALLINE 命令 ID](https://msdn.microsoft.com/library/aa769968.aspx) WebBrowser 控制項。  
  
##  <a name="a-namehyperlinka--chtmleditctrlbasehyperlink"></a><a name="hyperlink"></a>CHtmlEditCtrlBase::HyperLink  
 插入目前的選取範圍的超連結。  
  
```  
HRESULT HyperLink(LPCTSTR szUrl = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szUrl`  
 超連結 URL。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_HYPERLINK 命令 ID](https://msdn.microsoft.com/library/aa769874.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameie50pastea--chtmleditctrlbaseie50paste"></a><a name="ie50paste"></a>CHtmlEditCtrlBase::IE50Paste  
 執行與 Internet Explorer 5 貼上作業。  
  
```  
HRESULT IE50Paste(LPCTSTR szData) const;  
```  
  
### <a name="parameters"></a>參數  
 `szData`  
 要貼上的字串。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_IE50_PASTE 命令 ID](https://msdn.microsoft.com/library/aa769922.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameiframea--chtmleditctrlbaseiframe"></a><a name="iframe"></a>CHtmlEditCtrlBase::Iframe  
 會覆寫目前的選取範圍的內嵌框架。  
  
```  
HRESULT Iframe(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 內嵌框架 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_IFRAME 命令 ID](https://msdn.microsoft.com/library/aa769969.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameimagea--chtmleditctrlbaseimage"></a><a name="image"></a>CHtmlEditCtrlBase::Image  
 會覆寫目前的選取範圍上的影像。  
  
```  
HRESULT Image(LPCTSTR szUrl = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szUrl`  
 要插入的影像路徑和檔案名稱。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_IMAGE 命令 ID](https://msdn.microsoft.com/library/aa769970.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameindenta--chtmleditctrlbaseindent"></a><a name="indent"></a>CHtmlEditCtrlBase::Indent  
 一個縮排的增量，增加所選文字的縮排。  
  
```  
HRESULT Indent() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INDENT 命令 ID](https://msdn.microsoft.com/library/aa769963.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsfieldseta--chtmleditctrlbaseinsfieldset"></a><a name="insfieldset"></a>CHtmlEditCtrlBase::InsFieldSet  
 會覆寫目前的選取範圍中的方塊。  
  
```  
HRESULT InsFieldSet(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 方塊識別碼。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSFIELDSET 命令 ID](https://msdn.microsoft.com/library/aa769967.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputbuttona--chtmleditctrlbaseinsinputbutton"></a><a name="insinputbutton"></a>CHtmlEditCtrlBase::InsInputButton  
 會覆寫目前的選取範圍上的按鈕控制項。  
  
```  
HRESULT InsInputButton(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 Button 控制項 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTBUTTON 命令 ID](https://msdn.microsoft.com/library/aa769971.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputhiddena--chtmleditctrlbaseinsinputhidden"></a><a name="insinputhidden"></a>CHtmlEditCtrlBase::InsInputHidden  
 插入隱藏的控制項目前的選取範圍。  
  
```  
HRESULT InsInputHidden(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 隱藏控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTHIDDEN 命令 ID](https://msdn.microsoft.com/library/aa769974.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputimagea--chtmleditctrlbaseinsinputimage"></a><a name="insinputimage"></a>CHtmlEditCtrlBase::InsInputImage  
 影像控制項在目前選取範圍會覆寫。  
  
```  
HRESULT InsInputImage(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 [影像] 控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTIMAGE 命令 ID](https://msdn.microsoft.com/library/aa769975.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputpassworda--chtmleditctrlbaseinsinputpassword"></a><a name="insinputpassword"></a>CHtmlEditCtrlBase::InsInputPassword  
 會覆寫密碼上的控制項目前的選取範圍。  
  
```  
HRESULT InsInputPassword(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 密碼控制項 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTPASSWORD 命令 ID](https://msdn.microsoft.com/library/aa769976.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputreseta--chtmleditctrlbaseinsinputreset"></a><a name="insinputreset"></a>CHtmlEditCtrlBase::InsInputReset  
 重設控制項目前的選取範圍會覆寫。  
  
```  
HRESULT InsInputReset(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 重設控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTRESET 命令 ID](https://msdn.microsoft.com/library/aa769978.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputsubmita--chtmleditctrlbaseinsinputsubmit"></a><a name="insinputsubmit"></a>CHtmlEditCtrlBase::InsInputSubmit  
 在目前選取範圍的送出控制項將會覆寫。  
  
```  
HRESULT InsInputSubmit(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 送出控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTSUBMIT 命令 ID](https://msdn.microsoft.com/library/aa769979.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameinsinputuploada--chtmleditctrlbaseinsinputupload"></a><a name="insinputupload"></a>CHtmlEditCtrlBase::InsInputUpload  
 會覆寫檔案上傳控制項，在目前選取範圍。  
  
```  
HRESULT InsInputUpload(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 檔案上傳控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_INSINPUTUPLOAD 命令 ID](https://msdn.microsoft.com/library/aa769973.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameis1delementa--chtmleditctrlbaseis1delement"></a><a name="is1delement"></a>CHtmlEditCtrlBase::Is1DElement  
 判斷以靜態方式是否置於項目。  
  
```  
HRESULT Is1DElement(bool& bValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bValue`  
 如果項目處於靜態定位，則為 false，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_1D_ELEMENT 命令 ID](https://msdn.microsoft.com/library/aa769885.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameis2delementa--chtmleditctrlbaseis2delement"></a><a name="is2delement"></a>CHtmlEditCtrlBase::Is2DElement  
 判斷項目會具有絕對位置。  
  
```  
HRESULT Is2DElement(bool& bValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bValue`  
 如果項目處於絕對位置，則為 false，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_2D_ELEMENT 命令 ID](https://msdn.microsoft.com/library/aa769886.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameitalica--chtmleditctrlbaseitalic"></a><a name="italic"></a>CHtmlEditCtrlBase::Italic  
 切換目前的選取範圍斜體與 nonitalic 之間。  
  
```  
HRESULT Italic() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_ITALIC 命令 ID](https://msdn.microsoft.com/library/aa769988.aspx) WebBrowser 控制項。  
  
##  <a name="a-namejustifycentera--chtmleditctrlbasejustifycenter"></a><a name="justifycenter"></a>CHtmlEditCtrlBase::JustifyCenter  
 區塊的格式將置中在其目前的選取範圍位於。  
  
```  
HRESULT JustifyCenter() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_JUSTIFYCENTER 命令 ID](https://msdn.microsoft.com/library/aa769989.aspx) WebBrowser 控制項。  
  
##  <a name="a-namejustifylefta--chtmleditctrlbasejustifyleft"></a><a name="justifyleft"></a>CHtmlEditCtrlBase::JustifyLeft  
 靠左對齊格式區塊中目前選取範圍所在。  
  
```  
HRESULT JustifyLeft() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_JUSTIFYLEFT 命令 ID](https://msdn.microsoft.com/library/aa770011.aspx) WebBrowser 控制項。  
  
##  <a name="a-namejustifyrighta--chtmleditctrlbasejustifyright"></a><a name="justifyright"></a>CHtmlEditCtrlBase::JustifyRight  
 靠右對齊區塊的格式中目前選取範圍所在。  
  
```  
HRESULT JustifyRight() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_JUSTIFYRIGHT 命令 ID](https://msdn.microsoft.com/library/aa770013.aspx) WebBrowser 控制項。  
  
##  <a name="a-namelistboxa--chtmleditctrlbaselistbox"></a><a name="listbox"></a>CHtmlEditCtrlBase::ListBox  
 會覆寫目前的選取範圍的清單方塊選取項目控制項。  
  
```  
HRESULT ListBox(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 清單方塊控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_LISTBOX 命令 ID](https://msdn.microsoft.com/library/aa769985.aspx) WebBrowser 控制項。  
  
##  <a name="a-namemarqueea--chtmleditctrlbasemarquee"></a><a name="marquee"></a>CHtmlEditCtrlBase::Marquee  
 會覆寫目前的選取範圍的空跑馬燈。  
  
```  
HRESULT Marquee(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 跑馬燈的識別碼。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_MARQUEE 命令 ID](https://msdn.microsoft.com/library/aa769981.aspx) WebBrowser 控制項。  
  
##  <a name="a-namenewdocumenta--chtmleditctrlbasenewdocument"></a><a name="newdocument"></a>CHtmlEditCtrlBase::NewDocument  
 建立新的文件。  
  
```  
HRESULT NewDocument() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-nameorderlista--chtmleditctrlbaseorderlist"></a><a name="orderlist"></a>CHtmlEditCtrlBase::OrderList  
 切換目前的選取範圍的已排序的清單之間的一般格式的區塊。  
  
```  
HRESULT OrderList(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 如需已排序的清單識別碼。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_ORDERLIST 命令 ID](https://msdn.microsoft.com/library/aa769982.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameoutdenta--chtmleditctrlbaseoutdent"></a><a name="outdent"></a>CHtmlEditCtrlBase::Outdent  
 減少縮排格式區塊的一個遞增值中目前選取範圍所在。  
  
```  
HRESULT Outdent() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_OUTDENT 命令 ID](https://msdn.microsoft.com/library/aa770015.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameparagrapha--chtmleditctrlbaseparagraph"></a><a name="paragraph"></a>CHtmlEditCtrlBase::Paragraph  
 會覆寫目前的選取範圍的分行符號。  
  
```  
HRESULT Paragraph(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 段落編號。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_PARAGRAPH 命令 ID](https://msdn.microsoft.com/library/aa769983.aspx) WebBrowser 控制項。  
  
##  <a name="a-namepastea--chtmleditctrlbasepaste"></a><a name="paste"></a>CHtmlEditCtrlBase::Paste  
 會覆寫目前的選取範圍剪貼簿的內容。  
  
```  
HRESULT Paste() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_PASTE 命令 ID](https://msdn.microsoft.com/library/aa770017.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameprintdocumenta--chtmleditctrlbaseprintdocument"></a><a name="printdocument"></a>CHtmlEditCtrlBase::PrintDocument  
 列印目前文件。  
  
```  
HRESULT PrintDocument() const;  
HRESULT PrintDocument(LPCTSTR szPrintTemplate) const;  
HRESULT PrintDocument(bool bShowPrintDialog) const;  
```  
  
### <a name="parameters"></a>參數  
 `szPrintTemplate`  
 路徑的列印範本。如果未指定，則會使用預設的列印範本。  
  
 *bShowPrintDialog*  
 如果為 true，會顯示 [列印] 對話方塊。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_PRINT 命令 ID](https://msdn.microsoft.com/library/aa769937.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameprintpreviewa--chtmleditctrlbaseprintpreview"></a><a name="printpreview"></a>CHtmlEditCtrlBase::PrintPreview  
 開啟目前文件使用預設的預覽列印範本或自訂範本的預覽列印視窗。  
  
```  
HRESULT PrintPreview() const;  
HRESULT PrintPreview(LPCTSTR szPrintTemplate) const;  
```  
  
### <a name="parameters"></a>參數  
 `szPrintTemplate`  
 列印範本的路徑。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_PRINTPREVIEW 命令 ID](https://msdn.microsoft.com/library/aa769938.aspx) WebBrowser 控制項。  
  
##  <a name="a-namequerystatusa--chtmleditctrlbasequerystatus"></a><a name="querystatus"></a>CHtmlEditCtrlBase::QueryStatus  
 呼叫這個方法來查詢命令的狀態。  
  
```  
long QueryStatus(long cmdID) const;  
```  
  
### <a name="parameters"></a>參數  
 `cmdID`  
 命令 ID。 命令識別碼都取自`CGID_MSHTML`命令群組。 這些命令定義於 Mshtmcid.h。 您也可以尋找在線上的清單[MSHTML 命令識別碼](http://go.microsoft.com/fwlink/linkid=149220)。  
  
### <a name="return-value"></a>傳回值  
 傳回[OLECMDF](http://msdn.microsoft.com/library/windows/desktop/ms695237)的狀態，指出`cmdID`，或 0 失敗。  
  
##  <a name="a-nameradiobuttona--chtmleditctrlbaseradiobutton"></a><a name="radiobutton"></a>CHtmlEditCtrlBase::RadioButton  
 會覆寫目前的選取範圍的選項按鈕控制項。  
  
```  
HRESULT RadioButton(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 選項按鈕的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_RADIOBUTTON 命令 ID](https://msdn.microsoft.com/library/aa769977.aspx) WebBrowser 控制項。  
  
##  <a name="a-namerefreshdocumenta--chtmleditctrlbaserefreshdocument"></a><a name="refreshdocument"></a>CHtmlEditCtrlBase::RefreshDocument  
 重新整理目前的文件。  
  
```  
HRESULT RefreshDocument() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_REFRESH 命令 ID](https://msdn.microsoft.com/library/aa770020.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameremoveformata--chtmleditctrlbaseremoveformat"></a><a name="removeformat"></a>CHtmlEditCtrlBase::RemoveFormat  
 移除目前選取範圍的格式標記。  
  
```  
HRESULT RemoveFormat() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_REMOVEFORMAT 命令 ID](https://msdn.microsoft.com/library/aa770021.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesaveasa--chtmleditctrlbasesaveas"></a><a name="saveas"></a>CHtmlEditCtrlBase::SaveAs  
 將目前網頁儲存至檔案。  
  
```  
HRESULT SaveAs(LPCTSTR szPath = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szPath`  
 要用來儲存 Web 網頁的路徑和檔案名稱。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_SAVEAS 命令識別碼](https://msdn.microsoft.com/library/aa770024.aspx)WebBrowser 控制項。  
  
##  <a name="a-nameselectalla--chtmleditctrlbaseselectall"></a><a name="selectall"></a>CHtmlEditCtrlBase::SelectAll  
 選取整份文件。  
  
```  
HRESULT SelectAll() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_SELECTALL 命令 ID](https://msdn.microsoft.com/library/aa770025.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameset2dpositiona--chtmleditctrlbaseset2dposition"></a><a name="set2dposition"></a>CHtmlEditCtrlBase::Set2DPosition  
 允許拖曳移動的絕對定位項目。  
  
```  
HRESULT Set2DPosition(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則可以藉由拖曳線上移動絕對定位項目。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_2D_POSITION 命令 ID](https://msdn.microsoft.com/library/aa769887.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetabsolutepositiona--chtmleditctrlbasesetabsoluteposition"></a><a name="setabsoluteposition"></a>CHtmlEditCtrlBase::SetAbsolutePosition  
 項目的位置將屬性設定為 「 絕對 」 或 「 靜態 」。  
  
```  
HRESULT SetAbsolutePosition(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，此項目的位置屬性是 「 絕對 」;如果為 false，，則 「 靜態 」。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_ABSOLUTE_POSITION 命令 ID](https://msdn.microsoft.com/library/aa769889.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetatomicselectiona--chtmleditctrlbasesetatomicselection"></a><a name="setatomicselection"></a>CHtmlEditCtrlBase::SetAtomicSelection  
 設定不可部分完成的選取模式。  
  
```  
HRESULT SetAtomicSelection(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，且 ATOMICSELECTION 屬性設定為 TRUE 的任何項目可僅做為一個單位。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_ATOMICSELECTION 命令 ID](https://msdn.microsoft.com/library/aa769892.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetautourldetectmodea--chtmleditctrlbasesetautourldetectmode"></a><a name="setautourldetectmode"></a>CHtmlEditCtrlBase::SetAutoURLDetectMode  
 開啟自動 URL 偵測和關閉。  
  
```  
HRESULT SetAutoURLDetectMode(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會啟用自動的 URL 偵測。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_AUTOURLDETECT_MODE 命令 ID](https://msdn.microsoft.com/library/aa769893.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetbackcolora--chtmleditctrlbasesetbackcolor"></a><a name="setbackcolor"></a>CHtmlEditCtrlBase::SetBackColor  
 設定目前選取範圍的背景色彩。  
  
```  
HRESULT SetBackColor(int nColor) const;  
HRESULT SetBackColor(LPCTSTR szColor) const;  
```  
  
### <a name="parameters"></a>參數  
 `nColor`  
 色彩。 請參閱`pvaIn`中[IDM_BACKCOLOR 命令 ID](https://msdn.microsoft.com/library/aa769858.aspx)。  
  
 `szColor`  
 色彩。 請參閱`pvaIn`中[IDM_BACKCOLOR 命令 ID](https://msdn.microsoft.com/library/aa769858.aspx)。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BACKCOLOR_ 命令 ID](https://msdn.microsoft.com/library/aa769858.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetblockformata--chtmleditctrlbasesetblockformat"></a><a name="setblockformat"></a>CHtmlEditCtrlBase::SetBlockFormat  
 設定目前的區塊格式標記。  
  
```  
HRESULT SetBlockFormat(LPCTSTR szFormat) const;  
```  
  
### <a name="parameters"></a>參數  
 `szFormat`  
 格式標記。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BLOCKFMT_command 識別碼](https://msdn.microsoft.com/library/aa769883.aspx)WebBrowser 控制項。  
  
##  <a name="a-namesetbookmarka--chtmleditctrlbasesetbookmark"></a><a name="setbookmark"></a>CHtmlEditCtrlBase::SetBookMark  
 建立目前的選取範圍或插入點的書籤錨點。  
  
```  
HRESULT SetBookMark(LPCTSTR szAnchorName) const;  
```  
  
### <a name="parameters"></a>參數  
 *szAnchorName*  
 錨點名稱。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_BOOKMARK 命令 ID](https://msdn.microsoft.com/library/aa769873.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetcsseditinglevela--chtmleditctrlbasesetcsseditinglevel"></a><a name="setcsseditinglevel"></a>CHtmlEditCtrlBase::SetCSSEditingLevel  
 選取的 CSS 層級 （CSS1 或 CSS2） 編輯器將支援，如果有的話。  
  
```  
HRESULT SetCSSEditingLevel(short nLevel) const;  
```  
  
### <a name="parameters"></a>參數  
 `nLevel`  
 CSS 層級。 如果您不想 CSS 支援，請傳遞 0。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_CSSEDITING_LEVEL 命令 ID](https://msdn.microsoft.com/library/aa769903.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetdefaultcomposesettingsa--chtmleditctrlbasesetdefaultcomposesettings"></a><a name="setdefaultcomposesettings"></a>CHtmlEditCtrlBase::SetDefaultComposeSettings  
 呼叫這個方法，以設定預設撰寫設定。  
  
```  
HRESULT SetDefaultComposeSettings(
    LPCSTR szFontName = NULL,  
    unsigned short nFontSize = 3,  
    COLORREF crFontColor = 0xFF000000,  
    COLORREF crFontBgColor = 0xFF000000,  
    bool bBold = false,  
    bool bItalic = false,  
    bool bUnderline = false) const;  
```  
  
### <a name="parameters"></a>參數  
 *szFontName*  
 字型名稱。  
  
 *nFontSize*  
 字型大小。  
  
 *crFontColor*  
 字型色彩。  
  
 *crFontBgColor*  
 字型的背景色彩。  
  
 *bBold*  
 傳遞粗體文字，則為 true。  
  
 `bItalic`  
 傳遞斜體的文字，則為 true。  
  
 `bUnderline`  
 傳遞的加底線的文字，則為 true。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_COMPOSESETTINGS 命令識別碼](https://msdn.microsoft.com/library/aa769901.aspx)WebBrowser 控制項。  
  
##  <a name="a-namesetdesignmodea--chtmleditctrlbasesetdesignmode"></a><a name="setdesignmode"></a>CHtmlEditCtrlBase::SetDesignMode  
 設定設計模式。  
  
```  
BOOL SetDesignMode(BOOL bMode) const;  
```  
  
### <a name="parameters"></a>參數  
 `bMode`  
 如果為 true，會開啟設計模式。  
  
### <a name="return-value"></a>傳回值  
 成功時，會傳回 TRUE 失敗，則為 FALSE。  
  
##  <a name="a-namesetdisableeditfocusuia--chtmleditctrlbasesetdisableeditfocusui"></a><a name="setdisableeditfocusui"></a>CHtmlEditCtrlBase::SetDisableEditFocusUI  
 停用陰影的框線，並編輯焦點的項目控點。  
  
```  
HRESULT SetDisableEditFocusUI(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，會停用的陰影的框線和站台可選取的項目周圍的控制代碼時，項目在設計模式中，有 編輯焦點也就是說，當文字或項目的內容可供編輯。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM_DISABLE_EDITFOCUS_UI 命令 ID](https://msdn.microsoft.com/library/aa769905.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetdocumenthtmla--chtmleditctrlbasesetdocumenthtml"></a><a name="setdocumenthtml"></a>CHtmlEditCtrlBase::SetDocumentHTML  
 設定目前文件的 HTML。  
  
```  
HRESULT SetDocumentHTML(LPCTSTR szHTML) const;  
```  
  
### <a name="parameters"></a>參數  
 `szHTML`  
 HTML。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
##  <a name="a-namesetfontfacea--chtmleditctrlbasesetfontface"></a><a name="setfontface"></a>CHtmlEditCtrlBase::SetFontFace  
 設定目前選取範圍的字型。  
  
```  
HRESULT SetFontFace(LPCTSTR szFace) const;  
```  
  
### <a name="parameters"></a>參數  
 `szFace`  
 字型名稱。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM 字型名稱的命令 ID](https://msdn.microsoft.com/library/aa769880.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetfontsizea--chtmleditctrlbasesetfontsize"></a><a name="setfontsize"></a>CHtmlEditCtrlBase::SetFontSize  
 設定目前選取範圍的字型大小。  
  
```  
HRESULT SetFontSize(unsigned short size) const;  
```  
  
### <a name="parameters"></a>參數  
 `size`  
 HTML 字型大小 (1-7)。 值為 0 會將字型大小設定為 1。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM FONTSIZE 命令 ID](https://msdn.microsoft.com/library/aa769881.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetforecolora--chtmleditctrlbasesetforecolor"></a><a name="setforecolor"></a>CHtmlEditCtrlBase::SetForeColor  
 設定目前選取範圍 （文字） 的前景色彩。  
  
```  
HRESULT SetForeColor(LPCTSTR szColor) const;  
HRESULT SetForeColor(int nColor) const;  
```  
  
### <a name="parameters"></a>參數  
 `szColor`  
 色彩。  
  
 `nColor`  
 色彩。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM FORECOLOR 命令 ID](https://msdn.microsoft.com/library/aa769882.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetie5pastemodea--chtmleditctrlbasesetie5pastemode"></a><a name="setie5pastemode"></a>CHtmlEditCtrlBase::SetIE5PasteMode  
 設定 Microsoft Internet Explorer 5 與貼上作業。  
  
```  
HRESULT SetIE5PasteMode(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，貼上的所有作業都都與 Internet Explorer 5;如果為 false，貼上作業將會與 Internet Explorer 5.5 相容。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM IE50_PASTE_MODE 命令 ID](https://msdn.microsoft.com/library/aa769923.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetliveresizea--chtmleditctrlbasesetliveresize"></a><a name="setliveresize"></a>CHtmlEditCtrlBase::SetLiveResize  
 會導致 WebBrowser 更新項目的外觀，期間持續調整大小或移動作業，而非更新只在完成移動或調整大小。  
  
```  
HRESULT SetLiveResize(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，會導致更新項目的外觀持續調整大小或移動作業期間 WebBrowser如果為 false，便會更新只會在完成移動或調整大小。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM LIVERESIZE 命令 ID](https://msdn.microsoft.com/library/aa769928.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetmultiselecta--chtmleditctrlbasesetmultiselect"></a><a name="setmultiselect"></a>CHtmlEditCtrlBase::SetMultiSelect  
 可讓多個選取項目。  
  
```  
HRESULT SetMultiSelect(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，可讓使用者按住 SHIFT 或 CTRL 鍵一次選取多個站台可選取的項目。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM MULTIPLESELECTION 命令 ID](https://msdn.microsoft.com/library/aa769929.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetoverridecursora--chtmleditctrlbasesetoverridecursor"></a><a name="setoverridecursor"></a>CHtmlEditCtrlBase::SetOverrideCursor  
 命令永遠不會以變更滑鼠指標 WebBrowser。  
  
```  
HRESULT SetOverrideCursor(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，WebBrowser 不會變更滑鼠指標。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM OVERRIDE_CURSOR 命令 ID](https://msdn.microsoft.com/library/aa769932.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetoverwritemodea--chtmleditctrlbasesetoverwritemode"></a><a name="setoverwritemode"></a>CHtmlEditCtrlBase::SetOverwriteMode  
 切換插入之間的文字輸入模式，並覆寫。  
  
```  
HRESULT SetOverwriteMode(bool bMode) const;  
```  
  
### <a name="parameters"></a>參數  
 `bMode`  
 如果為 true，會覆寫文字輸入模式。如果為 false，則文字輸入模式為 insert。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM 覆寫命令 ID](https://msdn.microsoft.com/library/aa770016.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetrespectvisindesigna--chtmleditctrlbasesetrespectvisindesign"></a><a name="setrespectvisindesign"></a>CHtmlEditCtrlBase::SetRespectVisInDesign  
 會隱藏在設計模式中不可見的項目。  
  
```  
HRESULT SetRespectVisInDesign(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，任何已設定為 「 隱藏 」，或顯示屬性設定為 「 無 」 的可見性的項目將不會顯示在設計模式和瀏覽模式。如果為 false，這些項目將顯示只在瀏覽模式。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM RESPECTVISIBILITY_INDESIGN 命令 ID](https://msdn.microsoft.com/library/aa770023.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowalignedsitetagsa--chtmleditctrlbasesetshowalignedsitetags"></a><a name="setshowalignedsitetags"></a>CHtmlEditCtrlBase::SetShowAlignedSiteTags  
 顯示所有項目具有的圖像**styleFloat**屬性。  
  
```  
HRESULT SetShowAlignedSiteTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，會顯示所有項目具有的圖像**styleFloat**屬性。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWALIGNEDSITETAGS 命令 ID](https://msdn.microsoft.com/library/aa769947.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowalltagsa--chtmleditctrlbasesetshowalltags"></a><a name="setshowalltags"></a>CHtmlEditCtrlBase::SetShowAllTags  
 會顯示這些圖像會顯示文件中的所有標記的位置。  
  
```  
HRESULT SetShowAllTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示這些圖像會顯示文件中的所有標記的位置。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWALLTAGS 命令 ID](https://msdn.microsoft.com/library/aa769948.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowareatagsa--chtmleditctrlbasesetshowareatags"></a><a name="setshowareatags"></a>CHtmlEditCtrlBase::SetShowAreaTags  
 顯示區域的所有標記的圖像。  
  
```  
HRESULT SetShowAreaTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示區域的所有標記的圖像。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWAREATAGS 命令 ID](https://msdn.microsoft.com/library/aa769949.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowbrtagsa--chtmleditctrlbasesetshowbrtags"></a><a name="setshowbrtags"></a>CHtmlEditCtrlBase::SetShowBRTags  
 顯示所有 br 標記的圖像。  
  
```  
HRESULT SetShowBRTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示所有 br 標記的圖像。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWWBRTAGS 命令 ID](https://msdn.microsoft.com/library/aa769956.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowcommenttagsa--chtmleditctrlbasesetshowcommenttags"></a><a name="setshowcommenttags"></a>CHtmlEditCtrlBase::SetShowCommentTags  
 顯示所有註解標記的圖像。  
  
```  
HRESULT SetShowCommentTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示所有註解標記的圖像。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWCOMMENTTAGS 命令 ID](https://msdn.microsoft.com/library/aa769950.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowmisctagsa--chtmleditctrlbasesetshowmisctags"></a><a name="setshowmisctags"></a>CHtmlEditCtrlBase::SetShowMiscTags  
 會顯示在 Microsoft Internet Explorer 4.0 中所顯示的所有標記。  
  
```  
HRESULT SetShowMiscTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示在 Microsoft Internet Explorer 4.0 中所顯示的所有標記。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWMISCTAGS 命令 ID](https://msdn.microsoft.com/library/aa769952.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowscripttagsa--chtmleditctrlbasesetshowscripttags"></a><a name="setshowscripttags"></a>CHtmlEditCtrlBase::SetShowScriptTags  
 顯示所有指令碼標記的圖像。  
  
```  
HRESULT SetShowScriptTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示所有指令碼標記的圖像。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWSCRIPTTAGS 命令 ID](https://msdn.microsoft.com/library/aa769953.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowstyletagsa--chtmleditctrlbasesetshowstyletags"></a><a name="setshowstyletags"></a>CHtmlEditCtrlBase::SetShowStyleTags  
 顯示樣式的所有標記的圖像。  
  
```  
HRESULT SetShowStyleTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示樣式的所有標記的圖像。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWSTYLETAGS 命令 ID](https://msdn.microsoft.com/library/aa769954.aspx) WebBrowser 控制項。  
  
##  <a name="a-namesetshowunknowntagsa--chtmleditctrlbasesetshowunknowntags"></a><a name="setshowunknowntags"></a>CHtmlEditCtrlBase::SetShowUnknownTags  
 顯示所有未知的標記的圖像。  
  
```  
HRESULT SetShowUnknownTags(bool bNewValue) const;  
```  
  
### <a name="parameters"></a>參數  
 `bNewValue`  
 如果為 true，則會顯示所有未知的標記的圖像。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM SHOWUNKNOWNTAGS 命令 ID](https://msdn.microsoft.com/library/aa769955.aspx) WebBrowser 控制項。  
  
##  <a name="a-nametextareaa--chtmleditctrlbasetextarea"></a><a name="textarea"></a>CHtmlEditCtrlBase::TextArea  
 會覆寫目前的選取範圍上的多行文字輸入的控制項。  
  
```  
HRESULT TextArea(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 多行文字方塊中輸入控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM 文字區域命令 ID](https://msdn.microsoft.com/library/aa769986.aspx) WebBrowser 控制項。  
  
##  <a name="a-nametextboxa--chtmleditctrlbasetextbox"></a><a name="textbox"></a>CHtmlEditCtrlBase::TextBox  
 會覆寫目前的選取範圍的文字控制項。  
  
```  
HRESULT TextBox(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 文字控制項的 ID。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM 文字方塊命令 ID](https://msdn.microsoft.com/library/aa769980.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameunbookmarka--chtmleditctrlbaseunbookmark"></a><a name="unbookmark"></a>CHtmlEditCtrlBase::UnBookmark  
 移除目前選取範圍中的任何書籤。  
  
```  
HRESULT UnBookmark() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM UNBOOKMARK 命令 ID](https://msdn.microsoft.com/library/aa770034.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameunderlinea--chtmleditctrlbaseunderline"></a><a name="underline"></a>CHtmlEditCtrlBase::Underline  
 切換目前的選取範圍之間加上底線，而且不加上底線。  
  
```  
HRESULT Underline() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM 加上底線的命令 ID](https://msdn.microsoft.com/library/aa770035.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameunlinka--chtmleditctrlbaseunlink"></a><a name="unlink"></a>CHtmlEditCtrlBase::Unlink  
 移除目前選取範圍中的任何超連結。  
  
```  
HRESULT Unlink() const;  
```  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM 取消連結的命令 ID](https://msdn.microsoft.com/library/aa770037.aspx) WebBrowser 控制項。  
  
##  <a name="a-nameunorderlista--chtmleditctrlbaseunorderlist"></a><a name="unorderlist"></a>CHtmlEditCtrlBase::UnorderList  
 切換目前的選取範圍的已排序的清單之間的一般格式的區塊。  
  
```  
HRESULT UnorderList(LPCTSTR szId = NULL) const;  
```  
  
### <a name="parameters"></a>參數  
 `szId`  
 未排序清單的識別碼。  
  
### <a name="return-value"></a>傳回值  
 傳回 S_OK，如果成功或失敗的錯誤 HRESULT。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[IDM UNORDERLIST 命令 ID](https://msdn.microsoft.com/library/aa769987.aspx) WebBrowser 控制項。  
  
## <a name="see-also"></a>另請參閱  
 [階層架構圖表](../../mfc/hierarchy-chart.md)   
 [HTMLEdit 範例](../../visual-cpp-samples.md)


