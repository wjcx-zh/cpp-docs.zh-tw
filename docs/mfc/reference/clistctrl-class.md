...
|[CListCtrl::Arrange](#arrange)|對齊方格上的專案。|
...
|[CListCtrl::DeleteAllItems](#deleteallitems)|從控制項中刪除所有專案。|
|[CListCtrl::DeleteColumn](#deletecolumn)|刪除清單視圖控制項中的資料行。|
|[CListCtrl::DeleteItem](#deleteitem)|從控制項中刪除專案。|
|[CListCtrl::DrawItem](#drawitem)|當主控描繪控制項的視覺外觀變更時呼叫。|
...
以下是`CListCtrl`類別的簡短總覽。 如需詳細的概念性討論，請參閱[使用 CListCtrl](../../mfc/using-clistctrl.md)和[控制項](../../mfc/controls-mfc.md)。
...
- 報表檢視
   ...
   每個專案都會出現在自己的行中，而其他資訊則會排列在右邊的欄位中。 最左邊的資料行包含小型圖示和標籤，而後續的資料行則包含應用程式所指定的子標題。 內嵌的標題控制項（類別[CHeaderCtrl](../../mfc/reference/cheaderctrl-class.md)）會執行這些資料行。 如需報表檢視中標題控制項和資料行的詳細資訊，請[參閱使用 CListCtrl：將資料行加入至控制項（報表檢視](../../mfc/adding-columns-to-the-control-report-view.md)）。

控制項目前清單視圖的樣式會決定目前的視圖。 如需這些樣式和其使用方式的詳細資訊[，請參閱使用 CListCtrl：變更清單控制項樣式](../../mfc/changing-list-control-styles.md)。

## <a name="extended-styles"></a>延伸樣式

除了標準清單樣式以外，類別`CListCtrl`也支援一組大型的擴充樣式，提供豐富的功能。 這項功能的一些範例包括：
...
- 虛擬清單視圖
   ...
   啟用時，可讓控制項支援最多 DWORD 專案。 藉由將管理專案資料的額外負荷放在應用程式上，即可達成此目的。 除了專案選取和焦點資訊以外，所有專案資訊都必須由應用程式管理。 如需詳細資訊， [請參閱使用 CListCtrl：虛擬清單控制項](../../mfc/virtual-list-controls.md)。
...
如需使用這些新擴充樣式的詳細資訊[，請參閱使用 CListCtrl：變更清單控制項樣式](../../mfc/changing-list-control-styles.md)。
...
類別`CListCtrl`提供數個用來插入、刪除、尋找及修改這些專案的函式。 如需詳細資訊，請參閱[CListCtrl：： GetItem](#getitem)、 [CListCtrl：： InsertItem](#insertitem)和[CListCtrl：： FindItem](#finditem)、[將專案新增至控制項](../adding-items-to-the-control.md)，以及[在清單控制項中進行滾動、排列、排序和尋找](../scrolling-arranging-sorting-and-finding-in-list-controls.md)。

根據預設，清單視圖控制項會負責儲存專案的圖示和文字屬性。 不過，除了這些專案類型之外，類別`CListCtrl`也支援「回呼專案」。 「回呼專案」是一種清單視圖專案，應用程式（而不是控制項）會儲存文字、圖示或兩者。 回呼遮罩是用來指定應用程式所提供的專案屬性（文字和/或圖示）。 如果應用程式使用回呼專案，它必須能夠視需要提供文字和/或圖示屬性。 當您的應用程式已經維護部分資訊時，回呼專案會很有説明。 如需詳細資訊， [請參閱使用 CListCtrl：回呼專案和回呼遮罩](../callback-items-and-the-callback-mask.md)。
...
根據預設，清單視圖控制項會終結在終結時指派給它的影像清單;不過，開發人員可以自訂此行為，方法是在不再使用每個影像清單時予以終結，如同應用程式所決定。 如需詳細資訊， [請參閱使用 CListCtrl：列出專案和影像清單](../list-items-and-image-lists.md)。
...
`CSize`物件，包含顯示專案所需的大約寬度和高度（以圖元為單位）。

### <a name="remarks"></a>備註

此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_ApproximateViewRect](/windows/win32/api/commctrl/nf-commctrl-listview_approximateviewrect)的行為。
...
- LVA_DEFAULT 會根據清單視圖的目前對齊樣式（預設值）對齊專案。

- LVA_SNAPTOGRID 會將所有圖示貼齊最接近的格線位置。
...
*rect*<br/>
指定清單控制項的大小和位置。 它可以`CRect`是物件或[矩形](/previous-versions/dd162897\(v=vs.85\))結構。

*pParentWnd*<br/>
指定清單控制項的父視窗，通常是`CDialog`。 不得為 Null。
...
您可以使用`CListCtrl`兩個步驟來建立。 首先，呼叫此函式，然後`Create`呼叫，它會建立清單視圖控制項，並將其`CListCtrl`附加至物件。
...
`CreateEx`使用*dwExStyle*所指定的擴充 Windows 樣式來建立控制項。 若要設定控制項特定的擴充樣式，請呼叫[SetExtendedStyle](#setextendedstyle)。 例如，使用`CreateEx`將這類樣式設定為 WS_EX_CONTEXTHELP，但使用`SetExtendedStyle`來將這類樣式設定為 LVS_EX_FULLROWSELECT。 如需詳細資訊，請參閱 Windows SDK 中的[擴充清單視圖樣式](/windows/win32/Controls/extended-list-view-styles)一文中所述的樣式。
...
[DRAWITEMSTRUCT 結構](/windows/win32/api/winuser/ns-winuser-drawitemstruct)的成員會定義要執行的繪圖`itemAction`動作。

根據預設，此成員函式不會執行任何工作。 覆寫這個成員函式，以執行主控描繪`CListCtrl`物件的繪圖。
...
這個方法會執行 Win32 宏[ListView_GetBkImage](/windows/win32/api/commctrl/nf-commctrl-listview_getbkimage)的行為，如 Windows SDK 中所述。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetCheckState](/windows/win32/api/commctrl/nf-commctrl-listview_getcheckstate)的行為。
...
*pColumn*<br/>
[LVCOLUMN](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw)結構的位址，指定用來抓取和接收資料行相關資訊的資訊。 `mask`成員會指定要取得的資料行屬性。 如果成員指定 LVCF_TEXT 值`pszText` ，成員就必須包含接收專案文字的緩衝區位址，而且該`cchTextMax`成員必須指定緩衝區的大小。 `mask`
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetColumnOrderArray](/windows/win32/api/commctrl/nf-commctrl-listview_getcolumnorderarray)的行為。
...
這個方法會傳送[LVM_GETEMPTYTEXT](/windows/win32/Controls/lvm-getemptytext)訊息，如 Windows SDK 中所述。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetExtendedListViewStyle](/windows/win32/api/commctrl/nf-commctrl-listview_getextendedlistviewstyle)的行為。
...
這個方法會傳送[LVM_GETFOCUSEDGROUP](/windows/win32/Controls/lvm-getfocusedgroup)訊息，如 Windows SDK 中所述。 如需詳細資訊，請參閱`state` [LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構成員的 LVGS_FOCUSED 值。
...
|---------------|-----------------|
|*iIndex*|在以零為基底的群組索引。|
|*pGroup*|脫銷[LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的指標，接收*iIndex*參數所指定之群組的相關資訊。<br /><br /> 呼叫端負責初始化[LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的成員。 將成員設定為結構的大小，並將成員的旗標`mask`設為，以指定要取得的資訊。 `cbSize`|
...
下列程式碼範例會定義用來`m_listCtrl`存取目前清單視圖控制項的變數。 下一個範例中會使用此變數。
...
|*iCoords*|在指定要抓取的矩形座標。 使用下列其中一個值：<br /><br /> -LVGGR_GROUP-整個展開群組的（預設）座標。<br />-LVGGR_HEADER-僅限標頭（折迭的群組）的座標。<br />-LVGGR_SUBSETLINK-僅限子集連結的座標（標記子集）。|
...
這個方法會傳送[LVM_GETGROUPRECT](/windows/win32/Controls/lvm-getgrouprect)訊息，如 Windows SDK 中所述。
...
|*dwMask*|在遮罩，指定要針對指定的群組抓取的狀態值。 如需詳細資訊，請`mask`參閱[LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的成員。|
...
傳回值是*dwMask*參數上位 and 運算的結果，以及代表目前清單視圖控制項之`state` [LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的成員值。

這個方法會傳送[LVM_GETGROUPSTATE](/windows/win32/Controls/lvm-getgroupstate)訊息，如 Windows SDK 中所述。 如需詳細資訊，請參閱[ListView_GetGroupState](/windows/win32/api/commctrl/nf-commctrl-listview_getgroupstate)宏。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetHeader](/windows/win32/api/commctrl/nf-commctrl-listview_getheader)的行為。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetHotCursor](/windows/win32/api/commctrl/nf-commctrl-listview_gethotcursor)的行為。 當游標通過任何清單視圖專案時，就會出現作用中游標，只有在啟用 [暫留選取] 時才會顯示。 藉由設定 LVS_EX_TRACKSELECT 擴充樣式來啟用暫留選取專案。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetHotItem](/windows/win32/api/commctrl/nf-commctrl-listview_gethotitem)的行為。 當啟用熱追蹤（和暫留選取）時，熱專案會定義為目前選取的專案。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetHoverTime](/windows/win32/api/commctrl/nf-commctrl-listview_gethovertime)的行為。
...
*nImageList*<br/>
值，指定要取出的影像清單。 它可以是下列其中一個值：

- 具有大型圖示的 LVSIL_NORMAL 影像清單。

- 具有小型圖示的 LVSIL_SMALL 影像清單。
...
如果成功，則傳回 TRUE，否則傳回 FALSE。 如果`cbSize` `LVINSERTMARK`結構成員中的大小不等於結構的實際大小，則會傳回 FALSE。
...
抓取與所指定`nItem`之專案相關聯的32位應用程式特定值。
...
這個值是`lParam` [LVITEM](/windows/win32/api/commctrl/ns-commctrl-lvitemw)結構的成員，如 Windows SDK 中所述。
...
這個方法會傳送[LVM_GETITEMINDEXRECT](/windows/win32/Controls/lvm-getitemindexrect)訊息，如 Windows SDK 中所述。 如需詳細資訊，請參閱[ListView_GetItemIndexRect 宏](/windows/win32/api/commctrl/nf-commctrl-listview_getitemindexrect)。
...
這個方法會傳送[LVM_GETITEMSPACING](/windows/win32/Controls/lvm-getitemspacing)訊息，如 Windows SDK 中所述。
...
專案的狀態是由[LVITEM](/windows/win32/api/commctrl/ns-commctrl-lvitemw)結構的`state`成員指定，如 Windows SDK 中所述。 當您指定或變更專案的狀態時， `stateMask`成員會指定您想要變更的狀態位。
...
- LVNI_ABOVE 會搜尋位於指定專案上方的專案。

- LVNI_ALL 會依索引（預設值）搜尋後續專案。

- LVNI_BELOW 會搜尋位於指定專案底下的專案。

- LVNI_TOLEFT 會搜尋指定專案左側的專案。

- LVNI_TORIGHT 會搜尋指定專案右邊的專案。
...
這個方法會傳送[LVM_GETNEXTITEMINDEX](/windows/win32/controls/lvm-getnextitemindex)訊息，如 Windows SDK 中所述。
...
*採購*<br/>
先前對`GetNextSelectedItem`或`GetFirstSelectedItemPosition`的呼叫所傳回之位置值的參考。 這個呼叫會將值更新為下一個位置。
...
如果您使用`GetNextSelectedItem`的呼叫來`GetFirstSelectedItemPosition`建立初始位置，則可以在正向反復專案迴圈中使用。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetNumberOfWorkAreas](/windows/win32/api/commctrl/nf-commctrl-listview_getnumberofworkareas)的行為。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetSelectionMark](/windows/win32/api/commctrl/nf-commctrl-listview_getselectionmark)的行為。
...
- LVIR_LABEL 會傳回整個專案的周框，包括圖示和標籤。 這與 LVIR_BOUNDS 完全相同。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetSubItemRect](/windows/win32/api/commctrl/nf-commctrl-listview_getsubitemrect)的行為。
...
此成員函式會執行 Win32 message [LVM_GETTOOLTIPS](/windows/win32/Controls/lvm-gettooltips)的行為，如 Windows SDK 中所述。 的 MFC 執行`GetToolTips` `CToolTipCtrl`會傳回物件，此物件是由清單控制項所使用，而不是工具提示控制項的控制碼。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_GetWorkAreas](/windows/win32/api/commctrl/nf-commctrl-listview_getworkareas)的行為。
...
*pFlags*<br/>
整數的指標，可接收測試結果的相關資訊。 請參閱 Windows SDK 中`flags` [LVHITTESTINFO](/windows/win32/api/commctrl/ns-commctrl-lvhittestinfo)結構成員的說明。
...
您可以測試結構`flag`成員的 LVHT_ONITEM 值，以判斷指定的位置是否在清單視圖專案上。 這個值是結構`flag`成員的 LVHT_ONITEMICON、LVHT_ONITEMLABEL 和 LVHT_ONITEMSTATEICON 值上的位 or 運算。
...
*lParam*<br/>
與專案相關聯的32位應用程式特定值。 如果指定這個參數，您就必須設定*nMask*屬性 LVIF_PARAM。
...
呼叫這個方法可能會導致 LVM_INSERTITEM 訊息傳送至您的控制項視窗。 控制項的相關訊息處理常式可能無法在某些條件下設定專案文字（例如，使用 LVS_OWNERDRAW 之類的視窗樣式）。 如需這些條件的詳細資訊，請參閱 Windows SDK 中的[LVM_INSERTITEM](/windows/win32/Controls/lvm-insertitem) 。
...
這個方法會傳送[LVM_ISITEMVISIBLE](/windows/win32/Controls/lvm-isitemvisible)訊息，如 Windows SDK 中所述。
...
這個方法會傳送[LVM_MAPIDTOINDEX](/windows/win32/controls/lvm-mapidtoindex)訊息，如 Windows SDK 中所述。
...
這個方法會傳送[LVM_MAPINDEXTOID](/windows/win32/Controls/lvm-mapindextoid)訊息，如 Windows SDK 中所述。
...
> [!NOTE]
>  由於`CListCtrl::SetBkImage`會使用 ole COM 功能，因此必須先初始化 ole 程式庫，才能`SetBkImage`使用。 當應用程式初始化時，最好將 COM 程式庫初始化，並在應用程式終止時將程式庫解除初始化。 這會自動在利用 ActiveX 技術、OLE Automation、OLE 連結/內嵌或 ODBC/DAO 作業的 MFC 應用程式中執行。
...
*pColumn*<br/>
[LVCOLUMN](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw)結構的位址，其中包含新的資料行屬性，如 Windows SDK 中所述。 結構的`mask`成員會指定要設定的資料行屬性。 如果成員指定 LVCF_TEXT 值，結構的`pszText`成員就是以 null 終止之字串的位址，而且會忽略結構的`cchTextMax`成員。 `mask`
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetColumnOrderArray](/windows/win32/api/commctrl/nf-commctrl-listview_setcolumnorderarray)的行為。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetExtendedListViewStyle](/windows/win32/api/commctrl/nf-commctrl-listview_setextendedlistviewstyle)的行為。
...
這個方法會傳送[LVM_SETGROUPINFO](/windows/win32/Controls/lvm-setgroupinfo)訊息，如 Windows SDK 中所述。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetHotCursor](/windows/win32/api/commctrl/nf-commctrl-listview_sethotcursor)的行為。

只有在啟用 [暫留選取] 時，才會顯示作用中游標，因為游標通過任何清單視圖專案。 藉由設定 LVS_EX_TRACKSELECT 擴充樣式來啟用暫留選取專案。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetHotItem](/windows/win32/api/commctrl/nf-commctrl-listview_sethotitem)的行為。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetHoverTime](/windows/win32/api/commctrl/nf-commctrl-listview_sethovertime)的行為。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetIconSpacing](/windows/win32/api/commctrl/nf-commctrl-listview_seticonspacing)的行為。
...
如果成功，則傳回 TRUE，否則傳回 FALSE。 如果`cbSize` `LVINSERTMARK`結構成員中的大小不等於結構的實際大小，或當插入點不適用於目前的視圖時，就會傳回 FALSE。
...
`iItem` `iSubItem`結構的和成員以及 nItem 和 nSubItem 參數會識別要設定其屬性的專案和子專案。 `LVITEM`

結構的成員和 nMask 參數會指定要設定的專案屬性： `mask` `LVITEM`

- LVIF_TEXT 成員或 lpszItem 參數是以 null 終止之字串的位址，則會忽略該`cchTextMax`成員。 `pszText`

- LVIF_STATE member 或*nStateMask*參數指定`state`要變更的專案狀態，而 member 或 nState 參數包含這些狀態的值。 `stateMask`
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetItemCount](/windows/win32/api/commctrl/nf-commctrl-listview_setitemcount)的行為。
...
- LVSICF_NOINVALIDATEALL 清單視圖控制項不會重新繪製，除非受影響的專案目前為 view。 這是預設值。
...
此成員函式會執行 Win32 宏[ListView_SetItemCountEx](/windows/win32/api/commctrl/nf-commctrl-listview_setitemcountex)的行為，如 Windows SDKand 中所述，應僅針對虛擬清單視圖呼叫。
...
這個方法會傳送[LVM_SETITEMINDEXSTATE](/windows/win32/Controls/lvm-setitemindexstate)訊息，如 Windows SDK 中所述。
...
如果已設定[LVS_EX_BORDERSELECT](/windows/win32/Controls/list-view-window-styles)擴充視窗樣式，則設定清單視圖控制項的框線色彩。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetSelectionMark](/windows/win32/api/commctrl/nf-commctrl-listview_setselectionmark)的行為。
...
此成員函式會執行 Win32 message [LVM_SETTOOLTIPS](/windows/win32/Controls/lvm-settooltips)的行為，如 Windows SDK 中所述。
...
*nWorkAreas*<br/>
*LpRect*所指向`RECT`陣列中的結構（或[CRect](../../atl-mfc-shared/reference/crect-class.md)物件）數目。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SetWorkAreas](/windows/win32/api/commctrl/nf-commctrl-listview_setworkareas)的行為。
...
這個方法會傳送[LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems)訊息，如 Windows SDK 中所述。
...
int CALLBACK CompareFunc(LPARAM lParam1,
    ...
    LPARAM lParamSort);
```
這則訊息與[LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems)類似，但傳遞至比較函數的資訊類型除外。 在[LVM_SORTITEMS](/windows/win32/Controls/lvm-sortitems)中， *lParam1*和*lParam2*是要比較之專案的值。 在[LVM_SORTITEMSEX](/windows/win32/Controls/lvm-sortitemsex)中， *lParam1*是要比較之第一個專案的目前索引，而*lParam2*是第二個專案的目前索引。 您可以傳送[LVM_GETITEMTEXT](/windows/win32/Controls/lvm-getitemtext)訊息來取得專案的詳細資訊。
...
這個方法會傳送[LVM_SORTITEMSEX](/windows/win32/Controls/lvm-sortitemsex)訊息，如 Windows SDK 中所述。
...
此成員函式會依照 Windows SDK 中的說明，實作用 Win32 宏[ListView_SubItemHitTest](/windows/win32/api/commctrl/nf-commctrl-listview_subitemhittest)的行為。
