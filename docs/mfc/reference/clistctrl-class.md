...
以下是類別的簡短總覽 `CListCtrl` 。 如需詳細的概念性討論，請參閱[使用 CListCtrl](../../mfc/using-clistctrl.md)和[控制項](../../mfc/controls-mfc.md)。
...
除了標準清單樣式以外，類別也 `CListCtrl` 支援一組大型的擴充樣式，提供豐富的功能。 這項功能的一些範例包括：
...
根據預設，清單視圖控制項會負責儲存專案的圖示和文字屬性。 不過，除了這些專案類型之外，類別也 `CListCtrl` 支援「回呼專案」。 「回呼專案」是一種清單視圖專案，應用程式（而不是控制項）會儲存文字、圖示或兩者。 回呼遮罩是用來指定應用程式所提供的專案屬性（文字和/或圖示）。 如果應用程式使用回呼專案，它必須能夠視需要提供文字和/或圖示屬性。 當您的應用程式已經維護部分資訊時，回呼專案會很有説明。 如需詳細資訊，請參閱[使用 CListCtrl：回呼專案和回呼遮罩](../callback-items-and-the-callback-mask.md)。
...
*各種*<br/>
指定清單控制項的大小和位置。 它可以是 `CRect` 物件或[矩形](/windows/win32/api/windef/ns-windef-rect)結構。
...
您可以使用 `CListCtrl` 兩個步驟來建立。 首先，呼叫此函式，然後呼叫 `Create` ，它會建立清單視圖控制項，並將其附加至 `CListCtrl` 物件。
...
建立控制項（子視窗），並將它與物件產生關聯 `CListCtrl` 。
...
`CreateEx`使用*dwExStyle*所指定的擴充 Windows 樣式來建立控制項。 若要設定控制項特定的擴充樣式，請呼叫[SetExtendedStyle](#setextendedstyle)。 例如，使用將 `CreateEx` 這類樣式設定為 WS_EX_CONTEXTHELP，但使用 `SetExtendedStyle` 來設定 LVS_EX_FULLROWSELECT 這類樣式。 如需詳細資訊，請參閱 Windows SDK 中的[擴充清單視圖樣式](/windows/win32/Controls/extended-list-view-styles)一文中所述的樣式。
...
*lpPoint*<br/>
[點](/windows/win32/api/windef/ns-windef-point)結構的位址，會接收影像左上角的起始位置（以視圖座標表示）。
...
`CImageList`物件是永久的，而且您必須在完成時將它刪除。 例如：
...
根據預設，此成員函式不會執行任何工作。 覆寫這個成員函式，以執行主控描繪物件的繪圖 `CListCtrl` 。
...
*pColumn*<br/>
[LVCOLUMN](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw)結構的位址，指定用來抓取和接收資料行相關資訊的資訊。 `mask`成員會指定要取得的資料行屬性。 如果 `mask` 成員指定 LVCF_TEXT 值， `pszText` 成員就必須包含接收專案文字的緩衝區位址，而且該成員 `cchTextMax` 必須指定緩衝區的大小。
...
這個方法會傳送[LVM_GETFOCUSEDGROUP](/windows/win32/Controls/lvm-getfocusedgroup)訊息，如 Windows SDK 所述。 如需詳細資訊，請參閱 `state` [LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構成員的 LVGS_FOCUSED 值。
...
|---------------|-----------------|
|*iIndex*|在以零為基底的群組索引。|
|*pGroup*|脫銷[LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的指標，接收*iIndex*參數所指定之群組的相關資訊。<br /><br /> 呼叫端負責初始化[LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的成員。 將 `cbSize` 成員設定為結構的大小，並將成員的旗標設為， `mask` 以指定要取得的資訊。|
...
|---------------|-----------------|
|*iGroupId*|在指定群組。|
|*lpRect*|[in、out][RECT](/windows/win32/api/windef/ns-windef-rect)結構的指標。 如果此方法成功，結構會接收*iGroupId*所指定之群組的矩形座標。|
...
呼叫端負責配置*pRect*參數所指向的[矩形](/windows/win32/api/windef/ns-windef-rect)結構。
...
|*dwMask*|在遮罩，指定要針對指定的群組抓取的狀態值。 如需詳細資訊，請參閱 `mask` [LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的成員。|
...
傳回值是*dwMask*參數上位 and 運算的結果，以及 `state` 代表目前清單視圖控制項之[LVGROUP](/windows/win32/api/commctrl/ns-commctrl-lvgroup)結構的成員值。
...
如果成功，則傳回 TRUE，否則傳回 FALSE。 如果結構成員中的大小 `cbSize` `LVINSERTMARK` 不等於結構的實際大小，則會傳回 FALSE。
...
|*pRect*|脫銷[RECT](/windows/win32/api/windef/ns-windef-rect)結構的指標，可接收子工作周框的相關資訊。<br /><br /> 呼叫端負責配置[RECT](/windows/win32/api/windef/ns-windef-rect)結構。 此參數不可以是 NULL。|
...
*lpPoint*<br/>
[點](/windows/win32/api/windef/ns-windef-point)結構的位址，接收此專案左上角的位置，以視圖座標表示。
...
*lpRect*<br/>
接收周框之[RECT](/windows/win32/api/windef/ns-windef-rect)結構的位址。
...
專案的狀態是由 `state` [LVITEM](/windows/win32/api/commctrl/ns-commctrl-lvitemw)結構的成員指定，如 Windows SDK 中所述。 當您指定或變更專案的狀態時， `stateMask` 成員會指定您想要變更的狀態位。
...
|參數|說明|
|---------------|-----------------|
|*pItemIndex*|[in、out][LVITEMINDEX](/windows/win32/api/commctrl/ns-commctrl-lvitemindex)結構的指標，描述搜尋開始的專案，或-1，尋找符合*nFlags*參數中旗標的第一個專案。<br /><br /> 如果此方法成功，結構會 `LVITEMINDEX` 描述搜尋找到的專案。|
...
*採購*<br/>
先前對或的呼叫所傳回之位置值的參考 `GetNextSelectedItem` `GetFirstSelectedItemPosition` 。 這個呼叫會將值更新為下一個位置。
...
`GetNextSelectedItem`如果您使用的呼叫來建立初始位置，則可以在正向反復專案迴圈中使用 `GetFirstSelectedItemPosition` 。
...
*lpPoint*<br/>
接收視圖來源之[點](/windows/win32/api/windef/ns-windef-point)結構的位址。
...
此成員函式會依照 Windows SDK 中的說明，實作用[LVM_GETTOOLTIPS](/windows/win32/Controls/lvm-gettooltips)Win32 訊息的行為。 的 MFC 執行 `GetToolTips` `CToolTipCtrl` 會傳回物件，此物件是由清單控制項所使用，而不是工具提示控制項的控制碼。
...
*lpRect*<br/>
[RECT](/windows/win32/api/windef/ns-windef-rect)結構的位址。
...
*pFlags*<br/>
整數的指標，可接收測試結果的相關資訊。 請參閱 `flags` Windows SDK 中[LVHITTESTINFO](/windows/win32/api/commctrl/ns-commctrl-lvhittestinfo)結構成員的說明。
...
您可以使用結構成員的 LVHT_ABOVE、LVHT_BELOW、LVHT_TOLEFT 和 LVHT_TORIGHT 值， `flag` 來決定是否要滾動清單視圖控制項的內容。 其中有兩個旗標可以合併，例如，如果位置在工作區的上方和左側。

您可以測試結構成員的 LVHT_ONITEM 值 `flag` ，以判斷指定的位置是否在清單視圖專案上。 這個值是結構成員的 LVHT_ONITEMICON、LVHT_ONITEMLABEL 和 LVHT_ONITEMSTATEICON 值的位 OR 運算 `flag` 。
...
*pPoint*<br/>
[點](/windows/win32/api/windef/ns-windef-point)結構的指標，其中包含相對於清單控制項工作區的點擊測試座標。
...
*pColumn*<br/>
[LVCOLUMN](/windows/win32/api/commctrl/ns-commctrl-lvcolumnw)結構的位址，其中包含新的資料行屬性，如 Windows SDK 中所述。 結構的 `mask` 成員會指定要設定的資料行屬性。 如果 `mask` 成員指定 LVCF_TEXT 值，結構的 `pszText` 成員就是以 null 終止之字串的位址，而且 `cchTextMax` 會忽略結構的成員。
...
如果成功，則傳回 TRUE，否則傳回 FALSE。 如果結構成員中的大小 `cbSize` `LVINSERTMARK` 不等於結構的實際大小，或當插入點不適用於目前的視圖時，就會傳回 FALSE。
...
*pItem*<br/>
[LVITEM](/windows/win32/api/commctrl/ns-commctrl-lvitemw)結構的位址，其中包含新的專案屬性，如 Windows SDK 中所述。 結構的 `iItem` 和 `iSubItem` 成員會識別專案或子項，而結構的成員會 `mask` 指定要設定的屬性。 如需成員的詳細資訊 `mask` ，請參閱**備註**。
...
`iItem`結構的和 `iSubItem` 成員 `LVITEM` 以及*nItem*和*nSubItem*參數會識別要設定其屬性的專案和子專案。

`mask`結構的成員 `LVITEM` 和*nMask*參數會指定要設定的專案屬性：

- LVIF_TEXT `pszText` 成員或*lpszItem*參數是以 null 終止之字串的位址，則 `cchTextMax` 會忽略該成員。

- LVIF_STATE `stateMask` member 或*nStateMask*參數會指定要變更的專案狀態，而 `state` member 或*nState*參數會包含這些狀態的值。
...
*pt*<br/>
[點](/windows/win32/api/windef/ns-windef-point)結構，指定專案左上角的新位置（以視圖座標表示）。
...
若不要使用工具提示，請在建立物件時指定 LVS_NOTOOLTIPS 樣式 `CListCtrl` 。
...
*nWorkAreas*<br/>
`RECT` *LpRect*所指向陣列中的結構（或[CRect](../../atl-mfc-shared/reference/crect-class.md)物件）數目。

*lpRect*<br/>
`RECT`結構（或物件）陣列的位址 `CRect` ，指定清單視圖控制項的新工作區域。 這些區域必須在用戶端座標中指定。 如果此參數為 Null，則工作區域將會設定為控制項的工作區。
