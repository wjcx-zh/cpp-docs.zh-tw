---
title: CMapWordToPtr 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
## <a name="members"></a>成員  
 成員函式`CMapWordToPtr`類別的成員函式類似[CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md)。 由於此相似性，您可以針對成員函式特性使用 `CMapStringToOb` 參考文件。 無論在何處看到`CObject`指標做為函式參數或傳回值，取代指標**void**。 無論在何處看到`CString`或是**const**指標**char**身為函式參數或傳回值，以取代文字。  
...
## <a name="remarks"></a>備註  
 ...
 當`CMapWordToPtr`物件被刪除，或當其項目會遭到移除，移除文字和指標。 不會移除指標所參考的實體。  
  
 如需詳細資訊`CMapWordToPtr`，請參閱文章[集合](../../mfc/collections.md)。  
