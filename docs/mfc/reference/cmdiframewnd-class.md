...
`LoadFrame`需要的引數數目少於 `Create` ，而是從資源中抓取大部分的預設值，包括框架的標題、圖示、快速鍵對應表和功能表。 若要由存取 `LoadFrame` ，所有這些資源都必須具有相同的資源識別碼（例如 IDR_MAINFRAME）。
...
呼叫 `Create` 或成員函式 `LoadFrame` ，以建立可見的 MDI 框架視窗。
