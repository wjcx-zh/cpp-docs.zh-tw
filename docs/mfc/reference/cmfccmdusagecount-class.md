...
|-|-|  
|名稱|描述|  
|[CMFCCmdUsageCount::AddCmd](#addcmd)|其中一個指定的命令相關聯的計數器遞增。|  
...
|[CMFCCmdUsageCount::Serialize](#serialize)|從封存讀取此物件，或將它寫入至封存。 (覆寫 [CObject::Serialize](../../mfc/reference/cobject-class.md#serialize)。)|  
...
|`m_nMinUsagePercentage`|經常使用命令的最小的使用量百分比。|  
|`m_nStartCount`|用來判斷此物件是否已收集追蹤資料的最小數量開始計數器。|  
|`m_nTotalUsage`|所有追蹤的命令計數。|  
  
### <a name="remarks"></a>備註  
 `CMFCCmdUsageCount`類別會將每個數值的 Windows 訊息識別項對應至 32 位元不帶正負號的整數的計數器。 `CMFCToolBar`您可以使用這個類別來顯示常用的工具列項目。 如需有關`CMFCToolBar`，請參閱[CMFCToolBar 類別](../../mfc/reference/cmfctoolbar-class.md)。  
  
 您可以保存`CMFCCmdUsageCount`類別執行程式之間的資料。 使用[CMFCCmdUsageCount::Serialize](#serialize)序列化類別成員資料的方法和[CMFCCmdUsageCount::SetOptions](#setoptions)方法，以設定共用的成員資料。  
...
### <a name="return-value"></a>傳回值  
 如果此物件已接收的最小追蹤資料，則為非零否則便是 0。  
  
### <a name="remarks"></a>備註  
 如果此方法會傳回非零值的總計數，`m_nTotalUsage`的所有追蹤的命令是等於或大於的初始計數`m_nStartCount`。 根據預設，架構會設定初始計數 0。 您可以使用覆寫此值[CMFCCmdUsageCount::SetOptions](#setoptions)方法。  
...
### <a name="remarks"></a>備註  
 這個方法會傳回 0，如果總計 命令使用方式， `m_nTotalUsage`，為 0。 否則，這個方法會傳回非零，如果指定的命令使用其中的百分比大於最小的百分比， `m_nMinUsagePercentage`。 根據預設，架構會設定為 5 的最小百分比。 您可以使用覆寫此值[CMFCCmdUsageCount::SetOptions](#setoptions)方法。 如果最小的百分比為 0，則這個方法會傳回非零，如果指定的命令計數大於 0。  
...
### <a name="remarks"></a>備註  
 呼叫此方法以清除所有項目命令計數對應 from `m_CmdUsage`，以及重設 [總計] 命令使用方式， `m_nTotalUsage`、 計數器為 0。  
