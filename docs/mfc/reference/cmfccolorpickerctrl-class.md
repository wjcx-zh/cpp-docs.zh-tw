---
title: CMFCColorPickerCtrl 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
2.  插入與新的按鈕控制項到您的對話方塊類別相關聯的成員變數。 然後變更 來自變數的型別`CButton`至`CMFCColorPickerCtrl`。  
  
3.  插入`WM_INITDIALOG`對話方塊類別的訊息處理常式。 在處理常式中設定類型、 調色盤和初始選取的色彩`CMFCColorPickerCtrl`控制項。  
