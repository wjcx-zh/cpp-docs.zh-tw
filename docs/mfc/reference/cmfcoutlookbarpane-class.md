---
title: CMFCOutlookBarPane 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
## <a name="example"></a>範例  
 下列範例示範如何使用各種方法`CMFCOutlookBarPane`類別。 此範例示範如何建立 Outlook 功能區窗格，啟用頁面捲軸模式，啟用停駐，並設定 Outlook 功能區的背景色彩。 此程式碼片段是一部分[Outlook 多重檢視範例](../../visual-cpp-samples.md)。  
...
### <a name="remarks"></a>備註  
 使用[CMFCOutlookBarPane::SetTextColor](#settextcolor)設定 Outlook 功能區的目前 （一般和已選取） 的文字色彩。 您可以藉由呼叫取得的預設文字色彩[GetSysColor](https://msdn.microsoft.com/library/windows/desktop/ms724371) COLOR_WINDOW 索引的函式。  
