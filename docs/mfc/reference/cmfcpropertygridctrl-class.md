...
## <a name="example"></a>範例  
 下列範例示範如何設定使用中的各種方法的屬性方格控制項物件`CMFCPropertyGridCtrl`類別。 此範例示範如何啟用此標題控制項、 啟用 [說明] 區域中，以及設定屬性方格控制項的外觀。 此範例也示範如何設定控制項的排序，控制項的字母模式及其屬性名稱，所包含的所有屬性以及如何都設定各種屬性方格控制項的項目自訂色彩。 這個範例是屬於[新控制項範例](../../visual-cpp-samples.md)。  
...
### <a name="parameters"></a>參數  
 ...
 [輸入] `bExpandParents`  
 `TRUE` 若要展開父項目，將指定的屬性設為可見的。否則， `FALSE`。 預設值為 `FALSE`。  
...
### <a name="parameters"></a>參數  
 ...
 [輸入] `bPropsOnly`  
 `TRUE` 若要測試只能在內容區域。`FALSE`測試*描述區域*如果指定的點不在內容區域中。 預設值是 `FALSE`。 如需描述區域的詳細資訊，請參閱 < 備註 >。  
...
### <a name="return-value"></a>傳回值  
 `TRUE` 如果架構重新調整大小作業; 繪製的名稱和值的資料行，`FALSE`如果架構在拖曳作業執行完成之後，會重繪資料行。  
...
|參數|描述|  
|---------------|-----------------|  
|[輸入] `pNewSel`|指標，新選取的屬性。|  
|[輸入] `pOldSel`|指標先前選取的屬性。|  
...
### <a name="parameters"></a>參數  
 [輸入] `bRedraw`  
 `TRUE` 重繪內容清單。否則， `FALSE`。 預設值是 `TRUE`。  
...
### <a name="parameters"></a>參數  
 ...
 [輸入] `clrGroupBackground`  
 屬性群組的背景色彩。  
  
 [輸入] `clrGroupText`  
 屬性群組的新的文字色彩。  
  
 [輸入] `clrDescriptionBackground`  
 描述區域的背景色彩。  
  
 [輸入] `clrDescriptionText`  
 描述區域中的文字色彩。  
