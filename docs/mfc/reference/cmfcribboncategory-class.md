...
|[CMFCRibbonCategory::GetLastVisibleElement](#getlastvisibleelement)|取得屬於功能區分類的最後一個可見項目|  
|[CMFCRibbonCategory::GetLargeImages](#getlargeimages)|傳回功能區類別使用的大型影像清單的參考。|  
...
|[CMFCRibbonCategory::GetTabColor](#gettabcolor)|傳回目前的功能區分類索引標籤的色彩。|  
...
|[CMFCRibbonCategory::IsVisible](#isvisible)|決定是否為可見的功能區分類。|  
...
|[CMFCRibbonCategory::OnKey](#onkey)|當使用者按下鍵盤 按鈕時由架構呼叫。|  
...
|[CMFCRibbonCategory::SetCollapseOrder](#setcollapseorder)|定義摺疊的順序的功能區面板會出現在功能區分類。|  
|[CMFCRibbonCategory::SetData](#setdata)|將使用者定義資料儲存在功能區分類。|  
...
## <a name="remarks"></a>備註  
 通常您會建立功能區分類間接呼叫[CMFCRibbonBar::AddCategory](../../mfc/reference/cmfcribbonbar-class.md#addcategory)，其傳回的指標，新建的功能區分類。 藉由呼叫類別新增面板[CMFCRibbonCategory::AddPanel](#addpanel)。  
  
 `CMFCRibbonTab`類別繪製功能區分類。 它衍生自[CMFCRibbonBaseElement 類別](../../mfc/reference/cmfcribbonbaseelement-class.md)。  
  
 下列範例會示範如何建立功能區分類，並加入工作面板。  
...
##  <a name="addhidden"></a>CMFCRibbonCategory::AddHidden  
 指定的功能區項目將會顯示在 [自訂] 對話方塊的功能區項目的陣列。  
...
### <a name="remarks"></a>備註  
 在 [自訂] 對話方塊上的功能區項目是您可以加入至快速存取工具列命令。  
...
### <a name="return-value"></a>傳回值  
 指向第一個可見的項目。可能是`NULL`如果類別沒有任何可見的項目。  
...
### <a name="return-value"></a>傳回值  
 指向最後一個可見的項目。可能是`NULL`如果類別沒有任何可見的項目。  
  
### <a name="remarks"></a>備註  
  
##  <a name="getmaxheight"></a>CMFCRibbonCategory::GetMaxHeight  
 擷取包含在功能區類別中的功能區面板的最大高度。  
...
### <a name="return-value"></a>傳回值  
 功能區類別中所包含的功能區面板的最大高度。  
  
### <a name="remarks"></a>備註  
 擷取的值包含功能區面板的頂端和底端邊緣的高度。  
...
##  <a name="gettexttopline"></a>CMFCRibbonCategory::GetTextTopLine  
 擷取上顯示大型影像的功能區按鈕的功能區分類中的文字的垂直位置。  
...
##  <a name="notifycontrolcommand"></a>CMFCRibbonCategory::NotifyControlCommand  
 將 WM_NOTIFY 命令訊息傳遞至所有`CMFCRibbonPanel`中的項目`CMFCRibbonCategory`之前的訊息處理。  
...
### <a name="return-value"></a>傳回值  
 傳回`TRUE`如果已處理訊息，或`FALSE`如果不是。  
  
### <a name="remarks"></a>備註  
  
##  <a name="oncancelmode"></a>CMFCRibbonCategory::OnCancelMode  
 會叫用 [取消] 5d; 模式中所有`CMFCRibbonPanel`的項目`CMFCRibbonCategory`。  
...
### <a name="remarks"></a>備註  
 依預設這個方法沒有任何作用。 覆寫這個方法，以繪製框線的快顯功能表。  
...
##  <a name="onlbuttondown"></a>CMFCRibbonCategory::OnLButtonDown  
 由架構呼叫以擷取在指定的點之下的功能區項目，當使用者按下滑鼠左鍵時。  
...
### <a name="remarks"></a>備註  
 這個方法會調整版面配置的所有功能區面板和功能區類別中所包含的功能區項目。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果在水平方向; 中的功能區分類移動否則`FALSE`。  
...
### <a name="remarks"></a>備註  
 程式庫會定義摺疊順序。 不過，您可以自訂此行為藉由提供指定的摺疊順序的索引清單中的類別。  
  
 當類別偵測到它已摺疊功能區面板時，它會尋找指定的清單中的下一個項目。 如果清單是空的或您沒有指定足夠的項目，類別目錄會使用內部演算法。  
  
 比方說，類別具有三個功能區面板和可以摺疊數次直到所有面板中完全摺疊的狀態。 您可以設定下列摺疊順序： 0，0，2，2。 在此情況下，類別目錄會摺疊面板 0 兩次，面板 2 兩倍。 索引為 1 的面板會保持展開。  
  
### <a name="example"></a>範例  
 下列範例示範如何使用`SetCollapseOrder`方法中的`CMFCRibbonCategory`類別。 此範例示範如何建構的陣列，摺疊順序，以及如何設定摺疊順序的功能區分類。  
...
### <a name="remarks"></a>備註  
 當使用者按下 Alt 鍵或 F10 鍵，會顯示按鍵提示。  
  
##  <a name="setname"></a>CMFCRibbonCategory::SetName  
 將名稱和 keytip 指派給功能區類別中。  
