---
title: CMFCRibbonCategory 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
|[CMFCRibbonCategory::GetContextID](#getcontextid)|傳回功能區分類的內容識別碼。|
|[CMFCRibbonCategory::GetData](#getdata)|傳回功能區分類相關聯的使用者定義的資料。|
...
|[CMFCRibbonCategory::GetFirstVisibleElement](#getfirstvisibleelement)|取得第一個可見項目屬於的功能區分類。|
...
|[CMFCRibbonCategory::GetLastVisibleElement](#getlastvisibleelement)|取得最後一個可見項目屬於的功能區分類|
|[CMFCRibbonCategory::GetLargeImages](#getlargeimages)|傳回功能區分類使用的大型映像清單的參考。|
|[CMFCRibbonCategory::GetMaxHeight](#getmaxheight)||
|[CMFCRibbonCategory::GetName](#getname)||
|[CMFCRibbonCategory::GetPanel](#getpanel)|傳回位於指定索引處的功能區面板的指標。|
|[CMFCRibbonCategory::GetPanelCount](#getpanelcount)|傳回功能區類別中的功能區面板的數目。|
|[CMFCRibbonCategory::GetPanelFromPoint](#getpanelfrompoint)||
|[CMFCRibbonCategory::GetPanelIndex](#getpanelindex)|傳回指定的功能區面板的索引。|
...
|[CMFCRibbonCategory::GetTabColor](#gettabcolor)|傳回目前的功能區的分類索引標籤的色彩。|
|[CMFCRibbonCategory::GetTabRect](#gettabrect)||
|[CMFCRibbonCategory::GetTextTopLine](#gettexttopline)||
|[CMFCRibbonCategory::GetVisibleElements](#getvisibleelements)|取得所有可見的項目屬於的功能區分類。|
...
|[CMFCRibbonCategory::IsWindows7Look](#iswindows7look)|表示父功能區是否有 Windows 7 樣式外觀 （小型矩形應用程式按鈕）|
...
|[CMFCRibbonCategory::OnKey](#onkey)|當使用者按下 [鍵盤] 按鈕時由架構呼叫。|
...
|[CMFCRibbonCategory::SetCollapseOrder](#setcollapseorder)|定義摺疊的順序會出現在功能區分類的功能區面板。|
...
一般而言，您建立功能區分類間接藉由呼叫[CMFCRibbonBar::AddCategory](../../mfc/reference/cmfcribbonbar-class.md#addcategory)，這會傳回新建立的功能區類別的指標。 將面板新增至類別目錄的藉由呼叫[CMFCRibbonCategory::AddPanel](#addpanel)。

`CMFCRibbonTab`類別繪製功能區分類。 它衍生自[CMFCRibbonBaseElement 類別](../../mfc/reference/cmfcribbonbaseelement-class.md)。

下列範例示範如何建立功能區分類和面板新增至它。
...
將會顯示在 [自訂] 對話方塊中的功能區項目的陣列中指定的功能區項目。
...
在 [自訂] 對話方塊上的功能區項目是您可以將它新增至快速存取工具列命令。
...
*hIcon*<br/>
[in]新的功能區面板的預設圖示的控制代碼。

*pRTI*<br/>
[in]自訂功能區面板的執行階段類別資訊指標。

### <a name="return-value"></a>傳回值

如果方法成功; 的新功能區面板的指標如果不建立 [面板] 中，否則為 NULL。

### <a name="remarks"></a>備註

如果您想要建立自訂功能區面板，您必須指定在其執行階段類別資訊*pRTI*。 自訂功能區面板類別必須衍生自`CMFCRibbonPanel`類別。

空間不足，無法顯示功能區項目時，會顯示功能區面板的預設圖示。
...
[in]在 功能區類別的功能區項目所使用的小型影像的影像清單的資源識別碼。

*uiLargeImagesResID*<br/>
[in]功能區類別中的功能區項目所使用的大型映像的映像清單的資源識別碼。

*sizeSmallImage*<br/>
[in]預設的功能區中的項目功能區分類的小型影像的大小。

*sizeLargeImage*<br/>
[in]預設的功能區中的項目功能區分類的大型映像的大小。
...
*bVisibleOnly*<br/>
[in]要搜尋中包含的快速存取功能區項目，則為 TRUE若要排除在搜尋中的快速存取功能區項目，則為 FALSE。
...
內容識別碼為 0，如果功能區分類不是內容功能區分類。

##  <a name="getdata"></a>  CMFCRibbonCategory::GetData

擷取的功能區分類相關聯的使用者定義的資料。
...
*arElements*<br/>
[in、 out]若要參考[CArray](../../mfc/reference/carray-class.md)的功能區項目。

### <a name="remarks"></a>備註

專為快速存取工具列上的功能區項目會包含在陣列中。

##  <a name="getelementsbyid"></a>  CMFCRibbonCategory::GetElementsByID

擷取指定之命令識別碼相關聯的所有功能區項目
...
指向第一個可見的項目;如果類別沒有任何可見的項目時，就可能為 NULL。
...
*bIsLargeImage*<br/>
[in]適用於大型影像清單中的映像數目小型影像清單中的映像數目，則為 FALSE。
...
*bIsLargeImage*<br/>
[in]適用於大型影像; 的大小小型影像的大小，則為 FALSE。
...
*lstItems*<br/>
[out]功能區中的項目功能區分類的命令 Id 的清單。

*bHiddenOnly*<br/>
[in]若要排除在功能區分類中; 在功能區面板上顯示的功能區項目，則為 TRUE如果為 false，則包含在功能區類別中所有的功能區項目。
...
指向最後一個可見的項目;如果類別沒有任何可見的項目時，就可能為 NULL。

### <a name="remarks"></a>備註

##  <a name="getmaxheight"></a>  CMFCRibbonCategory::GetMaxHeight

擷取包含在功能區分類中的功能區面板的最大高度。
...
擷取的垂直位置上顯示大型影像的功能區按鈕的功能區分類中的文字。
...
指向先前反白顯示功能區面板;如果叫用此方法時，會反白不顯示任何功能區面板，否則為 NULL。

### <a name="remarks"></a>備註

如需有關反白顯示功能區面板的詳細資訊，請參閱[CMFCRibbonPanel::Highlight](../../mfc/reference/cmfcribbonpanel-class.md#highlight)。
...
[in]若要測試功能區面板標題;，則為 TRUE若要排除的功能區面板的標題，則為 FALSE。
...
如果某個點落在功能區分類的左或向右捲動按鈕，讓指標回到該按鈕。
...
如果*點*之內的週框矩形左邊或向右捲動按鈕的功能區分類的、 讓指標回到該按鈕，或，否則會傳回 NULL。

### <a name="remarks"></a>備註

##  <a name="isactive"></a>  CMFCRibbonCategory::IsActive

指出功能區分類是否為作用中的類別目錄位於功能區列。
...
如果功能區分類是作用中的類別;，則為 TRUE。否則為 FALSE。
...
表示父功能區是否有 Windows 7，看起來 （小型矩形應用程式按鈕）。
...
如果父功能區看起來; 的 Windows 7，則為 TRUE。否則為 FALSE。

### <a name="remarks"></a>備註

##  <a name="notifycontrolcommand"></a>  CMFCRibbonCategory::NotifyControlCommand

會提供所有的 WM_NOTIFY 命令訊息`CMFCRibbonPanel`中的項目`CMFCRibbonCategory`之前處理訊息的。
...
*bAccelerator*<br/>
[in]否則源自加速器或 FALSE 的則為 TRUE，如果這個命令。
...
*bIsLargeImage*<br/>
[in]如果影像大小相當大;，則為 TRUE。如果影像是較小，則為 FALSE。

*nImageIndex*<br/>
[in]中的映像陣列中的功能區分類包含影像的以零起始的索引。

*bCenter*<br/>
[in]TRUE 表示置中的顯示矩形; 中的映像要繪製影像的顯示矩形左上角，則為 FALSE。
...
依預設這個方法沒有任何作用。 覆寫這個方法來繪製框線的快顯功能表。
...
由架構呼叫以擷取功能區下的項目指定的點，當使用者按下滑鼠左的按鈕。
...
當使用者放開滑鼠左的按鈕，並在指標位於功能區分類，由架構呼叫。
...
這個方法會調整所有功能區面板並在功能區類別中所包含的功能區項目的版面的配置。
...
如果水平方向移動的功能區分類，則為 TRUE。否則為 FALSE。

### <a name="remarks"></a>備註

##  <a name="onupdatecmdui"></a>  CMFCRibbonCategory::OnUpdateCmdUI

呼叫`OnUpdateCmdUI`成員函式中每個`CMFCRibbonPanel`的項目`CMFCRibbonCategory`啟用或停用它們的使用者介面項目。
...
*pCmdUI*<br/>
[in]指標`CMFCRibbonCmdUI`物件，指定使用者介面項目為啟用、 哪些是要停用。

*pTarget*<br/>
[in]啟用或停用的使用者介面項目會控制視窗的指標。
...
*nIndex*<br/>
[in]要移除的面板的索引編號。 藉由呼叫取得[CMFCRibbonCategory::GetPanelIndex](#getpanelindex)方法。

*bDelete*<br/>
[in]若要刪除的面板物件從記憶體中; TRUE若要移除的面板物件而非加以刪除，則為 FALSE。
...
*arCollapseOrder*<br/>
[in]指定的摺疊的順序。 陣列包含的功能區面板之以零起始的索引。

### <a name="remarks"></a>備註

程式庫定義摺疊的順序。 不過，您可以自訂此行為，藉由提供類別，指定的摺疊順序清單中的索引。
...
下列範例示範如何使用`SetCollapseOrder`方法中的`CMFCRibbonCategory`類別。 此範例示範如何建構一個陣列，以摺疊的順序，以及如何設定摺疊順序的功能區分類。
