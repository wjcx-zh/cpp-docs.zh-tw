---
title: CMFCRibbonGallery 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
### <a name="parameters"></a>參數  
 ...
 [in]*nIconsNum*  
 指定群組中的圖示數目。 僅針對自訂 （擁有者繪製），應該指定這個參數群組。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*bOnTop*  
 若要指定功能區圖庫; 之前，應插入項目，則為 TRUE否則為 FALSE。  
...
### <a name="parameters"></a>參數  
 [in]*bEnable*  
 若要啟用功能表; 調整大小時，則為 TRUE否則為 FALSE。  
  
 [in]*bVertcalOnly*  
 TRUE 表示指定的資源庫可以垂直調整大小只;指定可以調整垂直和水平大小的資源庫，則為 FALSE。  
...
### <a name="parameters"></a>參數  
 [in]*bSet*  
 若要以下拉式選單按鈕; 顯示功能區圖庫，則為 TRUE在功能區上直接顯示功能區圖庫的內容，則為 FALSE。  
...
### <a name="parameters"></a>參數  
 [in]*imagesPalette*  
 指定映像清單，其中包含出現在資源庫上的圖示。  
  
 [in]*uiImagesPaletteResID*  
 指定映像清單，其中包含出現在資源庫圖示的資源識別碼。  
...
### <a name="parameters"></a>參數  
 [in]*nID*  
 指定當使用者選取主機庫項目，會將 WM_COMMAND 訊息中傳送的命令識別碼。  
