...
|[CMFCRibbonGallery::SetPaletteID](#setpaletteid)|定義已選取主機庫項目時，會將 WM_COMMAND 訊息中傳送的命令 ID。|  
...
## <a name="remarks"></a>備註  
 組件庫按鈕行為就像一般功能表按鈕一樣，不同之處在於它會顯示主機庫，當使用者開啟它。 當您選取主機庫項目時，架構會傳送以及按鈕的命令 ID 的 WM_COMMAND 訊息。 當您處理訊息時，您應該呼叫[CMFCRibbonGallery::GetLastSelectedItem](#getlastselecteditem)來判斷哪一個項目已從資源庫中選取。  
...
### <a name="return-value"></a>傳回值  
 當使用者在功能區圖庫中選取任何項目時，程式庫會傳送以及開啟功能區圖庫的功能表按鈕的命令 ID 的 WM_COMMAND 訊息。  
...
##  <a name="setpaletteid"></a>  CMFCRibbonGallery::SetPaletteID  
 定義當使用者選取主機庫項目，會將 WM_COMMAND 訊息中傳送的命令 ID。  
