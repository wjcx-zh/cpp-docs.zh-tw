...
|[CMFCRibbonPanel::FindByID](#findbyid)|傳回指定的命令識別碼所識別的項目。|  
|[CMFCRibbonPanel::GetCaptionHeight](#getcaptionheight)||  
|[CMFCRibbonPanel::GetCount](#getcount)|傳回功能區面板中的項目數目。|  
|[CMFCRibbonPanel::GetData](#getdata)|傳回與 [面板] 中相關聯的使用者定義資料。|  
...
|[CMFCRibbonPanel::GetGalleryRect](#getgalleryrect)|傳回的主機庫項目的週框。|  
...
|[CMFCRibbonPanel::IsCenterColumnVert](#iscentercolumnvert)|是否所有 panel 元素應該垂直置都中 （對齊），以指定資料行。|  
...
|[CMFCRibbonPanel::MakeGalleryItemVisible](#makegalleryitemvisible)|捲動以顯示指定的功能區項目組件庫。|  
...
|[CMFCRibbonPanel::Replace](#replace)|有一個項目取代另一個根據它們的個別索引值。|  
|[CMFCRibbonPanel::ReplaceByID](#replacebyid)|有一個項目取代另一個指定的命令 id。|  
|[CMFCRibbonPanel::SetCenterColumnVert](#setcentercolumnvert)|排序資料行的垂直對齊項目 [面板]。|  
|[CMFCRibbonPanel::SetData](#setdata)|將使用者定義的資料與功能區面板。|  
|[CMFCRibbonPanel::SetElementMenu](#setelementmenu)|將快顯功能表指派給項目，具有給定的命令識別碼。|  
|[CMFCRibbonPanel::SetElementRTC](#setelementrtc)|加入至功能區面板提供的執行階段類別資訊所指定的功能區項目。|  
|[CMFCRibbonPanel::SetElementRTCByID](#setelementrtcbyid)|加入至功能區面板提供的執行階段類別資訊所指定的功能區項目。|  
|[CMFCRibbonPanel::SetFocused](#setfocused)|指定的功能區項目設定焦點。|  
...
## <a name="remarks"></a>備註  
 功能區面板是您建立功能區分類的相關工作的邏輯群組。 為功能區變更的大小，面板版面配置會自動調整顯示盡可能多的項目。  
  
 您可以取得功能區面板藉由呼叫包含在功能區分類[CMFCRibbonCategory::GetPanel](../../mfc/reference/cmfcribboncategory-class.md#getpanel)方法。  
  
## <a name="example"></a>範例  
 下列範例示範如何設定`CMFCRibbonPanel`物件使用中的各種方法`CMFCRibbonPanel`類別。 此範例示範如何設定顯示功能區面板的鍵盤快速鍵、 垂直對齊面板中的元素，依資料行，並啟用資料行對齊方式。 此程式碼片段是部分[MS Office 2007 示範範例](../../visual-cpp-samples.md)。  
...
### <a name="return-value"></a>傳回值  
 具有下拉; 其快顯功能表功能區項目的指標否則`NULL`如果沒有任何功能區項目具有下拉其快顯功能表。  
...
### <a name="remarks"></a>備註  
 這個方法一律會傳回 `FALSE`。 覆寫這個方法，以指出功能區面板是主要的功能區面板。  
...
### <a name="remarks"></a>備註  
 如果您想要將自訂項目 （例如色彩按鈕） 加入至功能區面板，您必須指定自訂的項目執行階段類別資訊。 功能區儲存這項資訊、 建立自訂的項目，並取代現有的項目，位於所指定的命令識別碼。 然後，它會傳回指標至新建立的項目。  
