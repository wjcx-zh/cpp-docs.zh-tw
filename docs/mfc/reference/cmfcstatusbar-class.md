...
### <a name="remarks"></a>備註  
 如果啟用處理雙點選 [狀態] 列，則 Windows 傳送 WM_COMMAND 通知資源識別碼以及每次使用者按兩下狀態列窗格上狀態列的擁有者。  
...
### <a name="remarks"></a>備註  
 當 [狀態] 列會失效時，則會將其標示為重繪作業。 Windows 重新繪製時`UpdateWindow`方法會傳送至 WM_PAINT 訊息`OnPaint`方法。  
