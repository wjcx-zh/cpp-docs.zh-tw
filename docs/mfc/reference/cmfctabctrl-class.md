...
|名稱|描述|  
|----------|-----------------|  
|[CMFCTabCtrl::ActivateMDITab](#activatemditab)|顯示目前的索引標籤控制項的指定之索引標籤，並將焦點設在該索引標籤。|  
|[CMFCTabCtrl::AllowDestroyEmptyTabbedPane](#allowdestroyemptytabbedpane)||  
|[CMFCTabCtrl::AutoSizeWindow](#autosizewindow)|指定是否要調整大小工作區的所有索引標籤控制項視窗的索引標籤控制項變更的使用者介面項目時，framework 就是。|  
...
|[CMFCTabCtrl::EnableActiveTabCloseButton](#enableactivetabclosebutton)|顯示或隱藏 關閉 按鈕 ( **X**) 使用中 索引標籤上。|  
...
|[CMFCTabCtrl::EnableTabDocumentsMenu](#enabletabdocumentsmenu)|取代捲動按鈕可開啟功能表的索引標籤式視窗的視窗索引標籤的兩個按鈕。|  
|[CMFCTabCtrl::EnsureVisible](#ensurevisible)|請確定索引標籤會出現。|  
|[CMFCTabCtrl::GetDocumentIcon](#getdocumenticon)|擷取索引標籤式視窗的快顯功能表中的索引標籤相關聯的符號。|  
...
|[CMFCTabCtrl::GetScrollBar](#getscrollbar)|擷取相關聯的索引標籤控制項的捲軸列物件的指標。|  
|[CMFCTabCtrl::GetTabArea](#gettabarea)|擷取索引標籤的標籤區域的頂端或底部的索引標籤控制項目的週框。 (覆寫[CMFCBaseTabCtrl::GetTabArea](../../mfc/reference/cmfcbasetabctrl-class.md#gettabarea)。)|  
...
|[CMFCTabCtrl::GetTabsRect](#gettabsrect)|擷取範圍的目前索引標籤控制項索引標籤區域的矩形。 (覆寫[CMFCBaseTabCtrl::GetTabsRect](../../mfc/reference/cmfcbasetabctrl-class.md#gettabsrect)。)|  
|`CMFCTabCtrl::GetThisClass`|由架構用來取得指向[CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md)與此類別類型相關聯的物件。|  
|[CMFCTabCtrl::GetWndArea](#getwndarea)|擷取目前的索引標籤控制項的工作區的界限。|  
|[CMFCTabCtrl::HideActiveWindowHorzScrollBar](#hideactivewindowhorzscrollbar)|水平捲軸中，如果隱藏任何使用中視窗。|  
|[CMFCTabCtrl::HideInactiveWindow](#hideinactivewindow)|指定要顯示非作用中 索引標籤控制項視窗的架構。|  
|[CMFCTabCtrl::HideNoTabs](#hidenotabs)|啟用或停用任何可見的索引標籤是否繪製索引標籤區域。|  
...
|[CMFCTabCtrl::IsActiveTabBoldFont](#isactivetabboldfont)|指出是否使用中的索引標籤的文字會顯示使用粗體字型。|  
|[CMFCTabCtrl::IsActiveTabCloseButton](#isactivetabclosebutton)|指出是否 [關閉] 按鈕 ( **X**) 會顯示在使用中的索引標籤或索引標籤區域的右上角。|  
|[CMFCTabCtrl::IsDrawFrame](#isdrawframe)|指出索引標籤式的視窗是否繪製框架矩形以圍繞內嵌的窗格。|  
|[CMFCTabCtrl::IsFlatFrame](#isflatframe)|指出索引標籤區域周圍的框架是平面或 3D。|  
|[CMFCTabCtrl::IsFlatTab](#isflattab)|指出是否單層或不在目前的索引標籤控制項中索引標籤的外觀。|  
|[CMFCTabCtrl::IsLeftRightRounded](#isleftrightrounded)|指出是否要四捨五入的左側和右側，目前的索引標籤控制項中的索引標籤的外觀。|  
|[CMFCTabCtrl::IsMDITabGroup](#ismditabgroup)|指出目前的索引標籤控制項是否包含在工作區的多重文件介面視窗。|  
|[CMFCTabCtrl::IsOneNoteStyle](#isonenotestyle)|指出是否以 Microsoft onenote 樣式顯示目前的索引標籤控制項。|  
|`CMFCTabCtrl::IsPtInTabArea`|決定一個點是否在索引標籤區域的內部。 (覆寫[CMFCBaseTabCtrl::IsPtInTabArea](../../mfc/reference/cmfcbasetabctrl-class.md#isptintabarea)。)|  
|[CMFCTabCtrl::IsSharedScroll](#issharedscroll)|指出目前的索引標籤控制項是否有捲軸可以捲動其 索引標籤，為群組。|  
|[CMFCTabCtrl::IsTabDocumentsMenu](#istabdocumentsmenu)|指出索引標籤控制項是否顯示捲軸按鈕或顯示索引標籤式視窗的功能表按鈕。|  
|[CMFCTabCtrl::IsVS2005Style](#isvs2005style)|指出是否會顯示索引標籤中的 Visual Studio.NET 2005年樣式。|  
|[CMFCTabCtrl::ModifyTabStyle](#modifytabstyle)|指定目前的索引標籤控制項中的索引標籤的外觀。|  
|`CMFCTabCtrl::MoveTab`|將索引標籤移至另一個索引標籤的位置。 (覆寫[CMFCBaseTabCtrl::MoveTab](../../mfc/reference/cmfcbasetabctrl-class.md#movetab)。)|  
|[CMFCTabCtrl::OnDragEnter](#ondragenter)|資料指標第一次拖曳至索引標籤控制項視窗時，由架構呼叫。|  
...
|[CMFCTabCtrl::OnShowTabDocumentsMenu](#onshowtabdocumentsmenu)|顯示索引標籤式視窗的快顯功能表中，等到使用者選取索引標籤，並將選取的索引標籤設為作用中的索引標籤。|  
|`CMFCTabCtrl::PreTranslateMessage`|將視窗訊息轉譯分派至之前[TranslateMessage](http://msdn.microsoft.com/library/windows/desktop/ms644955)和[DispatchMessage](http://msdn.microsoft.com/library/windows/desktop/ms644934) Windows 函式。 (覆寫[CMFCBaseTabCtrl::PreTranslateMessage](../../mfc/reference/cmfcbasetabctrl-class.md#pretranslatemessage)。)|  
...
|[CMFCTabCtrl::SetActiveInMDITabGroup](#setactiveinmditabgroup)|將目前的索引標籤的索引標籤控制項設定為多個文件介面 索引標籤群組中的 作用中 索引標籤。|  
...
|[CMFCTabCtrl::SetActiveTabBoldFont](#setactivetabboldfont)|啟用或停用的粗體字作用中 索引標籤上使用。|  
|[CMFCTabCtrl::SetDrawFrame](#setdrawframe)|啟用或停用內嵌的列 drawinga 框矩形。|  
|[CMFCTabCtrl::SetFlatFrame](#setflatframe)|指定是否要繪製以平面或 3D 框架 索引標籤區域周圍。|  
|[CMFCTabCtrl::SetImageList](#setimagelist)|指定的映像清單。 (覆寫[CMFCBaseTabCtrl::SetImageList](../../mfc/reference/cmfcbasetabctrl-class.md#setimagelist)。)|  
|[CMFCTabCtrl::SetResizeMode](#setresizemode)|指定如何在目前的索引標籤控制項可以調整大小並再重新顯示控制項。|  
|[CMFCTabCtrl::SetTabMaxWidth](#settabmaxwidth)|指定在索引標籤式視窗的最大的索引標籤寬度。|  
|[CMFCTabCtrl::StopResize](#stopresize)|終止目前的調整大小作業 索引標籤控制項上。|  
|`CMFCTabCtrl::SwapTabs`|交換成對的索引標籤。 (覆寫[CMFCBaseTabCtrl::SwapTabs](../../mfc/reference/cmfcbasetabctrl-class.md#swaptabs)。)|  
|[CMFCTabCtrl::SynchronizeScrollBar](#synchronizescrollbar)|繪製水平捲軸上顯示一般索引標籤的索引標籤控制項。|  
  
### <a name="data-members"></a>資料成員  
  
|名稱|描述|  
|----------|-----------------|  
|[CMFCTabCtrl::m_bEnableActivate](#m_benableactivate)|現用檢視表可防止失去焦點時插入並啟用新的索引標籤。|  
  
## <a name="remarks"></a>備註  
 `CMFCTabCtrl`類別支援：  
  
-   索引標籤控制項的樣式，包括共用水平捲軸以 3D，一般模式，以及一般模式。  
...
-   中斷連結的索引標籤式的視窗。  
 ...
 `CMFCTabCtrl`類別可以使用對話方塊中，但適用於應用程式使用停駐控制列像[!INCLUDE[ofprexcel](../../mfc/reference/includes/ofprexcel_md.md)]和[!INCLUDE[vsprvs](../../assembler/masm/includes/vsprvs_md.md)]。 如需詳細資訊，請參閱[CDockablePane 類別](../../mfc/reference/cdockablepane-class.md)。  
...
3.  使用[cbasetabbedpane:: Addtab](../../mfc/reference/cbasetabbedpane-class.md#addtab)或[cmfcbasetabctrl:: Inserttab](../../mfc/reference/cmfcbasetabctrl-class.md#inserttab)將新的索引標籤。  
  
4.  呼叫[CBasePane::EnableDocking](../../mfc/reference/cbasepane-class.md#enabledocking)使目前的停駐索引標籤控制項可以停駐於主框架視窗。  
  
5.  呼叫[CFrameWndEx::DockPane](../../mfc/reference/cframewndex-class.md#dockpane)停駐於主框架的索引標籤式的視窗。  
  
 如需如何建立為停駐控制列的索引標籤式的視窗的範例，請參閱[CTabbedPane 類別](../../mfc/reference/ctabbedpane-class.md)。 若要使用`CMFCTabCtrl`做為非停駐控制項中，建立`CMFCTabCtrl`物件，然後呼叫[CMFCTabCtrl::Create](#create)。  
...
## <a name="example"></a>範例  
 下列範例示範如何使用各種方法的`CMFCTabCtrl`用來設定類別`CMFCTabCtrl`物件。 此範例說明如何加入索引標籤，在使用中的索引標籤上顯示 [關閉] 按鈕、 啟用可編輯的索引標籤，並顯示索引標籤式的視窗標籤的快顯功能表。 這個範例是屬於[狀態集合範例](../../visual-cpp-samples.md)。  
...
##  <a name="activatemditab"></a>CMFCTabCtrl::ActivateMDITab  
 顯示目前的索引標籤控制項的指定之索引標籤，並將焦點設在該索引標籤。  
...
##  <a name="autosizewindow"></a>CMFCTabCtrl::AutoSizeWindow  
 指定是否要調整大小工作區的所有索引標籤控制項視窗的索引標籤控制項變更的使用者介面項目時，framework 就是。  
...
|樣式|描述|  
|-----------|-----------------|  
|STYLE_3D|建立索引標籤控制項有 3d 外觀。|  
|STYLE_FLAT|一般索引標籤建立索引標籤控制項。|  
|STYLE_FLAT_SHARED_HORZ_SCROLL|建立具有一般索引標籤和捲軸可以捲動的索引標籤，如果父視窗裁剪 索引標籤控制項。|  
|STYLE_3D_ONENOTE|Microsoft onenote 樣式建立的索引標籤控制項。|  
|STYLE_3D_VS2005|建立的索引標籤控制項中的 Microsoft Visual Studio 2005 樣式。|  
...
|位置|描述|  
|--------------|-----------------|  
|LOCATION_BOTTOM|索引標籤位於索引標籤控制項的底部。|  
...
### <a name="remarks"></a>備註  
 當您變更索引標籤的標籤時，會呼叫這個方法。這個方法有一半的目前索引標籤高度，由 「 左邊和右邊指定的矩形洩氣 」 和 「 洩氣 」 的頂端和底部一個單位。  
  
##  <a name="enableactivetabclosebutton"></a>CMFCTabCtrl::EnableActiveTabCloseButton  
 顯示或隱藏 關閉 按鈕 ( **X**) 使用中 索引標籤上。  
...
##  <a name="enabletabdocumentsmenu"></a>CMFCTabCtrl::EnableTabDocumentsMenu  
 切換至捲動視窗索引標籤中使用兩個按鈕的使用者介面和顯示索引標籤式視窗的快顯功能表的介面。  
...
### <a name="remarks"></a>備註  
 當使用者按一下索引標籤時，架構會顯示對應的索引標籤式的視窗。 如果索引標籤為可見，而不需要變更它的位置開啟索引標籤式的視窗。 如果使用者從快顯功能表中選取文件和對應的索引標籤式的視窗已關閉螢幕，索引標籤式的視窗就會變成第一個索引標籤。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果登錄成功。`FALSE`如果`iTab`參數索引不正確。  
  
### <a name="remarks"></a>備註  
 使用這個方法，以確保指定的索引標籤會顯示。 在必要時，會捲動索引標籤控制項。  
  
##  <a name="getdocumenticon"></a>CMFCTabCtrl::GetDocumentIcon  
 擷取索引標籤式視窗的快顯功能表中的索引標籤相關聯的映像。  
...
### <a name="return-value"></a>傳回值  
 此索引標籤控制項中的索引標籤以零為起始的索引。  
  
### <a name="remarks"></a>備註  
 Microsoft OneNote 樣式顯示索引標籤控制項時，才使用這個方法。 使用[CMFCTabCtrl::IsOneNoteStyle](#isonenotestyle)方法，以判斷樣式。  
...
### <a name="return-value"></a>傳回值  
 其中一個`CMFCTabCtrl::ResizeMode`列舉值，指定如何在索引標籤控制項可以調整大小。 如需可能值的清單，請參閱的 < 備註 > 一節[CMFCTabCtrl::SetResizeMode](#setresizemode)方法。  
  
##  <a name="getscrollbar"></a>CMFCTabCtrl::GetScrollBar  
 擷取相關聯的索引標籤控制項的捲軸列物件的指標。  
...
### <a name="return-value"></a>傳回值  
 捲軸物件、 指標或`NULL`如果索引標籤控制項不是使用`STYLE_FLAT_SHARED_HORZ_SCROLL`樣式。  
  
### <a name="remarks"></a>備註  
 您可以使用這個方法來存取索引標籤控制項的內嵌的捲軸。 捲軸列物件會建立只有當索引標籤控制項具有`STYLE_FLAT_SHARED_HORZ_SCROLL`樣式。  
  
##  <a name="gettabarea"></a>CMFCTabCtrl::GetTabArea  
 擷取索引標籤的標籤區域的頂端或底部的索引標籤控制項目的週框。  
...
### <a name="return-value"></a>傳回值  
 最大的索引標籤上，單位為像素寬度。 如果傳回的值為 0，索引標籤寬度沒有限制。  
  
### <a name="remarks"></a>備註  
 使用[CMFCTabCtrl::SetTabMaxWidth](#settabmaxwidth)方法來設定最大的索引標籤的寬度。  
...
### <a name="remarks"></a>備註  
 若要防止此索引標籤控制項閃爍的使用者 索引標籤控制項頁面之間切換時，使用這個方法。  
  
##  <a name="hideinactivewindow"></a>CMFCTabCtrl::HideInactiveWindow  
 指定是否架構會顯示非作用中 索引標籤控制項視窗。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果目前的索引標籤索引標籤控制項的 MDI 索引標籤群組; 中的 [作用中] 索引標籤否則， `FALSE`。  
  
### <a name="remarks"></a>備註  
 您可以將多個文件視窗組織成任一個垂直或水平索引標籤群組，並輕鬆地從一個索引標籤群組隨機文件到另一個。  
  
##  <a name="isactivetabboldfont"></a>CMFCTabCtrl::IsActiveTabBoldFont  
 指出是否使用中的索引標籤的文字會顯示使用粗體字型。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果使用中的索引標籤會顯示使用粗體顯示的字型。否則， `FALSE`。  
...
##  <a name="isactivetabclosebutton"></a>CMFCTabCtrl::IsActiveTabCloseButton  
 指出是否 [關閉] 按鈕 ( **X**) 會顯示在使用中的索引標籤上，或索引標籤區域右上角。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果 [關閉] 按鈕會顯示在 [作用中] 索引標籤。`FALSE`如果 [關閉] 按鈕會顯示在索引標籤區域的右上角。  
  
### <a name="remarks"></a>備註  
  
##  <a name="isdrawframe"></a>CMFCTabCtrl::IsDrawFrame  
 指出索引標籤式的視窗是否繪製框架矩形以圍繞內嵌的窗格。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果繪製框矩形。否則， `FALSE`。  
  
### <a name="remarks"></a>備註  
 使用[CMFCTabCtrl::SetDrawFrame](#setdrawframe)方法啟用或停用繪製框矩形。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果索引標籤區域周圍的框架是平面;`FALSE`如果框架為 3d。  
  
### <a name="remarks"></a>備註  
 使用[CMFCTabCtrl::SetFlatFrame](#setflatframe)方法，以變更繪製畫面格的方式。  
  
##  <a name="isflattab"></a>CMFCTabCtrl::IsFlatTab  
 指出是否單層或不在目前的索引標籤控制項中索引標籤的外觀。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果目前的索引標籤控制項中索引標籤的外觀是一般;否則， `FALSE`。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果目前的索引標籤控制項在 MDI 工作區視窗;否則， `FALSE`。  
  
##  <a name="isonenotestyle"></a>CMFCTabCtrl::IsOneNoteStyle  
 指出是否以 Microsoft onenote 樣式顯示目前的索引標籤控制項。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果 Microsoft OneNote; 樣式顯示索引標籤控制項否則， `FALSE`。  
  
##  <a name="issharedscroll"></a>CMFCTabCtrl::IsSharedScroll  
 指出目前的索引標籤控制項是否有捲軸可以捲動其 索引標籤，為群組。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`如果索引標籤式的視窗會捲動，即使用快顯功能表的索引標籤式的視窗標籤。`FALSE`如果索引標籤式的視窗會捲動，即使用向前和向後捲動按鈕。  
  
### <a name="remarks"></a>備註  
 使用[CMFCTabCtrl::EnableTabDocumentsMenu](#enabletabdocumentsmenu)方法，以指定的方法捲動的索引標籤式視窗。  
  
##  <a name="isvs2005style"></a>CMFCTabCtrl::IsVS2005Style  
 指出索引標籤是否使用 Visual Studio 2005 樣式的繪製。  
...
### <a name="remarks"></a>備註  
 通常，由新的索引標籤式視窗來取得焦點，當插入並設為作用中 索引標籤。 設定`CMFCTabCtrl::m_bEnableActivate`成員變數`FALSE`保留原始的焦點。 預設值是 `TRUE`。  
...
|名稱|描述|  
|----------|-----------------|  
|STYLE_3D|顯示有圓角的 3d、 矩形的索引標籤。|  
|STYLE_3D_ONENOTE|顯示 3d 索引標籤具有垂直一邊與斜體的一端，而且，具有圓角。|  
|STYLE_3D_ROUNDED|顯示具有傾斜側邊和圓角的 3d 索引標籤。|  
|STYLE_3D_ROUNDED_SCROLL|顯示具有傾斜側邊和圓角的 3d 索引標籤。 如果有多個索引標籤數多於可以顯示在相同的時間，架構會顯示下拉式箭號，然後讓成為作用中的索引標籤的功能表。|  
|STYLE_3D_SCROLLED|顯示 3d、 矩形的索引標籤。 如果有多個索引標籤數多於可以顯示在相同的時間，架構會顯示下拉式箭號，然後讓成為作用中的索引標籤的功能表。|  
|STYLE_3D_VS2005|三維，顯示四捨五入有一端斜體和垂直一端的索引標籤。|  
|STYLE_FLAT|顯示具有傾斜左側和右側的二維索引標籤。|  
|STYLE_FLAT_SHARED_HORZ_SCROLL|顯示二維索引標籤。 如果有多個索引標籤數多於可以顯示在相同的時間，架構會顯示端點索引標籤區域的捲動箭號。|  
...
### <a name="return-value"></a>傳回值  
 一律`DROPEFFECT_NONE`，表示拖放目標無法接受資料。  
  
### <a name="remarks"></a>備註  
 使用這個方法支援拖放作業。 覆寫這個方法以實作您自己自訂的行為。  
...
##  <a name="onshowtabdocumentsmenu"></a>CMFCTabCtrl::OnShowTabDocumentsMenu  
 顯示索引標籤式視窗的快顯功能表中，等到使用者選取索引標籤，並將選取的索引標籤設為作用中的索引標籤。  
...
##  <a name="setactiveinmditabgroup"></a>CMFCTabCtrl::SetActiveInMDITabGroup  
 將目前的索引標籤的索引標籤控制項設定為多個文件介面 索引標籤群組中的 作用中 索引標籤。  
...
### <a name="remarks"></a>備註  
 ...
 使用[cmfcbasetabctrl::](../../mfc/reference/cmfcbasetabctrl-class.md#addtab)  索引標籤控制項，指定要顯示之影像的索引會繼承的方法。  
  
##  <a name="setresizemode"></a>CMFCTabCtrl::SetResizeMode  
 指定如何在目前的索引標籤控制項可以調整大小並再重新顯示控制項。  
...
|RESIZE_VERT|垂直但不是水平索引標籤控制項可以調整大小。|  
|RESIZE_HORIZ|索引標籤控制項可以調整大小，以水平方式，但不是會以垂直方式。|  
...
### <a name="remarks"></a>備註  
 您可以使用這個方法，在索引標籤式視窗中的每個索引標籤的寬度限制。 這個方法是適用於索引標籤有很長的標籤。 [CMFCTabCtrl](../../mfc/reference/cmfctabctrl-class.md)類別建構函式會初始化為 0，這實際表示寬度不受限制的最大的索引標籤寬度。  
...
### <a name="remarks"></a>備註  
 這個方法會影響只顯示一般索引標籤的索引標籤控制項。 捲軸會同時影響所有索引標籤。  
