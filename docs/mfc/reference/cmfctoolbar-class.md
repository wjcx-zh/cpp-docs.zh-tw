...
ms.locfileid: "78869995"
---
# <a name="cmfctoolbar-class"></a>CMFCToolBar 類別

`CMFCToolBar` 類別與[CToolBar 類別](../../mfc/reference/ctoolbar-class.md)類似，但會提供使用者介面功能的額外支援。 這包括一般工具列、含作用中影像的工具列、大圖示、頁面巡覽區按鈕、鎖定工具列、Rebar 控制項、影像下方文字、背景影像和索引標籤式工具列。 `CMFCToolBar` 類別的內建支援也包括工具列和功能表的使用者自訂、工具列和功能表之間的拖放、下拉式方塊按鈕、編輯方塊按鈕、色彩選擇器和縮合按鈕。
...
|[CMFCToolBar：： CreateEx](#createex)|建立使用其他樣式選項的 `CMFCToolBar` 物件，例如大型圖示。|
|[CMFCToolBar：:D eactivate](#deactivate)|停用工具列。|
...
|[CMFCToolBar：： FromHandlePermanent](#fromhandlepermanent)|抓取包含給定視窗控制碼之 `CMFCToolBar` 物件的指標。|
...
|[CMFCToolBar：： GetButton](#getbutton)|傳回具有指定的工具列按鈕索引之 `CMFCToolBarButton` 物件的指標。|
...
|[CMFCToolBar：： GetCustomizeButton](#getcustomizebutton)|抓取與工具列相關聯之 `CMFCCustomizeButton` 物件的指標。|
...
|[CMFCToolBar：:D oPaint](#dopaint)|重新繪製工具列。|
|[CMFCToolBar：:D rawButton](#drawbutton)|重新繪製工具列按鈕。|
|[CMFCToolBar：:D rawSeparator](#drawseparator)|在工具列上重新繪製分隔符號。|
...
若要將 `CMFCToolBar` 物件併入您的應用程式中，請遵循下列步驟：

1. 將 `CMFCToolBar` 物件加入主框架視窗。
...
若要插入特殊按鈕（例如下拉式方塊或下拉工具列），請在父資源中保留 [虛設] 按鈕，並使用[CMFCToolBar：： ReplaceButton](#replacebutton)取代執行時間的 [虛擬] 按鈕。 如需詳細資訊，請參閱[逐步解說：將控制項放在工具列上](../walkthrough-putting-controls-on-toolbars.md)。
...
覆寫這個方法，在您從 `CMFCToolbar`衍生的類別中提供您自己的動態配置。
...
如果工具列的父系不是 `CMFCReBar` 物件，則[CMFCToolBar：： AdjustLayout](#adjustlayout)方法會呼叫這個方法來重新計算大小。
...
覆寫衍生自 `CMFCToolBar` 之物件中的這個方法，並在您不想讓使用者決定文字標籤是否顯示在影像下的工具列按鈕時傳回 FALSE。
...
如果*iIndex*大於0，這個方法會忽略工具列上索引小於*iIndex*的任何按鈕。
...
這個方法會建立控制列，並將它附加至工具列。 它會建立具有 TBSTYLE_FLAT 樣式的控制列。 如果您想要不同的控制列樣式，請呼叫[CMFCToolBar：： CreateEx](#createex) 。
...
當您想要提供特定樣式時，請呼叫這個方法，而不是[CMFCToolBar：： Create](#create) 。 例如，將*dwCtrlStyle*設定為 TBSTYLE_FLAT |TBSTYLE_TRANSPARENT 建立類似 Internet Explorer 4 所使用工具列的工具列。

### <a name="example"></a>範例

下列範例示範如何使用 `CMFCToolBar` 類別的 `CreateEx` 方法。 此程式碼片段是[IE 示範範例](../../overview/visual-cpp-samples.md)的一部分。
...
覆寫這個方法，以自訂衍生自 `CMFCToolBar`之物件的外觀。
...
如果文字標籤已啟用，工具列上的所有按鈕都會放大，以提供空間讓標籤顯示在影像下方。 [自訂] 對話方塊在 [**工具列**] 頁面上有 [**顯示文字標籤**] 核取方塊。 當使用者選取工具列並核用此選項時，架構會為選取的工具列呼叫 `EnableTextLabels`。 您可以從[CMFCToolBar：： AllowChangeTextLabels](#allowchangetextlabels)傳回 FALSE，以停用衍生自[CMFCToolBar](../../mfc/reference/cmfctoolbar-class.md)之物件的核取方塊。

##  <a name="fromhandlepermanent"></a>CMFCToolBar：： FromHandlePermanent

抓取包含給定視窗控制碼之 `CMFCToolBar` 物件的指標。
...
包含指定的視窗控制碼之 `CMFCToolBar` 物件的指標; 如果沒有對應的 `CMFCToolBar` 物件存在，則為 Null。

### <a name="remarks"></a>備註

這個共用方法會檢查應用程式中的每個工具列是否包含指定的視窗控制碼 `CMFCToolBar` 物件。
...
[CObList 類別](../../mfc/reference/coblist-class.md)物件的 const 參考，其中包含 `CMFCToolBar` 物件的集合。
...
如果指定索引處的按鈕不存在，架構會將*nID*和*nStyle*設定為0，並在方法傳回時，將*iImage*設為-1。
...
抓取與工具列相關聯之 `CMFCCustomizeButton` 物件的指標。
...
與工具列相關聯之 `CMFCCustomizeButton` 物件的指標。

### <a name="remarks"></a>備註

這個方法會抓取出現在工具列結尾處的 [**自訂**] 按鈕。 使用[CMFCToolBar：： EnableCustomizeButton](#enablecustomizebutton)方法，將 [**自訂**] 按鈕新增至您的工具列。

您可以呼叫[CMFCToolBar：： IsExistCustomizeButton](#isexistcustomizebutton)方法，以判斷工具列是否包含有效的 `CMFCCustomizeButton` 物件。
...
如果工具列按鈕具有鍵盤焦點，則會反白顯示。 如果工具列按鈕在此應用程式中是熱追蹤的，工具列按鈕也會反白顯示（如需詳細資訊，請參閱[CMFCToolBar：： GetHotBorder](#gethotborder)和[CMFCToolBar：： SetHotBorder](#sethotborder)），而當沒有工具列按鈕或功能表項目具有鍵盤焦點時，滑鼠就會指向它。
...
當 `CMFCToolBar` 控制項收到輸入焦點時，它會將控制碼儲存到遺失焦點的視窗，以便稍後還原。
...
*lpRect*<br/>
脫銷`CRect` 物件的指標，這個物件會接收影像周框矩形的座標。

### <a name="remarks"></a>備註

如果指定索引處的按鈕不存在， *lpRect*點會設定為0的 `CRect` 物件。

### <a name="example"></a>範例

下列範例示範如何使用 `CMFCToolBar` 類別的 `GetItemRect` 方法。 此程式碼片段是[IE 示範範例](../../overview/visual-cpp-samples.md)的一部分。
...
`CSize` 結構，指定鎖定的工具列影像大小，如果工具列未鎖定，則為空的 `CSize` 結構。

### <a name="remarks"></a>備註

鎖定的影像是當使用者無法自訂工具列時，架構所使用之一般工具列按鈕影像的版本。

如果工具列未鎖定，這個方法會傳回具有零寬度和零高度的 `CSize` 結構。 如果工具列未鎖定，這個方法也會在 Debug 組建中產生判斷提示失敗。 如需鎖定工具列的詳細資訊，請參閱[CMFCToolBar：： IsLocked](#islocked)。
...
存在於所提供索引處之 `CMFCToolBarButton` 物件的指標，如果沒有這類物件存在，則為 Null。

### <a name="remarks"></a>備註

當此架構更新與工具列按鈕相關聯的工作區時，會呼叫這個方法。 它會呼叫[CWnd：： InvalidateRect](../../mfc/reference/cwnd-class.md#invalidaterect)方法，其中包含存在於所提供索引處之 `CMFCToolBarButton` 物件的用戶端矩形。
...
當下列一或多個條件發生時，`IsCommandRarelyUsed` 方法會傳回 FALSE：
...
下列範例示範如何在 `CMFCToolBar` 類別中使用 `LoadToolBar` 方法。 此程式碼片段是[IE 示範範例](../../overview/visual-cpp-samples.md)的一部分。
...
當您未指定一組大型影像時，此架構會使用此資料成員。 例如，如果您只提供大小為16x16 的小型影像集合，而且想要讓大型影像具有大小24x24，請將此資料成員設定為1.5。
...
預設實作不做任何動作。 當工具列具有虛擬按鈕時，覆寫衍生自 `CMFCToolBar` 的類別 `OnReset`，當工具列回到其原始狀態時必須加以取代。
...
當工具列按鈕的工具提示即將顯示時，架構會呼叫這個方法。 如果 `OnUserToolTip` 傳回 TRUE，則架構會顯示工具提示，其中包含*strTTText*中 `OnUserToolTip` 所傳回的文字。 否則，工具提示會包含按鈕文字。

覆寫 `OnUserToolTip` 以自訂工具列按鈕的工具提示。 預設的實值會呼叫[CMFCToolBar：： OnUserToolTip](#onusertooltip)以取得工具提示文字。
...
這個方法會藉由呼叫[CWnd：:P ostmessage](../../mfc/reference/cwnd-class.md#postmessage)並傳遞指定按鈕的命令 ID 作為*wParam*參數，將 WM_COMMAND 訊息張貼至擁有工具列的視窗中。
...
下列範例示範如何在 `CMFCToolBar` 類別中使用 `ReplaceButton` 方法。 此程式碼片段是[IE 示範範例](../../overview/visual-cpp-samples.md)的一部分。
...
如果*nStyle*是 TBBS_DISABLED，這個方法會移除 TBBS_PRESSED 的樣式，因為使用者無法按一下 [已停用] 按鈕。
...
如果*nMinUsagePercentage*等於或大於100，則為 FALSE;否則為 TRUE。
...
如果方法成功，則為非零;如果指定的 `CMFCToolBarImages` 物件無效，或者影像大小與工具列的預設影像大小不同，則為0。

### <a name="remarks"></a>備註

架構會使用使用者定義的影像來繪製由使用者自訂的工具列按鈕。 *PUserImages*所指定的影像清單會在應用程式中的所有工具列之間共用。

如果指定的 `CMFCToolBarImages` 物件無效，或其影像大小與工具列的預設影像大小不同，則這個方法會在 Debug 組建中產生判斷提示失敗。
