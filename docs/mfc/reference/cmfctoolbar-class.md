...
2.  當您處理主框架視窗的 WM_CREATE 訊息時，呼叫[CMFCToolBar::Create](#create)或是[CMFCToolBar::CreateEx](#createex)建立工具列，並指定其樣式。  
  
3.  呼叫[CBasePane::EnableDocking](../../mfc/reference/cbasepane-class.md#enabledocking)指定停駐樣式。  
...
### <a name="parameters"></a>參數  
 [in]*bEnable*  
 布林值，指定是否要將非使用中的映像變暗。 如果此參數為 TRUE，會以灰色顯示非作用中的映像。 否則，非作用中的映像未呈現灰色。  
  
 [in]*nGrayImagePercentage*  
 指定非使用中的映像的明亮度百分比。 如果*bEnable*為 FALSE，則會忽略此值。  
  
 [in]*bRedrawAllToolbars*  
 布林值，指定是否要在應用程式來重繪所有工具列。 如果此參數為 TRUE，則這個方法會重新繪製所有工具列。  
...
### <a name="parameters"></a>參數  
 [in]*bStretch*  
 如果為 true，則延展工具列的父框架的大小。  
  
 [in]*bHorz*  
 TRUE 表示引導工具列的水平空間。調整成工具列垂直，則為 FALSE。  
...
### <a name="parameters"></a>參數  
 [in]*bVertDock*  
 TRUE 表示指定的工具列停駐垂直;指定的工具列是否停駐水平，則為 FALSE。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*bHorz*  
 如果分隔符號是水平，FALSE 如果分隔符號是垂直的則為 TRUE。  
...
### <a name="parameters"></a>參數  
 [in]*bEnable*  
 若要停用快速自訂 FALSE 啟用快速自訂項目，則為 TRUE。  
...
### <a name="parameters"></a>參數  
 [in]*bEnable*  
 若要啟用命令反映;，則為 TRUE若要停用命令反映，則為 FALSE。  
...
### <a name="parameters"></a>參數  
 ...
 [out]*listButtons*  
 參考[CObList 類別](../../mfc/reference/coblist-class.md)接收的工具列按鈕清單物件。  
...
### <a name="parameters"></a>參數  
 [in]*iInsertAt*  
 指定要插入的分隔符號，在以零為起始的位置。 這個參數必須是大於 0。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*封鎖*  
 布林值，指定與否，是否要鎖定工具列。 如果這個參數是 *，則為 TRUE*，工具列已被鎖定。 否則工具列不會鎖定。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*params*  
 參考`CMFCToolBarInfo`物件，其中包含工具列影像的資源識別碼。  
  
 [in]*封鎖*  
 布林值，指定與否，是否要鎖定工具列。 如果此參數為 TRUE，則會鎖定工具列。 否則工具列不會鎖定。  
...
### <a name="parameters"></a>參數  
 [in]*iHot*  
 指定的工具列按鈕，則索引或如果沒有 [工具列] 按鈕已選取為-1。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*球*  
 布林值，指定是否要取代所有具有所指定的命令 ID 的按鈕*uiCmd*。 如果此參數為 TRUE，會取代所有具有指定的命令 ID 的按鈕。 否則，會取代第一個按鈕。  
...
### <a name="parameters"></a>參數  
 [in]*lpIDArray*  
 若要插入的按鈕的命令 Id 的陣列的指標。  
  
 [in]*nIDCount*  
 中的項目數*lpIDArray*。  
  
 [in]*bRemapImages*  
 布林值，指定是否要插入的按鈕相關聯的現有按鈕映像。 如果此參數為 TRUE，則會重新對應的映像。  
...
### <a name="parameters"></a>參數  
 [in]*nStartCount*  
 指定必須執行數次的命令，然後在架構顯示只有基本] 和 [最近使用過的命令。  
  
 [in]*nMinUsagePercentage*  
 必須執行命令，才會被視為最近使用的命令的時間百分比。  
...
### <a name="parameters"></a>參數  
 [in]*bSet*  
 布林值，指定是否要啟用或停用自訂模式。 將此參數設為 true 以啟用自訂模式或 FALSE 來停用它。  
...
### <a name="parameters"></a>參數  
 ...
 布林值，指定如何顯示無法使用的按鈕。 如果此參數為 TRUE 時，framework 變暗的按鈕。 否則，架構會使用按鈕無法使用的映像集合。  
...
### <a name="parameters"></a>參數  
 [in]*bShowHotBorder*  
 布林值，指定是否要熱追蹤的工具列按鈕。 如果此參數為 TRUE 時，工具列熱追蹤其按鈕。 否則工具列不會不熱追蹤其按鈕。  
...
### <a name="parameters"></a>參數  
 [in]*bLargeIcons*  
 布林值，指定要使用的圖示。 如果此參數為 TRUE，則架構會顯示大圖示。 否則，架構會顯示規則的圖示。  
...
### <a name="parameters"></a>參數  
 [in]*bPermament*  
 布林值，指定使用者是否可以關閉工具列。 如果此參數為 TRUE，則使用者無法關閉工具列。 否則，使用者可以關閉工具列。  
...
### <a name="parameters"></a>參數  
 [in]*bValue*  
 如果此參數為 TRUE 時，父框架會將命令傳送至工具列。 否則，擁有者會將命令傳送至工具列。  
...
### <a name="parameters"></a>參數  
 [in]*bValue*  
 如果此參數為 TRUE，則架構會顯示工具提示。 否則，framework 會隱藏工具提示。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*bShowText*  
 如果此參數為 TRUE，則架構會顯示文字標籤。 否則，framework 會隱藏文字標籤。  
  
 [in]*bShowImage*  
 如果此參數為 TRUE，則架構會顯示工具列按鈕影像。 否則，framework 會隱藏工具列按鈕影像。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*bVert*  
 如果為 TRUE，垂直伸展窗格。 如果為 FALSE，水平伸展窗格。  
