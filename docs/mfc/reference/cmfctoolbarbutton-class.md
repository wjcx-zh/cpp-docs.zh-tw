---
title: CMFCToolBarButton 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
### <a name="parameters"></a>參數  
 ...
 [in]*bUserButton*  
 布林值，判斷是否為使用者定義的按鈕。 如果此參數為 TRUE 時，按鈕是使用者定義的。 否則，從資源載入按鈕影像。  
  
 [in]*封鎖*  
 布林值，決定是否可以自訂按鈕。 如果此參數為 TRUE，則無法自訂按鈕。 否則，可以自訂按鈕。  
...
### <a name="parameters"></a>參數  
 ...
 [in]*bSelected*  
 指定是否已選取 按鈕。 如果此參數為 TRUE 時，會選取 [] 按鈕。 如果此參數為 FALSE，未選取 按鈕。  
...
### <a name="parameters"></a>參數  
 [in]*srcItem*  
 A`COleDataSource`儲存拖放作業的狀態資訊的物件。  
...
### <a name="parameters"></a>參數  
 [in]*bShow*  
 布林值，指定是否要顯示或隱藏 按鈕。 如果此參數為 TRUE 時，會顯示 [] 按鈕。 如果參數為 FALSE，則會隱藏按鈕。  
