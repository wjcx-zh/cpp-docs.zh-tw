...
|[COccManager::PostCreateDialog](#postcreatedialog)|釋放在對話建立期間配置的資源。|
...
基類`CNoTrackObject`是未記載的基類 (位於 AFXTLS 中。H)。 由 MFC 架構所設計, 衍生自類別的`CNoTrackObject`類別會豁免記憶體流失偵測。 不建議您直接從`CNoTrackObject`衍生。
...
每個控制項容器都可以裝載多個網站。 使用多個對的呼叫來`CreateSite`建立其他網站。
...
的預設行為`IsDialogMessage`是檢查鍵盤訊息, 並將其轉換成對應對話方塊的選取專案。 例如, 按下 TAB 鍵時, 會選取下一個控制項或控制項群組。
...
*pHandlerInfo*<br/>
如果不是 Null `OnEvent` , 則會`pTarget`填入`pmf` `AFX_CMDHANDLERINFO`結構的和成員, 而不是分派命令。 通常, 這個參數應該是 Null。
...
*pOccDialogInfo*<br/>
`_AFX_OCC_DIALOG_INFO`結構, 包含對話方塊範本和對話方塊所裝載的任何 ActiveX 控制項的資訊。
...
這個記憶體是由對的呼叫`SplitDialogTemplate`所配置, 並用於對話方塊中任何裝載的 ActiveX 控制項。
...
> [!NOTE]
>  配置給新範本的記憶體會在函式`PostCreateDialog`中釋放。
