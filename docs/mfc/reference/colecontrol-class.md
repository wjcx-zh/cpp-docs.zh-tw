...
|[COleControl：:D oClick](#doclick)|庫存 `DoClick` 方法的執行。|
|[COleControl：:D oPropExchange](#dopropexchange)|序列化 `COleControl` 物件的屬性。|
...
|[COleControl：： GetActivationPolicy](#getactivationpolicy)|改變支援 `IPointerInactive` 介面之控制項的預設啟用行為。|
...
|[COleControl：： GetFontTextMetrics](#getfonttextmetrics)|傳回 `CFontHolder` 物件的計量。|
...
|[COleControl：： InternalGetFont](#internalgetfont)|傳回 stock 字型屬性的 `CFontHolder` 物件。|
...
|[COleControl：： OnClose](#onclose)|通知控制項已呼叫 `IOleControl::Close`。|
...
|[COleControl：： OnGetColorSet](#ongetcolorset)|通知控制項已呼叫 `IOleObject::GetColorSet`。|
...
|[COleControl：： OnSetClientSite](#onsetclientsite)|通知控制項已呼叫 `IOleControl::SetClientSite`。|
...
|[COleControl：： ResetStockProps](#resetstockprops)|將 `COleControl` stock 屬性初始化為其預設值。|
...
|[COleControl：： SerializeStockProps](#serializestockprops)|序列化或初始化 `COleControl` 的內建屬性。|
...
使用無視窗啟用時，容器（有視窗）負責提供控制項自己的視窗所提供的服務。 例如，如果您的控制項需要查詢鍵盤焦點、查詢滑鼠捕捉，或取得裝置內容，則這些作業會由容器管理。 `COleControl` 無視窗作業成員函式會在容器上叫用這些作業。

啟用無視窗啟動時，容器會將輸入訊息委派給控制項的 `IOleInPlaceObjectWindowless` 介面（ [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject)的延伸模組，以提供無視窗支援）。 `COleControl`的此介面的執行，會在適當調整滑鼠座標後，透過控制項的訊息對應來分派這些訊息。 您可以藉由將對應的專案加入至訊息對應，來處理這些訊息，例如一般視窗訊息。

在無視窗控制項中，您應該一律使用 `COleControl` 成員函式，而不是對應的 `CWnd` 成員函式或其相關的 Windows API 函式。
...
這必須在每次屬性的值變更時呼叫，即使不是透過屬性 Set 方法進行變更也是如此。 特別留意對應至成員變數的系結屬性。 每當這類成員變數變更時，都必須呼叫 `BoundPropertyChanged`。

##  <a name="boundpropertyrequestedit"></a>COleControl：： BoundPropertyRequestEdit

要求 `IPropertyNotifySink` 介面的許可權，以變更控制項所提供的系結屬性值。
...
無視窗物件無法安全地顯示插入號，而不需要先檢查插入號是否為部分或完全由重迭的物件隱藏。 若要這麼做，物件可以使用 `ClipCaretRect` 來調整插入號（減少），以確保它符合裁剪區域。

建立插入號的物件應該提交插入號矩形以 `ClipCaretRect`，並針對插入號使用已調整的矩形。 如果插入號完全隱藏，這個方法會傳回 FALSE，而且在此情況下，不應該顯示插入號。
...
將會呼叫可覆寫的 `COleControl::OnClick` 成員函式，而且如果控制項支援，就會引發內建 Click 事件。

`COleControl` 的基類支援此函式作為 stock 方法，稱為 DoClick。 如需詳細資訊，請參閱[ActiveX 控制項：方法](../../mfc/mfc-activex-controls-methods.md)一文。
...
如果已使用 Control Wizard 來建立 OLE 控制項專案，此函式的覆寫版本將會序列化 `COleControl` 呼叫基類函式所支援的內建屬性，`COleControl::DoPropExchange`。 當您將使用者定義的屬性新增至您的 OLE 控制項時，您必須修改此函式以序列化新的屬性。 如需序列化的詳細資訊，請參閱[ActiveX 控制項：](../../mfc/mfc-activex-controls-serializing.md)序列化一文。
...
此函式通常是由 `COleControl::DoPropExchange`的預設實值所呼叫。
...
一般來說，這會是由控制項的 `COleControl::DoPropExchange`的覆寫所呼叫的第一個函式。 載入時，此函式會讀取持續性資料的版本號碼，並據此設定[CPropExchange](../../mfc/reference/cpropexchange-class.md)物件的 version 屬性。 儲存時，此函式會寫入持續性資料的版本號碼。
...
啟用 `IPointerInactive` 介面時，容器會委派 WM_SETCURSOR 並 WM_MOUSEMOVE 訊息。 `COleControl`的此介面的執行，會在適當調整滑鼠座標後，透過控制項的訊息對應來分派這些訊息。

每當容器收到 WM_SETCURSOR 或 WM_MOUSEMOVE 訊息，並將滑鼠指標放在支援 `IPointerInactive`的非使用中物件上時，它應該會在介面上呼叫 `GetActivationPolicy`，並從 POINTERINACTIVE 列舉傳回旗標。

您可以藉由將對應的專案加入至訊息對應，來處理這些訊息，就像一般視窗訊息一樣。 在您的處理常式中，請避免使用 `m_hWnd` 成員變數（或使用它的任何成員函式），而不需要先檢查其值是否為非 Null。

要做的任何物件都不會設定滑鼠游標及/或引發滑鼠移動事件（例如提供特別的視覺回饋），應該只傳回 POINTERINACTIVE_ACTI加值稅EONENTRY 旗標，並只在使用中時才繪製意見反應。 如果物件傳回此旗標，容器應立即啟用它，然後將它轉送至觸發呼叫 `GetActivationPolicy`的相同訊息。
...
啟用 `IPointerInactive` 介面通常表示您希望控制項隨時都能夠處理滑鼠訊息。 若要在不支援 `IPointerInactive` 介面的容器中取得此行為，您必須在可見時一律啟用控制項，這表示控制項在其其他旗標中應該有 OLEMISC_ACTI加值稅EWHENVISIBLE 旗標。 不過，若要防止此旗標在支援 `IPointerInactive`的容器中生效，您也可以指定 OLEMISC_IGNOREACTI加值稅EWHENVISIBLE 旗標：
...
如果您使用 `GetAmbientProperty` 來抓取環境 DisplayName 和 ScaleUnits 屬性，請將*vtProp*設定為 VT_BSTR 並*pvProp*至 `CString*`。 如果您正在抓取環境字型屬性，請將*vtProp*設定為 VT_FONT，並將*PVPROP*設為 LPFONTDISP *。
...
如果成功，傳回值會將目前的外觀設定指定為**短**（VT_I2）值。 如果控制項的外觀為平面，此值為零，如果控制項的外觀為3D，則為1。
...
如果成功，傳回值會將目前的背景色彩指定為 OLE_COLOR 值。 您可以使用 `TranslateColor`的呼叫，將此值轉譯為 COLORRE光圈值。
...
否則，會傳回具有滑鼠捕捉的 `CWnd` 物件（與 `CWnd::GetCapture`相同）。
...
OLE 控制項在其容器內有一個矩形區域。 控制項的工作區是框線和捲軸以外的控制項區域。 `GetClientOffset` 所抓取的位移，就是控制項矩形區域的左上角和其工作區的左上角之間的差異。 如果您的控制項具有標準框線和捲軸以外的非用戶端元素，請覆寫此成員函式以指定位移。
...
*lpRect*<br/>
`RECT` 結構的指標，其中包含無視窗控制項工作區的維度;也就是說，控制項的大小減去視窗框線、框架、捲軸等等。 *LpRect*參數會指出控制項的用戶端矩形大小，而不是其位置。
...
傳回的指標會指向 `IOleClientSite`的實例。 由容器所執行的 `IOleClientSite` 介面是物件的內容視圖：它錨定在檔中，它會在其中取得其儲存體、使用者介面和其他資源。
...
|`clipPaintDC`|如果未設定，則會停用 `COleControl` 所做 `IntersectClipRect` 的呼叫，並獲得較小的速度優勢。 如果您使用無視窗啟用，旗標不會有任何作用。|
|`pointerInactive`|如果設定，則會藉由啟用 `COleControl`的 `IPointerInactive` 介面（預設為停用），在控制項處於非使用中狀態時提供滑鼠互動。|
...
如需 OLE 控制項 `GetControlFlags` 和其他優化的詳細資訊，請參閱[ActiveX 控制項：優化](../../mfc/mfc-activex-controls-optimization.md)。
...
容器 `CWnd` 工作區的顯示裝置內容指標（如果成功）;否則，傳回值為 Null。 顯示裝置內容可用於後續的 GDI 函式，以在容器視窗的工作區中繪製。

### <a name="remarks"></a>備註

在繪製之後，必須呼叫[ReleaseDC](#releasedc)成員函式以釋放內容。 呼叫 `GetDC`時，物件會以自己的用戶端座標，將他們想要繪製的矩形傳遞至。 `GetDC` 會將這些轉譯成容器工作區的座標。 物件不應要求大於自己的工作區矩形的所需繪圖矩形，其大小可使用[GetClientRect](#getclientrect)抓取。 這可避免物件不小心地在不應的位置繪製。
...
否則，會傳回具有焦點的 `CWnd` 物件（與 `CWnd::GetFocus`相同）。

### <a name="remarks"></a>備註

當呼叫[SetFocus](#setfocus)時，啟動的無視窗控制項會收到焦點。

##  <a name="getfont"></a>COleControl：： Ivsfontandcolorstorage.getfont
...
請注意，呼叫端必須在完成時釋放物件。 在控制項的執行中，使用 `InternalGetFont` 來存取控制項的內建字型物件。 如需在控制項中使用字型的詳細資訊，請參閱[Activex 控制項：在 Activex 控制項中使用字型一](../../mfc/mfc-activex-controls-using-fonts.md)文。

##  <a name="getfonttextmetrics"></a>COleControl：： GetFontTextMetrics

測量控制項所擁有之任何 `CFontHolder` 物件的文字度量。
...
您可以使用[COleControl：： SelectFontObject](#selectfontobject)函數來選取這種字型。 `GetFontTextMetrics` 將會初始化*lptm*所指向的 `TEXTMETRIC` 結構，其中包含有關 `fontHolder`字型的有效計量資訊（如果成功），或在不成功的情況下，以零填滿結構。 繪製控制項時，您應該使用此函式，而不是[GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ，因為控制項（例如任何內嵌的 OLE 物件）可能需要將自己轉譯為中繼檔。

呼叫[SelectFontObject](#selectfontobject)函數時，會重新整理預設字型的 `TEXTMETRIC` 結構。 只有在選取 [內建字型] 屬性以確保其所提供的資訊有效時，才應該呼叫 `GetFontTextMetrics`。
...
傳回值會指定目前的前景色彩做為 OLE_COLOR 值（如果成功）。 您可以使用 `TranslateColor`的呼叫，將此值轉譯為[COLORREF](/windows/win32/gdi/colorref)值。
...
如果成功，`GetStockTextMetrics` 函式會將*lptm*所指向的 `TEXTMETRIC` 結構初始化為有效的計量資訊，或在不成功的情況下，以零填滿結構。 繪製控制項時，請使用此函式，而不是[GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ，因為控制項（例如任何內嵌的 OLE 物件）可能需要將自己轉譯為中繼檔。

呼叫 `SelectStockFont` 函數時，會重新整理預設字型的 `TEXTMETRIC` 結構。 只有在選取股票字型之後，才應該呼叫此函式，以確保其所提供的資訊有效。
...
請注意，此函式的呼叫端必須在傳回的字串上呼叫 `SysFreeString`，才能釋放資源。 在控制項的執行中，使用 `InternalGetText` 來存取控制項的內建文字或標題屬性。

##  <a name="getwindowlessdroptarget"></a>COleControl：： GetWindowlessDropTarget

當您想要讓無視窗控制項成為 OLE 拖放作業的目標時，請覆寫 `GetWindowlessDropTarget`。
...
物件之 `IDropTarget` 介面的指標。 因為沒有視窗，所以無視窗物件無法註冊 `IDropTarget` 介面。 不過，若要參與拖放，無視窗物件仍然可以執行介面，並在 `GetWindowlessDropTarget`中傳回。

### <a name="remarks"></a>備註

通常這會需要將控制項的視窗登錄為置放目標。 但由於控制項沒有自己的視窗，因此容器會使用它自己的視窗做為放置目標。 控制項只需要提供 `IDropTarget` 介面的執行，容器就可以在適當的時間委派呼叫。 例如，
...
當您將使用 VBX 控制項的表單轉換為使用 OLE 控制項的格式時，可能需要 OLE 控制項的特殊載入程式碼。 例如，如果您要載入 OLE 控制項的實例，您可能會呼叫 `DoPropExchange`中的[PX_Font](persistence-of-ole-controls.md#px_font) ：
...
另一種情況是，如果您的 VBX 控制項儲存了專屬的二進位資料（在它的 VBM_SAVEPROPERTY 訊息處理常式中），而您的 OLE 控制項以不同的格式來儲存其二進位資料。 如果您想要讓 OLE 控制項與 VBX 控制項回溯相容，您可以使用 `IsConvertingVBX` 函式，藉由辨別 VBX 控制項或 OLE 控制項是否正在載入，來讀取舊的和新的格式。
...
架構的 `IDispatch::Invoke` 的執行呼叫 `IsInvokeAllowed` 來判斷是否可能叫用指定的函式（由 `dispid`識別）。 OLE 控制項的預設行為是只允許在控制項已初始化時才叫用 automation 方法;不過，`IsInvokeAllowed` 是虛擬函式，而且可能會在必要時加以覆寫（例如，當控制項當做 automation 伺服器使用時）。
...
如果您想要在此屬性變更之後發出通知，請覆寫此函數。 預設的實值 `InvalidateControl`會呼叫。
...
預設的實值 `COleControl::FireClick`會呼叫。

覆寫此成員函式以修改或擴充預設處理。

##  <a name="onclose"></a>COleControl：： OnClose

當容器已呼叫控制項的 `IOleControl::Close` 函數時由架構呼叫。
...
通常會針對螢幕顯示呼叫 `OnDraw`，以*pDC*的形式傳遞螢幕裝置內容。 *RcBounds*參數會識別目標裝置內容中的矩形（相對於其目前的對應模式）。 *RcInvalid*參數是不正確實際矩形。 在某些情況下，這會是比*rcBounds*更小的區域。
...
預設的實值 `COleControl::InvalidateControl`會呼叫。 如果控制項是子類別化 Windows 控制項，則預設的執行也會將 WM_SETFONT 訊息傳送至控制項的視窗。
...
在容器呼叫 `IOleControl::FreezeEvents`之後，由架構呼叫。
...
*pExtentInfo*<br/>
指向指定大小資料的 `DVEXTENTINFO` 結構。 `DVEXTENTINFO` 結構為：
...
由架構呼叫，以取得與先前由 `COleControl::OnGetPredefinedStrings`的覆寫所傳回的其中一個預先定義字串對應的值。
...
*dwCookie*<br/>
先前由 `COleControl::OnGetPredefinedStrings`的覆寫所傳回的 cookie 值。

*lpvarOut*<br/>
將傳回屬性值之 `VARIANT` 結構的指標。
...
- 整個物件的 DVASPECT_CONTENT 周框。 物件原點的左上角，大小等於 `GetViewExtent`所傳回的範圍 *。*
...
|VIEWSTATUS_DVASPECTTRANSPARENT|物件支援 DVASPECT_TRANSPARENT。 所有採用繪圖外觀做為參數的 `IViewObjectEx` 方法，都可以使用這個層面來呼叫。|
...
執行應該會隱藏 `OnShowToolbars`顯示的所有工具列。
...
您也可以使用此函式，讓控制項的屬性頁顯示。 呼叫 `OnProperties` 函式，並在*hWndParent*參數中傳遞控制項父系的控制碼。 在此情況下，會忽略*lpMsg*和*lpRect*參數的值。
...
*ptlLoc*<br/>
`POINT` 結構的指標，指定要檢查的點擊點。 該點是以 OLE 工作區座標指定。
...
查詢物件的顯示矩形是否與指定的點重迭（點擊點）。 您可以覆寫 `QueryHitPoint`，以測試非矩形物件的命中。
...
*prcLoc*<br/>
`RECT` 結構的指標，指定要檢查是否有點擊的矩形（與物件矩形重迭），相對於物件的左上角。
...
查詢物件的顯示矩形是否與指定矩形中的任何點重迭（命中矩形）。 您可以覆寫 `QueryHitRect`，以測試非矩形物件的命中。
...
指定的格式是先前使用[DelayRenderData](../../mfc/reference/coledatasource-class.md#delayrenderdata)或[DelayRenderFileData](../../mfc/reference/coledatasource-class.md#delayrenderfiledata)成員函式進行延遲轉譯的控制項物件。 如果所提供的儲存媒體是檔案或記憶體，此函式的預設實作為分別會呼叫 `OnRenderFileData` 或 `OnRenderGlobalData`。 如果要求的格式為 CF_METAFILEPICT 或持續性屬性集格式，則預設的執行會轉譯適當的資料，並傳回非零。 否則，它會傳回0，而且不會執行任何操作。
...
如需詳細資訊，請參閱 Windows SDK 中的 `FORMATETC` 和 `STGMEDIUM` 結構。
...
如果資料是持續性屬性集格式，則預設的執行會據以修改控制項的狀態。 否則，預設的執行不會執行任何操作。 如果*bRelease*為 TRUE，則會進行 `ReleaseStgMedium` 的呼叫;否則不會。
...
*lpSizeL*<br/>
`SIZEL` 結構的指標，其使用長整數來表示控制項的寬度和高度（以 HIMETRIC 單位表示）。
...
預設的執行會處理控制項範圍的調整大小。 如果控制項處於作用中狀態，則會對容器的 `OnPosRectChanged` 進行呼叫。
...
處理無視窗控制項的視窗訊息。 `COleControl`的 `OnWindowlessMessage` 應該用於滑鼠訊息和鍵盤訊息以外的視窗訊息。 `COleControl` 提供[SetCapture](#setcapture)和[SetFocus](#setfocus) ，專門用來取得無視窗的 OLE 物件的滑鼠捕捉和鍵盤焦點。

因為無視窗物件沒有視窗，所以它們需要一個機制，讓容器分派訊息給它們。 無視窗的 OLE 物件會透過 `IOleInPlaceObjectWindowless` 介面上的 `OnWindowMessage` 方法（ [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject)的延伸模組），從其容器取得訊息。 `OnWindowMessage` 不接受 `HWND` 參數。
...
在顯示任何強制回應對話方塊之後，呼叫此函式。 您必須呼叫此函式，讓容器可以 `PreModalDialog`停用任何最上層的 windows。 此函式應該與 `PreModalDialog`的呼叫配對。
...
在顯示任何強制回應對話方塊之前呼叫此函式。 您必須呼叫此函式，讓容器可以停用其所有最上層視窗。 在顯示模式對話方塊之後，您必須接著呼叫 `PostModalDialog`。
...
`COleControl` 的基類支援此函式做為 stock 方法，稱為 Refresh。 這可讓您的 OLE 控制項的使用者在特定時間重新繪製控制項。 如需這個方法的詳細資訊，請參閱[ActiveX 控制項：方法](../../mfc/mfc-activex-controls-methods.md)一文。
...
應用程式必須針對[GetDC](#getdc)的每個呼叫呼叫 `ReleaseDC`。
...
將 `COleControl` stock 屬性的狀態初始化為其預設值。
...
您可以使用 `ResetStockProps` 和 `ResetVersion` 覆寫 `COleControl::OnResetState`，來改善控制項的二進位初始化效能。 請參閱下方的範例。 如需優化初始化的進一步資訊，請參閱[ActiveX 控制項：優化](../../mfc/mfc-activex-controls-optimization.md)。
...
先前選取之 `CFont` 物件的指標。 當您完成時，您應該使用[CDC：： SelectObject](../../mfc/reference/cdc-class.md#selectobject)來選取此字型回到裝置內容中。
...
您可以使用 `SerializeExtent`、`SerializeStockProps`和 `SerializeVersion` 覆寫 `COleControl::Serialize`，來改善控制項的二進位持續性效能。 請參閱下方的範例。 如需優化初始化的進一步資訊，請參閱[ActiveX 控制項：優化](../../mfc/mfc-activex-controls-optimization.md)。
...
序列化或初始化 `COleControl` 內建屬性的狀態：外觀、背景色彩、BorderStyle、標題、已啟用、字型、前景和文字。
...
您可以使用 `SerializeStockProps`、`SerializeExtent`和 `SerializeVersion` 覆寫 `COleControl::Serialize`，來改善控制項的二進位持續性效能。 如需範例，請參閱程式碼，網址為[SerializeExtent](#serializeextent)。 如需優化初始化的進一步資訊，請參閱[ActiveX 控制項：優化](../../mfc/mfc-activex-controls-optimization.md)。
...
控制項的版本號碼。 如果指定的封存正在載入，`SerializeVersion` 會傳回從該封存載入的版本。 否則，它會傳回目前載入的版本。

### <a name="remarks"></a>備註

您可以使用 `SerializeVersion`、`SerializeExtent`和 `SerializeStockProps` 覆寫 `COleControl::Serialize`，來改善控制項的二進位持續性效能。 如需範例，請參閱程式碼，網址為[SerializeExtent](#serializeextent)。 如需優化初始化的進一步資訊，請參閱[ActiveX 控制項：優化](../../mfc/mfc-activex-controls-optimization.md)。
...
接著會重新建立控制項視窗，並 `OnBorderStyleChanged` 呼叫。
...
先前收到滑鼠輸入之 `CWnd` window 物件的指標。

### <a name="remarks"></a>備註

如果控制項已啟動且無視窗，則此函式會讓控制項的容器視窗代表控制項擁有滑鼠捕捉。 否則，此函式會讓控制項本身擁有滑鼠捕捉（與 `CWnd::SetCapture`相同）。
...
當 `BoundPropertyRequestEdit` 失敗時，請呼叫此函式。 此函式會擲回 `COleDispScodeException` 類型的例外狀況，表示不允許設定作業。
...
如果控制項已開啟，則會調整大小;否則，會呼叫容器的 `OnPosRectChanged` 函式。
...
*lpptlHimetric*<br/>
`POINTL` 結構的指標，其中包含 HIMETRIC 單位的座標。
