...
|[COleControl::DoSuperclassPaint](#dosuperclasspaint)|重新繪製已從 Windows 控制項子類別化的 OLE 控制項。|
...
|[COleControl::OnGetColorSet](#ongetcolorset)|通知控制項`IOleObject::GetColorSet`已被呼叫。|
...
此函式通常是由的預設執行`COleControl::DoPropExchange`所呼叫。
...
通常由[IMPLEMENT_OLECREATE_EX](class-factories-and-licensing.md#implement_olecreate_ex)執行。
...
預設的執行會`COleControl::FireClick`呼叫。
...
這與叫用控制項的 OLEIVERB_UIACTI加值稅E 動詞具有相同的效果。
...
*dwCookie*<br/>
的覆寫`COleControl::OnGetPredefinedStrings`先前傳回的 cookie 值。

*lpvarOut*<br/>
將傳回屬性值之結構的指標。`VARIANT`
...
此實作為應該隱藏所有顯示的`OnShowToolbars`工具列。
...
接著會重新建立並`OnBorderStyleChanged`呼叫控制項視窗。
...
- XFORMCOORDS_CONTAINERTOHIMETRIC 會將容器的單位轉換成 HIMETRIC 單位。
