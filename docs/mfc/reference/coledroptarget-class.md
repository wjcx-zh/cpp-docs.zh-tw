...
|名稱|描述|
|----------|-----------------|
|[COleDropTarget：： System.windows.uielement.ondragenter](#ondragenter)|在資料指標第一次進入視窗時呼叫。|
|[COleDropTarget：： System.windows.uielement.ondragleave](#ondragleave)|當資料指標拖曳至視窗外時呼叫。|
|[COleDropTarget：： System.windows.uielement.ondragover](#ondragover)|將游標拖曳至視窗上方時重複呼叫。|
|[COleDropTarget::OnDragScroll](#ondragscroll)|呼叫以決定是否將游標拖曳至視窗的捲軸區域。|
|[COleDropTarget：： System.windows.uielement.ondrop](#ondrop)|將資料放入視窗、預設處理常式時呼叫。|
...
若要取得視窗以接受 drop 命令，您應該先建立 `COleDropTarget` 類別的物件，然後使用所需 `CWnd` 物件的指標來呼叫[Register](#register)函式，做為唯一的參數。
...
覆寫此函式，以允許在視窗中執行 drop 作業。 預設的執行會呼叫[CView：： system.windows.uielement.ondragenter](../../mfc/reference/cview-class.md#ondragenter)，這只會根據預設傳回 DROPEFFECT_NONE。
...
應該覆寫此函式，以允許在視窗中執行 drop 作業。 此函式的預設實作用會呼叫[CView：： system.windows.uielement.ondragover](../../mfc/reference/cview-class.md#ondragover)，預設會傳回 DROPEFFECT_NONE。 因為此函式會在拖放作業期間經常呼叫，所以最好盡可能優化。
...
架構會先呼叫[OnDropEx](#ondropex)。 如果 `OnDropEx` 函式不會處理卸載，架構就會呼叫這個成員函式，`OnDrop`。 一般而言，應用程式會覆寫 view 類別中的[OnDropEx](../../mfc/reference/cview-class.md#ondropex) ，以處理滑鼠右鍵拖放功能。 通常，view 類別[system.windows.uielement.ondrop](../../mfc/reference/cview-class.md#ondrop)是用來處理簡單的拖放。
...
*dropDefault*<br/>
根據目前的索引鍵狀態，使用者為預設的卸載作業所選擇的效果。 它可以 DROPEFFECT_NONE。 「備註」一節會討論捨棄效果。
...
`COleDropTarget::OnDropEx` 的預設執行會呼叫[CView：： OnDropEx](../../mfc/reference/cview-class.md#ondropex)。 根據預設， [CView：： OnDropEx](../../mfc/reference/cview-class.md#ondropex)只會傳回虛擬值，以指出應該呼叫[system.windows.uielement.ondrop](#ondrop)成員函式。
