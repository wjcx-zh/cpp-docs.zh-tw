...
此資料類型用於 OLE automation 中。 具體而言， [DISPPARAMS](/windows/win32/api/oaidl/ns-oaidl-dispparams)結構包含 VARIANT 結構陣列的指標。 結構是用來將參數傳遞給[IDispatch：： Invoke。](/windows/win32/api/oaidl/nf-oaidl-idispatch-invoke) `DISPPARAMS`
