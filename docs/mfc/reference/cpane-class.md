...
ms.locfileid: "68916914"
---
# <a name="cpane-class"></a>CPane Class

類別是 [CControlBar](../../mfc/reference/ccontrolbar-class.md) 類別的增強功能。`CPane` 如果您要升級現有的 MFC 專案, 請以`CControlBar` `CPane`取代所有出現的。
...
|[CPane::DockByMouse](#dockbymouse)|使用滑鼠停駐方法來停駐窗格。|
|[CPane::DockPane](#dockpane)|將浮動窗格停駐于基底窗格。|
|[CPane::DockPaneStandard](#dockpanestandard)|使用大綱 (標準) 銜接停駐窗格。|
|[CPane::DockToFrameWindow](#docktoframewindow)|將可停駐的窗格停駐在框架上。 (覆寫 `CBasePane::DockToFrameWindow`。)|
|[CPane::DoesAllowSiblingBars](#doesallowsiblingbars)|指出您是否可以將另一個窗格固定在目前窗格停駐所在的相同資料列上。|
...
|[CPane::GetBorders](#getborders)|傳回窗格框線的寬度。|
...
*bActiveTabOnly*<br/>
在未在中`CPane::ConvertToTabbedDocument`使用。
...
這個方法會建立 Windows 窗格, 並將它附加`CPane`至物件。
...
[CRect](../../atl-mfc-shared/reference/crect-class.md)物件, 包含窗格每一邊的目前寬度 (以圖元為單位)。 例如, `left` `CRect`物件的成員值是左邊框的寬度。
...
指向停駐窗格所在之 dock 資料列的*,如果窗格未停駐則為Null。`CDockingPanesRow`
...
類別包含物件,`m_dragFrameImpl`其負責繪製使用者在標準銜接模式下移動窗格時所顯示的矩形。 `CMFCDragFrameImpl` `CPane` 作用點是用來在使用者移動窗格時, 繪製相對於目前滑鼠位置的矩形。
...
如果應用程式中的一個或多個停`GetMinSize`駐窗格覆寫, 或`SetMinSize`如果您的應用程式呼叫, 您可能會想要將此靜態成員設定為 TRUE, 以便讓架構一致地處理窗格的大小。
...
物件, 包含新的和舊矩形 (舊的矩形-rectNew) 之間的寬度和高度差異。 `CSize`
...
根據預設, 會藉由呼叫`CDockSite::RemovePane`, 在停駐窗格中取消註冊窗格。
...
功能表包含數個專案, 可讓您指定窗格的行為, 亦即:**浮動**、停駐、自動隱藏和**隱藏**。 您可以藉由呼叫[CDockingManager:: EnableDockSiteMenu](../../mfc/reference/cdockingmanager-class.md#enabledocksitemenu), 為所有窗格啟用此功能表。
...
架構會在將窗格的狀態儲存至登錄時呼叫這個方法。 覆`SaveState`寫衍生類別中的, 以儲存其他資訊。
