...
|[CPrintDialog：:D oModal](#domodal)|顯示對話方塊，並允許使用者進行選取。|
...
|[CPrintDialog::GetDevMode](#getdevmode)|抓取 `DEVMODE` 結構。|
...
|[CPrintDialog：:P rintAll](#printall)|決定是否要列印檔案的所有頁面。|
|[CPrintDialog：:P rintCollate](#printcollate)|判斷是否要求自動分頁的複本。|
|[CPrintDialog：:P rintRange](#printrange)|決定是否只列印指定的頁面範圍。|
|[CPrintDialog：:P rintSelection](#printselection)|決定是否只列印目前選取的專案。|
...
`CPrintDialog`的功能會由[CPageSetupDialog](../../mfc/reference/cpagesetupdialog-class.md)所取代，其設計目的是為您提供列印設定和版面設定的通用對話方塊。
...
如果您想要讓應用程式在不需要架構介入的情況下處理列印，您可以使用所提供的函式來「依原樣」 `CPrintDialog` 類別，也可以從 `CPrintDialog` 衍生您自己的對話方塊類別，並撰寫符合您需求的函式。 不論是哪一種情況，這些對話方塊的行為都像標準 MFC 對話方塊，因為它們是從類別 `CCommonDialog`衍生而來。

若要使用 `CPrintDialog` 物件，請先使用 `CPrintDialog` 的「函數」來建立物件。 建立對話方塊之後，您可以設定或修改[m_pd](#m_pd)結構中的任何值，以初始化對話方塊控制項的值。 `m_pd` 結構的類型是[PRINTDLG](/windows/win32/api/commdlg/ns-commdlg-printdlga)。 如需此結構的詳細資訊，請參閱 Windows SDK。

如果您沒有在 `hDevMode` 和 `hDevNames` 成員的 `m_pd` 中提供自己的控制碼，請務必在完成對話方塊時，為這些控制碼呼叫 Windows 函式 `GlobalFree`。 使用 `CWinApp::OnFilePrintSetup`所提供的架構列印設定時，您不需要釋放這些控制碼。 這些控制碼是由 `CWinApp` 維護，並會在 `CWinApp`的析構函式中釋放。 只有在使用獨立 `CPrintDialog` 時，才需要釋放這些控制碼。

在初始化對話方塊控制項之後，請呼叫 `DoModal` 成員函式來顯示對話方塊，並允許使用者選取列印選項。 `DoModal` 會傳回使用者是否選取 [確定] （IDOK）或 [取消] （IDCANCEL）按鈕。

如果 `DoModal` 傳回 IDOK，您可以使用其中一個 `CPrintDialog`的成員函式來取出使用者輸入的資訊。
...
若要自訂對話方塊、從 `CPrintDialog`衍生類別、提供自訂對話方塊範本，以及新增訊息對應以處理來自擴充控制項的通知訊息。 任何未處理的訊息都應該傳遞至基類。 不需要自訂攔截函數。
...
這個成員函式只會建立物件。 使用 `DoModal` 成員函式來顯示對話方塊。

請注意，當您呼叫*bPrintSetupOnly*設定為 FALSE 的函式時，會自動使用 PD_RETURNDC 旗標。 呼叫 `DoModal`、`GetDefaults`或 `GetPrinterDC`之後，會在 `m_pd.hDC`中傳回印表機 DC。 此 DC 必須釋放 `CPrintDialog`的呼叫端[DeleteDC](/windows/win32/api/wingdi/nf-wingdi-deletedc) 。
...
如果您想要藉由設定 `m_pd` 結構的成員來初始化各種 [列印對話方塊] 選項，您應該先執行此動作，再呼叫 `DoModal`，但在構造對話方塊物件之後。
...
在某些情況下，呼叫此函式會呼叫 `CPrintDialog`[的函](#cprintdialog)式，並將*BPRINTSETUPONLY*設定為 FALSE。 在這些情況下，會自動設定印表機 DC 和 `hDevNames` 和 `hDevMode` （位於 `m_pd` 資料成員中的兩個控制碼）。

如果在*bPrintSetupOnly*設定為 FALSE 的情況下呼叫 `CPrintDialog` 的函式，此函式將不只會將位於 `m_pd.hDevNames` 和 `m_pd.hDevMode`）的 `hDevNames` 和 `hDevMode` 傳回給呼叫者，但也會在 `m_pd.hDC`中傳回印表機 DC。 當您完成 `CPrintDialog` 物件時，呼叫端必須負責刪除印表機 DC，並在控制碼上呼叫 Windows [GlobalFree](/windows/win32/api/winbase/nf-winbase-globalfree)函數。
...
呼叫[DoModal](#domodal)以抓取目前選取之印表機的名稱，或在呼叫[GetDefaults](#getdefaults)以抓取預設印表機的目前裝置預設值之後，呼叫此函式。 在[CDC：： CreateDC](../../mfc/reference/cdc-class.md#createdc)的呼叫中，使用 `GetDeviceName` 所傳回之 `CString` 物件的指標做為 `lpszDeviceName` 的值。
...
在呼叫[DoModal](#domodal)或[GetDefaults](#getdefaults)之後呼叫此函式，以取出系統定義的印表機設備磁碟機名稱。 在[CDC：： CreateDC](../../mfc/reference/cdc-class.md#createdc)的呼叫中，使用 `GetDriverName` 所傳回之 `CString` 物件的指標做為 `lpszDriverName` 的值。
...
呼叫 `DoModal` 以在要列印的頁面範圍內抓取起始頁碼後，呼叫這個函式。
...
如果 `CPrintDialog` 處理常式的*bPrintSetupOnly*參數為 FALSE （表示顯示 [列印] 對話方塊），則 `GetPrinterDC` 會傳回印表機裝置內容的控制碼。 當您使用完裝置內容時，您必須呼叫 Windows [DeleteDC](/windows/win32/api/wingdi/nf-wingdi-deletedc)函式來加以刪除。
...
呼叫 `DoModal` 以在要列印的頁面範圍內抓取結束頁碼後，呼叫這個函式。
...
呼叫 `DoModal` 以決定是否要列印檔案中的所有頁面之後，呼叫這個函式。
...
呼叫 `DoModal` 之後，請呼叫此函式，以判斷是否只列印檔案中的一系列頁面。
...
呼叫 `DoModal` 之後，請呼叫此函式，以判斷是否只列印目前選取的專案。
