...
若要使用`CPrintDialog`物件，請先使用此`CPrintDialog`函數建立物件。 建立對話方塊之後，您可以設定或修改[m_pd](#m_pd)結構中的任何值，以初始化對話方塊控制項的值。 `m_pd`結構的型別為[PRINTDLG](/windows/win32/api/commdlg/ns-commdlg-printdlga)。 如需此結構的詳細資訊，請參閱 Windows SDK。

如果您未在`m_pd`中為`hDevMode`和`hDevNames`成員提供自己的控制碼，請務必在完成對話方塊時， `GlobalFree`為這些控制碼呼叫 Windows 函式。 使用所提供的架構列印安裝程式時`CWinApp::OnFilePrintSetup`，您不需要釋放這些控制碼。 控制碼是由`CWinApp`維護，並在的`CWinApp`析構函式中釋放。 只有在使用`CPrintDialog`獨立時，才需要釋放這些控制碼。
...
請注意，當您呼叫*bPrintSetupOnly*設定為 FALSE 的函式時，會自動使用 PD_RETURNDC 旗標。 呼叫`DoModal`、 `GetDefaults`或`GetPrinterDC`之後，將會在中`m_pd.hDC`傳回印表機 DC。 此 DC 必須透過呼叫呼叫端的`CPrintDialog` [DeleteDC](/windows/win32/api/wingdi/nf-wingdi-deletedc)來釋放。
...
在某些情況下，呼叫此函式將會呼叫[的](#cprintdialog) *bPrintSetupOnly*的函式， `CPrintDialog`並將設定為 FALSE。 在這些情況下，系統會自動`hDevNames`配置`hDevMode`印表機 DC 和和（位於`m_pd`資料成員中的兩個控制碼）。

如果呼叫的的`CPrintDialog`函式，且*BPRINTSETUPONLY*設定為 FALSE，則此函式不`hDevNames`只`hDevMode`會傳回`m_pd.hDevNames`給`m_pd.hDevMode`呼叫者，並將它放在和中），但`m_pd.hDC`也會在中傳回印表機 DC。 當您完成`CPrintDialog`物件時，呼叫端會負責刪除印表機 DC，並在控制碼上呼叫 Windows [GlobalFree](/windows/win32/api/winbase/nf-winbase-globalfree)函數。
...
呼叫[DoModal](#domodal)以抓取目前選取之印表機的名稱，或在呼叫[GetDefaults](#getdefaults)以抓取預設印表機的目前裝置預設值之後，呼叫此函式。 在[CDC：： CreateDC](../../mfc/reference/cdc-class.md#createdc)的呼叫中`GetDeviceName` ，使用所傳回`lpszDeviceName`之`CString`物件的指標做為的值。
...
在呼叫[DoModal](#domodal)或[GetDefaults](#getdefaults)之後呼叫此函式，以取出系統定義的印表機設備磁碟機名稱。 在[CDC：： CreateDC](../../mfc/reference/cdc-class.md#createdc)的呼叫中`GetDriverName` ，使用所傳回`lpszDriverName`之`CString`物件的指標做為的值。
...
如果此*bPrintSetupOnly* `CPrintDialog`函式的 bPrintSetupOnly 參數為 FALSE （表示顯示 [列印] 對話方塊），則`GetPrinterDC`會傳回印表機裝置內容的控制碼。 當您使用完裝置內容時，您必須呼叫 Windows [DeleteDC](/windows/win32/api/wingdi/nf-wingdi-deletedc)函式來加以刪除。
