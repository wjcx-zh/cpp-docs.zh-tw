...
|名稱|描述|  
|----------|-----------------|  
|[Cprogressctrl:: Create](#create)|建立進度列控制項並將它附加`CProgressCtrl`物件。|  
|[CProgressCtrl::CreateEx](#createex)|建立進度控制項指定 Windows 延伸樣式，並將它附加`CProgressCtrl`物件。|  
...
|[CProgressCtrl::OffsetPos](#offsetpos)|由指定的遞增量進度列控制項的目前位置前移，並且重繪的列，以反映新的位置。|  
|[CProgressCtrl::SetBarColor](#setbarcolor)|設定目前的進度列控制項中的進度指示器列的色彩。|  
|[CProgressCtrl::SetBkColor](#setbkcolor)|設定進度列的背景色彩。|  
|[CProgressCtrl::SetMarquee](#setmarquee)|關閉目前的進度列控制項的開啟或關閉的跑馬燈模式。|  
...
|[CProgressCtrl::SetStep](#setstep)|指定進度列控制項的步驟遞增量。|  
|[CProgressCtrl::StepIt](#stepit)|進度列控制項的目前位置前移步驟遞增 (請參閱[SetStep](#setstep)) 並重新繪製的列，以反映新的位置。|  
  
## <a name="remarks"></a>備註  
 進度列控制項是一個視窗，應用程式可用來指示長時間作業的進度。 它包含一個矩形，會逐漸填滿，從左到右，使用系統醒目提示色彩隨著作業的進度。  
  
 進度列控制項有範圍和目前的位置。 範圍代表作業的總持續時間和目前的位置表示應用程式已完成作業的進度。 視窗程序使用的範圍和目前的位置來決定進度列，以醒目提示色彩填滿的百分比。 因為範圍和目前的位置值會表示為帶正負號的整數，目前的位置值的可能範圍是從 â €"2147483648 到 2147483647 之間。  
...
### <a name="parameters"></a>參數  
 `dwStyle`  
 指定進度列控制項的樣式。 套用視窗 stylesdescribed 中的任何結合[CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679)中[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]，除了下列進度列控制項的樣式，來控制︰  
  
- `PBS_VERTICAL`顯示進度資訊垂直，往前。 而不需要此旗標，進度列控制項會顯示水平、 左到右。  
  
- `PBS_SMOOTH`顯示進度列控制項填入漸進式平滑。 而不需要此旗標，區塊需要填入控制項。  
  
 `rect`  
 指定進度列控制項的大小和位置。 它可以是[CRect](../../atl-mfc-shared/reference/crect-class.md)物件或[RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897)結構。 控制項必須是子視窗，因為指定的座標是相對於用戶端區域`pParentWnd`。  
...
### <a name="remarks"></a>備註  
 您建構`CProgressCtrl`兩個步驟中的物件。 首先，呼叫建構函式，建立`CProgressCtrl`物件，然後再呼叫**建立**，這樣就可以建立進度列控制項。  
...
### <a name="parameters"></a>參數  
 ...
 `dwStyle`  
 指定進度列控制項的樣式。 套用的視窗樣式中所述的任何結合[CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679)中[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
### <a name="return-value"></a>傳回值  
 目前的進度列的色彩表示為[COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449)值，或`CLR_DEFAULT`如果進度指示器列色彩是預設色彩。  
...
### <a name="remarks"></a>備註  
 進度列控制項的位置不在畫面上的實體位置，但而之間上限和下限範圍中會指出[SetRange](#setrange)。  
...
### <a name="remarks"></a>備註  
 此函式會將下限和上限限制的值複製到所參考的整數`nLower`和`nUpper`分別。  
...
### <a name="remarks"></a>備註  
 這個方法會傳送[PBM_GETSTATE](http://msdn.microsoft.com/library/windows/desktop/bb760834)訊息中所述[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
### <a name="remarks"></a>備註  
 步驟增量是依據數量的呼叫[CProgressCtrl::StepIt](#stepit)增加進度列目前位置。  
  
 這個方法會傳送[PBM_GETSTEP](http://msdn.microsoft.com/library/windows/desktop/bb760836)訊息中所述[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
|參數|描述|  
|---------------|-----------------|  
|[in] `clrBar`|A [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449)值，指定新的進度指示器列色彩。 指定`CLR_DEFAULT`造成進度列，使用其預設色彩。|  
  
### <a name="return-value"></a>傳回值  
 前一個色彩的進度指示器列中，以表示[COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449)值，或`CLR_DEFAULT`如果進度指示器列的色彩是預設色彩。  
  
### <a name="remarks"></a>備註  
 `SetBarColor`方法會設定進度列色彩才[!INCLUDE[windowsver](../../build/reference/includes/windowsver_md.md)][佈景主題](https://msdn.microsoft.com/library/windows/desktop/hh270423.aspx)不在作用中。  
  
 這個方法會傳送[PBM_SETBARCOLOR](http://msdn.microsoft.com/library/windows/desktop/bb760838)訊息中所述[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
### <a name="example"></a>範例  
 下列程式碼範例會將進度列的色彩變更成紅色、 綠色、 藍色或預設值。  
...
### <a name="return-value"></a>傳回值  
 [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449)值，指出先前的背景色彩，或**CLR_DEFAULT**如果的背景色彩的預設色彩。  
...
|[in] `nInterval`|以毫秒為單位的跑馬燈動畫更新之間的時間。|  
  
### <a name="return-value"></a>傳回值  
 這個方法一律會傳回 `true`。  
  
### <a name="remarks"></a>備註  
 當跑馬燈模式啟動，進度列的動畫，且捲動像是登入劇場跑馬燈。  
  
 這個方法會傳送[PBM_SETMARQUEE](http://msdn.microsoft.com/library/windows/desktop/bb760842)訊息中所述[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
|參數|描述|  
|---------------|-----------------|  
|[in] `iState`|要設定的進度列狀態。 使用下列其中一個值：<br /><br /> - `PBST_NORMAL`-在進行中<br />- `PBST_ERROR`-錯誤<br />- `PBST_PAUSED`-暫停|  
  
### <a name="return-value"></a>傳回值  
 目前進度列控制項先前的狀態。  
  
### <a name="remarks"></a>備註  
 這個方法會傳送[PBM_SETSTATE](http://msdn.microsoft.com/library/windows/desktop/bb760850)訊息中所述[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
### <a name="remarks"></a>備註  
 步驟增量是依據數量的呼叫`CProgressCtrl::StepIt`增加進度列的目前位置。  
