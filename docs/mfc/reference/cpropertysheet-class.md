...
|[CPropertySheet::DoModal](#domodal)|顯示模式屬性工作表。|
...
|[CPropertySheet::PressButton](#pressbutton)|模擬屬性工作表中指定之按鈕的選擇。|
...
即使不`CPropertySheet` `CDialog`是衍生自 [CDialog](../../mfc/reference/cdialog-class.md), 管理物件就像管理物件一樣。  `CPropertySheet` 例如, 建立屬性工作表需要兩個部分的結構: 呼叫此函式, 然後呼叫模式屬性工作表的[DoModal](#domodal) , 或針對非模式屬性工作表[建立](#create)。 `CPropertySheet`有兩種類型的構造器:[CPropertySheet:: 結構](#construct)和[CPropertySheet:: CPropertySheet](#cpropertysheet)。

當您建立`CPropertySheet`物件時, 某些[視窗樣式](../../mfc/reference/styles-used-by-mfc.md#window-styles)可能會導致發生第一個可能的例外狀況。 這是系統在建立工作表之前, 嘗試變更屬性工作表樣式所產生的結果。 若要避免這個例外狀況, 請確定您在建立`CPropertySheet`時設定了下列樣式:
...
任何其他`Window Styles`專案則不允許, 您也不應加以啟用。

在`CPropertySheet`物件與外部物件之間交換資料類似于`CDialog`與物件交換資料。 重要的差異在於屬性工作表的設定通常是`CPropertyPage`物件的成員變數, 而不`CPropertySheet`是物件本身。
...
*pPage*<br/>
指向要加入至屬性工作表的頁面。 不可以是 NULL。

### <a name="remarks"></a>備註

依您想要顯示的順序, 將頁面加入至屬性工作表。

`AddPage`將[CPropertyPage](../../mfc/reference/cpropertypage-class.md#cpropertypage)物件新增至`CPropertySheet`物件的頁面清單, 但實際上不會建立頁面的視窗。 架構會延遲建立頁面的視窗, 直到使用者選取該頁面為止。

當您使用`AddPage`加入屬性頁時`CPropertySheet` , `CPropertyPage`是的父系。 若要從屬性頁取得屬性工作表的存取權, 請呼叫[CWnd:: GetParent](../../mfc/reference/cwnd-class.md#getparent)。

您不需要等到建立屬性工作表視窗來呼叫`AddPage`。 通常, 您會在`AddPage`呼叫[DoModal](#domodal)或[Create](#create)之前呼叫。

如果您在`AddPage`顯示內容頁之後呼叫, 索引標籤列會反映新加入的頁面。
...
*pszCaption*<br/>
字串的指標, 其中包含要用於屬性工作表的標題。 不可以是 NULL。
...
如果尚未呼叫其中一個類別的函式, 請呼叫這個成員函式。 例如, 當您`Construct`宣告或設定物件的`CPropertySheet`陣列時, 請呼叫。 在陣列的案例中, 您必須針對`Construct`陣列中的每個成員呼叫。

若要顯示內容表, 請呼叫[DoModal](#domodal)或[建立](#create)。 第一個參數中所包含的字串將會放在屬性工作表的標題列中。

如果您使用以上所列的第三個或第四個`Construct`原型, 而且您傳遞*hbmWatermark*、 *hpalWatermark*和/或*hbmHeader*參數的有效值, 則可以自動顯示浮水印和/或標頭影像。

### <a name="example"></a>範例

下列範例會在您要呼叫`Construct`的情況下示範。
...
如果您有多個參數 (例如, 如果您使用的是陣列), 請使用 [[結構`CPropertySheet`], 而不是。](#construct)

如果您使用上述的第三個或第四個`CPropertySheet`原型, 而且您傳遞*hbmWatermark*、 *hpalWatermark*和/或*hbmHeader*參數的有效值, 則可以自動顯示浮水印和/或標頭影像。
...
呼叫`Create`可以在此函式內, 您也可以在叫用此函式之後呼叫它。
...
`Create`成員函式會在建立屬性工作表之後立即傳回。 若要摧毀屬性工作表, 請呼叫[CWnd::D estroywindow](../../mfc/reference/cwnd-class.md#destroywindow)。

與的呼叫`Create`所顯示的非強制回應屬性工作表沒有 [確定]、[取消]、[立即套用] 和 [說明] 按鈕做為強制回應屬性工作表。 所需的按鈕必須由使用者建立。
...
如果函式成功, 則為 IDOK 或 IDCANCEL;否則為0或-1。 如果已將屬性工作表建立為 wizard (請參閱[SetWizardMode](#setwizardmode)), `DoModal`則會傳回 ID_WIZFINISH 或 IDCANCEL。

### <a name="remarks"></a>備註

傳回值會對應至關閉屬性工作表之控制項的識別碼。 此函式傳回之後, 對應至屬性工作表和所有頁面的視窗都會被終結。 物件本身仍會存在。 通常, 您會在傳回 IDOK 之後 `DoModal` , 從 [CPropertyPage](../../mfc/reference/cpropertypage-class.md) 物件中取出資料。
...
> [!NOTE]
>  如果您使用非同步例外狀況處理模型進行編譯, 作業系統就必須處理這個例外狀況。 如需例外狀況處理模型的詳細資訊, 請參閱[/EH (例外狀況處理模型)](../../build/reference/eh-exception-handling-model.md)。 在此情況下, 請不要使用`CPropertySheet::DoModal` C++ try-catch 區塊來包裝的呼叫`catch (...)`, 其中 catch 會處理所有例外狀況, 例如。 此區塊會處理適用于作業系統的例外狀況, 並造成無法預期的行為。 不過, 您可以安全地C++使用例外狀況處理, 搭配特定的例外狀況類型或結構化例外狀況處理, 其中的存取違規例外狀況會傳遞至作業系統。
...
根據預設, 如果屬性工作表的索引標籤數目超過屬性工作表寬度的單一資料列, 則索引標籤會堆疊在多個資料列中。 若要使用 [捲軸] 索引標籤`EnableStackedTabs` , 而不是堆疊索引標籤, 請在呼叫[DoModal](#domodal)或[建立](#create)之前, 使用*bStacked*設定為

當您建立`EnableStackedTabs`模式或非模式屬性工作表時, 必須呼叫。 若要在衍生類別中`CPropertySheet`併入此樣式, 請撰寫 WM_CREATE 的訊息處理常式。 在已覆寫的[CWnd:: OnCreate](../../mfc/reference/cwnd-class.md#oncreate)版本中`EnableStackedTabs( FALSE )` , 呼叫基類執行之前請先呼叫。
...
取得屬性工作表視窗之作用中頁面的索引編號, 然後使用傳回的索引編號做為的參數`GetPage`。
...
*nPage*<br/>
所需頁面的索引 (從0開始)。 必須介於0到 1 (含) 的屬性工作表中的頁面數之間。
...
例如, 您可以使用`GetPageIndex`來取得頁面索引, 以便使用[SetActivePage](#setactivepage)或[GetPage](#getpage)。
...
使用此結構來初始化屬性工作表在結構化後, 但在它與[DoModal](#domodal)成員函式一起顯示之前的外觀。 例如, 將的*dwSize*成員`m_psh`設定為您希望屬性工作表擁有的大小。
...
此成員函式會呼叫以回應 WM_INITDIALOG 訊息。 這則訊息會在[建立](#create)或[DoModal](#domodal)呼叫期間傳送至屬性工作表, 這會在屬性工作表顯示之前立即發生。
...
*pPage*<br/>
指向要從屬性工作表中移除的頁面。 不可以是 NULL。

*nPage*<br/>
要移除之頁面的索引。 必須介於0到 1 (含) 的屬性工作表中的頁面數之間。

### <a name="remarks"></a>備註

在關閉`CPropertySheet`視窗的擁有者之前, 不會終結 [CPropertyPage](../../mfc/reference/cpropertypage-class.md) 物件本身。
...
*nPage*<br/>
要設定之頁面的索引。 它必須介於0到 1 (含) 之間的頁面數目之間。

*pPage*<br/>
指向要在屬性工作表中設定的頁面。 不可以是 Null。
...
例如, 如果使用者`SetActivePage`在單一頁面上的動作應該導致另一個頁面變成使用中的頁面, 請使用。
...
呼叫`SetFinishText`以顯示 [完成] 命令按鈕上的文字, 並在使用者于 wizard 最後一頁完成動作之後, 隱藏 [下一步] 和 [上一頁] 按鈕。
...
*nStyle*<br/>
指定屬性工作表標題的樣式。 樣式必須指定為0或做為 PSH_PROPTITLE。 如果樣式設定為 PSH_PROPTITLE, 則 "Properties" 這個字會出現在指定為標題的文字後面。 例如, 呼叫`SetTitle`("Simple", PSH_PROPTITLE) 會產生屬性工作表標題「簡單屬性」。
...
- PSWIZB_DISABLEDFINISH 已停用完成按鈕

### <a name="remarks"></a>備註

只有`SetWizardButtons`在對話方塊開啟之後才呼叫; 您無法在`SetWizardButtons`呼叫[DoModal](#domodal)之前呼叫。 一般來說, 您應該從`SetWizardButtons` [CPropertyPage:: OnSetActive](../../mfc/reference/cpropertypage-class.md#onsetactive)呼叫。
...
有三個 wizard 屬性頁: `CStylePage`、 `CColorPage`和`CShapePage`。 `CPropertySheet`  下面的程式碼片段顯示如何啟用和停用 wizard 屬性頁上的 [**上一頁**] 和 **[下一步]** 按鈕。
...
在`SetWizardMode`呼叫[DoModal](#domodal)之前呼叫。 呼叫`SetWizardMode`之後, `DoModal`會傳回其中一個 ID_WIZFINISH (如果使用者以 [完成] 按鈕關閉) 或 IDCANCEL。
