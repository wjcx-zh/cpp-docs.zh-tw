...
> [!NOTE]
>  如果您已實作大量資料列擷取，您不能呼叫`AddNew`。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
>  動態集，新的記錄會新增至資料錄集，為最後一筆記錄。 加入的記錄不會加入至快照集;您必須呼叫**Requery**重新整理資料錄集。  
 ...
 如需詳細資訊，請參閱下列文章︰[資料錄集︰ 資料錄集更新資料錄的方式 (ODBC)](../../data/odbc/recordset-how-recordsets-update-records-odbc.md)，[資料錄集︰ 加入、 更新和刪除資料錄 (ODBC)](../../data/odbc/recordset-adding-updating-and-deleting-records-odbc.md)，和[異動 (ODBC)](../../data/odbc/transaction-odbc.md)。  
...
> [!NOTE]
>  此成員函式不適用於使用大量資料列擷取，因為無法呼叫這類資料錄集的資料錄集**編輯**， `AddNew`，或**更新**。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
|`SQL_STILL_EXECUTING`|以非同步方式啟動的函式仍在執行中。 請注意，根據預設，MFC 將永遠不會將此值傳遞至`CheckRowsetError`;MFC 將繼續呼叫**SQLExtendedFetch**之前不會再傳回`SQL_STILL_EXECUTING`。|  
 ...
 如需有關**SQLError**，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!NOTE]
>  如果您已實作大量資料列擷取，您不能呼叫**刪除**。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> `DoBulkFieldExchange`使用衍生自的類別時，才可以使用`CRecordset`。 如果您已經建立的資料錄集物件，直接從`CRecordset`，您必須呼叫[GetFieldValue](#getfieldvalue)成員函式來擷取資料。  
 ...
 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。 如需相關資訊，請參閱文章[資料錄欄位交換 (RFX)](../../data/odbc/record-field-exchange-rfx.md)。  
...
> [!NOTE]
>  如果您已實作大量資料列擷取，您不能呼叫**編輯**。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
### <a name="remarks"></a>備註  
 ...
 `FlushResultSet`呼叫 ODBC API 函式`SQLMoreResults`。 如果`SQLMoreResults`傳回`SQL_ERROR`或`SQL_INVALID_HANDLE`，然後`FlushResultSet`將會擲回例外狀況。 如需有關`SQLMoreResults`，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
> [!CAUTION]
>  資料表名稱將為空白，架構無法識別資料表名稱，如果提供了多個資料表名稱，或如果**呼叫**陳述式無法解譯。 請注意，當使用**呼叫**陳述式，必須插入大括號之間的空白字元和**呼叫**關鍵字，也應該插入大括號之前或之前的空白字元不**選取**關鍵字**選取**陳述式。  
...
|**SQL_C_BINARY**|**SQL_BINARYSQL_VARBINARYSQL_LONGVARBINARY**|  
 ...
 如需 ODBC 資料類型的詳細資訊，請參閱主題 < SQL 資料類型 > 和 < C 資料類型 > 中的 < 附錄 D [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
>  如果您宣告的資料錄集物件而不需從衍生`CRecordset`，不需要 ODBC 資料指標程式庫載入。 資料指標程式庫，必須具備資料錄集至少一個繫結的資料行;不過，當您使用`CRecordset`直接，沒有資料行已繫結。 成員函式[CDatabase::OpenEx](../../mfc/reference/cdatabase-class.md#openex)和[CDatabase::Open](../../mfc/reference/cdatabase-class.md#open)控制是否會載入資料指標程式庫。  
 ...
 `GetFieldValue`呼叫 ODBC API 函式**SQLGetData**。 如果您的驅動程式會輸出值**SQL_NO_TOTAL**欄位值的實際長度`GetFieldValue`擲回例外狀況。 如需有關**SQLGetData**，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
>  不同於 DAO 類別`CDaoRecordset`，`CRecordset`沒有`SetFieldValue`成員函式。 如果您建立的物件，直接從`CRecordset`，它是有效唯讀的。  
 ...
 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
### <a name="remarks"></a>備註  
 ...
 結果`IsDeleted`取決於許多因素，例如您的資料錄集類型，資料錄集是否可更新，您指定是否**CRecordset::skipDeletedRecords**選項開啟資料錄集時，是否驅動程式套件中刪除記錄時，是否有多個使用者。  
  
 如需有關**CRecordset::skipDeletedRecords**和驅動程式封裝，請參閱[開啟](#open)成員函式。  
  
> [!NOTE]
>  如果您已實作大量資料列擷取，您不應該呼叫`IsDeleted`。 請改為呼叫[GetRowStatus](#getrowstatus)成員函式。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!NOTE]
>  此成員函式不適用於使用大量資料列擷取的資料錄集。 如果您已實作大量資料列擷取，然後`IsFieldDirty`一律會傳回**FALSE**而導致失敗的判斷提示。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!NOTE]
>  此成員函式不適用於使用大量資料列擷取的資料錄集。 如果您已實作大量資料列擷取，然後`IsFieldNull`一律會傳回**FALSE**而導致失敗的判斷提示。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!NOTE]
>  如果您已實作大量資料列擷取，您不能呼叫`IsFieldNullable`。 請改為呼叫[GetODBCFieldInfo](#getodbcfieldinfo)成員函式來判斷欄位是否可以設定為 Null 值。 請注意，您一律可以呼叫`GetODBCFieldInfo`，不論您是否有實作大量資料列擷取。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
### <a name="remarks"></a>備註  
 ...
 其中排序規格是整數或資料行名稱。 您也可以將"ASC"或"DESC"附加到資料行清單排序字串中指定遞增或遞減順序 （預設為遞增順序）。 選取的記錄會先排序第一個資料行列出，然後是第二個，依此類推。 例如，您可能會排序姓氏，然後再按照名字的 「 客戶 」 資料錄的集。 您可以列出的資料行數目取決於資料來源。 如需詳細資訊，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
|`SQL_FETCH_ABSOLUTE`|如果`nRows`> 0，資料列集啟動`nRows`從一開始的資料錄集的資料列。 如果`nRows` < 0,="" the="" rowset="" starting=""> `nRows`個資料列從資料錄集的結尾。 如果`nRows`= 0，則會傳回開頭的檔案 (BOF) 條件。|[SetAbsolutePosition](#setabsoluteposition)|  
...
>  如果您呼叫**移動**目前的記錄時，更新或加入更新都將遺失，而不發出警告。  
 ...
 如需有關資料錄集瀏覽的詳細資訊，請參閱文章[資料錄集︰ 捲動 (ODBC)](../../data/odbc/recordset-scrolling-odbc.md)和[資料錄集︰ 書籤和絕對位置 (ODBC)](../../data/odbc/recordset-bookmarks-and-absolute-positions-odbc.md)。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。 如需相關資訊，請參閱 ODBC API 函式**SQLExtendedFetch**中[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
>  如果您呼叫任何**移動**函式時，目前的記錄更新或加入更新都將遺失，而不發出警告。  
 ...
 如需有關資料錄集瀏覽的詳細資訊，請參閱文章[資料錄集︰ 捲動 (ODBC)](../../data/odbc/recordset-scrolling-odbc.md)和[資料錄集︰ 書籤和絕對位置 (ODBC)](../../data/odbc/recordset-bookmarks-and-absolute-positions-odbc.md)。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
- `CRecordset::useMultiRowFetch`實作大量資料列擷取為允許在單一擷取作業中要擷取多個資料列。 這是一項進階的功能設計是為了增進效能;不過，大量資料錄欄位交換不受到 ClassWizard。 這個選項是互斥的**CRecordset::optimizeBulkAdd**。 請注意，如果您指定`CRecordset::useMultiRowFetch`，然後選擇**CRecordset::noDirtyFieldCheck**會自動開啟 （雙重緩衝將無法使用）; 在順向資料錄集，此選項**CRecordset::useExtendedFetch**會自動開啟。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!CAUTION]
>  請小心，不執行 SQL 字串中插入的額外空白。 例如，如果您要插入大括號之間的空白字元和**呼叫**關鍵字，MFC 會錯誤解譯為資料表名稱的 SQL 字串，並將它**選取**陳述式，這會導致擲回例外狀況。 同樣地，如果您預先定義的查詢會使用輸出參數，您不能插入大括號之間的空白字元和 ' 符號。 最後，您必須插入之前的大括號中的空白字元**呼叫**陳述式或之前**選取**關鍵字**選取**陳述式。  
 ...
 任何您要傳入，**開啟**建構查詢的最終 SQL 字串 (這個字串可能有 SQL**其中**和**ORDER BY**子句附加至`lpszSQL`您傳遞的字串)，然後執行查詢。 您可以藉由呼叫檢查建構的字串[GetSQL](#getsql)之後呼叫**開啟**。 其他詳細資料錄集如何建構 SQL 陳述式和選取的記錄，請參閱文章[資料錄集︰ 如何資料錄集選取資料錄 (ODBC)](../../data/odbc/recordset-how-recordsets-select-records-odbc.md)。  
...
|`SQL_LOCK_UNLOCK`|驅動程式或資料來源會解除鎖定資料列。 並非所有的資料來源都支援這種類型的鎖定。|  
 ...
 如需有關**SQLSetPos**，請參閱[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!NOTE]
>  絕對位置不是要做為 surrogate 資料錄數目。 書籤仍保有與之後刪除先前的記錄時，記錄的位置變更至指定的位置，傳回的建議的方式。 此外，您無法確定指定的記錄會有相同的絕對位置，如果資料錄集時都會重新建立一次資料錄集內的個別記錄的順序不保證除非建立與使用 SQL 陳述式因為**ORDER BY**子句。  
...
> [!NOTE]
>  此成員函式不適用於使用大量資料列擷取的資料錄集。 如果您已實作大量資料列擷取，然後`SetFieldDirty`會導致失敗的判斷提示。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
  
 Framework 標記變更欄位資料成員，以確保它們將寫入至資料來源上記錄的資料錄欄位交換 (RFX) 機制。 一般來說，變更欄位的值欄位已變更都會自動設定，因此您很少需要呼叫`SetFieldDirty`自己，不過您有時可能會想要確保的資料行會明確地更新或插入不論哪些值是將欄位資料成員中。  
...
> [!NOTE]
>  請勿使用大量資料列擷取的資料錄集上呼叫此成員函式。 如果您已實作大量資料列擷取，則呼叫`SetFieldNull`導致失敗的判斷提示。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
...
> [!NOTE]
>  呼叫時請特別小心`SetRowsetSize`。 如果您以手動方式配置的資料存放區 (依指定**CRecordset::userAllocMultiRowBuffers** dwOptions 參數中的選項**開啟**)，您應該檢查您是否需要重新配置儲存體的緩衝區之後您呼叫, `SetRowsetSize`，但之前執行任何資料指標巡覽作業。  
...
> [!NOTE]
>  如果您已實作大量資料列擷取，您不能呼叫**更新**。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱文章[資料錄集︰ 擷取記錄中大量 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)。  
