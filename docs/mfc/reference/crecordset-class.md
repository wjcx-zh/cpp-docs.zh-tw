...
-   參數化資料錄集，直到執行階段不知道的資訊來自訂其選項。  
 ...
 若要使用您的類別，開啟資料庫，並建構資料錄集物件，傳遞建構函式的指標，您`CDatabase`物件。 然後呼叫資料錄集的`Open`成員函式，供您指定的物件是否動態集或快照集。 呼叫`Open`選取資料來源中的資料。 開啟資料錄集物件之後，使用其成員函式和資料成員來捲動記錄，並在其上運作。 可用的作業取決於物件是動態集或快照集，它是可更新或唯讀狀態 （這取決於開放式資料庫連接 (ODBC) 資料來源的功能），以及您是否有實作大量資料列擷取。 若要重新整理記錄，可能會變更或加入之後`Open`呼叫，呼叫物件的`Requery`成員函式。 呼叫物件的`Close`成員函式，並終結物件，當您完成使用它。  
...
### <a name="remarks"></a>備註  
 您必須呼叫[Requery](#requery)成員函式，以查看新加入的記錄。 記錄的欄位是一開始為 Null。 (在資料庫詞彙中，Null 表示不擁有任何值 」，而且不是相同**NULL** c + + 中。)若要完成此作業，您必須呼叫[更新](#update)成員函式。 `Update` 將您的變更儲存至資料來源。  
...
>  如果您已實作大量資料列擷取，您不能呼叫`AddNew`。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱 [資料錄集：擷取大量資料錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)一文。  
 ...
 `AddNew` 準備新的空白記錄使用資料錄集的欄位資料成員。 在您呼叫後`AddNew`，設定資料錄集的欄位資料成員中您想要的值。 (您不需要呼叫[編輯](#edit)針對此用途的成員函式; 請改用`Edit`僅針對現有的記錄。)當您後續呼叫`Update`、 已變更的欄位資料成員中的值會儲存在資料來源上。  
...
>  動態集，新的記錄會新增至資料錄集，為最後一筆記錄。 加入的記錄不會加入至快照集;您必須呼叫`Requery`重新整理資料錄集。  
 ...
 不合法呼叫`AddNew`資料錄集其`Open`尚未呼叫成員函式。 A`CDBException`如果您呼叫會擲回`AddNew`以無法附加至資料錄集。 您可以判斷資料錄集藉由呼叫是否可以更新[CanAppend](#canappend)。  
...
>  此成員函式不適用於使用大量資料列擷取，因為無法呼叫這類資料錄集的資料錄集`Edit`， `AddNew`，或`Update`。 如需大量資料列擷取的詳細資訊，請參閱 [資料錄集：擷取大量資料錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)一文。  
 ...
 如果啟用自動變更欄位檢查時，`CancelUpdate`之前的值將會還原成員變數`Edit`或`AddNew`呼叫，否則，值的任何變更將會繼續。 根據預設，自動欄位啟用檢查開啟資料錄集時。 若要停用它，您必須指定**CRecordset::noDirtyFieldCheck**中*dwOptions*參數[開啟](#open)成員函式。  
...
### <a name="remarks"></a>備註  
 ...
 `CheckRowsetError` 會自動呼叫在資料指標巡覽作業，例如`Open`， `Requery`，或任何`Move`作業。 ODBC API 函式的傳回值傳遞**SQLExtendedFetch**。 下表列出可能的值為`nRetCode`參數。  
...
### <a name="remarks"></a>備註  
 ODBC **HSTMT**及所有配置給資料錄集的架構會被取消配置的記憶體。 通常在呼叫`Close`，您會刪除 c + + 資料錄集物件，如果其配置具有**新**。  
  
 您可以呼叫`Open`呼叫之後，再次`Close`。 這可讓您重複使用資料錄集物件。 替代方法是呼叫`Requery`。  
...
### <a name="parameters"></a>參數  
 *pDatabase*  
 包含的指標`CDatabase`物件或值**NULL**。 如果沒有**NULL**和`CDatabase`物件的`Open`尚未呼叫成員函式將它連接到資料來源，資料錄集會嘗試為您開啟自己期間`Open`呼叫。 如果您要傳入**NULL**、`CDatabase`物件建構，且您使用您指定當您衍生 ClassWizard 資料錄集類別的資料來源資訊的連接。  
...
### <a name="remarks"></a>備註  
 之後成功刪除資料錄集的欄位資料成員都會設定為 Null 值，以及您必須明確呼叫其中一個`Move`函式，才能移出已刪除的記錄。 一旦您將之移除刪除的記錄，它不傳回它。 如果資料來源支援交易，您可以進行`Delete`呼叫交易的一部分。 如需詳細資訊，請參閱文章[異動 (ODBC)](../../data/odbc/transaction-odbc.md)。  
...
> [!CAUTION]
>  資料錄集必須是可更新而且必須有有效的記錄資料錄集中目前當您呼叫`Delete`; 否則就會發生錯誤。 例如，如果您刪除記錄，但不是會捲動到新的記錄才能呼叫`Delete`同樣地，`Delete`會擲回[CDBException](../../mfc/reference/cdbexception-class.md)。  
  
 不同於[AddNew](#addnew)和[編輯](#edit)，呼叫`Delete`後面沒有接呼叫[更新](#update)。 如果`Delete`呼叫失敗，欄位資料成員則會保留不變。  
...
> `DoBulkFieldExchange` 使用衍生自的類別時，才可以使用`CRecordset`。 如果您已經建立的資料錄集物件，直接從`CRecordset`，您必須呼叫[GetFieldValue](#getfieldvalue)成員函式來擷取資料。  
 ...
 大量資料錄欄位交換 (Bulk RFX) 很相似資料錄欄位交換 (RFX)。 資料來源的資料錄集物件自動傳輸資料。 不過，您不能呼叫`AddNew`， `Edit`， `Delete`，或`Update`来傳送回資料來源的變更。 類別`CRecordset`目前不提供機制來更新大量資料列的資料; 不過，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。  
...
### <a name="remarks"></a>備註  
 在您呼叫後`Edit`，您可以藉由直接重設其值變更的欄位資料成員。 在作業完成時，後續呼叫[更新](#update)成員函式來儲存資料來源上的變更。  
...
>  如果您已實作大量資料列擷取，您不能呼叫`Edit`。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱 [資料錄集：擷取大量資料錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)一文。  
 ...
 `Edit` 將儲存的資料錄集的資料成員的值。 如果您呼叫`Edit`，進行變更，然後呼叫`Edit`同樣地，記錄的值就會還原至原本在第一個`Edit`呼叫。  
 ...
 如果資料來源支援交易，您可以進行`Edit`呼叫交易的一部分。 請注意，您應該呼叫[CDatabase::BeginTrans](../../mfc/reference/cdatabase-class.md#begintrans)之前先呼叫`Edit`及開啟資料錄集之後。 也請注意，呼叫[CDatabase::CommitTrans](../../mfc/reference/cdatabase-class.md#committrans)並非用於呼叫的替代做法`Update`完成`Edit`作業。 如需有關交易的詳細資訊，請參閱類別[CDatabase](../../mfc/reference/cdatabase-class.md)。  
  
 根據目前的鎖定模式，正在更新的記錄可能被鎖住`Edit`直到您呼叫`Update`或捲動到另一筆記錄，或可能只有在已鎖定`Edit`呼叫。 您可以變更鎖定模式與[SetLockingMode](#setlockingmode)。  
  
 當您捲動到新的記錄，然後再呼叫，會還原先前的值，目前資料錄的`Update`。 A`CDBException`如果您呼叫會擲回`Edit`無法更新資料錄集，或如果沒有目前記錄。  
...
> [!NOTE]
>  特定資料錄集作業之後，書籤可能不再有效。 例如，如果您呼叫`GetBookmark`後面`Requery`，您可能無法傳回與記錄`SetBookmark`。 呼叫[CDatabase::GetBookmarkPersistence](../../mfc/reference/cdatabase-class.md#getbookmarkpersistence)來檢查您是否能安全地呼叫`SetBookmark`。  
...
> [!CAUTION]
>  記錄計數維持為 「 上限標記，「 最高編號的記錄，但出現使用者記錄的移動。 使用者動作已超越最後一筆記錄之後，只已知的記錄總數。 基於效能考量，計數不會更新當您呼叫`MoveLast`。 若要自行計算記錄，請呼叫`MoveNext`重複直到`IsEOF`傳回非零值。 將記錄新增到透過`CRecordset:AddNew`和`Update`計數，會增加，刪除記錄，以透過`CRecordset::Delete`減少計數。  
...
### <a name="remarks"></a>備註  
 ...
 所傳回的字串`GetSQL`通常不同於您可能有傳送中資料錄集到的任何字串*lpszSQL*參數`Open`成員函式。 這是因為資料錄集建構完整的 SQL 陳述式，根據您傳遞給`Open`，ClassWizard，您可能必須以指定的內容與指定的內容**m_strFilter**和`m_strSort`資料成員，以及任何您已指定的參數。 針對詳細資料錄集如何建構 SQL 陳述式，請參閱文章[資料錄集： 如何資料錄集選取資料錄 (ODBC)](../../data/odbc/recordset-how-recordsets-select-records-odbc.md)。  
...
### <a name="remarks"></a>備註  
 您從資料錄捲動記錄，以了解是否有消失之前的資料錄集的第一個記錄之前，請呼叫此成員函式。 您也可以使用`IsBOF`連同`IsEOF`來判斷資料錄集是否包含任何記錄，或為空白。 之後立即呼叫`Open`，如果資料錄集不包含任何記錄，`IsBOF`傳回非零值。當您開啟具有至少一個資料錄的資料錄集時，第一筆記錄是目前的記錄和`IsBOF`傳回 0。  
...
### <a name="remarks"></a>備註  
 當您從資料錄捲動記錄，以了解是否您已經超過資料錄集的最後一筆記錄，請呼叫此成員函式。 您也可以使用`IsEOF`來判斷資料錄集是否包含任何記錄，或為空白。 之後立即呼叫`Open`，如果資料錄集不包含任何記錄，`IsEOF`傳回非零值。 當您開啟具有至少一個資料錄的資料錄集時，第一筆記錄是目前的記錄和`IsEOF`傳回 0。  
...
### <a name="return-value"></a>傳回值  
 為非零，如果指定的欄位資料成員已變更後呼叫`AddNew`或`Edit`; 否則為 0。  
  
### <a name="remarks"></a>備註  
 所有已變更的欄位資料成員中的資料會傳輸記錄資料來源上呼叫更新目前的記錄時[更新](#update)成員函式`CRecordset`(呼叫`Edit`或`AddNew`).  
...
>  此成員函式不適用於使用大量資料列擷取的資料錄集。 如果您已實作大量資料列擷取，然後`IsFieldDirty`一律會傳回**FALSE**而導致失敗的判斷提示。 如需大量資料列擷取的詳細資訊，請參閱 [資料錄集：擷取大量資料錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)一文。  
 ...
 呼叫`IsFieldDirty`會重設先前呼叫的效果[SetFieldDirty](#setfielddirty)因為欄位已變更狀態重新進行評估。 在`AddNew`的情況下，如果目前的欄位值不同於虛擬 null 值，欄位狀態會設定已變更。 在`Edit`情況下，如果欄位值不同於快取的值，則欄位狀態會設定已變更。  
...
>  如果您已實作大量資料列擷取，您不能呼叫`IsFieldNullable`。 請改為呼叫[GetODBCFieldInfo](#getodbcfieldinfo)成員函式來判斷欄位是否可以設定為 Null 值。 請注意，您一律可以呼叫`GetODBCFieldInfo`，不論您是否有實作大量資料列擷取。 如需大量資料列擷取的詳細資訊，請參閱 [資料錄集：擷取大量資料錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)一文。  
 ...
 不能是 Null 的欄位必須有值。 如果您嘗試將這類欄位設定為 Null，在加入或更新記錄時，資料來源會拒絕的新增或更新，以及[更新](#update)將會擲回例外狀況。 當您呼叫時，就會發生例外狀況`Update`，不會在您呼叫[SetFieldNull](#setfieldnull)。  
...
##  <a name="m_strfilter"></a>  CRecordset::m_strFilter  
 但建構資料錄集物件之後, 才能呼叫其`Open`成員函式，來儲存使用此資料成員`CString`包含 SQL**其中**子句。  
  
### <a name="remarks"></a>備註  
 資料錄集來限制 （或篩選） 就會選取期間的記錄會使用此字串`Open`或`Requery`呼叫。 這適用於選取子集的記錄，例如 「 位於加州的舊型的所有銷售人員 」 (「 狀態 = CA")。 ODBC SQL 語法**其中**子句  
...
##  <a name="m_strsort"></a>  CRecordset::m_strSort  
 但建構資料錄集物件之後, 才能呼叫其`Open`成員函式，來儲存使用此資料成員`CString`包含 SQL **ORDER BY**子句。  
  
### <a name="remarks"></a>備註  
 資料錄集使用這個字串，排序它所選取期間的記錄`Open`或`Requery`呼叫。 您可以使用這項功能，來排序一個或多個資料行的資料錄集。 ODBC SQL 語法**ORDER BY**子句  
 ...
 *wFetchType*  
 判斷資料列集的`Move`將會提取。 如需詳細資料，請參閱＜備註＞。  
  
### <a name="remarks"></a>備註  
 如果您要傳入的值為 0 *nRows*，`Move`重新整理目前的記錄。`Move`會結束目前`AddNew`或`Edit`模式中，而且會還原之前的目前記錄的值`AddNew`或`Edit`呼叫。  
  
> [!NOTE]
>  當您移動的資料錄集時，無法略過已刪除的記錄。 請參閱[CRecordset::IsDeleted](#isdeleted)如需詳細資訊。 當您開啟`CRecordset`與**skipDeletedRecords**選項集，`Move`判斷提示，如果*nRows*參數是 0。 此行為可避免資料列刪除的其他用戶端應用程式使用相同的資料重新整理。 請參閱*dwOption*中的參數[開啟](#open)的說明**skipDeletedRecords**。  
  
 `Move` 由資料列集，重新定位資料錄集。 根據值*nRows*和*wFetchType*，`Move`擷取適當資料列集，然後將第一筆記錄中該資料列集的目前記錄。 如果您未實作大量資料列擷取，然後資料列集大小永遠為 1。 在提取資料列集時，`Move`直接呼叫[CheckRowsetError](#checkrowseterror)成員函式，以處理所產生的擷取任何錯誤。  
  
 取決於您傳遞，值`Move`相當於其他`CRecordset`成員函式。 特別是，值*wFetchType*可能表示是更具直覺性的成員函式，並經常移動目前的資料錄的慣用的方法。  
  
 下表列出可能的值為*wFetchType*，資料列集的`Move`將會提取根據*wFetchType*和*nRows*，任何對等項目成員函式對應至*wFetchType*。  
...
> [!NOTE]
>  順向資料錄集的`Move`只適用於其值為`SQL_FETCH_NEXT`如`wFetchType`。  
  
> [!CAUTION]
>  呼叫`Move`擲回例外狀況，如果資料錄集不有任何記錄。 若要判斷資料錄集是否有任何記錄，請呼叫[IsBOF](#isbof)和[IsEOF](#iseof)。  
  
> [!NOTE]
>  如果您有捲動過去的開頭或結尾的資料錄集 (`IsBOF`或`IsEOF`傳回非零)，則呼叫`Move`函式可能會擲回`CDBException`。 例如，如果`IsEOF`傳回非零值和`IsBOF`不存在，然後`MoveNext`將會擲回例外狀況，但`MovePrev`則不會。  
  
> [!NOTE]
>  如果您呼叫`Move`目前的記錄時，更新或加入更新都將遺失，而不發出警告。  
...
> [!CAUTION]
>  呼叫上述任一`Move`函式擲回例外狀況，如果資料錄集不有任何記錄。 若要判斷資料錄集是否有任何記錄，請呼叫`IsBOF`和`IsEOF`。  
  
> [!NOTE]
>  如果您呼叫任何`Move`函式時，目前的記錄更新或加入更新都將遺失，而不發出警告。  
...
- **CRecordset::appendOnly**不允許`Edit`或`Delete`資料錄集。 允許`AddNew`只。 這個選項是互斥的**CRecordset::readOnly**。  
...
- **CRecordset::skipDeletedRecords**巡覽資料錄集時，略過所有已刪除的記錄。 這會降低效能，在某些相對提取。 這個選項不在順向資料錄集上有效。 如果您呼叫[移動](#move)與*nRows*參數設定為 0，而**CRecordset::skipDeletedRecords**選項集，`Move`會判斷提示。 請注意， **CRecordset::skipDeletedRecords**類似於*驅動程式封裝*，也就是說，已刪除資料列會從資料錄集。 不過，如果您的驅動程式封裝的記錄，然後它會略過只有在您刪除; 的記錄它不會略過資料錄集開啟時，由其他使用者刪除記錄。 **CRecordset::skipDeletedRecords**會略過其他使用者刪除的資料列。  
...
- **CRecordset::useExtendedFetch**實作`SQLExtendedFetch** instead of `SQLFetch`. This is designed for implementing bulk row fetching on forward-only recordsets. If you specify the option `CRecordset::useMultiRowFetch' 順向資料錄集，然後**CRecordset::useExtendedFetch**將會開啟自動的。  
...
### <a name="remarks"></a>備註  
 您必須呼叫此成員函式，以執行查詢資料錄集所定義。 然後再呼叫`Open`，您必須建構資料錄集物件。  
  
 這個資料錄集的資料來源連接，取決於您如何建構資料錄集，然後再呼叫`Open`。 如果您要傳入[CDatabase](../../mfc/reference/cdatabase-class.md)物件尚未連接到資料來源，資料錄集建構函式會使用此成員函式[GetDefaultConnect](#getdefaultconnect)嘗試開啟的資料庫物件。 如果您要傳入**NULL**資料錄集建構函式建構函式會建構`CDatabase`物件，和`Open`來嘗試連接的資料庫物件。 如需關閉資料錄集，並在這些不同的情況下連接的詳細資訊，請參閱[關閉](#close)。  
  
> [!NOTE]
>  透過資料來源的存取`CRecordset`一律共用物件。 不同於`CDaoRecordset`類別，您不能使用`CRecordset`物件具有獨佔存取權開啟資料來源。  
  
 當您呼叫`Open`，查詢時，通常是 SQL**選取**陳述式中，選取下表所示的準則為基礎的記錄。  
...
>  請小心，不執行 SQL 字串中插入的額外空白。 例如，如果您要插入大括號之間的空白字元和**呼叫**關鍵字，MFC 會錯誤解譯為資料表名稱的 SQL 字串，並將它**選取**陳述式，這會導致擲回的例外狀況。 同樣地，如果您預先定義的查詢會使用輸出參數，您不能插入大括號之間的空白字元和 ' 符號。 最後，您必須插入之前的大括號中的空白字元**呼叫**陳述式或之前**選取**關鍵字**選取**陳述式。  
 ...
 一般的程序會傳遞**NULL**至`Open`; 在此情況下，`Open`呼叫[GetDefaultSQL](#getdefaultsql)。 如果您使用衍生`CRecordset`類別**GetDefaultSQL**可讓您在 類別精靈中指定的資料表名稱。 您可以改為指定中的其他資訊`lpszSQL`參數。  
  
 無論您將傳遞，`Open`建構查詢的最終 SQL 字串 (這個字串可能有 SQL**其中**和**ORDER BY**子句附加至`lpszSQL`您傳遞的字串)，然後執行查詢。 您可以藉由呼叫檢查建構的字串[GetSQL](#getsql)之後呼叫 *`Open`。 其他詳細資料錄集如何建構 SQL 陳述式和選取的記錄，請參閱文章[資料錄集： 如何資料錄集選取資料錄 (ODBC)](../../data/odbc/recordset-how-recordsets-select-records-odbc.md)。  
  
 資料錄集類別的欄位資料成員的資料行之選取的資料繫結。 如果傳回任何記錄，第一筆記錄會變成目前的記錄。  
  
 如果您想要設定資料錄集篩選或排序，例如在建構資料錄集物件之後但在呼叫前先指定這些`Open`。 如果您想要重新整理資料錄集之後的記錄資料錄集已經開啟，請呼叫[Requery](#requery)。  
...
- **開放式**開放式鎖定鎖定期間的呼叫只更新資料錄`Update`。  
  
- **封閉式**封閉式鎖定會鎖定資料錄儘速`Edit`呼叫，並保留該鎖定直到`Update`呼叫完成，或移至新的記錄。  
  
### <a name="remarks"></a>備註  
 如果您需要指定哪一個資料錄集用來更新兩種記錄鎖定的策略，請呼叫此成員函式。 根據預設，資料錄集的鎖定模式是**開放式**。 您可以變更，更小心**封閉式**鎖定策略。 呼叫`SetLockingMode`建構，並開啟資料錄集物件之後但在您呼叫之前`Edit`。  
...
>  呼叫`SetRowsetSize`而不需要實作大量資料列擷取將會導致失敗的判斷提示。  
 ...
 呼叫`SetRowsetSize`之前先呼叫`Open`一開始設定資料錄集的資料列集大小。 實作大量資料列擷取時的預設資料列集大小為 25。  
  
> [!NOTE]
>  呼叫時請特別小心`SetRowsetSize`。 如果您以手動方式配置的資料存放區 (依指定**CRecordset::userAllocMultiRowBuffers** dwOptions 參數中的選項`Open`)，您應該檢查是否需要重新配置這些儲存體緩衝處理後呼叫`SetRowsetSize`，但之前執行任何資料指標巡覽作業。  
...
##  <a name="update"></a>  CRecordset::Update  
 完成`AddNew`或`Edit`藉由將新的或編輯資料儲存在資料來源上的作業。  
...
### <a name="remarks"></a>備註  
 呼叫此成員函式呼叫之後[AddNew](#addnew)或[編輯](#edit)成員函式。 此呼叫，才能完成`AddNew`或`Edit`作業。  
  
> [!NOTE]
>  如果您已實作大量資料列擷取，您不能呼叫`Update`。 這會導致失敗的判斷提示。 雖然類別`CRecordset`不會提供一個機制來更新資料的大量資料列，您可以撰寫自己的函式使用 ODBC API 函式**SQLSetPos**。 如需大量資料列擷取的詳細資訊，請參閱 [資料錄集：擷取大量資料錄 (ODBC)](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md)一文。  
  
 同時`AddNew`和`Edit`準備編輯緩衝區，用來放置新增或編輯過的資料儲存到資料來源。 `Update` 儲存資料。 只有這些欄位標記，或偵測到變更時都會更新。  
  
 如果資料來源支援交易，您可以進行`Update`呼叫 (和其相對應`AddNew`或`Edit`呼叫) 交易的一部分。 如需有關交易的詳細資訊，請參閱文章[異動 (ODBC)](../../data/odbc/transaction-odbc.md)。  
  
> [!CAUTION]
>  如果您呼叫`Update`但未先呼叫`AddNew`或`Edit`，`Update`會擲回`CDBException`。 如果您呼叫`AddNew`或`Edit`，您必須呼叫`Update`之前先呼叫`Move`作業或之前關閉資料錄集或資料來源連接。 否則，您的變更都會遺失而不另行通知。  
  
 如需詳細資訊，處理`Update`失敗，請參閱文章[資料錄集： 資料錄集更新資料錄的方式 (ODBC)](../../data/odbc/recordset-how-recordsets-update-records-odbc.md)。  
