...
### <a name="remarks"></a>備註  
 您建構`CRichEditCtrl`兩個步驟中的物件。 首先，呼叫[CRichEditCtrl](#cricheditctrl)建構函式，然後呼叫`Create`，建立 Windows 編輯控制項，並將它附加至`CRichEditCtrl`物件。  
 ...
 當`Create`執行時，Windows 會傳送[WM_NCCREATE](../../mfc/reference/cwnd-class.md#onnccreate)， [WM_NCCALCSIZE](../../mfc/reference/cwnd-class.md#onnccalcsize)， [WM_CREATE](../../mfc/reference/cwnd-class.md#oncreate)，和[WM_GETMINMAXINFO](../../mfc/reference/cwnd-class.md#ongetminmaxinfo)編輯控制項的訊息。  
...
### <a name="parameters"></a>參數  
 *則 nCode*  
 表示要採取的動作。 如需可能值的清單，請參閱參數描述*程式碼*中**EM_FINDWORDBREAK** Windows SDK 中。  
  
 *nStart*  
 開始處之以零為起始的字元位置。  
  
### <a name="return-value"></a>傳回值  
 根據參數*則 nCode*。 如需詳細資訊，請參閱[EM_FINDWORDBREAK](http://msdn.microsoft.com/library/windows/desktop/bb788018) Windows SDK 中。  
...
### <a name="return-value"></a>傳回值  
 在指定的行的字元索引*上線*或-1 表示大於指定的行號，然後在編輯控制項中的行數。  
...
### <a name="return-value"></a>傳回值  
 當`LineLength`稱為多行編輯控制項，則傳回值是長度 (在`TCHAR`) 所指定的行*上線*。  它不包含行尾的歸位字元。 當`LineLength`稱為單行編輯控制項，則傳回值是長度 (在`TCHAR`) 中編輯控制項的文字。 如果上線大於控制項中的字元數，則傳回值為零。
...
- `ECOOP_XOR` 邏輯互斥，或是目前的選項，與所指定的*dwFlags*。  
