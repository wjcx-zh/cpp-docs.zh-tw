...
|[CStatusBarCtrl::CreateEx](#createex)|建立具有指定之 Windows 擴充樣式的狀態列控制項, 並將其附加至`CStatusBarCtrl`物件。|
|[CStatusBarCtrl::DrawItem](#drawitem)|當主控描繪狀態列控制項的視覺外觀變更時呼叫。|
|[CStatusBarCtrl::GetBorders](#getborders)|抓取狀態列控制項的水準和垂直框線的目前寬度。|
...
|[CStatusBarCtrl::SetSimple](#setsimple)|指定狀態列控制項是否顯示簡單的文字, `SetParts`或顯示先前呼叫所設定的所有控制群組件。|
...
*pParentWnd*<br/>
指定狀態列控制項的父視窗, 通常是`CDialog`。 不得為 Null。
...
您可以使用`CStatusBarCtrl`兩個步驟來建立。 首先, 呼叫此函式, 然後呼叫`Create`, 它會建立狀態列控制項並將其附加`CStatusBarCtrl`至物件。

狀態視窗的預設位置沿著父視窗的底部, 但您可以指定 CCS_TOP 樣式, 讓它出現在父視窗的工作區頂端。 您可以指定 SBARS_SIZEGRIP 樣式, 以在狀態視窗的右端包含調整大小的底框。 不建議合併 CCS_TOP 和 SBARS_SIZEGRIP 樣式, 因為產生的調整大小的底框無法正常運作, 即使系統將它繪製在狀態視窗中也一樣。

若要建立具有延伸視窗樣式的狀態列, 請呼叫[CStatusBarCtrl:: CreateEx](#createex) , `Create`而不是。
...
根據預設, 此成員函式不會執行任何工作。 覆寫這個成員函式, 以執行主控描繪`CStatusBarCtrl`物件的繪圖。
...
這個方法會傳送[SB_GETICON](/windows/win32/Controls/sb-geticon)訊息, 如 Windows SDK 中所述。
...
下列程式碼範例會定義用來`m_statusBar`存取目前狀態列控制項的變數。 下一個範例中會使用此變數。
...
- SBT_NOBORDERS: 不以框線繪製文字。

- SBT_POPOUT: 以框線繪製的文字會比狀態列的平面更高。

- SBT_OWNERDRAW 如果文字具有 SBT_OWNERDRAW 繪製類型, *pType*會接收此訊息, 並傳回與文字相關聯的32位值, 而不是長度和運算類型。
...
此成員函式會執行 Win32 message [SB_GETTIPTEXT](/windows/win32/Controls/sb-gettiptext)的行為, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [SB_ISSIMPLE](/windows/win32/Controls/sb-issimple)的行為, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [SB_SETBKCOLOR](/windows/win32/Controls/sb-setbkcolor)的行為, 如 Windows SDK 中所述。
...
此成員函式會執行 Win32 message [SB_SETICON](/windows/win32/Controls/sb-seticon)的行為, 如 Windows SDK 中所述。
...
*lpszText*<br/>
以 Null 結束的字串位址，其指定要設定的文字。 如果*nType*為 SBT_OWNERDRAW, 則*lpszText*代表32位的資料。
...
此成員函式會執行 Win32 message [SB_SETTIPTEXT](/windows/win32/Controls/sb-settiptext)的行為, 如 Windows SDK 中所述。
