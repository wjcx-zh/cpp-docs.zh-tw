...
下列範例顯示如何使用`Seek` ，將指標1000位元組從檔案的開頭`cfile`移動。 請注意，不會讀取資料，因此您之後必須呼叫 [CStdioFile：：ReadString](#readstring)來讀取資料。`Seek`
