...
### <a name="return-value"></a>傳回值  
 如果成功擷取函式在提供的索引; 的子字串，則為 TRUE。否則為 FALSE。  
  
### <a name="remarks"></a>備註  
 此函式可用於擷取來源字串中的多個子字串時已知的單一字元會分隔每一個子字串。 此函式會搜尋的起點的*lpszFullString*參數每次呼叫時。  
  
 此函式會傳回 FALSE，如果*lpszFullString*設為 NULL 或函式達到結尾*lpszFullString*而不尋找*iSubString*+ 1指定的分隔符號字元的相符項目。 *RString*參數將不會修改從其原始值如果*lpszFullString*已設定為 NULL，否則*rString*如果參數設定為空字串無法擷取指定索引子字串。  
...
### <a name="remarks"></a>備註  
 新建構的字串儲存在*rString*。 例如，如果字串資料表中的字串是 「 檔案 %1 找不到 」 和*lpsz1*等於"C:\MYFILE。TXT"，然後*rString*將包含字串"File C:\MYFILE。TXT 找不到 」。 此函式在為傳送到訊息方塊和其他視窗的字串進行格式化時，便可派上用場。  
...
##  <a name="afxformatstring2"></a>  AfxFormatString2  
 替代字串所指向*lpsz1*字元"%1"的任何執行個體和所指向的字串*lpsz2*字元"%2"之範本字串資源中的任何執行個體由*nIDS*。  
...
### <a name="remarks"></a>備註  
 新建構的字串儲存在*rString*。 例如，如果字串資料表中的字串是 「 檔案 %1 目錄 %2 中找不到 」 *lpsz1*指向 「 MYFILE。TXT"和*lpsz2*指向 「 C:\MYDIR"，然後*rString*將包含字串"File MYFILE。TXT C:\MYDIR 目錄中找不到"  
...
- 選取 IDYES [是] 按鈕。  
 ...
 如果訊息中已取消 5d; 按鈕，在按下 ESC 鍵，或選取取消 5d; 按鈕時，將會傳回 IDCANCEL 值。 如果訊息方塊具有不到 [取消] 按鈕，按 ESC 鍵任何作用。  
