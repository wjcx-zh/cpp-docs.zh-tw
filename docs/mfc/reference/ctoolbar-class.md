---
title: CToolBar 類別 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
3.  呼叫[Create](#create) (或[CreateEx](#createex)) 函式來建立 Windows 工具列，並將其附加至`CToolBar`物件。  
...
2.  呼叫[Create](#create) (或[CreateEx](#createex)) 函式來建立 Windows 工具列，並將其附加至`CToolBar`物件。  
  
3.  呼叫[LoadBitmap](#loadbitmap)載入點陣圖，其中包含工具列按鈕影像。  
  
4.  呼叫[SetButtons](#setbuttons)將按鈕樣式設定，並將每個按鈕點陣圖中的映像產生關聯。  
