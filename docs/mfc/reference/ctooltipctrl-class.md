...
CToolInfo 所`uId`參考之[TOOLINFO](/windows/win32/api/commctrl/ns-commctrl-tttoolinfoa)結構的 和成員可識別此工具。`hwnd` 如果已透過先前的呼叫向`AddTool`工具提示控制項註冊該工具，此`TOOLINFO`結構會填入工具的相關資訊。
