...
|[CWinApp::DoMessageBox](#domessagebox)|為應用程式執行[AfxMessageBox](cstring-formatting-and-message-box-display.md#afxmessagebox) 。|
|[CWinApp::DoWaitCursor](#dowaitcursor)|開啟和關閉等待游標。|
...
|[CWinApp::HtmlHelp](#htmlhelp)|`HTMLHelp`呼叫 Windows 函式。|
...
|[CWinApp::LoadOEMCursor](#loadoemcursor)|載入**OCR_** 常數在 windows 中指定的 windows OEM 預先定義資料指標。H.|
|[CWinApp::LoadOEMIcon](#loadoemicon)|載入**OIC_** 常數在 windows 中指定的 windows OEM 預先定義圖示。H.|
|[CWinApp::LoadStandardCursor](#loadstandardcursor)|載入**IDC_** 常數在 windows 中指定的 windows 預先定義資料指標。H.|
...
|[CWinApp::ParseCommandLine](#parsecommandline)|剖析命令列中的個別參數和旗標。|
|[CWinApp::PreTranslateMessage](#pretranslatemessage)|在訊息分派至 Windows 函式[TranslateMessage](/windows/desktop/api/winuser/nf-winuser-translatemessage)和[DispatchMessage](/windows/desktop/api/winuser/nf-winuser-dispatchmessage)之前, 先進行篩選。|
|[CWinApp::ProcessMessageFilter](#processmessagefilter)|攔截特定訊息, 然後再到達應用程式。|
|[CWinApp::ProcessShellCommand](#processshellcommand)|處理命令列引數和旗標。|
|[CWinApp::ProcessWndProcException](#processwndprocexception)|攔截應用程式的訊息和命令處理常式擲回的所有未處理的例外狀況。|
...
|[CWinApp::Unregister](#unregister)|取消註冊所有已知由物件註冊的`CWinApp`專案。|
|[CWinApp::WinHelp](#winhelp)|`WinHelp`呼叫 Windows 函式。|
...
|名稱|說明|
|----------|-----------------|
|[CWinApp::EnableShellOpen](#enableshellopen)|允許使用者從 Windows 檔案管理員開啟資料檔案。|
...
使用 Microsoft Foundation 類別的每個應用程式都只能包含一個衍生自`CWinApp`的物件。 這個物件是在建立其他C++全域物件時所建立, 而且當 Windows 呼叫的`WinMain`函式是由 MFC 程式庫所提供時, 就會進行此工作。 在全域層`CWinApp`級宣告您的衍生物件。

當您從`CWinApp`衍生應用程式類別時, 請覆寫[InitInstance](#initinstance)成員函式, 以建立應用程式的主視窗物件。

除了`CWinApp`成員函式之外, MFC 程式庫還提供下列全域函數來存取您`CWinApp`的物件和其他全域資訊:
...
請[參閱 CWinApp:適用于`CWinApp`類別](../../mfc/cwinapp-the-application-class.md)的應用程式類別, 包括下列各項的總覽:

- `CWinApp`-由應用程式精靈撰寫的衍生程式碼。

- `CWinApp`在您的應用程式執行順序中的角色。

- `CWinApp`的預設成員函式實作為。

- `CWinApp`的金鑰 overridables。

`m_hPrevInstance`資料成員已不存在。 若要判斷應用程式的另一個實例是否正在執行, 請使用已命名的 mutex。 如果開啟 mutex 失敗, 則不會有任何其他的應用程式實例正在執行。
...
的預設執行`ApplicationRecoveryCallback`會`CDataRecoveryHandler`使用將目前開啟的檔案清單儲存至登錄。 這個方法不會自動儲存任何檔案。
...
在呼叫`CloseAllDocuments`之前呼叫 [HideApplication](#hideapplication)。
...
您應該建立衍生類別的`CWinApp`一個全域物件。 您的應用程式中`CWinApp`只能有一個物件。 此函式`CWinApp` `WinMain`會儲存物件的指標, 以便呼叫物件的成員函式來初始化和執行應用程式。
...
請勿呼叫這個成員函式來開啟訊息方塊;請`AfxMessageBox`改用。

覆寫這個成員函式, 以自訂整個應用`AfxMessageBox`程式的呼叫處理。
...
*nCode*<br/>
如果這個參數是 1, 就會出現等待游標。 如果為 0, 則會還原等候資料指標, 而不會遞增參考計數。 如果為-1, 則等待游標結束。

### <a name="remarks"></a>備註

預設會實行沙漏游標。 `DoWaitCursor`維護參考計數。 若為正數, 則會顯示沙漏游標。

雖然您通常不會直接`DoWaitCursor`呼叫, 但是您可以覆寫這個成員函式來變更等待游標, 或在等候游標顯示時執行其他處理。

如需更簡單、更有效率的方式來執行等候游標, `CWaitCursor`請使用。
...
從衍生類別的`CWinApp`函式內呼叫這個成員函式, 以使用 HTMLHelp 作為應用程式的說明。
...
呼叫此函式 (通常是`InitInstance`從您的覆寫), 讓應用程式的使用者在 Windows 檔案管理員中按兩下檔案時, 可以開啟資料檔案。
...
`RegisterShellFileTypes`呼叫成員函式搭配此函式, 或提供。具有應用程式的 REG 檔案, 以手動方式註冊檔案類型。
...
應用程式的結束代碼;0表示沒有錯誤, 而大於0的值表示發生錯誤。 這個值會當做的傳回值`WinMain`使用。
...
架構等候復原回呼函式傳回的時間長度是 ping 間隔。 您可以藉由覆寫`CWinApp::GetApplicationRecoveryPingInterval`或將自訂值提供給來`RegisterWithRestartManager`自訂 ping 間隔。
...
傳回 HKEY_CURRENT_USER\\"Software" \RegistryKey\ProfileName. 的索引鍵
...
使用重新開機管理員的每個應用程式都必須有一個[CDataRecoveryHandler 類別](../../mfc/reference/cdatarecoveryhandler-class.md)的實例。 這個類別負責監視開啟的檔和自動儲存檔。 的行為`CDataRecoveryHandler`取決於重新開機管理員的設定。 如需詳細資訊, 請參閱[CDataRecoveryHandler 類別](../../mfc/reference/cdatarecoveryhandler-class.md)。
...
如果您使用`GetNextDocTemplate`的呼叫來`GetFirstDocTemplatePosition`建立初始位置, 則可以在正向反復專案迴圈中使用。
...
*pPrintDlg*<br/>
[PRINTDLG](/windows/desktop/api/commdlg/ns-commdlg-tagpda)結構的指標。
...
> [!NOTE]
> `GetProfileBinary`配置緩衝區, 並傳回其在\* *ppData*中的位址。 呼叫端負責使用**delete []** 釋放緩衝區。
...
傳回值是來自應用程式之的字串。如果找不到字串, 則為 INI 檔案或*lpszDefault* 。 架構所支援的最大字串長度為 _MAX_PATH。 如果*lpszDefault*為 Null, 則傳回值為空字串。
...
傳回 HKEY_CURRENT_USER\\"Software" \RegistryKey\AppName\lpszSection. 的索引鍵
...
*dwData*<br/>
指定其他資料。 使用的值取決於*nCmd*參數的值。 預設值`0x000F`為, 表示[HH_HELP_CONTEXT](/previous-versions/windows/desktop/htmlhelp/hh-help-context-command)。

*nCmd*<br/>
指定要求的說明類型。 如需可能值的清單, 以及它們如何影響*dwData*參數, 請參閱 Windows SDK 的[HtmlHelpW](/windows/desktop/api/htmlhelp/nf-htmlhelp-htmlhelpw)或[HtmlHelpA](/windows/desktop/api/htmlhelp/nf-htmlhelp-htmlhelpa) API 函式中所述的*uCommand*參數。  
...
應用程式初始化在概念上分成兩個部分: 第一次執行程式時完成的一次性應用程式初始化, 以及在每次執行程式時執行的實例初始化, 包括第一次。 架構的實`WinMain`作用會呼叫這個函式。

覆`InitInstance`寫以初始化在 Windows 下執行之應用程式的每個新實例。 一般而言, 您會`InitInstance`覆寫以建立主視窗物件, 並`CWinThread::m_pMainWnd`將資料成員設定為指向該視窗。 如需覆寫此成員函式的詳細[資訊, 請參閱 CWinApp:應用程式類別](../../mfc/cwinapp-the-application-class.md)。

> [!NOTE]
> MFC 應用程式必須初始化為單一執行緒單元 (STA)。 如果您在覆`InitInstance`寫中呼叫 [CoInitializeEx](/windows/desktop/api/combaseapi/nf-combaseapi-coinitializeex), 請指定 COINIT_APARTMENTTHREADED (而不是 COINIT_MULTITHREADED)。
...
如果`EnableTaskbarInteraction`已呼叫, 而且作業系統是 Windows 7 或更新版本, 則傳回 TRUE。
...
*lpszResourceName*<br/>
指向以 null 終止的字串, 其中包含資料指標資源的名稱。 您可以使用`CString`做為此引數。

*nIDResource*<br/>
資料指標資源的識別碼。 如需資源的清單, 請參閱 Windows SDK 中的[LoadCursor](/windows/desktop/api/winuser/nf-winuser-loadcursora) 。
...
*lpszResourceName*<br/>
指向以 null 終止的字串, 其中包含圖示資源的名稱。 您也可以`CString`針對這個引數使用。
...
`LoadIcon`只有在先前尚未載入圖示時, 才會載入它;否則, 它會抓取現有資源的控制碼。

您可以使用[LoadStandardIcon](#loadstandardicon)或[LoadOEMIcon](#loadoemicon)成員函式來存取預先定義的 Windows 圖示。

> [!NOTE]
> 此成員函式會呼叫 WIN32 API 函式[LoadIcon](/windows/desktop/api/winuser/nf-winuser-loadicona), 它只能載入大小符合 SM_CXICON 和 SM_CYICON 系統度量值的圖示。
...
*nIDCursor*<br/>
指定預先定義之 Windows 資料指標的**OCR_** 資訊清單常數識別碼。 您必須先`#define OEMRESOURCE` `#include \<afxwin.h>`擁有, 才能取得 WINDOWS 中**OCR_** 常數的存取權。H.
...
使用或 [LoadStandardCursor](#loadstandardcursor) 成員函式來存取預先定義的 Windows 資料指標。`LoadOEMCursor`
...
*nIDIcon*<br/>
**OIC_** 資訊清單常數識別碼, 指定預先定義的 Windows 圖示。 您必須先`#define OEMRESOURCE`擁有`#include \<afxwin.h>` , 才能存取 WINDOWS 中的**OIC_** 常數。H.
...
使用或 [LoadStandardIcon](#loadstandardicon) 成員函式來存取預先定義的 Windows 圖示。`LoadOEMIcon`
...
- 當 Windows 執行耗時的工作時, 所使用的 IDC_WAIT 沙漏游標

- IDC_CROSS 選取範圍的交叉線游標

- IDC_UPARROW 箭號, 指向直線

- IDC_SIZE 已過時且不受支援;使用 IDC_SIZEALL

- IDC_SIZEALL 四個指向的箭號。 要用來調整視窗大小的游標。

- IDC_ICON 已過時且不受支援。 使用 IDC_ARROW。

- IDC_SIZENWSE 雙向箭號, 結尾為左上方和右下方

- IDC_SIZENESW 雙向箭號, 結尾是右上方和左下方
...
使用或 [LoadOEMCursor ](#loadoemcursor)成員函式來存取預先定義的 Windows 資料指標。`LoadStandardCursor`
...
*lpszIconName*<br/>
資訊清單常數識別碼, 指定預先定義的 Windows 圖示。 這些識別碼會在 WINDOWS 中定義。H. 如需可能預先定義的值及其描述的清單, 請參閱 Windows SDK 中[LoadIcon](/windows/desktop/api/winuser/nf-winuser-loadicona)的*lpIconName*參數。
...
使用或 [LoadOEMIcon](#loadoemicon) 成員函式來存取預先定義的 Windows 圖示。`LoadStandardIcon`
...
若要啟用重新開機管理員, `m_dwRestartManagerSupportFlags`請將設定為您想要的行為。 下表顯示可用的旗標。
...
這個資料成員的型別是列舉型別 AFX_HELP_TYPE, 它是在`CWinApp`類別內定義的。
...
- 若要將應用程式的說明設定為 HTML 說明 , 請呼叫`afxHTMLHelp`[SetHelpMode](#sethelpmode) 並指定。

- 若要將應用程式的說明設定為 WinHelp `SetHelpMode` , 請`afxWinHelp`呼叫並指定。

##  <a name="m_hinstance"></a>CWinApp:: m_hInstance

對應至 Windows傳遞至`WinMain`的 hInstance 參數。
...
對應至 Windows傳遞至`WinMain`的 lpCmdLine 參數。
...
指向以 null 終止的字串, 指定應用程式的命令列。 用`m_lpCmdLine`來存取使用者在啟動應用程式時所輸入的任何命令列引數。 `m_lpCmdLine`是 LPTSTR 類型的公用變數。
...
對應至 Windows傳遞至`WinMain`的 nCmdShow 參數。
...
當您針對`m_nCmdShow`應用程式的主視窗呼叫[CWnd:: ShowWindow](../../mfc/reference/cwnd-class.md#showwindow)時, 應該傳遞做為引數。 `m_nCmdShow`是**int**類型的公用變數。
...
應用程式名稱可以來自傳遞給[CWinApp](#cwinapp)的函式的參數, 或如果未指定, 則會指向識別碼為 AFX_IDS_APP_TITLE 的資源字串。 如果在資源中找不到應用程式名稱, 則來自程式的。EXE 檔案名。
...
> [!NOTE]
> 如果您將值指派給`m_pszAppName`, 它就必須在堆積上動態配置。 此函式會使用這個指標呼叫 free ()。 `CWinApp` 您想要使用`_tcsdup`() 執行時間程式庫函數來進行配置。 此外, 在指派新值之前, 請先釋放與目前指標相關聯的記憶體。 例如：
...
> [!NOTE]
> 如果您將值指派給`m_pszExeName`, 它就必須在堆積上動態配置。 此函式會使用這個指標呼叫 free ()。 `CWinApp` 您想要使用`_tcsdup`() 執行時間程式庫函數來進行配置。 此外, 在指派新值之前, 請先釋放與目前指標相關聯的記憶體。 例如：
...
根據預設, 架構會將`m_pszHelpFilePath`初始化為具有 "的應用程式名稱。未附加的「.HLP」。 若要變更說明檔的名稱, 請將`m_pszHelpFilePath`設定為, 以指向包含所需說明檔完整名稱的字串。 執行此動作的方便位置是在應用程式的[InitInstance](#initinstance)函數中。 `m_pszHelpFilePath`是**const char** <strong>\*</strong>類型的公用變數。

> [!NOTE]
> 如果您將值指派給`m_pszHelpFilePath`, 它就必須在堆積上動態配置。 此函式會使用這個指標呼叫 free ()。 `CWinApp` 您想要使用`_tcsdup`() 執行時間程式庫函數來進行配置。 此外, 在指派新值之前, 請先釋放與目前指標相關聯的記憶體。 例如：
...
> [!NOTE]
> 如果您將值指派給`m_pszProfileName`, 它就必須在堆積上動態配置。 此函式會使用這個指標呼叫 free ()。 `CWinApp` 您想要使用`_tcsdup`() 執行時間程式庫函數來進行配置。 此外, 在指派新值之前, 請先釋放與目前指標相關聯的記憶體。 例如：
...
- 值會儲存在登錄機碼中。 應用程式佈建檔設定的名稱會附加至下列登錄機碼:HKEY_CURRENT_USER/Software/LocalAppWizard 產生/。

如果您將值指派給`m_pszRegistryKey`, 它就必須在堆積上動態配置。 此函式會使用這個指標呼叫 free ()。 `CWinApp` 您想要使用`_tcsdup`() 執行時間程式庫函數來進行配置。 此外, 在指派新值之前, 請先釋放與目前指標相關聯的記憶體。 例如：
...
您必須將`ON_COMMAND( ID_CONTEXT_HELP, OnContextHelp )`語句加入`CWinApp`至類別訊息對應, 也可以加入快速鍵對應表專案 (通常是 SHIFT + F1) 來啟用此成員函式。

`OnContextHelp`讓應用程式進入說明模式。 游標會變更為箭號和問號, 然後使用者可以移動滑鼠指標, 然後按下滑鼠左鍵, 以選取對話方塊、視窗、功能表或命令按鈕。 此成員函式會抓取游標底下物件的說明內容, 並使用該說明內容呼叫 Windows 函式 WinHelp。
...
您必須將`ON_COMMAND( ID_FILE_NEW, OnFileNew )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 啟用時, 此函式會處理檔案 New 命令的執行。
...
您必須將`ON_COMMAND( ID_FILE_OPEN, OnFileOpen )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 若啟用此函式, 此函式會處理檔案 Open 命令的執行。
...
您必須將`ON_COMMAND( ID_FILE_PRINT_SETUP, OnFilePrintSetup )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 啟用時, 此函式會處理檔案列印命令的執行。
...
您必須將`ON_COMMAND( ID_HELP, OnHelp )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 啟用時, 由架構在使用者按下 F1 鍵時呼叫。

這個訊息處理常式函式的預設執行會決定對應至目前視窗、對話方塊或功能表項目的說明內容, 然後呼叫 WINHELP。CONVERT.EXE. 如果目前沒有可用的內容, 此函數會使用預設內容。

覆寫這個成員函式, 將說明內容設定為視窗、對話方塊、功能表項目或工具列按鈕 (目前具有焦點) 以外的內容。 使用`WinHelp`所需的說明內容識別碼來呼叫。
...
您必須將`ON_COMMAND( ID_HELP_FINDER, OnHelpFinder )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 啟用時, 當應用程式的使用者選取 [說明搜尋工具] 命令以`WinHelp`使用標準**HELP_FINDER**主題來叫用時, 架構會呼叫這個訊息處理常式函式。
...
您必須將`ON_COMMAND( ID_HELP_INDEX, OnHelpIndex )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 啟用時, 當應用程式的使用者選取 [說明索引] 命令來`WinHelp`叫用標準**HELP_INDEX**主題時, 架構會呼叫這個訊息處理常式函式。
...
您必須將`ON_COMMAND( ID_HELP_USING, OnHelpUsing )`語句加入`CWinApp`至類別訊息對應, 才能啟用此成員函式。 當您應用程式的使用者使用命令來`WinHelp`叫用具有標準**HELP_HELPONHELP**主題的應用程式時, 架構會呼叫這個訊息處理常式函式。
...
*lCount*<br/>
當應用程式的消息`OnIdle`佇列是空的時, 每次呼叫時, 計數器就會遞增。 每次處理新訊息時, 此計數就會重設為0。 您可以使用*lCount*參數來判斷應用程式閒置而不處理訊息的相對時間長度。
...
`OnIdle`應傳回 0, 表示不需要閒置處理時間。 每當訊息佇列為空白時, `OnIdle`每次呼叫時都會遞增 lCount 參數, 並在每次處理新訊息時, 重設為0。 您可以根據此計數來呼叫不同的閒置常式。

以下摘要說明閒置迴圈處理:

1. 如果 MFC 程式庫中的訊息迴圈檢查訊息佇列並找不到擱置中的訊息, 它`OnIdle`會呼叫應用程式物件, 並提供0作為*lCount*引數。

2. `OnIdle`會執行一些處理並傳回非零值, 以指出應該再次呼叫它來執行進一步的處理。

3. 訊息迴圈會再次檢查訊息佇列。 如果沒有擱置中的訊息, 它`OnIdle`會再次呼叫, 遞增*lCount*引數。

4. 最後, `OnIdle`會完成其所有閒置工作的處理, 並傳回0。 這會告知訊息迴圈停止呼叫`OnIdle` , 直到從訊息佇列接收下一個訊息為止, 此時會重新開機閒置迴圈, 並將引數設定為0。

請不要在期間`OnIdle`執行冗長的工作, 因為您的應用程式無法處理使用者輸入, 直到返回為止。 `OnIdle`

> [!NOTE]
> [ `OnIdle`更新] 命令的預設執行是使用者介面物件 (例如功能表項目和工具列按鈕), 而且它會執行內部資料結構清除。 因此, 如果您覆`OnIdle`寫, 就必須`CWinApp::OnIdle`使用覆`lCount`寫版本中的來呼叫。 首先呼叫所有的基類閒置處理 (也就是, 直到基類`OnIdle`傳回 0)。 如果您需要在基類處理完成之前執行工作, 請檢查基類的執行, 以選取適當的*lCount* , 在此期間執行您的工作。

如果您不想`OnIdle`要在每次從訊息佇列中抓取訊息時呼叫, 您可以覆寫[CWinThreadIsIdleMessage](../../mfc/reference/cwinthread-class.md#isidlemessage)。 如果應用程式已設定非常短的計時器, 或系統正在傳送 WM_SYSTIMER 訊息, 則`OnIdle`會重複呼叫, 並降低效能。

### <a name="example"></a>範例

下列兩個範例示範如何使用`OnIdle`。 第一個範例會使用*lCount*引數來處理兩個閒置工作, 以排定工作的優先順序。 第一個工作是高優先順序, 而且您應該盡可能執行。 第二個工作較不重要, 只有在使用者輸入中有長時間暫停時才會執行。 請注意基類版本的`OnIdle`呼叫。 第二個範例會管理具有不同優先順序的閒置工作群組。
...
如果成功, 則`CDocument`為的指標, 否則為 Null。
...
當您使用應用程式精靈啟動新的 MFC 專案時, 應用程式精靈將會建立的本機`CCommandLineInfo`實例, 然後在`ProcessShellCommand` [InitInstance](#initinstance)成員函式中呼叫和`ParseCommandLine` 。 命令列會遵循下面所述的路由:

1. 在中`InitInstance`建立之後`CCommandLineInfo` , 物件會傳遞至`ParseCommandLine`。

2. `ParseCommandLine`然後針對`CCommandLineInfo::ParseParam`每個參數重複呼叫一次。

3. `ParseParam`填滿物件, 然後傳遞給 [ProcessShellCommand](#processshellcommand)。`CCommandLineInfo`
...
覆寫此函式以在將視窗訊息分派至 Windows 函式[TranslateMessage](/windows/desktop/api/winuser/nf-winuser-translatemessage)之前進行篩選, 並[DispatchMessage](/windows/desktop/api/winuser/nf-winuser-dispatchmessage)預設的實作用會執行快速鍵轉譯, 因此您必須呼叫`CWinApp::PreTranslateMessage`覆寫版本中的成員函式。
...
*pMsg*<br/>
包含要處理之訊息的[MSG](/windows/desktop/api/winuser/ns-winuser-tagmsg)結構的指標。

### <a name="return-value"></a>傳回值

如果訊息在中`PreTranslateMessage`完整處理, 則不應進一步處理。 如果應該以正常方式處理訊息, 則為零。
...
*lpMsg*<br/>
Windows [MSG](/windows/desktop/api/winuser/ns-winuser-tagmsg)結構的指標。
...
[InitInstance](#initinstance)會呼叫這個成員函式, 以接受*rCmdInfo*所識別的`CCommandLineInfo`物件所傳遞的參數, 並執行指定的動作。
...
如果成功處理 shell 命令, 則為非零。 如果是 0, 則從[InitInstance](#initinstance)傳回 FALSE。

### <a name="remarks"></a>備註

當您使用應用程式精靈啟動新的 MFC 專案時, 應用程式精靈將會建立的本機`CCommandLineInfo`實例, 然後在`ProcessShellCommand` `InitInstance`成員函式中呼叫和[ParseCommandLine](#parsecommandline) 。 命令列會遵循下面所述的路由:

1. 在中`InitInstance`建立之後`CCommandLineInfo` , 物件會傳遞至`ParseCommandLine`。

2. `ParseCommandLine`接著會針對每個參數, 重複呼叫[CCommandLineInfo::P arseparam](../../mfc/reference/ccommandlineinfo-class.md#parseparam) 。

3. `ParseParam`填滿`ProcessShellCommand`物件, 然後將它傳遞給。 `CCommandLineInfo`

4. `ProcessShellCommand`處理命令列引數和旗標。

`CCommandLineInfo`物件的資料成員 (由[CCommandLineInfo:: m_nShellCommand](../../mfc/reference/ccommandlineinfo-class.md#m_nshellcommand)識別) 屬於下列列舉類型, 這`CCommandLineInfo`是在類別內定義的。
...
*pMsg*<br/>
[訊息](/windows/desktop/api/winuser/ns-winuser-tagmsg)結構, 其中包含導致架構擲回例外狀況之 windows 訊息的相關資訊。
...
執行未由處理的`RegisterShellFileTypes`任何註冊工作。
...
這可讓使用者在 [檔案管理員] 中按兩下, 以開啟您的應用程式所建立的資料檔案。 在`RegisterShellFileTypes`您針對應用程式中的每個檔範本呼叫[AddDocTemplate](#adddoctemplate)之後, 呼叫。 當您呼叫 `RegisterShellFileTypes`時, 也會呼叫新增 [EnableShellOpen](#enableshellopen) 成員函式。

`RegisterShellFileTypes`逐一查看應用程式所維護的[CDocTemplate](../../mfc/reference/cdoctemplate-class.md)物件清單, 並針對每個檔範本, 將專案新增至 Windows 維護檔案關聯的註冊資料庫。 當使用者按兩下資料檔案時, 檔案管理員會使用這些專案來開啟該檔案。 這樣就不需要寄送。REG 檔案與您的應用程式。

> [!NOTE]
> `RegisterShellFileTypes`只有在使用者以系統管理員許可權執行程式時才適用。 如果程式沒有系統管理員許可權, 就無法改變登錄機碼。
...
|*pRecoveryCallback*|在復原回呼函數。 此函式必須接受 LPVOID 參數作為輸入, 並傳回 DWORD。 預設的復原回呼函式`CWinApp::ApplicationRecoveryCallback`是。|
...
如果您的應用程式使用自動儲存檔的預設 MFC 執行, 您應該使用的簡單版本`RegisterWithRestartManager`。 `RegisterWithRestartManager`如果您想要自訂應用程式的自動儲存行為, 請使用的複雜版本。

如果您使用*strRestartIdentifier*的空字串來呼叫這個方法, `RegisterWithRestartManager`會為重新啟動管理員的這個實例建立唯一的識別碼字串。

當應用程式意外結束時, 重新開機管理員會從命令列重新開機應用程式, 並提供唯一的重新開機識別碼做為選擇性引數。 在此案例中, 架構會`RegisterWithRestartManager`呼叫兩次。 第一次呼叫的來源是[CWinApp:: InitInstance](#initinstance) , 其字串識別碼為空字串。 然後, 方法[CWinApp::P rocessshellcommand](#processshellcommand)會以`RegisterWithRestartManager`唯一的重新開機識別碼呼叫。
...
`Run`取得和分派 Windows 訊息, 直到應用程式接收到 WM_QUIT 訊息為止。 如果應用程式的訊息佇列目前未包含任何訊息`Run` , 則會呼叫[OnIdle](#onidle)來執行閒置時間處理。 傳入的訊息會移至[PreTranslateMessage](#pretranslatemessage)成員函式進行特殊處理, 然後傳送至`TranslateMessage`標準鍵盤轉譯的 windows 函式`DispatchMessage` ; 最後, 會呼叫 windows 函式。

`Run`很少遭到覆寫, 但是您可以覆寫它來提供特殊行為。
...
如果存在, 則會從命令列中移除選項。 如需有關內嵌的詳細資訊, 請[參閱 < 伺服器:執行伺服器](../../mfc/servers-implementing-a-server.md)。
...
*hDevNames*<br/>
[DEVNAMES](/windows/desktop/api/commdlg/ns-commdlg-tagdevnames)結構的控制碼, 可識別特定印表機的驅動程式、裝置和輸出埠名稱。
...
如果*hDevMode*和*hDevNames*都是 Null, `SelectPrinter`則會使用目前的預設印表機。
...
若要將應用程式的說明類型設定為 HTMLHelp, 您可以呼叫[EnableHTMLHelp](#enablehtmlhelp)。 呼叫`EnableHTMLHelp`之後, 您的應用程式必須使用 HTMLHelp 作為其說明應用程式。 如果您想要變更為使用 WinHelp, 您可以呼叫`SetHelpMode` , 並將 eHelpType `afxWinHelp`設定為。
...
此函式會設定*m_pszRegistryKey*, 然後`GetProfileInt`由的、 `GetProfileString` `WriteProfileInt`、和`WriteProfileString`成員`CWinApp`函式使用此功能。 如果已呼叫此函式, 則最近使用的 (MRU) 檔案清單也會儲存在登錄中。 登錄機碼通常是公司的名稱。 它會以下列格式儲存在索引鍵中:HKEY_CURRENT_USER\Software\\< 公司名稱\> \> \> <應用\\程式名稱\\< 區段名稱 < 值名稱\>。\\
...
函式會復原應用程式物件和 [Register](#register) 函數所執行的註冊。`Unregister` 一般來說, MFC 會以隱含方式呼叫這兩個函式, 因此不會出現在您的程式碼中。
...
*nCmd*<br/>
指定要求的說明類型。 如需可能值的清單, 以及它們如何影響*dwData*參數, 請參閱[WinHelp](/windows/desktop/api/winuser/nf-winuser-winhelpa) Windows 函式。
...
這個範例會`CWinApp* pApp = AfxGetApp();`使用來取得 CWinApp 類別, 其中`WriteProfileBinary`說明可從 MFC `GetProfileBinary`應用程式中的任何函式使用和的方式。
...
這個範例會`CWinApp* pApp = AfxGetApp();`使用來取得 CWinApp 類別`WriteProfileString`, 說明可從 MFC 應用程式`GetProfileString`中的`GetProfileInt`任何函式使用、 `WriteProfileInt`、和的方式。
