...
|[CWinApp：： HtmlHelp](#htmlhelp)|呼叫 Windows 函式 `HTMLHelp` 。|
...
|[CWinApp：： WinHelp](#winhelp)|呼叫 Windows 函式 `WinHelp` 。|
...
使用 Microsoft Foundation 類別的每個應用程式都只能包含一個衍生自的物件 `CWinApp` 。 這個物件是在建立其他 c + + 全域物件時所建立，而且當 Windows 呼叫由 MFC 程式庫所提供的函式時，它已經可以使用 `WinMain` 。 `CWinApp`在全域層級宣告您的衍生物件。

當您從衍生應用程式類別時 `CWinApp` ，請覆寫[InitInstance](#initinstance)成員函式，以建立應用程式的主視窗物件。

除了成員函式之外 `CWinApp` ，MFC 程式庫還提供下列全域函數來存取您的 `CWinApp` 物件和其他全域資訊：
...
在呼叫之前呼叫[HideApplication](#hideapplication) `CloseAllDocuments` 。
...
*lpszAppName*<br/>
以 null 終止的字串，其中包含 Windows 所使用的應用程式名稱。 如果未提供這個引數或其為 Null，會 `CWinApp` 使用資源字串 AFX_IDS_APP_TITLE 或可執行檔的檔案名。

### <a name="remarks"></a>備註

您應該建立衍生類別的一個全域物件 `CWinApp` 。 您 `CWinApp` 的應用程式中只能有一個物件。 此函式會儲存物件的指標， `CWinApp` 以便 `WinMain` 呼叫物件的成員函式來初始化和執行應用程式。
...
雖然您通常不會直接呼叫，但是您可以覆 `DoWaitCursor` 寫這個成員函式來變更等待游標，或在等候游標顯示時執行其他處理。
...
從衍生類別的函式內呼叫這個成員函 `CWinApp` 式，以使用 HTMLHelp 作為應用程式的說明。
...
呼叫此函式（通常是從您的覆 `InitInstance` 寫），讓應用程式的使用者在 Windows 檔案管理員中按兩下檔案時，可以開啟資料檔案。
...
呼叫成員函式 `RegisterShellFileTypes` 搭配此函式，或提供。具有應用程式的 REG 檔案，以手動方式註冊檔案類型。
...
應用程式的結束代碼;0表示沒有錯誤，而大於0的值表示發生錯誤。 這個值會當做的傳回值使用 `WinMain` 。

### <a name="remarks"></a>備註

請勿在成員函式內的任何位置呼叫此成員函式 `Run` 。
...
架構等候復原回呼函式傳回的時間長度是 ping 間隔。 您可以藉由覆寫或將 `CWinApp::GetApplicationRecoveryPingInterval` 自訂值提供給來自訂 ping 間隔 `RegisterWithRestartManager` 。
...
*採購*<br/>
先前呼叫或 GetFirstDocTemplatePosition 所傳回之位置值的參考 `GetNextDocTemplate` 。 [GetFirstDocTemplatePosition](#getfirstdoctemplateposition) 這個呼叫會將值更新為下一個位置。
...
`GetNextDocTemplate`如果您使用的呼叫來建立初始位置，則可以在正向反復專案迴圈中使用 `GetFirstDocTemplatePosition` 。
...
應用程式初始化在概念上分成兩個部分：第一次執行程式時完成的一次性應用程式初始化，以及在每次執行程式時執行的實例初始化，包括第一次。 架構的實作用會 `WinMain` 呼叫這個函式。

覆寫 `InitInstance` 以初始化在 Windows 下執行之應用程式的每個新實例。 一般而言，您會覆寫 `InitInstance` 以建立主視窗物件，並將 `CWinThread::m_pMainWnd` 資料成員設定為指向該視窗。 如需覆寫此成員函式的詳細資訊，請參閱[CWinApp：應用程式類別](../../mfc/cwinapp-the-application-class.md)。
...
如果已 `EnableTaskbarInteraction` 呼叫，而且作業系統是 Windows 7 或更新版本，則傳回 TRUE。
...
*lpszResourceName*<br/>
指向以 null 終止的字串，其中包含資料指標資源的名稱。 您可以使用做 `CString` 為此引數。
...
*nIDCursor*<br/>
指定預先定義之 Windows 資料指標的**OCR_** 資訊清單常數識別碼。 您必須先擁有，才能 `#define OEMRESOURCE` `#include \<afxwin.h>` 取得 WINDOWS 中**OCR_** 常數的存取權。H.
...
這個資料成員的型別是 AFX_HELP_TYPE 的列舉型別，它是在類別內定義的 `CWinApp` 。
...
對應至 Windows 傳遞至的*hInstance*參數 `WinMain` 。
...
對應至 Windows 傳遞至的*lpCmdLine*參數 `WinMain` 。
...
對應至 Windows 傳遞至的*nCmdShow*參數 `WinMain` 。
...
`m_nCmdShow`當您針對應用程式的主視窗呼叫[CWnd：： ShowWindow](../../mfc/reference/cwnd-class.md#showwindow)時，應該傳遞做為引數。 `m_nCmdShow`是類型的公用變數 **`int`** 。
...
由全域函數[AfxGetAppName](application-information-and-management.md#afxgetappname)傳回。 `m_pszAppName`是**const char**類型的公用變數 <strong>\*</strong> 。

> [!NOTE]
> 如果您將值指派給 `m_pszAppName` ，它就必須在堆積上動態配置。 `CWinApp`此函式會使用這個指標呼叫**free**（）。 您想要使用 `_tcsdup` （）執行時間程式庫函數來進行配置。 此外，在指派新值之前，請先釋放與目前指標相關聯的記憶體。 例如：
...
不同于[m_pszAppName](#m_pszappname)，此名稱不能包含空白。 `m_pszExeName`是**const char**類型的公用變數 <strong>\*</strong> 。

> [!NOTE]
> 如果您將值指派給 `m_pszExeName` ，它就必須在堆積上動態配置。 `CWinApp`此函式會使用這個指標呼叫**free**（）。 您想要使用 `_tcsdup` （）執行時間程式庫函數來進行配置。 此外，在指派新值之前，請先釋放與目前指標相關聯的記憶體。 例如：
...
根據預設，架構會將初始化 `m_pszHelpFilePath` 為具有 "的應用程式名稱。未附加的「.HLP」。 若要變更說明檔的名稱，請將設定為，以 `m_pszHelpFilePath` 指向包含所需說明檔完整名稱的字串。 執行此動作的方便位置是在應用程式的[InitInstance](#initinstance)函數中。 `m_pszHelpFilePath`是**const char**類型的公用變數 <strong>\*</strong> 。

> [!NOTE]
> 如果您將值指派給 `m_pszHelpFilePath` ，它就必須在堆積上動態配置。 `CWinApp`此函式會使用這個指標呼叫**free**（）。 您想要使用 `_tcsdup` （）執行時間程式庫函數來進行配置。 此外，在指派新值之前，請先釋放與目前指標相關聯的記憶體。 例如：
...
`m_pszProfileName`是**const char**類型的公用變數 <strong>\*</strong> 。

> [!NOTE]
> 如果您將值指派給 `m_pszProfileName` ，它就必須在堆積上動態配置。 `CWinApp`此函式會使用這個指標呼叫**free**（）。 您想要使用 `_tcsdup` （）執行時間程式庫函數來進行配置。 此外，在指派新值之前，請先釋放與目前指標相關聯的記憶體。 例如：
...
如果您將值指派給 `m_pszRegistryKey` ，它就必須在堆積上動態配置。 `CWinApp`此函式會使用這個指標呼叫**free**（）。 您想要使用 `_tcsdup` （）執行時間程式庫函數來進行配置。 此外，在指派新值之前，請先釋放與目前指標相關聯的記憶體。 例如：
...
您必須將 `ON_COMMAND( ID_CONTEXT_HELP, OnContextHelp )` 語句加入至 `CWinApp` 類別訊息對應，也可以加入快速鍵對應表專案（通常是 SHIFT + F1）來啟用此成員函式。
...
您必須將 `ON_COMMAND( ID_FILE_NEW, OnFileNew )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 啟用時，此函式會處理檔案 New 命令的執行。
...
您必須將 `ON_COMMAND( ID_FILE_OPEN, OnFileOpen )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 若啟用此函式，此函式會處理檔案 Open 命令的執行。
...
您必須將 `ON_COMMAND( ID_FILE_PRINT_SETUP, OnFilePrintSetup )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 啟用時，此函式會處理檔案列印命令的執行。
...
您必須將 `ON_COMMAND( ID_HELP, OnHelp )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 啟用時，由架構在使用者按下 F1 鍵時呼叫。

這個訊息處理常式函式的預設執行會決定對應至目前視窗、對話方塊或功能表項目的說明內容，然後呼叫 WINHELP.EXE。 如果目前沒有可用的內容，此函數會使用預設內容。

覆寫這個成員函式，將說明內容設定為視窗、對話方塊、功能表項目或工具列按鈕（目前具有焦點）以外的內容。 `WinHelp`使用所需的說明內容識別碼來呼叫。
...
您必須將 `ON_COMMAND( ID_HELP_FINDER, OnHelpFinder )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 啟用時，架構會在應用程式的使用者選取 [說明搜尋工具] 命令以 `WinHelp` 使用標準**HELP_FINDER**主題來叫用時，呼叫此訊息處理常式函式。
...
您必須將 `ON_COMMAND( ID_HELP_INDEX, OnHelpIndex )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 啟用時，當應用程式的使用者選取 [說明索引] 命令以 `WinHelp` 使用標準**HELP_INDEX**主題來叫用時，架構會呼叫這個訊息處理常式函式。
...
您必須將 `ON_COMMAND( ID_HELP_USING, OnHelpUsing )` 語句加入至 `CWinApp` 類別訊息對應，才能啟用此成員函式。 當您應用程式的使用者使用命令來叫 `WinHelp` 用具有標準**HELP_HELPONHELP**主題的應用程式時，架構會呼叫這個訊息處理常式函式。
...
*lCount*<br/>
`OnIdle`當應用程式的訊息佇列是空的時，每次呼叫時，計數器就會遞增。 每次處理新訊息時，此計數就會重設為0。 您可以使用*lCount*參數來判斷應用程式閒置而不處理訊息的相對時間長度。
...
`OnIdle`應傳回0，表示不需要閒置處理時間。 每當訊息佇列為空白時，每次呼叫時都會遞增*lCount*參數 `OnIdle` ，並在每次處理新訊息時，重設為0。 您可以根據此計數來呼叫不同的閒置常式。

以下摘要說明閒置迴圈處理：

1. 如果 MFC 程式庫中的訊息迴圈檢查訊息佇列並找不到擱置中的訊息，它會呼叫 `OnIdle` 應用程式物件，並提供0作為*lCount*引數。

2. `OnIdle`會執行一些處理並傳回非零值，以指出應該再次呼叫它來執行進一步的處理。

3. 訊息迴圈會再次檢查訊息佇列。 如果沒有擱置中的訊息，它會 `OnIdle` 再次呼叫，遞增*lCount*引數。

4. 最後，會 `OnIdle` 完成其所有閒置工作的處理，並傳回0。 這會告知訊息迴圈停止呼叫 `OnIdle` ，直到從訊息佇列接收下一個訊息為止，此時會重新開機閒置迴圈，並將引數設定為0。

請不要在期間執行冗長的 `OnIdle` 工作，因為您的應用程式無法處理使用者輸入 `OnIdle` ，直到返回為止。

> [!NOTE]
> [更新] 命令的預設執行是 `OnIdle` 使用者介面物件（例如功能表項目和工具列按鈕），而且它會執行內部資料結構清除。 因此，如果您覆寫 `OnIdle` ，就必須 `CWinApp::OnIdle` 使用覆 `lCount` 寫版本中的來呼叫。 首先呼叫所有的基類閒置處理（也就是，直到基類傳回 `OnIdle` 0）。 如果您需要在基類處理完成之前執行工作，請檢查基類的執行，以選取適當的*lCount* ，在此期間執行您的工作。

如果您不想要在 `OnIdle` 每次從訊息佇列中抓取訊息時呼叫，您可以覆寫[CWinThreadIsIdleMessage](../../mfc/reference/cwinthread-class.md#isidlemessage)。 如果應用程式已設定非常短的計時器，或系統正在傳送 WM_SYSTIMER 訊息，則 `OnIdle` 會重複呼叫，並降低效能。

### <a name="example"></a>範例

下列兩個範例示範如何使用 `OnIdle` 。 第一個範例會使用*lCount*引數來處理兩個閒置工作，以排定工作的優先順序。 第一個工作是高優先順序，而且您應該盡可能執行。 第二個工作較不重要，只有在使用者輸入中有長時間暫停時才會執行。 請注意基類版本的呼叫 `OnIdle` 。 第二個範例會管理具有不同優先順序的閒置工作群組。
...
當您使用應用程式精靈啟動新的 MFC 專案時，應用程式精靈將會建立的本機實例 `CCommandLineInfo` ，然後 `ProcessShellCommand` `ParseCommandLine` 在[InitInstance](#initinstance)成員函式中呼叫和。 命令列會遵循下面所述的路由：

1. 在中建立之後 `InitInstance` ， `CCommandLineInfo` 物件會傳遞至 `ParseCommandLine` 。

2. `ParseCommandLine`然後 `CCommandLineInfo::ParseParam` 針對每個參數重複呼叫一次。

3. `ParseParam`填滿 `CCommandLineInfo` 物件，然後傳遞給[ProcessShellCommand](#processshellcommand)。
...
覆寫此函式以在將視窗訊息分派至 Windows 函式[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)之前進行篩選，並[DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage)預設的實作為執行快速鍵轉譯，因此您必須 `CWinApp::PreTranslateMessage` 在覆寫的版本中呼叫成員函式。
...
[InitInstance](#initinstance)會呼叫這個成員函式，以接受 rCmdInfo 所識別的物件所傳遞的參數 `CCommandLineInfo` ，並執行指定的動作。 *rCmdInfo*
...
當您使用應用程式精靈啟動新的 MFC 專案時，應用程式精靈將會建立的本機實例 `CCommandLineInfo` ，然後 `ProcessShellCommand` 在成員函式中呼叫和[ParseCommandLine](#parsecommandline) `InitInstance` 。 命令列會遵循下面所述的路由：

1. 在中建立之後 `InitInstance` ， `CCommandLineInfo` 物件會傳遞至 `ParseCommandLine` 。

2. `ParseCommandLine`接著會針對每個參數，重複呼叫[CCommandLineInfo：:P arseparam](../../mfc/reference/ccommandlineinfo-class.md#parseparam) 。

3. `ParseParam`填滿 `CCommandLineInfo` 物件，然後將它傳遞給 `ProcessShellCommand` 。

4. `ProcessShellCommand`處理命令列引數和旗標。

`CCommandLineInfo` [CCommandLineInfo：： m_nShellCommand](../../mfc/reference/ccommandlineinfo-class.md#m_nshellcommand)所識別之物件的資料成員，屬於下列列舉型別（在類別中定義） `CCommandLineInfo` 。
...
執行未由處理的任何註冊工作 `RegisterShellFileTypes` 。
...
這可讓使用者在 [檔案管理員] 中按兩下，以開啟您的應用程式所建立的資料檔案。 在 `RegisterShellFileTypes` 您針對應用程式中的每個檔範本呼叫[AddDocTemplate](#adddoctemplate)之後，呼叫。 當您呼叫時，也會呼叫[新增 enableshellopen](#enableshellopen)成員函式 `RegisterShellFileTypes` 。
...
|*pRecoveryCallback*|在復原回呼函數。 此函式必須接受 LPVOID 參數作為輸入，並傳回 DWORD。 預設的復原回呼函式是 `CWinApp::ApplicationRecoveryCallback` 。|
...
如果您使用*strRestartIdentifier*的空字串來呼叫這個方法，會為 `RegisterWithRestartManager` 重新開機管理員的這個實例建立唯一的識別碼字串。

當應用程式意外結束時，重新開機管理員會從命令列重新開機應用程式，並提供唯一的重新開機識別碼做為選擇性引數。 在此案例中，架構會呼叫 `RegisterWithRestartManager` 兩次。 第一次呼叫的來源是[CWinApp：： InitInstance](#initinstance) ，其字串識別碼為空字串。 然後，方法[CWinApp：:P rocessshellcommand](#processshellcommand)會 `RegisterWithRestartManager` 以唯一的重新開機識別碼呼叫。
...
`Run`取得和分派 Windows 訊息，直到應用程式收到 WM_QUIT 訊息為止。 如果應用程式的訊息佇列目前未包含任何訊息，則會 `Run` 呼叫[OnIdle](#onidle)來執行閒置時間處理。 傳入的訊息會移至[PreTranslateMessage](#pretranslatemessage)成員函式進行特殊處理，然後傳送至 `TranslateMessage` 標準鍵盤轉譯的 windows 函式; 最後， `DispatchMessage` 會呼叫 windows 函式。
...
如果*hDevMode*和*HDEVNAMES*都是 Null，則會 `SelectPrinter` 使用目前的預設印表機。
...
若要將應用程式的說明類型設定為 HTMLHelp，您可以呼叫[EnableHTMLHelp](#enablehtmlhelp)。 呼叫之後 `EnableHTMLHelp` ，您的應用程式必須使用 HTMLHelp 作為其說明應用程式。 如果您想要變更為使用 WinHelp，您可以呼叫 `SetHelpMode` ，並將*eHelpType*設定為 `afxWinHelp` 。
...
此函式會設定*m_pszRegistryKey*，然後由的、 `GetProfileInt` 、 `GetProfileString` `WriteProfileInt` 和成員函 `WriteProfileString` 式使用 `CWinApp` 。 如果已呼叫此函式，則最近使用的（MRU）檔案清單也會儲存在登錄中。 登錄機碼通常是公司的名稱。 它會儲存在下列形式的索引鍵中： HKEY_CURRENT_USER \Software \\<公司名稱 \> \\<應用程式名稱 \> \\<區段名稱 \> \\<值名稱 \> 。
...
函式會 `Unregister` 復原應用程式物件和[Register](#register)函數所執行的註冊。 一般來說，MFC 會以隱含方式呼叫這兩個函式，因此不會出現在您的程式碼中。
...
這個範例 `CWinApp* pApp = AfxGetApp();` 會使用來取得 CWinApp 類別，其中說明 `WriteProfileBinary` `GetProfileBinary` 可從 MFC 應用程式中的任何函式使用和的方式。
...
這個範例 `CWinApp* pApp = AfxGetApp();` 會使用來取得 CWinApp 類別 `WriteProfileString` ，說明 `WriteProfileInt` `GetProfileString` `GetProfileInt` 可從 MFC 應用程式中的任何函式使用、、和的方式。
...
[如何：新增重新開機管理員支援](../../mfc/how-to-add-restart-manager-support.md)
