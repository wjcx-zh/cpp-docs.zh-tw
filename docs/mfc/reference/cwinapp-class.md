...
>  預設實作`OnIdle`更新命令的功能表項目和工具列按鈕等使用者介面物件，並且它清除內部資料結構。 因此，如果您覆寫`OnIdle`，您必須呼叫`CWinApp::OnIdle`與`lCount`的覆寫版本。 第一次呼叫閒置處理的所有基底類別 (也就是直到基底類別`OnIdle`會傳回 0)。 如果您需要執行的工作，基底類別處理完成之前，檢閱選取適當的基底類別實作`lCount`期間用來進行工作。  
 ...
 如果您不想`OnIdle`要呼叫從訊息佇列擷取訊息時，您可以覆寫[CWinThreadIsIdleMessage](../../mfc/reference/cwinthread-class.md#isidlemessage)。 如果應用程式已設定非常短的計時器，或如果系統傳送**WM_SYSTIMER**訊息，然後`OnIdle`會重複呼叫，並會降低效能。  
...
### <a name="remarks"></a>備註  
 ...
 `RegisterShellFileTypes`逐一查看清單[CDocTemplate](../../mfc/reference/cdoctemplate-class.md)物件的應用程式會維護並，每個文件範本，將項目加入至註冊資料庫，Windows 會維護檔案關聯性。 檔案管理員會使用這些項目開啟資料檔案，當使用者按兩下它。 這就不需要寄送。您的應用程式的登錄檔。  
...
|-|-|  
|參數|描述|  
|[in] `bRegisterRecoveryCallback`|`TRUE`指出這個執行個體的應用程式使用復原的回呼函式。`FALSE`表示不進行。 當應用程式意外結束時，架構會呼叫復原回呼函式。 如需詳細資訊，請參閱[CWinApp::ApplicationRecoveryCallback](#applicationrecoverycallback)。|  
|[in] `strRestartIdentifier`|識別這個執行個體重新啟動管理員的唯一字串。 重新啟動管理員識別項是唯一的應用程式的每個執行個體。|  
|[in] `pwzCommandLineArgs`|字串，包含任何額外的引數，從命令列。|  
|[in] `dwRestartFlags`|重新啟動管理員的選擇性旗標。 如需詳細資訊，請參閱＜備註＞一節。|  
|[in] `pRecoveryCallback`|復原的回呼函式。 此函式必須接受`LPVOID`參數做為輸入並傳回`DWORD`。 預設復原回呼函式是`CWinApp::ApplicationRecoveryCallback`。|  
|[in] `lpvParam`|復原回呼函式的輸入的參數。 如需詳細資訊，請參閱[CWinApp::ApplicationRecoveryCallback](#applicationrecoverycallback)。|  
|[in] `dwPingInterval`|重新啟動管理員會等到復原回呼函式傳回的時間長度。 這個參數是以毫秒為單位。|  
|[in] `dwCallbackFlags`|旗標傳遞給復原回呼函式。 保留供未來使用。|  
  
### <a name="return-value"></a>傳回值  
 `S_OK`如果方法成功。否則錯誤碼。  
  
### <a name="remarks"></a>備註  
 如果您的應用程式會使用預設 MFC 實作自動儲存檔案，您應該使用的簡單版本`RegisterWithRestartManager`。 使用複雜的版本`RegisterWithRestartManager`如果您想要自訂您的應用程式的自動儲存行為。  
  
 如果您呼叫這個方法以空字串`strRestartIdentifier`，`RegisterWithRestartManager`管理員會建立這個執行個體重新啟動的唯一識別碼字串。  
  
 當應用程式意外結束時，重新啟動管理員從命令列應用程式會重新啟動，並提供唯一的重新啟動與選擇性引數的識別項。 在此案例中，架構會呼叫`RegisterWithRestartManager`兩次。 第一個呼叫是來自[cwinapp:: Initinstance](#initinstance)以空字串的字串識別項。 然後，此方法[CWinApp::ProcessShellCommand](#processshellcommand)呼叫`RegisterWithRestartManager`具有唯一的重新啟動識別項。  
  
 您重新啟動管理員中註冊應用程式之後，重新啟動管理員會監視應用程式。 如果應用程式意外結束，重新啟動管理員就會呼叫關機程序期間復原的回呼函式。 重新啟動管理員等候`dwPingInterval`復原回呼函式的回應。 如果復原的回呼函式不在此時間內回應，應用程式會結束而不執行復原的回呼函式。  
  
 根據預設，dwRestartFlags 不支援，但可供日後使用。 可能值`dwRestartFlags`如下︰  
...
### <a name="remarks"></a>備註  
 當重新啟動管理員重新啟動應用程式時，架構會呼叫這個方法。 這個方法會擷取資料的復原處理常式，並將 autosaved 檔案還原。 這個方法會呼叫[CDataRecoveryHandler::RestoreAutosavedDocuments](../../mfc/reference/cdatarecoveryhandler-class.md#restoreautosaveddocuments)來判斷使用者是否想要還原 autosaved 檔案。  
  
 這個方法會傳回`FALSE`如果[CDataRecoveryHandler](../../mfc/reference/cdatarecoveryhandler-class.md)判斷已開啟的文件。 如果沒有任何開啟的文件，在應用程式通常啟動。  
...
##  <a name="a-namerestoreautosavedfilesatrestarta--cwinapprestoreautosavedfilesatrestart"></a><a name="restoreautosavedfilesatrestart"></a>CWinApp::RestoreAutosavedFilesAtRestart  
 決定是否重新啟動管理員重新啟動應用程式時還原 autosaved 檔案。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`指出重新啟動管理員還原 autosaved 檔案;`FALSE`表示並不會重新啟動管理員。  
...
### <a name="remarks"></a>備註  
 **執行**取得，而且會分派 Windows 訊息，直到應用程式收到**WM_QUIT**訊息。 如果應用程式的訊息佇列目前不包含訊息，**執行**呼叫[OnIdle](#onidle)執行閒置時間處理。 內送訊息會移至[PreTranslateMessage](#pretranslatemessage)成員函式進行特殊處理，然後以 Windows 函式**TranslateMessage**標準鍵盤翻譯; 最後， **DispatchMessage**呼叫 Windows 函式。  
  
 **執行**幾乎不覆寫，但是您可以覆寫它，以便提供特殊行為。  
  
##  <a name="a-namerunautomateda--cwinapprunautomated"></a><a name="runautomated"></a>CWinApp::RunAutomated  
 呼叫此函式來判斷是否 「 **/Automation**「 或 」 **-自動化**」 選項已存在，表示用戶端應用程式是否已啟動伺服器應用程式。  
...
##  <a name="a-namerunembeddeda--cwinapprunembedded"></a><a name="runembedded"></a>CWinApp::RunEmbedded  
 呼叫此函式來判斷是否 「 **/內嵌**「 或 」 **-內嵌**」 選項已存在，表示用戶端應用程式是否已啟動伺服器應用程式。  
...
##  <a name="a-namesaveallmodifieda--cwinappsaveallmodified"></a><a name="saveallmodified"></a>CWinApp::SaveAllModified  
 由架構呼叫儲存所有文件時應用程式的主框架視窗會關閉，或透過`WM_QUERYENDSESSION`訊息。  
...
##  <a name="a-nameselectprintera--cwinappselectprinter"></a><a name="selectprinter"></a>CWinApp::SelectPrinter  
 呼叫此成員函式，以選取特定的印表機，並釋放先前在 [列印] 對話方塊中選取的印表機。  
...
### <a name="remarks"></a>備註  
 ...
 若要設定 HTMLHelp 您的應用程式類型，您可以呼叫[EnableHTMLHelp](#enablehtmlhelp)。 一旦您呼叫`EnableHTMLHelp`，應用程式必須使用 HTMLHelp 作為其說明應用程式。 如果您想要使用 WinHelp 變更，您可以呼叫`SetHelpMode`，並設定`eHelpType`至**afxWinHelp**。  
...
### <a name="remarks"></a>備註  
 此函式會將*m_pszRegistryKey*，然後由`GetProfileInt`， `GetProfileString`， `WriteProfileInt`，和`WriteProfileString`的成員函式`CWinApp`。 如果已經呼叫此函式，最最近使用 (過 MRU) 檔案的清單也會儲存在登錄中。 登錄機碼通常是公司的名稱。 它會儲存在下列形式的索引鍵︰ HKEY_CURRENT_USER\Software\\<company></company>\>\\<application></application>\>\\<section></section>\>\\<value></value>\>。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`指出重新啟動管理員復原應用程式。`FALSE`表示並不會重新啟動管理員。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`指出重新啟動管理員時，自動儲存開啟的文件。`FALSE`表示並不會重新啟動管理員。  
...
##  <a name="a-namesupportsautosaveatrestarta--cwinappsupportsautosaveatrestart"></a><a name="supportsautosaveatrestart"></a>CWinApp::SupportsAutosaveAtRestart  
 決定是否重新啟動管理員時，自動儲存所有開啟的文件的應用程式重新啟動時。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`指出重新啟動管理員時，自動儲存開啟的文件，當應用程式重新啟動。`FALSE`表示並不會重新啟動管理員。  
...
### <a name="return-value"></a>傳回值  
 `TRUE`指出應用程式支援的重新啟動管理員。`FALSE`表示應用程式並不會。  
...
### <a name="remarks"></a>備註  
 `Unregister`函式會復原由應用程式物件的註冊和[註冊](#register)函式。 一般來說，這兩個函數由 MFC 隱含地呼叫，而因此不會出現在您的程式碼。  
...
##  <a name="a-nameunregistershellfiletypesa--cwinappunregistershellfiletypes"></a><a name="unregistershellfiletypes"></a>CWinApp::UnregisterShellFileTypes  
 呼叫此成員函式取消登錄所有的應用程式的文件類型與 Windows 檔案管理員。  
...
### <a name="parameters"></a>參數  
 ...
 `nCmd`  
 指定要求的說明類型。 如需可能的值，以及它們如何影響的清單`dwData`參數，請參閱[WinHelp](http://msdn.microsoft.com/library/windows/desktop/bb762267) Windows 函式。  
  
### <a name="remarks"></a>備註  
 架構也會呼叫此函式來叫用 WinHelp 的應用程式。  
  
 您的應用程式終止時，架構會自動關閉 WinHelp 的應用程式。  
...
##  <a name="a-namewriteprofilebinarya--cwinappwriteprofilebinary"></a><a name="writeprofilebinary"></a>CWinApp::WriteProfileBinary  
 呼叫此成員函式的指定區段中的應用程式的登錄寫入二進位資料或。INI 檔案。  
...
### <a name="parameters"></a>參數  
 `lpszSection`  
 指向以 null 終止的字串，這個字串指定包含項目的區段。 如果區段不存在，則會建立它。 區段的名稱是大小寫無關。字串可以是任何大寫和小寫字母。  
  
 `lpszEntry`  
 指向以 null 結束的字串，其中包含的值是要寫入其中的項目。 如果指定的區段中沒有項目，則會建立它。  
...
### <a name="example"></a>範例  
 這個範例會使用`CWinApp* pApp = AfxGetApp();`CWinApp 類別說明的方式取得，`WriteProfileBinary`和`GetProfileBinary`可從任何 MFC 應用程式中的函式。  
...
##  <a name="a-namewriteprofileinta--cwinappwriteprofileint"></a><a name="writeprofileint"></a>Cwinapp:: Writeprofileint  
 呼叫此成員函式的指定區段中的應用程式的登錄寫入指定的值或。INI 檔案。  
...
### <a name="example"></a>範例  
 這個範例會使用`CWinApp* pApp = AfxGetApp();`CWinApp 類別說明的方式取得， `WriteProfileString`， `WriteProfileInt`， `GetProfileString`，和`GetProfileInt`可從任何 MFC 應用程式中的函式。  
...
##  <a name="a-namewriteprofilestringa--cwinappwriteprofilestring"></a><a name="writeprofilestring"></a>CWinApp::WriteProfileString  
 呼叫此成員函式，將指定的字串寫入應用程式的登錄中的指定區段或。INI 檔案。  
...
### <a name="parameters"></a>參數  
 ...
 `lpszEntry`  
 指向以 null 結束的字串，其中包含的值是要寫入其中的項目。 如果指定的區段中沒有項目，則會建立它。 如果這個參數是`NULL`，指定之區段`lpszSection`會被刪除。  
  
 `lpszValue`  
 指向要寫入的字串。 如果這個參數是`NULL`，指定的項目`lpszEntry`就會刪除參數。  
...
##  <a name="a-namesetappida--cwinappsetappid"></a><a name="setappid"></a>CWinApp::SetAppID  
 明確地設定應用程式的應用程式使用者的模型識別碼。 任何使用者介面呈現給使用者 （最好是應用程式建構函式） 之前，應該呼叫這個方法。  
