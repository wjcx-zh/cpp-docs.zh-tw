...
|[CWnd：： Closewindowsg](#closewindow)|將視窗最小化。|
...
|[CWnd：:D eleteTempMap](#deletetempmap)|由 `CWinApp` 閒置時間處理常式自動呼叫，並刪除 `CWnd` 建立的任何暫存 `FromHandle` 物件。|
|[CWnd：:D estroyWindow](#destroywindow)|終結附加的 Windowd 視窗。|
|[CWnd：:D etach](#detach)|從 `CWnd` 物件卸離 Windows 控制代碼，並傳回控制代碼。|
|[CWnd：:D lgDirList](#dlgdirlist)|使用檔案或目錄清單填入清單方塊。|
|[CWnd：:D lgDirListComboBox](#dlgdirlistcombobox)|使用檔案或目錄清單來填入下拉式方塊的清單方塊。|
|[CWnd：:D lgDirSelect](#dlgdirselect)|從清單方塊擷取目前的選取範圍。|
|[CWnd：:D lgDirSelectComboBox](#dlgdirselectcombobox)|從下拉式方塊的清單方塊中擷取目前的選取範圍。|
|[CWnd：:D ragAcceptFiles](#dragacceptfiles)|指出視窗將接受拖曳的檔案。|
|[CWnd：:D ragDetect](#dragdetect)|擷取滑鼠並追蹤其移動，直到使用者放開左側按鈕、按下 ESC 鍵，或將滑鼠移到指定點周圍的拖曳矩形外。|
|[CWnd：:D rawAnimatedRects](#drawanimatedrects)|繪製框線矩形，並以動畫效果呈現它來指出圖示的開頭，或最小化或最大化視窗。|
|[CWnd：:D rawCaption](#drawcaption)|繪製標題。|
|[CWnd：:D rawMenuBar](#drawmenubar)|重新繪製功能表列。|
...
|[CWnd：： Ivsfontandcolorstorage.getfont](#getfont)|擷取目前的字型。|
...
|[CWnd：： GetSafeHwnd](#getsafehwnd)|傳回 `m_hWnd`，如果**this**指標是 null，則傳回 null。|
...
|[CWnd：： OpenClipboard](#openclipboard)|開啟剪貼簿。 其他應用程式將無法修改剪貼簿，直到呼叫 Windows [CloseClipboard](/windows/win32/api/winuser/nf-winuser-closeclipboard)函式為止。|
|[CWnd：:P aintWindowlessControls](#paintwindowlesscontrols)|在控制項容器上繪製無視窗控制項。|
|[CWnd：:P ostMessage](#postmessage)|在應用程式佇列中放置訊息，然後傳回，而不需等待視窗處理訊息。|
|[CWnd：:P reCreateWindow](#precreatewindow)|在建立附加至此 `CWnd` 物件的 Windows 視窗前呼叫。|
|[CWnd：:P reSubclassWindow](#presubclasswindow)|允許在呼叫[subclasswindow 前允許](#subclasswindow)之前，先進行其他必要的子類別。|
|[CWnd：:P reTranslateMessage](#pretranslatemessage)|由 `CWinApp` 使用，可先篩選視窗訊息，再將它們分派至 `TranslateMessage` 和 `DispatchMessage` Windows 函式。|
|[CWnd：:P rint](#print)|在指定的裝置內容中繪製目前的視窗。|
|[CWnd：:P rintClient](#printclient)|在指定的裝置內容中繪製任何視窗 (通常是印表機裝置內容)。|
|[CWnd：:P rintWindow](#printwindow)|將視覺視窗複製到指定的裝置內容，通常是印表機 DC。|
...
|[CWnd：： Setlayeredwindowattributes 簡單](#setlayeredwindowattributes)|設定分層視窗的不透明和透明色鍵。|
...
|[CWnd：： SetTimer](#settimer)|安裝系統計時器，以在觸發時傳送[WM_TIMER](#ontimer)訊息。|
...
|[CWnd：： Subclasswindow 前允許](#subclasswindow)|將視窗附加至 `CWnd` 物件，並使其透過 `CWnd` 的訊息對應來路由訊息。|
...
|名稱|描述|
|----------|-----------------|
|[CWnd：:D efault](#default)|呼叫預設視窗程序，提供應用程式不會處理的任何視窗訊息的預設處理程序。|
|[CWnd：:D efWindowProc](#defwindowproc)|呼叫預設視窗程序，提供應用程式不會處理的任何視窗訊息的預設處理程序。|
|[CWnd：:D oDataExchange](#dodataexchange)|適用於對話方塊資料交換和驗證。 由 `UpdateData` 呼叫。|
|[CWnd：： GetCurrentMessage](#getcurrentmessage)|傳回此視窗目前正在處理的訊息指標。 只有在 `On`*訊息*訊息處理常式成員函式中時，才應該呼叫。|
...
|[CWnd：： OnCharToItem](#onchartoitem)|由具有[LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles)樣式的子清單方塊呼叫，以回應[WM_CHAR](#onchar)訊息。|
...
|[CWnd：： OnDestroyClipboard](#ondestroyclipboard)|當剪貼簿透過呼叫 Windows [EmptyClipboard](/windows/win32/api/winuser/nf-winuser-emptyclipboard)函數清空時呼叫。|
...
|[CWnd：： OnMouseHover](#onmousehover)|當游標停留在視窗的工作區上，且在先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定的期間內時呼叫。|
|[CWnd：： OnMouseHWheel](#onmousehwheel)|當目前的視窗由桌面視窗管理員 (DWM) 組成，且該視窗最大化時呼叫。|
|[CWnd：： OnMouseLeave](#onmouseleave)|當游標離開先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定之視窗的工作區時呼叫。|
...
|[CWnd：： OnNcMouseHover](#onncmousehover)|當游標停留在視窗的非工作區上，且在先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定的期間內時呼叫。|
|[CWnd：： OnNcMouseLeave](#onncmouseleave)|當游標離開先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定之視窗的非工作區時，架構會呼叫這個成員函式。|
...
|[CWnd：： OnStyleChanged](#onstylechanged)|表示[SetWindowLong](/windows/win32/api/winuser/nf-winuser-setwindowlongw) Windows 函式已變更一或多個視窗的樣式。|
|[CWnd：： OnStyleChanging](#onstylechanging)|表示[SetWindowLong](/windows/win32/api/winuser/nf-winuser-setwindowlongw) Windows 函數即將變更一或多個視窗的樣式。|
...
|[CWnd：： OnUniChar](#onunichar)|按下按鍵時呼叫。 也就是說，目前的視窗具有鍵盤焦點，而[WM_KEYDOWN](/windows/win32/inputdev/wm-keydown)的訊息則是由[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)函數轉譯。|
...
|[CWnd：： OnVKeyToItem](#onvkeytoitem)|由 `CWnd` 所擁有的清單方塊呼叫，以回應[WM_KEYDOWN](#onkeydown)訊息。|
...
|[CWnd：:P ostNcDestroy](#postncdestroy)|在終結視窗之後，預設[OnNcDestroy](#onncdestroy)函式會呼叫這個虛擬函式。|
...
|[CWnd：： operator！ =](#operator_neq)|判斷視窗是否與其控制碼[m_hWnd](#m_hwnd)的視窗不同。|
|[CWnd：： operator = =](#operator_eq_eq)|判斷視窗是否與其控制碼[m_hWnd](#m_hwnd)的視窗相同。|

### <a name="public-data-members"></a>公用資料成員

|名稱|描述|
|----------|-----------------|
|[CWnd：： m_hWnd](#m_hwnd)|表示附加至此 `CWnd`的 HWND。|

## <a name="remarks"></a>備註

`CWnd` 物件不同於 Windows 視窗，但兩者緊密連結。 `CWnd` 物件是由 `CWnd` 建構函式和解構函式所建立或終結。 另一方面，Windows 視窗則是 Windows 內部的資料結構，由 `Create` 成員函式建立，並由 `CWnd` 虛擬的析構函數終結。 [DestroyWindow](#destroywindow)函式會終結 Windows 視窗，而不會終結物件。

`CWnd` 類別和訊息對應機制會隱藏 `WndProc` 函數。 傳入的 Windows 通知訊息會透過訊息對應自動路由至<em>訊息</em>`CWnd` 成員函式**上**的適當。 您可以覆寫**On**<em>Message</em>成員函式，以處理衍生類別中的成員特定訊息。

`CWnd` 類別也可讓您建立應用程式的 Windows 子視窗。 自 `CWnd` 衍生類別，然後將成員變數加入至衍生類別以儲存應用程式的特定資料。 實作訊息處理常式成員函式，和衍生類別中對應的訊息，以指定訊息被導向至視窗時會發生什麼事。

您在兩個步驟中建立子視窗。 首先，呼叫 `CWnd` 的函式，以建立 `CWnd` 物件，然後呼叫[create](#create)成員函式來建立子視窗，並將它附加至 `CWnd` 物件。

當使用者終止子視窗時，摧毀 `CWnd` 物件，或呼叫 `DestroyWindow` 成員函式來移除視窗並摧毀其資料結構。

在 Microsoft Foundation 類別庫中，會自 `CWnd` 衍生進一步類別以提供特定的視窗類型。 其中許多類別（包括[CFrameWnd](../../mfc/reference/cframewnd-class.md)、 [CMDIFrameWnd](../../mfc/reference/cmdiframewnd-class.md)、 [CMDIChildWnd](../../mfc/reference/cmdichildwnd-class.md)、 [CView](../../mfc/reference/cview-class.md)和[CDialog](../../mfc/reference/cdialog-class.md)）都是針對進一步的衍生而設計的。 衍生自 `CWnd`的控制項類別（例如[CButton](../../mfc/reference/cbutton-class.md)）可以直接使用，或可用於進一步衍生類別。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 的[IAccessible：： accDoDefaultAction](/windows/win32/api/oleacc/nf-oleacc-iaccessible-accdodefaultaction)中的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以執行物件的預設動作。 如需詳細資訊，請參閱 Windows SDK 中的[IAccessible：： accDoDefaultAction](/windows/win32/api/oleacc/nf-oleacc-iaccessible-accdodefaultaction) 。
...
*pvarChild*<br/>
接收在*xLeft*和*yTop*所指定的點上識別物件的資訊。 請參閱 Windows SDK 的[IAccessible：： accHitTest](/windows/win32/api/oleacc/nf-oleacc-iaccessible-acchittest)中的*pvarID* 。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::accHitTest` 的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

如果您有 nonwindowed 使用者介面專案（MFC 所處理的無視窗 ActiveX 控制項），請在您的 `CWnd`衍生類別中覆寫這個函式。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::accLocation` 的傳回**值**。

### <a name="remarks"></a>備註

如果您有 nonwindowed 使用者介面專案（MFC 所處理的無視窗 ActiveX 控制項），請在您的 `CWnd`衍生類別中覆寫這個函式。

如需詳細資訊，請參閱 Windows SDK 中的 `IAccessible::accLocation`。
...
*navDir*<br/>
指定要流覽的方向。 請參閱 Windows SDK 的[IAccessible：： accNavigate](/windows/win32/api/oleacc/nf-oleacc-iaccessible-accnavigate)中的*navDir* 。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::accNavigate` 的傳回**值**。
...
*flagsSelect*<br/>
指定如何變更目前的選取範圍或焦點。 請參閱 Windows SDK 的[IAccessible：： accSelect](/windows/win32/api/oleacc/nf-oleacc-iaccessible-accselect)中的*flagsSelect* 。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::accSelect` 的傳回**值**。
...
此成員函式會模擬函數[AnimateWindow](/windows/win32/api/winuser/nf-winuser-animatewindow)的功能，如 Windows SDK 中所述。
...
準備 `CWnd` 以使用繪製的相關資訊來繪製和填滿 `PAINTSTRUCT` 的資料結構。
...
識別 `CWnd`的裝置內容。 指標可能是暫時性的，不能儲存在[EndPaint](#endpaint)的範圍外。

### <a name="remarks"></a>備註

油漆結構包含矩形資料結構，其具有完全括住更新區域的最小矩形，以及指定是否已清除背景的旗標。

更新區域是由 [[無效](#invalidate)]、[ [InvalidateRect](#invalidaterect)] 或 [ [InvalidateRgn](#invalidatergn) ] 成員函式所設定，而且會在其大小、移動、建立、滾動或執行任何其他會影響工作區的作業之後，由系統來設定。 如果更新區域標示為要清除，`BeginPaint` 會傳送[WM_ONERASEBKGND](#onerasebkgnd)訊息。

除了回應[WM_PAINT](#onpaint)訊息以外，請勿呼叫 `BeginPaint` 成員函式。 `BeginPaint` 成員函式的每個呼叫都必須有相符的[EndPaint](#endpaint)成員函式呼叫。 如果插入號位於要繪製的區域中，則 `BeginPaint` 成員函式會自動隱藏插入號，以防止它被清除。
...
*pDSCWnd*<br/>
指向裝載屬性將系結之資料來源控制項的視窗。 使用 DC 主機視窗的資源識別碼呼叫 `GetDlgItem`，以取得此指標。

### <a name="remarks"></a>備註

您呼叫這個函式的 `CWnd` 物件必須是資料繫結控制項。

### <a name="example"></a>範例

`BindDefaultProperty` 可能會在下列內容中使用：
...
此函式只會呼叫 Win32 [BringWindowToTop](/windows/win32/api/winuser/nf-winuser-bringwindowtotop)函式。 呼叫[SetWindowPos](#setwindowpos)函式以變更視窗在迭置順序中的位置。 `BringWindowToTop` 函式不會將視窗樣式變更為最上層視窗。 如需詳細資訊，請參閱[HWND_TOP 和 HWND_TOPMOST 之間的差異](https://devblogs.microsoft.com/oldnewthing/?p=33263)
...
*nAdjustType*<br/>
在使用 `CWnd::adjustBorder` 來計算不含 WS_EX_CLIENTEDGE 樣式的視窗座標;否則，請使用 `CWnd::adjustOutside`。
...
從剪貼簿檢視器鏈中移除 `CWnd`，並藉由*hWndNext*鏈中 `CWnd` 上階的下階來指定視窗。
...
*nCheck*<br/>
指定要採取的動作。 如果*nCheck*為非零值，`CheckDlgButton` 成員函式會在按鈕旁邊放置核取記號;如果為0，則會移除核取記號。 針對三個狀態的按鈕，如果*nCheck*為2，則按鈕狀態為 [不確定]。

### <a name="remarks"></a>備註

`CheckDlgButton` 函式會將[BM_SETCHECK](/windows/win32/Controls/bm-setcheck)訊息傳送至指定的按鈕。
...
`CheckRadioButton` 函式會將[BM_SETCHECK](/windows/win32/Controls/bm-setcheck)訊息傳送至指定的選項按鈕。
...
識別包含點的子視窗。 如果指定的點位於工作區的外部，則為 Null。 如果該點在工作區中，但未包含在任何子視窗內，則會傳回 `CWnd`。

此成員函式會傳回隱藏或停用的子視窗，其中包含指定的點。

一個以上的視窗可能會包含指定的點。 不過，此函式只會傳回包含點之第一個視窗的 `CWnd`*。

傳回的 `CWnd`* 可能是暫時性的，而且不應該儲存以供稍後使用。
...
*lpPoint*<br/>
指向[點結構](/windows/win32/api/windef/ns-windef-point)或 `CPoint` 物件，其中包含要轉換的用戶端座標。

*lpRect*<br/>
指向[矩形結構](/windows/win32/api/windef/ns-windef-rect)或 `CRect` 物件，其中包含要轉換的用戶端座標。

### <a name="remarks"></a>備註

`ClientToScreen` 成員函式會使用 `POINT` 或 `RECT` 結構中的用戶端座標，或由*lpPoint*或*lpRect*所指向的 `CPoint` 或 `CRect` 物件，來計算新的螢幕座標;然後，它會以新座標取代結構中的座標。 新的螢幕座標會相對於系統顯示的左上角。

`ClientToScreen` 成員函式假設給定的點或矩形是在用戶端座標中。
...
此成員函式會模擬函數[closewindowsg](/windows/win32/api/winuser/nf-winuser-closewindow)的功能，如 Windows SDK 中所述。
...
根據預設，它會傳回非零，直到呼叫 `EndModalLoop` 為止。
...
> [!WARNING]
> `CWnd::PreCreateWindow` 現在會將其 `CREATESTRUCT` 參數的 hMenu 成員指派給**this**指標（如果功能表為 Null，且樣式包含 WS_CHILD）。 如需適當的功能，請確定您的對話方塊控制項具有不是 Null 的識別碼。
>
> 這項變更會修正 managed/原生 interop 案例中的損毀。 `CWnd::Create` 中的追蹤語句會對問題的開發人員發出警示。
...
[CWnd：： OnCreate](#oncreate)方法會在 `Create` 方法傳回之前，以及視窗變成可見之前呼叫。
...
點陣圖必須先前由[CBitmap：： CreateBitmap](../../mfc/reference/cbitmap-class.md#createbitmap)成員函式、 [CreateDIBitmap](/windows/win32/api/wingdi/nf-wingdi-createdibitmap) Windows 函數或[CBitmap：： LoadBitmap](../../mfc/reference/cbitmap-class.md#loadbitmap)成員函式建立。

`CreateCaret` 會自動終結先前的插入號圖形（如果有的話），而不論哪一個視窗擁有插入號。 一旦建立之後，插入號就會一開始隱藏。 若要顯示插入號，必須呼叫[ShowCaret](#showcaret)成員函式。

系統插入號是共用資源。 `CWnd` 只有在具有輸入焦點或為作用中時，才會建立插入號。 它應該先終結插入號，再失去輸入焦點或變成非作用中。
...
使用這個成員函式來建立 ActiveX 控制項，以 `CWnd` 物件在 MFC 程式中表示。
...
*pszClass*<br/>
這個字串可能包含類別的 OLE "short name" （ProgID），例如 "CIRC3。Circ3Ctrl。 1 "。 名稱必須符合控制項所註冊的相同名稱。 或者，此字串可能包含以括弧括住之 CLSID 的字串形式，例如 "{9DBAFCCF-592F-101B-85CE-00608CEC297B}"。 不論是哪一種情況，`CreateControl` 都會將字串轉換成對應的類別識別碼。
...
*ppt*<br/>
指向包含控制項左上角的[POINT 結構](/windows/win32/api/windef/ns-windef-point)或 `CPoint` 物件。
...
*pPersist*<br/>
[CFile](../../mfc/reference/cfile-class.md)的指標，其中包含控制項的持續狀態。 預設值為 Null，表示控制項會自行初始化，而不會從任何持續性儲存體還原其狀態。 如果不是 Null，它應該是包含控制項之持續性資料的 `CFile`衍生物件的指標，其格式為數據流或儲存體。 這項資料可能已儲存在用戶端先前的啟用中。 `CFile` 可以包含其他資料，但在呼叫 `CreateControl`時，必須將其讀寫指標設定為持續性資料的第一個位元組。
...
`CreateControl` 是[CWnd：： Create](#create)函數的直接類比，它會建立 `CWnd`的視窗。 `CreateControl` 會建立 ActiveX 控制項，而不是一般視窗。

`CreateControl`只支援一部分的 Windows *dwStyle*旗標：
...
> `CWnd::PreCreateWindow` 現在會將其 `CREATESTRUCT` 參數的 hMenu 成員指派給**this**指標（如果功能表為 Null，且樣式包含 WS_CHILD）。 如需適當的功能，請確定您的對話方塊控制項具有不是 Null 的識別碼。
>
> 這項變更會修正 managed/原生 interop 案例中的損毀。 `CWnd::Create` 中的 `TRACE` 語句會警示問題的開發人員。

預設的擴充視窗樣式為 WS_EX_LEFT。 預設視窗樣式為 WS_OVERLAPPED。

使用[AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass)函數來註冊視窗類別。 使用者定義 視窗的模組中的類別可註冊的位置。

子視窗的維度相對於父視窗工作區的左上角。 最上層視窗的維度相對於螢幕的左上角。

[CWnd：： OnCreate](#oncreate)方法會在 `CreateEx` 方法傳回之前，以及視窗變成可見之前呼叫。
...
*nWidth*<br/>
指定插入號的寬度（以邏輯單位表示）。 如果此參數為0，則寬度會設定為系統定義的視窗框線寬度。

*nHeight*<br/>
指定插入號的高度（以邏輯單位表示）。 如果此參數為0，則高度會設定為系統定義的視窗-框線高度。
...
[GetSystemMetrics](/windows/win32/api/winuser/nf-winuser-getsystemmetrics) Windows 函數可以使用 SM_CXBORDER 和 SM_CYBORDER 索引來抓取系統的視窗框線寬度或高度。 使用 [視窗框線寬度] 或 [高度] 可確保在高解析度顯示時，會顯示插入號。
...
刪除 `FromHandle` 成員函式所建立的任何暫存 `CWnd` 物件。
...
`DestroyWindow` 成員函式會將適當的訊息傳送至視窗，以將其停用，並移除輸入焦點。 它也會終結視窗的功能表、排清應用程式佇列、終結未處理的計時器、移除剪貼簿擁有權，以及中斷剪貼簿-檢視器鏈（如果 `CWnd` 位於檢視器鏈的頂端）。 它會將[WM_DESTROY](#ondestroy)和[WM_NCDESTROY](#onncdestroy)訊息傳送至視窗。 它不會損毀 `CWnd` 物件。

`DestroyWindow` 是執行清除的預留位置。 因為 `DestroyWindow` 是虛擬函式，所以會顯示在類別檢視的任何 `CWnd`衍生類別中。 但是即使您在 `CWnd`衍生的類別中覆寫這個函式，也不一定會呼叫 `DestroyWindow`。 如果未在 MFC 程式碼中呼叫 `DestroyWindow`，則您必須在您自己的程式碼中明確地呼叫它，以便在您想要呼叫它。

例如，假設您已在 `CView`衍生類別中覆寫 `DestroyWindow`。 因為 MFC 原始程式碼不會在其任何 `CFrameWnd`衍生的類別中呼叫 `DestroyWindow`，除非您明確地呼叫它，否則將不會呼叫覆寫的 `DestroyWindow`。

如果視窗是任何視窗的父系，則當父視窗終結時，會自動終結這些子視窗。 `DestroyWindow` 成員函式會先終結子視窗，然後再銷毀視窗本身。

`DestroyWindow` 成員函式也會終結[CDialog：： Create](../../mfc/reference/cdialog-class.md#create)所建立的非強制回應對話方塊。

如果要終結的 `CWnd` 是子視窗，而且沒有設定[WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles)樣式，則[WM_PARENTNOTIFY](/previous-versions/windows/desktop/inputmsg/wm-parentnotify)訊息會傳送至父代。
...
*nIDListBox*<br/>
指定清單方塊的識別碼。 如果*nIDListBox*為0，`DlgDirList` 會假設沒有清單方塊存在，而且不會嘗試填入一個。

*nIDStaticPath*<br/>
指定用來顯示目前磁片磁碟機和目錄之靜態文字控制項的識別碼。 如果*nIDStaticPath*為0，`DlgDirList` 會假設沒有這類文字控制項存在。

*nFileType*<br/>
指定要顯示之檔案的屬性。 它可以是下列值的任意組合：

- DDL_READWRITE 讀寫資料檔案，但不含其他屬性。
...
- DDL_POSTMSGS LB_DIR 旗標。 如果設定了 LB_DIR 旗標，Windows 就會將 `DlgDirList` 產生的訊息放在應用程式的佇列中;否則，它們會直接傳送至對話方塊程式。

- DDL_DRIVES 磁片磁碟機。 如果已設定 DDL_DRIVES 旗標，則會自動設定 DDL_EXCLUSIVE 旗標。 因此，若要建立包含磁片磁碟機和檔案的目錄清單，您必須呼叫 `DlgDirList` 兩次：一次是設定 DDL_DRIVES 旗標，而另一次是使用清單的其餘部分。

- DDL_EXCLUSIVE 獨佔位。 如果設定獨佔位，則只會列出指定類型的檔案。否則會列出所指定類型的一般檔案和檔案。
...
`DlgDirList` 會將[LB_RESETCONTENT](/windows/win32/Controls/lb-resetcontent)和[LB_DIR](/windows/win32/Controls/lb-dir)訊息傳送至清單方塊。 它會將*nIDListBox*所指定的清單方塊填入所有檔案的名稱，這些檔案符合*lpPathSpec*所指定的路徑。

*LpPathSpec*參數具有下列格式：

`[drive:] [ [\u]directory[\idirectory]...\u] [filename]`

在此範例中，*磁片磁碟機*是磁碟機號，*目錄*是有效的目錄名稱，*檔案名*則是至少必須包含一個萬用字元的有效檔案名。 萬用字元是一個問號（ **？** ），這表示符合任何字元，而星號（<strong>\*</strong>）表示符合任何字元數。
...
*lpPathSpec*<br/>
指向包含路徑或檔案名的以 null 終止的字串。 `DlgDirListComboBox` 修改此字串，因此此資料的格式不應為字串常值。 請參閱下列「備註」一節。

*nIDComboBox*<br/>
在對話方塊中指定下拉式方塊的識別碼。 如果*nIDComboBox*為0，`DlgDirListComboBox` 會假設沒有下拉式方塊存在，而且不會嘗試填入其中一個。

*nIDStaticPath*<br/>
指定用來顯示目前磁片磁碟機和目錄之靜態文字控制項的識別碼。 如果*nIDStaticPath*為0，`DlgDirListComboBox` 會假設沒有這類文字控制項存在。
...
- DDL_POSTMSGS CB_DIR 旗標。 如果設定了 CB_DIR 旗標，Windows 就會將 `DlgDirListComboBox` 產生的訊息放在應用程式的佇列中;否則，它們會直接傳送至對話方塊程式。

- DDL_DRIVES 磁片磁碟機。 如果已設定 DDL_DRIVES 旗標，則會自動設定 DDL_EXCLUSIVE 旗標。 因此，若要建立包含磁片磁碟機和檔案的目錄清單，您必須呼叫 `DlgDirListComboBox` 兩次：一次是設定 DDL_DRIVES 旗標，而另一次是使用清單的其餘部分。
...
`DlgDirListComboBox` 會將[CB_RESETCONTENT](/windows/win32/Controls/cb-resetcontent)和[CB_DIR](/windows/win32/Controls/cb-dir)訊息傳送至下拉式方塊。 它會將*nIDComboBox*所指定之下拉式方塊的清單方塊填入所有檔案的名稱，這些檔案符合*lpPathSpec*所指定的路徑。

*LpPathSpec*參數具有下列格式：

`[drive:] [ [\u]directory[\idirectory]...\u] [filename]`

在此範例中，*磁片磁碟機*是磁碟機號，*目錄*是有效的目錄名稱，*檔案名*則是至少必須包含一個萬用字元的有效檔案名。 萬用字元是一個問號（ **？** ），這表示符合任何字元，而星號（<strong>\*</strong>）表示比對任意數目的字元。
...
`DlgDirSelect` 成員函式會將選取範圍複製到*lpString*所指定的緩衝區。 如果沒有選取專案， *lpString*就不會變更。

`DlgDirSelect` 會將[LB_GETCURSEL](/windows/win32/Controls/lb-getcursel)和[LB_GETTEXT](/windows/win32/Controls/lb-gettext)訊息傳送至清單方塊。
...
`DlgDirSelectComboBox` 成員函式會將選取範圍複製到指定的緩衝區。 如果沒有選取專案，則不會變更緩衝區的內容。

`DlgDirSelectComboBox` 會將[CB_GETCURSEL](/windows/win32/Controls/cb-getcursel)和[CB_GETLBTEXT](/windows/win32/Controls/cb-getlbtext)訊息傳送至下拉式方塊。
...
若要自動產生這個成員函式的覆寫版本，請先使用對話方塊編輯器建立對話方塊資源，然後再衍生應用程式特定的對話方塊類別。 變數、 資料和驗證範圍與新的對話方塊中的各種控制項，然後使用 加入變數的精靈。 然後，此 wizard 會寫入已覆寫的 `DoDataExchange`，其中包含資料對應。 以下是加入變數 精靈所產生的範例 DDX/DDV 程式碼區塊：

[!code-cpp[NVC_MFCWindowing#90](../../mfc/reference/codesnippet/cpp/cwnd-class_29.cpp)]

`DoDataExchange` 覆寫的成員函式必須在原始程式檔中的巨集式之前。

如需對話方塊資料交換和驗證的詳細資訊，請參閱[顯示和操作表單中的資料](../../data/odbc/displaying-and-manipulating-data-in-a-form.md)和[對話方塊資料交換和驗證](../../mfc/dialog-data-exchange-and-validation.md)。 如需 [新增變數] wizard 所產生之 DDX_ 和 DDV_ 宏的說明，請參閱[技術提示 26](../../mfc/tn026-ddx-and-ddv-routines.md)。

##  <a name="cwnddragacceptfiles"></a><a name="dragacceptfiles"></a>CWnd：:D ragAcceptFiles

在應用程式的[CWinApp：： InitInstance](../../mfc/reference/cwinapp-class.md#initinstance)函式中，使用 `CWnd` 指標從視窗內呼叫這個成員函式，以指示視窗接受從 Windows 檔管理程式或檔案瀏覽器卸載的檔案。
...
只有呼叫*bAccept*參數設為 TRUE 之 `DragAcceptFiles` 的視窗已識別為能夠處理 Windows 訊息 WM_DROPFILES。 例如，在 MDI 應用程式中，如果 `CMDIFrameWnd` 的視窗指標用於 `DragAcceptFiles` 函式呼叫中，只有 `CMDIFrameWnd` 視窗會取得 WM_DROPFILES 訊息。 此訊息不會傳送至所有開啟的 `CMDIChildWnd` 視窗。 若要讓 `CMDIChildWnd` 視窗接收此訊息，您必須使用 `CMDIChildWnd` 視窗指標呼叫 `DragAcceptFiles`。
...
此成員函式會模擬函數[DragDetect](/windows/win32/api/winuser/nf-winuser-dragdetect)的功能，如 Windows SDK 中所述。
...
此成員函式會模擬函數[DrawAnimatedRects](/windows/win32/api/winuser/nf-winuser-drawanimatedrects)的功能，如 Windows SDK 中所述。
...
*uFlags*<br/>
指定繪圖選項。 如需值的完整清單，請參閱[DrawCaption](/windows/win32/api/winuser/nf-winuser-drawcaption)。
...
此成員函式會模擬函數[DrawCaption](/windows/win32/api/winuser/nf-winuser-drawcaption)的功能，如 Windows SDK 中所述。
...
MFC 的預設 Active Accessibility 支援對標準視窗和控制項（包括 ActiveX 控制項）而言已足夠。不過，如果您的 `CWnd`衍生類別包含 nonwindowed 的使用者介面專案，MFC 就無法得知這些元素。 在這種情況下，您必須在類別中覆寫適當的[Active Accessibility 成員](/windows/win32/winauto/sdk-components)函式，而且您必須在類別的函式中呼叫 `EnableActiveAccessibility`。
...
除非父視窗衍生自 `CFrameWnd`，否則只呼叫 `EnableToolTips` 不足以顯示子控制項的工具提示。 這是因為 `CFrameWnd` 會提供 TTN_NEEDTEXT 通知的預設處理常式。 如果您的父視窗不是衍生自 `CFrameWnd`，也就是說，如果它是對話方塊或表單檢視，除非您提供 TTN_NEEDTEXT 工具提示通知的處理常式，否則子控制項的工具提示不會正確顯示。 請參閱[工具提示](../../mfc/tool-tips-in-windows-not-derived-from-cframewnd.md)。

`EnableToolTips` 為您的視窗提供的預設工具提示，沒有相關聯的文字。 若要取得工具提示顯示的文字，在顯示工具提示視窗之前，TTN_NEEDTEXT 通知會傳送至工具提示控制項的父視窗。 如果此訊息沒有處理常式可將某個值指派給 `TOOLTIPTEXT` 結構的*pszText*成員，則工具提示不會顯示任何文字。
...
指出呼叫 `EnableWindow` 成員函式之前的狀態。 如果視窗先前已停用，則傳回值為非零。 如果先前已啟用視窗或發生錯誤，則傳回值為0。
...
若停用，則會隱含停用所有子視窗，但不會將它們傳送 WM_ENABLE 訊息。
...
*lpPaint*<br/>
指向包含[BeginPaint](#beginpaint)成員函式所抓取之繪製資訊的[PAINTSTRUCT](/windows/win32/api/winuser/ns-winuser-paintstruct)結構。

### <a name="remarks"></a>備註

`BeginPaint` 成員函式的每個呼叫都需要 `EndPaint` 成員函式，但只有在完成繪製之後。

如果 `BeginPaint` 成員函式已隱藏插入號，`EndPaint` 會將插入號還原到螢幕上。
...
`ExecuteDlgInit` 會使用系結至執行中模組的資源，或其他來源的資源。 若要達成此目的，`ExecuteDlgInit` 藉由呼叫 `AfxFindResourceHandle`來尋找資源控制碼。 如果您的 MFC 應用程式不使用共用 DLL （MFCx0 [U] [D]。DLL），`AfxFindResourceHandle` 呼叫[AfxGetResourceHandle](application-information-and-management.md#afxgetresourcehandle)，它會傳回可執行檔的目前資源控制碼。 如果您的 MFC 應用程式使用 MFCx0 [U] [D]。DLL，`AfxFindResourceHandle` 會取得共用和 MFC 延伸 Dll 的 `CDynLinkLibrary` 物件清單，以尋找正確的資源控制碼。
...
傳回最上層的 `CWnd`，其 window 類別是由*lpszClassName*所指定，而其視窗名稱（或標題）是由*lpszWindowName*提供。
...
*lpszClass*<br/>
以 null 結束的字串指標，指定類別名稱或先前呼叫[RegisterClass](/windows/win32/api/winuser/nf-winuser-registerclassw)或[RegisterClassEx](/windows/win32/api/winuser/nf-winuser-registerclassexw)所建立的類別 atom。
...
此成員函式會模擬函數[FindWindowEx](/windows/win32/api/winuser/nf-winuser-findwindowexw)的功能，如 Windows SDK 中所述。
...
*bInvert*<br/>
指定 `CWnd` 是否要快閃或回到其原始狀態。 如果*bInvert*為 TRUE，則會從某個狀態將 `CWnd` 閃至另一個狀態。 如果*bInvert*為 FALSE，則視窗會回到其原始狀態（作用中或非作用中）。

### <a name="return-value"></a>傳回值

如果視窗在呼叫 `FlashWindow` 成員函式之前為作用中，則為非零值;否則為0。

### <a name="remarks"></a>備註

若為連續閃爍，請建立系統計時器，並重複呼叫 `FlashWindow`。 閃爍 `CWnd` 表示變更標題列的外觀，如同 `CWnd` 從非作用中變更為作用中狀態，或反之亦然。 （非作用中的標題列會變更為使用中的標題列，而作用中的標題列則會變更為非作用中的標題列）。
...
*dwFlags*<br/>
指定 flash 狀態。 如需值的完整清單，請參閱[FLASHWINFO](/windows/win32/api/winuser/ns-winuser-flashwinfo)結構。
...
這個方法會模擬函數[FlashWindowEx](/windows/win32/api/winuser/nf-winuser-flashwindowex)的功能，如 Windows SDK 中所述。
...
*ppdispChild*<br/>
接收子物件之 `IDispatch` 介面的位址。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accChild](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accchild)的傳回**值**。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accChildCount](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accchildcount)的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

如果您有 nonwindowed 使用者介面專案（MFC 所處理的無視窗 ActiveX 控制項），請在您的 `CWnd`衍生類別中覆寫這個函式。 呼叫基底類別版本，然後再加入 nonwindowed 子項目。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accDefaultAction](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accdefaultaction)的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以描述物件的預設動作。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accDescription](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accdescription)的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以描述您的物件。 呼叫基類版本，並新增您的描述。
...
*pvarChild*<br/>
接收具有焦點之物件的相關資訊。 請參閱 Windows SDK 中的*pvarID* in [IAccessible：： get_accFocus](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accfocus) 。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::get_accFocus` 的傳回**值**。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accHelp](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_acchelp)的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以提供物件的解說文字。
...
*pidTopic*<br/>
識別與指定物件相關聯的說明檔主題。 請參閱 Windows SDK 中的*pidTopic* in [IAccessible：： get_accHelpTopic](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_acchelptopic) 。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::get_accHelpTopic` 的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以提供有關物件的說明資訊。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accKeyboardShortcut](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_acckeyboardshortcut)的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以識別物件的鍵盤快速鍵。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accName](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accname)的傳回**值**。

### <a name="remarks"></a>備註

此函式是 MFC [Active Accessibility](/windows/win32/WinAuto/microsoft-active-accessibility)支援的一部分。

在您的 `CWnd`衍生類別中覆寫這個函式，以傳回物件的名稱。
...
*ppdispParent*<br/>
接收父物件之 `IDispatch` 介面的位址。 如果父系不存在，或子系無法存取其父系，則會將變數設定為 Null。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accParent](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accparent)的傳回**值**。
...
*pvarRole*<br/>
接收角色資訊。 請參閱 Windows SDK 中的*pvarRole* in [IAccessible：： get_accRole](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accrole) 。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::get_accRole` 的傳回**值**。
...
*pvarChildren*<br/>
接收所選子系的相關資訊。 請參閱 Windows SDK 中的*pvarChildren* in [IAccessible：： get_accSelection](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accselection) 。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::get_accSelection` 的傳回**值**。
...
*pvarState*<br/>
接收物件狀態的相關資訊。 請參閱 Windows SDK 中的*pvarState* in [IAccessible：： get_accState](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accstate) 。

### <a name="return-value"></a>傳回值

傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中 `IAccessible::get_accState` 的傳回**值**。
...
傳回成功時的 S_OK、失敗時的 COM 錯誤碼。 請參閱 Windows SDK 中[IAccessible：： get_accValue](/windows/win32/api/oleacc/nf-oleacc-iaccessible-get_accvalue)的傳回**值**。
...
*gaFlags*<br/>
指定要抓取的祖系。 如需可能值的完整清單，請參閱[GetAncestor](/windows/win32/api/winuser/nf-winuser-getancestor)。
...
此成員函式會模擬函數[GetAncestor](/windows/win32/api/winuser/nf-winuser-getancestor)的功能，如 Windows SDK 中所述。
...
抓取插入號之目前位置的用戶端座標，並將其傳回為 `CPoint`。
...
將 `CWnd` 工作區的用戶端座標複製到*lpRect*所指向的結構中。
...
*lpRect*<br/>
指向[矩形結構](/windows/win32/api/windef/ns-windef-rect)或 `CRect` 物件，以接收用戶端座標。 `left` 和 `top` 成員將會是0。 `right` 和 `bottom` 成員將包含視窗的寬度和高度。

### <a name="remarks"></a>備註

用戶端座標會指定工作區的左上角和右下角。 因為用戶端座標是相對於 `CWnd` 工作區的左上角，所以左上角的座標為（0，0）。
...
這個 `CWnd` 物件所表示之 OLE 控制項的[IUnknown](/windows/win32/api/unknwn/nn-unknwn-iunknown)介面指標。 如果這個物件不代表 OLE 控制項，則傳回值會是 Null。

### <a name="remarks"></a>備註

您不應該釋放此 `IUnknown` 指標。 一般來說，您會使用來取得控制項的特定介面。

`GetControlUnknown` 傳回的介面指標不是參考計數。 請勿在指標上呼叫[iunknown：： Release](/windows/win32/api/unknwn/nf-unknwn-iunknown-release) ，除非您先前已在其上呼叫 iunknown [：： AddRef](/windows/win32/api/unknwn/nf-unknwn-iunknown-addref) 。
...
傳回包含視窗目前正在處理之訊息的[MSG](/windows/win32/api/winuser/ns-winuser-msg)結構的指標。 只有在<em>訊息</em>處理常式**上**的時，才應該呼叫。
...
根據為 `CWnd`所指定的類別樣式，抓取工作區之通用、類別或私用裝置內容的指標。
...
對於常見的裝置內容，`GetDC` 會在每次抓取時，將預設屬性指派給內容。 針對類別和私用內容，`GetDC` 將先前指派的屬性保留不變。 裝置內容可用於後續的圖形裝置介面（GDI）函式，以便在工作區中繪製。

除非裝置內容屬於視窗類別，否則必須呼叫[ReleaseDC](#releasedc)成員函式，以便在繪製之後釋放內容。

如果在註冊類別時，將 CS_CLASSDC、CS_OWNDC 或 CS_PARENTDC 指定為 `WNDCLASS` 結構中的樣式，`GetDC` 成員函式會傳回屬於 `CWnd` 類別的裝置內容。
...
- DCX_CLIPCHILDREN 在 [`CWnd`] 視窗底下排除所有子視窗的可見區域。

- DCX_CLIPSIBLINGS 排除 `CWnd` 視窗上方所有同輩視窗的可見區域。

- DCX_EXCLUDERGN 從傳回的裝置內容的可見區域中排除*prgnClip*所識別的裁剪區域。

- DCX_INTERSECTRGN 會與所傳回之裝置內容的可見區域內*prgnClip*所識別的裁剪區域相交。

- DCX_LOCKWINDOWUPDATE 即使有作用中的 `LockWindowUpdate` 呼叫，也會允許繪圖，否則會排除此視窗。 這個值會在追蹤期間用於繪製。

- DCX_PARENTCLIP 使用父視窗的可見區域，並忽略父視窗的 WS_CLIPCHILDREN 和 WS_PARENTDC 的樣式位。 這個值會將裝置內容的原點設定為 `CWnd` 視窗的左上角。
...
此函式是[GetDC](/windows/win32/api/winuser/nf-winuser-getdc)函數的擴充功能，可讓應用程式更充分掌控視窗的裝置內容是否已裁剪。

除非裝置內容屬於視窗類別，否則必須呼叫[ReleaseDC](/windows/win32/api/winuser/nf-winuser-releasedc)函式，以在繪製之後釋放內容。 由於在任何指定的時間都只能使用五個常見的裝置內容，因此無法釋放裝置內容，可能會導致其他應用程式無法存取裝置內容。

若要取得快取的裝置內容，應用程式必須指定[DCX_CACHE](/windows/win32/api/winuser/nf-winuser-getdcex)。 如果未指定 DCX_CACHE，而且視窗既不是 CS_OWNDC 也不是[CS_CLASSDC](/windows/win32/api/winuser/ns-winuser-wndclassw)，則此函式會傳回 Null。

如果在註冊類別時，在[WNDCLASS](/windows/win32/api/winuser/ns-winuser-wndclassw)結構中指定了 CS_CLASSDC、 [CS_OWNDC](/windows/win32/api/winuser/ns-winuser-wndclassw)或[CS_PARENTDC](/windows/win32/api/winuser/ns-winuser-wndclassw)樣式，則[GetDCEx](/windows/win32/api/winuser/nf-winuser-getdcex)函數會傳回具有特殊特性的裝置內容。

如需有關這些特性的詳細資訊，請參閱 Windows SDK 中的 `WNDCLASS` 結構的描述。
...
發生錯誤時（例如遇到非數值字元，超過上述最大值），`GetDlgItemInt` 會將0複製到*lpTrans*所指向的位置。 如果沒有任何錯誤， *lpTrans*會收到非零值。 如果*lpTrans*為 Null，`GetDlgItemInt` 不會警告有關錯誤。

### <a name="remarks"></a>備註

它會將指定之控制項的文字轉譯成整數值，方法是在文字開頭處去除任何多餘的空格並轉換小數位數。 它會在到達文字結尾或遇到任何非數值字元時停止轉譯。

如果*bSigned*為 TRUE，`GetDlgItemInt` 會檢查文字開頭的減號（-），並將文字轉譯成帶正負號的數位。 否則，它會建立不帶正負號的值。
...
`GetDlgItemText` 成員函式會將文字複製到*lpStr*所指向的位置，並傳回其複製的位元組數計數。
...
資料來源控制項所定義之游標的指標。 MFC 會負責呼叫指標的 `AddRef`。

### <a name="remarks"></a>備註

使用傳回的指標來設定複雜資料繫結控制項的 ICursor 屬性，例如資料繫結方格控制項。 在第一個繫結控制項要求其資料指標之前，資料來源控制項將不會變成作用中狀態。 這可能是由呼叫 `GetDSCCursor` 明確地進行，或由 MFC 系結管理員隱含地發生。 不論是哪一種情況，您都可以藉由呼叫 `GetDSCCursor` 然後在傳回的指標上呼叫 `Release`，來強制資料來源控制項變成使用中，以 `IUnknown`。 啟用會導致資料來源控制項嘗試連接到基礎資料來源。 傳回的指標可能會在下列內容中使用：
...
##  <a name="cwndgetfont"></a><a name="getfont"></a>CWnd：： Ivsfontandcolorstorage.getfont
...
除非視窗處理 WM_GETFONT 訊息，否則此方法不會有任何作用。 許多衍生自 `CWnd` 的 MFC 類別會處理此訊息，因為它們附加至預先定義的視窗類別，其中包含 WM_GETFONT 訊息的訊息處理常式。 若要使用這個方法，您從 `CWnd` 衍生的類別必須定義 WM_GETFONT 訊息的方法處理常式。
...
*pdwFlags*<br/>
接收分層旗標之 DWORD 的指標。 如果不需要引數，這可能是 Null。 如需可能值的完整清單，請參閱[GetLayeredWindowAttributes](/windows/win32/api/winuser/nf-winuser-getlayeredwindowattributes)。
...
此成員函式會模擬函數[GetLayeredWindowAttributes](/windows/win32/api/winuser/nf-winuser-getlayeredwindowattributes)的功能，如 Windows SDK 中所述。
...
*idItem*<br/>
指定要為其取得資訊的專案。 如果此參數為零，則函式會抓取功能表本身的相關資訊。 如果這個參數是1，則函式會抓取功能表上第一個專案的相關資訊，依此類推。
...
此成員函式會模擬函數[GetMenuBarInfo](/windows/win32/api/winuser/nf-winuser-getmenubarinfo)的功能，如 Windows SDK 中所述。
...
*pCurSiteOrWnd*<br/>
識別 `COleControlSiteOrWnd` 控制項。 如需 `COleControlSiteOrWnd`的詳細資訊，請參閱**備註**。
...
一組控制項的開頭是以[WS_GROUP](styles-used-by-mfc.md#window-styles)樣式建立的控制項，並以不是使用 WS_GROUP 樣式所建立的最後一個控制項做為結尾。

根據預設，`GetNextDlgGroupItem` 成員函式會傳回群組中下一個控制項的指標。 如果*pWndCtl*識別群組中的第一個控制項，而*bPrevious*為 TRUE，`GetNextDlgGroupItem` 會將指標傳回至群組中的最後一個控制項。

> [!NOTE]
> 因為 MFC 支援無視窗的 ActiveX 控制項、標準 ActiveX 控制項和 windows，所以只會使用 HWND 來參考控制項，而不會再尾碼。 `COleControlSiteOrWnd` 物件所包含的資訊會將物件識別為視窗型 ActiveX 控制項、無視窗的 ActiveX 控制項或視窗，如下所示：

|控制項或視窗類型|識別資訊|
|----------------------------|-----------------------------|
|視窗型 ActiveX 控制項|包含 HWND，並將[COleControlSite](../../mfc/reference/colecontrolsite-class.md)物件與它產生關聯。 `COleControlSiteOrWnd` 的 `m_hWnd` 成員會設定為控制項的 HWND，而 `m_pSite` 成員則指向控制項的 `COleControlSite`。|
|無視窗 ActiveX 控制項|不包含任何 HWND。 `COleControlSiteOrWnd` 的 `m_pSite` 成員指向控制項的 `COleControlSite`，而 `m_hWnd` 成員是 Null。|
|標準視窗|只包含 HWND。 `COleControlSiteOrWnd` 的 `m_hWnd` 成員會設定為視窗的 HWND，而 `m_pSite` 成員則為 Null。|
...
*pCurSiteOrWnd*<br/>
識別 `COleControlSiteOrWnd` 控制項。 如需 `COleControlSiteOrWnd`的詳細資訊，請參閱[CWnd：： GetNextDlgGroupItem](#getnextdlggroupitem)。
...
如需 `COleControlSiteOrWnd`的詳細資訊，請參閱[CWnd：： GetNextDlgGroupItem](#getnextdlggroupitem)。
...
*nFlag*<br/>
指定函數是否會傳回下一個視窗或上一個視窗的指標。 它可以是 GW_HWNDNEXT，它會傳回視窗管理員清單上 `CWnd` 物件後面的視窗，或 GW_HWNDPREV，這會傳回視窗管理員清單上的上一個視窗。
...
如果 `CWnd` 是最上層視窗，此函式會搜尋下一個（或上一個）最上層視窗;如果 `CWnd` 是子視窗，此函數會搜尋下一個（或上一個）子視窗。
...
此功能的擁有權概念與[GetWindow](/windows/win32/api/winuser/nf-winuser-getwindow)的擁有權概念不同。
...
`GetParent` 函式會傳回直屬父系的指標（如果有的話）。 相反地， [GetParentOwner](#getparentowner)函式會傳回不是子視窗（沒有 WS_CHILD 樣式）最直接的父系或主控視窗的指標。 如果您在子視窗中有子視窗 `GetParent`，`GetParentOwner` 會傳回不同的結果。
...
`GetParentOwner` 會傳回不是子視窗（沒有 WS_CHILD 樣式）最直接的父系或主控視窗的指標。 目前的擁有者視窗可以使用[SetOwner](#setowner)來設定。 根據預設，視窗的父系是其擁有者。

相反地， [GetParent](#getparent)函式會傳回直屬父系的指標，不論它是否為子視窗。 如果您在子視窗中有子視窗 `GetParent`，`GetParentOwner` 會傳回不同的結果。
...
`GetProperty` 會透過*pvProp*傳回值。

> [!NOTE]
> 此函式只能在代表 ActiveX 控制項的 `CWnd` 物件上呼叫。

如需搭配使用此成員函式與 ActiveX 控制項容器的詳細資訊，請參閱[Activex 控制項容器：在 Activex 控制項容器中程式設計 Activex 控制項一](../../mfc/programming-activex-controls-in-a-activex-control-container.md)文。
...
傳回 `m_hWnd`，如果**this**指標是 null，則傳回 null。
...
在建立視窗期間設定 WS_HSCROLL 或 WS_VSCROLL 位時，此成員函式不會在建立的捲軸上運作。 此函式的 `CWnd` 實作用只會傳回 Null。 衍生的類別（例如 `CView`）會執行所述的功能。
...
此成員函式會模擬函數[GetScrollBarInfo](/windows/win32/api/winuser/nf-winuser-getscrollbarinfo)的功能，如 Windows SDK 中所述。

##  <a name="cwndgetscrollinfo"></a><a name="getscrollinfo"></a>CWnd：： GetScrollInfo

呼叫這個成員函式，以抓取 `SCROLLINFO` 結構維護捲軸的相關資訊。
...
- SB_CTL 會抓取捲軸控制項的參數。 `m_hWnd` 的資料成員必須是捲軸控制項的控制碼。
...
*nMask*<br/>
指定要取出的捲軸參數。 預設值會指定 SIF_PAGE、SIF_POS、SIF_TRACKPOS 和 SIF_RANGE 的組合。 如需*nMask*值的詳細資訊，請參閱 `SCROLLINFO`。
...
[SCROLLINFO](/windows/win32/api/winuser/ns-winuser-scrollinfo)結構包含捲軸的相關資訊，包括最小和最大的滾動位置、頁面大小，以及捲動方塊的位置（thumb）。 如需變更結構預設值的詳細資訊，請參閱 Windows SDK 中的 `SCROLLINFO` 結構主題。

指出捲軸位置[CWnd：： OnHScroll](#onhscroll)和[CWnd：： OnVScroll](#onvscroll)的 MFC Windows 訊息處理常式，只提供16個位的位置資料。 `GetScrollInfo` 和 `SetScrollInfo` 提供32位的捲軸位置資料。 因此，應用程式可以在處理 `CWnd::OnHScroll` 或 `CWnd::OnVScroll` 時呼叫 `GetScrollInfo`，以取得32位的捲軸位置資料。
...
- SB_HORZ 會抓取水準捲軸的捲軸限制。

- SB_VERT 會抓取垂直捲動條的捲軸限制。
...
目前的位置是相依于目前滾動範圍的相對值。 例如，如果捲軸範圍是50到100，而且捲動方塊位於橫條的中間，則目前的位置是75。
...
如果 `CWnd` 沒有捲軸，則 `GetScrollRange` 成員函式會將0複製到*lpMinPos*和*lpMaxPos*。

標準捲軸的預設範圍為0到100。 捲軸控制項的預設範圍是空的（這兩個值都是0）。
...
*bRevert*<br/>
指定要採取的動作。 如果*bRevert*為 FALSE，`GetSystemMenu` 會將控制碼傳回至目前使用中的控制項功能表複本。 這個複本一開始與 [控制項] 功能表相同，但是可以修改。 如果*bRevert*為 TRUE，`GetSystemMenu` 會將 [控制] 功能表重設回預設狀態。 先前的可能修改過的 [控制項] 功能表（如果有的話）會終結。 在此情況下，傳回值是未定義的。
...
任何不使用 `GetSystemMenu` 來建立自己的 [控制] 功能表複本的視窗都會收到標準 [控制] 功能表。

`GetSystemMenu` 成員函式所傳回的指標可以與[CMenu：： AppendMenu](../../mfc/reference/cmenu-class.md#appendmenu)、 [CMenu：： InsertMenu](../../mfc/reference/cmenu-class.md#insertmenu)或[CMenu：： ModifyMenu](../../mfc/reference/cmenu-class.md#modifymenu)函式搭配使用，以變更 [控制] 功能表。
...
Windows 可能會自動使 [標準控制] 功能表上的專案無法使用。 `CWnd` 可以藉由回應[WM_INITMENU](#oninitmenu)訊息來執行自己的選擇或無法使用，在顯示任何功能表之前傳送。
...
此成員函式會模擬函數[GetTitleBarInfo](/windows/win32/api/winuser/nf-winuser-gettitlebarinfo)的功能，如 Windows SDK 中所述。
...
`GetTopLevelParent` 類似[GetTopLevelFrame](#gettoplevelframe)和[GetTopLevelOwner](#gettoplevelowner);不過，它會忽略設定為目前擁有者視窗的值。
...
*lpRect*<br/>
指向 `CRect` 物件或[矩形結構](/windows/win32/api/windef/ns-windef-rect)，其會接收包含更新區域之更新的用戶端座標。

將此參數設定為 Null，以判斷更新區域是否存在於 `CWnd`內。 如果*lpRect*為 Null，則如果更新區域存在，`GetUpdateRect` 成員函式會傳回非零，如果沒有的話，則傳回0。 這會提供一種方式來判斷是否從不正確區域產生 WM_PAINT 訊息。 請勿在 Windows 3.0 版和更早版本中將此參數設定為 Null。
...
如果 `CWnd` 是使用 CS_OWNDC 樣式所建立，且未 MM_TEXT 對應模式，則 `GetUpdateRect` 成員函式會以邏輯座標提供矩形。 否則，`GetUpdateRect` 會在用戶端座標中提供矩形。 如果沒有更新區域，`GetUpdateRect` 會將矩形設為空白（將所有座標設定為0）。

*BErase*參數會指定 `GetUpdateRect` 是否應清除更新區域的背景。 如果*bErase*為 TRUE，且更新區域不是空的，則會清除背景。 若要清除背景，`GetUpdateRect` 傳送[WM_ERASEBKGND](#onerasebkgnd)訊息。

[BeginPaint](#beginpaint)成員函式所抓取的更新矩形，與 `GetUpdateRect` 成員函式所抓取的不同。

`BeginPaint` 成員函式會自動驗證更新區域，因此在對 `BeginPaint` 進行呼叫之後立即進行的任何 `GetUpdateRect` 呼叫都會抓取空的更新區域。
...
[BeginPaint](#beginpaint)成員函式會自動驗證更新區域，因此在呼叫 `BeginPaint` 之後立即進行的任何 `GetUpdateRgn` 呼叫都會抓取空的更新區域。
...
*nCmd*<br/>
指定 `CWnd` 與傳回的視窗之間的關聯性。 它可以採用下列其中一個值：
...
傳回的指標可能是暫時性的，不應該儲存以供稍後使用。 每次成功呼叫 `GetWindowDC`時，都應呼叫一次[ReleaseDC](#releasedc) 。

### <a name="remarks"></a>備註

視窗顯示內容允許在 `CWnd`中的任何位置繪製，因為內容的原點是 `CWnd` 的左上角，而不是工作區。

預設屬性會在每次抓取內容時指派給顯示內容。 先前的屬性會遺失。

`GetWindowDC` 的目的是要用於 `CWnd` 非工作區中的特殊繪製效果。 不建議在任何視窗的非工作區中繪製。

[GetSystemMetrics](/windows/win32/api/winuser/nf-winuser-getsystemmetrics) Windows 函式可以用來抓取非工作區各部分的維度，例如標題列、功能表和捲軸。
...
此成員函式會模擬函數[GetWindowInfo](/windows/win32/api/winuser/nf-winuser-getwindowinfo)的功能，如 Windows SDK 中所述。
...
*lpwndpl*<br/>
指向接收顯示狀態和位置資訊的 `WINDOWPLACEMENT` 結構。
...
此函式所抓取之[WINDOWPLACEMENT](/windows/win32/api/winuser/ns-winuser-windowplacement)結構的 `flags` 成員一律為0。 如果 `CWnd` 已最大化，`WINDOWPLACEMENT` 的 `showCmd` 成員就會 SW_SHOWMAXIMIZED。 如果視窗最小化，則會 SW_SHOWMINIMIZED。 否則會 SW_SHOWNORMAL。

##  <a name="cwndgetwindowrect"></a><a name="getwindowrect"></a>CWnd：： GetWindowRect

將 `CWnd` 物件的周框的維度複製到*lpRect*所指向的結構。
...
*lpRect*<br/>
指向 `CRect` 物件或[矩形結構](/windows/win32/api/windef/ns-windef-rect)，其會接收左上角和右下角的螢幕座標。
...
將 `CWnd` 標題標題（如果有的話）複製到*lpszStringBuf*所指向的緩衝區中，或複製到目的地字串*rString*中。
...
如果 `CWnd` 物件是控制項，`GetWindowText` 成員函式會複製控制項內的文字，而不是複製標題。
...
如果 `CWnd` 是控制項，`GetWindowTextLength` 成員函式會傳回控制項內的文字長度，而不是標題。
...
隱藏是累計的。 如果 `HideCaret` 在資料列中被呼叫五次，`ShowCaret` 成員函式必須呼叫五次，才會顯示插入號。
...
*nHilite*<br/>
指定功能表項目是否已反白顯示或已移除醒目提示。 它可以是 MF_HILITE 或 MF_UNHILITE 與 MF_BYCOMMAND 或 MF_BYPOSITION 的組合。 這些值可以使用位 OR 運算子結合。 這些值具有以下意義：

- MF_BYCOMMAND 會將*NIDHiliteItem*解讀為功能表項目識別碼（預設轉譯）。

- MF_BYPOSITION 會將*NIDHiliteItem*解讀為功能表項目的以零為基底的位移。

- MF_HILITE 會反白顯示專案。 如果未提供此值，則會從專案中移除反白顯示。

- MF_UNHILITE 從專案中移除反白顯示。
...
##  <a name="cwndinvalidate"></a><a name="invalidate"></a>CWnd：：失效
...
當下一個[WM_PAINT](#onpaint)訊息發生時，工作區會標示為要繪製。 在[ValidateRect](#validaterect)或[ValidateRgn](#validatergn)成員函式發生 WM_PAINT 訊息之前，也可以驗證該區域。
...
*lpRect*<br/>
指向 `CRect` 物件或[矩形結構](/windows/win32/api/windef/ns-windef-rect)，其中包含要新增至更新區域的矩形（以工作區座標表示）。 如果*lpRect*為 Null，則會將整個工作區新增至區域。
...
當下一個[WM_PAINT](#onpaint)訊息傳送時，不正確矩形（以及更新區域中的所有其他區域）會標示為用於繪製。 失效的區域會累積在更新區域中，直到下一次 WM_PAINT 呼叫時處理區域，或在[ValidateRect](#validaterect)或[ValidateRgn](#validatergn)成員函式驗證該區域為止。
...
將給定區域內的工作區新增至 `CWnd`的目前更新區域，使其失效。
...
當下一次傳送[WM_PAINT](#onpaint)訊息時，不正確區域以及更新區域中的其他所有區域，都會標示為進行繪製。 失效的區域會累積在更新區域中，直到下一次傳送 WM_PAINT 訊息時，或在[ValidateRect](#validaterect)或[ValidateRgn](#validatergn)成員函式驗證該區域為止。
...
*pbParamInfo*<br/>
以 null 結束的位元組字串指標，指定*pbParamInfo*之後的參數類型。 如需可能的值，請參閱 `COleDispatchDriver::InvokeHelper`的「備註」一節。
...
此函式會將參數轉換為 VARIANTARG 值，然後在 ActiveX 控制項上叫用 `IDispatch::Invoke` 方法。 若呼叫 `IDispatch::Invoke` 失敗，此函式會擲回例外狀況。 如果 `IDispatch::Invoke` 傳回的 SCODE （狀態碼）為 DISP_E_EXCEPTION，則此函式會擲回[COleException](../../mfc/reference/coleexception-class.md)物件，否則會擲回[COleDispatchException](../../mfc/reference/coledispatchexception-class.md)。
...
指出*pWnd*所指定的視窗是否為子視窗或 `CWnd`的其他直接下階。
...
指定函數的結果。 如果*pWnd*所識別的視窗是 `CWnd`的子視窗，則此值為非零。否則為0。

### <a name="remarks"></a>備註

子視窗是 `CWnd` 的直接下階，如果 `CWnd` 物件是在從原始快顯視窗到子視窗的父視窗鏈中。
...
指定成員函式是否已處理指定的訊息。 如果訊息已經過處理，則為非零值;否則為0。 如果傳回的是0，請呼叫基類的[CWnd：:P retranslatemessage](#pretranslatemessage)成員函式來處理訊息。 在 `CWnd::PreTranslateMessage` 成員函式的覆寫中，程式碼看起來像這樣：

[!code-cpp[NVC_MFCWindowing#100](../../mfc/reference/codesnippet/cpp/cwnd-class_40.cpp)]

### <a name="remarks"></a>備註

當 `IsDialogMessage` 函式處理訊息時，它會檢查鍵盤訊息，並將它們轉換成對應對話方塊的選擇命令。 例如，TAB 鍵會選取下一個控制項或控制項群組，而向下鍵則會選取群組中的下一個控制項。

您不能將 `IsDialogMessage` 處理的訊息傳遞至[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)或[DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows 函式，因為它已經過處理。
...
如果 `CWnd` 是可見的，則為非零（已設定[WS_VISIBLE](styles-used-by-mfc.md#window-styles)樣式位，且父視窗為可見）。 由於傳回值會反映 WS_VISIBLE 樣式位的狀態，因此即使 `CWnd` 完全被其他視窗遮蔽，傳回值也可能是非零。

### <a name="remarks"></a>備註

視窗擁有以 WS_VISIBLE 樣式位表示的可見度狀態。 當此樣式位設定為呼叫[ShowWindow](#showwindow)成員函式時，只要視窗已設定樣式位，就會顯示視窗，並顯示後續繪製至視窗。
...
如果 `CWnd` 已最大化，則為非零值;否則為0。

##  <a name="cwndkilltimer"></a><a name="killtimer"></a>CWnd：： KillTimer

從先前的 `SetTimer`呼叫中，終止*nIDEvent*所識別的計時器事件。
...
與計時器相關聯的擱置[WM_TIMER](#ontimer)訊息不會從訊息佇列中移除。
...
如果函式成功，則為非零。 如果發生失敗或已使用 `LockWindowUpdate` 函式來鎖定另一個視窗，則為0。

### <a name="remarks"></a>備註

無法移動鎖定的視窗。 一次只能鎖定一個視窗。 若要解除鎖定具有 `LockWindowUpdate`的視窗，請呼叫[UnlockWindowUpdate](#unlockwindowupdate)。

如果具有鎖定視窗的應用程式（或任何已鎖定的子視窗）呼叫[GetDC、](/windows/win32/api/winuser/nf-winuser-getdc) [GetDCEx](/windows/win32/api/winuser/nf-winuser-getdcex)或[BeginPaint](/windows/win32/api/winuser/nf-winuser-beginpaint) windows 函數，則被呼叫的函式會傳回其可見區域為空白的裝置內容。 在應用程式藉由呼叫 `UnlockWindowUpdate` 成員函式來解除鎖定視窗之前，會發生這種情況。

當視窗更新被鎖定時，系統會將任何繪圖作業的周框矩形，追蹤到與鎖定視窗相關聯的裝置內容。 重新啟用繪製時，這個周框矩形在鎖定的視窗及其子視窗中會失效，以強制執行最終的[WM_PAINT](/windows/win32/gdi/wm-paint)訊息來更新螢幕。 如果視窗更新鎖定時沒有發生任何繪製，則沒有任何區域失效。

`LockWindowUpdate` 成員函式不會使指定的視窗變成可見，且不會清除[WS_VISIBLE](styles-used-by-mfc.md#window-styles)的樣式位。

##  <a name="cwndm_hwnd"></a><a name="m_hwnd"></a>CWnd：： m_hWnd

附加至此 `CWnd`之 Windows 視窗的控制碼。
...
*lpPoint*<br/>
[POINT 結構](/windows/win32/api/windef/ns-windef-point)陣列的指標，其中包含要轉換的點集合。

*nCount*<br/>
指定*lpPoint*所指向之陣列中的 `POINT` 結構數目。
...
*lpszCaption*<br/>
指向要用於訊息方塊標題的 `CString` 物件或以 null 結束的字串。 如果*lpszCaption*為 Null，則會使用預設標題 "Error"。
...
這個方法會利用 Windows SDK 中所定義的[MessageBox](/windows/win32/api/winuser/nf-winuser-messagebox)函數。 這個方法會傳回呼叫此函式的結果。
...
|![有&#40;説明嗎？&#41;圖示](../../mfc/reference/media/vc364f2.gif "有&#40;説明嗎？&#41;圖示")|MB_ICONQUESTION|
...
*nFlags*<br/>
要傳遞至[SetWindowPos](#setwindowpos)的旗標，如果不應該呼叫 `SetWindowPos`，則為零。 預設值為 0。 如需預設旗標的清單，請參閱備註一節。
...
您可以使用位 OR （&#124;）運算子來結合要加入或移除的樣式。 如需可用視窗樣式的詳細資訊，請參閱 Windows SDK 中的主題[視窗樣式](/windows/win32/winmsg/window-styles)和[CreateWindow](/windows/win32/api/winuser/nf-winuser-createwindoww) 。

如果*nFlags*為非零值，`ModifyStyle` 會呼叫 Windows API 函式[SetWindowPos](/windows/win32/api/winuser/nf-winuser-setwindowpos) ，並藉由結合*nFlags*與下列四個預設旗標來重新繪製視窗：
...
> [!NOTE]
> 對於某些控制項中的某些樣式（例如，編輯控制項中的 ES_READONLY 樣式），`ModifyStyle` 可能不會適當地變更樣式，因為控制項可能需要執行特殊的內部處理。 在這些情況下，將會提供要變更樣式的對應訊息（EM_SETREADONLY 在所述的範例中）。
...
您可以使用位 OR （&#124;）運算子來結合要加入或移除的樣式。 如需可用擴充樣式的詳細資訊，請參閱本書中的主題[延伸視窗樣式](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles)和 Windows SDK [CreateWindowEx](/windows/win32/api/winuser/nf-winuser-createwindowexw)

如果*nFlags*為非零值，`ModifyStyleEx` 會呼叫 Windows API 函式[SetWindowPos](/windows/win32/api/winuser/nf-winuser-setwindowpos) ，並藉由結合*nFlags*與下列四個預設旗標來重新繪製視窗：
...
*bRepaint*<br/>
指定是否要重新繪製 `CWnd`。 若為 TRUE，`CWnd` 會如往常般在其[OnPaint](#onpaint)訊息處理常式中接收[WM_PAINT](/windows/win32/gdi/wm-paint)訊息。 如果此參數為 FALSE，則不會重新繪製任何類型。 這適用于工作區、非工作區（包括標題和捲軸），以及父視窗的任何部分（做為 `CWnd`移動的結果）。 當此參數為 FALSE 時，應用程式必須明確地使必須重新繪製之 `CWnd` 和父視窗的任何部分失效或重繪。

*lpRect*<br/>
[CRect](../../atl-mfc-shared/reference/crect-class.md)物件或[RECT 結構](/windows/win32/api/windef/ns-windef-rect)，指定新的大小和位置。

### <a name="remarks"></a>備註

如果是最上層 `CWnd` 物件， *x*和*y*參數會相對於螢幕的左上角。 若為子 `CWnd` 物件，則會相對於父視窗工作區的左上角。

`MoveWindow` 函式會傳送[WM_GETMINMAXINFO](#ongetminmaxinfo)訊息。 處理此訊息可讓 `CWnd` 有機會修改最大和最小可能視窗的預設值。 如果 `MoveWindow` 成員函式的參數超出這些值，則可以使用 WM_GETMINMAXINFO 處理常式中的最小值或最大值來取代這些值。
...
*idObjectType*<br/>
識別產生事件的物件種類。 這個值是其中一個預先定義的[物件識別碼](/windows/win32/WinAuto/object-identifiers)或自訂物件識別碼值。
...
此成員函式會模擬函數[NotifyWinEvent](/windows/win32/api/winuser/nf-winuser-notifywinevent)的功能，如 Windows SDK 中所述。
...
*pWndOther*<br/>
要啟用或停用之 `CWnd` 的指標。 指標可以是 Null，而且可能是暫時性的。

*bMinimized*<br/>
指定要啟用或停用之 `CWnd` 的最小化狀態。 值為 TRUE 表示視窗最小化。

若為 TRUE，則表示正在啟用 `CWnd`;否則會停用。

### <a name="remarks"></a>備註

如果 `CWnd` 物件是以滑鼠按一下的方式啟動，它也會收到[OnMouseActivate](#onmouseactivate)成員函式呼叫。
...
*bActive*<br/>
指定是否要啟用或停用 `CWnd`。 TRUE 表示正在啟用 `CWnd`。 FALSE 表示要停用 `CWnd`。

*dwThreadID*<br/>
指定執行緒識別碼的值。 如果*bActive*為 TRUE， *dwThreadID*會識別擁有要停用 `CWnd` 的執行緒。 如果*bActive*為 FALSE，則*dwThreadID*會識別擁有要啟用之 `CWnd` 的執行緒。
...
*pvar*<br/>
呼叫端所配置 `VARIANT` 結構的指標，將傳回環境屬性的值。
...
|*nCmd*|在表示應用程式命令。 如需可能值的清單，請參閱[WM_APPCOMMAND](/windows/win32/inputdev/wm-appcommand) *lParam*參數的*cmd*區段底下的命令。|
|*nDevice*|在產生輸入事件的輸入裝置。 如需可能值的清單，請參閱[WM_APPCOMMAND](/windows/win32/inputdev/wm-appcommand)之*LParam*參數的*uDevice*區段底下的裝置。|
|*nKey*|在表示任何關閉的虛擬機器碼，例如 CTRL 鍵或滑鼠左鍵。 如需可能值的清單，請參閱[WM_APPCOMMAND](/windows/win32/inputdev/wm-appcommand)之*LParam*參數的*dwKeys*區段下的索引鍵。 如需詳細資訊，請參閱[關於滑鼠輸入](/windows/win32/inputdev/about-mouse-input)中的「訊息參數」子標題。|
...
如果 `CWnd` 物件具有焦點，則會在顯示對話方塊或訊息方塊時呼叫它的 `OnCancelMode` 成員函式。 這讓 `CWnd` 有機會取消模式，例如滑鼠捕捉。

預設的實作用是藉由呼叫[ReleaseCapture](/windows/win32/api/winuser/nf-winuser-releasecapture) Windows 函式來回應。 覆寫衍生類別中的這個成員函式，以處理其他模式。
...
視窗會接收此訊息，即使它呼叫[ReleaseCapture](/windows/win32/api/winuser/nf-winuser-releasecapture)本身也一樣。 應用程式不應該嘗試設定滑鼠捕捉以回應此訊息。 當它收到此訊息時，視窗應該視需要重繪本身，以反映新的滑鼠捕捉狀態。
...
接收 `OnChangeCbChain` 呼叫的每個 `CWnd` 物件都應該使用[SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) Windows 函式，將[WM_CHANGECBCHAIN](/windows/win32/dataxchg/wm-changecbchain)訊息傳送至剪貼簿檢視器鏈中的下一個視窗（由 `SetClipboardViewer`所傳回的控制碼）。 如果*hWndRemove*是鏈中的下一個視窗， *hWndAfter*所指定的視窗就會變成下一個視窗，並將剪貼簿訊息傳遞給它。
...
- UIS_CLEAR UI 狀態元素（由*nUIElement*指定）應該隱藏。

- UIS_INITIALIZE UI 狀態元素（由*nUIElement*指定）應該根據最後一個輸入事件來變更。 如需詳細資訊，請參閱[WM_CHANGEUISTATE](/windows/win32/menurc/wm-changeuistate)的「**備註**」一節。

- UIS_SET UI 狀態元素（由*nUIElement*指定）應該是可見的。

*nUIElement*<br/>
指定受影響的 UI 狀態元素或控制項的樣式。 可以是下列其中一個值：

- UISF_HIDEACCEL 鍵盤加速器。
...
|30|指定先前的索引鍵狀態。 如果在傳送訊息之前，金鑰已關閉，此值為1，如果索引鍵已啟動，則為0。|
|31|指定轉換狀態。 如果要釋放金鑰，此值為1，如果按下按鍵則為0。|

### <a name="remarks"></a>備註

這個函式會在[OnKeyUp](#onkeyup)成員函式之前，以及在呼叫[OnKeyDown](#onkeydown)成員函式之後呼叫。 `OnChar` 包含所按下或放開之鍵盤按鍵的值。

因為所按下的索引鍵與產生的 `OnChar` 呼叫之間不一定會有一對一的對應關係，所以*nFlags*中的資訊通常對應用程式而言並不實用。 *NFlags*中的資訊只會套用至 `OnKeyUp` 成員函式的最新呼叫，或在 `OnChar`的呼叫前面的 `OnKeyDown` 成員函式。
...
當具有[LBS_WANTKEYBOARDINPUT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles)樣式的清單方塊將[WM_CHARTOITEM](/windows/win32/Controls/wm-chartoitem)訊息傳送給其擁有者，以回應[WM_CHAR](#onchar)訊息時呼叫。
...
如果 `CWnd` 物件是多重文件介面（MDI）子視窗，則當使用者按一下視窗的標題列或視窗已啟動、移動或調整大小時，架構會呼叫 `OnChildActivate`。
...
這個方法會接收[WM_DWMNCRENDERINGCHANGED](/windows/win32/dwm/wm-dwmcolorizationcolorchanged)通知訊息，如 Windows SDK 所述。
...
覆寫衍生類別中的這個成員函式，以處理[WM_COMMAND](/windows/win32/menurc/wm-command)訊息。 除非呼叫基類 `OnCommand`，否則覆寫將不會處理訊息對應。
...
當 Windows 偵測到超過12.5% 的系統時間超過30到60秒的間隔時間，此架構會為所有最上層視窗呼叫這個成員函式。
...
*nCpuTime*<br/>
指定 Windows 壓縮記憶體到執行其他作業所花費的 CPU 時間時，目前花費的 CPU 時間比例。 例如，8000h 代表花在壓縮記憶體的 CPU 時間50%。

### <a name="remarks"></a>備註

這表示系統記憶體偏低。

當 `CWnd` 物件收到此呼叫時，應該盡可能釋放最多的記憶體，將應用程式的目前活動層級，以及在 Windows 中執行的應用程式總數列入考慮。 應用程式可以呼叫 Windows 函式來判斷有多少應用程式正在執行。
...
包含[COMPAREITEMSTRUCT](/windows/win32/api/winuser/ns-winuser-compareitemstruct)資料結構的長指標，其中包含下拉式方塊或清單方塊中兩個專案的識別碼和應用程式提供的資料。
...
如果組合或清單方塊是以 [ [CBS_SORT](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) ] 或 [ [LBS_SORT](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) ] 樣式所建立，則每當應用程式加入新專案時，Windows 就會傳送 WM_COMPAREITEM 訊息給下拉式方塊或清單方塊擁有者。

下拉式方塊或清單方塊中的兩個專案，會在*lpCompareItemStruct*所指向的 `COMPAREITEMSTRUCT` 結構中 reformed。 `OnCompareItem` 應該會傳回值，指出哪些專案應該出現在另一個之前。 一般來說，Windows 會進行此呼叫數次，直到判斷新專案的確切位置為止。

如果 `COMPAREITEMSTRUCT` 結構的 `hwndItem` 成員屬於[CListBox](../../mfc/reference/clistbox-class.md)或[CComboBox](../../mfc/reference/ccombobox-class.md)物件，則會呼叫適當類別的 `CompareItem` 虛擬函式。 覆寫衍生 `CListBox` 或 `CComboBox` 類別中 `CComboBox::CompareItem` 或 `CListBox::CompareItem`，以執行專案比較。
...
如果您沒有顯示內容功能表，您應該將此訊息傳遞至[DefWindowProc](#defwindowproc)函式。 如果您的視窗是子視窗，`DefWindowProc` 會將訊息傳送至父系。 否則，如果指定的位置是在視窗的標題中，`DefWindowProc` 會顯示預設的內容功能表。
...
*lpCreateStruct*<br/>
指向[CREATESTRUCT](/windows/win32/api/winuser/ns-winuser-createstructw)結構，其中包含所建立之 `CWnd` 物件的相關資訊。

### <a name="return-value"></a>傳回值

`OnCreate` 必須傳回0，才能繼續建立 `CWnd` 物件。 如果應用程式傳回-1，則會終結視窗。
...
若要變更文字色彩，請使用所需的紅色、綠色和藍色（RGB）值來呼叫 `SetTextColor` 成員函式。
...
不會針對下拉下拉式方塊的清單方塊呼叫 `OnCtlColor`，因為下拉式清單方塊實際上是下拉式方塊的子系，而不是視窗的子系。 若要變更下拉式清單方塊的色彩，請使用 `OnCtlColor` 的覆寫來建立 `CComboBox`，以檢查 `nCtlColor` 參數中的 CTLCOLOR_LISTBOX。 在此處理程式中，必須使用 `SetBkColor` 成員函式來設定文字的背景色彩。
...
|8|擴充索引鍵，例如數位鍵臺上的函式金鑰或索引鍵（如果它是擴充索引鍵，則為1），否則為0。|
|9-10|未使用。|
|11-12|由 Windows 在內部使用。|
|13|內容代碼（如果在按下按鍵時，會按住 ALT 鍵，則為1，否則為0）。|
|14|先前的索引鍵狀態（1，如果金鑰在呼叫前關閉，則為0，如果索引鍵已啟動）。|
|15|轉換狀態（如果正在釋放索引鍵，則為1，如果按下按鍵則為0）。|

### <a name="remarks"></a>備註

這個成員函式可以用來指定失效索引鍵的字元值。 無作用的索引鍵是一個索引鍵（例如，加上變音符號（雙點）字元），會與其他字元結合以形成複合字元。 例如，變音符號的 O 字元是由不正確索引鍵、變音符號和 O 鍵所組成。

應用程式通常會使用 `OnDeadChar`，為使用者提供每個已按下按鍵的意見反應。 例如，應用程式可以在目前的字元位置顯示輔色，而不移動插入號。

由於所按下的索引鍵和 `OnDeadChar` 呼叫之間不一定會有一對一的對應關係，因此*nFlags*中的資訊通常對應用程式而言並不實用。 *NFlags*中的資訊僅適用于對[OnKeyUp](#onkeyup)成員函式的最新呼叫，或在 `OnDeadChar` 呼叫之前的[OnKeyDown](#onkeydown)成員函式。
...
此架構會呼叫這個成員函式，通知主控描繪清單方塊或下拉式方塊的擁有者，清單方塊或下拉式方塊已終結，或已由 CComboBox 移除專案[：:D eletestring](../../mfc/reference/ccombobox-class.md#deletestring)、 [CListBox：:D eletestring](../../mfc/reference/clistbox-class.md#deletestring)、 [CComboBox：： ResetContent](../../mfc/reference/ccombobox-class.md#resetcontent)或[CListBox：： ResetContent](../../mfc/reference/clistbox-class.md#resetcontent)。
...
如果 `DELETEITEMSTRUCT` 結構的 `hwndItem` 成員屬於下拉式方塊或清單方塊，則會呼叫適當類別的 `DeleteItem` 虛擬函式。 覆寫適當控制項類別的 `DeleteItem` 成員函式，以刪除專案特定資料。
...
從螢幕中移除 `CWnd` 物件之後，就會呼叫 `OnDestroy`。

系統會先針對終結的 `CWnd` 呼叫 `OnDestroy`，然後在終結時，針對 `CWnd` 的子視窗。 在 `OnDestroy` 執行時，可以假設所有子視窗仍然存在。

如果要終結的 `CWnd` 物件是剪貼簿-檢視器鏈的一部分（藉由呼叫[SetClipboardViewer](#setclipboardviewer)成員函式設定），則 `CWnd` 必須先從剪貼簿-檢視器鏈中移除其本身，方法是呼叫[ChangeClipboardChain](#changeclipboardchain)成員函式，然後再從 `OnDestroy` 函數傳回。

##  <a name="cwndondestroyclipboard"></a><a name="ondestroyclipboard"></a>CWnd：： OnDestroyClipboard

當您透過呼叫[EmptyClipboard](/windows/win32/api/winuser/nf-winuser-emptyclipboard) Windows 函式清空剪貼簿時，架構會為剪貼簿擁有者呼叫這個成員函式。
...
- [DBT_DEVICEARRIVAL](/windows/win32/DevIO/dbt-devicearrival)已插入裝置，且現在已可使用。

- [DBT_DEVICEQUERYREMOVE](/windows/win32/DevIO/dbt-devicequeryremove)已要求移除裝置的許可權。 任何應用程式都可以拒絕此要求並取消移除。

- [DBT_DEVICEQUERYREMOVEFAILED](/windows/win32/DevIO/dbt-devicequeryremovefailed)已取消移除裝置的要求。
...
- [DBT_DEVICETYPESPECIFIC](/windows/win32/DevIO/dbt-devicetypespecific)裝置特定事件。
...
處理 WM_DEVMODECHANGE 訊息的應用程式可能會重新初始化其裝置模式設定。 使用 Windows `ExtDeviceMode` 功能來儲存和還原裝置設定的應用程式，通常不會處理此功能。

當使用者從 [控制台] 變更預設印表機時，不會呼叫此函式。 在此情況下，會呼叫 `OnWinIniChange` 函式。
...
接收 `OnDrawClipboard` 呼叫的每個視窗都應該呼叫[SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) Windows 函式，將[WM_DRAWCLIPBOARD](/windows/win32/dataxchg/wm-drawclipboard)訊息傳遞至剪貼簿檢視器鏈中的下一個視窗。 下一個視窗的控制碼是由[SetClipboardViewer](#setclipboardviewer)成員函式傳回;它可能會修改以回應[OnChangeCbChain](#onchangecbchain)成員函式呼叫。
...
覆寫衍生類別中的這個方法，並在指定的裝置內容上繪製，以便自訂縮圖和查看。 如果*bThumbnail*為 TRUE，則可以忽略*szRequiredThumbnailSize* 。 在此情況下，您應該知道您繪製的是完整大小的點陣圖（也就是涵蓋整個工作區的點陣圖）。 裝置內容（ *dc*）隨附選取的32位點陣圖。 預設的執行會使用 PRF_CLIENT、PRF_CHILDREN 和 PRF_NONCLIENT 旗標，將 WM_PRINT 傳送到此視窗。
...
[DRAWITEMSTRUCT](/windows/win32/api/winuser/ns-winuser-drawitemstruct)結構的 `itemAction` 成員會定義要執行的繪製作業。 這個成員中的資料可讓控制項的擁有者判斷需要何種繪圖動作。

在從處理此訊息傳回之前，應用程式應該確定 `DRAWITEMSTRUCT` 結構的*hDC*成員所識別的裝置內容已還原為預設狀態。

如果 `hwndItem` 成員屬於[CButton](../../mfc/reference/cbutton-class.md)、 [CMenu](../../mfc/reference/cmenu-class.md)、 [CListBox](../../mfc/reference/clistbox-class.md)或[CComboBox](../../mfc/reference/ccombobox-class.md)物件，則會呼叫適當類別的 `DrawItem` 虛擬函式。 覆寫適當控制項類別的 `DrawItem` 成員函式，以繪製專案。
...
*hDropInfo*<br/>
內部資料結構的指標，描述已卸載的檔案。 `DragFinish`、`DragQueryFile`和 `DragQueryPoint` Windows 函式會使用此控制碼來抓取已卸載檔案的相關資訊。
...
*bEnable*<br/>
指定 `CWnd` 物件是否已啟用或停用。 如果已啟用 `CWnd`，則此參數為 TRUE;如果 `CWnd` 已停用，則為 FALSE。
...
架構會在 `CWnd` 物件從[OnQueryEndSession](#onqueryendsession)成員函式呼叫傳回非零值之後，呼叫這個成員函式。
...
`OnEndSession` 呼叫會通知 `CWnd` 物件是否確實正在結束會話。

如果*彎曲*為 TRUE，則在所有應用程式從處理此呼叫傳回之後，Windows 可以隨時終止。 因此，讓應用程式執行在 `OnEndSession`內終止所需的所有工作。

當會話結束時，您不需要呼叫[DestroyWindow](#destroywindow)成員函式或[PostQuitMessage](/windows/win32/api/winuser/nf-winuser-postquitmessage) Windows 函式。
...
*pWho*<br/>
指定對話方塊的指標（如果*nWhy*是 MSGF_DIALOGBOX），或是包含所顯示功能表的視窗（如果 MSGF_MENU 為*nWhy* ）。 這個指標可能是暫時性的，不應該儲存以供稍後使用。
...
當使用者按一下視窗的標題列或調整大小框線，或當視窗將[WM_SYSCOMMAND](/windows/win32/menurc/wm-syscommand)訊息傳遞給[CWnd：:D efwindowproc](#defwindowproc)函數，而該訊息的*wParam*參數指定 SC_MOVE 或 SC_SIZE 時，視窗會進入移動或調整大小的強制回應迴圈。
...
預設的執行會使用 window 類別結構之 `hbrBackground` 成員所指定的視窗類別背景筆刷來清除背景。

如果 `hbrBackground` 成員是 Null，則覆寫版本的 `OnEraseBkgnd` 應該會清除背景色彩。 您的版本也應該先呼叫筆刷的[UnrealizeObject](/windows/win32/api/wingdi/nf-wingdi-unrealizeobject) ，然後選取筆刷，以便將預定筆刷的原點與 `CWnd` 座標組齊。

覆寫的 `OnEraseBkgnd` 應該在處理訊息並清除背景時，傳回非零的以回應 WM_ERASEBKGND這表示不需要進一步清除。 如果傳回0，視窗仍會標示為需要清除。 （一般而言，這表示 `PAINTSTRUCT` 結構的 `fErase` 成員會是 TRUE）。
...
在應用程式變更字型資源集區之後，系統中的所有最上層視窗都會收到來自架構的 `OnFontChange` 呼叫。
...
在系統中新增或移除字型的應用程式（例如，透過[AddFontResource](/windows/win32/api/wingdi/nf-wingdi-addfontresourcew)或[RemoveFontResource](/windows/win32/api/wingdi/nf-wingdi-removefontresourcew) Windows 函式），應將[WM_FONTCHANGE](/windows/win32/gdi/wm-fontchange)訊息傳送至所有最上層視窗。
...
- DLGC_UNDEFPUSHBUTTON 沒有預設的按鈕處理。 （應用程式可以搭配 DLGC_BUTTON 使用此旗標，以表示它會處理按鈕輸入，但會依賴系統進行預設的按鍵處理）。
...
一般來說，Windows 會處理 `CWnd` 控制項的所有箭號鍵和 TAB 鍵輸入。 藉由覆寫 `OnGetDlgCode`，`CWnd` 控制項可以選擇特定類型的輸入來處理本身。

預先定義之控制項類別的預設 `OnGetDlgCode` 函式會傳回適用于每個類別的程式碼。
...
*lpMMI*<br/>
指向 `MINMAXINFO` 結構，其中包含視窗最大化大小和位置及其最小和最大追蹤大小的相關資訊。 如需此結構的詳細資訊，請參閱[MINMAXINFO](/windows/win32/api/winuser/ns-winuser-minmaxinfo)結構。

### <a name="remarks"></a>備註

最大化的大小是當其框線完全延伸時的視窗大小。 視窗的追蹤大小上限是最大的視窗大小，可使用框線來調整視窗大小。 視窗的最小追蹤大小是最小的視窗大小，可使用框線調整視窗大小來達到。

Windows 會填入點陣列，以指定不同位置和維度的預設值。 應用程式可能會在 `OnGetMinMaxInfo`中變更這些值。
...
*lpHelpInfo*<br/>
[HELPINFO](/windows/win32/api/winuser/ns-winuser-helpinfo)結構的指標，其中包含所要求的功能表項目、控制項、對話方塊或視窗的相關資訊。
...
當按下 F1 鍵時，如果功能表處於作用中狀態，WM_HELP 會傳送至與功能表相關聯的視窗;否則，WM_HELP 會傳送至具有鍵盤焦點的視窗。 如果沒有視窗具有鍵盤焦點，WM_HELP 就會傳送至目前的使用中視窗。
...
|參數|描述|
|---------------|-----------------|
|*nHotKeyId*|在產生訊息之熱鍵的識別碼。 如果訊息是由系統定義的熱鍵所產生，此參數將會是下列其中一個值：<br /><br /> -IDHOT_SNAPDESKTOP-已按下 [貼齊桌面] 快速鍵。<br />-IDHOT_SNAPWINDOW-按下 [嵌入式管理單元] 快速鍵。|
|*nKey1*|在旗標的位元組合（OR），表示與*nKey2*參數所指定之索引鍵一起按下的按鍵。 可能的值包括：<br /><br /> -MOD_ALT-已按住 ALT 鍵。<br />-MOD_CONTROL-已按住 CTRL 鍵。<br />-MOD_SHIFT-按住 SHIFT 鍵。<br />-MOD_WIN-已保留 WINDOWS 金鑰。 這些金鑰會加上 Microsoft Windows 標誌的標籤。|
|*nKey2*|在熱鍵的虛擬按鍵碼。|

### <a name="remarks"></a>備註

這個方法會接收[WM_HOTKEY](/windows/win32/inputdev/wm-hotkey)通知，如 Windows SDK 所述。 此訊息會放在與登錄熱鍵的執行緒相關聯之訊息佇列的頂端。 使用[RegisterHotKey](/windows/win32/api/winuser/nf-winuser-registerhotkey)函數來註冊全系統的熱鍵。
...
*nSBCode*<br/>
指定指出使用者的滾動要求的捲軸程式碼。 此參數可以是下列其中一項：
...
- SB_THUMBPOSITION 滾動到絕對位置。 目前的位置是由*nPos*參數所指定。

- SB_THUMBTRACK 將捲動方塊拖曳至指定的位置。 目前的位置是由*nPos*參數所指定。

*nPos*<br/>
指定捲軸程式碼 SB_THUMBPOSITION 或 SB_THUMBTRACK 時的捲動方塊位置;否則，就不會使用。 視初始捲軸範圍而定， *nPos*可能是負值，而且應該在必要時轉換成**int** 。
...
剪貼簿擁有者的 `OnHScrollClipboard` 成員函式會在剪貼簿資料具有 `CF_OWNERDISPLAY` 格式，而且剪貼簿檢視器的水準捲軸中有事件時，由剪貼簿檢視器呼叫。
...
- SB_THUMBPOSITION 滾動到絕對位置。 目前的位置是在*nPos*中提供。

- SB_TOP 向左上方。

*nPos*<br/>
如果捲軸程式碼 SB_THUMBPOSITION，則包含捲動方塊位置。否則不會使用。
...
當圖示的背景必須在繪製圖示之前填滿時，架構會為最小化（iconic） `CWnd` 物件呼叫這個成員函式。
...
`CWnd` 只有在為視窗預設實定義類別圖示時，才會接收此呼叫;否則會呼叫[OnEraseBkgnd](#onerasebkgnd) 。
...
當使用者按一下功能表列上的專案或按下功能表鍵時，就會呼叫 `OnInitMenu`。 覆寫此成員函式，以在顯示功能表之前加以修改。

只有在第一次存取功能表時（例如，當使用者按一下功能表列上的專案時），才會呼叫 `OnInitMenu`。 這個方法不會提供功能表項目的相關資訊。 當使用者移至功能表中的專案時（例如，藉由將滑鼠移至數個功能表項目上），不會再次呼叫函式。 當使用者從功能表結束後（例如，按一下應用程式用戶端區域），然後按一下功能表列上的某個專案時，就會再次呼叫該函數。
...
|參數|描述|
|---------------|-----------------|
|*nCharSet*|在新地區設定的字元集。 如需詳細資訊，請參閱[LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw)結構的*lfCharSet*參數。|
|*nLocaleId*|在輸入地區設定識別碼。 如需詳細資訊，請參閱[語言識別項常數和字串](/windows/win32/Intl/language-identifier-constants-and-strings)。|

### <a name="remarks"></a>備註

這個方法會接收[WM_INPUTLANGCHANGE](/windows/win32/winmsg/wm-inputlangchange)通知訊息，如 Windows SDK 所述。
...
這個方法會接收[WM_INPUTLANGCHANGEREQUEST](/windows/win32/winmsg/wm-inputlangchangerequest)通知訊息，如 Windows SDK 所述。 當使用者使用鍵盤控制台應用程式中指定的熱鍵，或從系統工作列上的指標來選擇新的輸入語言時，就會張貼此訊息。
...
|-----------|-----------------|
|0-7|掃描程式碼（OEM 相依值）。|
|8|擴充的索引鍵，例如在數位鍵臺上的功能鍵或索引鍵（如果它是擴充索引鍵，則為1）。|
|9-10|未使用。|
|11-12|由 Windows 在內部使用。|
|13|內容代碼（如果在按下按鍵時，會按住 ALT 鍵，則為1，否則為0）。|
|14|先前的索引鍵狀態（1，如果金鑰在呼叫前關閉，則為0，如果索引鍵已啟動）。|
...
若為 WM_KEYDOWN 訊息，金鑰轉換位（位15）為0，且內容代碼位（位13）為0。

### <a name="remarks"></a>備註

非系統按鍵是鍵盤按鍵，在未按下 ALT 鍵時按下，或在 `CWnd` 具有輸入焦點時按下鍵盤按鍵。

由於自動重複，因此在進行[OnKeyUp](#onkeyup)成員函式呼叫之前，可能會有一個以上的 `OnKeyDown` 呼叫。 表示前一個索引鍵狀態的位，可以用來判斷 `OnKeyDown` 呼叫是第一個向下轉換還是重複的向下轉換。
...
對於 WM_KEYUP 訊息，金鑰轉換位（位15）是1，而內容代碼位（位13）為0。

### <a name="remarks"></a>備註

非系統按鍵是鍵盤按鍵，當未按下 ALT 鍵時按下，或在 `CWnd` 具有輸入焦點時按下鍵盤按鍵。
...
- 如果滑鼠左按鈕已關閉，MK_LBUTTON 設定。

- 如果中間滑鼠按鍵已關閉，MK_MBUTTON 設定。

- 如果滑鼠右鍵已關閉，MK_RBUTTON 設定。
...
只有具有 CS_DBLCLKS [WNDCLASS](/windows/win32/api/winuser/ns-winuser-wndclassw)樣式的 windows 才會收到 `OnLButtonDblClk` 呼叫。 這是 Microsoft Foundation Class windows 的預設值。 當使用者按下、放開，然後在系統的按兩下時間限制內再次按下滑鼠左鍵時，Windows 就會呼叫 `OnLButtonDblClk`。 按兩下滑鼠左鍵時，實際上會產生四個事件： [WM_LBUTTONDOWN](#onlbuttondown)、 [WM_LBUTTONUP](#onlbuttonup)訊息、WM_LBUTTONDBLCLK 呼叫，以及放開按鈕時的另一個 WM_LBUTTONUP 訊息。
...
只有具有 CS_DBLCLKS [WNDCLASS](/windows/win32/api/winuser/ns-winuser-wndclassw)樣式的 windows 才會收到 `OnMButtonDblClk` 呼叫。 這是所有 [Microsoft Foundation Class] 視窗的預設值。 當使用者按下、放開，然後在系統的按兩下時間限制內再次按下滑鼠按鍵時，Windows 就會產生 `OnMButtonDblClk` 呼叫。 按兩下滑鼠中間按鈕，實際上會產生四個事件： [WM_MBUTTONDOWN](#onmbuttondown)和[WM_MBUTTONUP](#onmbuttonup)訊息、WM_MBUTTONDBLCLK 呼叫，以及另一個 WM_MBUTTONUP 訊息。
...
MDI 子視窗會在 MDI 框架視窗之外獨立啟用。 當框架變成作用中時，最後使用 `OnMDIActivate` 呼叫啟動的子視窗會收到[WM_NCACTI加值稅E](#onncactivate)訊息，以繪製活動視窗框架和標題列，但不會收到另一個 `OnMDIActivate` 呼叫。
...
覆寫這個成員函式，並填寫*lpMeasureItemStruct*所指向的 `MEASUREITEMSTRUCT` 資料結構，然後傳回。這會通知視窗控制項的維度，並讓 Windows 能夠正確處理使用者與控制項的互動。

如果清單方塊或下拉式方塊是使用 [ [LBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#list-box-styles) ] 或 [ [CBS_OWNERDRAWVARIABLE](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) ] 樣式所建立，則架構會針對控制項中的每個專案，呼叫此函式的擁有者;否則，會呼叫此函數一次。

Windows 會在傳送[WM_INITDIALOG](/windows/win32/dlgbox/wm-initdialog)訊息之前，針對使用 OWNERDRAWFIXED 樣式所建立之下拉式方塊和清單方塊的擁有者，起始 `OnMeasureItem` 的呼叫。 因此，當擁有者收到此呼叫時，Windows 尚未決定控制項中所使用字型的高度和寬度;需要這些值的函式呼叫和計算，應出現在應用程式或程式庫的 main 函式中。

如果要測量的專案是 `CMenu`、`CListBox` 或 `CComboBox` 物件，則會呼叫適當類別的 `MeasureItem` 虛擬函式。 覆寫適當控制項類別的 `MeasureItem` 成員函式，以計算和設定每個專案的大小。

只有當控制項的類別是在執行時間建立，或是以 LBS_OWNERDRAWVARIABLE 或 CBS_OWNERDRAWVARIABLE 的樣式建立時，才會呼叫 `OnMeasureItem`。 如果控制項是由對話方塊編輯器所建立，則不會呼叫 `OnMeasureItem`。 這是因為在控制項的建立過程中，會提早傳送[WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem)訊息。 如果您使用 `DDX_Control`、`SubclassDlgItem`或 `SubclassWindow`子類別化，通常會在建立程式之後發生。 因此，沒有任何方法可以處理控制項的 `OnChildNotify` 函式中的[WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem)訊息，這是 MFC 用來執行 ON_WM_MEASUREITEM_REFLECT 的機制。
...
如果高序位字組包含0或1，則會忽略低序位字組。 當快速鍵（快捷方式）按鍵用來選取放在功能表中的點陣圖時，應用程式應該處理此訊息。

### <a name="remarks"></a>備註

它會傳送至擁有功能表的 `CWnd`。 當使用者按下 ALT 和任何其他按鍵時，也會呼叫 `OnMenuChar`，即使索引鍵並未對應到助憶鍵字元。 在此情況下， *pMenu*會指向 `CWnd`所擁有的功能表，而*nFlags*則是0。
...
|參數|描述|
|---------------|-----------------|
|*pMenu*|在[MENUGETOBJECTINFO](/windows/win32/api/winuser/ns-winuser-menugetobjectinfo)結構的指標，其中包含滑鼠游標所在之拖放功能表的相關資訊。|

### <a name="return-value"></a>傳回值

|傳回值|意義|
|------------------|-------------|
|MNGO_NOERROR|支援拖放作業的介面指標會在[MENUGETOBJECTINFO](/windows/win32/api/winuser/ns-winuser-menugetobjectinfo)結構的 `pvObj` 成員中傳回。 目前只支援[IDropTarget](/windows/win32/api/oleidl/nn-oleidl-idroptarget)介面。|
...
這個方法會接收[WM_MENURBUTTONUP](/windows/win32/menurc/wm-menurbuttonup)通知，如 Windows SDK 所述。 [WM_MENURBUTTONUP](/windows/win32/menurc/wm-menurbuttonup)訊息可讓應用程式為訊息中指定的功能表項目提供內容相關性功能表。
...
如果 `CWnd` 物件與功能表相關聯，則當使用者選取功能表項目時，架構會呼叫 `OnMenuSelect`。
...
*hSysMenu*<br/>
如果*nFlags*包含 MF_SYSMENU，會識別與訊息相關聯的功能表。 如果*nFlags*包含 MF_POPUP，則會識別主功能表的控制碼。 如果*nFlags*不包含 MF_SYSMENU 或 MF_POPUP，則不會使用。

### <a name="remarks"></a>備註

如果*nFlags*包含0xffff，而*hSysMenu*包含0，則 Windows 已關閉功能表，因為使用者已按下 ESC 鍵或在功能表外部按一下。
...
- MA_NOACTI加值稅E 不要啟動 `CWnd` 物件。

- MA_ACTI加值稅EANDEAT 啟動 `CWnd` 物件，並捨棄滑鼠事件。
...
當游標停留在視窗的工作區上，且在先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定的期間內時，架構會呼叫這個成員函式。
...
|*此處*|在[CPoint](../../atl-mfc-shared/reference/cpoint-class.md)物件，指定相對於工作區左上角之游標的*x*和*y*座標。|

### <a name="remarks"></a>備註

這個方法會接收[WM_MOUSEHOVER](/windows/win32/inputdev/wm-mousehover)通知，如 Windows SDK 所述。

*NFlags*參數可以是下表所列輔助按鍵的組合。 如需詳細資訊，請參閱[關於滑鼠輸入](/windows/win32/inputdev/about-mouse-input)。
...
|參數|描述|
|---------------|-----------------|
|*nFlags*|在旗標的位元組合（OR），指出所按的輔助按鍵。 例如，MK_CONTROL 旗標表示已按下 CTRL 鍵。<br /><br /> 如需旗標的清單，請參閱[關於滑鼠輸入](/windows/win32/inputdev/about-mouse-input)中的「訊息參數」子標題。|
|*zDelta*|在表示滾輪旋轉的距離，以 WHEEL_DELTA 的倍數或刻度表示，也就是120。 正數值表示滾輪已旋轉至右側;負數值表示滾輪已旋轉至左邊。|
|*pt*|在[CPoint](../../atl-mfc-shared/reference/cpoint-class.md)物件，指定相對於工作區左上角之游標的*x*和*y*座標。|

### <a name="remarks"></a>備註

這個方法會接收[WM_MOUSEHWHEEL](/windows/win32/inputdev/wm-mousehwheel)通知訊息，如 Windows SDK 所述。 當滑鼠的水準滾輪為傾斜或旋轉時，此訊息會傳送至具有焦點的視窗。
...
當游標離開先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定之視窗的工作區時，架構會呼叫這個成員函式。
...
如果未捕捉到滑鼠，WM_MOUSEMOVE 的訊息會由滑鼠游標下的 `CWnd` 物件接收;否則，訊息會移至已捕捉滑鼠的視窗。
...
除非覆寫，否則 `OnMouseWheel` 會呼叫[WM_MOUSEWHEEL](/windows/win32/inputdev/wm-mousewheel)的預設值。 Windows 會自動將訊息路由至具有焦點的控制項或子視窗。 Win32 函式[DefWindowProc](/windows/win32/api/winuser/nf-winuser-defwindowprocw)會將訊息向上傳播到處理它的視窗。

*ZDelta*參數是 WHEEL_DELTA 的倍數，其設定為120。 此值是要採取之動作的閾值，而每個差異都應該執行一個這類動作（例如，向前滾動一個凹槽）。

WHEEL_DELTA 已設定為120，以允許更精細的解析度滾輪，例如不含凹槽的自由旋轉滾輪。 更精細的解析度滾輪會針對每個旋轉傳送更多訊息，但每個訊息的差異值較小。 若要使用這類滾輪，請新增傳入的*zDelta*值，直到到達 WHEEL_DELTA 為止（如此一來，您就可以取得相同的差異旋轉的相同回應），或滾動部分行以回應較頻繁的訊息。 您也可以選擇捲軸資料細微性並累積差異，直到達到 WHEEL_DELTA 為止。
...
*lpncsp*<br/>
指向[NCCALCSIZE_PARAMS](/windows/win32/api/winuser/ns-winuser-nccalcsize_params)資料結構，其中包含應用程式可用來計算 `CWnd` 矩形新大小和位置的資訊（包括工作區、框線、標題、捲軸等等）。

### <a name="remarks"></a>備註

藉由處理此訊息，應用程式可以在視窗的大小或位置變更時，控制視窗工作區的內容。

不論*bCalcValidRects*的值為何，`NCCALCSIZE_PARAMS` 結構的 `rgrc` 結構成員所指定之陣列中的第一個矩形，都包含視窗的座標。 若為子視窗，座標會相對於父視窗的工作區。 若為最上層視窗，座標為螢幕座標。 應用程式應該修改 `rgrc[0]` 矩形，以反映工作區的大小和位置。

只有在*bCalcValidRects*為 TRUE 時，`rgrc[1]` 和 `rgrc[2]` 矩形才有效。 在此情況下，`rgrc[1]` 矩形會包含視窗在移動或調整大小之前的座標。 [`rgrc[2]`] 矩形包含視窗在移動之前的工作區座標。 所有座標都是相對於父視窗或螢幕。
...
*lpCreateStruct*<br/>
指向 `CWnd`的[CREATESTRUCT](/windows/win32/api/winuser/ns-winuser-createstructw)資料結構。

### <a name="return-value"></a>傳回值

如果建立非工作區，則為非零值。 如果發生錯誤，則為 0;在此情況下，`Create` 函數會傳回**失敗**。
...
如果您想要執行自己的清除作業，例如**刪除此**作業，請覆寫 `PostNcDestroy`。 如果您覆寫 `OnNcDestroy`，就必須在基類中呼叫 `OnNcDestroy`，以確保在內部為視窗配置的任何記憶體都會釋放出來。

##  <a name="cwndonnchittest"></a><a name="onnchittest"></a>CWnd：： OnNcHitTest

架構會在每次移動滑鼠時，針對包含游標（或使用[SetCapture](#setcapture)成員函式來捕獲滑鼠輸入的 `CWnd` 物件）的 `CWnd` 物件呼叫這個成員函式。
...
其中一個滑鼠點擊測試列舉值。 如需值清單，請參閱[WM_NCHITTEST](/windows/win32/inputdev/wm-nchittest) 。
...
當使用者按兩下滑鼠左鍵，而游標位於 `CWnd`的非工作區中時，此架構會呼叫這個成員函式。
...
*nHitTest*<br/>
指定[點擊測試程式碼](#onnchittest)。 點擊測試是判斷游標位置的測試。 如需值清單，請參閱[WM_NCHITTEST](/windows/win32/inputdev/wm-nchittest) 。

*此處*<br/>
指定包含游標位置之 x 和 y 螢幕座標的 `CPoint` 物件。 這些座標一律相對於螢幕的左上角。
...
當使用者按下滑鼠左鍵，而游標位於 `CWnd` 物件的非工作區中時，架構會呼叫這個成員函式。
...
當游標停留在視窗的非工作區上，且在先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定的期間內時，架構會呼叫這個成員函式。
...
|參數|描述|
|---------------|-----------------|
|*nHitTest*|在[CWnd：:D efwindowproc](#defwindowproc)函數所傳回的點擊測試值，是處理[WM_NCHITTEST](/windows/win32/inputdev/wm-nchittest)訊息的結果。|
|*此處*|在[CPoint](../../atl-mfc-shared/reference/cpoint-class.md)物件，指定相對於螢幕左上角之游標的*x*和*y*座標。|
...
當游標離開先前呼叫[TrackMouseEvent](/windows/win32/api/winuser/nf-winuser-trackmouseevent)中指定之視窗的非工作區時，架構會呼叫這個成員函式。
...
當使用者按兩下滑鼠右鍵，而游標位於 `CWnd`的非工作區中時，此架構會呼叫這個成員函式。
...
這個方法會接收[WM_XBUTTONDBLCLK](/windows/win32/inputdev/wm-xbuttondblclk)通知，如 Windows SDK 所述。 這則訊息會張貼至包含資料指標的視窗中。 如果視窗已捕捉滑鼠，則不會張貼此訊息。
...
這個方法會接收[WM_NCXBUTTONDOWN](/windows/win32/inputdev/wm-ncxbuttondown)通知，如 Windows SDK 所述。 這則訊息會張貼至包含資料指標的視窗中。 如果視窗已捕捉滑鼠，則不會張貼此訊息。
...
這個方法會接收[WM_NCXBUTTONUP](/windows/win32/inputdev/wm-ncxbuttonup)通知，如 Windows SDK 所述。 這則訊息會張貼至包含資料指標的視窗中。 如果視窗已捕捉滑鼠，則不會張貼此訊息。
...
|參數|描述|
|---------------|-----------------|
|*nKey*|在旗標的位元組合（OR），指出所按的輔助按鍵。 例如，MK_CONTROL 旗標表示已按下 CTRL 鍵。<br /><br /> 如需旗標的清單，請參閱[關於滑鼠輸入](/windows/win32/inputdev/about-mouse-input)中的「訊息參數」子標題。|
|*lpMdiNextMenu*|在[MDINEXTMENU](/windows/win32/api/winuser/ns-winuser-mdinextmenu)結構的指標，其中包含要啟用之功能表的相關資訊。|

### <a name="remarks"></a>備註

這個方法會接收[WM_UNINITMENUPOPUP](/windows/win32/menurc/wm-uninitmenupopup)通知，如 Windows SDK 所述。 回應此訊息時，您的應用程式可以設定[MDINEXTMENU](/windows/win32/api/winuser/ns-winuser-mdinextmenu)結構的 `hmenuNext` 成員，以指定要切換的功能表，以及 `hwndNext` 成員指定要接收功能表通知訊息的視窗。
...
*lParam*<br/>
通知訊息（`NMHDR`）結構的指標，其中包含通知碼和其他資訊。 對於某些通知訊息，此參數會指向具有 `NMHDR` 結構做為其第一個成員的較大結構。
...
`OnNotify` 會處理用於控制通知的訊息對應。

覆寫衍生類別中的這個成員函式，以處理 WM_NOTIFY 訊息。 除非呼叫基類 `OnNotify`，否則覆寫將不會處理訊息對應。

如需 WM_NOTIFY 訊息的詳細資訊，請參閱技術提示61（TN061）、 [ON_NOTIFY 和 WM_NOTIFY 訊息](../../mfc/tn061-on-notify-and-wm-notify-messages.md)。 您可能也會對[控制項主題](../../mfc/controls-mfc.md)和 TN062、 [Windows 控制項的訊息反映](../../mfc/tn062-message-reflection-for-windows-controls.md)中所述的相關主題感興趣。
...
|參數|描述|
|---------------|-----------------|
|*pWnd*|在`CWnd` 物件的指標，表示傳送[WM_NOTIFY](/windows/win32/controls/wm-notify)訊息的視窗。<br /><br /> 如果 NF_QUERY *nCommand*參數，則此參數為控制項的指標，如果 NF_REQUERY *nCommand* ，則為控制項父視窗的指標。|
|*nCommand*|在專門用 WM_NOTIFY 訊息的命令值。 可能的值包括：<br /><br /> -NF_QUERY-<br />     此訊息是用來判斷是否應該在 WM_NOTIFY 訊息中使用 ANSI 或 Unicode 結構的查詢。 這則訊息會在建立控制項時，從控制項傳送至其父視窗，並回應此訊息的 NF_REQUERY 形式。<br />-NF_REQUERY-<br />     訊息是控制項的要求，可將此訊息的 NF_QUERY 格式傳送至其父視窗。 此要求會從父視窗傳送，並要求控制項重新查詢要在 WM_NOTIFY 訊息中使用之結構類型的父系。 如果 NF_REQUERY *nCommand*參數，則傳回值會是重新查詢作業的結果。|
...
|NFR_UNICODE|Unicode 結構應該用於控制項所傳送的 WM_NOTIFY 訊息。|
|0|發生錯誤。|

### <a name="remarks"></a>備註

這個方法會接收[WM_NOTIFYFORMAT](/windows/win32/Controls/wm-notifyformat)通知，如 Windows SDK 所述。 WM_NOTIFY 訊息會從通用控制項傳送至其父視窗，並從父視窗傳送到通用控制項。
...
當呼叫 `RedrawWindow` 成員函式並設定 RDW_INTERNALPAINT 旗標時，視窗可能會收到內部繪製訊息。 在此情況下，視窗可能不會有更新區域。 應用程式應該呼叫[GetUpdateRect](#getupdaterect)成員函式，以判斷視窗是否具有更新區域。 如果 `GetUpdateRect` 傳回0，則應用程式不應呼叫[BeginPaint](#beginpaint)和[EndPaint](#endpaint)成員函式。

藉由查看每個 WM_PAINT 訊息的內部資料結構，讓應用程式負責檢查是否有任何必要的內部重新繪製或更新，因為 WM_PAINT 訊息可能是由不正確區域和使用 RDW_INTERNALPAINT 旗標設定的 `RedrawWindow` 成員函式的呼叫所造成。

內部 WM_PAINT 訊息只會由 Windows 傳送一次。 `UpdateWindow` 成員函式將內部 WM_PAINT 訊息傳送至視窗之後，將不會再傳送或張貼任何其他的 WM_PAINT 訊息，直到視窗失效或再次呼叫 `RedrawWindow` 成員函式，並已設定 RDW_INTERNALPAINT 旗標為止。
...
當剪貼簿擁有者將資料放在剪貼簿上的 CF_OWNERDISPLAY 格式，而剪貼簿檢視器的工作區需要重新繪製時，剪貼簿的 `OnPaintClipboard` 成員函式就會由剪貼簿檢視器呼叫。
...
若要判斷整個工作區或其中一部分是否需要重新繪製，剪貼簿擁有者必須將 `PAINTSTRUCT` 結構之 `rcpaint` 成員中所指定之繪圖區域的維度，與最近[OnSizeClipboard](#onsizeclipboard)成員函式呼叫中所指定的維度做比較。

`OnPaintClipboard` 應該使用[GlobalLock](/windows/win32/api/winbase/nf-winbase-globallock) Windows 函式鎖定包含 `PAINTSTRUCT` 資料結構的記憶體，並在結束前使用[GlobalUnlock](/windows/win32/api/winbase/nf-winbase-globalunlock) Windows 函式解除鎖定該記憶體。
...
會針對所有最上層和重迭的視窗呼叫 `OnPaletteChanged` 成員函式，包括已變更系統調色板並導致傳送 WM_PALETTECHANGED 訊息的工作。 如果任何子視窗使用調色板，則必須將此訊息傳遞給它。
...
當父代的 `OnParentNotify` 成員函式在建立或終結其子視窗時，或當使用者按一下滑鼠按鍵，而游標位於子視窗上方時，就會呼叫該成員函式。
...
*lParam*<br/>
如果*message*的事件（低序位字組） WM_CREATE 或 WM_DESTROY， *lParam*會指定子視窗的視窗控制碼;否則， *lParam*會包含游標的 x 和 y 座標。 X 座標是低序位字組，而 y 座標則是高序位單字。

### <a name="remarks"></a>備註

建立子視窗時，系統會在建立視窗的[Create](#create)成員函式傳回之前，立即呼叫 `OnParentNotify`。 當子視窗被終結時，系統會先呼叫 `OnParentNotify`，然後再進行任何處理以終結視窗。

會針對子視窗的所有上階視窗呼叫 `OnParentNotify`，包括最上層視窗。

除了具有[WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles)樣式的所有子視窗，都會將此訊息傳送至其父視窗。 根據預設，對話方塊中的子視窗會有 WS_EX_NOPARENTNOTIFY 樣式，除非未藉由呼叫[CreateEx](#createex)成員函式來建立子視窗而不使用此樣式。
...
這個方法會接收[WM_POWERBROADCAST](/windows/win32/Power/wm-powerbroadcast)訊息，如 Windows SDK 所述。

*NPowerEvent*參數指定的事件如電池電力偏低、電源狀態已變更、要求或拒絕暫停作業的許可權、作業會在事件之後自動復原、系統正在暫停作業，或在暫停後繼續作業。 通常不會使用*nEventData*參數。 如需詳細資訊，請參閱[WM_POWERBROADCAST](/windows/win32/Power/wm-powerbroadcast)訊息的*wParam*和*lParam*參數。
...
當使用者選擇結束 Windows 會話時，或當應用程式呼叫[ExitWindows](/windows/win32/api/winuser/nf-winuser-exitwindows) Windows 函式時，架構會呼叫這個成員函式。
...
如果有任何應用程式傳回0，則 Windows 會話不會結束。 只要有一個應用程式傳回0，Windows 就會停止呼叫 `OnQueryEndSession`，並針對已傳回非零值的任何應用程式，傳送參數值為 FALSE 的[WM_ENDSESSION](#onendsession)訊息。

##  <a name="cwndonquerynewpalette"></a><a name="onquerynewpalette"></a>CWnd：： OnQueryNewPalette

當 `CWnd` 物件即將收到輸入焦點時，架構會呼叫這個成員函式，讓 `CWnd` 有機會在收到焦點時實現其邏輯調色板。
...
如果 `CWnd` 發現其邏輯調色板，則為非零值;否則為0。

##  <a name="cwndonqueryopen"></a><a name="onqueryopen"></a>CWnd：： OnQueryOpen

當 `CWnd` 物件最小化，且使用者要求將 `CWnd` 還原至其 preminimized 的大小和位置時，架構會呼叫這個成員函式。
...
如果可以開啟圖示，則為非零，否則為0，以防止開啟圖示。

### <a name="remarks"></a>備註

在 `OnQueryOpen`中，`CWnd` 不應執行任何會導致啟用或焦點變更的動作（例如，建立對話方塊）。
...
- 隱藏 UISF_HIDEFOCUS 焦點指標。

- 隱藏 UISF_HIDEACCEL 鍵盤快速鍵。
...
|*hRawInput*|在[RAWINPUT](/windows/win32/api/winuser/ns-winuser-rawinput)結構的控制碼，其中包含來自裝置的原始輸入。|
...
只有具有 CS_DBLCLKS [WNDCLASS](/windows/win32/api/winuser/ns-winuser-wndclassw)樣式的視窗可以接收 `OnRButtonDblClk` 呼叫。 這是 MFC 程式庫內的 windows 預設值。 當使用者按下、放開，然後在系統的按兩下時間限制內，再次按下滑鼠右鍵時，Windows 就會呼叫 `OnRButtonDblClk`。 按兩下滑鼠右鍵會產生四個事件： [WM_RBUTTONDOWN](#onrbuttondown)和[WM_RBUTTONUP](#onrbuttonup)訊息、`OnRButtonDblClk` 呼叫，以及放開按鈕時的另一個 WM_RBUTTONUP 訊息。
...
除非覆寫，否則 `OnRegisteredMouseWheel` 會將訊息路由至適當的視窗（具有焦點的父視窗），並呼叫該視窗的[WM_MOUSEWHEEL](/windows/win32/inputdev/wm-mousewheel)處理常式。
...
當擁有者應用程式遭到終結時，架構會呼叫剪貼簿擁有者的 `OnRenderAllFormats` 成員函式。
...
剪貼簿擁有者應該會以能夠產生的所有格式來轉譯資料，並藉由呼叫[SetClipboardData](/windows/win32/api/winuser/nf-winuser-setclipboarddata) Windows 函式，將每個格式的資料控制碼傳遞至剪貼簿。 這可確保即使轉譯資料的應用程式已終結，剪貼簿還是包含有效的資料。 應用程式應該在呼叫[SetClipboardData](/windows/win32/api/winuser/nf-winuser-setclipboarddata) Windows 函式之前呼叫[OpenClipboard](#openclipboard)成員函式，然後在之後呼叫[CloseClipboard](/windows/win32/api/winuser/nf-winuser-closeclipboard) Windows 函式。

##  <a name="cwndonrenderformat"></a><a name="onrenderformat"></a>CWnd：： OnRenderFormat

當需要呈現具有延遲轉譯的特定格式時，架構會呼叫剪貼簿擁有者的 `OnRenderFormat` 成員函式。
...
接收者應會以該格式轉譯資料，並藉由呼叫[SetClipboardData](/windows/win32/api/winuser/nf-winuser-setclipboarddata) Windows 函式將它傳遞至剪貼簿。

請勿從 `OnRenderFormat`中呼叫 `OpenClipboard` 成員函式或 `CloseClipboard` Windows 函數。
...
*NSessionState*參數指定會話已連接或中斷與主控台或遠端終端機機、使用者登入或關閉、會話已鎖定或解除鎖定，或會話已變更為遠端控制狀態。 如需詳細資訊，請參閱[WM_WTSSESSION_CHANGE](/windows/win32/TermServ/wm-wtssession-change)訊息的*wParam*參數。
...
如果未捕捉到滑鼠輸入，而滑鼠導致游標在 `CWnd` 物件內移動，則此架構會呼叫這個成員函式。
...
在處理之前，預設的實值會呼叫父視窗的 `OnSetCursor`。 如果父視窗傳回 TRUE，則會暫停進一步的處理。 呼叫父視窗可讓父視窗控制子視窗中的游標設定。

預設的執行會將游標設定為箭號（如果它不在工作區中）或已註冊的類別資料指標（如果它是）。

如果*nHitTest*是 HTERROR，而*message*是滑鼠按鍵的訊息，則會呼叫 `MessageBeep` 成員函式。

當 `CWnd` 進入功能表模式時， *message*參數為0。
...
*pOldWnd*<br/>
包含失去輸入焦點的 `CWnd` 物件（可能是 Null）。 該指標可能是暫時性的，因此不應該儲存供日後使用。

### <a name="remarks"></a>備註

若要顯示插入號，`CWnd` 應該在此時呼叫適當的插入號函式。
...
當 Win32 SystemParametersInfo 函式變更全系統的設定時，架構會呼叫所有最上層視窗的 `OnSettingChange`。
...
*uFlags*<br/>
當系統將訊息當做 `SystemParametersInfo` 呼叫的結果傳送時，此參數會是表示已變更之系統參數的旗標。 如需值清單，請參閱 Windows SDK 中的[SystemParametersInfo](/windows/win32/api/winuser/nf-winuser-systemparametersinfow) 。 當應用程式傳送訊息時，這個參數必須是0。
...
您的訊息對應中應該只有其中一個宏。 若要撰寫可同時適用于 Windows 95/98 和 Windows NT 4.0 的程式，請撰寫 ON_WM_SETTINGCHANGE 的處理常式。 在 Windows NT 3.51 之下，`OnSettingChange` 和*uFlags*會呼叫您的處理常式，而且一定會是零。

##  <a name="cwndonshowwindow"></a><a name="onshowwindow"></a>CWnd：： OnShowWindow

當即將隱藏或顯示 `CWnd` 物件時，架構會呼叫這個成員函式。
...
*nStatus*<br/>
指定要顯示之視窗的狀態。 如果因為 `ShowWindow` 的成員函式呼叫而傳送訊息，則為 0;否則， *nStatus*是下列其中一項：
...
當呼叫 `ShowWindow` 成員函式、將重迭視窗最大化或還原，或關閉（進行 iconic）或開啟重迭或快顯視窗（顯示在螢幕上）時，就會隱藏或顯示視窗。 當重迭視窗關閉時，與該視窗相關聯的所有快顯視窗都會隱藏起來。
...
如果從 `OnSize`針對子視窗呼叫[SetScrollPos](#setscrollpos)或[MoveWindow](#movewindow)成員函式，`SetScrollPos` 或 `MoveWindow` 的 `bRedraw` 參數應為非零，導致重新繪製 `CWnd`。
...
剪貼簿擁有者的 `OnSizeClipboard` 成員函式是由剪貼簿檢視器包含具有 `CF_OWNERDISPLAY` 屬性的資料，而剪貼簿檢視器視窗的工作區大小已變更時所呼叫。
...
當剪貼簿應用程式即將終結或最小化時，會使用 null 矩形（0，0，0，0）來呼叫 `OnSizeClipboard` 成員函式做為新的大小。 這允許剪貼簿擁有者釋放其顯示資源。

在 `OnSizeClipboard`內，應用程式必須使用[GlobalLock](/windows/win32/api/winbase/nf-winbase-globallock) Windows 函數來鎖定包含 RECT 資料結構的記憶體。 讓應用程式在產生或傳回控制項之前，使用[GlobalUnlock](/windows/win32/api/winbase/nf-winbase-globalunlock) Windows 函式解除鎖定該記憶體。
...
架構會在[SetWindowLong](/windows/win32/api/winuser/nf-winuser-setwindowlongw)函式變更一個或多個視窗樣式之後，呼叫這個成員函式。
...
*lpStyleStruct*<br/>
指向包含視窗新樣式的[STYLESTRUCT](/windows/win32/api/winuser/ns-winuser-stylestruct)結構。 應用程式可以檢查樣式，但不能變更它們。
...
當[SetWindowLong](/windows/win32/api/winuser/nf-winuser-setwindowlongw)函數即將變更一個或多個視窗樣式時，架構會呼叫這個成員函式。
...
*lpStyleStruct*<br/>
指向包含視窗新樣式的[STYLESTRUCT](/windows/win32/api/winuser/ns-winuser-stylestruct)結構。 應用程式可以檢查樣式並加以變更。
...
如果 `CWnd` 有輸入焦點，而且[WM_SYSKEYUP](#onsyskeyup)和[WM_SYSKEYDOWN](#onsyskeydown)訊息已轉譯，則架構會呼叫這個成員函式。
...
當內容代碼為0時，WM_SYSCHAR 可以將[WM_SYSCHAR](/windows/win32/menurc/wm-syschar)訊息傳遞至[TranslateAccelerator](/windows/win32/api/winuser/nf-winuser-translateacceleratorw) Windows 函式，這將會處理它，就好像它是一般的索引鍵訊息，而不是系統字元按鍵。 這允許在使用中視窗中使用快速鍵，即使使用中視窗沒有輸入焦點也一樣。
...
- SC_HOTKEY 啟動與應用程式指定的熱鍵關聯的 `CWnd` 物件。 *LParam*的低序位單字會識別要啟動之視窗的 HWND。
...
- SC_HOTKEY 啟動與應用程式指定的熱鍵關聯的視窗。 *LParam*的低序位字組會識別要啟動的視窗。

- SC_SCREENSAVE 執行 [控制台] 的 [桌面] 區段中所指定的螢幕儲存應用程式。

### <a name="remarks"></a>備註

根據預設，`OnSysCommand` 會針對上表中指定的預先定義動作執行控制功能表要求。

在 WM_SYSCOMMAND 訊息中，Windows 會在內部使用*nID*參數的四個低序位位。 當應用程式測試*nid*的值時，它必須使用位 and 運算子來結合值0XFFF0 與*nid*值，以取得正確的結果。

您可以使用 [`GetSystemMenu`]、[`AppendMenu`]、[`InsertMenu`] 和 [`ModifyMenu`] 成員函式來修改 [控制] 功能表中的功能表項目。 修改 [控制] 功能表的應用程式必須處理 WM_SYSCOMMAND 訊息，而且應用程式未處理的任何 WM_SYSCOMMAND 訊息必須傳遞至 `OnSysCommand`。 應用程式所加入的任何命令值都必須由應用程式處理，而且無法傳遞至 `OnSysCommand`。

應用程式可以將 WM_SYSCOMMAND 訊息傳遞給 `OnSysCommand`，隨時執行任何系統命令。

定義用來從 [控制] 功能表中選取專案的快速鍵（快捷方式）按鍵會轉譯成 `OnSysCommand` 呼叫;所有其他快速鍵按鍵都會轉譯成[WM_COMMAND](#oncommand)訊息。
...
如果在呼叫[OnSysKeyUp](#onsyskeyup)或[OnSysKeyDown](#onsyskeydown)成員函式時，`CWnd` 物件具有輸入焦點，則架構會呼叫這個成員函式。
...
如果 `CWnd` 物件具有輸入焦點，則當使用者按住 ALT 鍵，然後按下另一個按鍵時，架構就會呼叫 `OnSysKeyDown` 成員函式。
...
|9-10|未使用。|
|11-12|由 Windows 在內部使用。|
|13|內容代碼（如果 ALT 鍵在按下按鍵時保持關閉，則為1，否則為0）。|
|14|先前的索引鍵狀態（1，如果金鑰在傳送訊息之前已關閉，則為0，如果金鑰已啟動）。|
|15|轉換狀態（如果正在釋放索引鍵，則為1，如果按下按鍵則為0）。|

針對 `OnSysKeyDown` 呼叫，金鑰轉換位（位15）為0。 如果 ALT 鍵在按下按鍵時關閉，則內容代碼位（位13）為 1;如果訊息傳送到使用中視窗，則為0，因為沒有視窗具有輸入焦點。

### <a name="remarks"></a>備註

如果目前沒有任何視窗具有輸入焦點，則會呼叫使用中視窗的 `OnSysKeyDown` 成員函式。 接收訊息的 `CWnd` 物件可以藉由檢查 `nFlags`中的內容程式碼來區分這兩個內容。

當內容代碼為0時，`OnSysKeyDown` 所收到的 WM_SYSKEYDOWN 訊息可以傳遞至[TranslateAccelerator](/windows/win32/api/winuser/nf-winuser-translateacceleratorw) Windows 函式，這將會處理它，就如同它是一般的索引鍵訊息，而不是系統金鑰訊息。 這允許在使用中視窗中使用快速鍵，即使使用中視窗沒有輸入焦點也一樣。

因為自動重複，在收到[WM_SYSKEYUP](#onsyskeyup)訊息之前，可能會有一個以上的 `OnSysKeyDown` 呼叫。 先前的索引鍵狀態（位14）可以用來判斷 `OnSysKeyDown` 呼叫是否表示第一個向下轉換或重複的向下轉換。
...
如果 `CWnd` 物件具有焦點，則當使用者放開按住 ALT 鍵時所按下的按鍵時，架構會呼叫 `OnSysKeyUp` 成員函式。
...
針對 `OnSysKeyUp` 呼叫，金鑰轉換位（位15）為1。 如果 ALT 鍵在按下按鍵時關閉，則內容代碼位（位13）為 1;如果訊息傳送到使用中視窗，則為0，因為沒有視窗具有輸入焦點。

### <a name="remarks"></a>備註

如果目前沒有任何視窗具有輸入焦點，則會呼叫使用中視窗的 `OnSysKeyUp` 成員函式。 接收呼叫的 `CWnd` 物件可以藉由檢查*nFlags*中的內容程式碼來區分這兩個內容。

當內容代碼為0時，`OnSysKeyUp` 所收到的 WM_SYSKEYUP 訊息可以傳遞至[TranslateAccelerator](/windows/win32/api/winuser/nf-winuser-translateacceleratorw) Windows 函式，這將會處理它，就如同它是一般的索引鍵訊息，而不是系統金鑰訊息。 這可讓快速鍵（快捷方式）索引鍵與使用中視窗搭配使用，即使使用中視窗沒有輸入焦點也一樣。

針對 IBM 增強的101和102金鑰鍵盤，增強式金鑰是鍵盤主要區段上的右 ALT 和右 CTRL 鍵;數位鍵臺上左邊的叢集中的 INS、DEL、HOME、END、PAGE UP、PAGE DOWN 和方向鍵;和斜線（/），然後在數位鍵臺上輸入按鍵。 有些其他鍵盤可能支援*nFlags*中的延伸金鑰位。

針對非美國增強的102鍵鍵盤，右 ALT 鍵會以 CTRL + ALT 鍵組合的方式處理。 以下顯示訊息的順序，以及當使用者按下並釋放此金鑰時所產生的呼叫：
...
只有在應用程式已起始含有 Windows 說明的訓練卡時，才會呼叫此函式。 應用程式會在對[WinHelp](../../mfc/reference/cwinapp-class.md#winhelp)函式的呼叫中指定 HELP_TCARD 命令，以起始定型卡。
...
若有任何應用程式變更系統時間，請將此訊息傳送至所有最上層視窗。 若要將 WM_TIMECHANGE 訊息傳送至所有最上層視窗，應用程式可以使用[SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) windows 函數，並將其*hwnd*參數設定為 HWND_BROADCAST。
...
當應用程式的訊息佇列中沒有其他訊息時， [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows 函數會傳送[WM_TIMER](/windows/win32/winmsg/wm-timer)訊息。
...
如果與工具提示相關聯的區域不是按鈕，`OnToolHitTest` 會將結構旗標設定為 TTF_NOTBUTTON 並 TTF_CENTERTIP。

覆寫 `OnToolHitTest` 以提供不同于預設提供的資訊。
...
當按下按鍵時，架構會呼叫這個成員函式。 也就是說，目前的視窗具有鍵盤焦點，而[WM_KEYDOWN](/windows/win32/inputdev/wm-keydown)的訊息則是由[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)函數轉譯。
...
|*nFlags*|在指定掃描碼、擴充金鑰、內容代碼、先前的索引鍵狀態和轉換狀態的旗標，如下表所示：<br /><br /> **0-7：** 指定掃描程式碼。 此值取決於原始設備製造商（OEM）。<br /><br /> **8：** 指定擴充的索引鍵，例如在增強的101或102鍵鍵盤上出現的右 ALT 和 CTRL 鍵。 如果索引鍵是擴充的索引鍵，則旗標為 1;否則，它是0。<br /><br /> **9-12：** 由 Windows 在內部使用。<br /><br /> **13：** 指定內容代碼。 如果在按下按鍵時，會按住 ALT 鍵，則旗標為 1;否則，此值為0。<br /><br /> **14：** 指定先前的索引鍵狀態。 如果金鑰在傳送訊息之前已關閉，則旗標為1，如果索引鍵已啟動，則為0。<br /><br /> **15：** 指定轉換狀態。 如果要釋放金鑰，旗標會是1，如果按下按鍵則為0。|

### <a name="remarks"></a>備註

這個方法會接收[WM_UNICHAR](/windows/win32/inputdev/wm-unichar)通知，如 Windows SDK 所述。 [WM_UNICHAR](/windows/win32/inputdev/wm-unichar)訊息是設計用來將 Unicode 字元傳送或張貼到 ANSI 視窗。 它相當於[WM_CHAR](/windows/win32/inputdev/wm-char)訊息，但使用 Unicode 轉換格式-32 編碼（UTF-32），而[WM_CHAR](/windows/win32/inputdev/wm-char)訊息使用 utf-16。
...
- UIS_INITIALIZE UI 狀態元素（由*nUIElement*指定）應該根據最後一個輸入事件來變更。 如需詳細資訊，請參閱[WM_UPDATEISTATE](/windows/win32/menurc/wm-updateuistate)的「**備註**」一節。
...
這個方法會接收[WM_USERCHANGED](/windows/win32/winmsg/wm-userchanged)通知訊息，如 Windows SDK 所述。 當使用者登入或關閉時，作業系統會更新使用者特定的設定。 系統會在更新設定之後立即傳送此訊息。
...
*nPos*<br/>
如果捲軸程式碼 SB_THUMBPOSITION 或 SB_THUMBTRACK，則包含目前的捲動方塊位置。否則不會使用。 視初始捲軸範圍而定， *nPos*可能是負值，而且應該在必要時轉換成**int** 。
...
`OnVScroll` 通常是在拖曳捲動方塊時，提供一些意見反應的應用程式使用。
...
剪貼簿擁有者的 `OnVScrollClipboard` 成員函式會在剪貼簿資料具有 CF_OWNERDISPLAY 格式，而且剪貼簿檢視器的垂直捲動條中有事件時，由剪貼簿檢視器呼叫。
...
*nPos*<br/>
如果捲軸程式碼 SB_THUMBPOSITION，則包含捲動方塊位置。否則不會使用*nPos* 。
...
這個方法會接收[WM_DWMWINDOWMAXIMIZEDCHANGE](/windows/win32/dwm/wm-dwmwindowmaximizedchange)通知訊息，如 Windows SDK 所述。
...
當大小、位置或迭置順序因呼叫[SetWindowPos](/windows/win32/api/winuser/nf-winuser-setwindowpos)成員函式或其他視窗管理功能而變更時，架構會呼叫這個成員函式。
...
預設的執行會將[WM_SIZE](/windows/win32/winmsg/wm-size)和[WM_MOVE](/windows/win32/winmsg/wm-move)訊息傳送至視窗。 如果應用程式處理 `OnWindowPosChanged` 呼叫，而未呼叫其基類，則不會傳送這些訊息。 在呼叫 `OnWindowPosChanged` 期間執行任何移動或大小變更處理，而不需要呼叫其基類，會更有效率。
...
當大小、位置或迭置順序即將因呼叫[SetWindowPos](/windows/win32/api/winuser/nf-winuser-setwindowpos)成員函式或其他視窗管理功能而變更時，架構會呼叫這個成員函式。
...
*lpwndpos*<br/>
指向 `WINDOWPOS` 的資料結構，其中包含視窗新大小和位置的相關資訊。

### <a name="remarks"></a>備註

應用程式可以藉由在[WINDOWPOS](/windows/win32/api/winuser/ns-winuser-windowpos)結構的**flags**成員中設定或清除適當的位，來防止變更視窗。

針對具有[WS_OVERLAPPED](styles-used-by-mfc.md#window-styles)或[WS_THICKFRAME](styles-used-by-mfc.md#window-styles)樣式的視窗，預設的執行會將[WM_GETMINMAXINFO](/windows/win32/winmsg/wm-getminmaxinfo)訊息傳送至視窗。 這是為了驗證視窗的新大小和位置，以及強制執行 CS_BYTEALIGNCLIENT 並 CS_BYTEALIGN 用戶端樣式而完成。 應用程式可以藉由不呼叫其基類來覆寫這項功能。
...
[SystemParametersInfo](/windows/win32/api/winuser/nf-winuser-systemparametersinfow) Windows 函數會在應用程式使用函式來變更 WIN 中的設定之後，呼叫 `OnWinIniChange`。INI 檔案。

若要將 WM_WININICHANGE 訊息傳送至所有最上層視窗，應用程式可以使用[SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) windows 函數，並將其*hwnd*參數設定為 HWND_BROADCAST。

如果應用程式變更 WIN 中的許多不同區段。INI 時，應用程式應該傳送一個 WM_WININICHANGE 訊息，並將*lpszSection*設為 Null。 否則，應用程式應該會在每次變更時傳送 WM_WININICHANGE.INI.

如果應用程式收到 `OnWinIniChange` 呼叫，並將*lpszSection*設為 Null，則應用程式應該檢查 WIN 中的所有區段。會影響應用程式的 INI。
...
`OnWndMsg` 決定訊息類型，並呼叫適當的架構函式（例如，WM_COMMAND 的[OnCommand](#oncommand) ），或在訊息對應中尋找適當的訊息。
...
這個方法會接收[WM_XBUTTONDBLCLK](/windows/win32/inputdev/wm-xbuttondblclk)通知，如 Windows SDK 所述。 如果未捕捉到滑鼠，訊息就會張貼到游標下方的視窗中。 否則，訊息就會張貼到已捕獲滑鼠的視窗中。
...
這個方法會接收[WM_XBUTTONDOWN](/windows/win32/inputdev/wm-xbuttondown)通知，如 Windows SDK 所述。 如果未捕捉到滑鼠，訊息就會張貼到游標下方的視窗中。 否則，訊息就會張貼到已捕獲滑鼠的視窗中。
...
這個方法會接收[WM_XBUTTONUP](/windows/win32/inputdev/wm-xbuttonup)通知，如 Windows SDK 所述。 如果未捕捉到滑鼠，訊息就會張貼到游標下方的視窗中。 否則，訊息就會張貼到已捕獲滑鼠的視窗中。
...
如果透過 `CWnd`開啟剪貼簿，則為非零，如果另一個應用程式或視窗已開啟剪貼簿，則為0。

### <a name="remarks"></a>備註

在呼叫[CloseClipboard](/windows/win32/api/winuser/nf-winuser-closeclipboard) Windows 函數之前，其他應用程式將無法修改剪貼簿。

在呼叫[EmptyClipboard](/windows/win32/api/winuser/nf-winuser-emptyclipboard) Windows 函數之前，目前 `CWnd` 物件不會成為剪貼簿的擁有者。
...
比較兩個 `CWnd` 的物件，以判斷它們是否沒有相同的[m_hWnd](#m_hwnd)。
...
比較兩個 `CWnd` 的物件，以判斷它們是否具有相同的[m_hWnd](#m_hwnd)。
...
訊息佇列中的訊息是透過呼叫[GetMessage](/windows/win32/api/winuser/nf-winuser-getmessage)或[PeekMessage](/windows/win32/api/winuser/nf-winuser-peekmessagew) Windows 函數來抓取。

Windows [PostMessage](/windows/win32/api/winuser/nf-winuser-postmessagew)函數可用來存取另一個應用程式。
...
在建立附加到這個 `CWnd` 物件的 Windows 視窗之前，由架構呼叫。
...
> [!WARNING]
> `CWnd::PreCreateWindow` 現在會將*cs*的 hMenu 成員指派給**此**指標（如果功能表為 Null，且樣式包含 WS_CHILD）。 如需適當的功能，請確定您的對話方塊控制項具有不是 Null 的識別碼。
>
> 這項變更會修正 managed/原生 interop 案例中的損毀。 `CWnd::Create` 中的追蹤語句會對問題的開發人員發出警示。

永遠不要直接呼叫此函式。

此函式的預設實作用會檢查 Null 視窗類別名稱，並替代適當的預設值。 覆寫這個成員函式，以在建立視窗之前修改 `CREATESTRUCT` 結構。

每個衍生自 `CWnd` 的類別都會在其 `PreCreateWindow`的覆寫中加入自己的功能。 根據設計，這些 `PreCreateWindow` 的衍生並未記載。 若要判斷適用于每個類別的樣式，以及樣式之間的相互相關性，您可以檢查應用程式基類的 MFC 原始程式碼。 如果您選擇覆寫 `PreCreateWindow,` 您可以判斷應用程式的基類中所使用的樣式，是否使用從 MFC 原始程式碼收集而來的資訊，提供您所需的功能。
...
- 如果指定了 PRF_CHECKVISIBLE，而且看不到視窗，則不執行任何動作。

- 如果指定了 PRF_NONCLIENT，請在指定的裝置內容中繪製非工作區。

- 如果指定了 PRF_ERASEBKGND，請將[WM_ERASEBKGND](/windows/win32/winmsg/wm-erasebkgnd)訊息傳送給視窗。

- 如果指定了 PRF_CLIENT，請將[WM_PRINTCLIENT](/windows/win32/gdi/wm-printclient)訊息傳送給視窗。

- 如果已設定 PRF_CHILDREN，請將[WM_PRINT](/windows/win32/gdi/wm-print)訊息傳送給每個可見的子視窗。

- 如果已設定 PRF_OWNED，請將 WM_PRINT 訊息傳送給每個可見的擁有視窗。
...
此成員函式會模擬函數[PrintWindow](/windows/win32/api/winuser/nf-winuser-printwindow)的功能，如 Windows SDK 中所述。
...
*lpRectUpdate*<br/>
指向包含更新矩形座標的[矩形結構](/windows/win32/api/windef/ns-windef-rect)。 如果*prgnUpdate*包含有效的區域控制碼，則會忽略這個參數。
...
- RDW_ERASE 會在重新繪製視窗時，讓視窗接收[WM_ERASEBKGND](/windows/win32/winmsg/wm-erasebkgnd)訊息。 也必須指定 RDW_INVALIDATE 旗標;否則 RDW_ERASE 不會有任何作用。

- RDW_FRAME 會使視窗中非工作區的任何部分與更新區域相交，以接收[WM_NCPAINT](/windows/win32/gdi/wm-ncpaint)訊息。 也必須指定 RDW_INVALIDATE 旗標;否則 RDW_FRAME 不會有任何作用。

- 無論視窗是否包含不正確區域，RDW_INTERNALPAINT 都會使[WM_PAINT](/windows/win32/gdi/wm-paint)訊息張貼至視窗。

- RDW_INVALIDATE 使*lpRectUpdate*或*prgnUpdate*失效（只有一個可能不是 Null）。 如果兩者都是 Null，則整個視窗都會失效。
...
- RDW_NOINTERNALPAINT 會隱藏任何擱置的內部 WM_PAINT 訊息。 此旗標不會影響因無效區域而產生的 WM_PAINT 訊息。

- RDW_VALIDATE 驗證*lpRectUpdate*或*prgnUpdate* （只有一個可能不是 Null）。 如果兩者都是 Null，則會驗證整個視窗。 此旗標不會影響內部 WM_PAINT 訊息。

發生重新繪製時的下列旗標控制項。 除非指定了其中一個位，否則 `RedrawWindow` 函數不會執行繪製。

- RDW_ERASENOW 會使受影響的視窗（如 RDW_ALLCHILDREN 和 RDW_NOCHILDREN 旗標所指定）在函式傳回之前，收到 WM_NCPAINT 和 WM_ERASEBKGND 訊息。 WM_PAINT 訊息會延遲。

- RDW_UPDATENOW 會在函式傳回之前，讓受影響的視窗（如 RDW_ALLCHILDREN 和 RDW_NOCHILDREN 旗標所指定）接收 WM_NCPAINT、WM_ERASEBKGND 和 WM_PAINT 訊息。

根據預設，受 `RedrawWindow` 函數影響的 windows 取決於指定的視窗是否具有 WS_CLIPCHILDREN 樣式。 WS_CLIPCHILDREN 視窗的子視窗不受影響。 不過，那些不 WS_CLIPCHILDREN 視窗的視窗會以遞迴方式進行驗證或失效，直到遇到 WS_CLIPCHILDREN 視窗為止。 下列旗標可控制 `RedrawWindow` 函式會影響哪些視窗：

- RDW_ALLCHILDREN 包括重新繪製作業中的子視窗（如果有的話）。

- RDW_NOCHILDREN 從重新繪製作業排除子視窗（如果有的話）。
...
當 `RedrawWindow` 成員函式用來使部分的桌面視窗失效時，該視窗不會收到[WM_PAINT](/windows/win32/gdi/wm-paint)訊息。 若要重新繪製桌面，應用程式應該使用[CWnd：： ValidateRgn](#validatergn)、 [Cwnd：： InvalidateRgn](#invalidatergn)、 [CWnd：： UpdateWindow](#updatewindow)或[RedrawWindow](/windows/win32/api/winuser/nf-winuser-redrawwindow)
...
應用程式必須在每次呼叫[GetWindowDC](#getwindowdc)成員函式，以及每次呼叫[GetDC](#getdc)成員函式時，呼叫 `ReleaseDC` 成員函式。
...
- `CWnd::reposDefault` 會執行控制列的版面配置。 不使用*lpRectParam* ，而且可以是 Null。

- `CWnd::reposQuery` 控制列的配置尚未完成;相反地， *lpRectParam*會使用工作區的大小進行初始化，如同已實際完成配置一樣。

- `CWnd::reposExtra` 會將*lpRectParam*的值新增至*nIDLast*的工作區，也會執行版面配置。

*lpRectParam*<br/>
指向[矩形結構](/windows/win32/api/windef/ns-windef-rect);的使用方式取決於*nFlag*的值。
...
*dwFlags*<br/>
指定要傳送的 Windows 訊息。 可以是下列其中一個值：

- MLF_NOIDLEMSG 不會將[WM_ENTERIDLE](/windows/win32/dlgbox/wm-enteridle)訊息傳送至父系。

- MLF_NOKICKIDLE 不要將 WM_KICKIDLE 訊息傳送至視窗。

- MLF_SHOWONIDLE 在訊息佇列閒置時顯示視窗。
...
根據預設，在呼叫 `EndModalLoop` 之後，`ContinueModal` 會傳回 FALSE。 傳回提供給 `EndModalLoop`的*n 結果*值。
...
*lpPoint*<br/>
指向[CPoint](../../atl-mfc-shared/reference/cpoint-class.md)物件或[點結構](/windows/win32/api/windef/ns-windef-point)，其中包含要轉換的螢幕座標。

*lpRect*<br/>
指向[CRect](../../atl-mfc-shared/reference/crect-class.md)物件或[矩形結構](/windows/win32/api/windef/ns-windef-rect)，其中包含要轉換的螢幕座標。

### <a name="remarks"></a>備註

`ScreenToClient` 成員函式會以用戶端座標取代*lpPoint*或*lpRect*中所指定的螢幕座標。 新座標是相對於 `CWnd` 工作區的左上角。
...
*lpRect*<br/>
指向[CRect](../../atl-mfc-shared/reference/crect-class.md)物件或[RECT 結構](/windows/win32/api/windef/ns-windef-rect)，指定要滾動的工作區部分。 如果*lpRect*為 Null，則會滾動整個工作區。 如果游標矩形與捲軸矩形相交，插入號就會重新置放。

*lpClipRect*<br/>
指向 `CRect` 物件或 `RECT` 結構，指定要滾動的裁剪矩形。 只有這個矩形內的位會進行滾動。 這個矩形外的位不會受到影響，即使它們位於*lpRect*矩形中也一樣。 如果*lpClipRect*為 Null，則不會在捲軸矩形上執行任何裁剪。

### <a name="remarks"></a>備註

如果插入號是在要滾動的 `CWnd` 中，`ScrollWindow` 會自動隱藏插入號以防止它被清除，然後在捲軸完成後還原插入號。 插入號位置會據以調整。

`ScrollWindow` 成員函式所發現的區域不會重新繪製，而是會合並到目前 `CWnd` 物件的更新區域中。 應用程式最後會收到[WM_PAINT](/windows/win32/gdi/wm-paint)訊息，通知它該區域需要重新繪製。 若要在滾動完成時重新繪製未發現的區域，請在呼叫 `ScrollWindow`之後立即呼叫[UpdateWindow](#updatewindow)成員函式。

如果*lpRect*為 Null，則視窗中任何子視窗的位置都會以*xAmount*和*yAmount*所指定的數量來位移，而且 `CWnd` 中的任何無效（未上色）區域也會位移。 當*lpRect*為 Null 時，`ScrollWindow` 會更快。

如果*lpRect*不是 Null，則不會變更子視窗的位置，而且 `CWnd` 中的無效區域不會位移。 若要在*lpRect*不是 Null 時避免更新問題，請在呼叫 `ScrollWindow`之前，呼叫 `UpdateWindow` 成員函式來重新繪製 `CWnd`。
...
*lpRectScroll*<br/>
指向[矩形結構](/windows/win32/api/windef/ns-windef-rect)，指定要滾動的工作區部分。 如果此參數為 Null，則會滾動整個工作區。

*lpRectClip*<br/>
指向指定要滾動之裁剪矩形的 `RECT` 結構。 這個結構優先于*lpRectScroll*所指向的矩形。 只有這個矩形內的位會進行滾動。 這個矩形外的位不會受到影響，即使它們位於*lpRectScroll*矩形中也一樣。 如果此參數為 Null，則不會在捲軸矩形上執行任何裁剪。

*prgnUpdate*<br/>
識別已修改的區域，藉由滾動來將區域保留為無效。 此參數可能是 NULL。

*lpRectUpdate*<br/>
指向 `RECT` 結構，其會透過捲軸來接收不正確矩形界限。 此參數可能是 NULL。

*flags*<br/>
可以具有下列其中一個值：

- 當使用 SW_INVALIDATE 指定時 SW_ERASE，會將[WM_ERASEBKGND](/windows/win32/winmsg/wm-erasebkgnd)訊息傳送至視窗，以清除新的無效區域。

- SW_INVALIDATE 會使*prgnUpdate*在滾動後所識別的區域失效。

- SW_SCROLLCHILDREN 會以*dx*和*dy*中指定的圖元數*lpRectScroll* ，將所有與所指向之矩形相交的子視窗全部滾動。 Windows 會將[WM_MOVE](/windows/win32/winmsg/wm-move)訊息傳送到與*lpRectScroll*相交的所有子視窗，即使沒有移動也一樣。 當子視窗滾動時，插入號會重新置放，而游標矩形會與滾動矩形相交。
...
此函式與[ScrollWindow](/windows/win32/api/winuser/nf-winuser-scrollwindow)函式類似，還有一些額外的功能。

如果未指定[SW_INVALIDATE](/windows/win32/api/winuser/nf-winuser-scrollwindowex)和[SW_ERASE](/windows/win32/api/winuser/nf-winuser-scrollwindowex) ，`ScrollWindowEx` 成員函式不會使從外滾動的區域無效。 如果設定了其中一個旗標，`ScrollWindowEx` 會使此區域失效。 除非應用程式呼叫[UpdateWindow](/windows/win32/api/winuser/nf-winuser-updatewindow)成員函式、呼叫[RedrawWindow](/windows/win32/api/winuser/nf-winuser-redrawwindow)成員函式（指定[RDW_UPDATENOW](/windows/win32/api/winuser/nf-winuser-redrawwindow)或[RDW_ERASENOW](/windows/win32/api/winuser/nf-winuser-redrawwindow)），或從應用程式佇列抓取[WM_PAINT](/windows/win32/gdi/wm-paint)訊息，否則此區域不會更新。

如果視窗具有[WS_CLIPCHILDREN](/windows/win32/api/winuser/nf-winuser-createwindoww)樣式，則*prgnUpdate*和*lpRectUpdate*所指定的傳回區域會代表必須更新之滾動視窗的總區域，包括需要更新之子視窗中的任何區域。

如果指定了[SW_SCROLLCHILDREN](/windows/win32/api/winuser/nf-winuser-scrollwindowex)旗標，Windows 將不會在子視窗的一部分滾動時，適當地更新畫面。 位於來源矩形外之滾動子視窗的部分不會被清除，而且不會在其新的目的地中正確地重新繪製。 您可以使用[DeferWindowPos](/windows/win32/api/winuser/nf-winuser-deferwindowpos) Windows 函式來移動未完全落在*lpRectScroll*矩形內的子視窗。 如果設定 SW_SCROLLCHILDREN 旗標，且插入號矩形與捲軸矩形相交，則游標會重新置放。

所有的輸入和輸出座標（針對*lpRectScroll*、 *lpRectClip*、 *lpRectUpdate*和*prgnUpdate*）都會假設為在用戶端座標中，不論視窗是否具有 CS_OWNDC 或 CS_CLASSDC 類別樣式。 如有需要，請使用[LPtoDP](/windows/win32/api/wingdi/nf-wingdi-lptodp)和[DPtoLP](/windows/win32/api/wingdi/nf-wingdi-dptolp) Windows 函式來轉換為邏輯座標。
...
`SendChildNotifyLastMsg` 將目前訊息傳送至來源（如果它是反映的訊息）。
...
在處理訊息之前，`SendDlgItemMessage` 成員函式不會傳回。

使用 `SendDlgItemMessage` 等同于取得給定控制項的 `CWnd`*，並呼叫[SendMessage](#sendmessage)成員函式。
...
`SendMessage` 成員函式會直接呼叫視窗程式，而且必須等到該視窗程式處理訊息之後，才會傳回該進程。 這與[PostMessage](#postmessage)成員函式相反，它會將訊息放入視窗的訊息佇列中，並立即傳回。
...
如果視窗是由呼叫執行緒所建立，`SendNotifyMessage` 會呼叫視窗的視窗程式，而不會傳回，直到視窗程式處理訊息為止。 如果視窗是由不同的執行緒所建立，`SendNotifyMessage` 會將訊息傳遞至視窗程式，並立即傳回。它不會等候視窗程式完成訊息的處理。
...
`SetActiveWindow` 成員函式應該謹慎使用，因為它可讓應用程式任意接管使用中視窗和輸入焦點。 一般來說，Windows 會負責所有啟用。

##  <a name="cwndsetcapture"></a><a name="setcapture"></a>CWnd：： SetCapture

不論游標的位置為何，都會將所有後續的滑鼠輸入傳送至目前的 `CWnd` 物件。
...
當 `CWnd` 不再需要所有的滑鼠輸入時，應用程式應該呼叫[ReleaseCapture](/windows/win32/api/winuser/nf-winuser-releasecapture)函數，讓其他視窗可以接收滑鼠輸入。

捕捉到滑鼠輸入時，不會有任何 WM_NCHITTEST 或 WM_SETCURSOR 訊息傳送到使用中視窗。
...
`SetCaretPos` 成員函式只有在目前工作中的視窗擁有時，才會移動插入號。 無論插入號是否隱藏，`SetCaretPos` 都會移動插入號。
...
每當剪貼簿的內容變更時，就會將此視窗加入通知的視窗鏈（藉由 WM_DRAWCLIPBOARD 訊息）。
...
屬於剪貼簿檢視器鏈一部分的視窗必須回應[WM_DRAWCLIPBOARD](#ondrawclipboard)、 [WM_CHANGECBCHAIN](#onchangecbchain)和[WM_DESTROY](#ondestroy)訊息，並將訊息傳遞至鏈中的下一個視窗。
...
*bSigned*<br/>
指定整數值是否為帶正負號或不帶正負號。 如果此參數為 TRUE，則*n 值*會經過簽署。 如果此參數為 TRUE 且*n 值*小於0，則負號會放在字串中的第一個數位之前。 如果此參數為 FALSE，則表示*n 值*不帶正負號。
...
`SetFocus` 成員函式會將[WM_KILLFOCUS](/windows/win32/inputdev/wm-killfocus)訊息傳送至失去輸入焦點的視窗，並將[WM_SETFOCUS](/windows/win32/inputdev/wm-setfocus)訊息傳送至接收輸入焦點的視窗。 它也會啟用視窗或其父系。

如果目前的視窗作用中，但沒有焦點（也就是沒有任何視窗具有焦點），則按下的任何按鍵都會產生[WM_SYSCHAR](#onsyschar)、 [WM_SYSKEYDOWN](#onsyskeydown)或[WM_SYSKEYUP](#onsyskeyup)的訊息。
...
除非視窗處理 WM_SETFONT 訊息，否則此方法不會有任何作用。 許多衍生自 `CWnd` 的 MFC 類別會處理此訊息，因為它們附加至預先定義的視窗類別，其中包含 WM_SETFONT 訊息的訊息處理常式。 若要使用這個方法，您從 `CWnd` 衍生的類別必須定義 WM_SETFONT 訊息的方法處理常式。
...
##  <a name="cwndsetlayeredwindowattributes"></a><a name="setlayeredwindowattributes"></a>CWnd：： Setlayeredwindowattributes 簡單
...
*bAlpha*<br/>
用來描述分層視窗之不透明度的 Alpha 值。 如需詳細資訊，請參閱[BLENDFUNCTION](/windows/win32/api/wingdi/ns-wingdi-blendfunction)結構的 `SourceConstantAlpha` 成員。 當*bAlpha*為0時，視窗會完全透明。 當*bAlpha*為255時，視窗是不透明的。

*dwFlags*<br/>
指定要採取的動作。 這個參數可以是下列一個或多個值。 如需可能值的清單，請參閱[setlayeredwindowattributes 簡單](/windows/win32/api/winuser/nf-winuser-setlayeredwindowattributes)。
...
此成員函式會模擬函數[setlayeredwindowattributes 簡單](/windows/win32/api/winuser/nf-winuser-setlayeredwindowattributes)的功能，如 Windows SDK 中所述。
...
在與視窗階層無關的視窗物件之間建立連接通常會很有用。 例如， [CToolBar](../../mfc/reference/ctoolbar-class.md)會將通知傳送給其擁有者，而不是傳送至其父系。 這可讓工具列成為一個視窗（例如 OLE 容器應用程式視窗）的子系，同時將通知傳送至另一個視窗（例如就地框架視窗）。 此外，當您在就地編輯期間停用或啟用伺服器視窗時，框架視窗所擁有的任何視窗都會隱藏或顯示。 此擁有權是透過呼叫 `SetOwner`進行明確設定。
...
> [!NOTE]
> 此函式只能在代表 OLE 控制項的 `CWnd` 物件上呼叫。

如需搭配使用這個成員函式與 OLE 控制項容器的詳細資訊，請參閱[Activex 控制項容器：在 Activex 控制項容器中程式設計 Activex 控制項一](../../mfc/programming-activex-controls-in-a-activex-control-container.md)文。
...
- SB_CTL 包含捲軸控制項的參數。 `m_hWnd` 的資料成員必須是捲軸控制項的控制碼。
...
指出捲軸位置[CWnd：： OnHScroll](#onhscroll)和[CWnd：： OnVScroll](#onvscroll)的 MFC Windows 訊息處理常式，只提供16個位的位置資料。 [GetScrollInfo](#getscrollinfo)和 `SetScrollInfo` 提供32位的捲軸位置資料。 因此，應用程式可以在處理 `CWnd::OnHScroll` 或 `CWnd::OnVScroll` 時呼叫 `GetScrollInfo`，以取得32位的捲軸位置資料。
...
如果 `SetScrollRange` 的呼叫緊接在呼叫[SetScrollPos](#setscrollpos)成員函式之後，`SetScrollPos` 成員函式中的*bRedraw*參數應為0，以防止捲軸繪製兩次。

標準捲軸的預設範圍為0到100。 捲軸控制項的預設範圍是空的（ *nMinPos*和*nMaxPos*值都是0）。 *NMinPos*和*nMaxPos*所指定的值之間的差異，不得大於 INT_MAX。
...
*nIDEvent*<br/>
指定非零的計時器識別碼。 如果計時器識別碼是唯一的，則 `SetTimer`會傳回這個相同的值。 否則，`SetTimer` 會判斷新的唯一值，並傳回它。 對於視窗計時器（具有 Null 回呼函式），只有與目前視窗相關聯的其他 windows 計時器值才必須是唯一的。 針對回呼計時器，所有進程中的所有計時器的值都必須是唯一的。 因此，當您建立回呼計時器時，傳回的值可能會與您指定的值不同。

*nElapse*<br/>
指定超時值或間隔（以毫秒為單位）。

*lpfnTimer*<br/>
指定處理[WM_TIMER](/windows/win32/winmsg/wm-timer)訊息之應用程式提供的 `TimerProc` 回呼函數的位址。 如果此參數為 Null，WM_TIMER 的訊息會放在應用程式的訊息佇列中，並由 `CWnd` 物件處理。
...
系統會指定間隔值，而且每次間隔結束時，系統會將 WM_TIMER 訊息張貼到安裝應用程式的安裝訊息佇列，或將訊息傳遞至應用程式定義的 `TimerProc` 回呼函數。
...
這個範例會使用 `CWnd::SetTimer`、`CWnd::OnTimer`和 `CWnd::KillTimer` 來處理 WM_TIMER 訊息。 第一個計時器的設定是在 `OnStartTimer`中每隔2秒，將 WM_TIMER 訊息傳送到主框架視窗。 `OnTimer` 事件處理常式會處理主框架視窗的 WM_TIMER 訊息。 此方法會使電腦說話者每2秒發出嗶聲。 第二個計時器每隔3.75 秒就會將訊息傳送至回呼函式。 `OnStopTimer` 會藉由呼叫每個計時器識別碼的 `CWnd::KillTimer` 來停止這兩個計時器。
...
*lpwndpl*<br/>
指向指定新顯示狀態和位置的[WINDOWPLACEMENT](/windows/win32/api/winuser/ns-winuser-windowplacement)結構。
...
*pWndInsertAfter*<br/>
識別將在迭置順序中（高於）這個 `CWnd` 物件的 `CWnd` 物件。 這個參數可以是 `CWnd` 的指標，或是下列其中一個值的指標：

- `wndBottom` 會將視窗放在迭置順序的底部。 如果此 `CWnd` 是最上層的視窗，視窗會失去其最上層狀態;系統會將視窗放在所有其他視窗的底部。

- `wndTop` 會將視窗放在迭置順序的頂端。

- `wndTopMost` 會將視窗放在所有非最上層視窗的上方。 視窗會維持其最上層的位置，即使它已停用也一樣。

- `wndNoTopMost` 會將視窗重新置放至所有非最上層視窗的頂端（也就是在所有最上層視窗後方）。 如果視窗已經是非最上層的視窗，這個旗標沒有任何作用。
...
- SWP_NOCOPYBITS 捨棄工作區的整個內容。 如果未指定此旗標，則會在視窗大小或重新置放後，將工作區的有效內容儲存並複製回工作區。
...
藉由將*pWndInsertAfter*參數設定為 **& wndTopMost** ，並確保不會設定 SWP_NOZORDER 旗標，或設定視窗的迭置順序，使其位於任何現有的最上層視窗上方，可以將視窗移到迭置順序的頂端。 當非最上層的視窗設為最上層時，其擁有的視窗也會成為最上層。 其擁有者不會變更。

如果最上方的視窗重新置放至迭置順序的底部（ **& wndBottom**），或在任何非最上層視窗之後，就不再是最上層。 當最上層的視窗設為非最上層時，其所有擁有者及其擁有的視窗也會成為非最上層視窗。

如果未指定 SWP_NOACTI加值稅E 或 SWP_NOZORDER （也就是當應用程式要求同時啟用視窗並以指定的迭置順序）時，在下列情況下只會使用*pWndInsertAfter*中指定的值：
...
若要建立最上層視窗，請呼叫 `SetWindowPos`，並將*pWndInsertAfter*參數等於 **& wndTopMost**，或在建立視窗時設定 WS_EX_TOPMOST 樣式。

如果迭置順序包含任何具有 WS_EX_TOPMOST 樣式的視窗，則以 **& wndTopMost**值移動的視窗會放在所有非最上層視窗的頂端，但在任何最上層視窗底下。 當應用程式啟動沒有 WS_EX_TOPMOST 位的非作用中視窗時，視窗會移至所有非最上層視窗上方，但在任何最上層視窗的上方。

如果在*pWndInsertAfter*參數是 **& wndBottom**時呼叫 `SetWindowPos`，而 `CWnd` 是最上層的視窗，則視窗會失去其最上層狀態（WS_EX_TOPMOST 已清除），系統會將視窗放在迭置順序的底部。
...
成功呼叫 `SetWindowRgn`之後，作業系統會擁有區域控制碼*hRgn*所指定的區域。 作業系統不會建立區域的複本，因此不會對此區域控制碼進行任何進一步的函式呼叫，也不會關閉此區域控制碼。
...
此函式會將[WM_SETTEXT](/windows/win32/winmsg/wm-settext)訊息傳送到這個視窗。
...
`ShowCaret` 成員函式只有在有目前的圖形，且未連續重複出現兩次或多次時，才會顯示插入號。 如果插入號不是此視窗的擁有者，則不會顯示插入號。

隱藏插入號是累計的。 如果已多次呼叫[HideCaret](#hidecaret)成員函式，`ShowCaret` 必須呼叫五次，才能顯示插入號。
...
應用程式不應呼叫 `ShowScrollBar` 在處理捲軸通知訊息時隱藏捲軸。
...
*nCmdShow*<br/>
指定要如何顯示 `CWnd`。 它必須是下列其中一個值：

- SW_HIDE 隱藏此視窗，並將啟用傳遞至另一個視窗。

- SW_MINIMIZE 會將視窗最小化，並啟動系統清單中的最上層視窗。

- SW_RESTORE 會啟用並顯示視窗。 如果視窗最小化或最大化，Windows 就會將它還原成原始大小和位置。

- SW_SHOW 會啟動視窗，並以其目前的大小和位置顯示。

- SW_SHOWMAXIMIZED 會啟動視窗，並將它顯示為最大化的視窗。

- SW_SHOWMINIMIZED 會啟動視窗，並將它顯示為圖示。

- SW_SHOWMINNOACTIVE 會將視窗顯示為圖示。 目前作用中的視窗仍為使用中狀態。

- SW_SHOWNA 會以目前的狀態顯示視窗。 目前作用中的視窗仍為使用中狀態。

- SW_SHOWNOACTI加值稅E 會在視窗中顯示最新的大小和位置。 目前作用中的視窗仍為使用中狀態。

- SW_SHOWNORMAL 會啟用並顯示視窗。 如果視窗最小化或最大化，Windows 就會將它還原成原始大小和位置。

### <a name="return-value"></a>傳回值

如果視窗先前為可見，則為非零;如果先前已隱藏 `CWnd`，則為0。

### <a name="remarks"></a>備註

針對具有[CWinApp：： m_nCmdShow](../../mfc/reference/cwinapp-class.md#m_ncmdshow)的主視窗，每個應用程式都只能呼叫一次 `ShowWindow`。 `ShowWindow` 的後續呼叫必須使用上述其中一個值，而不是由 `CWinApp::m_nCmdShow`所指定的其中一個值。
...
呼叫這個成員函式以「動態子類別化」從對話方塊範本建立的控制項，並將它附加至這個 `CWnd` 物件。
...
當控制項動態子類別化時，windows 訊息會透過 `CWnd`的訊息對應來路由傳送，並先在 `CWnd`的類別中呼叫訊息處理常式。 傳遞至基類的訊息會傳遞至控制項中的預設訊息處理常式。

此成員函式會將 Windows 控制項附加至 `CWnd` 物件，並取代控制項的 `WndProc` 和 `AfxWndProc` 函數。 函式會將舊的 `WndProc` 儲存在 `GetSuperWndProcAddr` 成員函式所傳回的位置。

### <a name="example"></a>範例

[!code-cpp[NVC_MFCWindowing#122](../../mfc/reference/codesnippet/cpp/cwnd-class_63.cpp)]

##  <a name="cwndsubclasswindow"></a><a name="subclasswindow"></a>CWnd：： Subclasswindow 前允許

呼叫這個成員函式以「動態子類別化」視窗，並將它附加到這個 `CWnd` 物件。
...
當視窗動態子類別化時，windows 訊息會透過 `CWnd`的訊息對應來路由傳送，並先在 `CWnd`的類別中呼叫訊息處理常式。 傳遞至基類的訊息會傳遞至視窗中的預設訊息處理常式。

此成員函式會將 Windows 控制項附加至 `CWnd` 物件，並取代視窗的 `WndProc` 和 `AfxWndProc` 函數。 函式會將舊 `WndProc` 的指標儲存在 `CWnd` 物件中。
...
呼叫這個成員函式可解除鎖定已 `CWnd::LockWindowUpdate`的視窗。
...
一次只能使用 `LockWindowUpdate`來鎖定一個視窗。 如需有關鎖定視窗的詳細資訊，請參閱[CWnd：： LockWindowUpdate](#lockwindowupdate)或 Win32 函數[LockWindowUpdate](/windows/win32/api/winuser/nf-winuser-lockwindowupdate) 。

##  <a name="cwndunsubclasswindow"></a><a name="unsubclasswindow"></a>CWnd：： UnsubclassWindow

呼叫這個成員函式，將 `WndProc` 設定回其原始值，並將 HWND 所識別的視窗從 `CWnd` 物件中卸離。
...
在預設的[CDialog：： OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog)執行中建立強制回應對話方塊時，架構會自動呼叫*BSAVEANDVALIDATE*設定為 FALSE 的 `UpdateData`。 呼叫會在對話方塊可見前發生。 [CDialog：： OnOK](../../mfc/reference/cdialog-class.md#onok)的預設實值會呼叫這個成員函式，並將*BSAVEANDVALIDATE*設為 TRUE 來抓取資料，如果成功，將會關閉對話方塊。 （如果在對話方塊中按一下 [取消] 按鈕，則會關閉對話方塊，而不會抓取資料）。

##  <a name="cwndupdatedialogcontrols"></a><a name="updatedialogcontrols"></a>CWnd：： UpdateDialogControls

呼叫這個成員函式，以更新使用[ON_UPDATE_COMMAND_UI](message-map-macros-mfc.md#on_update_command_ui)回呼機制之對話方塊或視窗中對話方塊按鈕和其他控制項的狀態。
...
*pblend*<br/>
[BLENDFUNCTION](/windows/win32/api/wingdi/ns-wingdi-blendfunction)結構的指標，指定撰寫分層視窗時要使用的透明度值。

*dwFlags*<br/>
指定要採取的動作。 這個參數可以是下列一個或多個值。 如需可能值的清單，請參閱[UpdateLayeredWindow](/windows/win32/api/winuser/nf-winuser-updatelayeredwindow)。
...
此成員函式會模擬函數[UpdateLayeredWindow](/windows/win32/api/winuser/nf-winuser-updatelayeredwindow)的功能，如 Windows SDK 中所述。
...
[BeginPaint](#beginpaint)成員函式會自動驗證整個工作區。 如果必須先驗證更新區域的一部分，才能[WM_PAINT](/windows/win32/gdi/wm-paint)下一次產生，則不應呼叫 `ValidateRect` 或[ValidateRgn](#validatergn)成員函式。
...
[BeginPaint](#beginpaint)成員函式會自動驗證整個工作區。 如果必須在產生下一個[WM_PAINT](/windows/win32/gdi/wm-paint)訊息之前驗證更新區域的一部分，則不應呼叫[ValidateRect](#validaterect)或 `ValidateRgn` 成員函式。
...
*此處*<br/>
指定[CPoint](../../atl-mfc-shared/reference/cpoint-class.md)物件或[點](/windows/win32/api/windef/ns-windef-point)資料結構，其定義要檢查的點。
...
*nCmd*<br/>
指定要求的說明類型。 如需可能值的清單，以及它們對*dwData*參數的影響，請參閱 Windows SDK 中的[WinHelp](/windows/win32/api/winuser/nf-winuser-winhelpw) Windows 函式。
