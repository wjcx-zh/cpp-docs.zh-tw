...
MFC 資料庫 （或 DAO） 支援從動態連結至 MFC 之標準 MFC DLL，會呼叫這個函式增益程式標準的 MFC DLL`CWinApp::InitInstance`函式來初始化 MFC 資料庫 DLL。  
...
### <a name="remarks"></a>備註  
 ...
 叫用之前`AFX_ODBC_CALL`，您必須宣告一個變數， `nRetCode`，RETCODE 型別。  
  
 請注意 MFC ODBC 類別現在使用只同步處理。 為了執行非同步作業，您必須呼叫 ODBC API 函式`SQLSetConnectOption`。 如需詳細資訊，請參閱 「 非同步執行函式 」 Windows SDK 中的主題。  

  
### <a name="example"></a>範例  
 這個範例會使用`AFX_ODBC_CALL`呼叫`SQLColumns`ODBC API 函式，它會傳回名為資料表中的資料行清單`strTableName`。 請注意的宣告`nRetCode`以及將參數傳遞給函式的資料錄集的資料成員使用。 此範例也說明檢查與呼叫的結果`Check`，類別的成員函式`CRecordset`。 變數`prs`是指向`CRecordset`宣告其他位置的物件。  
...
### <a name="remarks"></a>備註  
 `AFX_SQL_ASYNC` 只會呼叫巨集[AFX_ODBC_CALL](#afx_odbc_call)並忽略*pr*參數。 在 MFC 4.2 之前的版本中，`AFX_SQL_ASYNC` 是用來呼叫可能傳回 `SQL_STILL_EXECUTING` 的 ODBC API 函式。 如果 ODBC API 函式確實傳回 `SQL_STILL_EXECUTING`，則 `AFX_SQL_ASYNC` 會呼叫 `prs->OnWaitForDataSource`。  
...
### <a name="remarks"></a>備註  
 ...
 然後再呼叫`AFX_SQL_SYNC`，您必須宣告一個變數， `nRetCode`，RETCODE 型別。 您可以檢查的值`nRetCode`巨集呼叫之後。  
