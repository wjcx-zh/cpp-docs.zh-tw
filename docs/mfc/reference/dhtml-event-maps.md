...
## <a name="dhtml-event-map-macros"></a>DHTML 事件對應巨集  
 下列巨集可以用來處理在 DHTML 事件[CDHtmlDialog](../../mfc/reference/cdhtmldialog-class.md)-衍生的類別。  
...
|[DHTML_EVENT](#dhtml_event)|用來處理在單一的 HTML 項目的文件層級事件。|  
...
## <a name="url-event-map-macros"></a>URL 事件對應巨集  
 下列巨集可以用來處理在 DHTML 事件[CMultiPageDHtmlDialog](../../mfc/reference/cmultipagedhtmldialog-class.md)-衍生的類別。  
...
|[BEGIN_URL_ENTRIES](#begin_url_entries)|標示 URL 事件項目對應的開始。|  
...
|[END_URL_ENTRIES](#end_url_entries)|標示 URL 事件項目對應的結尾。|  
...
##  <a name="begin_dhtml_event_map"></a>BEGIN_DHTML_EVENT_MAP  
 將標示放在所識別的類別的原始程式檔中時，才會進行 DHTML 事件對應的開頭`className`。  
...
### <a name="remarks"></a>備註  
 這個巨集的定義中使用的[CDHtmlDialog](../../mfc/reference/cdhtmldialog-class.md)-衍生的類別。  
  
 使用[BEGIN_DHTML_EVENT_MAP](#begin_dhtml_event_map)或[BEGIN_DHTML_EVENT_MAP_INLINE](#begin_dhtml_event_map_inline)實作對應。  
  
 [DECLARE_DHTML_EVENT_MAP](#declare_dhtml_event_map)下列函式宣告為：  
...
##  <a name="dhtml_event_onchange"></a>DHTML_EVENT_ONCHANGE  
 （在項目層級） 會處理`onchange`事件。 這是 nonbubbling 事件。  
...
##  <a name="begin_dhtml_url_event_map"></a>BEGIN_DHTML_URL_EVENT_MAP  
 多頁對話方塊中，會啟動 DHTML 和 URL 事件對應的定義。  
 ...
 *mapName*  
 指定的事件對應的頁面。 這符合*mapName*中[URL_EVENT_ENTRY](#url_event_entry)實際定義的 URL 或 HTML 資源的巨集。  
...
### <a name="remarks"></a>備註  
 這個巨集的定義中使用的[CMultiPageDHtmlDialog](../../mfc/reference/cmultipagedhtmldialog-class.md)-衍生的類別。  
  
 DHTML 和 URL 事件對應包含[內嵌 DHTML 事件對應](#begin_embed_dhtml_event_map)和[URL 事件項目](#begin_url_entries)DHTML 事件對應至每個頁面為基礎的處理常式。 使用[BEGIN_DHTML_URL_EVENT_MAP](#begin_dhtml_url_event_map)實作對應。  
 ...
 *mapName*  
 指定的網頁的 URL 是*url*。 這符合*mapName*中[BEGIN_EMBED_DHTML_EVENT_MAP](#begin_embed_dhtml_event_map)巨集，對應從這個頁面的事件。  
