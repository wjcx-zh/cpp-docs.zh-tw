...
在您的執行檔案中，所有對的呼叫 **`new`** 都會儲存發生記憶體配置的檔案名和行號。 函數[CMemoryState：:D umpallobjectssince](cmemorystate-structure.md#dumpallobjectssince)會顯示此額外資訊，讓您能夠識別記憶體流失。 如需診斷輸出的詳細資訊，請參閱類別[CDumpCoNtext](../../mfc/reference/cdumpcontext-class.md) 。
...
|||
|-|-|
|[判斷提示](#assert)|如果指定的運算式在程式庫的偵錯版本中評估為 FALSE，則會列印訊息，再中止程式。|
...
|[檢驗](#verify)|類似於 ASSERT，但不僅會評估此程式庫之發行版本中的運算式，也會評估偵錯版本中的運算式。|

### <a name="mfc-general-diagnostic-variables-and-functions"></a>MFC 一般診斷變數和函式

|||
|-|-|
|[afxDump](#afxdump)|全域變數，可將[CDumpCoNtext](../../mfc/reference/cdumpcontext-class.md)資訊傳送至偵錯工具輸出視窗或 debug 終端機。|
...
|[AfxDoForAllObjects](#afxdoforallobjects)|在以配置的所有衍生物件上執行指定的函 `CObject` 式 **`new`** 。|
...
呼叫這個函式會 `AfxDebugBreak` 在執行 MFC 應用程式的 debug 版本時，造成中斷（在呼叫的位置）。
...
## <a name="assert"></a><a name="assert"></a>判斷提示
...
*Pobject*參數應該是物件的指標，而且可以是 **`const`** 。 指向的物件，且類別必須支援 `CObject` 執行時間類別資訊。 例如，為了確保 `pDocument` 是類別的物件指標 `CMyDoc` 或其任何衍生，您可以撰寫程式碼：
...
*pObject*<br/>
指定衍生自的類別物件 `CObject` ，其具有成員函式的覆寫版本 `AssertValid` 。

### <a name="remarks"></a>備註

ASSERT_VALID 會呼叫當做 `AssertValid` 其引數傳遞之物件的成員函式。

在 MFC 的發行版本中，ASSERT_VALID 不會執行任何操作。 在 Debug 版本中，它會驗證指標、針對 Null 進行檢查，以及呼叫物件本身的 `AssertValid` 成員函式。 如果其中任何一項測試失敗，則會以與[ASSERT](#assert)相同的方式來顯示警示訊息。
...
一旦您插入這個指示詞，預處理器就會在您使用的地方插入 DEBUG_NEW **`new`** ，而 MFC 則會執行其餘工作。 當您編譯器的發行版本時，DEBUG_NEW 會解析為簡單的作業 **`new`** ，而且不會產生檔案名和行號資訊。
...
DEBUG_ONLY 宏相當於使用和括*expression*住的 `#ifdef _DEBUG` 運算式 `#endif` 。
...
## <a name="verify"></a><a name="verify"></a>檢驗
...
`afxDump`是預先定義的[CDumpCoNtext](../../mfc/reference/cdumpcontext-class.md)物件，可讓您將 `CDumpContext` 資訊傳送至偵錯工具的 [輸出] 視窗或 [debug] 終端機。 一般來說，您會將當做 `afxDump` 參數提供給 `CObject::Dump` 。

在 Windows NT 和所有版本的 Windows 中， `afxDump` 當您在偵錯工具時，輸出會傳送至 Visual C++ 的 [輸出-debug] 視窗。

這個變數只會在 MFC 的 Debug 版本中定義。 如需的詳細資訊 `afxDump` ，請參閱[調試 MFC 應用程式](/visualstudio/debugger/mfc-debugging-techniques)。
...
`AfxDump`呼叫物件的成員函式 `Dump` ，並將資訊傳送至變數所指定的位置 `afxDump` 。 `AfxDump`只有在 MFC 的 Debug 版本中才可使用。

您的程式碼不應呼叫 `AfxDump` ，而應改為呼叫 `Dump` 適當物件的成員函式。
...
`afxMemDF`可以具有列舉所指定的下列值 `afxMemDF` ：
...
系統會檢查堆積上目前配置的所有記憶體區塊，包括配置的配置， **`new`** 而不是直接呼叫基礎記憶體配置器（例如**malloc**函數或 Windows 函式）所配置的區塊 `GlobalAlloc` 。 如果發現任何區塊損毀，則會將訊息列印至偵錯工具輸出。

如果您包含這一行

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

在程式模組中，後續的呼叫會 `AfxCheckMemory` 顯示配置給記憶體的檔案名和行號。

> [!NOTE]
> 如果您的模組包含一或多個可序列化類別的執行，則您必須將該行放在 `#define` 最後一個 IMPLEMENT_SERIAL 宏呼叫之後。
...
- AFX_STACK_DUMP_TARGET_ODS 藉由 Win32 函式，將輸出直接傳送至偵錯工具 `OutputDebugString()` 。 當偵錯工具附加至進程時，這個選項會在 debug 和 release 組建中產生偵錯工具輸出。 AFX_STACK_DUMP_TARGET_ODS 一律會到達偵錯工具（如果已附加），而且無法重新導向。

### <a name="remarks"></a>備註

下列範例反映從 `AfxDumpStack` MFC 對話方塊應用程式中的按鈕處理常式呼叫所產生的單一輸出行：
...
|`+ 181 bytes`|從函數原型的位址（在此案例中為）到傳回位址的位移（以位元組為單位）（在此案例中 `void AfxDumpStack(unsigned long)` 為 `00427D55` ）。|
...
- 檔案 IMAGEHLP.DLL 必須位於您的路徑上。 如果您沒有此 DLL，函式會顯示錯誤訊息。 如需 IMAGEHLP.DLL 所提供之函數集的詳細資訊，請參閱[Image Help Library](/windows/win32/Debug/image-help-library) 。
...
如需的詳細資訊 `AfxEnableMemoryTracking` ，請參閱[調試 MFC 應用程式](/visualstudio/debugger/mfc-debugging-techniques)。
...
測試記憶體位址，以確定它代表的診斷版本所配置的目前使用中記憶體區塊 **`new`** 。
...
它也會根據原始配置的大小來檢查指定的大小。 如果函式傳回非零值，則會在*plRequestNumber*中傳回配置序號。 此數位代表配置區塊相對於所有其他配置的順序 **`new`** 。
...
針對應用程式記憶體空間中的所有可序列化衍生類別呼叫指定的反復專案函 `CObject` 式。
...
*pfn*<br/>
指向要針對每個類別呼叫的反復專案函式。 函式引數是物件的指標 `CRuntimeClass` ，以及呼叫端提供給函式之額外資料的 void 指標。
...
Serializable `CObject` 衍生類別是使用 DECLARE_SERIAL 宏衍生的類別。 `AfxDoForAllClasses`在*pCoNtext*中傳遞至的指標，會在每次呼叫時傳遞至指定的反復專案函式。
...
針對衍生自且已配置的所有物件，執行指定的反復專案函式 `CObject` **`new`** 。
...
不列舉 Stack、全域或内嵌物件。 `AfxDoForAllObjects`在*pCoNtext*中傳遞至的指標，會在每次呼叫時傳遞至指定的反復專案函式。
