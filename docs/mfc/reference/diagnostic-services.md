...
這些巨集和函式都可以在 MFC 的偵錯和發行版本中，供所有衍生自 `CObject` 的類別使用。 不過, 除了 DEBUG_NEW 和 VERIFY 以外的所有內容都不會在發行版本中執行任何動作。
...
|||
|-|-|
|[ASSERT](#assert)|如果指定的運算式在程式庫的偵錯工具版本中評估為 FALSE, 則會列印訊息, 然後中止程式。|
...
|[DEBUG_ONLY](#debug_only)|類似于 ASSERT, 但不會測試運算式的值;適用于只能在 Debug 模式中執行的程式碼。|
|[確保和 ENSURE_VALID](#ensure)|使用來驗證資料正確性。|
|[THIS_FILE](#this_file)|展開為要編譯的檔案名。|
|[TRACE](#trace)|提供此程式庫之偵錯版本中類似 `printf`的功能。|
|[VERIFY](#verify)|類似于判斷提示, 但會評估程式庫發行版本和 Debug 版本中的運算式。|
...
|[AfxCheckError](#afxcheckerror)|全域變數, 用來測試傳遞的 SCODE 以查看其是否為錯誤; 如果是, 則會擲回適當的錯誤。|
...
呼叫這個函式會在執行 MFC 應用程式的 debug 版本時, `AfxDebugBreak`造成中斷 (在呼叫的位置)。
...
`AfxDebugBreak`在 MFC 應用程式的發行版本中不會有任何作用, 應予以移除。 此函式應該只在 MFC 應用程式中使用。 使用 WIN32 API 版本, `DebugBreak`在非 MFC 應用程式中造成中斷。
...
如果結果為 0, 宏會列印診斷訊息, 並中止程式。 如果條件為非零, 則不會執行任何操作。
...
*Pobject*參數應該是物件的指標, 而且可以是**const**。 指向的物件, 且類別必須支援`CObject`執行時間類別資訊。 例如, 為了確保`pDocument`是`CMyDoc`類別的物件指標或其任何衍生, 您可以撰寫程式碼:

[!code-cpp[NVC_MFCDocView#194](../../mfc/codesnippet/cpp/diagnostic-services_3.cpp)]

`ASSERT_KINDOF`使用宏與撰寫程式碼完全相同:

[!code-cpp[NVC_MFCDocView#195](../../mfc/codesnippet/cpp/diagnostic-services_4.cpp)]

此函式僅適用于使用 [DECLARE_DYNAMIC] (執行時間------------------DECLARE_DYNAMIC 或[DECLARE_SERIAL](run-time-object-model-services.md#declare_serial)宏所宣告的類別。
...
*pObject*<br/>
指定衍生自`CObject`的類別物件, 其具有`AssertValid`成員函式的覆寫版本。

### <a name="remarks"></a>備註

ASSERT_VALID 會呼叫`AssertValid`當做其引數傳遞之物件的成員函式。

在 MFC 的發行版本中, ASSERT_VALID 不會執行任何操作。 在 Debug 版本中, 它會驗證指標、針對 Null 進行檢查, 以及呼叫物件本身`AssertValid`的成員函式。 如果其中任何一項測試失敗, 則會以與[ASSERT](#assert)相同的方式來顯示警示訊息。
...
您可以在程式中使用 DEBUG_NEW 的所有位置, 通常會使用**NEW**運算子來配置堆積儲存區。

在 [偵錯工具] 模式中 (定義 **_debug**符號時), DEBUG_NEW 會針對它所配置的每個物件, 持續追蹤檔案名和行號。 然後, 當您使用[CMemoryState::D umpallobjectssince](cmemorystate-structure.md#dumpallobjectssince)成員函式時, 每個以 DEBUG_NEW 配置的物件都會顯示其配置所在的檔案名和行號。

若要使用 DEBUG_NEW, 請將下列指示詞插入至原始程式檔:

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

一旦您插入此指示詞, 預處理器就會在您使用**NEW**的位置插入 DEBUG_NEW, 而 MFC 則會執行其餘部分。 當您編譯器的發行版本時, DEBUG_NEW 會解析為簡單的**新**作業, 而且不會產生檔案名和行號資訊。

> [!NOTE]
>  在舊版 MFC (4.1 和更早版本) 中, 您需要將`#define`語句放在所有呼叫 IMPLEMENT_DYNCREATE 或 IMPLEMENT_SERIAL 宏的語句之後。 這不再是必要的。
...
DEBUG_ONLY 宏相當於使用和`#endif`括住的`#ifdef _DEBUG`運算式。
...
### <a name="ensure"></a>確保和 ENSURE_VALID
...
宏的運作方式有兩種, 視專案設定而定。 宏會呼叫 ASSERT, 然後在判斷提示失敗時擲回例外狀況。 因此, 在 Debug 設定 (也就是已定義 _DEBUG 的位置) 中, 宏會產生判斷提示和例外狀況, 而在發行設定中, 宏只會產生例外狀況 (判斷提示不會評估發行設定中的運算式)。
...
`afxDump`是預先定義的[CDumpCoNtext](../../mfc/reference/cdumpcontext-class.md)物件, 可讓您`CDumpContext`將資訊傳送至偵錯工具的 [輸出] 視窗或 [debug] 終端機。 一般來說, 您會`afxDump`將當做參數提供`CObject::Dump`給。

在 windows NT 和所有版本的 windows 中`afxDump` , 當您在偵錯工具時, 輸出會傳送C++至視覺效果的 [輸出-debug] 視窗。

這個變數只會在 MFC 的 Debug 版本中定義。 如需的詳細`afxDump`資訊, 請參閱[調試 MFC 應用程式](/visualstudio/debugger/mfc-debugging-techniques)。
...
*pOb*<br/>
衍生自之`CObject`類別物件的指標。

### <a name="remarks"></a>備註

`AfxDump`呼叫物件的`Dump`成員函式, 並將資訊傳送至`afxDump`變數所指定的位置。 `AfxDump`只有在 MFC 的 Debug 版本中才可使用。

您的程式碼不應`AfxDump`呼叫, 而應改為`Dump`呼叫適當物件的成員函式。
...
- `delayFreeMemDF`延遲釋放記憶體。 當您的程式釋放記憶體區塊時, 配置器不會將該記憶體傳回給基礎作業系統。 這會將最大記憶體壓力放在您的程式上。

- `checkAlwaysMemDF`每`AfxCheckMemory`次配置或釋放記憶體時呼叫。 這會大幅降低記憶體配置和取消配置。
...
系統會檢查堆積上目前配置的所有記憶體區塊, 包括由**新**配置, 而不是由直接呼叫基礎記憶體配置器 (例如**malloc** `GlobalAlloc`函數或 Windows 函式) 所配置的區塊。 如果發現任何區塊損毀, 則會將訊息列印至偵錯工具輸出。

如果您包含這一行

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

在程式模組中, 後續的呼叫`AfxCheckMemory`會顯示配置給記憶體的檔案名和行號。

> [!NOTE]
>  如果您的模組包含一個或多個可序列化類別的執行, 則必須`#define`將該行放在最後一個 IMPLEMENT_SERIAL 宏呼叫的後面。
...
- AFX_STACK_DUMP_TARGET_TRACE 會透過[TRACE](#trace)宏來傳送輸出。 TRACE 宏只會在 debug build 中產生輸出;它在發行組建中不會產生任何輸出。 此外, 追蹤也可以重新導向至偵錯工具以外的其他目標。

- AFX_STACK_DUMP_TARGET_DEFAULT 會將傾印輸出傳送至預設目標。 若為 debug 組建, 輸出會移至 TRACE 宏。 在發行組建中, 輸出會移至剪貼簿。

- AFX_STACK_DUMP_TARGET_CLIPBOARD 只會將輸出傳送至剪貼簿。 資料會使用 CF_TEXT 剪貼簿格式, 以純文字的形式放在剪貼簿上。

- AFX_STACK_DUMP_TARGET_BOTH 會同時將輸出傳送至剪貼簿和追蹤宏。

- AFX_STACK_DUMP_TARGET_ODS 會透過 Win32 函數`OutputDebugString()`, 將輸出直接傳送至偵錯工具。 當偵錯工具附加至進程時, 這個選項會在 debug 和 release 組建中產生偵錯工具輸出。 AFX_STACK_DUMP_TARGET_ODS 一律會到達偵錯工具 (如果已附加), 而且無法重新導向。

### <a name="remarks"></a>備註

下列範例反映從 MFC 對話方塊應用程式中的按鈕處理常式`AfxDumpStack`呼叫所產生的單一輸出行:
...
|`+ 181 bytes`|從函數原型的位址 (在此案例`void AfxDumpStack(unsigned long)`中為) 到傳回位址的位移 (以位元組為單位) (在此`00427D55`案例中為)。|
...
如需的詳細`AfxEnableMemoryTracking`資訊, 請參閱[調試 MFC 應用程式](/visualstudio/debugger/mfc-debugging-techniques)。
...
*bObject*<br/>
如果配置是針對`CObject`衍生的物件, 則為 TRUE, 否則為 FALSE。
...
針對應用程式記憶體空間中的所有`CObject`可序列化衍生類別呼叫指定的反復專案函式。
...
*pfn*<br/>
指向要針對每個類別呼叫的反復專案函式。 函式引數是`CRuntimeClass`物件的指標, 以及呼叫端提供給函式之額外資料的 void 指標。
...
Serializable `CObject`衍生類別是使用 DECLARE_SERIAL 宏衍生的類別。 在`AfxDoForAllClasses` *pCoNtext*中傳遞至的指標, 會在每次呼叫時傳遞至指定的反復專案函式。
...
針對所有衍生自`CObject`且使用**new**所配置的物件, 執行指定的反覆運算函數。
...
*pfn*<br/>
指向要針對每個物件執行的反復專案函式。 函式引數是的指標`CObject` , 以及呼叫端提供給函式之額外資料的 void 指標。
...
不列舉 Stack、全域或内嵌物件。 在 pCoNtext 中傳遞`AfxDoForAllObjects`至的指標, 會在每次呼叫時傳遞至指定的反復專案函式。
