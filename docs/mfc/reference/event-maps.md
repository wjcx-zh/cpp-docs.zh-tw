 ...
 **EVENT_STOCK_CLICK**巨集表示控制項就會引發內建 Click 事件，每次偵測到滑鼠。 如需其他內建事件更詳細的清單，請參閱文章[ActiveX 控制項︰ 事件](../../mfc/mfc-activex-controls-events.md)。 巨集也可用於表示自訂事件。  
...
### <a name="remarks"></a>備註  
 使用`DECLARE_EVENT_MAP`巨集，在類別宣告的結尾。 接著，在定義類別成員函式的.cpp 檔案，使用`BEGIN_EVENT_MAP`巨集、 巨集項目，每個控制項的事件和`END_EVENT_MAP`巨集來宣告事件清單的結尾。  
...
### <a name="remarks"></a>備註  
 在定義類別成員函式的實作 (.cpp) 檔案，啟動的事件對應`BEGIN_EVENT_MAP`巨集，然後將巨集項目加入每個事件，並完成的事件對應`END_EVENT_MAP`巨集。  
...
### <a name="remarks"></a>備註  
 `vtsParams`參數是以空格分隔的清單中的值**VTS_**常數。 一或多個以空格 （不是逗號） 分隔這些值會指定函式的參數清單。 例如:   
  
 [!code-cpp[NVC_MFCActiveXControl #&13;](../../mfc/codesnippet/cpp/event-maps_2.cpp)]  
  
 指定清單，其中包含 32 位元的整數，表示 RGB 色彩值的指標**IFontDisp** OLE 字型物件介面。  
...
### <a name="parameters"></a>參數  
 ...
 `vtsParams`  
 引發事件時，變數的參數清單會傳遞至控制項容器。  
  
### <a name="remarks"></a>備註  
 `vtsParams`引數是以空格分隔的清單中的值**VTS_**常數。 一或多個以空格，不是逗號分隔這些值會指定函式的參數清單。 例如:   
  
 [!code-cpp[NVC_MFCActiveXControl #&13;](../../mfc/codesnippet/cpp/event-maps_2.cpp)]  
  
 指定清單，其中包含 32 位元的整數，表示 RGB 色彩值的指標**IFontDisp** OLE 字型物件介面。  
  
 如需清單**VTS_**常數，請參閱[EVENT_CUSTOM](#event_custom)。  
...
### <a name="remarks"></a>備註  
 標準動詞索引格式為**OLEIVERB_**，後面接著動作。 `OLEIVERB_SHOW`、`OLEIVERB_HIDE` 和 `OLEIVERB_UIACTIVATE` 是標準動詞的一些範例。  
  
 請參閱[ON_OLEVERB](#on_oleverb)的函式原型，可用來做說明`memberFxn`參數。  
