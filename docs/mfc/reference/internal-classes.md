...
|`CFrameImpl Class`|`CFrameImpl`類別會處理下列框架視窗類別的工具列自訂： [CFrameWndEx 類別](../../mfc/reference/cframewndex-class.md)， [CMDIFrameWndEx 類別](../../mfc/reference/cmdiframewndex-class.md)，和[COleIPFrameWndEx 類別](../../mfc/reference/coleipframewndex-class.md).|  
|`CFullScreenImpl Class`|實作全螢幕通用功能 SDI 和 MDI 應用程式。|  
|`CMDIClientAreaWnd Class`|`CMDIClientAreaWnd`類別是 helper 類別，可簡化 MDI 索引標籤和 MDI 索引群組的實作。|  
|`CMemDC Class`|記憶體裝置內容的協助程式類別。 記憶體裝置內容支援螢幕的繪圖。|  
|`CMenuHash Class`|讀取的狀態`CMFCToolBar`物件從封存或寫入保存的狀態。|  
|`CMenuImages Class`|提供的功能來顯示預先定義的映像，例如 [關閉] 按鈕、 最大化按鈕、 選項按鈕和箭號。|  
|`CMFCCaptionButtonEx Class`|保存系統標題按鈕的非工作區。|  
|`CMFCCaptionMenuButton Class`|表示位於標題區域的工作窗格或工具列中的功能表按鈕。|  
|`CMFCColorPropertySheet Class`|使用[CMFCColorDialog 類別](../../mfc/reference/cmfccolordialog-class.md)提供索引標籤式的對話方塊。|  
|`CMFCControlBarImpl Class`|提供實作細節，以控制列類別，例如`CMFCReBar`和`CMFCToolBar`。|  
|`CMFCControlRenderer Class`|處理影像轉譯協助程式類別。|  
|`CMFCControlRendererInfo Class`|協助程式類別來傳遞參數至`CMFCControlRenderer`類別。|  
|`CMFCCustomizeButton Class`|支援功能表命令或工具列按鈕，啟動一個自訂對話方塊。|  
|`CMFCCustomizeMenuButton Class`|代表自訂功能表上的單一按鈕。|  
|`CMFCDropDownListBox Class`|提供下拉式清單方塊的功能與功能區下拉式方塊。|  
|`CMFCMousePropertyPage Class`|屬性頁，可讓使用者自訂滑鼠行為。|  
...
|`CMFCOutlookBarToolBar Class`|實作具有 Outlook 2003 視覺化樣式的工具列。|  
|`CMFCPropertySheetCategoryInfo Class`|代表樹狀結構控制項中的節點。|  
|`CMFCPropertySheetTabCtrl Class`|擴充`CMFCTabCtrl`類別提供屬性工作表的功能。|  
|`CMFCProperySheetListBox Class`|擴充`CListBox`類別提供屬性工作表的功能。|  
...
|`CMFCRibbonKeyboardCustomizeDialog Class`|實作包含功能區列的應用程式鍵盤自訂對話方塊。|  
|`CMFCRibbonKeyTip Class`|實作顯示在功能區列的索引鍵提示快顯視窗。|  
|`CMFCRibbonPanelMenu Class`||  
|`CMFCRibbonPanelMenuBar Class`|代表功能區列中的快顯功能表。|  
|`CMFCRibbonQuickAccessToolBar Class`|實作快速存取工具列功能區項目。|  
|`CMFCRibbonRichEditCtrl Class`|實作編輯控制項放在功能區列上。|  
|`CMFCRibbonTab Class`|代表功能區列上的索引標籤。|  
|`CMFCShadowRenderer Class`||  
|`CMFCShowAllButton Class`|表示位於展開以顯示隱藏的命令的快顯功能表底部的按鈕。|  
|`CMFCStatusBarPaneInfo Class`|描述內容和外觀`CMFCStatusBar`窗格。|  
|`CMFCTabButton Class`|提供索引標籤控制項 索引標籤按鈕功能。|  
|`CMFCTabInfo Class`|提供索引標籤索引標籤控制項的相關資訊。|  
|`CMFCTasksPaneFrameWnd Class`|管理自訂的標題按鈕的 [工作] 窗格。|  
|`CMFCTasksPanePropertyPage Class`|管理之間的關聯性`CMFCTasksPane`物件和其名稱。|  
|`CMFCTasksPaneToolBar Class`|實作位於頂端的巡覽工具列`CMFCTasksPane`物件。|  
|`CMFCToolBarButtonsListButton Class`|顯示一份可在使用者選取期間中的自訂映像**按鈕外觀** 對話方塊。|  
...
|`CMFCToolBarMenuButtonsButton Class`|表示系統標題按鈕位於右邊的功能表列。|  
|`CMFCToolBarNameDialog Class`|表示對話方塊，可讓使用者能夠指定新的工具列標題。|  
...
|`CMFCToolBarsListCheckBox Class`|顯示一份工具列上**工具列**頁面**自訂** 對話方塊。|  
|`CMFCToolBarsListPropertyPage Class`|表示屬性頁顯示的工具列中清單**自訂** 對話方塊。|  
|`CMFCToolBarsMenuPropertyPage Class`|代表包含功能表中的自訂選項的屬性頁**自訂** 對話方塊。|  
|`CMFCToolBarsOptionsPropertyPage Class`|代表**選項**頁面**自訂** 對話方塊。|  
|`CMFCToolBarSpinEditBoxButton Class`|表示位於編輯方塊的微調按鈕。|  
|`CMFCToolBarsToolsPropertyPage Class`|代表可讓使用者自訂使用者工具的屬性頁。|  
|`CMFCToolBarSystemMenuButton Class`|實作架構將加入至主功能表列，當使用者將 MDI 子視窗最大化時的系統標題按鈕。|  
|`CMFCVisualManagerBitmapCache Class`|管理`CMFCControlRenderer`物件來改善效能的點陣圖上的作業。|  
|`CMultiDocTemplateEx Class`|擴充`CMultiDocTemplate`類別來存取功能表資源識別碼。|  
...
|`CRecentPaneContainerInfo Class`|保留先前的停駐窗格的狀態。|  
...
|`CSmartDockingGroupGuidesWnd Class`|實作多層式的非矩形視窗架構用來顯示智慧停駐的輔助線的中央的群組。|  
|`CSmartDockingHighlighterWnd Class`|實作所涵蓋的區域會停駐的可停駐窗格物件所要採取的半透明視窗。|  
|`CSmartDockingManager Class`|提供智慧停駐功能`CDockingManager`物件。|  
|`CSmartDockingStandaloneGuide Class`|實作智慧停駐的指南。|  
|`CSmartDockingStandaloneGuideWnd Class`|實作多層式的非矩形視窗架構用來顯示智慧停駐的指南。|  
|`CTagManager Class`|`CTagManager`類別要讀取儲存在 XML 之類的標記格式的值。|  
