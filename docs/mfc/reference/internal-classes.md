...
ms.locfileid: "46411725"
---
# <a name="internal-classes"></a>內部類別

下列類別可在 MFC 中的內部。 為求完整起見，本節會說明這些內部類別，但是它們並不適合直接在您的程式碼中使用。
...
|`CMenuHash Class`|讀取的狀態`CMFCToolBar`從封存物件或狀態寫入封存。|
|`CMenuImages Class`|提供的功能，以顯示預先定義的映像，例如 [關閉] 按鈕、 最大化按鈕、 選項按鈕和箭號。|
|`CMFCCaptionButtonEx Class`|保存系統標題按鈕的非工作區。|
|`CMFCCaptionMenuButton Class`|代表標題區域的工作窗格或工具列中的功能表按鈕。|
|`CMFCColorPropertySheet Class`|供[CMFCColorDialog 類別](../../mfc/reference/cmfccolordialog-class.md)提供索引標籤式的對話方塊。|
|`CMFCControlBarImpl Class`|提供實作詳細資料，以控制列類別，例如`CMFCReBar`和`CMFCToolBar`。|
|`CMFCControlRenderer Class`|協助程式類別來處理映像轉譯。|
|`CMFCControlRendererInfo Class`|協助程式類別來傳遞參數至`CMFCControlRenderer`類別。|
|`CMFCCustomizeButton Class`|支援功能表命令或工具列按鈕會在自訂對話方塊。|
|`CMFCCustomizeMenuButton Class`|表示自訂功能表上的單一按鈕。|
|`CMFCDropDownListBox Class`|提供功能區下拉式方塊的下拉式清單方塊功能。|
|`CMFCMousePropertyPage Class`|可讓使用者自訂的滑鼠行為的屬性頁面。|
|`CMFCOutlookBarPaneAdapter Class`||
|`CMFCOutlookBarPaneButton Class`|實作 framework 中新增的按鈕`CMFCOutlookBarPane`物件。|
...
|`CMFCOutlookBarToolBar Class`|會實作具有 Outlook 2003 的視覺化樣式的工具列。|
...
|`CMFCRibbonCaptionButton Class`|會實作系統標題按鈕。|
|`CMFCRibbonCmdUI Class`|視需要在應用程式閒置時，更新功能區列的物件。|
...
|`CMFCTabInfo Class`|提供索引標籤 [到] 索引標籤控制項的相關資訊。|
...
|`CMFCToolBarNameDialog Class`|表示對話方塊中，可讓使用者指定新的工具列標題。|
|`CMFCToolBarsCommandsListBox Class`|表示包含一份工具列命令的清單方塊。|
|`CMFCToolBarsCommandsPropertyPage Class`|在 [屬性] 頁面上的實作命令自訂。|
|`CMFCToolBarsKeyboardPropertyPage Class`||
|`CMFCToolBarsListCheckBox Class`|會顯示一份工具列**工具列**頁**自訂** 對話方塊。|
|`CMFCToolBarsListPropertyPage Class`|表示顯示的工具列中清單的屬性頁**自訂** 對話方塊。|
|`CMFCToolBarsMenuPropertyPage Class`|表示包含功能表中的自訂選項的屬性頁**自訂** 對話方塊。|
|`CMFCToolBarsOptionsPropertyPage Class`|代表**選項**頁面**自訂** 對話方塊。|
|`CMFCToolBarSpinEditBoxButton Class`|表示位於編輯方塊的微調按鈕。|
|`CMFCToolBarsToolsPropertyPage Class`|表示屬性頁，可讓使用者自訂使用者工具。|
|`CMFCToolBarSystemMenuButton Class`|實作架構將加入至主功能表列，當使用者達到最大的 MDI 子視窗的系統標題按鈕。|
|`CMFCVisualManagerBitmapCache Class`|管理`CMFCControlRenderer`改善點陣圖上的作業效能的物件。|
|`CMultiDocTemplateEx Class`|擴充`CMultiDocTemplate`類別，以提供存取權 功能表資源識別碼。|
|`COleCntrFrameWndEx Class`||
|`COleDocIPFrameWndEx Class`|提供 OLE 支援。|
|`COleServerDocEx Class`|提供 OLE 伺服器文件上的作業。|
|`CPaneContainerGC Class`||
|`CPngImage Class`|提供使用.png 檔案格式的映像資源的存取權。|
|`CPreviewViewEx Class`||
|`CRecentPaneContainerInfo Class`|保留先前的停駐窗格的狀態。|
|`CRibbonCategoryScroll Class`|實作時，會出現不符合的功能區列區域中的所有項目中的捲軸按鈕。|
|`CSmartDockingGroupGuide Class`|表示智慧停駐的群組的中央的項目。|
|`CSmartDockingGroupGuidesManager Class`|管理智慧停駐群組輔助線。|
|`CSmartDockingGroupGuidesWnd Class`|實作多層式的非矩形視窗，架構會顯示智慧停駐輔助線的中央的群組。|
|`CSmartDockingHighlighterWnd Class`|實作半透明的視窗所涵蓋的區域，由所停駐的可停駐窗格物件。|
|`CSmartDockingManager Class`|提供智慧停駐功能`CDockingManager`物件。|
|`CSmartDockingStandaloneGuide Class`|實作智慧停駐的指南。|
|`CSmartDockingStandaloneGuideWnd Class`|實作多層式的非矩形視窗，架構會顯示智慧停駐輔助線。|
|`CTagManager Class`|`CTagManager`類別要讀取以類似 XML 的標記格式儲存的值。|
|`CVSListBoxBase Class`|提供基本功能`CVSListBox`類別。|
|`CVSListBoxEditCtrl Class`|顯示開啟的對話方塊中的小按鈕，以擴充標準的清單方塊編輯控制項。|
