...
> [!NOTE]
>  在 MFC 4.2 之前的 MFC 版本中, 無法使用**int** *nRegFlags*參數。 另請注意, `afxRegInsertable`此旗標不是屬性頁的有效選項, 如果已設定, 則會在 MFC 中產生判斷提示
...
*pszHelpDir*<br/>
可以找到此類型程式庫的說明檔的目錄名稱。 如果是 Null, 說明檔就會假設與型別程式庫本身位於相同的目錄中。
