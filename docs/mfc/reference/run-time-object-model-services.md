...
|[IMPLEMENT_OLECTLTYPE](#implement_olectltype)|實 `GetUserTypeNameID` `GetMiscStatus` 作用控制項類別的和成員函式。|
|[IMPLEMENT_OLECREATE_FLAGS](#implement_olecreate_flags)|這個宏或[IMPLEMENT_OLECREATE](#implement_olecreate)必須出現在任何使用之類別的執行檔中 `DECLARE_OLECREATE` 。 |
...
您可以使用這個宏來決定通用控制項程式庫函式所*指定的函數（而*不是呼叫[GetProcAddress](/windows/win32/api/libloaderapi/nf-libloaderapi-getprocaddress)）。
...
您可以使用這個宏來決定通用控制項程式庫函式所*指定的函數（而*不是呼叫[GetProcAddress](/windows/win32/api/libloaderapi/nf-libloaderapi-getprocaddress)）。 這個宏是 Unicode 版本的 AFX_COMCTL32_IF_EXISTS。
...
如果您依照所述使用 DECLARE_ 動態和 IMPLEMENT_DYNAMIC 宏，則可以使用 RUNTIME_CLASS 宏和函式， `CObject::IsKindOf` 在執行時間判斷物件的類別。
...
在類別的 .h 模組中加入 DECLARE_DYNCREATE 宏，然後在需要存取此類別之物件的所有 .cpp 模組中包含該模組。
...
`GetUserTypeNameID`和 `GetMiscStatus` 是純虛擬函式，宣告于中 `COleControl` 。 因為這些函式是純虛擬的，所以必須在控制項類別中覆寫這些函式。 除了 DECLARE_OLECTLTYPE 以外，您還必須將 IMPLEMENT_OLECTLTYPE 宏加入至控制項類別宣告。
...
`DECLARE_PROPPAGEIDS`在類別宣告的結尾使用宏。 然後，在定義類別之成員函式的 .cpp 檔案中，使用 `BEGIN_PROPPAGEIDS` 宏、每個控制項屬性頁的宏專案，以及用 `END_PROPPAGEIDS` 來宣告屬性頁清單結尾的宏。

如需屬性頁的詳細資訊，請參閱[ActiveX 控制項：屬性頁](../mfc-activex-controls-property-pages.md)一文。
...
產生可序列化的衍生類別所需的 c + + 標頭 `CObject` 程式碼。
...
使用 .h 模組中的 DECLARE_SERIAL 宏，然後在需要存取此類別之物件的所有 .cpp 模組中包含該模組。

如果 DECLARE_SERIAL 包含在類別宣告中，則 IMPLEMENT_SERIAL 必須包含在類別實中。

DECLARE_SERIAL 宏包括 DECLARE_DYNAMIC 和 DECLARE_DYNCREATE 的所有功能。

您可以使用 AFX_API 宏，自動匯出 `CArchive` 使用 DECLARE_SERIAL 和 IMPLEMENT_SERIAL 宏之類別的抽取運算子。 使用下列程式碼括住類別宣告（位於 .h 檔案中）：
...
產生動態衍生類別所需的 c + + 程式碼， `CObject` 其具有在階層中的類別名稱和位置的執行時間存取權。
...
使用 .cpp 模組中的 IMPLEMENT_DYNAMIC 宏，然後只將產生的物件程式碼連結一次。
...
`CObject`當搭配 DECLARE_DYNCREATE 宏使用時，可讓衍生類別的物件在執行時間以動態方式建立。
...
如果您使用 DECLARE_DYNCREATE 並 IMPLEMENT_DYNCREATE 宏，則可以使用 RUNTIME_CLASS 宏和成員函式， `CObject::IsKindOf` 在執行時間判斷物件的類別。

如果 DECLARE_DYNCREATE 包含在類別宣告中，則 IMPLEMENT_DYNCREATE 必須包含在類別實中。

請注意，這個巨集定義會叫用類別的預設函式。 如果類別明確地執行非一般的函式，它也必須明確地執行預設的函式。 預設的函式可以新增至類別的 **`private`** 或 **`protected`** 成員區段，以防止它從類別實作為外部呼叫。
...
OLE 類別 ID 是物件的唯一128位識別碼。 其中包含一個 **`long`** 、兩個**單字**和8個**位元組**s，在語法*b8*描述中是以*l*、 *w1*、 *w2*和*b1*來表示。 [應用程式精靈] 和 [程式碼] 嚮導會視需要為您建立唯一的 OLE 類別 Id。
...
實 `GetUserTypeNameID` `GetMiscStatus` 作用控制項類別的和成員函式。
...
除了 IMPLEMENT_OLECTLTYPE 以外，您還必須將 DECLARE_OLECTLTYPE 宏加入至控制項類別宣告。
...
使用 .cpp 模組中的 IMPLEMENT_SERIAL 宏;然後只將產生的物件程式碼連結一次。
...
RUNTIME_CLASS 會針對*class_name*所指定的類別，傳回[CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md)結構的指標。 只有 `CObject` 使用 DECLARE_DYNAMIC、DECLARE_DYNCREATE 或 DECLARE_SERIAL 宣告的衍生類別，才會傳回結構的指標 `CRuntimeClass` 。
...
可讓 `CCmdTarget` 衍生類別的物件透過 OLE automation 建立。
...
在類別的 .h 模組中加入 DECLARE_OLECREATE 宏，然後在需要存取此類別之物件的所有 .cpp 模組中包含該模組。
...
這個宏或[IMPLEMENT_OLECREATE_FLAGS](#implement_olecreate_flags)必須出現在任何使用之類別的執行檔中 `DECLARE_OLECREATE` 。
