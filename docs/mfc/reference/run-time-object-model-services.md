...
|[DECLARE_OLECTLTYPE](#declare_olectltype)|宣告控制項類別的 `GetUserTypeNameID` 和 `GetMiscStatus` 成員函式。|
|[DECLARE_PROPPAGEIDS](#declare_proppageids)|宣告 OLE 控制項提供屬性頁清單，以顯示其屬性。|
|[IMPLEMENT_OLECREATE](#implement_olecreate)|讓物件經由 OLE 系統建立。|
|[IMPLEMENT_OLECTLTYPE](#implement_olectltype)|執行控制項類別的 `GetUserTypeNameID` 和 `GetMiscStatus` 成員函式。|
...
加入從 `CObject`衍生類別時，存取物件類別之執行時間資訊的能力。
...
如果您使用 DECLARE_ 動態和 IMPLEMENT_DYNAMIC 宏（如所述），則可以使用 RUNTIME_CLASS 宏和 `CObject::IsKindOf` 函式，在執行時間判斷物件的類別。
...
宣告控制項類別的 `GetUserTypeNameID` 和 `GetMiscStatus` 成員函式。
...
`GetUserTypeNameID` 和 `GetMiscStatus` 是純虛擬函式，在 `COleControl`中宣告。 因為這些函式是純虛擬的，所以必須在控制項類別中覆寫這些函式。 除了 DECLARE_OLECTLTYPE 以外，您還必須將 IMPLEMENT_OLECTLTYPE 宏加入至控制項類別宣告。
...
在類別宣告的結尾使用 `DECLARE_PROPPAGEIDS` 宏。 然後，在定義類別之成員函式的 .cpp 檔案中，使用 `BEGIN_PROPPAGEIDS` 宏、每個控制項屬性頁的宏專案，以及 `END_PROPPAGEIDS` 宏來宣告屬性頁清單的結尾。
...
產生可C++序列化之 `CObject`衍生類別所需的標頭程式碼。
...
您可以使用 AFX_API 宏，針對使用 DECLARE_SERIAL 和 IMPLEMENT_SERIAL 宏的類別，自動匯出 `CArchive` 的抽取運算子。 使用下列程式碼括住類別宣告（位於 .h 檔案中）：
...
當搭配 DECLARE_DYNCREATE 宏使用時，可讓 `CObject`衍生類別的物件在執行時間以動態方式建立。
...
如果您使用 DECLARE_DYNCREATE 並 IMPLEMENT_DYNCREATE 宏，則可以使用 RUNTIME_CLASS 宏和 `CObject::IsKindOf` 成員函式，在執行時間判斷物件的類別。
...
執行控制項類別的 `GetUserTypeNameID` 和 `GetMiscStatus` 成員函式。
...
`GetUserTypeNameID` 成員函式會傳回識別控制項類別的資源字串。 `GetMiscStatus` 會傳回您控制項的 OLEMISC 位。 此列舉會指定描述控制項之其他特性的設定集合。 如需 OLEMISC 設定的完整描述，請參閱 Windows SDK 中的[OLEMISC](/windows/win32/api/oleidl/ne-oleidl-olemisc) 。
...
RUNTIME_CLASS 會針對*class_name*所指定的類別，傳回[CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md)結構的指標。 只有使用 DECLARE_DYNAMIC、DECLARE_DYNCREATE 或 DECLARE_SERIAL 宣告的 `CObject`衍生類別，才會傳回 `CRuntimeClass` 結構的指標。
