...
函式會 `DDX_CBIndex` 管理 **`int`** 對話方塊、表單檢視或控制項視圖物件中下拉式方塊控制項與 **`int`** 對話方塊、表單檢視或控制項視圖物件之資料成員之間的資料傳輸。
...
函式會 `DDX_CBString` 管理 `CString` 對話方塊、表單檢視或控制項視圖物件中下拉式方塊控制項的編輯控制項與 `CString` 對話方塊、表單檢視或控制項視圖物件的資料成員之間的資料傳輸。
...
函式會 `DDX_CBStringExact` 管理 `CString` 對話方塊、表單檢視或控制項視圖物件中下拉式方塊控制項的編輯控制項與 `CString` 對話方塊、表單檢視或控制項視圖物件的資料成員之間的資料傳輸。
...
函式會 `DDX_Check` 管理 **`int`** 對話方塊、表單檢視或控制項視圖物件中核取方塊控制項與 **`int`** 對話方塊、表單檢視或控制項視圖物件之資料成員之間的資料傳輸。
...
函式會子 `DDX_Control` 類別化對話方塊、表單檢視或控制項視圖物件的*nIDC*所指定的控制項。
...
呼叫函數時，架構會提供*pDX*物件 `DoDataExchange` 。 因此， `DDX_Control` 應該只在的覆寫內呼叫 `DoDataExchange` 。
...
函式會 `DDX_DateTimeCtrl` 管理對話方塊或表單檢視物件中日期和時間選擇器控制項（ [CDateTimeCtrl](../../mfc/reference/cdatetimectrl-class.md)）與對話方塊或表單檢視物件之[CTime](../../atl-mfc-shared/reference/ctime-class.md)或[COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md)資料成員之間的日期和/或時間資料傳送。
...
*value*<br/>
在前兩個版本中，是用來 `CTime` `COleDateTime` 交換資料的或成員變數、對話方塊、表單檢視或控制項視圖物件的參考。 在第三個版本中，是 `CString` 資料成員控制項 view 物件的參考。

### <a name="remarks"></a>備註

當 `DDX_DateTimeCtrl` 呼叫時，[*值*] 會設定為 [日期和時間選擇器] 控制項的目前狀態，或 [控制項] 設定為 [*值*] （視交換的方向而定）。

在上述的第三個版本中，會 `DDX_DateTimeCtrl` 管理 `CString` 日期時間控制項與 control view 物件的[CString](../../atl-mfc-shared/reference/cstringt-class.md)資料成員之間的資料傳輸。 字串會使用目前地區設定的日期和時間格式規則來格式化。
...
函式會 `DDX_LBIndex` 管理 **`int`** 對話方塊、表單檢視或控制項視圖物件中清單方塊控制項與 **`int`** 對話方塊、表單檢視或控制項視圖物件之資料成員之間的資料傳輸。
...
函式會 `DDX_LBString` 管理 `CString` 對話方塊、表單檢視或控制項視圖物件中清單方塊控制項與 `CString` 對話方塊、表單檢視或控制項視圖物件之資料成員之間的資料傳輸。
...
`DDX_LBString`呼叫以將資料傳送至清單方塊控制項時，會選取控制項中的第一個專案，其開頭符合*值*為。 （若要比對整個專案，而不只是前置詞，請使用[DDX_LBStringExact](#ddx_lbstringexact)）。如果沒有相符專案，則不會選取任何專案。 比對不區分大小寫。

`DDX_LBString`呼叫以從清單方塊控制項傳送資料時，*值*會設定為目前的清單方塊選取專案。 如果未選取任何專案，則 [*值*] 會設定為長度為零的字串。
...
函式會 `DDX_CBStringExact` 管理 `CString` 對話方塊、表單檢視或控制項視圖物件中清單方塊控制項的編輯控制項與 `CString` 對話方塊、表單檢視或控制項視圖物件的資料成員之間的資料傳輸。
...
`DDX_LBStringExact`呼叫以將資料傳送至清單方塊控制項時，會選取符合 [*值*] 之控制項中的第一個專案。 （若只要比對前置詞，而不是整個專案，請使用[DDX_LBString](#ddx_lbstring)）。如果沒有相符專案，則不會選取任何專案。 比對不區分大小寫。

`DDX_CBStringExact`呼叫以從清單方塊控制項傳送資料時，*值*會設定為目前的清單方塊選取專案。 如果未選取任何專案，則 [*值*] 會設定為長度為零的字串。
...
函式會 `DDX_MonthCalCtrl` 管理對話方塊、表單檢視或控制項視圖物件中月曆控制項（ [CMonthCalCtrl](../../mfc/reference/cmonthcalctrl-class.md)）與對話方塊、表單檢視或控制項視圖物件之[CTime](../../atl-mfc-shared/reference/ctime-class.md)或[COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md)資料成員之間的日期資料傳送。
...
*value*<br/>
`CTime` `COleDateTime` 與交換資料的對話方塊、表單檢視或控制項視圖物件之或成員變數的參考。

### <a name="remarks"></a>備註

> [!NOTE]
> 控制項只會管理日期值。 時間物件中的時間欄位會設定為反映控制項視窗的建立時間，或在控制項中使用呼叫所設定的任何時間 `CMonthCalCtrl::SetCurSel` 。
...
函式會 `DDX_Radio` 管理 **`int`** 對話方塊、表單檢視或控制項視圖物件中的選項按鈕控制項群組與 **`int`** 對話方塊、表單檢視或控制項視圖物件的資料成員之間的資料傳輸。 資料成員的值 **`int`** 是根據群組內所選取的選項按鈕來決定。
...
例如，如果已核取群組中的第一個選項按鈕（具有 WS_GROUP 樣式的按鈕），則成員的值 **`int`** 為0，依此類推。
...
函式會 `DDX_Scroll` 管理 **`int`** 對話方塊、表單檢視或控制項視圖物件中的捲軸控制項與 **`int`** 對話方塊、表單檢視或控制項視圖物件的資料成員之間的資料傳輸。
...
函式會 `DDX_Slider` 管理 **`int`** 對話方塊或表單檢視中滑杆控制項與 **`int`** 對話方塊或表單檢視物件之資料成員之間的資料傳輸。
...
函 `DDX_Text` 式會在 **`int`** **UINT** **`long`** `CString` **`float`** **`double`** 對話方塊、表單檢視或控制項視圖的編輯控制項與對話方塊、表單檢視或控制項視圖物件的[CString](../../atl-mfc-shared/reference/cstringt-class.md)資料成員之間，管理、UINT、、DWORD、、或資料的傳送。
...
*value*<br/>
對話方塊、表單檢視或控制項視圖物件中之資料成員的參考。 *值*的資料類型取決於您所使用的多載版本 `DDX_Text` 。
