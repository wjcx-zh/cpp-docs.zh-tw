...
使用下列樣式旗標來指定視窗或控制項外觀和行為，當您建立相對應 MFC 物件。 在大部分情況下，在中設定這些樣式`dwStyle`類別的參數`Create`函式。  
  
## <a name="button-styles"></a> 按鈕樣式  
  
按鈕樣式套用至[CButton 類別](../../mfc/reference/cbutton-class.md)物件，例如選項按鈕、 核取方塊和按鈕。 指定的樣式組合`dwStyle`參數[CButton::Create](../../mfc/reference/cbutton-class.md#create)。 如需有關在 Windows 中的按鈕樣式的詳細資訊，請參閱[按鈕樣式 (Windows)](https://msdn.microsoft.com/library/windows/desktop/bb775951)。  
...
### <a name="other-button-options"></a>其他按鈕的選項  
...
MFC 中提供下列下拉式方塊樣式。 如需在 Windows 中的下拉式方塊樣式的詳細資訊，請參閱[下拉式方塊樣式 (Windows)](https://msdn.microsoft.com/library/windows/desktop/bb775796)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`CBS_AUTOHSCROLL`|當使用者輸入的行結尾字元時，會自動捲動右邊編輯控制項中的文字。 如果未設定此樣式，則只會允許符合矩形界限的文字。|  
|`CBS_DISABLENOSCROLL`|清單方塊會顯示已停用的垂直捲軸當清單方塊中未包含足夠的項目捲動。 若沒有這個樣式，在清單方塊中未包含足夠的項目時，捲軸會隱藏。|  
|`CBS_DROPDOWN`|類似於`CBS_SIMPLE`，只不過除非使用者選取編輯控制項旁的圖示，不會顯示清單方塊。|  
|`CBS_DROPDOWNLIST`|類似於`CBS_DROPDOWN`，除了清單方塊中，顯示目前選取範圍的靜態文字項目取代為編輯控制項。|  
...
|`CBS_OEMCONVERT`|在下拉式方塊編輯控制項中輸入的文字會從 ANSI 字元集轉換，然後回到 ANSI 和 OEM 字元集。 如此可確保當應用程式呼叫 `AnsiToOem` Windows 函式將下拉式方塊中的 ANSI 字串轉換成 OEM 字元時，會進行適當的字元轉換。 此樣式是最適合用於包含檔案名稱，且只適用於建立與下拉式方塊的下拉式方塊`CBS_SIMPLE`或`CBS_DROPDOWN`樣式。|  
|`CBS_OWNERDRAWFIXED`|清單方塊的擁有者會負責繪製其內容。清單方塊中的項目都有相同的高度。|  
|`CBS_OWNERDRAWVARIABLE`|清單方塊的擁有者會負責繪製其內容。清單方塊中的項目高度是可變的。|  
...
編輯樣式套用至[CEdit 類別](../../mfc/reference/cedit-class.md)物件。 指定的樣式組合`dwStyle`參數[CEdit::Create](../../mfc/reference/cedit-class.md#create)。 如需在 Windows 中的編輯控制項樣式的詳細資訊，請參閱[編輯控制項樣式 (Windows)](http://msdn.microsoft.com/library/windows/desktop/bb775464)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`ES_AUTOHSCROLL`|當使用者輸入的行結尾字元時，會自動捲動 10 個字元的文字靠右。 當使用者按下 ENTER 鍵時，控制項捲動至位置 0 的所有文字。|  
|`ES_AUTOVSCROLL`|當使用者按下 ENTER 的最後一行時，會自動捲動向上移動一個頁面的文字。|  
|`ES_CENTER`|置中的單行或多行編輯控制項中的文字。|  
|`ES_LEFT`|在單行或多行編輯控制項中的靠左對齊文字。|  
|`ES_LOWERCASE`|將所有輸入至編輯控制項為小寫字元轉換。|  
|`ES_MULTILINE`|將指定的多行編輯控制項。 （預設值是單一行）。如果`ES_AUTOVSCROLL`指定樣式中，顯示盡可能多行編輯控制項，並垂直捲動時使用者按下 ENTER 鍵。 如果`ES_AUTOVSCROLL`是沒有指定，編輯控制項會顯示行數儘可能地嗶聲，如果沒有更多行可以顯示時按下 ENTER。 如果`ES_AUTOHSCROLL`指定的樣式、 多行編輯控制項自動水平捲動時將插入號會超過控制項的右邊緣。 若要開始新行，使用者必須按 ENTER。 如果`ES_AUTOHSCROLL`是沒有指定，控制項自動換至下一行時必要的開頭，則按下 ENTER 時，也已啟動新的一行。 自動換行的位置取決於視窗大小。 如果視窗大小變更，則會重新顯示自動換行的位置變更和文字。 多行編輯控制項可以有捲軸。 含捲軸的編輯控制處理自己捲軸的訊息。 編輯不使用捲軸捲軸上面所述的控制項和處理任何捲動訊息傳送的父視窗。|  
...
|`ES_WANTRETURN`|指定當使用者按下 ENTER 鍵時多行編輯控制項在對話方塊中輸入文字時插入所傳回的歸位字元。 若沒有這個樣式中，按下 ENTER 鍵已按下對話方塊方塊的預設按鈕相同的效果。 此樣式會有對單行編輯控制項。|  
  
## <a name="frame-window-styles-mfc"></a> 框架視窗樣式  
  
框架視窗樣式套用至[CFrameWnd 類別](../../mfc/reference/cframewnd-class.md)物件。 指定的樣式組合`dwStyle`參數[CFrameWnd::Create](../../mfc/reference/cframewnd-class.md#create)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`FWS_ADDTOTITLE`|指定要附加至結尾的框架視窗標題的資訊。 例如，"Microsoft 繪製-繪圖中文件 1 」。 您可以指定在應用程式精靈的 [文件樣板字串] 索引標籤中顯示的字串。 如果您要關閉此選項，請覆寫`CWnd::PreCreateWindow`成員函式。|  
|`FWS_PREFIXTITLE`|框架視窗標題中顯示文件名稱之前的應用程式名稱。 例如，「 文件-WordPad 」。 您可以指定在應用程式精靈的 [文件樣板字串] 索引標籤中顯示的字串。 如果您要關閉此選項，請覆寫`CWnd::PreCreateWindow`成員函式。|  
|`FWS_SNAPTOBARS`|框架視窗圍住控制列，當它的浮動視窗中，而非停駐在框架視窗的控制項調整大小。 此樣式會調整大小以配合控制項列視窗。|
  
## <a name="list-box-styles"></a> 清單方塊樣式  
  
清單方塊樣式套用至[CListBox 類別](../../mfc/reference/clistbox-class.md)物件。 指定的樣式組合`dwStyle`參數[CListBox::Create](../../mfc/reference/clistbox-class.md#create)。 如需在 Windows 中的清單方塊樣式的詳細資訊，請參閱[清單方塊樣式 (Windows)](https://msdn.microsoft.com/library/windows/desktop/bb775149)。  
...
|`LBS_HASSTRINGS`|指定包含由字串組成的項目之主控描繪清單方塊。 清單方塊會維護記憶體和字串的指標，讓應用程式可以使用 `GetText` 成員函式來擷取特定項目的文字。|  
|`LBS_MULTICOLUMN`|指定要水平捲動的資料行清單方塊。 `SetColumnWidth` 成員函式可設定資料行的寬度。|  
|`LBS_MULTIPLESEL`|每次使用者按一下或按兩下字串切換字串選取範圍。 您可以選取任何數目的字串。|  
|`LBS_NODATA`|指定無資料清單方塊。 當清單方塊的項目計數會超過一千時，請指定此樣式。 無資料清單方塊也必須具有`LBS_OWNERDRAWFIXED`樣式，但是不能有`LBS_SORT`或`LBS_HASSTRINGS`樣式。 <br /></b / > 無資料清單方塊類似於主控描繪清單方塊中，不同之處在於它包含的項目沒有字串或點陣圖資料。 加入、插入或刪除項目的命令永遠會忽略任何指定的項目資料；在清單方塊尋找字串的要求永遠會失敗。 需要繪製項目時，系統會傳送 `WM_DRAWITEM` 訊息至主控視窗。 `DRAWITEMSTRUCT` 結構的 itemID 成員 (使用 `WM_DRAWITEM` 訊息傳遞) 指定要繪製項目的行號。 無資料清單方塊不會傳送 `WM_DELETEITEM` 資訊。|  
|`LBS_NOINTEGRALHEIGHT`|清單方塊的大小是建立清單方塊時，應用程式所指定的大小相同。 通常，Windows 會調整清單方塊的大小，使清單方塊不會只顯示一部分的項目。|  
|`LBS_NOREDRAW`|變更時，就不會更新清單方塊顯示。 可以隨時變更此樣式，藉由傳送`WM_SETREDRAW`訊息。|  
...
|`LBS_STANDARD`|清單方塊中的字串會依照字母順序排序，每當使用者按一下或按兩下字串時，父視窗會收到輸入的訊息。 清單方塊的每一邊都包含邊界。|  
...
訊息方塊樣式套用至[AfxMessageBox](../../mfc/reference/cstring-formatting-and-message-box-display.md#afxmessagebox)項目。 指定的樣式組合`nType`參數`AfxMessageBox`。 如需在 Windows 中的訊息方塊樣式的詳細資訊，請參閱[MessageBox 函式 (Windows)](https://msdn.microsoft.com/library/windows/desktop/ms645505)。  
...
|樣式|描述|  
|-----------|-----------------|  
|`MB_ABORTRETRYIGNORE`|訊息方塊包含三個按鈕： Abort、 重試 」，並忽略。|  
|`MB_OK`|訊息方塊包含一個按鈕: [確定]。|  
...
|樣式|描述|  
|-----------|-----------------|  
|`MB_APPLMODAL`|使用者必須回應才能繼續工作，在目前的視窗訊息方塊。 不過，使用者可以移動到其他應用程式的視窗，並使用這些視窗中。 預設值是`MB_APPLMODAL`如果既未`MB_SYSTEMMODAL`也`MB_TASKMODAL`指定。|  
|`MB_SYSTEMMODAL`|所有應用程式會暫停，直到使用者回應訊息方塊。 系統強制回應訊息方塊用來通知使用者的嚴重，可能有害的錯誤，需要立即關注，應該謹慎使用。|  
|`MB_TASKMODAL`|類似於`MB_APPLMODAL`，但在 Microsoft Foundation 類別應用程式中不實用。 這個旗標被保留給呼叫的應用程式或程式庫，但是沒有可用的視窗控制代碼。|  
...
|`MB_ICONINFORMATION`|組成的"I"是圓形中有一個圖示會出現在訊息方塊。|  
...
|樣式|描述|  
|-----------|-----------------|  
|`MB_DEFBUTTON1`|第一個按鈕是預設值。 請注意，第一個按鈕是永遠預設除非`MB_DEFBUTTON2`或`MB_DEFBUTTON3`指定。|  
|`MB_DEFBUTTON2`|第二個按鈕是預設值。|  
|`MB_DEFBUTTON3`|第三個按鈕是預設值。|  
  
## <a name="scroll-bar-styles"></a> 捲軸樣式  
  
捲軸樣式套用至[CScrollBar 類別](../../mfc/reference/cscrollbar-class.md)物件。 指定的樣式組合`dwStyle`參數[CScrollBar::Create](../../mfc/reference/cscrollbar-class.md#create)。 捲軸控制項樣式，在 Windows 中的詳細資訊，請參閱[捲軸控制項樣式 (Windows)](http://msdn.microsoft.com/library/windows/desktop/bb787533)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`SBS_BOTTOMALIGN`|搭配`SBS_HORZ`樣式。 捲軸的下邊緣與中指定的矩形的下邊緣對齊`Create`成員函式。 捲軸有系統捲軸的預設高度。|  
|`SBS_HORZ`|指定水平捲軸。 如果沒有`SBS_BOTTOMALIGN`也`SBS_TOPALIGN`指定的樣式，捲軸具有高度、 寬度和位置中指定`Create`成員函式。|  
|`SBS_LEFTALIGN`|搭配`SBS_VERT`樣式。 捲軸的左邊的緣與中指定的矩形的左邊緣對齊`Create`成員函式。 捲軸有系統捲軸的預設寬度。|  
|`SBS_RIGHTALIGN`|搭配`SBS_VERT`樣式。 捲軸的右邊緣與中指定的矩形的右邊緣對齊`Create`成員函式。 捲軸有系統捲軸的預設寬度。|  
|`SBS_SIZEBOX`|將指定的大小方塊。 如果沒有`SBS_SIZEBOXBOTTOMRIGHTALIGN`也`SBS_SIZEBOXTOPLEFTALIGN`指定的樣式、 大小方塊具有高度、 寬度和位置中指定`Create`成員函式。|  
|`SBS_SIZEBOXBOTTOMRIGHTALIGN`|搭配`SBS_SIZEBOX`樣式。 對齊右下角的 [大小] 方塊中指定的矩形的右下角`Create`成員函式。 [大小] 中有系統大小方塊的預設大小。|  
|`SBS_SIZEBOXTOPLEFTALIGN`|搭配`SBS_SIZEBOX`樣式。 對齊左上角的 [大小] 方塊中指定的矩形的左上角`Create`成員函式。 [大小] 中有系統大小方塊的預設大小。|  
|`SBS_SIZEGRIP`|與相同`SBS_SIZEBOX`，但搭配凸起的邊緣。|  
|`SBS_TOPALIGN`|搭配`SBS_HORZ`樣式。 捲軸的上邊緣與中指定的矩形的頂端對齊`Create`成員函式。 捲軸有系統捲軸的預設高度。|  
|`SBS_VERT`|指定垂直捲軸。 如果沒有`SBS_RIGHTALIGN`也`SBS_LEFTALIGN`指定的樣式，捲軸具有高度、 寬度和位置中指定`Create`成員函式。|  
  
## <a name="static-styles"></a> 靜態樣式  
  
靜態樣式套用至[CStatic 類別](../../mfc/reference/cstatic-class.md)物件。 指定的樣式組合`dwStyle`參數[CStatic::Create](../../mfc/reference/cstatic-class.md#create)。 如需 Windows 中的靜態控制項樣式的詳細資訊，請參閱[靜態控制項樣式 (Windows)](http://msdn.microsoft.com/library/windows/desktop/bb760773)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`SS_BITMAP`|指定要顯示在靜態控制項的點陣圖。 指定的文字是點陣圖 （不是檔名） 的資源檔中其他位置定義的名稱。 樣式會忽略 nWidth 和 nHeight 參數;控制自動調整大小以容納點陣圖。|  
|`SS_BLACKFRAME`|指定框架，以相同的視窗框架色彩繪製的方塊。 預設為黑色。|  
|`SS_BLACKRECT`|指定用來繪製視窗框架的色彩填滿的矩形。 預設為黑色。|  
|`SS_CENTER`|指定一個簡單的矩形，並顯示指定的矩形中置中對齊的文字。 文字格式顯示之前。 會延伸超過一行結尾的文字會自動換置中的下一行的開頭。|  
|`SS_CENTERIMAGE`|指定，是否點陣圖或圖示小於靜態控制項的工作區，其餘的工作區會填入點陣圖或圖示的左上角中的像素的色彩。 如果靜態控制項包含單行文字，文字是垂直置中控制項的工作區中。|  
|`SS_ENDELLIPSIS`|或`SS_PATHELLIPSIS`取代給定字串的一部分省略符號，如有必要，使結果符合指定的矩形中。 <br /><br />您可以指定`SS_END_ELLIPSIS`取代字元在字串結尾或`SS_PATHELLIPSIS`取代字串的中間的字元。 如果字串包含反斜線 (\\) 字元，`SS_PATHELLIPSIS`會保留最多的文字在最後一個反斜線後越好。|  
|`SS_ENHMETAFILE`|指定要顯示在靜態控制項的增強型中繼檔。 指定的文字是中繼檔的名稱。 增強型中繼檔靜態控制項的大小固定。中繼檔會調整為適合靜態控制項工作區。|  
|`SS_ETCHEDFRAME`|繪製靜態控制項使用的框架`EDGE_ETCHED`邊緣樣式。|  
|`SS_ETCHEDHORZ`|繪製靜態控制項使用的頂端和底端邊緣`EDGE_ETCHED`邊緣樣式。|  
|`SS_ETCHEDVERT`|繪製使用 EDGE_ETCHED 邊緣樣式靜態控制項的左右邊緣。|  
|`SS_GRAYFRAME`|指定與框架，以做為螢幕背景 (desktop) 相同的色彩繪製的方塊。 預設為灰色。|  
|`SS_GRAYRECT`|指定用來填滿螢幕背景的色彩填滿的矩形。 預設為灰色。|  
|`SS_ICON`|指定在對話方塊中所顯示的圖示。 指定的文字是圖示 （不是檔名） 的資源檔中其他位置定義的名稱。 `nWidth`和`nHeight`參數被忽略; 圖示自動調整大小。|  
|`SS_LEFT`|指定一個簡單的矩形，並顯示指定的文字的矩形中排清左。 文字格式顯示之前。 會延伸超過一行結尾的文字會自動換到下一個排清左一行的開頭。|  
|`SS_LEFTNOWORDWRAP`|指定一個簡單的矩形，並顯示指定的文字的矩形中排清左。 會展開索引標籤，但未包裝的文字。 延伸超過一行結尾的文字會被裁剪。|  
|`SS_NOPREFIX`|除非指定此樣式，則 Windows 將解譯任何連字號 (&) 是對應的前置詞字元控制項的文字中的字元。 在此情況下，會移除連字號，然後在字串中的下一個字元會加上底線。 如果靜態控制項包含文字，其中這項功能不想要的是，`SS_NOPREFIX`可能會增加。 這個靜態控制項樣式可能包含與任何已定義靜態控制項。 您可以結合`SS_NOPREFIX`具有其他樣式使用位元 OR 運算子。 這最常使用的檔案名稱或其他字串可能包含連字號需要靜態控制項的對話方塊中顯示。|  
|`SS_NOTIFY`|父視窗會將傳送`STN_CLICKED`， `STN_DBLCLK`， `STN_DISABLE`，和`STN_ENABLE`通知訊息，使用者按一下或按兩下控制項時。|  
|`SS_OWNERDRAW`|指定靜態控制項的擁有者會負責繪製控制項。 擁有者視窗收到`WM_DRAWITEM`訊息必須繪製控制項時。|  
|`SS_REALSIZEIMAGE`|防止靜態圖示或點陣圖的控制項 (也就是靜態控制項有`SS_ICON`或`SS_BITMAP`樣式) 會載入或繪製，所以無法調整。 如果圖示或點陣圖大於目的區域，則會裁剪影像。|  
|`SS_RIGHT`|指定一個簡單的矩形，並顯示指定的文字的矩形中排清權限。 文字格式顯示之前。 會延伸超過一行結尾的文字會自動換到下一個排清右邊一行的開頭。|  
|`SS_RIGHTJUST`|指定的靜態控制項的右下角`SS_BITMAP`或`SS_ICON`樣式會保持固定當調整控制項大小的。 僅線上方和左側會調整以符合新的點陣圖或圖示。|  
|`SS_SIMPLE`|指定一個簡單的矩形，並顯示單行文字的矩形中排清左。 無法縮短的一行文字，或以任何方式改變。 (必須處理控制項的父視窗或對話方塊方塊`WM_CTLCOLOR`訊息。)|  
|`SS_SUNKEN`|繪製圍繞靜態控制項半下凹的框線。|  
|`SS_USERITEM`|指定使用者定義的項目。|  
|`SS_WHITEFRAME`|指定與畫面格使用相同的視窗背景色彩繪製的方塊。 預設為白色。|  
|`SS_WHITERECT`|指定用來填滿視窗背景的色彩填滿的矩形。 預設為白色。|  
|`SS_WORDELLIPSIS`|會截斷不適合，並將省略符號的文字。|  
  
## <a name="window-styles"></a> 視窗樣式  
  
視窗樣式套用至[CWnd 類別](../../mfc/reference/cwnd-class.md)物件。 指定的樣式組合`dwStyle`參數[cwnd:: Create](../../mfc/reference/cwnd-class.md#create)或[CWnd::CreateEx](../../mfc/reference/cwnd-class.md#createex)。 如需在 Windows 中的視窗樣式的詳細資訊，請參閱[視窗樣式 (Windows)](https://msdn.microsoft.com/library/windows/desktop/ms632600)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`WS_BORDER`|建立有框線的視窗。|  
|`WS_CAPTION`|建立視窗具有標題列 (表示`WS_BORDER`樣式)。 不能與`WS_DLGFRAME`樣式。|  
|`WS_CHILD`|建立子視窗。 不能與`WS_POPUP`樣式。|  
|`WS_CHILDWINDOW`|與相同`WS_CHILD`樣式。|  
|`WS_CLIPCHILDREN`|在父視窗內繪圖時，應排除子視窗佔用的區域。 用於當您建立父視窗。|  
|`WS_CLIPSIBLINGS`|裁剪相對於其他; 的子視窗也就是說，當特定的子視窗接收繪製訊息，`WS_CLIPSIBLINGS`樣式裁剪子視窗，更新的區域的所有其他重疊的子視窗。 (如果`WS_CLIPSIBLINGS`未指定，且子視窗重疊，當您繪製工作區內的子視窗時，可能會繪製相鄰子視窗的工作區內。)搭配`WS_CHILD`只樣式。|  
|`WS_DISABLED`|建立一個一開始即停用的視窗。|  
|`WS_DLGFRAME`|建立視窗框線為雙線，但沒有標題。|  
|`WS_GROUP`|指定一組控制項所在使用者可以移動一個控制項到下一步 箭號索引鍵的第一個控制項。 所有控制項，以定義`WS_GROUP`樣式`FALSE`之後的第一個控制項必須屬於相同的群組。 下一個控制項與`WS_GROUP`樣式啟動下一個群組 （也就是說，一個群組結束開始下）。|  
|`WS_HSCROLL`|建立具有水平捲軸的視窗。|  
|`WS_ICONIC`|建立一開始最小化視窗。 與相同`WS_MINIMIZE`樣式。|  
|`WS_MAXIMIZE`|建立視窗的大小上限。|  
|`WS_MAXIMIZEBOX`|建立具有最大化按鈕的視窗。|  
|`WS_MINIMIZE`|建立一開始最小化視窗。 搭配`WS_OVERLAPPED`只樣式。|  
|`WS_MINIMIZEBOX`|建立具有最小化按鈕的視窗。|  
|`WS_OVERLAPPED`|建立的重疊的視窗。 重疊的視窗通常會有標題和框線。|  
|`WS_OVERLAPPEDWINDOW`|建立具有重疊的視窗`WS_OVERLAPPED`， `WS_CAPTION`， `WS_SYSMENU`， `WS_THICKFRAME`， `WS_MINIMIZEBOX`，和`WS_MAXIMIZEBOX`樣式。|  
|`WS_POPUP`|建立快顯視窗。 不能與`WS_CHILD`樣式。|  
|`WS_POPUPWINDOW`|建立快顯視窗與`WS_BORDER`， `WS_POPUP`，和`WS_SYSMENU`樣式。 `WS_CAPTION`樣式必須結合`WS_POPUPWINDOW`樣式來顯示控制項功能表。|  
|`WS_SIZEBOX`|建立有調整大小框線的視窗。 與相同`WS_THICKFRAME`樣式。|  
|`WS_SYSMENU`|建立擁有控制功能表方塊標題列中的視窗。 僅適用於 windows 的標題列。|  
|`WS_TABSTOP`|指定的控制項，使用者可以使用 TAB 鍵移動任何數目的其中一個。 TAB 鍵移至所指定的下一個控制項的使用者`WS_TABSTOP`樣式。|  
|`WS_THICKFRAME`|建立可以用來調整視窗大小粗框架的視窗。|  
|`WS_TILED`|建立的重疊的視窗。 重疊的視窗具有標題列，並加上框線。 與相同`WS_OVERLAPPED`樣式。|  
|`WS_TILEDWINDOW`|建立具有重疊的視窗`WS_OVERLAPPED`， `WS_CAPTION`， `WS_SYSMENU`， `WS_THICKFRAME`， `WS_MINIMIZEBOX`，和`WS_MAXIMIZEBOX`樣式。 與相同`WS_OVERLAPPEDWINDOW`樣式。|  
|`WS_VISIBLE`|建立一個一開始即可見的視窗。|  
|`WS_VSCROLL`|建立具有垂直捲軸的視窗。|  
  
## <a name="extended-window-styles"></a> 延伸的視窗樣式  
  
延伸的視窗樣式套用至[CWnd 類別](../../mfc/reference/cwnd-class.md)物件。 指定的樣式組合`dwExStyle`參數[CWnd::CreateEx](../../mfc/reference/cwnd-class.md#createex)。 如需在 Windows 中的延伸的視窗樣式的詳細資訊，請參閱[延伸視窗樣式 (Windows)](https://msdn.microsoft.com/library/windows/desktop/ff700543)。  
  
|樣式|描述|  
|-----------|-----------------|  
|`WS_EX_ACCEPTFILES`|指定以這個樣式建立的視窗會接受拖放檔案。|  
|`WS_EX_APPWINDOW`|當視窗為可見時，會強制將最上層視窗放在工作列上。|  
...
|`WS_EX_CONTROLPARENT`|可讓使用者使用 TAB 鍵在視窗的子視窗之間巡覽。|  
|`WS_EX_DLGMODALFRAME`|指定的視窗標題列中指定時可能會 （選擇性） 建立雙框線`WS_CAPTION`樣式中的旗標`dwStyle`參數。|  
|`WS_EX_LAYERED`|視窗是[分層視窗](http://msdn.microsoft.com/library/ms632599.aspx#layered)。 無法使用此樣式，如果視窗[類別樣式](http://msdn.microsoft.com/library/ms633574.aspx#class_styles)的其中一個`CS_OWNDC`或`CS_CLASSDC`。 不過，[!INCLUDE[win8_first](../../mfc/reference/includes/win8_first_md.md)]支援`WS_EX_LAYERED`子視窗，其中先前的 Windows 版本支援僅針對最上層視窗的樣式。|  
...
|`WS_EX_NOPARENTNOTIFY`|指定以這個樣式建立的子視窗不會傳送`WM_PARENTNOTIFY`建立或終結子視窗時，其父視窗的訊息。|  
...
|`WS_EX_RIGHTSCROLLBAR`|放置垂直捲軸 （如果有的話） 用戶端區域的右側。 這是預設值。|  
|`WS_EX_RTLREADING`|顯示視窗文字使用由右至左讀取順序屬性。|  
|`WS_EX_STATICEDGE`|建立視窗與適用於不接受使用者輸入的項目有 3d 框線樣式。|  
|`WS_EX_TOOLWINDOW`|建立工具視窗，也就是用來當做浮動工具列的視窗。 工具視窗的標題列較一般標題列短，而且使用小字型繪製視窗標題。 工具視窗不會在工作列中顯示，當使用者按下 ALT+TAB 時也不會在出現的視窗中顯示。|  
|`WS_EX_TOPMOST`|指定以這個樣式建立的視窗應該放優先於所有產生的 windows，而且即使視窗已停用保持。 應用程式可以使用 `SetWindowPos` 成員函式來加入或移除此屬性。|  
...
[編輯控制項樣式 (Windows)](http://msdn.microsoft.com/library/windows/desktop/bb775464)  
...
[捲軸控制項樣式 (Windows)](http://msdn.microsoft.com/library/windows/desktop/bb787533)  
