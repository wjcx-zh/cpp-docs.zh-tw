...
此架構可讓您輕鬆地初始化對話方塊中的控制項值，以及從控制項抓取值。 較繁瑣的手動方法是呼叫函式，例如，@no__t 的 `SetDlgItemText` 和 class 的成員函式 `CWnd`，其適用于控制項視窗。 使用這些函式，您可以個別存取每個控制項來設定或取得其值，並呼叫 `SetWindowText` 和 `GetWindowText` 之類的函數。 架構的方法會自動化初始化和抓取。
...
對於強制回應對話方塊，您可以在 `DoModal` 傳回 IDOK，但在終結對話方塊物件之前，取得使用者輸入的任何資料。 對於非強制回應對話方塊，您可以隨時使用引數**TRUE**呼叫 `UpdateData`，然後存取對話方塊類別成員變數，以從對話方塊物件中取出資料。 在[對話資料交換和驗證](../mfc/dialog-data-exchange-and-validation.md)中，將會更詳細地討論此主題。
