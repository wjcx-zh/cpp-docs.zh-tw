...
應用程式可以呼叫滑桿控制項成員函式，來擷取滑桿控制項的相關資訊 ([CSliderCtrl](../mfc/reference/csliderctrl-class.md))，並變更其特性。  
 ...
 應用程式可以使用 TBS_AUTOTICKS 樣式，將滑桿控制項刻度標記會自動顯示。 如果應用程式需要控制的位置或刻度標記的頻率，不過，可以使用許多成員函式。  
