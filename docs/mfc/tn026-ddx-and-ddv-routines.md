...
所有的對話方塊資料函式已完成 c + + 程式碼。 沒有任何特殊的資源或 magic 巨集。 虛擬函式，是類別中覆寫每個對話方塊中，沒有對話方塊資料交換和驗證機制的核心。 在這種形式一律找到它：
...
標準 c + + 建構函式中，通常在區塊中以設定初始值的資料`//{{AFX_DATA_INIT`和`//}}AFX_DATA_INIT`註解。
...
- `m_pDlgWnd`: 包含的控制項 [] 視窗 （通常的對話方塊）。 這是為了防止 DDX_ 和 DDV_ 全域函式的呼叫端来傳遞 'this' 每個 DDX/DDV 常式時。

- `PrepareCtrl`與`PrepareEditCtrl`： 準備的對話方塊控制項交換資料。 儲存該控制項的控制代碼設定焦點，如果驗證失敗。 `PrepareCtrl` 用於非編輯控制項和`PrepareEditCtrl`用於編輯控制項。

- `Fail`： 呼叫後啟動 提示使用者輸入錯誤的訊息方塊。 這個常式會將焦點還原至最後一個控制項 (在上次呼叫`PrepareCtrl`或`PrepareEditCtrl`)，則擲回例外狀況。 可從 DDX_ 和 DDV_ 常式呼叫此成員函式。
...
有`DoDialogExchange`成員函式包含的條件式或混合的交換和驗證函式呼叫的任何其他有效的 c + + 陳述式。
...
若要這樣做，DDX 進行特殊的項目。CLW （舊版的 Visual c + + 會儲存在 APSTUDIO 這項資訊。INI) 或您的專案中。CLW 檔案。 特殊的項目可以是輸入您的專案的 [一般資訊] 區段中。CLW 檔案或 [ExtraDDX] 區段的 DDX。CLW \Program Files\Microsoft Visual Studio\Visual C + + \bin 目錄中的檔案。 若要建立的 DDX。如果不存在，就會 CLW 檔案。 如果您打算只在特定專案中使用自訂的 DDX_/DDV_ 常式，加入您專案的 [一般資訊] 區段項目。CLW 改為檔案。 如果您打算使用的常式，在許多專案時，請 DDX [ExtraDDX] 區段中新增項目。CLW。
...
> ExtraDDX？ =*金鑰*;*vb 金鑰*;*提示字元*;*型別*;*initValue*;*DDX_Proc* [;*DDV_Proc*;*prompt1*;*arg1* [;*prompt2*;*fmt2*]]
...
- *initValue*
  ...
  初始值 — 0 或空白。 如果是空白的會將 //{{AFX_DATA_INIT 區段的實作檔中寫入沒有初始化行。 空白項目應該用於 c + + 物件 (例如`CString`， `CTime`，依此類推) 具有建構函式，確保正確地初始化。
...
- *arg*
  ...
  DDV_ 程序的單一識別項。 C + + 函式名稱開頭必須是 「 DDV_"，但不是包含"DDX_ 」 中\<DDX_Proc > 識別項。
