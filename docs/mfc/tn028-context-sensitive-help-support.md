...
## <a name="context-sensitive-help-f1-help"></a>即時線上說明 (F1 說明)  
 F1 鍵通常會轉譯成與識別碼的 ID_HELP 命令方式是將加速器置於主視窗的快速鍵對應表。 ID_HELP 命令可能也會產生具有識別碼的 ID_HELP 主視窗或對話方塊上的按鈕。  
  
 不論如何產生 ID_HELP 命令，它是路由傳送做為一般的命令一直是命令處理常式。 如需 MFC 命令路由架構的詳細資訊，請參閱[技術提示 21](../mfc/tn021-command-and-message-routing.md)。 如果應用程式已啟用 [說明]，將由處理 ID_HELP 命令[CWinApp::OnHelp](../mfc/reference/cwinapp-class.md#onhelp)。 應用程式物件收到說明訊息，然後適當地路由轉送命令。 這是必要的，因為只靠預設命令路由仍不足以判斷最特定的內容。  
 ...
 *lParam*  
 包含目前可用的說明內容。 *lParam*為零，如果尚未決定任何說明內容。 實作`OnCommandHelp`可以使用中的內容識別碼*lParam*以判斷不同的內容，或可以將它傳遞到`CWinApp::WinHelp`。  
  
 *wParam*  
 未使用且將為零。  
  
 如果`OnCommandHelp`函式呼叫`CWinApp::WinHelp`，它應該傳回**TRUE**。 傳回**TRUE**會停止路由至其他類別及其他視窗，此命令。  
...
## <a name="help-mode-shiftf1-help"></a>說明模式 (Shift+F1 說明)  
 ...
 如果有特定轉譯或動作置於`PreTranslateMessage`函式，不應該在進行 SHIFT + F1 說明模式中，您應該檢查*m_bHelpMode*隸屬`CWinApp`然後再執行這些作業。 在呼叫 `CDialog` 之前，`PreTranslateMessage` 的 `IsDialogMessage` 實作會檢查這部分。 在 SHIFT+F1 模式期間，這會停用非強制回應對話方塊上的「對話方塊巡覽」鍵。 此外，在這個迴圈期間仍會呼叫 `CWinApp::OnIdle`。  
  
 如果使用者從功能表選擇命令，就會處理該命令的說明 （透過 WM_COMMANDHELP，請參閱下方）。 如果使用者按一下應用程式視窗的可見區域，便會判斷其是否為非用戶端點按或用戶端點按。 `OnContextHelp` 會自動處理非用戶端點按與用戶端點按的對應。 如果用戶端按一下，然後將傳送 WM_HELPHITTEST 已按下的視窗。 如果該視窗傳回非零值，該值便會用做說明的內容。 如果傳回零，`OnContextHelp` 會嘗試父視窗 (若失敗，再嘗試其父代，依此類推)。 如果無法判斷說明內容，預設會將 ID_DEFAULT_HELP 命令傳送至主要的視窗中，接著 （通常） 對應到`CWinApp::OnHelpIndex`。  
...
```  
 ...
 WM_HELPHITTEST 是接收已按下 SHIFT + F1 說明模式期間，使用中視窗的 MFC 私人 windows 訊息。 當視窗收到這個訊息時，它會傳回**DWORD**說明 ID 供 WinHelp 使用。  
 ...
 *wParam*  
 ...
 在大部分情況下，您可以應用您可能已經擁有的點擊測試碼。 請參閱實作`CToolBar::OnHelpHitTest`處理 WM_HELPHITTEST 訊息的範例 (程式碼會利用上按鈕和工具提示中的使用的點擊測試程式碼`CControlBar`)。  
