---
title: TN035： 使用 Visual c + + 的多個資源檔和標頭檔 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
- 當您可能想要分割成多個資源檔和/或標頭檔的專案，以及如何進行，所以

- 您要如何共用常見的標頭。兩個 H 檔案。RC 檔

- 如何您將專案資源分割成多個。RC 檔

- 您如何 （和工具） 管理之間的組建相依性。RC。CPP、 和。H 檔案
...
- **共用兩個標頭檔。RC 檔**示範您如何共用一個標頭檔之間多個。RC 檔在不同的專案中，或可能是相同專案中。
...
AFXRES.RC 和 AFXPRINT.RC 包含 Framework 某些功能所需的標準資源。 就像 RES\MYAPP.RC2，這兩個 Framework 提供的資源檔以 #include 包含在 MYAPP.RC 結尾，並且是在 [Set Includes] 對話方塊的編譯時間指示詞中指定。 因此，當您在 Visual C++ 中編輯 MYAPP.RC 時，這些 Framework 資源無法直接加以檢視或編輯，但還是會編譯至應用程式的二進位 .RES 檔案與最終 .EXE 檔案。 如需有關標準架構資源，包括程序來修改它們，請參閱[技術提示 23](../mfc/tn023-standard-mfc-resources.md)。
...
您可以使用，以包含其他唯讀標頭檔**Resource Includes**命令，以指定 #include 陳述式，因為第二個唯讀符號指示詞中，為：
...
ID_TOOLS_SPELL 符號是保留在共用標頭檔 MYSHARED.H 中。 您可以使用文字編輯器手動維護這個共用標頭檔，Visual C++ 無法直接進行編輯。 在兩個資源檔 MYSTRS。RC 和 MYMENUS。您指定 RC，#include MYSHARED。在 MYAPP 的唯讀指示詞中的小時。RC，使用**Resource Includes**命令，如先前所述。
...
`_APS_NEXT_CONTROL_VALUE` 下一步，將會用於對話方塊控制項符號值。 對話方塊控制項符號值的有效範圍是 8 至 0xDFFF。

`_APS_NEXT_SYMED_VALUE` 您手動指派符號值時，會發出下一個符號值在符號瀏覽器中使用新的命令。
