...
在 MFC/OLE1，`COleClientItem::GetBounds`和`SetBounds`用來查詢與管理的項目範圍 (`left`和`top`成員永遠是零)。 MFC/OLE 2 中支援這種更直接`COleClientItem::GetExtent`和`SetExtent`，其處理**大小**或`CSize`改為。
