...
MFC OLE 範例[OCLIENT](../overview/visual-cpp-samples.md)示範使用追蹤器與 OLE 用戶端項目從觀點來看的容器應用程式的通用介面。 如需示範不同的樣式以及追蹤器物件的能力，請參閱 MFC 一般範例[TRACKER](../overview/visual-cpp-samples.md)。

如需有關在 OLE 應用程式中實作追蹤器的詳細資訊，請參閱[追蹤器：在 OLE 應用程式中實作追蹤器](../mfc/trackers-implementing-trackers-in-your-ole-application.md)
