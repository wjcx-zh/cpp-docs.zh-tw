...
樹狀結構控制項中的任何項目 ([CTreeCtrl](../mfc/reference/ctreectrl-class.md)) 可以有子項目，稱為子項目，與它相關聯的清單。 有一個或多個子項目的項目稱為父項目。 子項目會在其父項目下方以縮排顯示，表示其附屬於父代。 沒有父代的項目會位於階層架構的頂端並稱為根項目。  
 ...
 將項目加入至樹狀目錄控制項的藉由呼叫[InsertItem](../mfc/reference/ctreectrl-class.md#insertitem)成員函式。 此函式傳回的控制代碼**HTREEITEM**唯一識別項目的型別。 在加入項目時，您必須指定新項目之父項目的控制代碼。 如果您指定**NULL**或**TVI_ROOT**值而不是父項目控制代碼，以[TVINSERTSTRUCT](http://msdn.microsoft.com/library/windows/desktop/bb773452)結構或*hParent*參數，做為根項目會加入項目。  
