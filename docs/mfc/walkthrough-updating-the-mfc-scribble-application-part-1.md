...
2. 在 mainfrm.cpp 檔案中，加入下列程式碼之前最終`return`陳述式結尾處`CMainFrame::OnCreate`函式。 這會建立功能區列的執行個體。
...
您已經修改傳統 Scribble 1.0 MFC 範例使用功能區設計工具。 現在請移至[第 2 部分](../mfc/walkthrough-updating-the-mfc-scribble-application-part-2.md)。
