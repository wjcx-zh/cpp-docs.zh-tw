...
7. 接下來，建立功能表項目和事件處理常式粗的畫筆。

   1. 在**資源檢視**視窗中，開啟 IDR_SCRIBBTYPE 功能表資源。

   2. 按一下**畫筆**以開啟 [畫筆]。 然後按一下 **在這裡輸入**和型別`Thic&k Pen`。

   3. 以滑鼠右鍵按一下您剛才輸入顯示的文字**屬性**視窗。 變更 ID 屬性，以`ID_PEN_THICK_WIDTH`。

   4. 以滑鼠右鍵按一下**粗畫筆**您剛剛建立，然後按一下功能表項目**加入事件處理常式**。 **事件處理常式精靈**隨即出現。

   5. 在**類別清單**的精靈中，選取  **CScribbleDoc** ，然後按一下 **新增和編輯**。 這會建立名為事件處理常式`CScribbleDoc::OnPenThickWidth`。
...
2. 每個文件包含一份 stokes，使用者已繪製。 每個筆觸由定義`CStroke`物件。 `CStroke`類別不包含資訊的畫筆顏色。 因此，您必須修改的類別。 在 scribdoc.h，在`CStroke`類別之後，加入下列程式碼的定義`m_nPenWidth`資料成員。
...
6. 設定預設的畫筆色彩的文件類別。 在 scribdoc.cpp，加入下列行以`CScribbleDoc::InitDocument`，之後`m_nThickWidth = 5;`陳述式。
...
2. 儲存檔案繪圖的色彩。 在中加入下列陳述式，scribdoc.cpp，`CStroke::Serialize`方法，之後`ar << (WORD)m_nPenWidth;`陳述式。

   ```cpp
   ar << (COLORREF)m_penColor;
    ```

3. 最後，載入從檔案繪圖的色彩。 在中加入下列程式碼，行`CStroke::Serialize`方法，之後`m_nPenWidth = w;`陳述式。

   ```cpp
   ar >> m_penColor;
   ```

4. 現在徒手畫的色彩，並儲存至檔案的繪圖。
