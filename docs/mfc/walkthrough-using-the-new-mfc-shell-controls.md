...
1. 在**新的專案**對話方塊方塊中，展開**視覺化C++** 中的節點**專案類型**窗格，然後選取**MFC**。 然後，在**範本**窗格中，選取**MFC 應用程式**。 輸入專案名稱，例如`MFCShellControls`，按一下  **確定**。 在後**MFC 應用程式精靈**出現時，請使用下列選項：
