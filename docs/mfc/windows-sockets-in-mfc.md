...
> [!NOTE]
>  在 Windows NT 和 Windows 2000，Windows Sockets 支援 16 位元應用程式為基礎 WINSOCK。DLL。 32 位元應用程式，支援處於 WSOCK32。DLL。 應用程式開發介面，提供完全相同，不同之處在於 32 位元版本有擴大為 32 位元的參數。 在 Win32 中，會提供安全執行緒。  
