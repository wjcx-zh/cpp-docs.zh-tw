...
在 Visual Studio 2017 15.9 版中，該程式碼會引發下列錯誤：*error C3615: constexpr function 'S<int>::f' cannot result in a constant expression. note: failure was caused by call of undefined function or one not declared 'constexpr' note: see usage of 'g'*。 若要避免此錯誤，請從函式 f() 的明確具現化移除 `constexpr` 限定詞。 
