...
## <a name="construction"></a>建構  
 [1] 建構函式  
 取得上的 D3D 存取鎖定給定[accelerator_view](accelerator-view-class.md)物件。 建構區塊之前已取得鎖定。  
  
 [2] 建構函式  
 採用 D3D 存取鎖定的給定[accelerator_view](accelerator-view-class.md)物件。  
  
 [3] 移動建構函式  
 採用現有 D3D 存取鎖定的另一個`scoped_d3d_access_lock`物件。 建構不會封鎖。  
