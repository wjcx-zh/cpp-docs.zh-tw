...
5. 選取**新增監看式**再次資料行標頭中，輸入`localA[localIdx[0]]`，然後選擇**Enter**索引鍵。  
     ...
     ![已排序的結果使用平行監看式視窗](../../parallel/amp/media/campf.png "campf")  
 排序結果  
...
3. 開啟 作用中的資料列的捷徑功能表，然後選擇**凍結**。  
     ...
     ![GPU 執行緒 視窗顯示已凍結的執行緒](../../parallel/amp/media/campk.png "campk")  
凍結執行緒**GPU 執行緒**視窗  
...
1. 其中一個執行緒中的捷徑功能表上**GPU 執行緒** 視窗中，選擇**Group By**，**位址**。  
     ...
     ![依位址分組的 GPU 執行緒 視窗中的執行緒](../../parallel/amp/media/campl.png "campl")  
群組中的執行緒**GPU 執行緒**視窗  
