...
> [!CAUTION]
> 如果您未將解除標記傳遞給 `task` 的函式或[concurrency：： create_task](reference/concurrency-namespace-functions.md#create_task)函式，則該工作不會可取消。 此外，您必須傳遞相同的取消語彙基元給任何巢狀工作的建構函式 (也就是在另一個工作主體內建立的工作)，以便同時取消所有工作。
...
> [!TIP]
> 當您呼叫接受 `cancellation_token` 物件的函式或函式，但不想要可取消作業時，請使用[concurrency：： cancellation_token：： none](reference/cancellation-token-class.md#none)方法。

您也可以提供取消語彙基元給 `task_group` 或 `structured_task_group` 物件的建構函式。 其中的一個重要環節是子工作群組會繼承這個取消語彙基元。 如需使用[concurrency：： run_with_cancellation_token](reference/concurrency-namespace-functions.md#run_with_cancellation_token)函式來呼叫 `parallel_for`來示範此概念的範例，請參閱本檔稍後的[取消平行演算法](#algorithms)。
...
當您將解除標記提供給 `when_all` 和 `when_any` 函式時，該函式只會在解除標記取消時，或其中一個參與者工作以已取消狀態結束或擲回例外狀況時取消。

當您未提供取消語彙基元給 `when_all` 函式時，它會從組合整體作業之每個工作繼承取消語彙基元。 當任何一個標記取消，而且至少有一個參與者工作尚未啟動或正在執行時，會取消從 `when_all` 傳回的工作。 當其中一個工作擲回例外狀況時，就會發生類似的行為-從 `when_all` 傳回的工作會立即取消並出現該例外狀況。
