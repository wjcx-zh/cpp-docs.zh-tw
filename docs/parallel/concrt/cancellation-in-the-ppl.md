...
> [!NOTE]
>  執行階段使用例外狀況處理來實作取消。 請勿在您的程式碼中攔截或處理這些例外狀況。 此外，我們建議您在工作的函式主體中撰寫例外狀況安全的程式碼。 例如, 您可以使用資源取得*為初始化*(RAII) 模式, 以確保在工作主體中擲回例外狀況時, 會正確處理資源。 如需使用 RAII 模式清除可取消之工作中的資源的完整範例, 請參閱[逐步解說:從使用者介面執行緒](../../parallel/concrt/walkthrough-removing-work-from-a-user-interface-thread.md)中移除工作。
...
- 當您使用解除標記時, 請使用[concurrency:: cancellation_token_source:: cancel](reference/cancellation-token-source-class.md#cancel)方法來起始取消和[concurrency:: cancel_current_task](reference/concurrency-namespace-functions.md#cancel_current_task)函數, 以回應取消。 使用[concurrency:: cancellation_token:: is_canceled](reference/cancellation-token-class.md#is_canceled)方法來檢查是否有任何其他工作已要求取消。

- 取消不會立即發生。 雖然取消工作或工作組時, 不會啟動新的工作, 但作用中的工作必須檢查並回應取消。

- 以值為基礎的接續會繼承其前項工作的取消語彙基元。 以工作為基礎的接續絕不會繼承其前項工作的語彙基元。

- 當您呼叫接受`cancellation_token`物件但不想要可取消作業的函式或函式時, 請使用[concurrency:: cancellation_token:: none](reference/cancellation-token-class.md#none)方法。 此外, 如果您未將解除標記傳遞給[concurrency:: task](../../parallel/concrt/reference/task-class.md)函數或[concurrency:: create_task](reference/concurrency-namespace-functions.md#create_task)函式, 則該工作不會可取消。
...
![平行工作樹狀結構](../../parallel/concrt/media/parallelwork_trees.png "平行工作樹狀結構")

下列範例顯示在圖中建立樹狀結構時需要的程式碼。 在此範例中`tg1` , `tg2`和是[concurrency:: structured_task_group](../../parallel/concrt/reference/structured-task-group-class.md)物件;`t1`、 、、`t3`和是[concurrency:: task_handle](../../parallel/concrt/reference/task-handle-class.md)物件。 `t5` `t4` `t2`
...
如需取消平行工作的更多範例[, 請參閱逐步解說:使用工作和 XML HTTP 要求](../../parallel/concrt/walkthrough-connecting-using-tasks-and-xml-http-requests.md)進行連接, [如何:使用取消來中斷平行迴圈](../../parallel/concrt/how-to-use-cancellation-to-break-from-a-parallel-loop.md), 以及[如何:使用例外狀況處理來中斷平行迴圈](../../parallel/concrt/how-to-use-exception-handling-to-break-from-a-parallel-loop.md)。

###  <a name="tokens"></a>使用解除標記來取消平行工作
...
- 若是物件, 請使用[concurrency:: cancel_current_task 函數。](reference/concurrency-namespace-functions.md#cancel_current_task) `task` `cancel_current_task` 會取消目前的工作及任何其以值為基礎的接續。 (它不會取消與工作或其接續相關聯的取消*標記*)。

- 針對工作組和平行演算法, 請使用[concurrency:: is_current_task_group_canceling](reference/concurrency-namespace-functions.md#is_current_task_group_canceling)函式來偵測取消, 並在此函數傳回**true**時, 儘快從工作主體傳回。 (請勿從工作群組呼叫 `cancel_current_task`。)
...
> [!TIP]
> 或者, 您可以呼叫[concurrency:: interruption_point](reference/concurrency-namespace-functions.md#interruption_point)函式, 而`cancel_current_task`不是。
...
> [!CAUTION]
> 如果您不將解除標記傳遞給此`task`函式或[concurrency:: create_task](reference/concurrency-namespace-functions.md#create_task)函式, 則該工作不會可取消。 此外，您必須傳遞相同的取消語彙基元給任何巢狀工作的建構函式 (也就是在另一個工作主體內建立的工作)，以便同時取消所有工作。

您可能會想要在取消取消語彙基元時，執行任意程式碼。 例如, 如果您的使用者在使用者介面上選擇 [**取消**] 按鈕來取消作業, 您可以停用該按鈕, 直到使用者啟動另一個作業為止。 下列範例示範如何使用[concurrency:: cancellation_token:: register_callback](reference/cancellation-token-class.md#register_callback)方法, 來註冊解除標記取消時執行的回呼函式。
...
> [!TIP]
> 當您呼叫接受`cancellation_token`物件的函式或函式, 而不想要可取消作業時, 請使用[concurrency:: cancellation_token:: none](reference/cancellation-token-class.md#none)方法。

您也可以提供取消語彙基元給 `task_group` 或 `structured_task_group` 物件的建構函式。 其中的一個重要環節是子工作群組會繼承這個取消語彙基元。 如需使用[concurrency:: run_with_cancellation_token](reference/concurrency-namespace-functions.md#run_with_cancellation_token)函式執行呼叫`parallel_for`來示範此概念的範例, 請參閱本檔稍後的[取消平行演算法](#algorithms)。
...
當您提供解除標記給`when_all`和`when_any`函式時, 該函式只會在解除標記取消時, 或其中一個參與者工作以已取消狀態結束或擲回例外狀況時取消。

當您未提供取消語彙基元給 `when_all` 函式時，它會從組合整體作業之每個工作繼承取消語彙基元。 當任何一個標記取消, `when_all`而且至少有一個參與者工作尚未啟動或正在執行時, 會取消從傳回的工作。 當其中一個工作擲回例外狀況時, 就會發生類似的行為-從`when_all`傳回的工作會立即取消並出現該例外狀況。
...
這個範例會檢查工作迴圈的每<sup></sup>個100次反覆運算上是否有取消。 您檢查取消的頻率取決於您的工作執行的工作量，以及您需要工作回應取消的速度。
...
`structured_task_group` 類別不是安全執行緒。 因此，呼叫其父系 `structured_task_group` 物件的子工作會產生未指定的行為。 此規則`structured_task_group::cancel`的例外狀況為和[concurrency:: structured_task_group:: is_canceling](reference/structured-task-group-class.md#is_canceling)方法。 子工作可以呼叫這些方法來取消父工作群組，並檢查取消。
...
第三個範例類似於第二個，不同之處在於工作 `t4` 會擲回例外狀況以取消工作群組 `tg2`。 這個範例會使用`try` - `catch`區塊, 在工作組`tg2`等候子工作完成時, 檢查是否有取消作業。 就像第一個範例，這會使工作群組 `tg2` 進入已取消的狀態，但它並不會取消工作群組 `tg1`。
...
|[工作平行處理](../../parallel/concrt/task-parallelism-concurrency-runtime.md)|描述工作如何與工作群組產生關聯，以及如何在應用程式中使用非結構化和結構化工作。|
