...
複製範例程式碼，並將它貼入 Visual Studio 專案中，或貼入名為 `concrt-omp-parallel-any-of.cpp` 的檔案中，然後在 [Visual Studio 命令提示字元] 視窗中執行下列命令。
