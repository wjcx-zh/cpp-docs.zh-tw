---
title: "C + + 中建立非同步作業，用於 Windows 8.x 應用程式 |Microsoft 文件"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="creating-asynchronous-operations-in-c-for-windows-8x-apps"></a>C + + 中建立非同步作業，用於 Windows 8.x 應用程式
這份文件描述一些重要觀念，當您在 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 應用程式中 task 類別產生以 Windows 執行緒集區為基礎的非同步作業時，務必牢記這些觀念。  
 ...
 使用非同步程式設計是 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 應用程式模型中的主要元件，因為它會使應用程式保持回應使用者輸入。 您可以啟動長時間執行的工作，而不封鎖 UI 執行緒，而且可以稍後再接收工作的結果。 工作在背景執行時，您也可以取消工作和接收進度通知。 [C++ 的非同步程式設計 (Windows 執行階段應用程式)](http://msdn.microsoft.com/library/windows/apps/hh780559.aspx) 文件提供了 Visual C++ 中可用來建立 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 應用程式的非同步模式概觀。 該文件教您如何使用及建立非同步 Windows 執行階段作業的鏈結。 本章節描述如何使用 ppltasks.h 中的型別，產生可由另一個 Windows 執行階段元件使用的非同步作業以及如何控制如何非同步工作會執行。 另請閱讀 [Hilo 的非同步程式設計模式和祕訣 (使用 C++ 和 XAML 的 Windows 市集應用程式)](http://msdn.microsoft.com/library/windows/apps/jj160321.aspx) ，了解我們如何在 Hilo (使用 C++ 和 XAML 的 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 應用程式) 中使用工作類別實作非同步作業。  
  
> [!NOTE]
>  您可以在 [應用程式中使用](../../parallel/concrt/parallel-patterns-library-ppl.md) 平行模式程式庫 (PPL) [平行模式程式庫 (PPL)](../../parallel/concrt/asynchronous-agents-library.md) 和 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 應用程式中 task 類別產生以 Windows 執行緒集區為基礎的非同步作業時，務必牢記這些觀念。 不過，您無法使用工作排程器或資源管理員。 本文件將描述 PPL 提供的其他功能，這些功能僅適用於 [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] 應用程式，不適用桌面應用程式。  
...
##  <a name="create-async"></a> 建立非同步作業  
 ...
 Windows 執行階段是程式設計介面，可用來建立[!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)]只在特殊作業系統環境中執行的應用程式。 這類應用程式會使用授權的功能、資料類型和裝置，並且從 [!INCLUDE[win8_appstore_long](../../build/reference/includes/win8_appstore_long_md.md)]發佈。 Windows 執行階段由*應用程式二進位介面*(ABI)。 ABI 是讓 Visual c + + 等程式設計語言中可用的 Windows 執行階段 Api 的基礎二進位合約。  
...
>  若要讓 PPL 正確回報給 Windows 執行階段就已取消該作業，不要攔截這個內部例外狀況類型。 這表示，您不應該攔截所有例外狀況 (`catch (...)`)。 如果您必須攔截所有例外狀況，會重新擲回的例外狀況，以確保 Windows 執行階段可以完成取消作業。  
 ...
 ![Windows 市集 Primes 應用程式](../../parallel/concrt/media/concrt_windows_primes.png "concrt_windows_primes")  
...
>  這個範例會示範如何指定執行內容以及如何撰寫接續鏈結。 前面說過，根據預設，從非同步作業建立的工作會在呼叫 `task::then`的 Apartment 上執行接續。 因此，這個範例會使用 `task_continuation_context::use_arbitrary` 指定不涉及 UI 的作業將在背景執行緒上執行。  
 ...
 ![Windows 市集 CommonWords 應用程式](../../parallel/concrt/media/concrt_windows_common_words.png "concrt_windows_common_words")  
