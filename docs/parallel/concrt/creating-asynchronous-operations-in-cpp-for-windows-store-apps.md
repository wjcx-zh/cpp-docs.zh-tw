...
本文件說明一些重要觀念，當您使用 task 類別產生以 Windows 執行緒集區為基礎的通用 Windows 執行階段 (UWP) 應用程式中的非同步作業，請記住。  
 ...
 使用非同步程式設計是 Windows 執行階段應用程式模型的重要元件，因為它可讓應用程式仍能繼續回應使用者輸入。 您可以啟動長時間執行的工作，而不封鎖 UI 執行緒，而且可以稍後再接收工作的結果。 工作在背景執行時，您也可以取消工作和接收進度通知。 文件[c + + 中的非同步程式設計](/windows/uwp/threading-async/asynchronous-programming-in-cpp-universal-windows-platform-apps)提供可用來建立 UWP 應用程式的 Visual c + + 中的非同步模式概觀。 該文件教您如何使用及建立非同步 Windows 執行階段作業的鏈結。 本章節描述如何使用 ppltasks.h 中的型別，產生可由另一個 Windows 執行階段元件使用的非同步作業以及如何控制如何非同步工作會執行。 另請閱讀[非同步程式設計模式和祕 Hilo （使用 c + + 和 XAML Windows 市集應用程式）](http://msdn.microsoft.com/library/windows/apps/jj160321.aspx)若要了解我們如何使用工作類別來實作非同步作業，使用 c + + 和 XAML 的 Windows 執行階段應用程式。  
  
> [!NOTE]
>  您可以使用[平行模式程式庫](../../parallel/concrt/parallel-patterns-library-ppl.md)(PPL) 和[非同步代理程式程式庫](../../parallel/concrt/asynchronous-agents-library.md)的 UWP 應用程式。 不過，您無法使用工作排程器或資源管理員。 本文件說明其他 PPL 提供的可用功能只有在 UWP 應用程式，以及不桌面應用程式。  
...
-   [範例： 控制在 Windows 執行階段應用程式中使用 c + + 和 XAML 的執行](#example-app)  
...
##  <a name="create-async"></a> 建立非同步作業  
 ...
 Windows 執行階段是程式設計介面，可用來建立只在特殊作業系統環境中執行的 UWP 應用程式。 這類應用程式使用授權的函式、 資料類型及裝置，並會從 Microsoft 市集散發。 Windows 執行階段由*應用程式二進位介面*(ABI)。 ABI 是讓 Visual c + + 等程式設計語言中可用的 Windows 執行階段 Api 的基礎二進位合約。  
  
 藉由使用 Windows 執行階段，您可以使用各種程式設計語言的最佳功能，並將它們合併到單一應用程式。 例如，您可能在 JavaScript 中建立 UI，並且在 C++ 元件中執行密集運算的應用程式邏輯。 在背景執行這些密集運算作業的能力，就是讓 UI 保持回應的主要因素。 因為`task`類別為 c + + 專屬，因此您必須使用 Windows 執行階段介面來與其他元件 （可能以 c + + 以外的語言撰寫） 溝通非同步作業。 Windows 執行階段提供可用來代表非同步作業的四個介面：  
...
|`IAsyncActionOperationWithProgress<TProgress, TProgress>`|`T` 或 `task<T>`|(`progress_reporter`)|(`progress_reporter`, `cancellation_token`)|  
 ...
 下列範例示範建立的各種方式`IAsyncAction`可由另一個 Windows 執行階段元件的物件。  
  
 [!code-cpp[concrt-windowsstore-primes#100](../../parallel/concrt/codesnippet/cpp/creating-asynchronous-operations-in-cpp-for-windows-store-apps_1.cpp)]  
  
##  <a name="example-component"></a> 範例: 建立 C ++ Windows 執行階段元件並從 C# 使用它  
 請考慮使用 XAML 和 C# 來定義 UI 和 c + + Windows 執行階段元件以執行密集運算作業的應用程式。 在這個範例中，C++ 元件會計算某個範圍中哪些數字是質數。 為了說明四個 Windows 執行階段非同步工作介面之間的差異，開始，在 Visual Studio 中，藉由建立**空白方案**並將它命名`Primes`。 然後在方案中新增 [Windows 執行階段元件]  專案，並將它命名為 `PrimesLibrary`。 將下列程式碼加入至產生的 C++ 標頭檔 (這個範例會將 Class1.h 重新命名為 Primes.h)。 每個 `public` 方法都會定義四個非同步介面的其中一個。 傳回值的方法傳回[Windows::Foundation::Collections::IVector\<int >](http://msdn.microsoft.com/library/windows/apps/br206631.aspx)物件。 報告進度的方法會產生 `double` 值，用以定義整體工作已完成的百分比。  
...
>  依照慣例，通常以"Async"結尾 Windows 執行階段中的非同步方法名稱。  
 ...
 在非同步作業完成後，這些方法會使用 `async` 和 `await` 關鍵字更新 UI。 如需非同步 UWP 應用程式中撰寫程式碼，請參閱[執行緒和非同步程式設計](/windows/uwp/threading-async)。  
  
 `getPrimesCancellation` 和 `cancelGetPrimes` 方法可一起使用，讓使用者取消作業。 當使用者選擇**取消** 按鈕，`cancelGetPrimes`方法呼叫[IAsyncOperationWithProgress\<Iasyncoperationwithprogress<tresult，TProgress >:: 取消](http://msdn.microsoft.com/library/windows/apps/windows.foundation.iasyncinfo.cancel.aspx)取消作業。 並行執行階段，這是管理基礎非同步作業，會擲回通訊取消已完成 Windows 執行階段被攔截的內部例外狀況類型。 如需取消模型的詳細資訊，請參閱[取消](../../parallel/concrt/cancellation-in-the-ppl.md)。  
  
> [!IMPORTANT]
>  若要讓 PPL 正確回報給 Windows 執行階段就已取消該作業，不要攔截這個內部例外狀況類型。 這表示，您不應該攔截所有例外狀況 (`catch (...)`)。 如果您必須攔截所有例外狀況，會重新擲回的例外狀況，以確保 Windows 執行階段可以完成取消作業。  
...
##  <a name="exethread"></a> 控制執行緒  
 ...
 在有 UI 的應用程式中，ASTA (應用程式 STA) 執行緒負責提取視窗訊息，而且是處理序中唯一可更新 STA 裝載 UI 控制項的執行緒。 這有兩種結果。 首先，為了讓應用程式保持回應，所有 CPU 密集和 I/O 作業都不應該在 ASTA 執行緒上執行。 其次，來自背景執行緒的結果必須封送處理回 ASTA 才能更新 UI。 在 c + + UWP 應用程式中，`MainPage`和其他的 XAML 頁面全都是在 ATSA 上執行。 因此根據預設，在 ASTA 上宣告的工作接續都會在該處執行，所以您可以直接在接續主體中更新控制項。 不過，如果您是以巢狀方式處理工作，則該巢狀工作上的任何接續都會在 MTA 中執行。 因此，您需要考慮是否要明確指定這些接續要在哪些內容上執行。  
...
> [!IMPORTANT]
>  UWP 應用程式的主要 UI 執行緒在 STA 下執行，因為依預設該 STA 上所建立的接續在 STA 上執行 同樣地，您在 MTA 上建立的接續工作也會在 MTA 上執行。  
