...
如需使用自訂的排程器原則，以及非同步訊息區的基本範例，請參閱 <<c0> [ 如何： 指定特定的排程器原則](../../parallel/concrt/how-to-specify-specific-scheduler-policies.md)。
...
若要提供`printer`類別公平存取運算資源，此範例會使用所述的步驟[如何： 管理排程器執行個體](../../parallel/concrt/how-to-manage-a-scheduler-instance.md)建立具有自訂原則的排程器執行個體。 自訂原則指定的執行緒優先順序最高的優先權類別。
