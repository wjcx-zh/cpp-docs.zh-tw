...
資料管線中的階段通常會輸出其類型與輸入值不同的值。 在此範例中，第二個階段會採用類型 `int` 的值做為其輸入，並產生該值的平方根（`double`）做為其輸出。
...
複製範例程式碼，並將它貼入 Visual Studio 專案中，或貼入名為 `data-pipeline.cpp` 的檔案中，然後在 [Visual Studio 命令提示字元] 視窗中執行下列命令。

> **cl/EHsc data-pipeline.cpp .cpp**
