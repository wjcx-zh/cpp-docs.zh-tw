...
`parallel_for_each` 會平行套用指定的函式到範圍內的每個項目。 在語意上，它相當於 `for_each` 命名空間中的 `std` 函式，但項目的反覆項目會平行執行，而且不會指定反覆項目的順序。 `_Func` 引數必須支援 `operator()(T)` 形式的函式呼叫運算子，其中 `T` 參數是要逐一查看之容器的項目類型。
...
範本函式是在容器類別上特製化的演算法，`concurrent_vector` 來執行成員函式 `_A`。 [concurrent_vector：： swap](concurrent-vector-class.md#swap)（`_B`）。 這是編譯器所執行函式樣板部分排序的執行個體。 當因範本函式多載而導致範本與函式呼叫的比對不是唯一的時，編譯器就會選取最特製化的範本函式版本。 在演算法類別中，樣板函式的一般版本（`template <class T> void swap(T&, T&)`）會依指派運作，而且是緩慢的作業。 每個容器中的特製化版本運作速度會更快，因為它可以與容器類別的內部表示法一起運作。
...
如果其中一個工作已取消或擲回例外狀況，則傳回的工作將會提早完成（處於已取消狀態），而且如果您在該工作上呼叫[task：： get](task-class.md#get)或 `task::wait`，就會擲回例外狀況（如果發生的話）。
