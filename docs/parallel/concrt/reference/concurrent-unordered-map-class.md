...
|名稱|說明|
|----------|-----------------|
|[at](#at)|已多載。 使用指定的索引鍵值，尋找中的專案 `concurrent_unordered_map` 。。 這個方法是並行安全的。|
|[hash_function](#hash_function)|取得儲存的雜湊函式物件。|
|[insert](#insert)|已多載。 將元素加入至 `concurrent_unordered_map` 物件。|
|[key_eq](#key_eq)|取得預存的相等比較函式物件。|
|[調換](#swap)|交換兩個 `concurrent_unordered_map` 物件的內容。 這個方法不是並行安全的。|
|[unsafe_erase](#unsafe_erase)|已多載。 從中的 `concurrent_unordered_map` 指定位置移除元素。 這個方法不是並行安全的。|

### <a name="public-operators"></a>公用運算子

|名稱|說明|
|----------|-----------------|
|[操作\[\]](#operator_at)|已多載。 尋找或插入具有指定索引鍵的項目。 這個方法是並行安全的。|
|[operator =](#operator_eq)|已多載。 將另一個物件的內容指派 `concurrent_unordered_map` 給這個物件。 這個方法不是並行安全的。|

## <a name="remarks"></a>備註

如需類別的詳細資訊 `concurrent_unordered_map` ，請參閱[平行容器和物件](../../../parallel/concrt/parallel-containers-and-objects.md)。
...
使用指定的索引鍵值，尋找中的專案 `concurrent_unordered_map` 。。 這個方法是並行安全的。
...
`operator[]`可以用來將專案插入對應中 `m` `m[key] = DataValue;` ，其中是索引 `DataValue` 鍵值為之元素的值 `mapped_type` `key` 。

當使用 `operator[]` 插入項目時，傳回的參考不會指出插入是變更預先存在的項目，還是建立新的項目。 成員函式 `find` 和[insert](#insert)可以用來判斷具有指定之索引鍵的專案是否已經存在於插入之前。

## <a name="operator"></a><a name="operator_eq"></a>operator =

將另一個物件的內容指派 `concurrent_unordered_map` 給這個物件。 這個方法不是並行安全的。
...
這個物件的參考 `concurrent_unordered_map` 。
...
從中的 `concurrent_unordered_map` 指定位置移除元素。 這個方法不是並行安全的。
...
前兩個成員函式會傳回反覆運算器，指定移除任何元素之後剩餘的第一個元素， `concurrent_unordered_map::end` 如果沒有這類元素，則傳回（）。 第三個成員函式會傳回它所移除的元素數目。
