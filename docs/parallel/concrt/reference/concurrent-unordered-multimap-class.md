...
#### <a name="parameters"></a>參數  
 ...
 `_Allocator_type`  
 表示封裝有關配置和解除配置之記憶體的並行向量的詳細資訊的預存配置器物件的型別。 這個引數是選擇性的預設值是`std::allocator<std::pair<``K`， `_Element_type``>>`。  
...
### <a name="return-value"></a>傳回值  
 參考`concurrent_unordered_multimap`物件。  
...
### <a name="return-value"></a>傳回值  
 前兩個成員函式會傳回迭代器，以指定的任何項目移除，剩餘的第一個項目或`concurrent_unordered_multimap::end`（)，如果沒有這類項目。 第三個成員函式傳回的項目數，它會移除。  
