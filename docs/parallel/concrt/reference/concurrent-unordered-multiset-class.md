...
`concurrent_unordered_multiset`類別是並行安全容器，可控制變動長度的項目序列的型別 k。序列表示方式，可讓並行安全附加項目存取、 迭代器存取和迭代器周遊作業。  
...
#### <a name="parameters"></a>參數  
 ...
 `_Allocator_type`  
 表示封裝有關配置和解除配置之記憶體的並行向量的詳細資訊的預存配置器物件的型別。 這個引數是選擇性的，而且預設值是 `std::allocator<``K``>`。  
...
### <a name="return-value"></a>傳回值  
 參考`concurrent_unordered_multiset`物件。  
