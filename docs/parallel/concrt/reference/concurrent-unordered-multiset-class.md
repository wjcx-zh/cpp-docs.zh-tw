...
|[unsafe_erase](#unsafe_erase)|已多載。 從 `concurrent_unordered_multiset` 中的指定位置移除元素。 這個方法不是並行安全的。|

### <a name="public-operators"></a>公用運算子

|名稱|描述|
|----------|-----------------|
|[operator=](#operator_eq)|已多載。 將另一個 `concurrent_unordered_multiset` 物件的內容指派給這一個。 這個方法不是並行安全的。|
