---
title: concurrent_unordered_multiset 類別 |Microsoft 文件
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-concrt
...
ms.locfileid: "33694793"
---
# <a name="concurrentunorderedmultiset-class"></a>concurrent_unordered_multiset 類別
`concurrent_unordered_multiset`類別是一種並行安全容器，可控制不同長度序列 K.型別的項目序列的表示方式啟用並行安全附加、 項目存取、 迭代器存取及迭代器周遊作業。  
...
#### <a name="parameters"></a>參數  
 ...
 `_Allocator_type`  
 表示封裝有關配置和解除配置記憶體的並行向量的詳細資訊的預存配置器物件的型別。 這個引數是選擇性的，而且預設值是 `std::allocator<K>`。  
...
|名稱|描述|  
|----------|-----------------|  
|[operator=](#operator_eq)|多載。 另一個內容指派`concurrent_unordered_multiset`給這一個物件。 這個方法不是並行安全。|  
...
##  <a name="operator_eq"></a> 運算子 = 
 ...
 另一個內容指派`concurrent_unordered_multiset`給這一個物件。 這個方法不是並行安全。  
