...
|Name|描述|
|----------|-----------------|
|[concurrent_unordered_set](#ctor)|多載。 構造並行未排序的集合。|
...
|[insert](#insert)|多載。 將元素加入至 `concurrent_unordered_set` 物件。|
|[key_eq](#key_eq)|傳回預存的相等比較函式物件。|
|[swap](#swap)|交換兩個 `concurrent_unordered_set` 物件的內容。 這個方法不是並行安全的。|
|[unsafe_erase](#unsafe_erase)|多載。 從 `concurrent_unordered_set` 中的指定位置移除元素。 這個方法不是並行安全的。|

### <a name="public-operators"></a>公用運算子

|Name|描述|
|----------|-----------------|
|[operator=](#operator_eq)|多載。 將另一個 `concurrent_unordered_set` 物件的內容指派給這一個。 這個方法不是並行安全的。|

## <a name="remarks"></a>備註

如需 `concurrent_unordered_set` 類別的詳細資訊，請參閱[平行容器和物件](../../../parallel/concrt/parallel-containers-and-objects.md)。
...
將另一個 `concurrent_unordered_set` 物件的內容指派給這一個。 這個方法不是並行安全的。
...
這個 `concurrent_unordered_set` 物件的參考。
...
從 `concurrent_unordered_set` 中的指定位置移除元素。 這個方法不是並行安全的。
