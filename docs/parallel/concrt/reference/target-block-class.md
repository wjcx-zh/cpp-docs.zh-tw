...
|名稱|說明|  
|----------|-----------------|  
|[target_block 建構函式](#ctor)|建構 `target_block` 物件。|  
...
|[send 方法](#send)|以同步方式將訊息從來源區塊傳遞到此目標區塊。|  
  
### <a name="protected-methods"></a>受保護的方法  
  
|名稱|說明|  
|----------|-----------------|  
|[async_send 方法](#async_send)|以非同步方式傳送訊息以進行處理。|  
...
|[initialize_target 方法](#initialize_target)|初始化基底物件。 具體來說，`message_processor`需要初始化物件。|  
|[link_source 方法](#link_source)|將指定的來源區塊連結至這個`target_block`物件。|  
|[process_input_messages 方法](#process_input_messages)|處理收到的輸入訊息。|  
|[process_message 方法](#process_message)|在衍生類別中覆寫時，處理這個 `target_block` 物件接受的訊息。|  
|[propagate_message 方法](#propagate_message)|當在衍生類別中覆寫時，這個方法以非同步方式將訊息傳遞從`ISource`區塊至此`target_block`物件。 它會叫用`propagate`方法，由來源區塊呼叫時。|  
|[register_filter 方法](#register_filter)|註冊將會叫用每個收到的訊息的篩選方法。|  
|[remove_sources 方法](#remove_sources)|等候未處理的非同步傳送作業完成之後，取消連結所有來源。|  
|[send_message 方法](#send_message)|當在衍生類別中覆寫時，這個方法以同步方式將訊息傳遞從`ISource`區塊至此`target_block`物件。 它會叫用`send`方法，由來源區塊呼叫時。|  
|[sync_send 方法](#sync_send)|以同步方式傳送訊息以進行處理。|  
|[unlink_source 方法](#unlink_source)|取消連結指定的來源區塊從此`target_block`物件。|  
|[unlink_sources 方法](#unlink_sources)|取消連結所有來源區塊，從這個`target_block`物件。 (覆寫[itarget:: Unlink_sources](itarget-class.md#unlink_sources)。)|  
|[wait_for_async_sends 方法](#wait_for_async_sends)|等候所有完成的非同步傳播。|  
...
##  <a name="a-namepropagatemessagea-propagatemessage"></a><a name="propagate_message"></a>propagate_message 
 ...
 當在衍生類別中覆寫時，這個方法以非同步方式將訊息傳遞從`ISource`區塊至此`target_block`物件。 它會叫用`propagate`方法，由來源區塊呼叫時。  
