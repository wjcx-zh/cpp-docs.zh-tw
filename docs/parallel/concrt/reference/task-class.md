...
ms.locfileid: "77142621"
---
# <a name="task-class-concurrency-runtime"></a>task 類別 (並行執行階段)

平行模式程式庫 (PPL) `task` 類別。 `task` 物件代表可以在並行執行階段中以非同步方式和其他工作執行，以及平行演算法所產生的平行工作。 成功完成時，會產生 `_ResultType` 類型的結果。 `task<void>` 類型的工作不會產生任何結果。 工作可以獨立於其他工作，個別等候及取消。 也可以使用接續（`then`）和聯結（`when_all`）和選擇（`when_any`）模式，與其他工作一起撰寫。 將 task 物件指派給新的變數時，其行為會是 `std::shared_ptr`的;換句話說，這兩個物件都代表相同的基礎工作。
...
|[Promise.Promise.then 方法 方法](#then)|已多載。 將接續工作加入至此工作。|
...
如果工作已取消，對 `get` 的呼叫將會擲回[task_canceled](task-canceled-class.md)例外狀況。 如果工作發生不同的例外狀況，或例外狀況從前項工作傳播至它，則呼叫 `get` 將會擲回該例外狀況。

> [!IMPORTANT]
> 在通用 Windows 平臺（UWP）應用程式中，請勿在使用者介面執行緒上執行的程式碼中，呼叫[concurrency：： task：： wait](#wait)或 `get` （`wait` 呼叫 `get`）。 否則，執行時間會擲回[concurrency：： invalid_operation](invalid-operation-class.md) ，因為這些方法會封鎖目前的執行緒，而且可能會導致應用程式沒有回應。 不過，您可以呼叫 `get` 方法，在以工作為基礎的接續中接收 antecedent 工作的結果，因為結果會立即可用。
...
如果工作解除包裝 `IAsyncInfo` 介面或從這類工作衍生，則**為 true** ，否則為**false** 。
...
*_Param*<br/>
從中要建構工作的參數。 如果您在 Windows 執行階段應用程式中使用工作，這可能是 lambda、函式物件、`task_completion_event<result_type>` 物件或 Windows：： Foundation：： IAsyncInfo。 Lambda 或函式物件應該是相當於 `std::function<X(void)>`的類型，其中 X 可以是類型 `result_type`、`task<result_type>`或 Windows 執行階段應用程式中的 Windows：： Foundation：： IAsyncInfo 的變數。
...
出現 `task` 的預設建構函式，只是為了讓工作在容器內使用。 只有在指派有效的工作給預設建構的工作之後，您才能使用它。 `get`、`wait` 或 `then` 之類的方法，會在預設的結構工作上呼叫時擲回[invalid_argument](../../../standard-library/invalid-argument-class.md)例外狀況。
...
從 `Windows::Foundation::IAsyncInfo` 介面或從傳回 `IAsyncInfo` 介面的 Lambda 建立的工作，當內含的 Windows 執行階段非同步作業或動作完成時，該工作會到達其終止狀態。 同樣地，從傳回 `task<result_type>` 的 lambda 建立的工作，會在內部工作到達其終止狀態時達到其終止狀態，而不會在 lambda 傳回時到達。
...
採用 lambda 或仿函數（可傳回 Windows：： Foundation：： IAsyncInfo 介面）之 `then` 的多載僅適用于 Windows 執行階段應用程式。
...
> [!IMPORTANT]
> 在通用 Windows 平臺（UWP）應用程式中，請勿呼叫在使用者介面執行緒上執行之程式碼中的 `wait`。 否則，因為這個方法會封鎖目前的執行緒，而且可能會導致應用程式沒有回應，所以執行階段會擲回 [concurrency::invalid_operation](invalid-operation-class.md) 。 不過，您可以呼叫 [concurrency::task::get](#get) 方法來以工作為基礎連續的形式接收前項工作的結果。
