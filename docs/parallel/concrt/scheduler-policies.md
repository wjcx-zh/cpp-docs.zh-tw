...
當您使用[concurrency:: CurrentScheduler:: create](reference/currentscheduler-class.md#create)、 [concurrency:: 排程器:: create](reference/scheduler-class.md#create)或[concurrency::](reference/scheduler-class.md#setdefaultschedulerpolicy)排程器:: SetDefaultSchedulerPolicy 方法來建立排程器實例時, 您會提供[concurrency::SchedulerPolicy](../../parallel/concrt/reference/schedulerpolicy-class.md)物件, 其中包含指定排程器行為的機碼值組集合。 此`SchedulerPolicy`函式會採用可變數目的引數。 第一個引數是您即將指定的原則元素數目。 其餘的引數是每個原則元素的機碼值組。 下列範例會建立`SchedulerPolicy`指定三個原則元素的物件。 對於未指定的原則機碼，執行階段會使用預設值。
...
每個排程器會在排程工作時, 使用自己的原則。 與某個排程器相關聯的原則並不會影響任何其他排程器的行為。 此外, 您無法在建立`Scheduler`物件之後變更排程器原則。
...
若要檢查使用特定排程器原則來控制排程器行為的範例, 請[參閱如何:指定特定排程器](../../parallel/concrt/how-to-specify-specific-scheduler-policies.md)原則[和如何:建立使用特定排程器原則](../../parallel/concrt/how-to-create-agents-that-use-specific-scheduler-policies.md)的代理程式。
