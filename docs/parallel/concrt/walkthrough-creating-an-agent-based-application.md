...
1. 在函式中 `_tmain` ，建立[concurrency：： event](../../parallel/concrt/reference/event-class.md)物件，以指示處理結束。
...
若要防止平行存取資料成員，建議您將執行工作的方法新增至類別的 **`protected`** 或 **`private`** 區段。 只會將傳送或接收訊息的方法加入至 **`public`** 類別的區段。
