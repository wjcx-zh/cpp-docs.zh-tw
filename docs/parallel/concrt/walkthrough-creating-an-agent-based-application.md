...
ms.locfileid: "69631654"
---
# <a name="walkthrough-creating-an-agent-based-application"></a>逐步解說：建立以代理程式為基礎的應用程式
...
1. 從專案類型的篩選清單中，選擇 [主控台應用程式]，然後選擇 [下一步]。 在下一個頁面中, `BasicAgent`輸入做為專案的名稱, 並視需要指定專案位置。

1. 選擇 [建立] 按鈕以建立專案。

1. 以滑鼠右鍵按一下**方案總管**中的專案節點, 然後選擇 [**屬性**]。 在 [設定**屬性** >  **CC++/**  > 先行編譯**頭** > 先行**編譯頭**檔] 下, 選擇 [**建立**]
...
1. 在 [**新增專案**] 對話方塊中, 選取 [**專案類型**] 窗格中的**視覺效果C++** 節點, 然後在 [**範本**] 窗格中選取 [ **Win32 主控台應用程式**]。 輸入專案的名稱, `BasicAgent`例如, 然後按一下 **[確定]** 以顯示 [ **Win32 主控台應用程式]** 。
...
1. 將新的C++標頭檔新增至您的專案。 若要這樣做, 請在**方案總管**中的 [**標頭檔**] 節點上按一下滑鼠右鍵, 按一下 [**加入**], 然後按一下 [**新增專案**]。 在 [**範本**] 窗格中, 選取 [**標頭檔 (.h)** ]。 在 [**加入新專案**] 對話方塊的 [ `file_reader.h` **名稱**] 方塊中輸入, 然後按一下 [**新增**]。

1. 在 file_reader 中, 新增下列程式碼。

[!code-cpp[concrt-basic-agent#17](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_2.h)]

1. 在 file_reader 中, 建立一個名`file_reader`為且衍生自`agent`的類別。
...
[!code-cpp[concrt-basic-agent#3](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_4.h)]
   ...
   `_file_name`成員是代理程式讀取來源的檔案名。 成員是一個[concurrency:: ITarget](../../parallel/concrt/reference/itarget-class.md)物件, 代理程式會將檔案的內容寫入其中。 `_target` `_error`成員會保存代理程式生命週期期間所發生的任何錯誤。

1. 將下列程式`file_reader`代碼加入`public`至`file_reader`類別的區段。

[!code-cpp[concrt-basic-agent#4](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_5.h)]

   每個函數的`file_reader`多載都會設定資料成員。 第二個和第三個函式多載可讓您的應用程式使用特定排程器搭配您的代理程式。 第一個多載會將預設排程器與您的代理程式搭配使用。

1. 將方法新增至`file_reader`類別的公用區段。 `get_error`
...
1. 在函式中, 建立[concurrency:: event](../../parallel/concrt/reference/event-class.md)物件, 以指示處理結束。 `_tmain`

[!code-cpp[concrt-basic-agent#10](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_11.cpp)]

1. `call`建立物件, 以在收到資料時更新總和檢查碼。

[!code-cpp[concrt-basic-agent#11](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-an-agent-based-application_12.cpp)]

   這個`call`物件也會在`event`收到空字串時設定物件, 以表示處理結束。

1. 建立從檔案 test.txt 讀取的`call` 物件,並將該檔案的內容寫入物件。`file_reader`
...
若要防止平行存取資料成員, 建議您將執行工作的方法新增至類別的`protected`或`private`區段。 只會將傳送或接收訊息`public`的方法加入至類別的區段。

一律呼叫[concurrency:: agent::d 一種](reference/agent-class.md#done)方法, 將代理程式移至已完成狀態。 您通常會先呼叫這個方法, 然後再`run`從方法傳回。

## <a name="next-steps"></a>後續步驟

如需以代理程式為基礎的另一個範例[, 請參閱逐步解說:使用聯結來避免死](../../parallel/concrt/walkthrough-using-join-to-prevent-deadlock.md)鎖。
