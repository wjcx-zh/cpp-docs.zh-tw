...
##  <a name="problem"></a> 哲學家用餐問題  
 ...
 若要吃，哲學家必須具有兩個筷子。 如果每個哲學家保存一個 chopstick，而且正在等候另一個，然後可以吃沒有哲學家和所有變空。  
...
##  <a name="deadlock"></a> 貝氏實作  
 ...
 A`philosopher`物件鎖定兩者`critical_section`物件來模擬從之前的持有者筷子的權限移除呼叫`eat`方法。 在呼叫之後`eat`、`philosopher`物件傳回給持有者藉由設定筷子`critical_section`回解除鎖定狀態的物件。  
  
 `pickup_chopsticks`方法說明便會發生死結。 如果每個`philosopher`物件取得存取權的鎖定，其中一個則否`philosopher`物件可以繼續，因為其他鎖定由另一個`philosopher`物件。  
...
## <a name="compiling-the-code"></a>編譯程式碼  
 範例程式碼複製並將它貼入 Visual Studio 專案中，或將它貼入名為的檔案中`philosophers-deadlock.cpp`，然後在 Visual Studio 命令提示字元視窗中執行下列命令。  
...
##  <a name="solution"></a> 使用聯結以避免死結  
 ...
 這個範例會使用`unbounded_buffer`類別，因為當目標會接收來自訊息`unbounded_buffer`物件，從訊息佇列中移除訊息。 這可讓`unbounded_buffer`chopstick 保存訊息指出的物件可供使用。 `unbounded_buffer`不保存任何訊息的物件表示 chopstick 正在使用。  
  
 這個範例會使用非窮盡`join`物件，因為非窮盡聯結可讓每個`philosopher`存取這兩個筷子的物件時，才兩者`unbounded_buffer`物件包含一則訊息。 窮盡聯結不會使死結，因為窮盡聯結接受訊息，因為它們變成可用。 如果所有窮盡，便會發生死結`join`物件收到訊息，但是永遠等候彼此變成可用。  
  
 如需窮盡和非窮盡聯結，以及各種訊息緩衝區類型之間的差異的詳細資訊，請參閱[非同步訊息區](../../parallel/concrt/asynchronous-message-blocks.md)。  
...
2.  變更的類型`_left`和`_right`資料成員`philosopher`類別`unbounded_buffer`。  
  
 [!code-cpp[concrt-philosophers-join#2](../../parallel/concrt/codesnippet/cpp/walkthrough-using-join-to-prevent-deadlock_3.cpp)]  
  
3.  修改`philosopher`建構函式才能`unbounded_buffer`物件做為其參數。  
  
 [!code-cpp[concrt-philosophers-join#3](../../parallel/concrt/codesnippet/cpp/walkthrough-using-join-to-prevent-deadlock_4.cpp)]  
  
4.  修改`pickup_chopsticks`方法來使用非窮盡`join`從這兩個筷子的訊息緩衝區接收訊息的物件。  
  
 [!code-cpp[concrt-philosophers-join#4](../../parallel/concrt/codesnippet/cpp/walkthrough-using-join-to-prevent-deadlock_5.cpp)]  
  
5.  修改`putdown_chopsticks`方法，以釋放所傳送訊息至訊息緩衝區，這兩個筷子筷子的存取。  
  
 [!code-cpp[concrt-philosophers-join#5](../../parallel/concrt/codesnippet/cpp/walkthrough-using-join-to-prevent-deadlock_6.cpp)]  
  
6.  修改`run`方法，以保存結果的`pickup_chopsticks`方法以及傳遞這些結果`putdown_chopsticks`方法。  
  
 [!code-cpp[concrt-philosophers-join#6](../../parallel/concrt/codesnippet/cpp/walkthrough-using-join-to-prevent-deadlock_7.cpp)]  
  
7.  修改的宣告`chopsticks`變數中`wmain`函式的陣列`unbounded_buffer`每一個訊息保留的物件。  
...
### <a name="description"></a>描述  
 下列範例示範完成的範例使用非窮盡`join`物件以避免發生死結的風險。  
...
## <a name="compiling-the-code"></a>編譯程式碼  
 範例程式碼複製並將它貼入 Visual Studio 專案中，或將它貼入名為的檔案中`philosophers-join.cpp`，然後在 Visual Studio 命令提示字元視窗中執行下列命令。  
  
 **cl.exe /EHsc 用餐 join.cpp**  
