...
|[OnIdle](../mfc/reference/cwinthread-class.md#onidle)|執行特定執行緒的閒置時間處理。 通常不會覆寫。 |  
|[PreTranslateMessage](../mfc/reference/cwinthread-class.md#pretranslatemessage)|篩選訊息，再將它們分派至`TranslateMessage`和`DispatchMessage`。 通常不會覆寫。 |  
|[ProcessWndProcException](../mfc/reference/cwinthread-class.md#processwndprocexception)|攔截未處理的執行緒的訊息和命令處理常式所擲回的例外狀況。 通常不會覆寫。 |  
|[執行](../mfc/reference/cwinthread-class.md#run)|控制執行緒的函式。 包含訊息幫浦。 幾乎不覆寫。 |  
