...
有了 MSVC, 有幾種方式可以使用多個執行緒來進行程式設計:您可以使用C++/WinRT 和 Windows 執行階段程式庫、Microsoft Foundation CLASS (MFC) 程式庫、 C++/cli 和 .Net Runtime, 或 C 執行時間程式庫和 WIN32 API。 本文是關於 C 中的多執行緒處理。如需範例程式碼, 請參閱[C 中的範例多執行緒程式](sample-multithread-c-program.md)。
...
CRT 程式庫提供下列函式來建立和終止執行緒: [_beginthread、_beginthreadex](../c-runtime-library/reference/beginthread-beginthreadex.md)、 [_endthread 和 _endthreadex](../c-runtime-library/reference/endthread-endthreadex.md)。

`_beginthread` 和`_beginthreadex`函式會建立新的執行緒, 並在作業成功時傳回執行緒識別碼。 執行緒會在完成執行時自動終止。 或者, 它可以透過呼叫或`_endthread` `_endthreadex`來終止自己。

> [!NOTE]
> 如果您從以 libcmt.lib 建立的程式呼叫 C 執行時間常式, 就必須使用`_beginthread`或`_beginthreadex`函式來啟動您的執行緒。 請勿使用 Win32 函數`ExitThread`和。 `CreateThread` 當`SuspendThread`多個執行緒被封鎖, 等候暫止的執行緒完成其對 C 執行時間資料結構的存取時, 使用可能會導致鎖死。

###  <a name="_core_the__beginthread_function"></a>_Beginthread 和 _beginthreadex 函式

`_beginthread` 和`_beginthreadex`函式會建立新的執行緒。 執行緒會與進程中的其他執行緒共用進程的程式碼和資料區段, 但有它自己的唯一暫存器值、堆疊空間和目前的指令位址。 系統會提供 CPU 時間給每個執行緒, 以便處理常式中的所有線程都可以同時執行。
...
[_Endthread](../c-runtime-library/reference/endthread-endthreadex.md)函式會終止由建立的`_beginthread`執行緒 (同樣地`_endthreadex` , 終止所`_beginthreadex`建立的執行緒)。 執行緒完成時, 會自動終止。 `_endthread`和`_endthreadex`適用于執行緒內的條件式終止。 例如, 專門用於通訊處理的執行緒若無法取得通訊埠的控制權, 就可以結束。
...
> [!NOTE]
> 如需 MFC 觀點的類似討論, 請參閱[多執行緒:程式設計](multithreading-programming-tips.md)提示[和多執行緒:使用同步處理類別](multithreading-when-to-use-the-synchronization-classes.md)的時機。
...
[範例多執行緒 c 程式](sample-multithread-c-program.md)中顯示的彈跳程式會使用名`ScreenMutex`為的 mutex 來協調螢幕更新。 每當其中一個顯示執行緒準備好寫入畫面時, 它會使用的`WaitForSingleObject` `ScreenMutex`控制碼和常數`WaitForSingleObject`無限地呼叫, 以指出呼叫應該在 mutex 上封鎖, 而不會超時。如果`ScreenMutex`明確, wait 函式會設定 mutex, 讓其他執行緒無法干擾顯示, 並繼續執行執行緒。 否則, 執行緒會封鎖, 直到 mutex 清除為止。 當執行緒完成顯示更新時, 它會呼叫`ReleaseMutex`來釋放 mutex。

畫面顯示, 而靜態資料只是兩個需要謹慎管理的資源。 例如, 您的程式可能會有多個執行緒存取相同的檔案。 因為另一個執行緒可能已移動檔案指標, 所以每個執行緒必須先重設檔案指標, 才能讀取或寫入。 此外, 每個執行緒都必須確定它在放置指標的時間和存取檔案的時間之間不會被佔用。 這些執行緒應該使用信號來協調檔案的存取, 方法是括弧每個檔案存取`WaitForSingleObject` , `ReleaseMutex`並呼叫。 下列程式碼範例說明這項技術:
...
`_beginthread`呼叫中的第一個引數是函式的`BounceProc`指標, 該函式會執行執行緒。 第二個引數會指定執行緒的預設堆疊大小。 最後一個引數是傳遞給`BounceProc`的識別碼編號。 `BounceProc`使用 ID 編號來植入亂數產生器, 並選取執行緒的 color 屬性和顯示字元。

呼叫 C 執行時間程式庫或 WIN32 API 的執行緒, 必須針對其所呼叫的程式庫和 API 函式, 允許足夠的堆疊空間。 C `printf`函式需要超過500個位元組的堆疊空間, 而且在呼叫 WIN32 API 常式時, 您應該會有2k 個位元組的堆疊空間可供使用。
...
建立、連結或執行多執行緒 C 程式時, 您可能會遇到幾個問題。 下表說明一些較常見的問題。 (如需 MFC 觀點的類似討論, 請參閱[多執行緒:程式設計](multithreading-programming-tips.md)提示)。
