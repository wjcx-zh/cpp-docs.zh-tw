...
```  
 ...
 由於有這麼多運算子 << definitions，這種錯誤可能會讓人心生畏懼。 在更仔細地查看可用的多載之後，我們發現大多數多載無關；接著更仔細地查看 `mstream` 類別定義，我們找出認為在這種情況下應該會呼叫的下列函式。  
...
```  
 ...
 Spy++ 程式碼中的訊息 WM_CTLCOLORDLG 已經過處理，因此唯一需要的變更是刪除不再需要之 SetDialogBkColor 的任何參考。  
...
```  
 ...
 由於連結器輸入中包含 MFC 的過時靜態程式庫版本，因此會發生 LNK1181。 不再需要這個版本，因為可以動態方式連結 MFC，所以我們只需要從專案屬性 [連結器] 區段的 [輸入] 屬性，移除所有 MFC 靜態程式庫。 這個專案也使用 /NODEFAULTLIB 選項，但卻列出所有程式庫相依性。  
...
```  
 ...
 以下是一些常見的案例：針對 memcpy，當切換為 memcpy_s 時，我們經常會加入正在複製的目標結構大小。 同樣地，針對大多數字串和緩衝區，藉由宣告緩衝區或尋找緩衝區的原始配置位置，即可輕鬆判斷陣列或緩衝區的大小。 在某些情況下，您必須判斷緩衝區實際可用的大小，如果您要修改的函式範圍中沒有該項資訊，則應該將其加入做為額外的參數，並且應該修改呼叫程式碼以提供該項資訊。  
