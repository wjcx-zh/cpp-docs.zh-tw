...
在 Visual Studio 2015 和更新版本中，隨著不斷地改善編譯器一致性，有時可能會變更編譯器解讀您現有原始程式碼的方式。 當發生這種情況時，可能會在您建置時發生不同或新的錯誤，甚至程式碼的行為與先前所建置的不同，而且看似正常運作。
...
- **/Zc:forScope- 選項**
   ...
   該選項通常是在非標準程式碼的迴圈變數應該已經超出範圍 (根據該標準) 之後，用來允許此非標準程式碼。 只有當您使用 `/Za` 選項編譯時才需要這樣做，因為在沒有指定 `/Za` 的情況下，一律允許在迴圈結束之後使用 for 迴圈變數。 如果您不在意標準一致性 (例如，如果您並不打算將程式碼設計成可移植到其他編譯器)，則可以關閉 `/Za` 選項 (或將**停用語言延伸模組**屬性設為**否**)。 如果您在意撰寫可攜式且符合標準的程式碼，就應該要重寫程式碼，將這類變數的宣告移至該迴圈外，使其符合此標準。
...
- **例外狀況處理**
   ...
   有幾個例外狀況處理的變更。 首先，例外狀況物件必須可複製或可移動。 下列程式碼在 Visual Studio 2013 中編譯，但不會在 Visual Studio 2015 中編譯：
...
- **placement new 與 delete**
   ...
   您已定義的 **placement delete** 運算子與調整大小的全域新 **delete** 運算子之間，因為函式簽章相符，而導致此問題發生。 對於任何 `size_t`placement new**和**delete**運算子，請考慮是否可以使用** 以外的其他類型。  請注意，`size_t` **typedef**的類型與編譯器相依;它是在視覺效果C++中不**帶正負號 int**的**typedef** 。 較佳的解決方案是使用這類列舉類型：
...
- **私用的虛擬基底類別與間接繼承**
   ...
   舊版編譯器允許衍生類別呼叫其*間接衍生*之 `private virtual` 基類的成員函式。 這個舊的行為不正確，而且不符合 C++ 標準。 編譯器不再接受以這種方式撰寫的程式碼，並會發出編譯器錯誤 C2280。
...
- 先行**編譯標頭檔（PCH）和不相符的 include 目錄**（只會影響 `/Wall` `/WX`）
   ...
   使用先行編譯的標頭檔 (PCH) 時，舊版編譯器接受 `-I` 與 `-Yc` 編譯之間編譯器的 include 目錄 (`-Yu`) 命令列引數不相符。 編譯器現在已不再接受以此方式撰寫的程式碼。   現在在使用 PCH 檔案時，編譯器會發出編譯器警告 CC4599 協助識別不相符的 include 目錄 (`-I`) 命令列引數。
...
### <a name="standard-template-library"></a>標準範本程式庫
...
- **支援值結構中的 Boxed 類型。**
   ...
   您現在可以使用可為 null 的欄位（例如，`IBox<int>^`，而不是**int**）來定義實數值型別。這表示欄位可以有值或等於**nullptr**。
...
- 影像和模型編輯器。 [影像編輯器] 和 [模型編輯器] 中的 [著色器設計工具] 現在支援建立使用者定義的篩選條件。
...
**nullptr 和 __nullptr 關鍵字。** MSVC 可讓您搭配使用 **nullptr** 關鍵字與機器碼或受控碼。 **nullptr** 關鍵字會指出物件控制代碼、內部指標或原生指標類型未指向物件。 當您使用 **編譯器選項時，編譯器會將**nullptr`/clr` 解譯為受控碼，不使用 `/clr` 選項時，則解譯為機器碼。
...
**ATL 控制項精靈。** ATL 控制項精靈不會自動填入 `ProgID` 欄位。 如果 ATL 控制項沒有 `ProgID`，其他工具可能無法加以使用。 [插入使用中控制項]`ProgID`**對話方塊，即為工具需要控控制項擁有** 的其中一例。 如需對話方塊的詳細資訊，請參閱**插入 ActiveX 控制項對話方塊**。
...
- 當您使用 `/Qimprecise_fwaits` 編譯器選項時，`/fp:except` 會移除 try 區塊內部的 fwait 命令。
...
- 在所有 `#using` 編譯中，`/clr` 指示詞現在都會隱含參考 mscorlib.dll。
- 已新增 _OPENMP 預先定義巨集。
...
- 在使用 `/LTCG` 所編譯的模組上，現在支援 `/clr` 連結器選項。 `/LTCG`也已更新，以支援特性指引最佳化。
...
- 已新增 integer 類型的 typedef `errno_t`。 只要函式傳回類型或參數處理了來自 `errno_t` 的錯誤碼，就會使用 `errno`。 `errno_t` 取代了 `errcode`。
...
- `fpos_t` 現在於 **下方 (C 程式碼) 以及手動設定**STDC`/Za` (C++ 程式碼) 時，都是 ____int64__。 過去則是 **struct**。
