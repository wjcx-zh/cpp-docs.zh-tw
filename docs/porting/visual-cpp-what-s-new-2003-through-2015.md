...
- **例外狀況處理**
   ...
   問題是出在於此複製建構函式是私用的，無法如同處理例外狀況的正常過程中發生的物件一樣複製，因此並不能複製該物件。 這同樣適用于宣告複製的函式時 **`explicit`** 。
...
**流覽至。** [巡覽至]**** 對話方塊可讓您搜尋專案中符合指定字串的所有符號和檔案。 當您在搜尋字串中鍵入其他字元時，會立即修訂搜尋結果。 [結果]**** 回饋欄位會告知找到的項目數，並協助決定是否限制搜尋。 [種類/範圍]****、[位置]**** 和 [預覽]**** 回饋欄位可協助您釐清具有類似名稱的項目。 此外，您可以延伸這項功能，來支援其他程式設計語言。
