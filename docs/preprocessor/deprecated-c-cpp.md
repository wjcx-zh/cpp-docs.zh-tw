...
我們建議您盡可能使用 c + + 14 `[[deprecated]]` 屬性，而不是已被**取代**的 pragma。 在許多情況下，Microsoft 專有的[__declspec （已淘汰）](../cpp/deprecated-cpp.md)宣告修飾詞也是更好的選擇，而不是已被**取代**的 pragma。 `[[deprecated]]` 屬性和 `__declspec(deprecated)` 修飾詞可讓您針對特定形式的多載函式指定已被取代的狀態。 只有當屬性或修飾詞套用至特定多載函式的參考時，診斷警告才會出現。
