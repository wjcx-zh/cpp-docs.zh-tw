...
`#define` *識別項* `(` *識別項*opt`,`*...* `,`*識別項*opt`)`*語彙基元字串*選擇  
...
第二個語法形式會定義類似函式且具有參數的巨集。 這種形式可接受選擇性參數清單，但必須以括號括住。 巨集的定義，每個後續相符項目後*識別碼*(*識別項*...，選擇*識別碼*選擇) 的版本會取代*語彙基元字串*實際引數替代型式參數的引數。  
