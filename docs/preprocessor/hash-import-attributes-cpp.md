...
ms.locfileid: "70220102"
---
# <a name="import-attributes-c"></a>#import 屬性 (C++)

提供與指示詞搭配使用之`#import`屬性的連結。
...
|[named_guids](../preprocessor/named-guids.md)|告訴編譯器以舊樣式定義和初始化 GUID `LIBID_MyLib`變數, 格式為、 `CLSID_MyCoClass`、 `IID_MyInterface`和`DIID_MyDispInterface`。|
...
|[no_search_namespace](../preprocessor/no-search-namespace.md)|具有與[no_namespace](../preprocessor/no-namespace.md)屬性相同的功能, 但會用於您使用 #import 指示詞搭配[auto_search](../preprocessor/auto-search.md)屬性的類型程式庫。|
|[no_smart_pointers](../preprocessor/no-smart-pointers.md)|不為類型程式庫中的所有介面建立智慧型指標。|
|[raw_dispinterfaces](../preprocessor/raw-dispinterfaces.md)|指示編譯器針對會呼叫`IDispatch::Invoke`並傳回 HRESULT 錯誤碼的分派介面方法和屬性, 產生低層級的包裝函式。|
...
|[rename_search_namespace](../preprocessor/rename-search-namespace.md)|具有與[rename_namespace](../preprocessor/rename-namespace.md)屬性相同的功能, 但會用於您使用 #import 指示詞搭配[auto_search](../preprocessor/auto-search.md)屬性的類型程式庫。|
