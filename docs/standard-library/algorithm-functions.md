...
**`true`** 如果在指定範圍內的每個專案上偵測到條件，或如果範圍是空的，則傳回， **`false`** 否則傳回。

### <a name="remarks"></a>備註

**`true`** 只有 `N` 在範圍中的每個 `[0, last - first)` 都有述詞 `pred(*(first + N))` 為時，範本函數才會傳回 **`true`** 。
...
如果未偵測到 **`true`** 條件，則會在指定的範圍內偵測到至少一次的條件 **`false`** 。

### <a name="remarks"></a>備註

此範本函式 **`true`** 只會針對範圍中的某些部分傳回 `N`
...
*pred*\
使用者定義的述詞函式物件，定義一個元素小於另一個元素的意義。 二元述詞接受兩個引數，並 **`true`** 在滿足和 **`false`** 不滿意時傳回。

### <a name="return-value"></a>傳回值

**`true`** 如果在等於或等同于指定值的範圍中找到元素，則為;否則為 **`false`** 。
...
*pred*\
用來比較*值*與*lower*或*上限*的述詞。 比較述詞會採用兩個引數，並 **`true`** 在第一個的意義小於第二個時傳回，否則會傳回 **`false`** 。
...
*pred*\
使用者定義的述詞函式物件，定義要接受兩個元素為對等時所要符合的條件。 二元述詞接受兩個引數，並 **`true`** 在滿足和 **`false`** 不滿意時傳回。

### <a name="return-value"></a>傳回值

**`true`** 只有在以元素進行比較時，和範圍在二元述詞底下完全相同或相等時，才會是，否則為 **`false`** 。
...
*pred*\
使用者定義的述詞函式物件，定義一個元素小於另一個元素的意義。 比較述詞會採用兩個引數，並 **`true`** 在滿足和 **`false`** 不滿意時傳回。
...
*pred*\
使用者定義的述詞函式物件或 [Lambda 運算式](../cpp/lambda-expressions-in-cpp.md)，定義所搜尋的元素所要符合的條件。 一元述詞會接受單一引數，並 **`true`** 在滿足時傳回，如果不符合，則傳回 **`false`** 。 *Pred*的簽章必須有效地是 `bool pred(const T& arg);` ，其中 `T` 是可在取值 `InputIterator` 時以隱含方式轉換的類型。 **`const`** 關鍵字的顯示只是為了說明函式物件或 lambda 不應修改引數。
...
*pred*\
使用者定義的述詞函式物件或 [Lambda 運算式](../cpp/lambda-expressions-in-cpp.md)，定義所搜尋的元素不符合的條件。 一元述詞會接受單一引數，並 **`true`** 在滿足時傳回，如果不符合，則傳回 **`false`** 。 *Pred*的簽章必須有效地是 `bool pred(const T& arg);` ，其中 `T` 是可在取值 `InputIterator` 時以隱含方式轉換的類型。 **`const`** 關鍵字的顯示只是為了說明函式物件或 lambda 不應修改引數。

### <a name="return-value"></a>傳回值

輸入反覆運算器，參考範圍中不符合述詞所指定之條件的第一個元素（述詞會產生 **`false`** ）。 如果所有元素都符合述詞（述詞會 **`true`** 針對每個元素產生），則會傳回*last*。
...
**`true`** 如果第一個排序的範圍包含第二個排序範圍中的所有元素，則為，否則為 **`false`** 。
...
*pred*\
使用者定義的述詞函式物件，定義一個元素小於另一個元素的意義。 比較述詞會採用兩個引數，而且 **`true`** 當第一個專案小於第二個元素時，應該會傳回，否則會傳回 **`false`** 。
...
**`true`** 如果指定範圍中的元素形成堆積，則傳回， **`false`** 否則傳回。
...
**`true`** 如果指定範圍中測試條件的所有元素都在 **`true`** 測試的任何元素之前，則傳回 **`false`** 。
...
**`true`** 當指定範圍中測試條件的所有元素出現在 **`true`** 測試的任何專案之前，傳回，否則傳回 **`false`** **`false`** 。

### <a name="remarks"></a>備註

**`true`** 只有當中的所有專案都是由 pred 分割時，此範本函式才會傳回; 也就是說，中的所有專案若為 true，則會在的 `[first, last)` *pred* `X` `[first, last)` `pred (X)` 所有元素之前發生 `Y` `pred (Y)` **`false`** 。
...
**`true`** 當範圍可以根據比較子述詞重新排列，使其完全相同時，否則為 **`false`** 。

### <a name="remarks"></a>備註

`is_permutation` 在最壞的情況有二次方的複雜性。

第一個樣板函式假設範圍中的元素數目從*first2*開始，與指定的範圍相同 `[first1, last1)` 。 如果第二個範圍中有多個項目，就會忽略它們；如果更少，就會發生未定義的行為。 第三個樣板函式 (C++ 14 和更新版本) 不會進行這項假設。 這兩個專案都 **`true`** 只會傳回，但在 x = = Y 的相同範圍內，每個專案 x 的最 `[first1, last1)` 多個元素 Y，與從*first2*或開始的範圍相同 `[first2, last2)` 。 在這裡， `operator==` 必須在其運算元之間執行成對比較。
...
第二個版本可讓您提供比較函式物件，以在 **`true`** 兩個指定的專案為排序次序時傳回， **`false`** 否則為。
...
**`true`** 如果第一個範圍的詞典編纂小於第二個範圍，則為，否則為 **`false`** 。
...
`max` 是不尋常的演算法，因為它將物件做為參數傳遞。 大部分 C++ 標準程式庫演算法可對某範圍的元素作業，其位置由迭代器作為參數傳遞來加以指定。 如果您需要對某範圍元素運作的函式，請改用 [max_element](algorithm-functions.md#max_element)。 Visual Studio 2017 **`constexpr`** 會在接受 initializer_list 的多載上啟用。
...
*pred*\
使用者定義的述詞函式物件，定義一個元素小於另一個元素的意義。 比較述詞會採用兩個引數，而 **`true`** 當第一個專案小於第二個元素時，應該會傳回，否則會傳回 **`false`** 。
...
*pred*\
使用者定義的述詞函式物件，定義其中一個專案小於另一個元素的意義。 比較述詞會採用兩個引數，而且 **`true`** 當第一個小於第二個時，應該會傳回，否則會傳回 **`false`** 。
...
*pred*\
使用者定義述詞函式物件，比較每個範圍中的目前項目，並判斷是否相等。 **`true`** 當滿意且不符合時，它會傳回 **`false`** 。
...
*pred*\
使用者定義的述詞函式物件，定義順序中後續元素所要符合的比較準則。 二元述詞接受兩個引數，並 **`true`** 在滿足和 **`false`** 不滿意時傳回。

### <a name="return-value"></a>傳回值

**`true`** 如果詞典編纂下一個排列存在，並且已取代範圍的原始順序，則為;否則 **`false`** ，在這種情況下，會將順序轉換成詞典編纂最小排列。
...
*pred*\
使用者定義的述詞函式物件，定義順序中後續元素所要符合的比較準則。 比較述詞會採用兩個引數，並 **`true`** 在滿足和 **`false`** 不滿意時傳回。
...
**`true`** 當條件永遠不會出現在給定範圍中的專案之間時，會傳回。
...
**`true`** 如果在指定的範圍內至少偵測不到一次條件，則傳回，如果偵測到條件，則傳回 **`false`** 。

### <a name="remarks"></a>備註

**`true`** 只有在範圍中的某些部分，此樣板函式會傳回，但此範本函式 `N` `[0, last - first)` `pred(*(first + N))` 一律為 **`false`** 。
...
將條件是的專案複製 **`true`** 到一個目的地，並將其條件設為另一個目的地 **`false`** 。 項目必須來自指定的範圍。
...
此範本函式 `it` 會在的中尋找第一個反覆運算器 `[first, last)` `pred(*it)` **`false`** 。 順序必須依*pred*排序。
...
**`true`** 如果詞典編纂上一個排列存在，並且已取代範圍的原始順序，則為，否則 **`false`** ，在這種情況下，會將順序轉換成詞典編纂最大的排列。
...
*pred*\
使用者定義的述詞函式物件，定義一個元素小於另一個元素的意義。 二元述詞接受兩個引數，而且 **`true`** 當第一個專案小於第二個元素時，應該會傳回，否則會傳回 **`false`** 。
...
*pred*\
使用者定義的述詞函式物件，定義順序中後續元素所要符合的比較準則。 這個二元述詞會採用兩個引數，並 **`true`** 在兩個引數為順序時傳回，否則會傳回 **`false`** 。 這個比較子函式必須對序列中項目的配對強制執行嚴格的弱式順序。 如需詳細資訊，請參閱[演算法](algorithms.md)。
...
*pred*\
使用者定義的述詞函式物件，定義分類元素時所要符合的條件。 一元述詞會接受單一引數，並 **`true`** 在滿足時傳回，如果不符合，則傳回 **`false`** 。
...
*pred*\
使用者定義的比較述詞函式物件，定義其中一個專案小於另一個元素的意義。 比較述詞會採用兩個引數，並 **`true`** 在滿足和 **`false`** 不滿意時傳回。
