...
|[all_of](algorithm-functions.md#all_of)|當指定範圍內的每個專案都有條件時，傳回**true** 。|
|[any_of](algorithm-functions.md#any_of)|當條件出現在指定的專案範圍中至少一次時，傳回**true** 。|
...
|[copy_if](algorithm-functions.md#copy_if)|複製指定範圍中的所有元素，測試**true**是否符合指定的條件|
...
|[is_heap](algorithm-functions.md#is_heap)|如果指定範圍內的元素形成堆積，則傳回**true** 。|
|[is_heap_until](algorithm-functions.md#is_heap_until)|如果指定的範圍形成堆積直到最後一個元素，則傳回**true** 。|
|[is_partitioned](algorithm-functions.md#is_partitioned)|如果指定範圍中的條件測試**true**的所有專案都在測試**false**的任何專案之前，則傳回**true** 。|
|[is_permutation](algorithm-functions.md#is_permutation)|判斷指定的範圍中的元素是否構成有效的排列。|
|[is_sorted](algorithm-functions.md#is_sorted)|如果指定範圍內的專案為排序次序，則傳回**true** 。|
|[is_sorted_until](algorithm-functions.md#is_sorted_until)|如果指定範圍內的專案為排序次序，則傳回**true** 。|
...
|[merge](algorithm-functions.md#merge)|將兩個排序來源範圍內的所有項目結合成單一排序目的範圍，其中順序準則可由二元述詞指定。|
...
|[none_of](algorithm-functions.md#none_of)|當條件永遠不會出現在給定範圍中的專案之間時，傳回**true** 。|
...
|[partition_copy](algorithm-functions.md#partition_copy)|將條件為**true**的元素複製到一個目的地，並將其條件為**false** 。 項目必須來自指定的範圍。|
