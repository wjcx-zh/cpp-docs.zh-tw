...
> [!NOTE]
> \<演算法 > 程式庫也會使用 `#include <initializer_list>` 語句。
...
- `_if` 後置詞表示演算法會與函式物件搭配使用，而不是在元素本身的值上運作。 `find_if` 演算法尋找值符合函式物件指定之準則的項目，而 `find` 演算法則搜尋特定值。
...
|[all_of](../standard-library/algorithm-functions.md#all_of)|當指定範圍內的每個專案都有條件時，傳回**true** 。|
|[any_of](../standard-library/algorithm-functions.md#any_of)|當條件出現在指定的專案範圍中至少一次時，傳回**true** 。|
|[binary_search](../standard-library/algorithm-functions.md#binary_search)|測試已排序的範圍中是否有等於指定之值 (或在二元述詞指定的意義上，相當於該值) 的項目。|
|[夾具](../standard-library/algorithm-functions.md#clamp)||
...
|[copy_if](../standard-library/algorithm-functions.md#copy_if)|複製指定範圍中的所有元素，測試**true**是否符合指定的條件|
...
|[is_heap](../standard-library/algorithm-functions.md#is_heap)|如果指定範圍內的元素形成堆積，則傳回**true** 。|
|[is_heap_until](../standard-library/algorithm-functions.md#is_heap_until)|如果指定的範圍形成堆積直到最後一個元素，則傳回**true** 。|
|[is_partitioned](../standard-library/algorithm-functions.md#is_partitioned)|如果指定範圍中的條件測試**true**的所有專案都在測試**false**的任何專案之前，則傳回**true** 。|
|[is_permutation](../standard-library/algorithm-functions.md#is_permutation)|判斷指定的範圍中的元素是否構成有效的排列。|
|[is_sorted](../standard-library/algorithm-functions.md#is_sorted)|如果指定範圍內的專案為排序次序，則傳回**true** 。|
|[is_sorted_until](../standard-library/algorithm-functions.md#is_sorted_until)|如果指定範圍內的專案為排序次序，則傳回**true** 。|
...
|[none_of](../standard-library/algorithm-functions.md#none_of)|當條件永遠不會出現在給定範圍中的專案之間時，傳回**true** 。|
...
|[partition_copy](../standard-library/algorithm-functions.md#partition_copy)|將條件為**true**的元素複製到一個目的地，並將其條件為**false** 。 項目必須來自指定的範圍。|
...
|[抽樣](../standard-library/algorithm-functions.md#sample)||
