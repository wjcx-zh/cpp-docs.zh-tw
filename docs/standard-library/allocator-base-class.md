...
*同步*\
配置器的同步處理原則，即 [sync_none 類別](sync-none-class.md)、[sync_per_container 類別](sync-per-container-class.md)、[sync_per_thread 類別](sync-per-thread-class.md)或 [sync_shared 類別](sync-shared-class.md)。
...
|[size_type](#size_type)|不帶正負號的整數類型，可以代表類型的物件可以配置之任何序列的長度 `allocator_base` 。|
...
|成員函數|描述|
|-|-|
|[_Charalloc](#charalloc)|配置類型陣列的儲存區 **`char`** 。|
|[_Chardealloc](#chardealloc)|釋出包含類型元素之陣列的儲存體 **`char`** 。|
...
使用無法編譯重新繫結的編譯器進行編譯時，容器會使用此成員函式。 它會實作使用者定義配置器的 `_Chardealloc`，方法是呼叫同步處理篩選的 `deallocate` 函式。 針對比較等於的配置器物件，必須先為指標指標傳回 `_Charalloc` **`*this`** ，然後配置相同大小和類型的陣列物件。 `_Chardealloc` 絕不會擲回例外狀況。
...
不帶正負號的整數類型，可以代表類型的物件可以配置之任何序列的長度 `allocator_base` 。
