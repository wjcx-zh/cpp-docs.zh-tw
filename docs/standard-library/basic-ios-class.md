...
受保護的成員函式會將儲存在*右方*的`*this`所有值移`stream buffer pointer`至以外的位置，但在中，不會變更 *，而是*在中`*this`設定為 null 指標。 預存`tie pointer`的會設定為*右邊*的 null 指標。
...
受保護的成員函式會交換*儲存在中*的`*this`所有值， `stream buffer pointer`但儲存的除外。
