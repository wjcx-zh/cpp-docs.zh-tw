...
### <a name="remarks"></a>備註  
 字串的第一個項目具有索引零和下列項目會以正值的整數，連續索引以便長度的字串*n*具有*n*th 項目數目以做為索引*n-* 1。  
