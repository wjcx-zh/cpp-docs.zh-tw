...
由樣板類別 `basic_string` 的物件控制的序列為標準 C++ 字串類別，且通常稱為字串，但它們不應該和 C++ 標準程式庫中所使用之以 Null 結束的 C 樣式字串混淆。 標準 C++ 字串是一個容器，讓字串用作標準類型，例如比較和串連運算、迭代器、C++ 標準程式庫演算法，以及使用類別配置器 Managed 記憶體進行複製和指派。 如果您需要將標準 C++ 字串轉換為以 Null 結束的 C 樣式字串，請使用 [basic_string::c_str](#basic_string__c_str) 成員。  
...
#### <a name="parameters"></a>參數  
 `CharType`  
 若要儲存在字串中之單一字元的資料類型。 C++ 標準程式庫提供此樣板類別的特製化，以及 `char` 類型元素的類型定義 [string](../standard-library/string-typedefs.md#string)，例如 `wchar_t` 的 [wstring](../standard-library/string-typedefs.md#wstring)、`char16_t` 的 [u16string](../standard-library/string-typedefs.md#u16string)，以及 `char32_t` 的 [u32string](../standard-library/string-typedefs.md#u32string)。  
...
|||  
|-|-|  
|[basic_string](#basic_string__basic_string)|建構空的或由特定字元初始化的字串，或為其他字串物件的所有或部分複本的字串，或 C 字串。|  
  
### <a name="typedefs"></a>Typedefs  
  
|||  
|-|-|  
|[allocator_type](#basic_string__allocator_type)|類型，表示字串物件的 `allocator` 類別。|  
|[const_iterator](#basic_string__const_iterator)|類型，提供可以存取和讀取字串中 `const` 元素的隨機存取迭代器。|  
|[const_pointer](#basic_string__const_pointer)|類型，提供字串中 `const` 元素的指標。|  
|[const_reference](#basic_string__const_reference)|類型，提供儲存在字串中供讀取和執行 `const` 作業之 `const` 元素的參考。|  
|[const_reverse_iterator](#basic_string__const_reverse_iterator)|類型，提供可以讀取字串中任何 `const` 元素的隨機存取迭代器。|  
|[difference_type](#basic_string__difference_type)|類型，提供兩個指出相同字串內之元素的迭代器間的差異。|  
|[iterator](#basic_string__iterator)|類型，提供可以讀取或修改字串中之任何元素的隨機存取迭代器。|  
|[npos](#basic_string__npos)|搜尋函式失敗時不帶正負號整數值初始化為 –1，表示「找不到」或「所有剩餘字元」。|  
|[pointer](#basic_string__pointer)|類型，提供字串或字元陣列中之字元元素的指標。|  
|[reference](#basic_string__reference)|類型，提供儲存在字串中之元素的參考。|  
|[reverse_iterator](#basic_string__reverse_iterator)|類型，提供可以讀取或修改反轉字串中的元素的隨機存取迭代器。|  
|[size_type](#basic_string__size_type)|字串中元素數的不帶正負號整數類型。|  
|[traits_type](#basic_string__traits_type)|儲存在字串中之元素的字元特性的類型。|  
|[value_type](#basic_string__value_type)|類型，代表儲存在字串中的字元類型。|  
  
### <a name="member-functions"></a>成員函式  
  
|||  
|-|-|  
|[append](#basic_string__append)|將字元加入至字串的結尾。|  
|[assign](#basic_string__assign)|將新的字元值指派給字串的內容。|  
|[at](#basic_string__at)|傳回位於字串中指定位置的元素參考。|  
|[back](#basic_string__back)||  
|[begin](#basic_string__begin)|傳回定址字串中第一個元素的迭代器。|  
|[c_str](#basic_string__c_str)|將字串的內容轉換為 C 樣式且以 Null 結尾的字串。|  
|[capacity](#basic_string__capacity)|傳回可儲存在字串中且不增加字串的記憶體配置的最大元素數目。|  
|[cbegin](#basic_string__cbegin)|傳回定址字串中的第一個元素的 const 迭代器。|  
|[cend](#basic_string__cend)|傳回定址字串中最後一個元素的下一個位置的 const 迭代器。|  
|[clear](#basic_string__clear)|清除字串的所有元素。|  
|[compare](#basic_string__compare)|將某個字串與指定的字串比較，以判斷兩個字串是否相等，或其中一個字串的字數小於另一個字串。|  
|[copy](#basic_string__copy)|從來源字串中的索引位置，最多複製指定的字元數到目標字元陣列。 已取代。 請改用 [basic_string::_Copy_s](#basic_string___copy_s)。|  
|[crbegin](#basic_string__crbegin)|傳回定址反轉字串中的第一個元素的 const 迭代器。|  
|[crend](#basic_string__crend)|傳回定址反轉字串中最後一個元素的下一個位置的 const 迭代器。|  
|[_Copy_s](#basic_string___copy_s)|從來源字串中的索引位置，最多複製指定的字元數到目標字元陣列。|  
|[data](#basic_string__data)|將字串的內容轉換成字元的陣列。|  
|[empty](#basic_string__empty)|測試字串是否包含字元。|  
|[end](#basic_string__end)|傳回定址字串中最後一個元素的下一個位置的迭代器。|  
|[erase](#basic_string__erase)|從指定位置移除字串中的某個元素或某個元素範圍。|  
|[find](#basic_string__find)|以正向方向搜尋字串中，第一個符合指定之字元序列的子字串。|  
|[find_first_not_of](#basic_string__find_first_not_of)|搜尋字串中，不是指定字串之任何元素的第一個字元。|  
|[find_first_of](#basic_string__find_first_of)|搜尋字串中，符合指定字串之任何元素的第一個字元。|  
|[find_last_not_of](#basic_string__find_last_not_of)|搜尋字串中，不是指定字串之任何元素的最後一個字元。|  
|[find_last_of](#basic_string__find_last_of)|搜尋字串中，是指定字串之元素的最後一個字元。|  
|[front](#basic_string__front)|傳回字串中第一個元素的參考。|  
|[get_allocator](#basic_string__get_allocator)|傳回用來建構字串的 `allocator` 物件複本。|  
|[insert](#basic_string__insert)|將某個元素或一些元素或某個元素範圍，插入字串的指定位置。|  
|[length](#basic_string__length)|傳回字串中目前的元素數目。|  
|[max_size](#basic_string__max_size)|傳回字串中可能包含的字元數上限。|  
|[pop_back](#basic_string__pop_back)|清除字串的最後一個元素。|  
|[push_back](#basic_string__push_back)|將元素加入至字串結尾。|  
|[rbegin](#basic_string__rbegin)|傳回指向反轉字串中第一個元素的迭代器。|  
|[rend](#basic_string__rend)|傳回指向反轉字串中最後一個元素後方的迭代器。|  
|[replace](#basic_string__replace)|使用指定的字元，或從其他範圍或字串或 C 字串複製的字元，取代位於字串中指定位置的元素。|  
|[reserve](#basic_string__reserve)|將字串的容量數字，設定為至少和指定的數字一樣大。|  
|[resize](#basic_string__resize)|指定字串的新大小，視需要附加或清除元素。|  
|[rfind](#basic_string__rfind)|以向後方向搜尋字串中，第一個符合指定之字元序列的子字串。|  
|[shrink_to_fit](#basic_string__shrink_to_fit)|丟棄多餘的字串容量。|  
|[size](#basic_string__size)|傳回字串中目前的元素數目。|  
|[substr](#basic_string__substr)|從開始於指定位置的字串，複製最多一定字元數量的子字串。|  
|[swap](#basic_string__swap)|交換兩個字串的內容。|  
  
### <a name="operators"></a>運算子  
  
|||  
|-|-|  
|[operator+=](#basic_string__operator_add_eq)|將字元附加至字串。|  
|[operator=](#basic_string__operator_eq)|將新的字元值指派給字串的內容。|  
|[operator&#91;&#93;](#basic_string__operator_at)|使用字串中的指定索引，提供字元的參考。|  
  
## <a name="remarks"></a>備註  
 如果要求函式產生超過 [max_size](#basic_string__max_size) 個元素的序列長度，則此函式會藉由擲回 [length_error](../standard-library/length-error-class.md) 類型的物件，報告長度錯誤。  
  
 指定受控制序列之元素的參考、指標和迭代器，在呼叫改變受控制序列的函式之後，或是第一次呼叫非 **const** 成員函式之後，可能會變得無效。  
...
### <a name="remarks"></a>備註  
 字元可使用 [operator+=](#basic_string__operator_add_eq) 或是成員函式 **append** 或 [push_back](#basic_string__push_back) 附加至字串。 `operator+=` 會附加單一引數值，而多引數 **append** 成員函式則允許指定要新增的特定字串部分。  
...
### <a name="remarks"></a>備註  
 字串可以指派新的字元值。 新的值可以是字串和 C 字串，或是單一字元。 如果可由單一參數描述新的值，則可以使用 [operator=](#basic_string__operator_eq)；否則，可使用具有多個參數的成員函式 **assign**，來指定要將字串的哪個部分指派給目標字串。  
...
### <a name="remarks"></a>備註  
 ...
 成員 [operator&#91;&#93;](#basic_string__operator_at) 比成員函式 **at** 更快提供字串元素的讀取和寫入權限。  
  
 成員 `operator[]` 不會檢查當做參數傳遞的索引是否有效，但成員函式 **at** 會執行檢查，因此如果不確定是否有效，則應該使用後者。 傳遞給成員函式 **at** 的索引若無效 (小於零或大於等於字串大小)，則會擲回 [out_of_range 類別](../standard-library/out-of-range-class.md)例外狀況。 傳遞給 `operator[]` 的索引若無效會導致未定義的行為，但等於字串長度的索引是 const 字串的有效索引，而且當傳遞此索引時，運算子會傳回 Null 字元。  
...
### <a name="remarks"></a>備註  
 所有建構函式都會儲存 [basic_string::allocator_type](#basic_string__allocator_type)，並初始化受控制的序列。 配置器物件是引數 `al` (如果存在)。 若是複製建構函式，則為 `right.`[basic_string::get_allocator](#basic_string__get_allocator)`()`。 否則為 `Alloc()`。  
...
### <a name="remarks"></a>備註  
 屬於 C++ 樣板類別 basic_string\<char> 的字串類型物件不一定是以 Null 結束。 Null 字元 ' \0 ' 會用作 C 字串中的特殊字元來標記字串結尾，但在字串類型的物件中則沒有任何特殊意義，而且可與任何其他字元一樣當做字串的一部分。 const **char\*** 可自動轉換為字串，但此字串類別不提供從 C 樣式字串到 **basic_string\<char>** 類型物件的自動轉換。  
...
### <a name="remarks"></a>備註  
 此成員函式會傳回目前所配置以保留受控制序列的儲存空間，該值至少會與 [size](#basic_string__size) 一樣大。  
...
### <a name="remarks"></a>備註  
 ...
 您可以使用此成員函式取代 `begin()` 成員函式，以確保傳回值是 `const_iterator`。 通常，它是與 [auto](../cpp/auto-cpp.md) 類型推算關鍵字一起使用，如下列範例所示。 在此範例中，請考慮將 `Container` 視為支援 `begin()` 和 `cbegin()` 之任何類型的可修改的 (非 `const`) 容器。  
...
### <a name="remarks"></a>備註  
 **compare** 成員函式會比較參數和運算元字串的全部或一部分，視使用內容而定。  
...
### <a name="example"></a>範例  
  如需如何宣告及使用 `const_iterator` 的範例，請參閱 [begin](#basic_string__begin) 的範例。  
...
### <a name="remarks"></a>備註  
 此類型與 **allocator_type::const_pointer** 同義。  
  
 針對 **string** 類型，它相當於 `char`*。  
...
### <a name="remarks"></a>備註  
 ...
 此類型與 **allocator_type::const_reference** 同義。 針對 string **類型**，它相當於 const **char&**。  
  
### <a name="example"></a>範例  
  如需如何宣告及使用 `const_reference` 的範例，請參閱 [at](#basic_string__at) 的範例。  
...
### <a name="example"></a>範例  
  如需如何宣告及使用 `const_reverse_iterator` 的範例，請參閱 [rbegin](#basic_string__rbegin) 的範例。  
...
##  <a name="a-namebasicstringcopya--basicstringcopy"></a><a name="basic_string__copy"></a>  basic_string::copy  
 ...
 此方法有賴於呼叫者檢查傳遞的值是否正確，因此可能不安全。 請考慮改用 [basic_string::_Copy_s](#basic_string___copy_s)。  
...
### <a name="remarks"></a>備註  
 屬於 C++ 樣板類別 basic_string \<char> 的字串類型物件不一定是以 Null 結束。 **data** 的傳回型別不是有效的 C 字串，因為未附加任何 Null 字元。 Null 字元 ' \0 ' 會用作 C 字串中的特殊字元來標記字串結尾，但在字串類型的物件中則沒有任何特殊意義，而且可與任何其他字元一樣當做字串物件的一部分。  
...
### <a name="remarks"></a>備註  
 此成員函式相當於 [size](#basic_string__size) == 0。  
...
### <a name="remarks"></a>備註  
 **end** 通常可用來測試迭代器是否已達到其字串的結尾。 **end** 所傳回的值不應該被取值。  
  
 如果 **end** 的傳回值已指派給 `const_iterator`，則無法修改字串物件。 如果 **end** 的傳回值已指派給 **iterator**，則可以修改字串物件。  
...
### <a name="remarks"></a>備註  
 類型 **iterator** 可用來修改字元的值，且會用來依正向方向逐一查看字串。  
  
### <a name="example"></a>範例  
  如需如何宣告及使用 **iterator** 的範例，請參閱 [begin](#basic_string__begin) 的範例。  
...
### <a name="remarks"></a>備註  
 此成員函式與 [size](#basic_string__size) 相同。  
...
### <a name="remarks"></a>備註  
 若要檢查傳回值是否有 `npos` 值，除非傳回值屬於 [size_type](#basic_string__size_type) 類型且不是 `int` 或 `unsigned`，否則可能會無法運作。  
  
### <a name="example"></a>範例  
  如需如何宣告及使用 `npos` 的範例，請參閱 [find](#basic_string__find) 的範例。  
...
### <a name="remarks"></a>備註  
 字元可使用 `operator+=` 或是成員函式 [append](#basic_string__append) 或 [push_back](#basic_string__push_back) 附加至字串。 `operator+=` 會附加單一引數值，而多引數 append 成員函式則允許指定要新增的特定字串部分。  
...
### <a name="remarks"></a>備註  
 字串可以指派新的字元值。 新的值可以是字串和 C 字串，或是單一字元。 如果可由單一參數描述新的值，則可以使用 `operator=`；否則，可使用具有多個參數的成員函式 [assign](#basic_string__assign)，來指定要將字串的哪個部分指派給目標字串。  
...
### <a name="remarks"></a>備註  
 ...
 `operator[]` 會比成員函式 [at](#basic_string__at) 更快提供字串元素的讀取和寫入權限。  
  
 `operator[]` 不會檢查當做參數傳遞的索引是否有效，但成員函式 **at** 會執行檢查，因此如果不確定是否有效，則應該使用後者。 傳遞給成員函式 **at** 的索引若無效 (小於零或大於等於字串大小)，則會擲回 [out_of_range 類別](../standard-library/out-of-range-class.md)例外狀況。 傳遞給 `operator[]` 的索引若無效會導致未定義的行為，但等於字串長度的索引是 const 字串的有效索引，而且當傳遞此索引時，運算子會傳回 Null 字元。  
...
### <a name="remarks"></a>備註  
 此類型與 **allocator_type::pointer** 同義。  
...
### <a name="remarks"></a>備註  
 此成員函式會有效呼叫 [insert](#basic_string__insert)( [end](#basic_string__end), _ *Ch* )。  
...
### <a name="remarks"></a>備註  
 `rbegin` 會與反向字串搭配使用，就如同 [begin](#basic_string__begin) 與字串搭配使用一樣。  
...
### <a name="remarks"></a>備註  
 ...
 此類型與 **allocator_type::reference** 同義。  
  
 針對 **string** 類型，它相當於 **chr&**。  
  
### <a name="example"></a>範例  
  如需如何宣告及使用 **reference** 的範例，請參閱 [at](#basic_string__at) 的範例。  
...
### <a name="remarks"></a>備註  
 `rend` 會與反向字串搭配使用，就如同 [end](#basic_string__end) 與字串搭配使用一樣。  
...
### <a name="remarks"></a>備註  
 ...
 字串類型物件的容量概念與向量類型物件的容量概念相同。 不同於向量，您可以呼叫 **reserve** 成員函式來壓縮物件的容量。 此要求是非繫結要求，而且不一定會發生。 由於參數的預設值為零，因此 **reserve** 的呼叫是非繫結要求，可壓縮字串的容量以符合字串中目前的字元數。 此容量永遠不會縮減到低於目前的字元數。  
...
### <a name="example"></a>範例  
  如需如何宣告及使用 `reverse_iterator` 的範例，請參閱 [rbegin](#basic_string__rbegin) 的範例。  
...
### <a name="remarks"></a>備註  
 此類型與第二個樣板參數 **Traits** 同義。  
  
 針對 **string** 類型，它相當於 **char_traits\<char>**。  
  
### <a name="example"></a>範例  
  如需如何宣告及使用 `traits_type` 的範例，請參閱 [copy](../standard-library/char-traits-struct.md#char_traits__copy) 的範例。  
...
### <a name="remarks"></a>備註  
 它相當於 **traits_type::char_type**，且相當於 **string** 類型物件的 `char`。  
