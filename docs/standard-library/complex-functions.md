...
### <a name="remarks"></a>備註  
 引數是複平面正實軸的複向量角度。 若為複數 *a + bi*，引數等於 arctan( *b/a*) *。* 從正實軸逆時針方向測得的角度為正向；順時針方向測得的角度為負向。 主值會大於 –pi 且小於或等於 +pi。  
...
### <a name="remarks"></a>備註  
 複數的極式可提供模數 *r* 和引數，而這些參數與笛卡兒的實數與虛數分量 *a* 和 *b* 相關，方程式為 *a* = r \* cos (  ) 和 b = *r* \* sin () *。*  
