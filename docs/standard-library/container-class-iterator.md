---
title: "容器類別::iterator | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- cpp-standard-libraries
ms.tgt_pltfrm: 
ms.topic: reference
dev_langs:
- C++
helpviewer_keywords:
- iterator method
ms.assetid: f9c49d1c-17cb-4b17-8e54-09e3ea41ca26
caps.latest.revision: 
author: corob-msft
ms.author: corob
manager: ghogen
ms.workload:
- cplusplus
ms.openlocfilehash: 9eface3b112071f372c18e59bf87fb23a6849a2f
ms.sourcegitcommit: d51ed21ab2b434535f5c1d553b22e432073e1478
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 02/23/2018
---
# <a name="container-classiterator"></a>容器類別::iterator
> [!NOTE]
>  本主題位於 Visual C++ 文件內，可做為 C++ 標準程式庫中所用容器的無作用範例。 如需詳細資訊，請參閱 [C++ 標準程式庫容器](../standard-library/stl-containers.md)。  
  
 描述的物件可作為受控制序列的迭代器。  
  
## <a name="syntax"></a>語法  
  
```  
 
typedef T5 iterator;  
```  
  
## <a name="remarks"></a>備註  
 在此將其描述為未指定之 **T5** 類型的同義字。 **iterator** 類型的物件可以轉型為 [const_iterator](../standard-library/container-class-const-iterator.md) 類型的物件。  
  
## <a name="see-also"></a>請參閱  
 [範例容器類別](../standard-library/sample-container-class.md)
