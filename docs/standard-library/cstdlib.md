...
函式會註冊函數*func*，當呼叫時`quick_exit` ，不需要引數就會呼叫它。 `at_quick_exit()` 對`at_quick_exit()`的呼叫不會在所有`quick_exit`呼叫都成功之前發生。 這些`at_quick_exit()`函數不會引進資料競爭。 如果`at_quick_exit`從多個執行緒呼叫，則註冊的順序可能會是不確定的。 由於`at_quick_exit`註冊與`atexit`註冊不同，因此應用程式可能需要使用相同的引數呼叫這兩個註冊函式。 MSVC 支援至少註冊32個函式。
...
函`atexit()`式會在正常程式終止時，以不含引數*的方式來註冊要呼叫*的函式。 呼叫不會在`exit()`呼叫之前才會成功。 `atexit()` 這些`atexit()`函數不會引進資料競爭。
...
接下來，系統會終結具有靜態儲存期的物件，並`atexit`呼叫透過呼叫註冊的函式。 當呼叫時`exit()` ，自動物件不會損毀。 如果控制項離開由呼叫的`exit`已註冊函式，因為函式未提供所擲回例外狀況的處理常式， `std::terminate()`則會呼叫。 函式會在每次註冊時呼叫一次。 具有自動儲存期的物件會在其`main`函式未包含任何自動物件的程式中終結，並執行對的`exit()`呼叫。 控制項可以直接傳送到這類`main`函式，方法是擲回中`main`攔截到的例外狀況。

接下來，會清除所有開啟的 c 資料流程（如 cstdio> > 中\<宣告的函式簽章所 mediated）與不成文緩衝的資料，所有開啟的 c 資料流程都會關閉， `tmpfile()`而且會移除所有以呼叫所建立的檔案。
...
一般來說，呼叫`at_quick_exit`所註冊的函式會以其註冊的反向順序呼叫。 此順序不適用於已呼叫其他已註冊函式之後註冊的函式。 呼叫時`quick_exit` ，不會終結任何物件。 如果控制項離開由呼叫的`quick_exit`已註冊函式，因為函式未提供所擲回例外狀況的處理常式， `std::terminate()`則會呼叫。 透過註冊`at_quick_exit`的函式是由呼叫的執行緒所`quick_exit`叫用，它可以是與註冊它不同的執行緒。 這表示已註冊的函式不應依賴具有線程儲存期的物件身分識別。 呼叫已註冊的函`quick_exit`式`_Exit(status)`之後，會呼叫。 標準檔案緩衝區不會排清。 當使用`quick_exit` `at_quick_exit`註冊的函式為時，函數是信號安全。
...
這些函式具有 C 標準程式庫中指定的語法。 MSVC 不支援`aligned_alloc`函數。 C11 指定`aligned_alloc()`的方式與 Microsoft 的`free()`實作為不相容，亦即`free()`必須能夠處理高度對齊的配置。
