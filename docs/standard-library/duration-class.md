...
第二個範本引數會使用的預設期間`std::ratio<1>`, 來建立一個物件, 代表*R*時鐘刻度的時間間隔。 若要避免滴答計數的舍入, 從標記法類型*Rep2*中建立 duration 物件是錯誤的, 當無法將它視為浮點類型時`duration::rep` , 可以將它視為浮點類型。
