...
**`true`** 如果 `error_condition` 物件不等於 `error_condition` *直接*傳入的物件，則為，否則為 **`false`** 。
...
**`true`** 如果 `error_condition` 物件小於 `error_condition` 傳入以進行比較的物件，則為，否則為 **`false`** 。
