...
如果不是多型類別類型，或無法 `nested_exception` 存取或不明確，則不會有任何作用。 否則，會執行動態轉換。
...
## <a name="terminate"></a><a name="terminate"></a>終止
...
函式會呼叫終止處理常式，也就是類型的函式 **`void`** 。 如果 `terminate` 程式直接呼叫，則終止處理常式是[set_terminate](../standard-library/exception-functions.md#set_terminate)的呼叫所設定的。 如果在 `terminate` 評估 throw 運算式時，針對任何其他原因呼叫，則終止處理常式會在評估 throw 運算式之後立即生效。

終止處理常式可能無法傳回至其呼叫端。 在程式啟動時，終止處理常式是呼叫的函式 `abort` 。
...
**`true`** 在完成 throw 運算式的評估之後，以及在比對處理常式中完成例外狀況宣告的初始化之前，或呼叫非[預期](../standard-library/exception-functions.md#unexpected)的 throw 運算式結果之前，會傳回。 特別是， `uncaught_exception` 從在 **`true`** 例外狀況回溯期間叫用的析構函式呼叫時，會傳回。 `uncaught_exception` 僅支援 Windows CE 5.00 或更新版本的裝置，包括 Windows Mobile 2005 平台。
...
## <a name="unexpected"></a><a name="unexpected"></a>意料
...
函式會呼叫未預期的處理常式，也就是類型的函數 **`void`** 。 如果程式直接呼叫 `unexpected`，未預期的處理常式即為 [set_unexpected](../standard-library/exception-functions.md#set_unexpected) 呼叫最近設定的項目。
...
- 呼叫[terminate](../standard-library/exception-functions.md#terminate)、 `abort` 或 `exit` 。
