...
ms.locfileid: "68240717"
---
# <a name="ltfilesystemgt-functions"></a>&lt;filesystem&gt; 函式

這些免費函式中的[ \<filesystem >](../standard-library/filesystem.md)標頭執行修改及查詢路徑、 檔案、 符號連結、 目錄和磁碟區上的作業。 如需詳細資訊與程式碼範例，請參閱[檔案系統巡覽 (C++)](../standard-library/file-system-navigation.md)。
...
- 如果`options & copy_options::skip_symlinks`，然後執行任何動作。
...
- 如果`opts & copy_options::directories_only`，然後執行任何動作。
...
- 否則，如果`is_directory(f)`，然後`copy_file(from, to`  /  `from.filename(), opts)`。
...
如果`exists(to) && !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options::update_existing))`，則回報的錯誤，檔案已經存在。

否則，如果`!exists(to) || opts & copy_options::overwrite_existing || opts & copy_options::update_existing&& last_write_time(to) < last_write_time(from) || !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options:update_existing))`，然後嘗試將內容和檔案屬性，複製*從*檔案*到*。 如果複製嘗試失敗，則報告錯誤。
...
為路徑名稱，例如\/b\/c，函式會建立目錄 a 和 a\/b 所需，讓它可以建立該目錄\/b\/視 c。 它會傳回**真**確實建立目錄時，才*pval*。
...
函式會傳回 **，則為 true**只有當*左*並*右*選擇相同的檔案系統實體。
...
函式會傳回以位元組為單位的所選檔案的大小*pval*，如果`exists(pval) && is_regular_file(pval)`，並且可以判斷檔案的大小。 否則它們會報告錯誤並傳回`uintmax_t(-1)`。
...
如果`is_directory(pval)`，則此函式會傳回`directory_iterator(pval) == directory_iterator()`; 否則會傳回`file_size(pval) == 0`。
...
函式會將所選擇的路徑名稱的權限*pval*要`mask & perms::mask`控制`perms & (perms::add_perms | perms::remove_perms)`。 *遮罩*應該包含最多是其中一個`perms::add_perms`和`perms::remove_perms`。
...
函式會傳回**真**只有當`exists(symlink_status(pval))`，而且已成功移除檔案。 符號連結本身就是移除不是它所選擇的檔案。
...
函式重新命名*從*要*至*。 符號連結本身就是重新命名，不是它所選擇的檔案。
...
函式會傳回所選磁碟區的相關資訊*pval*，在結構中的型別`space_info`。 此結構包含`uintmax_t(-1)`無法判斷任何值。
...
函式會傳回路徑名稱狀態、 檔案類型和權限，與相關聯*pval*。 符號連結本身並未經過測試，但它會選擇的檔案。
...
函式會傳回路徑名稱符號連結狀態、 檔案類型和權限，與相關聯*pval*。 函式的行為相同`status(pval)`不同之處在於符號連結是測試本身，不是檔案選擇。
...
第一個函式的行為相同`path(source)`和第二個函式的行為相同`path(first, last)`不同之處在於每個案例中選擇的來源會做為編碼為 utf-8，任何檔案系統的一串字元項目。
