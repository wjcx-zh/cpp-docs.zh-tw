...
如果 `begin` 的傳回值指派給 `const_iterator`，則無法修改 hash_multiset 物件中的元素。 如果 `begin` 的傳回值指派給 `iterator`，則可以修改 hash_multiset 物件中的元素。
...
`end` 可用來測試反覆運算器是否已到達其 hash_multiset 的結尾。 `end` 所傳回的值不應該取值。
