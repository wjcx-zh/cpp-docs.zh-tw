...
通常，項目必須是小於比較才能建立此順序：因此若提供了兩個項目，可以判斷它們相等 (任一個都不小於另一個的意義)，或者一個小於另一個。 這會導致非對等元件之間的排序。 一個技術提示，比較函式是在標準數學概念上產生嚴格弱式順序的二元述詞。 二元述詞 *f* ( *x*， *y*) 是具有兩個引數物件 x 和 y 以及傳回值 true 或 false 的函式物件。 如果二元述詞是非自反、反對稱性且可轉移的，而且如果等價是可轉移的，其中兩個物件 x 和 y 是定義為當 *f*( *x*, *y*) 和 *f*( *y*, *x*) 皆為 false 時即相等，則施加於 hash_multiset 的排序是嚴格弱式排序。 如果更強的索引鍵相等條件取代等價條件，順序會變成總計 (也就是所有項目彼此相關的排序)，因此相符的索引鍵之間將難以辨別。
...
|[const_pointer](#const_pointer)|一種類型，提供 **`const`** 中的元素指標 `hash_multiset` 。|
|[const_reference](#const_reference)|一種類型，提供 **`const`** 儲存在中的元素參考，以 `hash_multiset` 供讀取和執行 **`const`** 作業。|
...
|[迭 代](#iterator)|類型，其提供可讀取或修改 `hash_multiset` 中任何項目的雙向迭代器。|
...
如果的傳回值 `begin` 已指派給 `const_iterator` ，則無法修改 hash_multiset 物件中的元素。 如果的傳回值已 `begin` 指派給 `iterator` ，則可以修改 hash_multiset 物件中的元素。
...
一種類型，提供 **`const`** hash_multiset 中的元素指標。
...
在大部分的情況下，應該使用 [const_iterator](#const_iterator) 來存取 hash_multiset 物件中的元素 **`const`** 。
...
一種類型，提供 **`const`** 儲存在 hash_multiset 中的元素參考，以讀取和執行 **`const`** 作業。
...
*：*\
要開始搜尋正確的插入點的地方 如果插入點緊接在 *where 的位置*， (插入可能會在分攤的常數時間（而不是對數時間）發生。 ) 
...
如果插入點緊接在 *where*之後，則會在分攤的常數時間（而不是對數時間）進行插入。
...
**`true`** 如果 hash_multiset 空白，則為， **`false`** 如果 hash_multiset 不是空的。
...
成員函式會傳回反覆運算器，此反覆運算器會定址 hash_multiset 中的專案，其排序索引鍵會是二元述詞 `equivalent` 下的引數索引鍵，該二元述詞會根據小於可比較性的關聯引發排序。

如果的傳回值 `find` 已指派給 `const_iterator` ，則無法修改 hash_multiset 物件。 如果的傳回值已 `find` 指派給 `iterator` ，則可以修改 hash_multiset 物件。
...
前三個函式會指定空的初始專案 `hash_multiset` ，第二個是指定比較函式的類型 (要用來建立元素順序的 *複合*) ，而第三個則明確指定要使用的配置器類型 (*Al*) 。 關鍵字會 **`explicit`** 隱藏某些類型的自動類型轉換。
...
如果插入點緊接在 *where 的位置*，則插入可能會針對插入的提示版本（而不是對數時間）進行分攤的常數時間。
...
如需有關 *金鑰*的詳細資訊，請參閱 [hash_multiset 類別](../standard-library/hash-multiset-class.md) 主題的備註一節。
...
*對*\
要複製到 `hash_multiset` 中的 [hash_multiset](../standard-library/hash-multiset-class.md)。

### <a name="remarks"></a>備註

清除中的任何現有元素之後 `hash_multiset` ，會 `operator=` 將 *右邊* 的內容複寫或移動到 `hash_multiset` 。
