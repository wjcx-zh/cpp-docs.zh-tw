---
title: "&lt;iterator&gt; 函式 | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
### <a name="remarks"></a>備註  
 ...
 如果 **InputIterator** 滿足雙向迭代器的需求，則 `Off` 可以是負數。 如果 **InputIterator** 是輸入或正向迭代器類型，`Off` 就必須是非負值。  
  
 當 **InputIterator** 滿足隨機存取迭代器的需求時，advance 函式會具有常數複雜度；否則，它會具有線性複雜度，因此可能會耗用相當多的資源。  
...
### <a name="return-value"></a>傳回值  
 與容器物件 `_Cont` 關聯的 `back_insert_iterator`。  
...
### <a name="remarks"></a>備註  
 ...
 您可以使用此成員函式來取代 [end()](../standard-library/iterator-functions.md#end) 範本函式，以確保傳回值是 `const_iterator`。 通常，它是與 [auto](../cpp/auto-cpp.md) 類型推算關鍵字一起使用，如以下範例所示。 在此範例中，請將 `Container` 視為任何一種支援 `end()` 和 `cend()` 的可修改 (非 `const`) 容器或 `initializer_list`。  
...
### <a name="return-value"></a>傳回值  
 必須遞增 ` first` 直到與 ` last` 相等的次數。  
  
### <a name="remarks"></a>備註  
 當 **InputIterator** 滿足隨機存取迭代器的需求時，distance 函式會具有常數複雜度；否則，它會具有線性複雜度，因此可能會耗用相當多的資源。  
...
### <a name="return-value"></a>傳回值  
 與容器物件 `_Cont` 關聯的 `front_insert_iterator`。  
  
### <a name="remarks"></a>備註  
 也可以使用 front_insert_iterator 類別的成員函式 [front_insert_iterator](../standard-library/front-insert-iterator-class.md#front_insert_iterator__front_insert_iterator)。  
...
##  <a name="a-nameinsertera--inserter"></a><a name="inserter"></a>  inserter  
 可讓您使用 `inserter(``_Cont``,``_Where``)` 而不使用 `insert_iterator<Container>(``_Cont`, `_Where``)` 的協助程式範本函式。  
...
### <a name="remarks"></a>備註  
 此範本函式會傳回 [insert_iterator](../standard-library/insert-iterator-class.md#insert_iterator__insert_iterator)`<Container>(``_Cont``,` `_Where``)`。  
...
### <a name="return-value"></a>傳回值  
 在逐一查看 `_Off` 次之後，傳回新的迭代器位置。  
  
### <a name="remarks"></a>備註  
 此範本函式會傳回遞增 `_Off` 次後的 `next`  
