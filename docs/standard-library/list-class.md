...
ms.locfileid: "88833331"
---
# <a name="list-class"></a>list 類別

C + + 標準程式庫清單類別是順序容器的類別範本，會以線性相片順序維護其元素，並允許在序列內的任何位置有效率地插入及刪除。 此序列會儲存為雙向連結的元素清單，每一個都包含某種 *Type* 類型的成員。
...
包含 c + + 標準程式庫標準標頭， \<list> 以定義 [容器](../standard-library/stl-containers.md) 類別範本清單及數個支援的範本。
...
|[const_iterator](#const_iterator)|一種類型，提供可讀取清單中元素的雙向反覆運算器 **`const`** 。|
|[const_pointer](#const_pointer)|一種類型，提供 **`const`** 清單中元素的指標。|
|[const_reference](#const_reference)|一種類型，提供 **`const`** 儲存在清單中供讀取和執行作業之專案的參考 **`const`** 。|
|[const_reverse_iterator](#const_reverse_iterator)|一種類型，提供可讀取清單中任何元素的雙向反覆運算器 **`const`** 。|
|[difference_type](#difference_type)|類型，提供兩個指出相同清單內之元素的迭代器間的差異。|
|[迭 代](#iterator)|類型，提供可以讀取或修改清單中之任何元素的雙向迭代器。|
|[指標](#pointer)|類型，提供清單中的元素指標。|
|[reference](#reference)|一種類型，提供 **`const`** 儲存在清單中供讀取和執行作業之專案的參考 **`const`** 。|
...
如果的傳回值 `begin` 已指派給，則 `const_iterator` 無法修改清單物件中的元素。 如果的傳回值已 `begin` 指派給，則 `iterator` 可以修改清單物件中的元素。
...
一種類型，提供 **`const`** 儲存在清單中供讀取和執行作業之專案的參考 **`const`** 。
...
一種類型，提供可讀取清單中任何元素的雙向反覆運算器 **`const`** 。
...
*：*\
目標 [清單](../standard-library/list-class.md) 中插入第一個元素的位置。
...
*瓦爾*\
加入至 [清單](../standard-library/list-class.md)結尾的元素。
...
**`true`** 如果清單是空的，則為。 **`false`** 如果清單不是空的，則為。
...
所有的函式都會將配置器物件儲存 (*Al*) ，並將清單初始化。

[get_allocator](#get_allocator) 會傳回一份用來建構清單的配置器物件複本。

前兩個函式會指定空的初始清單，第二個會指定要使用的配置器類型 (*Al*) 。

第三個函式會針對類別的預設值 (*Count*) 的元素，指定重複的指定數目 `Type` 。

第四個和第五個函式會指定值*Val*) 元素的 (*計數*重複。
...
引數和目標清單必須以相同的比較關聯排序，而結果序列會依此排序。 第一個成員函式的預設排序為遞增的順序。 第二個成員函式會強加使用者 *指定之類別的比較* 作業 `Traits` 。
...
清除中的任何現有元素之後 `list` ，運算子會將 *右邊* 的內容複寫或移動到 `list` 。
...
如果清單的大小小於所要求的大小， *_Newsize*，則會將專案新增至清單，直到達到所要求的大小為止。
...
成員範本函式會根據使用者 *指定之類別的比較* 作業，來排列元素的順序 `Traits` 。
...
第一對成員函式會將來源清單中的所有專案插入到目的地清單中，並在 *Where* 和移除來源清單中的所有元素之前。  (`&Source` 不能相等 **`this`** 。 ) 

第二對成員函式會將 *Iter* 所參考的專案插入目的地清單中 *的位置，並從* 來源清單中移除 *Iter* 。 (若 `Where == Iter || Where == ++Iter`，則不會產生任何變更)。

第三對成員函式會將 [，) 指定的範圍插入 `First` `Last` 目的清單中的專案之前，並從*Where*來源清單中移除該範圍的專案。  (如果 `&Source == this` 為，則範圍 `[First, Last)` 不得包含 *Where*所指向的元素 ) 
...
第二個成員函式會移除滿足述詞函式 *pred* 的每個專案（相較于其前面的元素）。 您可以使用 \<functional> *pred* 引數的標頭中所宣告的任何二元函式物件，也可以建立自己的函式物件。
