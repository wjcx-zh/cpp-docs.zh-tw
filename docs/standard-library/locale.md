...
ms.locfileid: "72687804"
---
# <a name="ltlocalegt"></a>&lt;locale&gt;

定義類別樣板和函式C++ ，程式可用來封裝和操作與數位、貨幣和 calendric 資料的表示和格式有關的不同文化慣例，包括國際化支援適用于字元分類和字串定序。
...
|[codecvt_base](../standard-library/codecvt-base-class.md)|Codecvt 類別的基類，用來定義稱為 `result` 的列舉類型，做為 facet 成員函式的傳回型別，以指示轉換的結果。|
|[codecvt_byname](../standard-library/codecvt-byname-class.md)|衍生的類別範本，描述可以做為指定地區設定之 collate facet 的物件，可讓您抓取關於轉換的文化特性區域特定資訊。|
|[collate](../standard-library/collate-class.md)|Collate 類別範本，提供處理字串排序慣例的 facet。|
...
|[ctype \<char >](../standard-library/ctype-char-class.md)|屬於類別樣板之明確特製化的類別，`ctype<CharType>` 為**char**類型，描述可以做為地區設定 facet 的物件，以表示**char**類型字元的各種屬性。|
...
|[time_get_byname](../standard-library/time-get-byname-class.md)|衍生類別範本，描述可做為 time_get 類型之地區設定 facet 的物件，\<**CharType**， **InputIterator**>。|
|[time_put](../standard-library/time-put-class.md)|類別樣板，描述可以做為地區設定 facet 的物件，以控制時間值轉換為**CharType**類型的序列。|
|[time_put_byname](../standard-library/time-put-byname-class.md)|衍生類別範本，描述可做為類型地區設定 facet 的物件，`time_put` \<**CharType**， **OutputIterator**>。|
