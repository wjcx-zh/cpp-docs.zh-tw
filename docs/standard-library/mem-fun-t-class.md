...
ms.locfileid: "72689373"
---
# <a name="mem_fun_t-class"></a>mem_fun_t 類別

介面卡類別，允許在使用指標引數初始化時，不接受引數的 `non_const` 成員函式，當做一元函式物件來呼叫。 在 c + + 11 中已被取代，在 c + + 17 中移除
...
類別樣板會儲存 *_Pm*的複本，這必須是私用成員物件中 `Type` 類別之成員函式的指標。 它會將其成員函式 `operator()` 定義為傳回（`_Pleft` ->*  `_Pm`）（）。
