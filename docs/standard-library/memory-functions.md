...
### <a name="parameters"></a>參數  
 ...
 `Ptr`  
 要使用的可用連續儲存集區的開始位址。 這個參數也是輸出參數，如果對齊成功，則會包含新的開始位址。  
...
### <a name="remarks"></a>備註  
 如果 `const_cast<Ty*>(sp.get())` 傳回 Null 指標，此範本函式就會傳回空的 shared_ptr 物件；否則會傳回擁有 `sp` 所擁有之資源的 [shared_ptr 類別](../standard-library/shared-ptr-class.md)`<Ty>`物件。 運算式 `const_cast<Ty*>(sp.get())` 必須有效。  
...
### <a name="remarks"></a>備註  
 如果 `dynamic_cast<Ty*>(sp.get())` 傳回 Null 指標，此範本函式就會傳回空的 shared_ptr 物件；否則會傳回擁有 `sp` 所擁有之資源的 [shared_ptr 類別](../standard-library/shared-ptr-class.md)`<Ty>`物件。 運算式 `dynamic_cast<Ty*>(sp.get())` 必須有效。  
...
### <a name="remarks"></a>備註  
 如果 `sp` 是空的 `shared_ptr` 物件，此範本函式就會傳回空的 shared_ptr 物件 ；否則會傳回擁有 `sp` 所擁有之資源的 [shared_ptr 類別](../standard-library/shared-ptr-class.md)`<Ty>`物件。 運算式 `static_cast<Ty*>(sp.get())` 必須有效。  
