...
ms.locfileid: "72687674"
---
# <a name="money_get-class"></a>money_get 類別

類別樣板描述可以做為地區設定 facet 的物件，以控制 `CharType` 為貨幣值之類型序列的轉換。
...
第一個虛擬的受保護成員函式會嘗試比對序列 [ `first`, `last`) 中從 first 開始的一系列元素，直到它辨識出完整、非空白的貨幣輸入欄位為止。 如果成功，它會將此欄位轉換成一或多個十進位數的序列，並選擇性地在前面加上減號（`-`）來代表數量，並將結果儲存在[string_type](#string_type)物件*val*中。 它會傳回迭代器，此迭代器指定貨幣輸入欄位後的第一個元素。 否則，函式會以*val*儲存空的序列，並將 `ios_base::failbit` 設定為*狀態*。 它會傳回迭代器，此迭代器指定有效貨幣輸入欄位之任何前置詞後的第一個元素。 不論是上述哪一種情況，如果傳回值等於 `last`，函式就會在 `State` 中設定 `ios_base::eofbit`。

第二個虛擬受保護成員函式的行為與第一個相同，不同的是，如果成功，它會將選擇性帶正負號的數位序列轉換成**long double**類型的值，並將該值儲存在*val*中。
