...
> [!WARNING]
> 從 Visual Studio 2015 開始, C++標準程式庫同步處理類型是以 Windows 同步處理原始物件為基礎, 且不再使用 ConcRT (除非目標平臺是 windows XP)。 \<Mutex> 中定義的類別不應與任何 ConcRT 類型或函式搭配使用。
...
- `try_lock` 方法會嘗試在不造成封鎖的情況下取得 mutex 擁有權。 其傳回型別可以轉換成**bool** , 如果方法取得擁有權, 則為**true** , 否則為**false**。
...
「計時 mutex 類型」可滿足 mutex 類型的需求。 此外, 它具有`try_lock_for`和`try_lock_until`方法, 必須可使用一個引數呼叫, 而且必須傳回可轉換成**bool**的類型。 計時 mutex 型別可以使用其他引數來定義這些函式，前提是這些所有其他引數都有預設值。
...
|[once_flag 結構](../standard-library/once-flag-structure.md)|表示搭配範本`call_once`函式使用的結構, 以確保初始化程式碼只會呼叫一次, 即使有多個執行緒執行也一樣。|
|[try_to_lock_t 結構](../standard-library/try-to-lock-t-structure.md)|表示定義 `try_to_lock`物件的結構, 並用來選取的其中一個多載的`unique_lock`函式。|
