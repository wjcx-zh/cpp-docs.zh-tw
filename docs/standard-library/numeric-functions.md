...
*第一個*輸入迭代器，定址範圍中的第一個項目加總或根據指定的二進位運算合併。

*最後一個*輸入迭代器，定址範圍中的最後一個項目加總或根據指定的二進位運算，在反覆累積中實際包含的最後一個元素的後面一個位置，合併。

*val*初始值的每個項目是依序新增或根據指定的二進位運算合併使用。

*binary_op*二進位運算所要套用至指定的範圍和其先前的應用程式的結果中的每個項目。
...
*第一個*輸入迭代器定址的第一個元素，其項目是以其各自的前置項差異，或一對值要由另一個輸入範圍中指定的二進位運算。

*最後一個*輸入迭代器定址的最後一個元素，其項目是以其各自的前置項差異，或一對值要由另一個輸入範圍中指定的二進位運算。

*結果*輸出迭代器定址的第一個元素，其中的一系列差異或指定作業的結果都是儲存在目的範圍。

*binary_op*套用在一般化作業取代差異程序中的減法運算的二進位運算。
...
*first1*輸入迭代器，定址對象之內乘積或一般化的內乘積的第二個範圍是要計算的第一個範圍中第一個元素。

*last1*輸入迭代器，定址對象之內乘積或一般化的內乘積的第二個範圍是要計算的第一個範圍中最後一個元素。

*first2*輸入迭代器的第一個項目定址在第二個範圍之內乘積或一般化的內乘積的第一個範圍是要計算。

*val*的內乘積或一般化的內乘積的範圍之間是要加入的初始值。

*binary_op1*二進位運算取代內乘積的作業套用至內乘積一般化內乘積的總和。

*binary_op2*二進位運算取代內乘積項目中的作業的乘數的內乘積一般化。
...
*第一個*輸入迭代器，定址要填入的範圍內的第一個項目。

*最後一個*輸入迭代器，定址要填滿範圍中的最後一個項目。

*值*來儲存在第一個項目和進行後續增量針對後續元素的起始值。
...
*第一個*輸入迭代器，定址範圍中的第一個項目部分加總或根據指定的二進位運算合併。

*最後一個*輸入迭代器，定址範圍中的最後一個項目部分加總或根據指定的二進位運算，在反覆累積中實際包含的最後一個元素的後面一個位置，合併。

*結果*輸出迭代器定址系列部分總和或指定作業的結果為儲存在目的範圍的第一個項目。

*binary_op*套用在一般化作業取代的部分總和程序中的總和運算的二進位運算。
