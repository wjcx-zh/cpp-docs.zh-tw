...
*型別*\
正在測試、查詢或設定其屬性的基本項目資料類型。 *型別*也可以宣告**const**， **volatile**，或**const volatile**。
...
|[has_quiet_NaN](#has_quiet_nan)|測試類型是否有為無訊息模式的表示法不是數字 (NAN)，其為非信號。|
