...
*類型*\
正在測試、查詢或設定其屬性的基本項目資料類型。 *型* 別也可以宣告為 **`const`** 、 **`volatile`** 或 **`const volatile`** 。

## <a name="remarks"></a>備註

標頭會定義類型、、、、、、、、、、、、、、、、和的明確特製化 **`wchar_t`** **`bool`** **`char`** **`signed char`** **`unsigned char`** **`short`** **`unsigned short`** **`int`** **`unsigned int`** **`long`** **`unsigned long`** **`float`** **`double`** **`long double`** **`long long`** **`unsigned long long`** **`char16_t`** **`char32_t`** 。 針對這些明確特製化，成員 [numeric_limits：： is_specialized](#is_specialized) 為 **`true`** ，而且所有相關成員都具有有意義的值。 這個程式可提供額外的明確特製化。 類別的大部分成員函式會描述或測試的可能實作為 **`float`** 。

針對任意特製化，沒有任何成員包含有意義的值。 沒有有意義值的成員物件會儲存零 (或 **`false`**) ，而不會傳回有意義值的成員函式會傳回 `Type(0)` 。
...
|[小量](#epsilon)|傳回資料類型可以表示之介於 1 到大於 1 的最小值之間的差異。|
...
|[has_quiet_NaN](#has_quiet_nan)|測試類型是否有無訊息 (NAN) 的標記法，而非信號。|
...
|[is_specialized](#is_specialized)|測試類型是否有在類別範本中定義的明確特製化 `numeric_limits` 。|
|[最低](#lowest)|傳回最大負數的有限值。|
|[max](#max)|傳回類型的最大有限值。|
...
|[陷阱](#traps)|測試要報告算術例外狀況的設限是否會針對類型實作。|
...
函數會傳回類型的最小值，如果[has_denorm](#has_denorm)不等於則為[min](#min) `denorm_present` 。
...
### <a name="digits"></a><a name="digits"></a> 數位
...
### <a name="epsilon"></a><a name="epsilon"></a> 小量
...
此值為類型的 FLT_EPSILON **`float`** 。 `epsilon`針對類型，這是最小的正整數浮點數*N* ，因此可以表示*n*  +  `epsilon`  +  *n* 。
...
型別的列舉值 `const float_denorm_style` ，指出型別是否允許反正規化的值。

#### <a name="remarks"></a>備註

具有反正規化 `denorm_present` 值之浮點數類型的成員存放區，實際上是可變數的指數位數目。
...
**`true`** 如果偵測到精確度遺失，則為非正規化遺失; **`false`** 如果不是，則為。
...
**`true`** 如果類型有正無限大的標記法，則為， **`false`** 如果不是，則為。

#### <a name="remarks"></a>備註

**`true`** 如果[is_iec559](#is_iec559)為，就會傳回成員 **`true`** 。
...
**`true`** 如果 **類型** 具有無訊息 NAN 的表示，則為， **`false`** 如果不是，則為。

#### <a name="remarks"></a>備註

無訊息 NAN 是代表「不是數字」(not a number) 的編碼，不會在運算式中發出其存在訊號。 **`true`** 如果[is_iec559](#is_iec559)為 true，則傳回值。
...
帶訊號 NAN 是代表「不是數字」的編碼，會在運算式中發出其存在訊號。 **`true`** 如果[is_iec559](#is_iec559)為 true，則傳回值。
...
**`true`** 如果類型有一組限定的表示值，則為， **`false`** 如果不是，則為。

#### <a name="remarks"></a>備註

所有預先定義的類型都有一組限定的可表示值並傳回 **`true`** 。
...
**`true`** 如果計算沒有舍入錯誤，則為， **`false`** 如果不是，則為。
...
**`true`** 如果類型符合 IEC 559 標準; **`false`** 如果不是，則為。
...
**`true`** 如果類型有模數標記法，則為， **`false`** 如果不是，則為。
...
**`true`** 如果類型具有帶正負號的標記法; **`false`** 如果不是，則為。
...
測試類型是否有在類別範本中定義的明確特製化 `numeric_limits` 。
...
**`true`** 如果類型具有在類別範本中定義的明確特製化，則為， **`false`** 如果不是，則為。

#### <a name="remarks"></a>備註

指標以外的所有純量類型都有針對類別範本定義的明確特製化 `numeric_limits` 。
...
### <a name="lowest"></a><a name="lowest"></a> 最低
...
傳回類型的最大負數有限值 (如果是整數類型，通常是 `min()`，如果是浮點數類型則是 `-max()`)。 如果為，則傳回值是有意義的 `is_bounded` **`true`** 。
...
類型的最大有限值是 INT_MAX 類型 **`int`** 和 FLT_MAX **`float`** 。 如果 [is_bounded](#is_bounded) 為，則傳回值是有意義的 **`true`** 。
...
此成員函式傳回值僅對浮點數類型有意義。 `max_exponent`是類型 FLT_MAX_EXP 的值 **`float`** 。
...
此成員函式傳回值僅對浮點數類型有意義。 `max_exponent`是類型 FLT_MAX_10 的值 **`float`** 。
...
類型的最小正規化值是 INT_MIN 類型 **`int`** 和 FLT_MIN **`float`** 。 如果 [is_bounded](#is_bounded) 是 **`true`** 或 [is_signed](#is_signed) 為，則傳回值會有意義 **`false`** 。
...
此成員函式僅對浮點數類型有意義。 `min_exponent`是類型 FLT_MIN_EXP 的值 **`float`** 。
...
此成員函式僅對浮點數類型有意義。 `min_exponent10`是類型 FLT_MIN_10_EXP 的值 **`float`** 。
...
### <a name="radix"></a><a name="radix"></a> 基數
...
**`true`** 如果類型可以在四捨五入之前偵測出小值; **`false`** 如果不是，則為。
...
### <a name="traps"></a><a name="traps"></a> 陷阱
...
**`true`** 如果已針對類型執行補漏設，則為， **`false`** 如果不是，則為。
