...
類範本描述恆定轉發反覆運算器物件。 它會將其規則運算式物件 `match_results<BidIt>` 重複套用至迭代器範圍 `*pregex` 所定義的字元序列，藉以擷取 `[begin, end)`類型的物件。
...
|運算子|描述|
|-|-|
|[操作員!](#op_neq)|比較迭代器是否不相等。|
|[運算子*](#op_star)|存取指定的相符項目。|
|[運算子*](#op_add_add)|遞增迭代器。|
|[運算子*](#op_eq)|比較迭代器是否相等。|
|[運算子>](#op_arrow)|存取指定的相符項目。|
...
- [交換](../standard-library/regex-functions.md#swap)
...
## <a name="regex_iteratordifference_type"></a><a name="difference_type"></a>regex_iterator::d)類型
...
## <a name="regex_iteratoroperator"></a><a name="op_neq"></a>regex_iterator:操作員!
...
## <a name="regex_iteratoroperator"></a><a name="op_star"></a>regex_iterator::操作員*
...
## <a name="regex_iteratoroperator"></a><a name="op_add_add"></a>regex_iterator::操作員*
...
## <a name="regex_iteratoroperator"></a><a name="op_eq"></a>regex_iterator::操作員*
...
如果`*this`*和右側*都是序列結尾反覆運算器,或者序列結尾反覆運算器`begin == right.begin`和`end == right.end``pregex == right.pregex`、`flags == right.flags`和, 則成員函數將返回 true。 否則會傳回 false。

## <a name="regex_iteratoroperator-gt"></a><a name="op_arrow"></a>regex_iterator::操作員-&gt;
...
## <a name="regex_iteratorpointer"></a><a name="pointer"></a>regex_iterator::p奧米特
...
## <a name="regex_iteratorreference"></a><a name="reference"></a>regex_iterator:參考
...
第一個建構函式會建構結束序列 (end-of-sequence) 迭代器。 第二個建構函數用`begin`*第一*個初始化儲存值`end`,使用 最後一*個*儲存值,`pregex`使用`&re`的記憶體值,`flags`用 f*的記憶體*值。 然後呼叫 `regex_search(begin, end, match, *pregex, flags)`。 如果搜尋失敗，此運算子便會將物件設定為結束序列迭代器。
