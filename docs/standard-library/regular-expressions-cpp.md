...
除了「正判斷提示」、「負判斷提示」或「錨點」之外的任何項目後面都可接著重複計數。 在 `basic` 和 `grep` 中，最常見的重複計數格式為 "{`min`,`max`}" 或 "\\{`min`,`max`\\}"。 後面接著這個重複計數格式的項目至少符合與項目相符之序列的 `min` 後續發生次數，但不超過 `max` 後續發生次數。 例如，"{2,3}"符合目標序列"aa"和目標序列"aaa"，但目標序列"a"或目標序列"aaaa"。
...
- 「{2}"符合目標序列"aa"，但目標序列"a"或目標序列"aaa"。

- 「{2,}"符合目標序列"aa"、 目標序列"aaa"，並依此類推，但不符合目標序列"a"。

- 「\*"符合目標序列""，目標序列"a"、 目標序列"aa"，依此類推。
...
在  `ECMAScript`，所有重複計數格式都後面可以接著字元 '？ '，其會指定*非窮盡重複*。

### <a name="concatenation"></a>串連

規則運算式項目 (不論是否有「重複計數」) 可串連來形成較長的規則運算式。 結果運算式符合由個別項目所比對之序列串連而成的目標序列。 例如，"{2,3}b"符合目標序列"aab"和目標序列"aaab"，但不符合目標序列"ab"或目標序列"aaaab"。
...
負判斷提示符合任何項目，但其內容除外。 它不會使用目標序列中的任何字元。 比方說，"(!aa) (\*)"符合目標序列"a"並將擷取群組 1 與子序列"a"。 它不符合目標序列 "aa" 或目標序列 "aaa"。
...
非窮盡重複使用目標序列中符合模式的最短子序列。 窮盡重複使用最長子序列。 比方說，"(a+) (\*b)"符合目標序列"aaab"。 使用非窮盡重複時，它會將擷取群組 1 與目標序列開頭的子序列 "a" 關聯，並將擷取群組 2 與目標序列結尾的子序列 "aab" 關聯。 使用窮盡重複時，它會將擷取群組 1 與子序列 "aaa" 關聯，並將擷取群組 2 與子序列 "b" 關聯。
...
- '\*' 在所有情況下，除非它是規則運算式中的第一個字元或是初始的第一個字元具有特殊意義 ' ^' 時是第一個字元，或在規則運算式中，擷取的群組] 或 [第一個字元初始 ' ^' 中的擷取群組。
