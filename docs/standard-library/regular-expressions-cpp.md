...
ms.locfileid: "81368519"
---
# <a name="regular-expressions-c"></a>規則運算式 (C++)

C++標準庫支援多個正則運算式語法。 本主題討論使用正則表達式時可用的語法變體。

## <a name="regular-expression-grammar"></a><a name="regexgrammar"></a> 規則運算式文法

要使用的正則表達式語法通過使用其中一個`std::regex_constants::syntax_option_type`枚舉值來指定。 這些正則運算式語法在std::regex_constants 中定義:

- `ECMAScript`:這是最接近 JAVAScript 和 .NET 語言使用的語法。
- `basic`:POSIX 基本正則表達式或BRE。
- `extended`:POSIX 擴展正則表達式或ERE。
- `awk`:這是`extended`,但它具有非列印字元的其他轉義。
- `grep`:這是`basic`,但它也允許換行符分隔。
- `egrep`:這是`extended`,但它也允許換行符分隔。

預設情況下,如果未指定語法,`ECMAScript`則假定為。 只能指定一個語法。

除了語法之外,還可以應用幾個標誌:

- `icase`:匹配時忽略大小寫。
- `nosubs`:忽略標記的匹配項(即括弧中的運算式);不存儲替換。
- `optimize`:使匹配更快,並可能犧牲更大的施工時間。
- `collate`:使用區域設置敏感的排序規則序列(例如,窗體的範圍"[a-z])。"

零個或多個標誌可以與語法組合,以指定正則表達式引擎行為。 如果僅指定標誌,`ECMAScript`則假定為語法。
...
- 格式為 "[`expr`]" 的「括號運算式」**，其符合目標序列中且在運算式 `expr` 所定義集合中的字元或定序項目，或者，若格式為 "[^`expr`]"，則符合目標序列中但不在運算式 `expr` 所定義集合中的字元或定序項目。
  ...
  - 表單`ch1`-`ch2`字元「 字*元範圍*」。 會將封閉範圍 [`ch1`, `ch2`] 中的值所代表的字元加入到 `expr` 定義的這個集合。
...
在 `basic` 和  中，格式為 "( *subexpression* )" 或 "\\( *subexpression`grep`* \\)" 的「擷取群組」**，其符合目標序列中透過分隔符號之間的模式進行比對的字元序列。

- 表單\\`k`""的*標識轉義*,與目標序列`k`中的字元 匹配。
...
在`ECMAScript``basic`中`grep`,和 , 元素也可以是表\\`dd`單"*的反向參考*,`dd`其中表示與目標序列中與 N 捕獲*組*匹配的字元序列匹配的十進位值 N。 例如，"(a)\1" 符合目標序列 "aa"，因為第一個 (且唯一) 擷取群組符合初始序列 "a"，而 \1 符合最終序列 "a"。

在 `ECMAScript` 中，項目也可以是下列其中一項：

- 表單物件(?:*子表示式*)'*的非捕獲群組*。 符合目標序列中由分隔符號之間的模式所比對的字元序列。
...
- 表單「\a」、「\b」、「\f」、「\n」、「\r」、「\t」\\\\或"\v」*的檔案格式轉義*。 這些分別符合目標序列中的反斜線、警示、退格鍵、換頁字元、新行字元、歸位字元、水平索引標籤和垂直索引標籤。

- 形式「」\\`ooo`的*八進位逸出逸出序列*。 符合目標序列中由一個、兩個或三個八進位數字 `ooo` 表示的字元。

### <a name="repetition"></a>重複

除了「正判斷提示」**、「負判斷提示」** 或「錨點」** 之外的任何項目後面都可接著重複計數。 在 `basic` 和 `grep` 中，最常見的重複計數格式為 "{`min`,`max`}" 或 "\\{`min`,`max`\\}"。 後面接著這個重複計數格式的項目至少符合與項目相符之序列的 `min` 後續發生次數，但不超過 `max` 後續發生次數。 例如,"a"{2,3}匹配目標序列"aa"和目標序列"aaa",但不匹配目標序列"a"或目標序列"aaa"。
...
- {2}a" 符合目標序列"aa",但不匹配目標序列"a"或目標序列"aa"。

- a"{2,}符合目標序列"aa",目標序列"aaa"等,但與目標序列"a"不匹配。

- "匹配\*目標序列","目標序列"a",目標序列"a",目標序列"aa",等等。
...
- "a? 匹配目標序列""和目標序列"a",但不匹配目標序列"aa"。

- "a+" 符合目標序列 "a"、目標序列 "aa" 等等，但不符合目標序列 ""。

在`ECMAScript`中,所有形式的重複計數可以跟後面跟字元"?",它指定*非貪婪的重複*。

### <a name="concatenation"></a>串連

規則運算式項目 (不論是否有「重複計數」**) 可串連來形成較長的規則運算式。 結果運算式符合由個別項目所比對之序列串連而成的目標序列。 例如,"a{2,3}b"匹配目標序列"aab"和目標序列"aaab",但與目標序列"ab"或目標序列"aaab"不匹配。
...
## <a name="grammar-summary"></a><a name="grammarsummary"></a> 文法摘要

下表摘要說明各種規則運算式文法的功能：

|項目|basic|延伸|ECMAScript|grep|egrep|awk|
|-------------|---------|---------|----------|----------|-----------|---------|
|使用 '&#124;' 的替代||+|+||+|+|
|使用 '\n' 的替代||||+|+||
|錨點|+|+|+|+|+|+|
...
## <a name="semantic-details"></a><a name="semanticdetails"></a> 語意詳細資料
...
- \*a" 符合目標序列"aaa",但與目標序列"a"\*不匹配。

- a" 與目標序列"aaa"不匹配,但與目標序列"a"\*\\\*匹配。
...
|`extended`, `egrep`|[''',"","","","'","'","","$","\","?","&#124;"|\\ \*|
|`awk`|`extended` 加上 { '"', '/' }|
|`ECMAScript`|所有字元，但可以是識別項一部分的字元除外。 通常,這包括字母、數位、"$"和\_單碼轉義序列。 如需詳細資訊，請參閱＜ECMAScript 語言規格＞。|
...
負判斷提示符合任何項目，但其內容除外。 它不會使用目標序列中的任何字元。 例如,"(!aa)(a)"\*匹配目標序列"a",並將捕獲組 1 與子序列"a"關聯。 它不符合目標序列 "aa" 或目標序列 "aaa"。
...
非擷取群組將其內容標記為規則運算式文法中的單一單位，但不會標記目標文字。 例如,"(a)(??:b)(c)"\*將目標文本"abbc"匹配,並將捕獲組 1 與子序列"a"關聯,捕獲組 2 與子序列"c"。

### <a name="non-greedy-repetition"></a>非窮盡重複

非窮盡重複使用目標序列中符合模式的最短子序列。 窮盡重複使用最長子序列。 例如,"(a+)(a\*b)"與目標序列"aaab"匹配。 使用非窮盡重複時，它會將擷取群組 1 與目標序列開頭的子序列 "a" 關聯，並將擷取群組 2 與目標序列結尾的子序列 "aab" 關聯。 使用窮盡重複時，它會將擷取群組 1 與子序列 "aaa" 關聯，並將擷取群組 2 與子序列 "b" 關聯。
...
- '\*' 在所有情況下都有特殊的含義,除非它是正則運算式中的第一個字元或正則運算式中初始"*"之後的第一個字元,或者它是捕獲組中捕獲組的第一個字元或初始"*"之後的第一個字元。
...
- "[aaa)(a)]"\*匹配目標序列"aaa",並將捕獲組 1 與子序列"aaaa"關聯。

- "(aa)(a\*)"匹配目標序列"aaa",並將捕獲組 1 與目標序列開頭的子序列"aa"關聯,捕獲組 2 與目標序列末尾的子序列"aa"。
...
## <a name="matching-and-searching"></a><a name="matchingandsearching"></a> 比對和搜尋
...
## <a name="format-flags"></a><a name="formatflags"></a> 格式旗標
...
|"$$"||"$"|
||"&"\\|"&"|
|"$"(\`美元符號後跟回報價)*與正規表示式符合的子序列前面的字元序列 (`[match.prefix().first, match.prefix().second)`)|
|"$'" (貨幣符號，後面接著正引號)||在符合規則運算式的子序列之後的字元序列 (`[match.suffix().first, match.suffix().second)`)|
|"$n"|"\n"|與位置`n`擷取群組的匹配的字元序列,`n`其中的數位介於 0`[match[n].first, match[n].second)`和 9 之間 ()|
||"\\\n"|"\n"|
|"$nn"||與位置`nn`擷取群組匹配的字元序列,`nn`其中的數位介於 10 和`[match[nn].first, match[nn].second)`99 之間 ()|

## <a name="see-also"></a>另請參閱

[C++標準庫概述](../standard-library/cpp-standard-library-overview.md)
