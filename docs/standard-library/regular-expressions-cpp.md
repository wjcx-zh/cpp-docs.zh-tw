...
ms.locfileid: "68451382"
---
# <a name="regular-expressions-c"></a>規則運算式 (C++)

C++標準程式庫支援多個正則運算式語法。 本主題討論使用正則運算式時可用的文法變化。

## <a name="regexgrammar"></a> 規則運算式文法

要使用的正則運算式文法是藉由使用其中一個`std::regex_constants::syntax_option_type`列舉值來指定。 這些正則運算式文法定義于 std:: RegEx_constants:

- `ECMAScript`：這最接近 JavaScript 和 .NET 語言所使用的文法。
- `basic`：POSIX 基本正則運算式或 BRE。
- `extended`：POSIX 擴充的正則運算式或 ERE。
- `awk`：這是`extended`, 但它對於非列印字元有額外的轉義。
- `grep`：這是`basic`, 但它也允許分行符號 (' \n ') 字元來分隔替代。
- `egrep`：這是`extended`, 但它也允許分行符號分隔替代。

根據預設, 如果未指定任何文法, `ECMAScript`則會假設為。 只能指定一個文法。

除了文法外, 還可以套用數個旗標:
- `icase`：比對時忽略大小寫。
- `nosubs`：忽略標記的相符專案 (也就是以括弧括住的運算式);不會儲存任何替代。
- `optimize`：以更高的結構時間為代價, 讓比對更快。
- `collate`：使用區分地區設定的定序序列 (例如, 格式為 "[a-z]" 的範圍)。

零個或多個旗標可能會與文法結合, 以指定正則運算式引擎行為。 如果只指定旗標, `ECMAScript`則會假設為文法。
...
- 格式為 "(？:*子運算式*)" 的*非 capture 群組*。 符合目標序列中由分隔符號之間的模式所比對的字元序列。
...
- 「答？」 比對目標序列 "" 和目標序列 "a", 但不符合目標序列 "aa"。

- "a+" 符合目標序列 "a"、目標序列 "aa" 等等，但不符合目標序列 ""。

在`ECMAScript`中, 所有的重複計數形式後面都可以接著字元 '？ ', 這會指定*非貪婪的重複*專案。
...
|`extended`、 `egrep`|{ '(', ')', '{', '.', '[', '\\', '\*', '^', '$', '+', '?', '&#124;' }|
|`awk`|`extended` 加上 { '"', '/' }|
|`ECMAScript`|所有字元，但可以是識別項一部分的字元除外。 一般來說, 這包括字母、數位、' $ '、'\_' 和 unicode 逸出序列。 如需詳細資訊，請參閱＜ECMAScript 語言規格＞。|
...
負判斷提示符合任何項目，但其內容除外。 它不會使用目標序列中的任何字元。 例如, "(! aa) (a\*)" 符合目標序列 "a", 並將 capture 群組1與子序列 "a" 產生關聯。 它不符合目標序列 "aa" 或目標序列 "aaa"。
...
非擷取群組將其內容標記為規則運算式文法中的單一單位，但不會標記目標文字。 例如, "(a) (？: b)\*(c)" 符合目標文字 "abbc", 並將 capture 群組1與子序列 "a" 和 capture 群組2與子序列 "c" 產生關聯。
...
- 在\*所有情況下, ' ' 具有特殊意義, 除非它是正則運算式中的第一個字元, 或正則運算式中初始 ' ^ ' 後面的第一個字元, 或者它是 capture 群組的第一個字元, 或是第一個在 capture 群組中的初始 ' ^ ' 之後。
...
- "(= aa) (a\*)" 符合目標序列 "aaaa", 並將捕捉群組1與子序列 "aaaa" 產生關聯。

- "(aa) (a\*)" 符合目標序列 "aaaa", 並將 capture 群組1與目標序列開頭的子序列 "aa" 建立關聯, 並在目標序列結尾使用子序列 "aa" 的 capture 群組2。
...
|"$$"||"$"|
||"\\&"|"&"|
|"$\`" (貨幣正負號後面接著後引號)||在符合規則運算式的子序列之前的字元序列 (`[match.prefix().first, match.prefix().second)`)|
|"$'" (貨幣符號，後面接著正引號)||在符合規則運算式的子序列之後的字元序列 (`[match.suffix().first, match.suffix().second)`)|
|"$n"|"\n"|在位置`n`符合 capture 群組的字元序列, 其中`n`是介於0和 9 (`[match[n].first, match[n].second)`) 之間的數位|
||"\\\n"|"\n"|
|"$nn"||在位置`nn`符合 capture 群組的字元序列, 其中`nn`是介於10到 99 (`[match[nn].first, match[nn].second)`) 之間的數位|
