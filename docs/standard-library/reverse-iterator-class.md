...
## <a name="remarks"></a>備註  
 ...
 迭代器的範圍是 [*第一個*，*最後一個*)，其中在左邊的大括號表示包含*第一個*和右括號表示包含的項目，最多，但不含*最後*本身。 反向序列中包含相同項目 [ **rev** - *第一個*， **rev** - *最後一個*) 以便如果*最後一個*是一個過去的結尾中的項目序列，則第一個項目**rev** - *第一個*在反向的序列點\*(*最後*-1)。 將所有反向迭代器與其基礎迭代器關聯的識別為：  
