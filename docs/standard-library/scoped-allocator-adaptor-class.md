---
title: scoped_allocator_adaptor 類別
ms.date: 11/04/2016
f1_keywords:
...
ms.locfileid: "81373403"
---
# <a name="scoped_allocator_adaptor-class"></a>scoped_allocator_adaptor 類別
...
類範本封裝一個或多個分配器的嵌套。 這類每個類別都具備類型 `outer_allocator_type` (`Outer` 的同義字) 的最外層配置器，其為 `scoped_allocator_adaptor` 物件的公用基底。 `Outer` 可用來配置容器所使用的記憶體。 您可以呼叫 `outer_allocator`，來取得此配置器基底物件的參考。
...
不屬於可見介面的幾個概念有助於描述此類範本的行為。 「最外層的配置器」** 會調解所有對建構和終結方法的呼叫。 這實際上是透過遞迴函式 `OUTERMOST(X)` 所定義，其中 `OUTERMOST(X)` 為下列其中一項。
...
|名稱|描述|
|----------|-----------------|
|[scoped_allocator_adaptor:重新繫結結構](#rebind_struct)|將 `Outer::rebind\<Other>::other` 類型定義為 `scoped_allocator_adaptor\<Other, Inner...>` 的同義字。|

### <a name="methods"></a>方法

|名稱|描述|
|----------|-----------------|
|[配置](#allocate)|使用 `Outer` 配置器來配置記憶體。|
|[建構](#construct)|建構物件。|
|[去分配](#deallocate)|使用外部配置器來取消配置物件。|
|[摧毀](#destroy)|終結指定的物件。|
...
## <a name="scoped_allocator_adaptorconstruct"></a><a name="construct"></a>scoped_allocator_adaptor:構造
...
第一種方法通過呼叫`Outermost_traits::construct(OUTERMOST(*this), ptr, xargs...)`在*ptr*建`xargs...`構物件, 其中為以下方法之一。
...
第二種方法通過呼叫`Outermost_traits::construct(OUTERMOST(*this), &ptr->first, xargs...)`在*ptr*建構`xargs...`對`first...`物件, 其中 修改與上述`Outermost_traits::construct(OUTERMOST(*this), &ptr->second, xargs...)`清單`xargs...`中相同,`second...`並在上 文清單中修改。
...
## <a name="a-nameop_as--scoped_allocator_adaptoroperator"></a><a name="op_as">scoped_allocator_adaptor::操作員*
...
## <a name="a-nameop_eq_eq--scoped_allocator_adaptoroperator"></a><a name="op_eq_eq">scoped_allocator_adaptor::操作員*
...
## <a name="a-nameop_noeq--scoped_allocator_adaptoroperator"></a><a name="op_noeq">scoped_allocator_adaptor::操作員!
...
## <a name="scoped_allocator_adaptorrebind-struct"></a><a name="rebind_struct"></a>scoped_allocator_adaptor:重新繫結結構

將 `Outer::rebind\<Other>::other` 類型定義為 `scoped_allocator_adaptor\<Other, Inner...>` 的同義字。

結構重新綁定* 類型defOther_traits:重新綁\<定 其他>Other_alloc;類型\<defscoped_allocator_adaptorOther_alloc, 內部>其他;*;

## <a name="scoped_allocator_adaptorscoped_allocator_adaptor-constructor"></a><a name="scoped_allocator_adaptor"></a>scoped_allocator_adaptor:scoped_allocator_adaptor構造函數

建構 `scoped_allocator_adaptor` 物件。 還包括析構函數。
...
第一個建構函式預設會建構它的預存配置器物件。 接下來的三個構造函數中的每一個都從*右側*的相應物件構造其存儲的分配器物件。 最後一個建構函式會從引數清單的對應引數中建立它的預存配置器物件。
...
這個方法會有效地傳回 `scoped_allocator_adaptor(Outer_traits::select_on_container_copy_construction(*this), inner_allocator().select_on_container_copy_construction())`。 結果是一個新`scoped_allocator_adaptor`物件,每個儲存的分配器對象通過`al.select_on_container_copy_construction()`調用 相應的分配器*al*進行初始化。
