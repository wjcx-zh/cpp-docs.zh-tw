...
*Traits*類型，提供函式物件可比較兩個項目值做為排序鍵來判斷集合中的相對順序。 這是選擇性引數，而二元述詞 **less** *\<Key>* 為預設值。
...
*配置器*表示預存配置器物件，封裝有關集合的配置和解除配置之記憶體的詳細資訊的型別。 這個引數為選擇性，而且預設值是 **allocator *\<機碼 >。*
...
*索引鍵*要與所搜尋之 set 中元素的排序鍵比較的引數索引鍵。
