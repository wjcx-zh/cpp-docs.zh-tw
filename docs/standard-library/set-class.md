...
`Traits` 類型，提供可以比較兩個項目值做為排序索引鍵，來判斷集合中的相對順序的函式物件。 這是選擇性引數，而二元述詞 **less** *\<Key>* 為預設值。
...
`Allocator` 表示封裝有關集合的配置和解除配置記憶體的詳細資訊的預存配置器物件的型別。 這個引數是選擇性的而且預設值是 **配置器 * * *\<金鑰 >。*
...
`key` 要與所搜尋之 set 中元素的排序索引鍵比較的引數索引鍵。
