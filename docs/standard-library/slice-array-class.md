...
您只能針對`slice_array<Type>` `sl` valarray [ &#91; &#93; ](../standard-library/valarray-class.md#op_at)的配量,撰寫一個格式為vasl`va`的運算式來建立物件。 類別 slice_array 的成員函式的行為就像是針對定義的`valarray<Type>`對應函式簽章, 但只有所選項目的順序會受到影響。 slice_array 所控制的序列是由配量建構函式的三個參數所定義，亦即配量中第一個項目的索引、項目數，以及項目之間的距離。 從`va` **va** [`slice`(2, 5, 3)] 宣告的 valarray 剪下的 slice_array 會從中`va`選取索引2、5、8、11和14的元素。 索引必須有效，程序才能有效。
