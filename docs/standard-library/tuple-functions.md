...
*元組*\
包含任意數目之元素的 `std::tuple`。

### <a name="remarks"></a>備註

範本函式會傳回*位於索引索引*或 `tuple` 物件中類型*T*之值的參考。
...
此範本函式會傳回 `tuple<V1, V2, ..., VN>(t1, t2, ..., tN)`，其中，當對應的類型 `Ti` `cv` `reference_wrapper<X>`時，會 `X&` 每個類型 `Vi`。否則，它會 `Ti`。
