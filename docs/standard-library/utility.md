...
> [!NOTE]
> \<公用程式 > 標頭使用陳述式`#include <initializer_list>`。 它也是指`class tuple`中所定義\<tuple >。
...
|||
|-|-|
|[chars_format](../standard-library/chars-format-class.md)|基本的數值轉換的浮點數格式。|
...
|||
|-|-|
|[as_const](../standard-library/utility-functions.md#asconst)|傳回型別。|
|[declval](../standard-library/utility-functions.md#declval)|速記運算式評估。|
|[exchange](../standard-library/utility-functions.md#exchange)|將新的值指派給物件，並傳回其舊值。|
...
|||
|-|-|
|[from_chars_result](../standard-library/from-chars-result-structure.md)|結構，用於`from_chars`。|
...
|[piecewise_construct_t](../standard-library/piecewise-construct-t-structure.md)|用來保留個別的建構函式和函式多載的類型。|
|[to_chars_result](../standard-library/to-chars-result-structure.md)|結構，用於`to_chars`。|
