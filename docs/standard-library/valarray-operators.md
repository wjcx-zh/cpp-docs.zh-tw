...
第一個範本運算子會傳回[valarray \<bool> ](../standard-library/valarray-bool-class.md)類別的物件，其中每個元素 `I` 都是 `left[I] != right[I]` 。

第二個範本運算子會儲存在專案中 `I` `left[I] != right` 。

第三個範本運算子會將儲存在專案中 `I` `left != right[I]` 。
...
## <a name="operator"></a><a name="op_mod"></a>操作
...
## <a name="operatoramp"></a><a name="op_amp"></a>操作&amp;
...
位元 `AND` 與邏輯 `AND` 有相同的真值表，但是適用於個別位元層級上的資料型別。 [operator&&](#op_amp_amp) 適用於項目層級，會將所有非零的值視為 true，且結果為布林值的 valarray。 相反地 `AND` ，位[運算子&](#op_amp)可能會導致 valarray 的值不是0或1，視位運算的結果而定。
...
## <a name="operatorampamp"></a><a name="op_amp_amp"></a>操作&amp;&amp;
...
Valarray，其元素的類型為 bool，而是 `AND` *left*和*right*邏輯運算的元素組合。

### <a name="remarks"></a>備註

邏輯 `ANDoperator&&` 會套用在專案層級上，並將所有非零的值計算為 true，而結果會是布林值的 valarray。 相較之下，運算子的位版本 `AND` [&，](#op_amp)會產生0或1以外的值 valarray，視位運算的結果而定。
...
- **`true`** 如果*左*元素或值大於對應的*右*元素或值，則為。

- **`false`** 如果*左*元素或值不大於對應的*右*元素或值，則為。
...
- **`true`** 如果*左*元素或值大於或等於對應的*右*元素或值，則為。

- **`false`** 如果*左*元素或值小於對應的*右*元素或值，則為。
...
- **`true`** 如果*左*元素或值小於對應的*右*元素或值，則為。

- **`false`** 如果*左*元素或值不小於對應的*右*元素或值，則為。
...
- **`true`** 如果*左*元素或值小於或等於對應的*右*元素或值，則為。

- **`false`** 如果*左*元素或值大於對應的*右*元素或值，則為。
...
第一個範本運算子會傳回[valarray \<bool> ](../standard-library/valarray-bool-class.md)類別的物件，其中每個元素 `I` 都是 `left[I] == right[I]` 。 第二個範本運算子會儲存在專案中 `I` `left[I] == right` 。 第三個範本運算子會將儲存在專案中 `I` `left == right[I]` 。
...
## <a name="operator"></a><a name="op_xor"></a>運算子 ^
...
位互斥 `OR` （ **XOR**）具有下列的語義：給定位*b*1 和*b*2，如果只有一個位為 true，則*b*1 **XOR** *b*2 為; 如果兩個位都是 false，則為 **`true`** ; 如果兩個位皆為 true，則為 **`false`** 。
...
## <a name="operator124"></a><a name="op_or"></a>運算子&#124;
...
Valarray，其元素為 `OR` *left*和*right*之位運算的元素組合。

### <a name="remarks"></a>備註

位運算只能用來操作 **`char`** 和 **`int`** 資料類型和 variant 中的位，而不能用於 **`float`** 、 **`double`** 、 **longdouble**、 **`void`** **`bool`** 或其他更複雜的資料類型。

位元 OR 與邏輯 `OR` 有相同的真值表，但是適用於個別位元層級上的資料型別。 假設位*b*1 和*b*2， *b*1 `OR` *b*2 是 **`true`** 至少其中一個位為 true，或是 **`false`** 兩個位皆為 false。 邏輯 `OR` [運算子&#124;&#124;](../standard-library/valarray-operators.md#op_lor)適用于元素層級、將所有非零值計算為 **`true`** ，且結果為布林值的 valarray。 相反地，位元 OR `operator|` 會導致 valarray 的值非 0 即 1，取決於位元運算的結果。
...
## <a name="operator124124"></a><a name="op_lor"></a>運算子&#124;&#124;
...
Valarray，其元素的類型為 **`bool`** ，且為*left*和*right*的邏輯 OR 運算的元素組合。

### <a name="remarks"></a>備註

邏輯 `OR` `operator||` 會套用在專案層級上，並將所有非零值計算為 **`true`** ，而結果會是布林值的 valarray。 相反地，位元版本的 `OR`、[operator&#124;](../standard-library/valarray-operators.md#op_or)，會導致 valarray 的值非 0 即 1，取決於位元運算的結果。
