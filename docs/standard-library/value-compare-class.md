...
在 hash_map 所包含的整個元素`value_types`之間 value_compare 所提供的比較準則, 是由輔助類別結構的個別元素索引鍵之間的比較引發。 成員函式運算子會使用儲存`comp`在 value_compare `key_compare`所提供的函式物件中之類型的物件, 來比較兩個專案的排序關鍵字元件。
