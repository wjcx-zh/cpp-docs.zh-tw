...
類別樣板描述產生使用者指定之浮點類型值的分佈，如果沒有提供，則為**double**類型，這是根據 Weibull 分佈散發。 下表提供各個成員的文章連結。
...
如需 Weibull 分配的詳細資訊，請參閱 Wolfram MathWorld 文章 [Weibull 分配 (英文)](http://mathworld.wolfram.com/WeibullDistribution.html)。
