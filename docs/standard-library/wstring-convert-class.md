...
ms.locfileid: "81366358"
---
# <a name="wstring_convert-class"></a>wstring_convert 類別

類範本`wstring_convert`在寬字串和位元組字串之間執行轉換。
...
類範本描述控制類`std::basic_string<Elem>`的寬字串物件和類`std::basic_string<char>`的位元組位元串物件(`std::string`也稱為 )之間的轉換的物件。 類範本定義這些類型`wide_string`,並`byte_string`作為 這兩種類型的同義詞。 `Elem` 值的序列 (儲存在 `wide_string` 物件中) 與多位元組序列 (儲存在 `byte_string` 物件中) 之間的轉換，是由類別 `Codecvt<Elem, char, std::mbstate_t>` 的物件所執行，其符合標準程式碼轉換 facet `std::codecvt<Elem, char, std::mbstate_t>` 的需求。
...
## <a name="wstring_convertconverted"></a><a name="converted"></a>wstring_convert:轉換
...
如果[轉換狀態](../standard-library/wstring-convert-class.md)物件*不是*用顯式值建構的,則在轉換開始之前,該物件將設置為其預設值(初始轉換狀態)。 否則會維持不變。
...
## <a name="wstring_convertstate"></a><a name="state"></a>wstring_convert:狀態
...
|*沃斯*|要轉換的 [wide_string](#wide_string)。|
|*第一*|項目範圍中要轉換的第一個項目。|
|*最後*|項目範圍中要轉換的最後一個項目。|
