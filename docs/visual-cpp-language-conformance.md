...
|&nbsp;&nbsp;[N2634 運算式 SFINAE](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2008/n2634.html)|部分 <sup>[C](#note_C)</sup>|
...
<a name="note_D"></a>__D__ 編譯器的 C99 前置處理器規則支援在 Visual Studio 2017 中仍未完善。 支援 Variadic 巨集，但在前置處理器的行為方面有很多 Bug。  
...
<a name="note_H"></a>__H__ 此功能原先於 Visual Studio 2017 15.3 版的預覽中提供，但由於發現錯誤，因此已將它從該版本中移除。  
<a name="note_J"></a>__J__ Visual Studio 2015 中未完成的功能分別列於此表各處。  
<a name="note_K"></a>__K__ 基於歷程原因，\<experimental/filesystem> 和 \<filesystem> 都會實作 Filesystem TS，但其實作必須在移動其命名空間之前加以修正。 未完成上述工作之前，功能都會標示為尚未實作。  
<a name="note_L"></a>__L__ 由編譯器內建支援。 此內建尚未於 Clang 中提供。 該功能已於 Intellisense 中提供但尚未啟用。   
<a name="note_14"></a>__14__ 這些 C++17 功能一律會啟用，就算在指定 [/std:c++14](./build/reference/std-specify-language-standard-version.md) (預設值) 的情況下也一樣。 這是因為該功能已在 **/std** 選項推出之前實作，或是因為條件式實作過於複雜。  
