...
- **/std:c++14** 和 **/std:c++latest**︰這些編譯器選項可讓您在專案中加入特定的 ISO C++ 程式設計語言版本。 如需詳細資訊，請參閱 [/std (指定語言標準版本)](build/reference/std-specify-language-standard-version.md)。 大多數新草稿標準功能都在 **/std:c++latest** 選項的防護範圍內。
...
- [/permissive-](build/reference/permissive-standards-conformance.md)︰啟用所有嚴格的標準一致性編譯器選項，並停用大部分 Microsoft 專用的編譯器延伸模組 (但 `__declspec(dllimport)` 即為不包含在內的例子)。 根據預設，這個選項會關閉，但將來某個時候會改為根據預設開啟。
   ...
   **/permissive-** 一致性模式包含對兩階段名稱查詢的部分支援。 如需詳細資訊，請參閱 [Visual Studio 2017 中的 C++ 一致性改善](cpp-conformance-improvements-2017.md)。
