...
如需預先定義的快速鍵的清單，請參閱 <<c0> [ 快速鍵](../windows/predefined-accelerator-keys.md)。
...
**識別碼**屬性參考程式碼中的每個快速鍵對應表項目。 此項目是在使用者按下快速鍵或按鍵組合，則程式會收到的命令值。 若要讓功能表項目一樣加速器，請他們**識別碼**相同，因此只要**識別碼**加速器的資料表是相同**識別碼**功能表資源。

有三個屬性的每個加速器**識別碼**:**修飾詞**，**金鑰**，和**類型**

#### <a name="modifier-property"></a>輔助按鍵屬性
...
以下是合法的項目**修飾詞**快速鍵對應表中的屬性：
   ...
   |**Ctrl**|使用者必須按下**Ctrl**之前**金鑰**值。 ASCII 類型無效。|
   |**Shift**|使用者必須按下**Shift**之前**金鑰**值。|
   |**Ctrl+Alt**|使用者必須按下**Ctrl**並**Alt**之前**金鑰**值。 ASCII 類型無效。|
   |**Ctrl+Shift**|使用者必須按下**Ctrl**並**Shift**之前**金鑰**值。 ASCII 類型無效。|
   |**Alt+Shift**|使用者必須按下**Alt**並**Shift**之前**金鑰**值。 ASCII 類型無效。|
   |**Ctrl+Alt+Shift**|使用者必須按下**Ctrl**， **Alt**，並**Shift**之前**金鑰**值。 ASCII 類型無效。|

#### <a name="key-property"></a>索引鍵屬性
...
以下是合法的項目**金鑰**快速鍵對應表中的屬性：
   ...
   |鍵盤字元 A – Z 範圍內 （僅有大寫） 加上插入號 (^)。<br/><br/>比方說，^ c。|此選項與按下時，請輸入索引鍵的 ASCII 值**Ctrl**按住按鍵。|
...
> [!TIP]
> 若要定義快速鍵快顯是以滑鼠右鍵按一下某個項目或快速鍵對應表中的多個項目。 選擇**下一步 輸入的按鍵**，然後在鍵盤上按任何按鍵或按鍵組合。
>
> **下一步 輸入的按鍵**命令也會提供**編輯**功能表。

#### <a name="type-property"></a>類型屬性

**型別**屬性會決定是否與加速器相關聯的快速鍵組合**識別碼**會解譯為 ASCII/ANSI 金鑰值或 (VIRTKEY) 的虛擬按鍵組合。

- 如果**型別**屬性是**ASCII**，則**修飾詞**屬性只能`None`或`Alt`，或者可以有加速器使用**Ctrl**機碼 (由上述的金鑰`^`)。

- 如果**型別**屬性是**VIRTKEY**的任意組合**修飾詞**並**金鑰**值無效。

> [!NOTE]
> 如果您想要進入快速鍵對應表中的值，且值為 ASCII/ANSI，選取**型別**中的資料表和選取的項目**ASCII**從下拉式清單。 不過，如果您使用**下一個輸入的索引鍵**命令**編輯**指定的功能表**金鑰**，您必須變更**類型**屬性**VIRTKEY**要**ASCII** *之前*輸入**金鑰**程式碼。
...
1. 型別*金鑰*您想要做為快速鍵，或以滑鼠右鍵按一下並選擇 [**下一步] 輸入的按鍵**從捷徑功能表，來設定按鍵組合，或移至功能表**編輯** > **具類型的下一個索引鍵**。
