---
title: 陣列 （c + + 元件延伸模組） |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-windows
 ...
 *限定詞*[選用]  
 一或多個這些儲存類別規範：[可變](../cpp/mutable-data-members-cpp.md)， [volatile](../cpp/volatile-cpp.md)， [const](../cpp/const-cpp.md)， [extern](../cpp/using-extern-to-specify-linkage.md)，[靜態](../cpp/static-members-cpp.md).  
 ...
 *初始設定式清單*[選用]  
 初始化陣列的元素的大括號中值的逗號分隔清單。 例如，如果*陣序大小清單*已`(3)`，其中宣告的 3 個元素，一維陣列*初始化清單*可能`{1,2,3}`。  
 ...
 *陣序規範*[選用]  
 陣列維度的數目。 預設值為 1;最大值為 32。 每個維度本身是陣列的一個陣列。  
 ...
 *初始設定式清單*[選用]  
 初始化陣列的元素的大括號中值的逗號分隔清單。 或以逗號分隔清單的巢狀*初始設定式清單*初始化多維陣列中的元素的項目。  
