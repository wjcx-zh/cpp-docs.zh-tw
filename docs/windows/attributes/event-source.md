...
**event_source**用於結合[event_receiver](event-receiver.md)屬性並[__event](../../cpp/event.md)關鍵字。 使用`event_receiver`建立事件接收器。 使用 **__event**為這些方法指定為事件的事件來源內的方法上。
