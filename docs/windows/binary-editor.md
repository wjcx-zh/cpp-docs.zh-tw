...
- 若要在新的檔案上開啟**二進位編輯器**，請移**至 功能表** 檔案 > **新增** > 檔案，選取您要編輯的檔**類型，然後**選取 **開啟** 按鈕旁邊的下拉箭號，然後選擇 以 > **二進位編輯器** **開啟**。

- 若要在現有檔案上開啟**二進位編輯器**，請移至**功能表** 檔案 > **開啟** > 檔案，選取您要編輯的**檔案，然後**選取 **開啟** 按鈕旁邊的下拉箭號，然後選擇 以 > **二進位編輯器** **開啟**。
...
> [!NOTE]
> 您可以使用[影像編輯器](../windows/image-editor-for-icons.md)和**二進位編輯器**來處理 managed 專案中的資源檔。 您想要編輯的任何 Managed 資源皆必須為連結的資源。 Visual Studio 資源編輯器並不支援對內嵌資源的編輯功能。
...
1. 在 [**編譯時期**指示詞] 方塊中，輸入 `#include` 語句，以提供包含自訂資源的檔案名，例如：
...
建立自訂資源的另一種方式是匯入外部檔案作為自訂資源，請參閱 [如何：](../windows/how-to-import-and-export-resources.md)管理資源。
