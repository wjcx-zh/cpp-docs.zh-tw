...
COM 屬性插入程式碼，來支援許多方面的 COM 開發和。NET Framework 通用語言執行階段開發。  這些區域是從自訂介面執行與現有的介面支援到支援的內建屬性、 方法和事件。  此外，可以找到支援複合稿和 ActiveX 控制項的實作。  
...
|[vi\_progid](../windows/vi-progid.md)|定義控制項與版本無關的 ProgID。|  
