 ...
 *dbtype* （選擇性）  
 OLE DB[型別指示器](https://msdn.microsoft.com/library/ms711251.aspx)的資料行項目。  
  
 *有效位數*（選擇性）  
 要用於資料行項目有效位數。 如需詳細資訊，請參閱說明`bPrecision`項目[DBBINDING 結構](https://msdn.microsoft.com/library/ms716845.aspx)  
  
 *擴展*（選擇性）  
 要用於資料行項目小數位數。 如需詳細資訊，請參閱說明`bScale`項目[DBBINDING 結構](https://msdn.microsoft.com/library/ms716845.aspx)  
  
 *狀態*（選擇性）  
 成員變數，用來保存此資料行的狀態。 狀態會指出資料行的值是資料值或其他值，例如 NULL。 如需可能的值，請參閱[狀態](https://msdn.microsoft.com/library/ms722617.aspx)中*OLE DB 程式設計人員參考*。  
  
 *長度*（選擇性）  
 成員變數，用來保存資料行的大小，以位元組為單位。  
  
## <a name="remarks"></a>備註  
 **db_column**將指定的資料表資料行繫結至資料列中的變數。 它會分隔可以參與 OLE DB 中的成員資料`IAccessor`為基礎的繫結。 這個屬性會設定通常使用的 OLE DB 取用者巨集來定義資料行對應[BEGIN_COLUMN_MAP](../data/oledb/begin-column-map.md)， [END_COLUMN_MAP](../data/oledb/end-column-map.md)，並[COLUMN_ENTRY](../data/oledb/column-entry.md)。 這些操作 OLE DB [DBBINDING 結構](https://msdn.microsoft.com/library/ms716845.aspx)繫結指定的資料行。 您將標記與每個成員**db_column**屬性會佔用的資料行項目表單中的資料行對應中的一個項目。 因此，您呼叫這個屬性會放置資料行對應，也就是命令或資料表類別中。  
