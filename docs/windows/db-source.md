 ...
 *db_source*  
 用來連接到資料來源連接字串。 如需連接字串的格式，請參閱[連接字串和資料連結](https://msdn.microsoft.com/library/ms718376.aspx)在 Microsoft Data Access Components (MDAC) SDK。  
 ...
 **db_source**類別或方法中新增資料來源屬性。 它用於搭配`db_command`(這個方法會接受*db_source* *名稱*參數做為其*source_name*參數)。  
 ...
 如需在應用程式中使用這個屬性的範例，請參閱範例[AtlAgent](http://msdn.microsoft.com/52bef5da-c1a0-4223-b4e6-9e464b6db409)並[MultiRead](http://msdn.microsoft.com/5a2a915a-77dc-492f-94b2-1b809995dd5e)。  
