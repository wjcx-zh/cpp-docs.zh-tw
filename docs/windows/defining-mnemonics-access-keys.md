...
- 若要查看所有控制項的目前定位順序，請移至 [功能表**格式**] 索引標籤  >  **順序**，或按**Ctrl**  +  **D**。

   每個控制項左上角的數位會顯示其在目前定位順序中的位置。

- 若要變更所有控制項的定位順序，請移至 [功能表**格式**] 索引標籤  >  **順序**，並依您想要**tab**鍵遵循的順序選取每個控制項，以設定 tab 順序。

- 若要變更兩個或多個控制項的定位順序，請移至 [功能表] [**格式**] 索引標籤  >  **順序**。 按住**ctrl**鍵，並選取要開始進行變更的控制項，然後放開**ctrl**鍵，並依您想要**Tab**鍵在該時間點遵循的順序來選取控制項。

   例如，如果您想要變更控制項的順序 `7` `9` ，請按住**Ctrl 鍵**，然後選取 [先控制項] `6` 。

- 若要將特定的控制項設定為數字 `1` ，或在定位順序中的第一個上，請按兩下控制項。
...
1. 在 [靜態文字] 標題中，于 `&` 您想要作為存取金鑰的字母前面輸入連字號（）。

1. 請確定靜態文字控制項緊接在控制其在定位順序中的標籤之前。

> [!NOTE]
> 對話方塊內的所有存取金鑰都必須是唯一的。 若要檢查是否有重複的存取金鑰，請移至功能表**格式**  >  **檢查助憶鍵**。
...
1. 在 [[屬性] 視窗](/visualstudio/ide/reference/properties-window)中，請將定位順序 **第一** 之選項按鈕的 [群組] ** 屬性設為 **True**。
   ...
   > [!NOTE]
   > 群組中，應該只有第一個選項按鈕的 [群組] **** 屬性設為 **True**。 如果您有不屬於按鈕群組的其他控制項，請將*群組之外*第一個控制項的 [**群組**] 屬性設為 [ **True** ]。 您可以使用**Ctrl**D 來快速識別群組外部的第一個控制項， + **D**以查看定位順序。
...
1. 在 [[加入成員變數精靈]](../ide/add-member-variable-wizard.md)中，選取 [控制項變數] **** 核取方塊，然後選取 [值] **** 選項按鈕。
   ...
   您現在可以修改程式碼，指定應該顯示為已選取的按鈕。 例如，會 `m_radioBox1 = 0;` 選取群組中的第一個選項按鈕。
