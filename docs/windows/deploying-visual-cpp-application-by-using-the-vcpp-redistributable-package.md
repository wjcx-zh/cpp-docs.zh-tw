...
1. 建立自我解壓縮安裝檔：
   ...
   1. 在 [Packaged files] \(封裝檔案\)**** 頁面上，新增下列檔案，然後選擇 [下一步]****。
      ...
      - vcredist_x86.exe。 從 Visual Studio 2015 中,此檔案位於*\\%VCINSTALLDIR%redist\\1033*。 Visual Studio 2017 和 Visual Studio 2019 中,此檔案位於 *%VCToolsRedistDir%* 中。 您也可以[從微軟下載最新的受支援的紅紅檔](https://support.microsoft.com/help/2977003/the-latest-supported-visual-c-downloads)。
...
1. 在沒有 Visual C++ 程式庫的另一部電腦上測試自我解壓縮安裝檔：
   ...
   1. 執行 MFC 應用程式。
      ...
      > [!IMPORTANT]
      > 要\\確定安裝的運行時版本,安裝程式將檢查註冊表項\\\\HKLM\\軟體\\微軟 VisualStudio\\_版本_\\VC\\運行時_平臺_\\版本。 如果目前安裝的版本比安裝程式嘗試安裝的版本還要新，則安裝程式會傳回成功而不安裝較舊版本，並在 [控制台] 的已安裝程式頁面上保留另一個項目。
