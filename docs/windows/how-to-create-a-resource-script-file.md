...
>
> 這項資訊不適用於 Windows 桌面應用程式或通用 Windows 平台應用程式中的資源。 .NET 語言中的專案不使用資源指令碼檔。 如需將資源加入 managed 專案的詳細資訊，請參閱[資源檔](../windows/resource-files-visual-studio.md)，[應用程式資源和資源管理系統](/windows/uwp/app-resources/)，並[桌面應用程式中的資源](/dotnet/framework/resources/index)中 *.NET Framework 開發人員指南*。

## <a name="create-a-resource-script"></a>建立資源指令碼
...
1. 提供資源指令碼中的名稱**名稱**文字，然後選取**開啟**。

### <a name="to-open-a-resource-script"></a>若要開啟資源指令碼

您可以檢視資源指令碼中的資源，而不需要開啟一個專案。 指令碼檔案會在相對於在文件視窗中開啟**資源檢視**視窗 （如同在專案內開啟檔案時）。

> [!NOTE]
> 有些命令卻是提供檔案時才開啟的獨立 （外部專案中）。 開啟檔案的獨立表示開啟而不先載入專案。
>
...
1. 在 [**開啟的檔案**] 對話方塊中，瀏覽至資源指令碼檔，反白顯示檔案，然後選擇**開啟**。
...
1. 這兩個.rc 檔時開啟，請在** 視窗**功能表 （或滑鼠右鍵按一下其中一個.rc 檔） 選擇**新增水平索引標籤群組**或是**新增垂直索引標籤群組**。
...
> [!TIP]
> 是以滑鼠右鍵按一下.rc 檔中的快顯**方案總管**，選擇**開啟...** ，然後選取**原始程式碼 （文字） 編輯器**。

### <a name="to-add-mfc-support"></a>將 MFC 支援

一般來說，如果 Windows 使用 Microsoft Foundation Class (MFC) 應用程式建置[MFC 應用程式精靈](../mfc/reference/mfc-application-wizard.md)，精靈會產生一組基本的檔案 （包括資源指令碼 (.rc) 檔），其中包含核心功能MFC。 不過，編輯.rc 檔案，不以 MFC 為基礎的 Windows 應用程式時，無法使用下列的 MFC 功能：

特定 MFC 的一些功能包括 MFC 程式碼精靈、 功能表提示字串、 清單下拉式方塊控制項和 ActiveX 控制項裝載的內容。
...
>
> 使用滑鼠右鍵按一下**資源檢視**以啟動命令的捷徑功能表，或按兩下標題列來停駐或取消停駐視窗的視窗。 以滑鼠右鍵按一下標題列會提供其他命令，可讓您控制視窗的行為。 如需詳細資訊，請參閱 < [Windows Management](/visualstudio/ide/customizing-window-layouts-in-visual-studio)。

使用**加入資源**對話方塊，即可將資源新增至 c + + Windows 桌面應用程式專案，具有下列屬性：
...
使用**新的自訂資源**對話方塊來建立新的自訂資源，在 c + + 專案中，使用下列屬性：
...
- 在 **資源檢視**，選取您的.rc 檔，然後選取**編輯**功能表，然後選擇 **加入資源**。 在 **加入資源**對話方塊方塊中，選擇您想要新增至您的專案資源的類型。

   > [!TIP]
   > 您也可以以滑鼠右鍵按一下.rc 檔中的**資源檢視**，然後選擇**加入資源**從捷徑功能表。

- 在 **方案總管 中**，以滑鼠右鍵按一下專案資料夾，然後選取**新增**，然後選擇 **加入資源**快顯功能表上。 在 **加入資源**對話方塊方塊中，選擇您想要新增至您的專案資源的類型。

   > [!NOTE]
   > 如果您在專案中還沒有.rc 檔，此步驟會建立一個。 接著，您可以重複此步驟，將特定資源類型加入至新的 .rc 檔。

- 在 [類別檢視](/visualstudio/ide/viewing-the-structure-of-code)，以滑鼠右鍵按一下您的類別，然後選擇**新增**。 選取 **加入資源**從捷徑功能表，然後選擇您想要新增至專案的資源類型。

- 在 **專案**功能表上，選擇**加入資源**。

## <a name="create-a-resource-template"></a>建立資源範本
...
1. 在 **加入新項目**對話方塊中，於**範本：** 窗格中，選取**資源範本檔 (.rct)**。
...
1. 在 **加入資源**對話方塊方塊中，選取加號 (**+**) 旁邊要展開資源節點，然後檢視該資源可用的所有範本的資源。
