...
不搭配使用時[COM 屬性](../windows/com-attributes.md)，IDL 屬性可讓您定義介面。 當編譯的原始程式碼時，屬性用來定義所產生的.idl 檔案。 ATL 專案中的 COM 屬性搭配使用時，某些 IDL 屬性，例如`coclass`，導致插入至專案的程式碼。

請注意， [idl_quote](../windows/idl-quote.md)可讓您使用目前版本的 Visual c + + 中不支援的 MIDL 建構。 這和其他屬性，例如[importlib](../windows/importlib.md)並[includelib](../windows/includelib-cpp.md)幫助您使用您目前的 Visual c + + 專案中現有的.idl 檔案。
...
|[coclass](../windows/coclass.md)|位置的類別到.idl 檔為 coclass 的定義。|
...
[屬性的限制](https://msdn.microsoft.com/6e6c4329-f667-4869-b991-cbe9cb7a8f61)