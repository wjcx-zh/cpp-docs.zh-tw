...
ms.locfileid: "88846299"
---
# <a name="mfc-predefined-symbols"></a>MFC 預先定義的符號

MFC 專案一律包含數個支援 windows 的標頭檔。 這些是透過檔案 `#include` 中的語句新增的 `StdAfx.h` ：
...
標頭檔包含 MFC 通用值的符號識別碼值。 只有當您在 MFC 專案中工作時，才能使用這些符號。 `AFX_`前置詞後面接著標準符號名稱首碼。
...
這個資料表中的符號只有在加入資料庫支援時，才會出現在 MFC 專案中。 它們支援 SQL server 與資料庫程式設計的 MFC 連接。
...
這些符號也是 MFC 支援檔案的一部分，但是它們支援標準 Windows 應用程式功能和動作。 這些符號會與功能表一起使用。

當您使用 **功能表編輯器**中的功能表時，這些符號會出現在與通用功能表命令相關聯的 [ **屬性** ] 視窗中。 比方說，如果您的應用程式有一個具有**Exit** **命令的 [檔案] 功能表，** 該命令就會與 `ID_APP_EXIT` [屬性視窗](/visualstudio/ide/reference/properties-window)中的符號相關聯。
