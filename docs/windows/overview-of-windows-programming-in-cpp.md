...
有數種廣泛的類別，您可以使用建立的 Windows 應用程式的C++。 每個都有它自己的程式設計模型和設定 Windows 特定文件庫，但C++標準程式庫和第三方C++程式庫可用在任何一個。
...
A*原生桌面用戶端應用程式*是 C 或C++視窗型的應用程式，會使用原始的原生[Windows C Api 或元件物件模型 (COM) Api](/windows/desktop/apiindex/windows-api-list)來存取作業系統。 這些 Api 大部分是以 c 撰寫本身就是沒有建立原生桌面應用程式的多個方法：您可以直接使用 Win32 Api 使用程式設計的 C 樣式訊息迴圈處理作業系統事件。 或者，您可以使用程式設計*Microsoft Foundation Classes* (MFC) 少量物件導向C++包裝 Win32 的程式庫。 既不方法被視為 「 現代化 」 比較到通用 Windows 平台 (UWP)，但是這兩仍受到完整支援，而且有上百萬行的目前執行的世界中的程式碼。 在視窗中執行的 Win32 應用程式會要求開發人員明確地使用 Windows 程序函式內的 Windows 訊息。 名稱，即使 Win32 應用程式可以編譯為 32 位元 (x86) 或 64 位元 (x64) 二進位。 在 Visual Studio IDE 中，Win32 與條款 x86 的意義相同。
...
在一般情況下，.NET 程式設計中C#是較不複雜且較不容易發生錯誤，並具有更現代化物件導向 API，比 Win32 或 MFC。 在大部分情況下，其效能已綽綽有餘。 .NET 的功能豐富的圖形的 Windows Presentation Foundation (WPF)，以及您可以使用 Win32 和現代化的 Windows 執行階段 API。 一般而言，我們建議使用C++桌面應用程式，當您需要：
...
您也可結合的電源和效率C++.NET 程式設計。 您可以建立中的使用者介面C#，並使用C++若要啟用的應用程式使用原生 /CLIC++程式庫。 如需詳細資訊，請參閱 <<c0> [ 使用的.NET 程式設計C++/CLI](../dotnet/dotnet-programming-with-cpp-cli-visual-cpp.md)。</c0>

## <a name="com-components"></a>COM 元件

[元件物件模型 (COM)](/windows/desktop/com/the-component-object-model)是一種規格，可讓以與彼此進行通訊的不同語言撰寫的程式。 許多 Windows 元件會實作為 COM 物件，並遵循建立物件、 介面探索及物件解構的標準 COM 規則。  使用 COM 物件從C++桌面應用程式是相當簡單，但更進階撰寫您自己的 COM 物件。 [Active Template Library (ATL)](../atl/atl-com-desktop-components.md)提供巨集和簡化 COM 開發的協助程式函式。 如需詳細資訊，請參閱 < [ATL COM 桌面元件](../atl/atl-com-desktop-components.md)。
...
原始C++UWP 所組成 (1) 的支援C++的方言，/CXC++語法延伸模組，或 （2) 的 Windows 執行階段範本庫 (WRL)，這根據標準C++和 com。 同時C++/CX WRL 仍受支援。 對於新專案，我們建議您[ C++/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)，這完全以標準為基礎C++，並提供更快的效能。

## <a name="desktop-bridge"></a>傳統型橋接器

在 Windows 10 中，您可以封裝您現有的桌面應用程式或 COM 物件，為 UWP 應用程式，和新增 UWP 的功能，例如觸控，或從最新的 Windows API 集呼叫 Api。 您也可以新增至桌面的方案在 Visual Studio 中，並將它們在單一封裝，並使用 Windows Api 來彼此之間通訊的封裝中的 UWP 應用程式。

Visual Studio 2017 15.4 版和更新版本可讓您建立 Windows 應用程式封裝專案，來大幅簡化封裝現有傳統型應用程式的工作。 有一些限制的登錄呼叫或桌面應用程式可以使用的 Api。 不過，在許多情況下，您可以建立替代的程式碼路徑，以在執行中應用程式套件時達到類似的功能。 如需詳細資訊，請參閱[傳統型橋接器](/windows/uwp/porting/desktop-to-uwp-root)。
...
Windows*服務*是可以在幾乎不需要使用者互動的背景中執行的程式。 這些程式會呼叫*精靈*UNIX 系統上。 如需詳細資訊，請參閱 < [Services](/windows/desktop/services/services)。

## <a name="sdks-libraries-and-header-files"></a>Sdk、 程式庫和標頭檔

Visual Studio 包含 C 執行階段程式庫 (CRT)，C++標準程式庫和其他 Microsoft 特定程式庫。 大部分的 include 資料夾包含這些程式庫標頭檔位於 Visual Studio 安裝目錄下的 \VC\ 資料夾中。 在 Windows 和 CRT 標頭檔位於 Windows SDK 安裝資料夾中。

[Vcpkg 套件管理員](../build/vcpkg.md)可讓您方便地針對 Windows 中安裝數百個第三方開放原始碼程式庫。
