...
許多 Windows 程式設計案例也需要使用 Windows SDK，它包含了可讓您存取 Windows 作業系統元件的標頭檔。 根據預設，Visual Studio 會安裝 Windows SDK，讓您能夠開發通用 Windows 應用程式。 若要開發 Windows 10 的通用 Windows 應用程式，您需要 Windows 10 版本的 Windows SDK。 Windows 10 SDK 的相關資訊，請參閱[Windows 10 SDK](https://dev.windows.com/downloads/windows-10-sdk)。 (如需有關適用於舊版的 Windows 版本的 Windows Sdk 的詳細資訊，請參閱[Windows SDK 封存](https://developer.microsoft.com/windows/downloads/sdk-archive))。
