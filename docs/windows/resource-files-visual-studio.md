...
- 開發環境所產生的標頭檔。 此類型包含從 .rc 檔參考的 `Resource.h`。
...
| 檔案名稱 | 描述 |
|---|---|
| Resource.h | 包含符號定義的開發環境所產生的標頭檔。<br/><br/>將此檔案包含在原始檔控制中。 |
...
對於 Windows XP 或 Windows Vista 應用程式，資訊清單資源應指定最新版本的 Windows 通用控制項，以供應用程式使用。 上述範例使用支援[Syslink 控制項](/windows/win32/Controls/syslink-overview)的版本 `6.0.0.0`。
