...
ms.locfileid: "91353021"
---
# <a name="toolbar-editor-c"></a> (c + +) 的工具列編輯器

**工具列編輯器**可讓您建立工具列資源，並將點陣圖轉換成工具列資源。 **工具列編輯器**會使用圖形顯示來顯示工具列和按鈕，其會與他們在完成的應用程式中查看的方式相似。

[ **工具列編輯器** ] 視窗會顯示按鈕影像的兩個視圖，與 [ **影像編輯器** ] 視窗相同。 分割列可分隔兩個窗格，而且您可以從側邊拖曳分割列以變更窗格的相對大小。 [使用中] 窗格會顯示選取框線，而影像的兩個視圖上方則是 [主旨] 工具列。

![工具列編輯器](../mfc/media/vctoolbareditor.gif "vcToolbarEditor")<br/>
**工具列編輯器**

**工具列編輯器**與功能中的**影像編輯器**類似，而且兩者之間的功能表項目、圖形工具和點陣圖方格相同。 [ **影像** ] 功能表中有一個功能表命令，可以在 [ **工具列編輯器** ] 和 [ **影像編輯器**] 之間切換。 如需使用 [ **圖形** ] 工具列、[ **色彩** 選擇區] 或 [ **影像** ] 功能表的詳細資訊，請參閱 [影像編輯器](../windows/image-editor-for-icons.md)。

您可以藉由轉換點陣圖，在 c + + 專案中建立新的工具列。 點陣圖中的圖形會轉換成工具列的按鈕影像。 點陣圖通常會在單一點陣圖上包含數個按鈕影像，每個按鈕都有一個影像。 影像可以是任何大小，因為預設值為16圖元寬和影像的高度。 當您在**影像編輯器**中從 [**影像**] 功能表選擇 [**工具列編輯器**] 時，可以在 [**新的工具列資源**] 對話方塊中指定按鈕影像的大小。

[ **新增工具列資源** ] 對話方塊可讓您指定要加入至 c + + 專案中工具列資源之按鈕的寬度和高度。 預設值為16×15圖元。

用來建立工具列的點陣圖具有最大寬度2048，因此，如果您將 **按鈕寬度** 設定為 *512*，則只能有四個按鈕。 如果您將寬度設定為 *513*，則只能有三個按鈕。
...
依預設，新的或空白的按鈕會顯示在工具列的右端。 您可以先移動這個按鈕，再進行編輯。 當您建立新按鈕時，[編輯] 按鈕的右邊會出現另一個空白按鈕。 當您儲存工具列時，不會儲存空白按鈕。
...
**寬度** 和 **高度** 適用于所有按鈕。 用來建立工具列的點陣圖寬度上限為2048，因此，如果您將按鈕寬度設定為 *512*，您只能有四個按鈕，如果您將寬度設定為 [ *513*]，就只能有三個按鈕。
...
1. 在 [影像編輯器](../windows/image-editor-for-icons.md)中開啟現有的點陣圖資源。 如果該點陣圖尚未存在 *.rc* 檔中，請以滑鼠右鍵按一下 *.rc* 檔，然後選擇 [匯 **入**]，然後流覽至您要新增至 *.rc* 檔的點陣圖，然後選取 [ **開啟**]。
...
1. 展開 **工具列** 資料夾，然後選取要編輯的工具列，然後執行下列其中一項動作：
...
- 若要從工具列複製按鈕，請按住 **Ctrl** 鍵，然後在 **工具列視圖** 窗格中，將按鈕拖曳至工具列上的新位置，或拖曳到另一個工具列上的位置。

- 若要刪除工具列按鈕，請選取工具列按鈕，然後將它拖曳到工具列上。

- 若要在工具列上的按鈕之間插入或移除空格，請在工具列上將它們拖曳到另一個按鈕的上方或向外拖曳。
...
|移除工具列上按鈕之間的空格|將某個空間的按鈕拖曳到空間的另一端上的按鈕，直到它重迭 [下一步] 按鈕的一半為止。|
