...
轉換點陣圖，您可以在 c + + 專案中建立新的工具列。 從點陣圖圖形將轉換成工具列按鈕影像。 通常，點陣圖包含數個按鈕上的映像單一的點陣圖，使用一個映像的每個按鈕。 映像可以是影像的任何大小的預設值為 16 個像素寬的高度。 您可以指定在按鈕影像的大小**新增工具列資源**當您選擇的對話方塊**工具列編輯器**從**映像**功能表時**影像編輯器**。
